...
void dbg_write(unsigned long dcc_data) {
  ...
  do { asm volatile("mrc p14, 0, %0, c0, c0" : "=r"(dcc_status)); } while (dcc_status & 0x2);
...
void dbg_write_u8(const unsigned char* val, long len) {
  ...
  while (len > 0) {
    dcc_data = val[0] | ((len > 1) ? val[1] << 8 : 0x00) | ((len > 2) ? val[2] << 16 : 0x00) |
               ((len > 3) ? val[3] << 24 : 0x00);
...
void dbg_write_str(const char* msg) {
  ...
  while (len > 0) {
    dcc_data = msg[0] | ((len > 1) ? msg[1] << 8 : 0x00) | ((len > 2) ? msg[2] << 16 : 0x00) |
               ((len > 3) ? msg[3] << 24 : 0x00);
    dbg_write(dcc_data);
