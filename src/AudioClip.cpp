...
AudioClip::~AudioClip() {
  ...
  // Sirhc actually got this in a V3.0.5 RC! No idea how. Also Qui got around
  // V3.1.3. I suspect that recorder is somehow still set when this Clip gets
  // "deleted" by being put in a ConsequenceClipExistence. Somehow neither
  // abortRecording() nor finishLinearRecording() would be being called when
  // some other AudioClip becomes the Output's activeClip. These get called in
  // slightly roundabout ways, so this seems the most likely. I've added some
  // further error code diversification.
}
...
bool AudioClip::wantsToBeginLinearRecording(Song* song) {
  return (Clip::wantsToBeginLinearRecording(song) && !sampleHolder.audioFile &&
          ((AudioOutput*)output)->inputChannel);
}
...
int AudioClip::beginLinearRecording(ModelStackWithTimelineCounter* modelStack,
  ...
  int numChannels = (inputChannel >= AUDIO_INPUT_CHANNEL_FIRST_INTERNAL_OPTION ||
                     inputChannel == AUDIO_INPUT_CHANNEL_STEREO) ?
                        2 :
                        1;
  ...
  recorder = AudioEngine::getNewRecorder(numChannels,
                                         AUDIO_RECORDING_FOLDER_CLIPS,
                                         inputChannel,
                                         true,
                                         shouldRecordMarginsNow,
                                         buttonPressLatency);
...
void AudioClip::finishLinearRecording(ModelStackWithTimelineCounter* modelStack,
                                      Clip*                          nextPendingOverdub,
                                      int buttonLatencyForTempolessRecord) {
  if (!recorder) return; // Shouldn't ever happen?

  // Got to check reachedMaxFileSize here, cos that'll go true a bit before
  // cardRoutine() sets status to ERROR
  if (recorder->status == RECORDER_STATUS_ABORTED || recorder->reachedMaxFileSize) {
    abortRecording();
    return;
  }

  // Have to do this before setting currentlyRecordingLinearly to false, for
  // vertical scroll reasons
  ...
  recorder->endSyncedRecording(buttonLatencyForTempolessRecord); // Must call before setSample(),
                                                                 // cos it sets up important stuff
                                                                 // like the sample length

  if (getRootUI())
    getRootUI()->clipNeedsReRendering(
        this); // SampleRecorder will also call sampleNeedsReRendering() when
               // "capturing" is finished, but in plenty of cases, that will
               // have happened in the above call to endSyncedRecording(), and
               // our sample hasn't been set yet, so that won't have been
               // effective. So, we have to call this here too, to cover our
               // bases.

  sampleHolder.filePath.set(&recorder->sample->filePath);
  sampleHolder.setAudioFile(recorder->sample,
                            sampleControls.reversed,
                            true,
                            CLUSTER_DONT_LOAD); // Adds a reason to the first Cluster(s). Must call
                                                // this after endSyncedRecording(), which puts some
                                                // final values in the Sample
...
Clip* AudioClip::cloneAsNewOverdub(ModelStackWithTimelineCounter* modelStackOldClip,
...
void AudioClip::processCurrentPos(ModelStackWithTimelineCounter* modelStack,
  ...
  // If we have a recorder that's gotten into error/aborted state, but we
  // haven't registered that here yet, do that now. This isn't really the ideal
  // place for this...
  if (recorder && recorder->status == RECORDER_STATUS_ABORTED) { abortRecording(); }

  // If at pos 0, that's the only place where anything really important happens:
  // play the Sample
  if (!lastProcessedPos) {
    ...
    if (sampleHolder.audioFile && !((Sample*)sampleHolder.audioFile)->unplayable) {
      ...
      guide.sequenceSyncStartedAtTick =
          playbackHandler.lastSwungTickActioned; // Must do this even if we're
                                                 // going to return due to time
                                                 // stretching being active

      // Obviously if no voiceSample yet, need to reset envelope for first
      // usage. Otherwise, If the envelope's now releasing (e.g. from playback
      // just being stopped and then restarted, e.g. by pressing <> + play), and
      // if not doing a late start (which would cause more sound to start
      // happening after the envelope finishes releasing), then we'd absolutely
      // better reset that envelope because there's no other way any sound's
      // going to come out!
      bool shouldResetEnvelope =
          !voiceSample || (((AudioOutput*)output)->envelope.state >= ENVELOPE_STAGE_RELEASE &&
      ...
      if (voiceSample) {
        ...
        // But, if it's reading from (or writing to) a cache, we have to
        // manually cut it out right now and tell it to restart, so the new
        // play-through will remain perfectly synced up to this start-tick we're
        // at now.
        if (voiceSample->cache

            // Or, if no time-stretcher, which also means we're not "fudging" -
            // probably because there was no pre-margin - then yup we'll wanna
            // force restart now too - because AudioClip VoiceSamples don't obey
            // their loop points, so it won't have happened otherwise.
            || !voiceSample->timeStretcher

            // Or, if time stretching is on but the "newer" play-head is no
            // longer active - because it's shot past the end of the waveform,
            // and this waveform didn't have an extra "margin" at the end, then
            // we want to just cut everything and start from scratch again.
        ...
        // Or if none of those several conditions were met...
          ...
          // If no pre-margin, then still do go and do the unassignment and
          // start afresh - cos we'll wanna hear that sharp start-point
          // perfectly rather than just fading into it after a play-head reaches
          // its end
          uint32_t waveformStartByte = ((Sample*)sampleHolder.audioFile)->audioDataStartPosBytes;
          if (sampleControls.reversed)
            waveformStartByte += ((Sample*)sampleHolder.audioFile)->audioDataLengthBytes -
                                 sampleHolder.audioFile->numChannels *
                                     ((Sample*)sampleHolder.audioFile)
                                         ->byteDepth; // The actual first sample of the waveform
                                                      // in our given direction, regardless of
                                                      // our elected start-point
          ...
          // If we were "fudging" a time-stretch just to get a free crossfade,
          // then we can now stop doing all of that. (It should automatically
          // stop eventually anyway, but let's be clean and efficient and just
          // kill it now.)
          if (voiceSample->fudging) { voiceSample->endTimeStretching(); }
          // Otherwise, if we're just regular time-stretching (not for mere
          // "fudging" reasons), don't do anything and just get out. The regular
          // time-stretching algorithm takes care of causing playback to jump
      ...
      voiceSample->setupClusersForInitialPlay(&guide,
                                              ((Sample*)sampleHolder.audioFile),
                                              0,
                                              false,
                                              1);
...
void AudioClip::resumePlayback(ModelStackWithTimelineCounter* modelStack, bool mayMakeSound) {
  ...
  // If reading or writing cache, that's not gonna be valid now that we've moved
  // our play position, so gotta stop that.
  if (voiceSample && voiceSample->cache) {
    int  priorityRating = 1;
    bool success        = voiceSample->stopUsingCache(&guide,
                                               ((Sample*)sampleHolder.audioFile),
                                               priorityRating,
                                               getLoopingType(modelStack) == LOOP_LOW_LEVEL);
    if (!success) { unassignVoiceSample(); }
  }

  // For synced time-stretching, that syncing is done by noting what the
  // internal tick count "was" at the start of the Clip, so we work that out
  // here. However, due to the efficient ticking system and the fact that the
  // clip length may have just been reduced, it's possible that this would then
  // give us a result which would indicate that our current play position is
  // actually beyond the length of the Clip. So, we do some wrapping here to
  // ensure that guide.sequenceSyncStartedAtTickTrivialValue is not longer ago
  // (from the "actual" current internal ticket time) than the length of the
  // Clip. Despite the fact that we generally here deal with the positions /
  // counts of the last swung tick, which is why this problem exists in the
  // first place. To be clear, where wrapping occurs,
  // guide.sequenceSyncStartedAtTickTrivialValue will be after the last actioned
  // swung tick.
  int32_t sequenceSyncStartedAtTickTrivialValue =
      playbackHandler.lastSwungTickActioned - lastProcessedPos;
  int32_t currentInternalTickCount = playbackHandler.getCurrentInternalTickCount();
  int32_t sequenceSyncStartedNumTicksAgo =
      currentInternalTickCount - sequenceSyncStartedAtTickTrivialValue;
  if (sequenceSyncStartedNumTicksAgo < 0) { // Shouldn't happen
    if (ALPHA_OR_BETA_VERSION)
      numericDriver.freezeWithError(
          "nofg");                      // Ron got, Nov 2021. Wait no, he didn't have playback on!
    sequenceSyncStartedNumTicksAgo = 0; // The show must go on
  }
  sequenceSyncStartedNumTicksAgo =
      (uint32_t)sequenceSyncStartedNumTicksAgo % (uint32_t)loopLength; // Wrapping
  guide.sequenceSyncStartedAtTick =
      currentInternalTickCount - sequenceSyncStartedNumTicksAgo; // Finally, we've got our value

  setupPlaybackBounds();
  sampleZoneChanged(modelStack); // Will only do any thing if there is in fact a
                                 // voiceSample - which is what we want

  if (!mayMakeSound) return;

  // If already time stretching, no need to do anything - that'll take care of
  // the new play-position. Also can only do this if envelope not releasing,
  // which (possibly not anymore since I fixed other bug) it can still be if
  // this is our second quick successive <>+play starting playback partway
  // through Clip
  if (voiceSample && voiceSample->timeStretcher &&
  ...
  if (voiceSample) {
    ...
    // TODO: probably not super necessary now we've got time-stretching taking
    // care of sorta doing a crossfade, above. We'd only actually need to do any
    // such fade manually if we weren't time-stretching before, and we're also
    // not going to be after (though it'd be hard to predict whether we're going
    // to be after)
...
void AudioClip::sampleZoneChanged(ModelStackWithTimelineCounter const* modelStack) {
  if (voiceSample) {
    ...
    voiceSample->sampleZoneChanged(&guide,
                                   ((Sample*)sampleHolder.audioFile),
                                   MARKER_END,
                                   getLoopingType(modelStack),
                                   priorityRating,
                                   true);
  }
}
...
int64_t AudioClip::getNumSamplesTilLoop(ModelStackWithTimelineCounter* modelStack) {
  ...
  uint32_t loopTime =
      playbackHandler.getInternalTickTime(playbackHandler.lastSwungTickActioned + ticksTilLoop);
...
void AudioClip::render(ModelStackWithTimelineCounter* modelStack,
  ...
  // First, if we're still attempting to do a "late start", see if we can do
  // that (perhaps not if relevant audio data hasn't loaded yet)
  if (doingLateStart && ((AudioOutput*)output)->envelope.state < ENVELOPE_STAGE_FAST_RELEASE) {
    ...
    if (result) {
      if (result == LATE_START_ATTEMPT_FAILURE) { unassignVoiceSample(); }
      return;
    }
  ...
  uint32_t sampleLengthInSamples =
      sampleHolder.getDurationInSamples(true); // In the sample rate of the file!
  uint32_t clipLengthInSamples = (playbackHandler.getTimePerInternalTickBig() * loopLength) >>
                                 32; // We haven't rounded... should we?
  ...
  if (sampleControls.pitchAndSpeedAreIndependent) {
    ...
    // And if pitch was manually adjusted too (or file's sample rate wasn't
    // 44100kHz, that's fine - counteract that by adjusting the time-stretch
    // amount more
    ...
    // Or if no manual pitch adjustment...
      ...
      if (timeStretchRatio >= 16661327 && timeStretchRatio < 16893911) {
        ...
        // And if playback has stopped or the envelope is doing a fast release
        // before we begin another "late start"...
        if (!playbackHandler.isEitherClockActive() || doingLateStart) {
        justDontTimeStretch:
          // We can just not time-stretch... for now, until we end up more
          // out-of-sync later
        ...
        // Or...
          ...
          // If we're less than 7.8mS out of sync, then that's another (even
          // more common) reason not to time-stretch
  ...
  // Or if we're squishing pitch...
    ...
    // If no prior pitch adjustment, we play back 100% natively, with no pitch
    // shifting / time stretching. Just with pitch / speed changed like
    // speeding/slowing a record
    if (!sampleHolder.transpose && !sampleHolder.cents) {
      uint64_t phaseIncrementNew =
          ((uint64_t)(uint32_t)phaseIncrement * requiredSpeedAdjustment) >> 24;
    ...
    // Or if yes prior pitch adjustment, then we'll be pitch shifting / time
    // stretching
    else {
  ...
  bool clipWillLoopAtEnd =
      playbackHandler.playbackState && currentPlaybackMode->willClipLoopAtSomePoint(modelStack);
  // I don't love that this is being called for every AudioClip at every render.
  // This is to determine the "looping" parameter for the call to
  // VoiceSample::render() below. It'd be better to maybe have that just query
  // the currentPlaybackMode when it needs to actually use this info, cos it
  // only occasionally needs it
  ...
  if (clipWillLoopAtEnd) {
    ...
    // If no time-stretcher, and not reading cache, we might want to "fudge" to
    // eliminate the click at the loop point (if it's gonna loop)
    if (timeStretchRatio == 16777216 && !voiceSample->timeStretcher &&
        (!voiceSample->cache || voiceSample->writingToCache)) {

      // First, see if there is actually any pre-margin at all for us to
      // crossfade to
      ...
      if (guide.playDirection != 1)
        startByte +=
            sample->audioDataLengthBytes - bytesPerSample; // The actual first sample of the
                                                           // waveform in our given direction,
      ...
      if (numBytesOfPreMarginAvailable > 0) {
        ...
        if (numSamplesTilLoop <= ANTI_CLICK_CROSSFADE_LENGTH) {
          ...
          if (numSamplesOfPreMarginAvailable > 2) {
            ...
            int crossfadeLength =
                getMin(numSamplesOfPreMarginAvailable, ANTI_CLICK_CROSSFADE_LENGTH);

            // If we're right at the end and it's time to crossfade...
            if (numSamplesTilLoop <= crossfadeLength) {

              // Fudge some time-stretching
              bool success = voiceSample->fudgeTimeStretchingToAvoidClick(sample,
                                                                          &guide,
                                                                          phaseIncrement,
                                                                          numSamplesTilLoop,
                                                                          guide.playDirection,
  ...
  // Or if Clip won't loop at any point...
    ...
    // We want to do a fast release *before* the end, to finish right as the end
    // is reached. So that any waveform after the end isn't heard.
    // TODO: in an ideal world, would we only do this if there actually is some
    // waveform "margin" after the end that we want to avoid hearing, and
    // otherwise just do the release right at the end (does that already happen,
    // I forgot?) It's perhaps a little bit surprising, but this even works and
    // sounds perfect (you never hear any of the margin) when time-stretching is
    // happening! Down to about half speed. Below that, you hear some of the
    // margin.
    if (((AudioOutput*)output)->envelope.state < ENVELOPE_STAGE_FAST_RELEASE) {
      ...
      if (cutPos < 2147483647) {
        ...
        uint32_t loopTime    = playbackHandler.getInternalTickTime(
            playbackHandler.lastSwungTickActioned + ticksTilCut);
        int32_t timeTilLoop = loopTime - AudioEngine::audioSampleTimer;

        if (timeTilLoop < 1024) {
          ((AudioOutput*)output)
              ->envelope.unconditionalRelease(ENVELOPE_STAGE_FAST_RELEASE,
                                              8192); // Let's make it extra fast?
        }
      }
    }
  }
  ...
  if (maySetupCache) {
    ...
    // We want it to think this, otherwise problems if we've put the end point
    // after the actual waveform end.
    bool everythingOk = voiceSample->possiblySetUpCache(&sampleControls,
                                                        &guide,
                                                        phaseIncrement,
                                                        timeStretchRatio,
                                                        1,
...

  {
    ...
    stillActive = voiceSample->render(&guide,
                                      outputBuffer,
                                      numSamples,
                                      sample,
                                      sample->numChannels,
                                      ...
                                      sampleControls.interpolationMode,
                                      1);
  }
...
int AudioClip::getLoopingType(ModelStackWithTimelineCounter const* modelStack) {
  ...
  // We won't loop at the low level. We may want to loop at time-stretcher
  // level, in the following case (not if the end-point is set to beyond the
  // waveform's length).

  bool shouldLoop = (sampleControls.reversed ||
  ...
  // Normally we'll loop at the lowest level - but not if user has inserted
  // silence at the end (put the end-pos beyond the end of the Sample)
  return (sampleControls.reversed ||
          sampleHolder.endPos <= ((Sample*)sampleHolder.audioFile)->lengthInSamples);
  // Note that the actual "loop points" don't get obeyed for AudioClips - if any
  // looping happens at the low level, it'll only be at the very end of the
  // waveform if we happen to reach it. But, looping may still happen as
  // normally expected at the TimeStretcher level...
}
...
void AudioClip::expectNoFurtherTicks(Song* song, bool actuallySoundChange) {
  ...
  if (voiceSample) {
    if (actuallySoundChange) {
      ...
      if (doingLateStart) {
        // If waiting to do a late start, and we're not waiting for a past bit
        // to fade out, well there's no sound right now, so just cut out.
        if (((AudioOutput*)output)->envelope.state < ENVELOPE_STAGE_FAST_RELEASE) {
          unassignVoiceSample();
        }

        // Or if we were planning to do a late start as soon as the current
        // sound fades out, then just abandon the late start, but keep doing the
        // fade
        else {
          doingLateStart = false;
        }
      } else {
        // Or normal case - do a fade when we weren't going to before. And no
        // late start is or was happening.
        ((AudioOutput*)output)->envelope.unconditionalRelease(ENVELOPE_STAGE_FAST_RELEASE);
      }
    }
  }
...
void AudioClip::posReachedEnd(ModelStackWithTimelineCounter* modelStack) {
  ...
  if (playbackHandler.recording == RECORDING_ARRANGEMENT && isArrangementOnlyClip()) {
    ...
    modelStack->song->arrangementOnlyClips.insertClipAtIndex(newClip,
                                                             0); // Can't fail - checked above
    ...
    newClip->paramManager.getUnpatchedParamSet()->copyOverridingFrom(
...
void AudioClip::detachAudioClipFromOutput(Song* song,
                                          bool  shouldRetainLinksToOutput,
                                          bool  shouldTakeParamManagerWith) {
  unassignVoiceSample();

  if (isActiveOnOutput()) { output->detachActiveClip(song); }

  // Normal case, where we're leaving ParamManager behind with old Output
  if (!shouldTakeParamManagerWith) {
  doNormal:
    song->backUpParamManager((ModControllableAudio*)output->toModControllable(),
                             this,
                             &paramManager);
  }
  ...
  // Or, special case where we're keeping the ParamManager
    ...
    // If our ParamManager was the only one that the old Output had, we have to
    // clone it
    if (!song->getBackedUpParamManagerPreferablyWithClip(
      ...
      song->backUpParamManager((ModControllableAudio*)output->toModControllable(),
                               NULL,
                               &newParamManager);
      // Obscure bug fixed, Oct 2022. Previously, we filed it away under not
      // just the ModControllable, but also this Clip. Perhaps that's still
      // theoretically what we'd like to do? But, it caused an E170 because when
      // we do a song swap and call Song::deleteSoundsWhichWontSound(), that
      // calls deleteAllBackedUpParamManagersWithClips(), which would then
      // delete this ParamManager here, because we gave it a Clip. But it's
      // actually still needed by the associated ModControllable / AudioOutput.
    }
  }
    ...
    *xZoomSamples = getSamplesFromTicks(xZoom);
    ...
    if (sampleControls.reversed) {
      *xScrollSamples = sampleHolder.getEndPos(true) - xScrollSamplesWithinZone -
                        (*xZoomSamples << displayWidthMagnitude);
    } else {
      int64_t sampleStartPos =
          recorder ? recorder->sample->fileLoopStartSamples : sampleHolder.startPos;
...
bool AudioClip::renderAsSingleRow(ModelStackWithTimelineCounter* modelStack,
  ...
  Sample* sample;
  if (recorder)
    sample = ((recorder->status == RECORDER_STATUS_ABORTED || recorder->reachedMaxFileSize) ?
                  NULL :
                  recorder->sample);
  else
  ...
  if (sample) {
    ...
    if (!success) {
      // If card being accessed and waveform would have to be re-examined, come
      // back later
      return false;
    }
  }
...
void AudioClip::writeDataToFile(Song* song) {
  ...
  storageManager.writeAttribute("filePath",
                                sampleHolder.audioFile ? sampleHolder.audioFile->filePath.get() :
  ...
  storageManager.writeAttribute("pitchSpeedIndependent",
                                sampleControls.pitchAndSpeedAreIndependent);
...
int AudioClip::claimOutput(ModelStackWithTimelineCounter* modelStack) {
  ...
  if (!output) { return ERROR_FILE_CORRUPTED; }
...
void AudioClip::quantizeLengthForArrangementRecording(ModelStackWithTimelineCounter* modelStack,
                                                      int32_t  lengthSoFarInternalTicks,
                                                      uint32_t timeRemainder,
                                                      int32_t  suggestedLength,
                                                      int32_t  alternativeLongerLength) {

  double numTicksDone = (double)lengthSoFarInternalTicks +
                        (double)timeRemainder / playbackHandler.getTimePerInternalTickFloat();
...
bool AudioClip::currentlyScrollableAndZoomable() {
  bool shouldLock = (getCurrentlyRecordingLinearly() && (!playbackHandler.isEitherClockActive() ||
...
void AudioClip::clear(Action* action, ModelStackWithTimelineCounter* modelStack) {
  ...
  if (recorder) {
    if (!playbackHandler.isEitherClockActive()) { abortRecording(); }
  }
