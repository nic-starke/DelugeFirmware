...
bool LoadInstrumentPresetUI::opened() {
  ...
  if (instrumentClipToLoadFor) {
    instrumentClipToLoadFor->backupPresetSlot(); // Store this now cos we won't be storing it
                                                 // between each navigation we do
  }
  ...
  if (error) {
    renderingNeededRegardlessOfUI(); // Because unlike many UIs we've already
                                     // gone and drawn the QWERTY interface on
                                     // the pads, in call to
                                     // setupForInstrumentType().
    goto gotError;
  }
...
int LoadInstrumentPresetUI::setupForInstrumentType() {
  ...
  // I don't have this calling arrivedInNewFolder(), because as you can see
  // below, we want to either just display the existing preset, or call
  // confirmPresetOrNextUnlaunchedOne() to skip any which aren't "available".

  // If same Instrument type as we already had...
  if (instrumentToReplace->type == instrumentTypeToLoad) {

    // Then we can start by just looking at the existing Instrument, cos they're
    // the same type...
  ...
  // Or if the Instruments are different types...
    ...
    // If we've got a Clip, we can see if it used to use another Instrument of
    // this new type...
    if (instrumentClipToLoadFor) {
    ...
    // Otherwise we just start with nothing. currentSlot etc remain set to
    // "zero" from before
    else {
    useDefaultFolder:
  ...
  // The redrawing of the sidebar only actually has to happen if we just changed
  // to a different type *or* if we came in from (musical) keyboard view, I
  // think
#if DELUGE_MODEL != DELUGE_MODEL_40_PAD
...
void LoadInstrumentPresetUI::enterKeyPress() {
  ...
  if (currentFileItem->isFolder) {
    ...
    if (currentFileItem->instrument) { // When would this not have something? Well ok,
                                       // maybe now that we have folders.
      convertToPrefixFormatIfPossible();
    }

    if (instrumentTypeToLoad == INSTRUMENT_TYPE_KIT && showingAuditionPads()) {
      // New NoteRows have probably been created, whose colours haven't been
      // grabbed yet.
      instrumentClipView.recalculateColours();
    }
...
int LoadInstrumentPresetUI::timerCallback() {
  if (currentUIMode == UI_MODE_HOLDING_BUTTON_POTENTIAL_LONG_PRESS) {
    ...
    if (sdRoutineLock)
      return ACTION_RESULT_REMIND_ME_OUTSIDE_CARD_ROUTINE; // The below needs to
                                                           // access the card.
    ...
    // We want to open the context menu to choose to reload the original file
    // for the currently selected preset in some way. So first up, make sure
    // there is a file, and that we've got its pointer
    String filePath;
...
void LoadInstrumentPresetUI::changeInstrumentType(int newInstrumentType) {
  ...
  // If MIDI or CV, we have a different method for this, and the UI will be
  // exited
  if (newInstrumentType == INSTRUMENT_TYPE_MIDI_OUT || newInstrumentType == INSTRUMENT_TYPE_CV) {
    ...
    if (newInstrument) {
      ...
      // If going back to a view where the new selection won't immediately be
      // displayed, gotta give some confirmation
...
void LoadInstrumentPresetUI::revertToInitialPreset() {
  ...
  // Can only do this if we've changed Instrument in one of the two ways, but
  // not both.
  // TODO: that's very limiting, and I can't remember why I mandated this, or
  // what would be so hard about allowing this. Very often, the user might enter
  // this interface for a Clip sharing its Output/Instrument with other Clips,
  // so when user starts navigating through presets, it'll first do a "change
  // just for Clip", but then on the new preset, this will now be the only Clip,
  // so next time it'll do a "replace whole Instrument".
  ...
  // If we're looking to replace the whole Instrument, but we're not allowed,
  // that's obviously a no-go
  ...
  initialInstrument = currentSong->getInstrumentFromPresetSlot(initialInstrumentType,
                                                               initialChannel,
                                                               initialChannelSuffix,
                                                               initialName.get(),
                                                               initialDirPath.get(),
                                                               false,
                                                               true);
  ...
  if (initialInstrument) {
    ...
    if (availabilityRequirement == AVAILABILITY_INSTRUMENT_UNUSED) {
      ...
      if (currentSong->doesOutputHaveActiveClipInSession(initialInstrument)) { return; }
    }
  }
  ...
  // Or if we did not find it as a non-hibernating one...
    ...
    if (initialInstrumentType == INSTRUMENT_TYPE_MIDI_OUT ||
        initialInstrumentType == INSTRUMENT_TYPE_CV) {

      // One MIDIInstrument may be hibernating...
      if (initialInstrumentType == INSTRUMENT_TYPE_MIDI_OUT) {
        initialInstrument =
            currentSong->grabHibernatingMIDIInstrument(initialChannel, initialChannelSuffix);
        if (initialInstrument) goto gotAnInstrument;
      }

      // Otherwise, create a new one
      initialInstrument = storageManager.createNewNonAudioInstrument(initialInstrumentType,
                                                                     initialChannel,
                                                                     initialChannelSuffix);
    ...
    // Synth / kit...
      ...
      // Or if could not find hibernating synth or kit...
        ...
        // Set this stuff so that getCurrentFilePath() will return what we want.
        // This is just ok because we're exiting anyway
        ...
        error = storageManager.loadInstrumentFromFile(currentSong,
                                                      instrumentClipToLoadFor,
...
gotAnInstrument:
  ...
  if (replacedWholeInstrument) {
    ...
    // We know the Instrument hasn't been added to the Song, and this call will
    // do it
  ...
  // Otherwise, just changeInstrument() for this one Clip.
    ...
    int error =
        instrumentClipToLoadFor->changeInstrument(modelStack,
                                                  initialInstrument,
                                                  NULL,
                                                  INSTRUMENT_REMOVAL_DELETE_OR_HIBERNATE_IF_UNUSED);
    // TODO: deal with errors!

    if (needToAddInstrumentToSong) { currentSong->addOutput(initialInstrument); }
...
int LoadInstrumentPresetUI::performLoad(bool doClone) {
  ...
  if (currentFileItem->instrument == instrumentToReplace && !doClone)
    return NO_ERROR; // Happens if navigate over a folder's name (Instrument
                     // stays the same), then back onto that neighbouring
                     // Instrument - you'd incorrectly get a "USED" error
                     // without this line.
  ...
  if (!doClone && newInstrument) {
    ...
    newInstrumentWasHibernating =
        isInstrumentInList(newInstrument, currentSong->firstHibernatingInstrument);
    ...
    if (availabilityRequirement == AVAILABILITY_INSTRUMENT_UNUSED) {
      ...
      if (!newInstrumentWasHibernating &&
          currentSong->doesOutputHaveActiveClipInSession(newInstrument)) {
        goto giveUsedError;
      }
    }
  ...
  // Or, if we need to load from file - perhaps forcibly because the user
  // manually chose to clone...
  else {
    ...
    int error = storageManager.loadInstrumentFromFile(currentSong,
                                                      instrumentClipToLoadFor,
                                                      instrumentTypeToLoad,
                                                      false,
                                                      &newInstrument,
                                                      &currentFileItem->filePointer,
                                                      &enteredText,
                                                      &currentDir);
  ...
  // Otherwise, just changeInstrument() for this one Clip
    ...
    if (needToAddInstrumentToSong) { currentSong->addOutput(newInstrument); }

    changedInstrumentForClip = true;
  }

  // Check if old Instrument has been deleted, in which case need to update the
  // appropriate FileItem.
  ...
  if (instrumentClipToLoadFor) {
    view.instrumentChanged(modelStack, newInstrument); // modelStack's TimelineCounter is set to
                                                       // instrumentClipToLoadFor, FYI

    if (showingAuditionPads()) { renderingNeededRegardlessOfUI(0, 0xFFFFFFFF); }
  } else {
...
int LoadInstrumentPresetUI::verticalEncoderAction(int offset, bool inCardRoutine) {
  if (showingAuditionPads()) {
    ...
    if (getRootUI() == &keyboardScreen) { uiNeedsRendering(this, 0, 0xFFFFFFFF); }
