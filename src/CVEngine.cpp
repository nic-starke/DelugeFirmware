...
*/
#if HAVE_OLED
  enqueueCVMessage(SPI_CHANNEL_CV,
                   0b00000101000000100000000000000000); // LIN = 1
#else
  R_RSPI_SendBasic32(SPI_CHANNEL_CV,
                     0b00000101000000100000000000000000); // LIN = 1
#endif
  delayMS(10);
...
#if HAVE_OLED
  enqueueCVMessage(SPI_CHANNEL_CV,
                   0b00000101000000000000000000000000); // LIN = 0
#else
  R_RSPI_SendBasic32(SPI_CHANNEL_CV,
                     0b00000101000000000000000000000000); // LIN = 0
#endif
...
void CVEngine::updateGateOutputs() {
  if (gateOutputPending || clockOutputPending || asapGateOutputPending) {
    for (int g = 0; g < NUM_GATE_CHANNELS; g++) { physicallySwitchGate(g); }
...
// In the future, it'd be great if manually-auditioned notes could supply
// doInstantlyIfPossible as true. Currently there's no infrastructure for an
// Instrument to know whether a note is manually auditioned.
void CVEngine::switchGateOn(int channel, int doInstantlyIfPossible) {
  gateChannels[channel].on = true;

  if (doInstantlyIfPossible) {
    uint32_t timeSinceLastSwitchedOff =
        AudioEngine::audioSampleTimer - gateChannels[channel].timeLastSwitchedOff;
  ...
  // If this gate was switched off more recently than any previous gate
  // switch-off of a pending note-on, update the running record of that
  if ((int32_t)(gateChannels[channel].timeLastSwitchedOff -
                mostRecentSwitchOffTimeOfPendingNoteOn) > 0) {
    mostRecentSwitchOffTimeOfPendingNoteOn = gateChannels[channel].timeLastSwitchedOff;
  }
}

// note -32768 means switch "all notes off", or switch on without changing
// actual CV voltage output
void CVEngine::sendNote(bool on, uint8_t channel, int16_t note) {
  ...
  if (!on) {
    ...
    // Switch off, unless the note that's playing is a different one (i.e. if a
    // new one had already cut short this one that we're now saying we wanted to
    // stop)
    if (gateChannels[channel].on && (channel >= NUM_CV_CHANNELS || note == -32768 ||
                                     cvChannels[channel].noteCurrentlyPlaying == note)) {

      // Physically switch it right now, to get a head-start before it turns
      // back on
...
// Does it even if the corresponding gate isn't "on", because the note might
// still be audible on the connected physical synth
...
int32_t CVEngine::calculateVoltage(int note, uint8_t channel) {
  ...
  // Volts per octave
    ...
    voltage = (transposedNoteCode - 24) * 5.46 * cvChannels[channel].voltsPerOctave +
              0.5; // The 0.5 rounds it. And it's 5.46 rather than 546 because
                   // voltsPerOctave is in 0.01's of a volt.
  }
...
void CVEngine::updateRunOutput() {
  ...
  if (runState) {
    switchGateOn(WHICH_GATE_OUTPUT_IS_RUN,
                 true); // Try to do instantly, because it's actually good if
                        // RUN can switch on before the first clock is sent
  } else {
