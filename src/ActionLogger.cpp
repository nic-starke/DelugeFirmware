...
void ActionLogger::deleteLastActionIfEmpty() {
  if (firstAction[BEFORE]) {
    ...
    // There are probably more cases where we might want to do this, but I've
    // only done it for recording so far
...
Action* ActionLogger::getNewAction(int newActionType, int addToExistingIfPossible) {
  ...
  else if (addToExistingIfPossible && firstAction[BEFORE] &&
  ...
  // If we can't do that...
    ...
    int numClips = currentSong->sessionClips.getNumElements() +
                   currentSong->arrangementOnlyClips.getNumElements();

    ActionClipState* clipStates =
        (ActionClipState*)generalMemoryAllocator.alloc(numClips * sizeof(ActionClipState),
                                                       NULL,
                                                       true);
  ...
  traverseClips:
    ...
    // Only now put the new action into the list of undo actions - because in
    // the above steps, we may have decided to delete it and get out (if we ran
    // out of RAM while creating the ActionClipStates)
    newAction->nextAction = firstAction[BEFORE];
    firstAction[BEFORE]   = newAction;

    // And fill out all the snapshot stuff that the Action captures at a
    // song-wide level
...
void ActionLogger::updateAction(Action* newAction) {
  ...
  if (newAction->numClipStates) {
    ...
    if (newAction->numClipStates != currentSong->sessionClips.getNumElements() +
                                        currentSong->arrangementOnlyClips.getNumElements()) {
...
void ActionLogger::recordSwingChange(int8_t swingBefore, int8_t swingAfter) {
  ...
  if (action->firstConsequence) {
    ...
    if (consMemory) {
      ConsequenceSwingChange* newConsequence =
          new (consMemory) ConsequenceSwingChange(swingBefore, swingAfter);
...
// doNavigation and updateVisually are only false when doing one of those
// undo-Clip-resize things as part of another Clip resize. You must not call
// this during the card routine - though I've lost track of the exact reason why
// not - is it just because we could then be in the middle of executing
// whichever function accessed the card and we don't know if things will break?
bool ActionLogger::revert(int time, bool updateVisually, bool doNavigation) {
  ...
  if (firstAction[time]) {
    ...
    // If we're in a UI mode, and reverting this Action would mean changing UI,
    // we have to disallow that.
    if (toRevert->view != getCurrentUI() && !isNoUIModeActive()) { return false; }
...
// doNavigation and updateVisually are only false when doing one of those
// undo-Clip-resize things as part of another Clip resize
  ...
  if (doNavigation) {
    ...
    if (action->type == ACTION_ARRANGEMENT_RECORD) {
      ...
      if (getCurrentUI() == &arrangerView || getCurrentUI() == &sessionView) {
        ...
        // If in arranger view, don't go scrolling anywhere - that'd just
        // visually confuse things
    ...
    if (updateVisually) {
      ...
      // Or if none of those is happening, we might like to do a horizontal zoom
      // or scroll - only if [vertical scroll isn't changed], and we're not on
      // keyboard view
      else {
        if (getCurrentUI() != &keyboardScreen) {
          ...
          if (getCurrentUI() == &arrangerView) {
            ...
            else if (currentSong->xScroll[NAVIGATION_ARRANGEMENT] !=
    ...
    if (action->numClipStates) {
      int totalNumClips = currentSong->sessionClips.getNumElements() +
                          currentSong->arrangementOnlyClips.getNumElements();
    ...
    // Now do the animation we decided on - for animations which we prefer to
    // set up before reverting the actual action
    ...
    otherOption:
      if (getCurrentUI() != &arrangerView || whichAnimation != ANIMATION_ZOOM) {
        currentSong->xScroll[NAVIGATION_ARRANGEMENT] =
            action->xScrollArranger[time]; // Have to do this if we didn't do
                                           // the actual scroll animation yet
    ...
    if (whichAnimation == ANIMATION_SCROLL && getCurrentUI() != &arrangerView) {
      ...
      currentSong->xScroll[NAVIGATION_CLIP] =
          action->xScrollClip[time]; // Have to do this if we didn't do the
                                     // actual scroll animation yet some
                                     // scrolling happened
    ...
    if (whichAnimation == ANIMATION_ZOOM) {
      ...
      goto currentClipSwitchedOver; // Skip the below - our call to
                                    // transitionToViewForClip will switch it
                                    // over for us
    }

    // Swap currentClip over. Can only do this after calling
    // transitionToSessionView(). Previously, we did this much earlier, causing
    // a crash. Hopefully moving it later here is ok...
    if (action->currentClip) { // If song just loaded and we hadn't been into
                               // ClipMinder yet, this would be NULL, and we
                               // don't want to set currentSong->currentClip
                               // back to this
...
currentClipSwitchedOver:
  ...
  if (whichAnimation == ANIMATION_ENTER_KEYBOARD_VIEW) {
    ...
    if (action->view != getCurrentUI())
      ...
      renderingNeededRegardlessOfUI(); // Didn't have this til March 2020, and
                                       // stuff didn't update. Guess this is
                                       // just needed? Can't remember specifics
                                       // just now
    }
  }
  ...
  if (updateVisually) {
    ...
    if (getCurrentUI() == &instrumentClipView) {
      if (whichAnimation != ANIMATION_CLIP_MINDER_TO_SESSION &&
          whichAnimation != ANIMATION_CLIP_MINDER_TO_ARRANGEMENT) { // If we're not animating
                                                                    // away from this view
                                                                    // (but something like
                                                                    // scrolling sideways
                                                                    // would be allowed)
        instrumentClipView.recalculateColours();
        if (!whichAnimation) { uiNeedsRendering(&instrumentClipView); }
      }
    } else if (getCurrentUI() == &audioClipView) {
      if (!whichAnimation) { uiNeedsRendering(&audioClipView); }
      ...
      uiNeedsRendering(&sessionView,
                       0xFFFFFFFF,
                       0xFFFFFFFF); // Got to try this even if we're supposedly doing a
                                    // horizontal scroll animation or something cos that may
                                    // have failed if the Clip wasn't long enough before we
                                    // did the action->revert() ...
    } else if (getCurrentUI() == &arrangerView) {
      arrangerView.repopulateOutputsOnScreen(!whichAnimation);
    }

    // Usually need to re-display the mod LEDs etc, but not if either of these
    // animations is happening, which means that it'll happen anyway when the
    // animation finishes - and also, if we just deleted the Clip which was the
    // activeModControllableClip, well that'll temporarily be pointing to
    // invalid stuff. Check the actual UI mode rather than the whichAnimation
    // variable we've been using in this function, because under some
    // circumstances that'll bypass the actual animation / UI-mode.
    if (!isUIModeActive(UI_MODE_AUDIO_CLIP_COLLAPSING) &&
        !isUIModeActive(UI_MODE_INSTRUMENT_CLIP_COLLAPSING)) { // We would also put the
                                                               // "explode" animation for
                                                               // transitioning *to*
                                                               // arranger here, but it just
                                                               // doesn't get used during
                                                               // reversion.
      view.setKnobIndicatorLevels();
      view.setModLedStates();
    }

    switch (whichAnimation) { // So long as we're not gonna animate to different
                              // UI...
...
// You must not call this during the card routine - though I've lost track of
// the exact reason why not - is it just because we could then be in the middle
// of executing whichever function accessed the card and we don't know if things
// will break?
void ActionLogger::undo() {

  // Before we go and revert the most recent Action, there are a few
  // recording-related states we first want to have a go at cancelling out of.
  // These are treated as special cases here rather than being Consequences
  // because they're never redoable: their "undoing" is a special case of
  // cancellation.

  // But, this is to be done very sparingly! I used to have more of these which
  // did things like deleting Clips for which linear recording was ongoing. But
  // then what if other Consequences, e.g. param automation, had been recorded
  // for those? Reverting those would call functions on invalid pointers. So
  // instead, do just use regular Actions and Consequences for everything
  // possible. And definitely don't delete any Clips here.

  // If currently recording an arrangement from session, we have to stop doing
  // so first
  if (playbackHandler.recording == RECORDING_ARRANGEMENT) {
  ...
  else if (playbackHandler.playbackState && playbackHandler.recording &&
...
const uint32_t reversionUIModes[] = {UI_MODE_AUDITIONING,
...
void ActionLogger::notifyClipRecordingAborted(Clip* clip) {
  ...
  // If there's an Action which only recorded the beginning of this Clip
  // recording, we don't want it anymore.
...
// This function relies on Consequences having been sequentially added for each
// subsequent "mini action", so looking at the noteRowId of the most recent one,
// we can then know that all further Consequences until we see the same
// noteRowId again are part of the same "mini action". This will get called in
// some cases (Action types) were only one NoteRow, not many, could have had the
// editing done to it: that's fine too, and barely any time is really wasted
// here. Returns whether whole Action was reverted, which is the only case where
// visual updating / rendering, and also the calling of expectEvent(), would
// have taken place
bool ActionLogger::undoJustOneConsequencePerNoteRow(ModelStack* modelStack) {
  ...
  if (firstConsequence) { // Should always be true
    ...
    // If multiple Consequences per NoteRow, just revert most recent one per
    // NoteRow
    if (false) {
    gotMultipleConsequencesPerNoteRow:
      do {
        firstConsequence->revert(BEFORE, modelStack); // Unlike reverting a whole Action, this
                                                      // doesn't update anything visually or call
                                                      // any expectEvent() functions
