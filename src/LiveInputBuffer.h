...
public:
  ...
  bool
  getAveragesForCrossfade(int32_t* totals, int startPos, int lengthToAverageEach, int numChannels);
  ...
  int32_t rawBuffer[INPUT_RAW_BUFFER_SIZE]; // Must be last!!! Cos we're gonna
                                            // allocate and access it
                                            // double-length for stereo
};
