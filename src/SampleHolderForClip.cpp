  ...
  if (audioFile) {
    ...
    if (((Sample*)audioFile)->sampleRate != 44100) {
      neutralPhaseIncrement =
          (uint64_t)neutralPhaseIncrement * ((Sample*)audioFile)->sampleRate / 44100;
    }

    if (cents) {
      int32_t multiplier =
          interpolateTable(2147483648u + (int32_t)cents * 42949672, 32, centAdjustTableSmall);
...
void SampleHolderForClip::sampleBeenSet(bool reversed, bool manuallySelected) {
  ...
  if (manuallySelected && ((Sample*)audioFile)->fileLoopEndSamples &&
    ...
    if (((Sample*)audioFile)->fileLoopStartSamples < ((Sample*)audioFile)->lengthInSamples &&
      ...
      startPos = ((Sample*)audioFile)->fileLoopStartSamples; // If it's 0, that'll translate to
