...
void Patcher::recalculateFinalValueForParamWithNoCables(int                      p,
                                                        Sound*                   sound,
  ...
  if (p < patchableInfo->firstHybridParam) {
    ...
    if (p < patchableInfo->firstExpParam) {
      finalValue =
          getFinalParameterValueHybrid(paramNeutralValue, cableCombination); // Hybrid - add
    } else {
      finalValue =
          getFinalParameterValueExpWithDumbEnvelopeHack(paramNeutralValue, cableCombination, p);
    }
  }
...
int32_t rangeFinalValues[MAX_NUM_PATCH_CABLES]; // TODO: storing these in permanent
                                                // memory per voice could save a
                                                // tiny bit of time... actually so
                                                // minor though, maybe not worth it.

// You may as well check sourcesChanged before calling this.
void Patcher::performPatching(uint32_t                 sourcesChanged,
                              Sound*                   sound,
                              ParamManagerForTimeline* paramManager) {
  ...
  // Go through regular Destinations going directly to a param
  {
    ...
    for (; destination->destinationParamDescriptor.data < firstHybridParam; destination++) {
      ...
      cableCombinations[numParamsPatched] =
  ...
  params[numParamsPatched] = 255; // To stop the below code from going past the
                                  // end of the list we just created

  // Now, turn those cableCombinations into paramFinalValues. Splitting these up
  // like this caused a massive speed-up on all presets tested, somewhat
  // surprisingly!
  {
    int i = 0;
    ...
    for (; i < numParamsPatched; i++) {
      int p               = params[i];
      paramFinalValues[p] = getFinalParameterValueExpWithDumbEnvelopeHack(paramNeutralValues[p],
                                                                          cableCombinations[i],
                                                                          p);
    }
  }
...
inline void Patcher::applyRangeAdjustment(int32_t* patchedValue, PatchCable* patchCable) {
  ...
  *patchedValue = signed_saturate(small, 32 - 5)
                  << 3; // Not sure if these limits are as wide as they could be...
}

// Declaring these next 4 as inline made no performance difference.
inline void Patcher::cableToLinearParamWithoutRangeAdjustment(int32_t  sourceValue,
                                                              int32_t  cableStrength,
                                                              int32_t* runningTotalCombination) {
  int32_t scaledSource = multiply_32x32_rshift32(sourceValue, cableStrength);
  int32_t madePositive = (scaledSource + 536870912); // 0 to 1073741824; 536870912 counts as "1"
...
inline void Patcher::cableToLinearParam(int32_t     sourceValue,
                                        int32_t     cableStrength,
                                        int32_t*    runningTotalCombination,
...
inline void Patcher::cableToExpParamWithoutRangeAdjustment(int32_t  sourceValue,
                                                           int32_t  cableStrength,
                                                           int32_t* runningTotalCombination) {
  int32_t scaledSource = multiply_32x32_rshift32(sourceValue, cableStrength);
...
inline void Patcher::cableToExpParam(int32_t     sourceValue,
                                     int32_t     cableStrength,
                                     int32_t*    runningTotalCombination,
                                     PatchCable* patchCable) {
  int32_t scaledSource = multiply_32x32_rshift32(sourceValue, cableStrength);
...
inline int32_t Patcher::combineCablesLinearForRangeParam(Destination const* destination,
                                                         ParamManager*      paramManager) {
  int32_t runningTotalCombination =
      536870912; // 536870912 means "1". runningTotalCombination will not be
                 // allowed to get bigger than 2147483647, which means "4".
  ...
  for (int c = destination->firstCable; c < destination->endCable; c++) {
    ...
    // Special exception: If we're patching aftertouch to range. Normally,
    // unlike other sources, aftertouch goes from 0 to 2147483647. This is
    // because we want it to have no effect at its negative extreme, which isn't
    // normally what we want. However, when patched to range, we do want this
    // again, so "transpose" it here.
    if (s == PATCH_SOURCE_AFTERTOUCH) { sourceValue = (sourceValue - 1073741824) << 1; }
  ...
  // return value is ideally between -536870912 and 536870912, but may get up to
  // 1610612736 if the result above was "4" because of multiple patch cables
  // being multiplied together
}

// Linear param - combine all cables by multiplying their values (values centred
// around 1). Inputs effectively range from "0" to "2". Output (product) clips
// off at "4". Call this if (p < getFirstHybridParam()) - "Pan" sits at the end
// of the linear params and is the exception to the rule - it doesn't want this
// multiplying treatment Having this inline makes huge ~40% performance
// difference to performInitialPatching, I think because in that case it knows
// there are no cables.
inline int32_t Patcher::combineCablesLinear(Destination const* destination,
                                            unsigned int       p,
                                            Sound*             sound,
                                            ParamManager*      paramManager) {
  int32_t runningTotalCombination =
  ...
  cableToLinearParamWithoutRangeAdjustment(sound->getSmoothedPatchedParamValue(p, paramManager),
                                           paramRanges[p],
...
// Having this inline makes huge ~40% performance difference to
// performInitialPatching, I think because in that case it knows there are no
// cables.
inline int32_t Patcher::combineCablesExp(Destination const* destination,
                                         unsigned int       p,
                                         Sound*             sound,
                                         ParamManager*      paramManager) {
  ...
  if (destination) {
    ...
    // Hack for wave index params - make the patching (but not the preset value)
    // stretch twice as far, to allow the opposite end to be reached even if the
    // user's preset value is all the way to one end. These params are "hybrid"
    // ones, and probably in a perfect world I would have made the other ones
    // behave the same way. But I can't break users' songs.
  ...
  // Do the "preset value" (which we treat like a "cable" here)
  cableToExpParamWithoutRangeAdjustment(sound->getSmoothedPatchedParamValue(p, paramManager),
                                        paramRanges[p],
                                        &runningTotalCombination);
...
// NOTE: parameter preset values can't be bigger than 536870912, otherwise
// overflowing will occur
...
*/
  ...
  // In this function, we are sneaky and write the "cable combination" working
  // value in to paramFinalValues, before going back over that array with the
  // final step. This saves memory. Didn't seem to have a real effect on speed.

  {
    // The contents of this bit could be optimized further by doing even more
    // passes, splitting up e.g. combineCablesLinear into iterations, which
    // sometimes could be done on all params at once. Or, could just pre-compute
    // and copy "final values" for where there's no patching
    int p = patchableInfo->firstParam;

    // First for the params whose cables get multiplied, we go do them all as if
    // they have no patching, and then for the few which do, we overwrite those
    // values
    int firstHybridParam = patchableInfo->firstHybridParam;
    ...
    Destination* destination =
        paramManager->getPatchCableSet()->destinations[patchableInfo->globality];
    if (destination) {
      ...
      for (; destination->destinationParamDescriptor.data < firstHybridParamAsDescriptor;
           destination++) {
        int p               = destination->destinationParamDescriptor.getJustTheParam();
  ...
  // And now convert those "cable combinations" into "final values".
  {
    ...
    for (; p < firstNonVolumeParam; p++) {
      paramFinalValues[p] =
          getFinalParameterValueVolume(paramNeutralValues[p], paramFinalValues[p]);
    }
    ...
    for (; p < firstHybridParam; p++) {
      paramFinalValues[p] =
          getFinalParameterValueLinear(paramNeutralValues[p], paramFinalValues[p]);
    }
    ...
    for (; p < firstExpParam; p++) {
      paramFinalValues[p] =
          getFinalParameterValueHybrid(paramNeutralValues[p], paramFinalValues[p]);
    }
