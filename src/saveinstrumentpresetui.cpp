...
bool SaveInstrumentPresetUI::opened() {
  ...
  instrumentTypeToLoad = currentInstrument->type; // Must set this before calling SaveUI::opened(),
                                                  // which uses this to work out folder name
...
bool SaveInstrumentPresetUI::performSave(bool mayOverwrite) {
  ...
  if (isDifferentSlot) {
    ...
    // We can't save into this slot if another Instrument in this Song already
    // uses it
    if (currentSong->getInstrumentFromPresetSlot(instrumentTypeToLoad,
                                                 0,
                                                 0,
                                                 enteredText.get(),
                                                 currentDir.get(),
      ...
      numericDriver.displayPopup(
          HAVE_OLED ? "Another instrument in the song has the same name / number" : "CANT");
#if HAVE_OLED
    ...
    // Alright, we know the new slot isn't used by an Instrument in the Song,
    // but there may be an Instrument lurking in memory with that slot, which we
    // need to just delete
  ...
  char const* endString =
      (instrumentTypeToLoad == INSTRUMENT_TYPE_SYNTH) ? "\n</sound>\n" : "\n</kit>\n";

  error = storageManager.closeFileAfterWriting(filePath.get(),
  ...
  // There's now no chance that we saved over a preset that's already in use in
  // the song, because we didn't allow the user to select such a slot
...
void SaveInstrumentPresetUI::selectEncoderAction(int8_t offset) {
    ...
    if (numberEditPos == -1) {
        ...
        int previouslySavedSlot = instrument->name.isEmpty() ? instrument->slot
: -1;

        int error = storageManager.decideNextSaveableSlot(offset,
                &currentSlot, &currentSubSlot, &enteredText,
&currentFileIsFolder, previouslySavedSlot, &currentFileExists,
numInstrumentSlots, getThingName(instrumentType), currentDir.get(),
instrumentType, (Instrument*)currentSong->currentClip->output); if (error) {
            numericDriver.displayError(error);
        ...
        enteredTextEditPos = getMin(enteredTextEditPos,
enteredText.getLength());

        currentFilename.set(&enteredText); // Only used for folders.
    }

    // Editing specific digit of numeric names. Pretty much no one will really
be using this anymore else { int jumpSize; if (numberEditPos == 0) jumpSize = 1;
        ...
        storageManager.findNextInstrumentPreset(-1, instrumentType,
                &bestSlotFound, &bestSubSlotFound, NULL, NULL, // No folders
allowed. currentSlot + 1, -1, NULL, currentDir.get(), &nothing,
AVAILABILITY_ANY, &nothingInstrument, &nothing2);

        if (bestSlotFound == currentSlot) {
            // If the preset was already saved in this slot, offer a brand new
subslot if (currentSlot ==
((Instrument*)currentSong->currentClip->output)->slot) { currentSubSlot =
bestSubSlotFound + 1; currentFileExists = false;

                if (currentSubSlot >= 26) goto doNormalThing; // Watch out for
end of alphabet
            }
