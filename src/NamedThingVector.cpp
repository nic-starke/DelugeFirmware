...
int NamedThingVector::insertElement(void* namedThing, int i) {
  int error = insertAtIndex(i, 1, this); // While inserting, the stealing of any AudioFiles would
                                         // cause a simultaneous delete. They all know not to allow
                                         // theft when passed this AudioFileVector.
...
void NamedThingVector::removeElement(int i) {
  getMemory(i)->~NamedThingVectorElement(); // Have to call this so String gets
                                            // destructed!
  deleteAtIndex(i);
}
