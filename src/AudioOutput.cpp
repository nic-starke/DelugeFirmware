...
void AudioOutput::renderGlobalEffectableForClip(ModelStackWithTimelineCounter* modelStack,
                                                StereoSample*                  renderBuffer,
  ...
  if (activeClip) {
    ...
    renderEnvelope:
      int32_t amplitudeLocal =
          (envelope.render(numSamples, attack, 8388608, 2147483647, 0, decayTableSmall4) >> 1) +
          1073741824;
      ...
      if (envelope.state >= ENVELOPE_STAGE_OFF) {
        ...
        if (!amplitudeLastTime && attack > 245632) { amplitudeLastTime = amplitudeLocal; }

        int32_t amplitudeEffectiveStart =
            multiply_32x32_rshift32(amplitudeLastTime,
                                    amplitudeAtStart); // Reduces amplitude another >>1
        int32_t amplitudeEffectiveEnd =
            multiply_32x32_rshift32(amplitudeLocal,
                                    amplitudeAtEnd); // Reduces amplitude another >>1

        int32_t amplitudeIncrementEffective =
            (amplitudeEffectiveEnd - amplitudeEffectiveStart) / numSamples;

        int32_t* intBuffer = (int32_t*)renderBuffer;
        activeAudioClip->render(modelStack,
                                intBuffer,
                                numSamples,
                                amplitudeEffectiveStart,
                                amplitudeIncrementEffective,
                                pitchAdjust);
        ...
        if (AudioOutput::willRenderAsOneChannelOnlyWhichWillNeedCopying()) {
          ...
          // Or, if duplicating within same rendering buffer (cos there's FX to
          // be applied)
          else {
            int i = numSamples - 1;
  ...
  if (echoing && modelStack->song->isOutputActiveInArrangement(this)) {
    StereoSample* __restrict__ outputPos =
        bufferToTransferTo ? (StereoSample*)bufferToTransferTo : renderBuffer;
    ...
    do {
      ...
      switch (inputChannelNow) {
        ...
        default: // STEREO
          ...
          // Remember, there is no case for echoing out the "output" channel -
          // that function doesn't exist, cos you're obviously already hearing the
          // output channel
      }
...
bool AudioOutput::willRenderAsOneChannelOnlyWhichWillNeedCopying() {
  return (activeClip && ((AudioClip*)activeClip)->voiceSample &&
          (((AudioClip*)activeClip)->sampleHolder.audioFile->numChannels == 1 ||
...
void AudioOutput::cutAllSound() {
  if (activeClip) {
    ...
    ((AudioClip*)activeClip)->abortRecording(); // Needed for when this is being called as part of a
                                                // song-swap - we can't leave recording happening in
                                                // such a case.
  }
}

void AudioOutput::getThingWithMostReverb(Sound**                   soundWithMostReverb,
                                         ParamManagerForTimeline** paramManagerWithMostReverb,
                                         Kit**                     kitWithMostReverb,
                                         int32_t*                  highestReverbAmountFound) {}

// Unlike for Instruments, AudioOutputs will only be written as part of a Song,
// so clipForSavingOutputOnly will always be NULL
bool AudioOutput::writeDataToFile(Clip* clipForSavingOutputOnly, Song* song) {
  ...
  // be a backedUpParamManager that we should use / save
...
// clip will always be NULL and is of no consequence - see note in parent
// Output.h
...
bool AudioOutput::setActiveClip(ModelStackWithTimelineCounter* modelStack, int maySendMIDIPGMs) {
  if (activeClip && (activeClip != modelStack->getTimelineCounter() ||
