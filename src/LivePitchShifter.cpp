...
#if INPUT_ENABLE_REPITCHED_BUFFER
  if (repitchedBuffer) { generalMemoryAllocator.dealloc(repitchedBuffer); }
#endif
}

void LivePitchShifter::render(int32_t* __restrict__ outputBuffer,
  ...
  uint64_t numRawSamplesProcessedAtStart =
      liveInputBuffer->numRawSamplesProcessed - numSamplesThisFunctionCall;
  ...
  if (repitchedBuffer) {
    for (int i = 0; i < numSamplesThisFunctionCall; i++) {
      ...
      for (int b = INTERPOLATION_MAX_NUM_SAMPLES - 1; b >= 1; b--) {
        ...
        if (numChannels == 2) { interpolationBuffer[1][b] = interpolationBuffer[1][b - 1]; }
      }
      ...
      if (numChannels == 2) { interpolationBuffer[1][0] = inputSample->r; }

      while (oscPos < 16777216) {

        // Interpolate and put in buffer
        interpolate(&repitchedBuffer[repitchedBufferWritePos * numChannels],
...
startRenderAgain:
  ...
  if (!justDidHop && phaseIncrement > 16777216) {
    ...
    if (samplesTilHopEnd + nextCrossfadeLength > maxPlayableSamplesNewer) {
      ...
      if (samplesTilHopEnd < 0) {
        ...
        uint32_t minCrossfadeIncrement =
            (uint32_t)(16777216 - crossfadeProgress) / samplesTilHopEnd + 1;
    ...
    if (samplesTilHopEnd && olderPlayHeadIsCurrentlySounding()) {
      ...
      if (!maxPlayableSamplesOlder) {
        ...
        uint32_t minCrossfadeIncrement =
            (uint32_t)(16777216 - crossfadeProgress) / maxPlayableSamplesOlder + 1;
        // crossfadeIncrement = getMax(crossfadeIncrement,
        // minCrossfadeIncrement);
        if (minCrossfadeIncrement > crossfadeIncrement) {
          crossfadeIncrement = minCrossfadeIncrement;
          // Uart::println("c");
        }
  ...
  // If the percussiveness is higher at "now" time than at the newer play head,
  // end the hop. This was designed with pitching-down in mind, but sounds good
  // on pitching-up too
  if (!justDidHop && !olderPlayHeadIsCurrentlySounding() && samplesTilHopEnd &&
      playHeads[PLAY_HEAD_NEWER].mode != PLAY_HEAD_MODE_RAW_DIRECT) {

    int howFarBack = playHeads[PLAY_HEAD_NEWER].getNumRawSamplesBehindInput(liveInputBuffer,
                                                                            this,
                                                                            phaseIncrement);

    uint32_t newerPlayHeadPercPos = (liveInputBuffer->numRawSamplesProcessed - howFarBack - 1) >>
                                    PERC_BUFFER_REDUCTION_MAGNITUDE;

    uint32_t latestPercPosBefore =
        (numRawSamplesProcessedAtStart - 1) >> PERC_BUFFER_REDUCTION_MAGNITUDE;
    uint32_t latestPercPosNow =
        (liveInputBuffer->numRawSamplesProcessed - 1) >> PERC_BUFFER_REDUCTION_MAGNITUDE;
    ...
    if (latestPercPosNow != newerPlayHeadPercPos &&
      ...
      int percNewerPlayHead =
          liveInputBuffer->percBuffer[newerPlayHeadPercPos & (INPUT_PERC_BUFFER_SIZE - 1)];
  ...
  int numSamplesThisTimestretchedRead =
      getMin((int32_t)numSamplesThisFunctionCall, samplesTilHopEnd);
  ...
  // If the older play-head is still fading out, which means we need to work out
  // the crossfade envelope too...
  if (olderPlayHeadAudibleHere) {

    // Use linear crossfades. These sound less jarring when doing short hops.
    // Square root ones sound better for longer hops, with more difference in
    // material between hops (i.e. time more stretched)
    ...
    int32_t newerHopAmplitudeIncrement = (int32_t)(newerHopAmplitudeAfter - newerHopAmplitudeNow) /
                                         (int32_t)numSamplesThisTimestretchedRead;
    int32_t olderHopAmplitudeIncrement = -newerHopAmplitudeIncrement;

    int32_t hopAmplitudeChange = multiply_32x32_rshift32(amplitude, newerHopAmplitudeIncrement)
                                 << 1;
  ...
  playHeads[PLAY_HEAD_NEWER].render(outputBuffer,
  ...
  if (olderPlayHeadAudibleHere) {
    playHeads[PLAY_HEAD_OLDER].render(outputBuffer,
                                      numSamplesThisTimestretchedRead,
                                      numChannels,
                                      phaseIncrement,
...
void LivePitchShifter::hopEnd(int32_t          phaseIncrement,
                              LiveInputBuffer* liveInputBuffer,
                              uint64_t         numRawSamplesProcessedAtNowTime,
  ...
  // Uart::println((uint32_t)(liveInputBuffer->numRawSamplesProcessed -
  // playHeads[PLAY_HEAD_NEWER].rawBufferReadPos) & (INPUT_RAW_BUFFER_SIZE -
  // 1)); // How far behind raw buffer is being read
  ...
  */
  ...
  // Collect info on those moving average for the now-older play-head, which
  // we're going to fade out - so that we can then fine-tune the position of the
  // new play-head to match it
  ...
  lengthPerMovingAverage =
      getMin(lengthPerMovingAverage,
             TIME_STRETCH_CROSSFADE_MOVING_AVERAGE_LENGTH * 2); // Keep things sensible

  // Ok, and this crossfade we're about to do, how long will it be in samples of
  // (unpitched) source material?
  int crossfadeLengthSamplesSource = ((uint64_t)thisCrossfadeLength * phaseIncrement) >> 24;

  // What's the maximum amount further forward than that older play-head that
  // data actually exists yet for us to examine?
  int maxOffsetFromHead =
      (uint32_t)(numRawSamplesProcessedLatest - playHeads[PLAY_HEAD_OLDER].rawBufferReadPos) &
  ...
  averagesEndOffsetFromHead = getMin(averagesEndOffsetFromHead,
                                     maxOffsetFromHead); // And make sure it's not beyond the end of
                                                         // the existent data

  // We now know the length of the *total* moving-averages region, so divide
  // down to get the length of *each* moving-average region If commenting out
  // this next line, must make sure we still don't search back before we started
  // writing to buffer
  lengthPerMovingAverage =
      getMin(lengthPerMovingAverage,
             averagesEndOffsetFromHead >> 1); // / TIME_STRETCH_CROSSFADE_NUM_MOVING_AVERAGES
  ...
  // Occasionally (if we've only just switched pitch shifting on), that'll be
  // zero, so we can'd do the moving averages. But if it's ok, then...
  if (lengthPerMovingAverage) {
    int averagesStartPosOldHead =
        (playHeads[PLAY_HEAD_OLDER].rawBufferReadPos + averagesStartOffsetFromHead) &
        (INPUT_RAW_BUFFER_SIZE - 1);
    liveInputBuffer->getAveragesForCrossfade(oldHeadTotals,
                                             averagesStartPosOldHead,
                                             lengthPerMovingAverage,
                                             numChannels);
  }
  ...
  // Now we're going to pick a position for the new play-head based on searching
  // the percussiveness data. This is totally different for pitching up vs down.
  // And while we're at it, also decide our search parameters for our
  // fine-tuning of the new play head pos - if we're going to do that (we might
  // not - if lengthPerMovingAverage == 0)
  ...
  if (phaseIncrement > 16777216) {
    ...
    if (liveInputBuffer) {
      ...
      // Search backwards, looking for the area (ending at now-time) with the
      // highest average percussiveness. This particularly helps to keep the
      // hops from all happening at the same length, which leads to a clearly
      // audible sort of "tone". But, some sort of random element could actually
      // work just as well - I haven't tried yet.

#if !MEASURE_HOP_END_PERFORMANCE
      minSearch += (multiply_32x32_rshift32(minSearch,
      ...
      int percPos            = (numRawSamplesProcessedAtNowTime + PERC_BUFFER_REDUCTION_SIZE - 1) >>
                    PERC_BUFFER_REDUCTION_MAGNITUDE;
      ...
      while (backEdge < (maxSearch >> PERC_BUFFER_REDUCTION_MAGNITUDE)) {
        ...
        while (howFarBackSearched < backEdge) {
          ...
          int percHere = liveInputBuffer->percBuffer[(uint32_t)(percPos - howFarBackSearched) &
    ...
    samplesTilHopEnd =
        ((uint64_t)howFarBack << 24) / (uint32_t)(phaseIncrement - 16777216) - nextCrossfadeLength;
    if (samplesTilHopEnd < 100)
      samplesTilHopEnd = 100; // Must be 100, not 200. Otherwise, shifting up 2 octaves gets
                              // messed up. (Though maybe not anymore?)
  ...
  if (lengthPerMovingAverage && playHeads[PLAY_HEAD_OLDER].mode != PLAY_HEAD_MODE_RAW_DIRECT) {
    ...
    // Ok, and now we're going to get that moving-average info for the new
    // play-head in the position where we're currently proposing we put it - but
    // we're then going to experiment with shifting that, below

    if (((uint32_t)(averagesStartPosNewHead - averagesStartOffsetFromHead) &
         (INPUT_RAW_BUFFER_SIZE - 1)) >= numRawSamplesProcessedLatest)
      goto stopSearch;
    ...
    liveInputBuffer->getAveragesForCrossfade(newHeadTotals,
                                             averagesStartPosNewHead,
                                             lengthPerMovingAverage,
                                             numChannels);
  ...
  startSearch:
    ...
    for (int i = 0; i < TIME_STRETCH_CROSSFADE_NUM_MOVING_AVERAGES; i++) {
      ...
      readPos[i + 1] =
          (uint32_t)(readPos[i] + lengthPerMovingAverage) & (INPUT_RAW_BUFFER_SIZE - 1);
    }
    ...
    if (searchDirection == -1) {
      if (numRawSamplesProcessedLatest < INPUT_RAW_BUFFER_SIZE) { // Is this right?
        searchSizeBoundary = averagesStartPosNewHead - averagesStartOffsetFromHead -
                             1; // The -1 is becacuse of the 1 we subtract from
                                // readPos[0] above when searching left
        if (searchSizeBoundary <= 0) goto searchNextDirection;
      }
    } else {
      searchSizeBoundary = (uint32_t)(numRawSamplesProcessedLatest -
                                      readPos[TIME_STRETCH_CROSSFADE_NUM_MOVING_AVERAGES + 1]) &
                           (INPUT_RAW_BUFFER_SIZE - 1);
    }
    ...
    do {
      ...
      for (int i = 0; i < TIME_STRETCH_CROSSFADE_NUM_MOVING_AVERAGES + 1; i++) {
        ...
        if (numChannels == 2) { readValue += liveInputBuffer->rawBuffer[readPos[i] * 2 + 1] >> 16; }
      ...
      if ((thisTotalChange >= 0) != (lastTotalChange >= 0)) {
        ...
        // lining-up-wise. The benefit of this is visible on a spectrum analysis
        // if you're pitching a high-pitched sine wave right down, while also
        // time stretching it
        if (phaseIncrement != 16777216 &&
            (thisOffsetIsBestMatch ||
             offsetNow == bestOffset)) { // If best was this one or last one
        ...
        // After sign has flipped twice (in total, including both search
        // directions), we can be fairly sure we've found a good fit
        timesSignFlipped++;
#if !MEASURE_HOP_END_PERFORMANCE
...
stopSearch:
  ...
  playHeads[PLAY_HEAD_NEWER].rawBufferReadPos =
      (uint32_t)(playHeads[PLAY_HEAD_NEWER].rawBufferReadPos + bestOffset) &
      (INPUT_RAW_BUFFER_SIZE - 1);

#if INPUT_ENABLE_REPITCHED_BUFFER
  // If pitching up, use repitched buffer if possible
  if (stillWritingToRepitchedBuffer && repitchedBufferNumSamplesWritten &&
    ...
    if (repitchedBufferNumSamplesWritten >= howFarBackRepitched) {
      ...
      playHeads[PLAY_HEAD_NEWER].repitchedBufferReadPos =
          (uint32_t)(repitchedBufferWritePos - howFarBackRepitched) &
  ...
  if (phaseIncrement == 16777216) {
    playHeads[PLAY_HEAD_NEWER].mode = PLAY_HEAD_MODE_RAW_DIRECT;
    playHeads[PLAY_HEAD_NEWER].rawBufferReadPos =
        numRawSamplesProcessedAtNowTime & (INPUT_RAW_BUFFER_SIZE - 1);
...
thatsDone:
  ...
  /*
  if (phaseIncrement > 16777216) {
      uint64_t totalPlayableSamples = ((uint64_t)howFarBack << 24) /
  (uint32_t)(phaseIncrement - 16777216); totalPlayableSamples =
  getMax(totalPlayableSamples, (uint64_t)2);

      totalPlayableSamples = getMin(totalPlayableSamples, (uint64_t)16384);

      if (nextCrossfadeLength > totalPlayableSamples - 1) nextCrossfadeLength =
  totalPlayableSamples - 1;
...
void LivePitchShifter::considerRepitchedBuffer(int32_t phaseIncrement) {
  ...
  if (phaseIncrement > 16777216) {
    if (!repitchedBuffer) {
      ...
      repitchedBuffer = (int32_t*)generalMemoryAllocator.alloc(INPUT_REPITCHED_BUFFER_SIZE *
    ...
    if (repitchedBuffer) { stillWritingToRepitchedBuffer = false; }
  }
#endif
}
...
void LivePitchShifter::interpolate(int32_t* sampleRead,
                                   int      interpolationBufferSize,
                                   int      numChannelsNow,
                                   int      whichKernel) {
