...
int ConsequenceScaleAddNote::revert(int time, ModelStack* modelStack) {
  ...
  // The only thing we actually have to do is delete any NoteRows that had the
  // new yNoteWithinOctave. The changing back of the scale itself is handled by
  // the Action, which keeps a record
  if (time == BEFORE) {
