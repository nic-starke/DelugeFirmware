...
// Returns -1 if not found. All times this is called, it actually should get
// found - but some bugs remain, and the caller must deal with these.
int AudioFileVector::searchForExactObject(AudioFile* audioFile) {
  ...
  // If object doesn't match, then we were looking for a Sample and got a
  // Wavetable, or vice versa. So check neighbours.
