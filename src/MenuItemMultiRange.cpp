...
void MenuItemMultiRange::beginSession(MenuItem* navigatedBackwardFrom) {
  ...
  // If there's already a range (e.g. because we just came back out of a
  // menu)...
  ...
  if (soundEditor.currentSource->defaultRangeI < 0 ||
      soundEditor.currentSource->defaultRangeI >=
          numRanges) { // If default is invalid, work it out afresh
  ...
  if (soundEditor.editingRangeEdge) {
    ...
    if (soundEditor.editingRangeEdge == RANGE_EDIT_LEFT) {
      ...
      if (offset >= 0) {
        ...
        if (soundEditor.currentValue < soundEditor.currentSource->ranges.getNumElements() - 1) {
          MultiRange* currentRange =
              soundEditor.currentSource->ranges.getElement(soundEditor.currentValue);
          maximum = currentRange->topNote - 1;
        } else
          maximum = 127;

        if (lowerRange->topNote < maximum) { lowerRange->topNote++; }
      }
      ...
      // Lowering
        ...
        if (lowerRange->topNote > minimum) { lowerRange->topNote--; }
      }
    }
    ...
    // Editing right
      ...
      if (offset >= 0) {
        ...
        if (currentRange->topNote < maximum) { currentRange->topNote++; }
      }
      ...
      // Lowering
        ...
        if (currentRange->topNote > minimum) { currentRange->topNote--; }
      }
    }
  ...
  // Or, normal mode
    ...
    if (Buttons::isShiftButtonPressed()) {
      ...
      if (soundEditor.currentValue == soundEditor.currentSource->ranges.getNumElements() - 1) {
        ...
        currentRangeTop =
            soundEditor.currentSource->ranges.getElement(soundEditor.currentValue)->topNote;
      }
...
#if HAVE_OLED
        soundEditor.menuCurrentScroll++; // Won't go past end of list, cos list
                                         // just grew.
#endif
      }
      ...
      else if (soundEditor.menuCurrentScroll <
               soundEditor.currentValue - OLED_MENU_NUM_OPTIONS_VISIBLE + 1)
...
void MenuItemMultiRange::deletePress() {
  ...
  // Otherwise...
    ...
    // If middle-ish one
      ...
      soundEditor.currentMultiRange->topNote =
          (soundEditor.currentMultiRange->topNote + oldTopNote) >> 1;
    }
  }
...
void MenuItemMultiRange::drawPixelsForOled() {
  ...
  doHilightJustOneEdge:
    ...
    OLED::invertArea(hilightStartX,
                     hilightWidth,
                     baseY,
                     baseY + TEXT_SPACING_Y,
                     OLED::oledMainImage);
