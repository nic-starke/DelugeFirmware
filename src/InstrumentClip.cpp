...
// Supplying song is optional, and basically only for the purpose of setting
// yScroll according to root note
InstrumentClip::InstrumentClip(Song* song) : Clip(CLIP_TYPE_INSTRUMENT) {
  ...
  if (song) {
    ...
    yScroll = 0; // Only for safety. Shouldn't actually get here if we're not
                 // going to overwrite this elsewhere I think...
  }
...
// You must call prepareForDestruction() before this, preferably by calling
// Song::deleteClipObject() Will call audio routine!!! Necessary to avoid voice
// cuts, especially when switching song
InstrumentClip::~InstrumentClip() {

  // Note: it's possible that we might be currentlyRecordingLinearly if we're
  // being destructed because of a song-swap. That's ok. Whereas, for
  // AudioClips, it's made sure that all linear recording is stopped first
...
void InstrumentClip::copyBasicsFrom(Clip* otherClip) {
  ...
  memcpy(backedUpInstrumentSlot,
         otherInstrumentClip->backedUpInstrumentSlot,
         sizeof(backedUpInstrumentSlot));
  memcpy(backedUpInstrumentSubSlot,
         otherInstrumentClip->backedUpInstrumentSubSlot,
...
int InstrumentClip::clone(ModelStackWithTimelineCounter* modelStack, bool shouldFlattenReversing) {
  ...
  InstrumentClip* newClip = new (clipMemory) InstrumentClip(); // Don't supply Song. yScroll will
                                                               // get set in copyBasicsFrom()
  ...
  int error =
      newClip->paramManager.cloneParamCollectionsFrom(&paramManager, true, true, reverseWithLength);
  if (error) {
  deleteClipAndGetOut:
  ...
  for (int i = 0; i < newClip->noteRows.getNumElements(); i++) {
    ...
    // If that fails, we have to keep going, cos otherwise some NoteRows'
    // NoteVector will be left pointing to stuff it shouldn't be
  }
  ...
  // Leave PINGPONG as it is, because we haven't actually flattened that - its
  // effect wouldn't be seen until a repeat happened. And we may be about to
  // flatten it with a increaseLengthWithRepeats(), so need to keep this
  // designation for now.

  return NO_ERROR;
}

// newLength might not be any longer than we already were - but this function
// still gets called in case any shorter NoteRows need lengthening. So, this
// function must allow for that case (Clip length staying the same).
void InstrumentClip::increaseLengthWithRepeats(ModelStackWithTimelineCounter* modelStack,
                                               int32_t                        newLength,
                                               int     independentNoteRowInstruction,
                                               bool    completelyRenderOutIterationDependence,
                                               Action* action) {
  ...
  for (int i = 0; i < noteRows.getNumElements(); i++) {
    ...
    if (thisNoteRow->loopLengthIfIndependent) {
      ...
      switch (independentNoteRowInstruction) {
        ...
        case INDEPENDENT_NOTEROW_LENGTH_INCREASE_ROUND_UP:
          newLengthHere =
              ((uint32_t)(newLength - 1) / (uint32_t)thisNoteRow->loopLengthIfIndependent + 1) *
    ...
    if (newLengthHere > oldLengthHere) { // Or do nothing if length staying the same
      ...
      thisNoteRow->generateRepeats(modelStackWithNoteRow,
                                   oldLengthHere,
                                   newLengthHere,
                                   numRepeatsRoundedHere,
                                   action);
  ...
  if (pingponging)
    sequenceDirectionMode = SEQUENCE_DIRECTION_FORWARD; // Pingponging has been flattened out, and
                                                        // although there are arguments either way,
                                                        // I think removing that setting now is
                                                        // best.
...
// Call this *after* you've set length to its new value (why did I do it this
// way?)
void InstrumentClip::lengthChanged(ModelStackWithTimelineCounter* modelStack,
  ...
  if (loopLength < oldLength) {
    for (int i = 0; i < noteRows.getNumElements(); i++) {
      ...
      // Only if NoteRow doesn't have independent length set, then trim it and
      // stuff
      if (!thisNoteRow->loopLengthIfIndependent) {
      ...
      // Or if it does have independent length, are we now the same length as
      // it?
      else {
...
// Does this individually for each NoteRow, because they might be different
// lengths, and some might need repeating while others need chopping.
void InstrumentClip::repeatOrChopToExactLength(ModelStackWithTimelineCounter* modelStack,
    ...
    if (oldLengthHere != newLength) {
      ...
      if (newLength > oldLengthHere) {
        int numRepeatsRounded =
            (uint32_t)(newLength + (oldLengthHere >> 1)) / (uint32_t)oldLengthHere;
        thisNoteRow->generateRepeats(modelStackWithNoteRow,
                                     oldLengthHere,
                                     newLength,
                                     numRepeatsRounded,
  ...
  if (newLength > loopLength) {
    ...
    if (pingponging)
      sequenceDirectionMode = SEQUENCE_DIRECTION_FORWARD; // Pingponging has been flattened out, and
                                                          // although there are arguments either
                                                          // way, I think removing that setting now
                                                          // is best.
  }
  ...
  Clip::lengthChanged(modelStack,
                      oldLength,
                      NULL); // Call this on Clip::, not us InstrumentClip, because we've done
                             // our own version above of what that call would do.
...
void InstrumentClip::halveNoteRowsWithIndependentLength(ModelStackWithTimelineCounter* modelStack) {
  for (int i = 0; i < noteRows.getNumElements(); i++) {
    ...
    ModelStackWithNoteRow* modelStackWithNoteRow =
        modelStack->addNoteRow(getNoteRowId(noteRow, i), noteRow);

    if (noteRow->loopLengthIfIndependent) {
      noteRow->setLength(modelStackWithNoteRow,
                         noteRow->loopLengthIfIndependent >> 1,
                         NULL,
                         modelStackWithNoteRow->getLastProcessedPos(),
                         true);
    }
  }
}
...
void InstrumentClip::setPos(ModelStackWithTimelineCounter* modelStack,
  ...
  Clip::setPos(modelStack,
               newPos,
               useActualPosForParamManagers); // This will also call our own
                                              // virtual expectEvent()

  noteRowsNumTicksBehindClip = 0;

  Clip::setPosForParamManagers(modelStack,
                               useActualPosForParamManagers); // Call on Clip:: only - below in this
                                                              // function, we're going to do the
  ...
  for (int i = 0; i < noteRows.getNumElements(); i++) {
    ...
    // This function is "supposed" to call setPosForParamManagers() on this
    // InstrumentClip, but instead, we'll do our own thing here, so we only have
    // to iterate through NoteRows once.
    if (thisNoteRow->paramManager.mightContainAutomation()) {
      ModelStackWithThreeMainThings* modelStackWithThreeMainThings =
          modelStack->addNoteRowAndExtraStuff(i, thisNoteRow);
      thisNoteRow->paramManager.setPlayPos(
          posForParamManagers,
    ...
    if (thisNoteRow->hasIndependentPlayPos()) {
      ...
      int effectiveSequenceDirectionMode =
      ...
      thisNoteRow->currentlyPlayingReversedIfIndependent =
          (effectiveSequenceDirectionMode ==
               SEQUENCE_DIRECTION_REVERSE // Syncing pingponging with
                                          // repeatCount is particularly
  ...
  if (output->type == INSTRUMENT_TYPE_KIT) {
    ...
    for (Drum* thisDrum = kit->firstDrum; thisDrum; thisDrum = thisDrum->next) {
      ...
      if (velocity) {
        ...
        int      noteRowIndex;
        NoteRow* noteRow =
            getNoteRowForDrum(thisDrum, &noteRowIndex); // Remember, I'm planning to introduce a
                                                        // faster search/index for this
        if (noteRow) {
    ...
    if (melodicInstrument->earlyNotes.getNumElements()) {
      ...
      if (action && scaleAltered) { action->updateYScrollClipViewAfter(); }
    }
...
// Grabs automated values from current play-pos. To be called after a possible
// big change made to automation data, e.g. after an undo. This is only to be
// called if playbackHandler.isEitherClockActive().
...
int InstrumentClip::transferVoicesToOriginalClipFromThisClone(
    ModelStackWithTimelineCounter* modelStackOriginal,
  ...
  if (output->type == INSTRUMENT_TYPE_KIT) {
    ...
    for (int i = 0; i < noteRows.getNumElements(); i++) {
      ...
      ModelStackWithNoteRow* modelStackWithNoteRowClone =
          modelStackClone->addNoteRow(i, clonedNoteRow);
      ModelStackWithNoteRow* modelStackWithNoteRowOriginal =
          modelStackOriginal->addNoteRow(i, originalNoteRow);
    ...
    for (int i = 0; i < noteRows.getNumElements(); i++) {
      ...
      ModelStackWithNoteRow* modelStackWithNoteRowOriginal =
          modelStackOriginal->addNoteRow(clonedNoteRow->y,
                                         originalNoteRow); // May end up with NULL noteRow
  ...
  if (output->type == INSTRUMENT_TYPE_KIT) {
    ...
    for (int i = 0; i < noteRows.getNumElements(); i++) {
      ...
      if (otherNoteRow->loopLengthIfIndependent)
        continue; // Skip NoteRows with independent length - they'll take care
                  // of themselves.
      ...
      ModelStackWithNoteRow* otherModelStackWithNoteRow =
          otherModelStack->addNoteRow(i, otherNoteRow);

      int error = thisNoteRow->appendNoteRow(thisModelStackWithNoteRow,
                                             otherModelStackWithNoteRow,
                                             loopLength,
                                             whichRepeatThisIs,
                                             otherInstrumentClip->loopLength);
    ...
    for (int i = 0; i < otherInstrumentClip->noteRows.getNumElements(); i++) {
      ...
      ModelStackWithNoteRow* thisModelStackWithNoteRow =
          getNoteRowForYNote(noteRowId, thisModelStack);
...
bool InstrumentClip::wantsToBeginLinearRecording(Song* song) {
  ...
  if (isPendingOverdub)
    return true; // Must take precedence - because we may have already placed
                 // some new notes at 0 if user hit key just now
...
void InstrumentClip::processCurrentPos(ModelStackWithTimelineCounter* modelStack,
  ...
  // We already incremented / decremented noteRowsNumTicksBehindClip and
  // ticksTilNextNoteRowEvent, in the call to incrementPos().
  ...
  if (ticksTilNextNoteRowEvent <= 0) {
    ...
    static PendingNoteOnList pendingNoteOnList; // Making this static, which it really should have
                                                // always been, actually didn't help max stack usage
                                                // at all somehow...
    ...
    for (int i = 0; i < noteRows.getNumElements(); i++) {
      ...
      int32_t noteRowTicksTilNextEvent = thisNoteRow->processCurrentPos(modelStackWithNoteRow,
    ...
    for (int i = 0; i < pendingNoteOnList.count; i++) {
      ...
      // Add to probability total sum - only if we hadn't already found a
      // pending note-on with this probability value
      // if (probabilityCount[pendingNoteOnList.pendingNoteOns[i].probability -
      // 1] == 0)
    ...
    for (int i = 0; i < pendingNoteOnList.count; i++) {
      ...
      // Otherwise...
        ...
        if (probability > NUM_PROBABILITY_VALUES) {
          ...
          ModelStackWithNoteRow* modelStackWithNoteRow =
              modelStack->addNoteRow(pendingNoteOnList.pendingNoteOns[i].noteRowId,
                                     pendingNoteOnList.pendingNoteOns[i].noteRow);
        ...
        // Or if it's an actual probability kind...
          ...
          if (pendingNoteOnList.pendingNoteOns[i].probability & 128) {
            ...
            // Check that that previous probability value is still valid. It
            // normally should be, unless the user has changed the probability
            // of that "previous" note
          ...
          // Or if not based on a previous probability...
            ...
            doNewProbability:
              // If the outcome of this probability has already been decided (by
              // another note with same probability)
...
void InstrumentClip::sendPendingNoteOn(ModelStackWithTimelineCounter* modelStack,
  ...
  if (output->type == INSTRUMENT_TYPE_KIT) {
    ModelStackWithThreeMainThings* modelStackWithThreeMainThings =
        modelStackWithNoteRow->addOtherTwoThings(pendingNoteOn->noteRow->drum->toModControllable(),
                                                 &pendingNoteOn->noteRow->paramManager);

    pendingNoteOn->noteRow->drum->noteOn(modelStackWithThreeMainThings,
                                         pendingNoteOn->velocity,
                                         (Kit*)output,
                                         mpeValues,
                                         MIDI_CHANNEL_NONE,
                                         pendingNoteOn->sampleSyncLength,
    ...
    ((MelodicInstrument*)output)
        ->sendNote(modelStackWithThreeMainThings,
                   true,
                   pendingNoteOn->noteRow->getNoteCode(),
                   mpeValues,
                   MIDI_CHANNEL_NONE,
                   pendingNoteOn->velocity,
                   pendingNoteOn->sampleSyncLength,
...
ModelStackWithNoteRow*
InstrumentClip::getNoteRowOnScreen(int yDisplay, ModelStackWithTimelineCounter* modelStack) {
  int      noteRowIndex;
...
ModelStackWithNoteRow*
InstrumentClip::getNoteRowForYNote(int yNote, ModelStackWithTimelineCounter* modelStack) {
  int      noteRowIndex;
...
ModelStackWithNoteRow*
InstrumentClip::getNoteRowForSelectedDrum(ModelStackWithTimelineCounter* modelStack) {
  int      noteRowId;
  NoteRow* noteRow = NULL;
  if (output->type == INSTRUMENT_TYPE_KIT) {
    Kit* kit = (Kit*)output;
    if (kit->selectedDrum) { noteRow = getNoteRowForDrum(kit->selectedDrum, &noteRowId); }
  }
  return modelStack->addNoteRow(noteRowId, noteRow);
}
...
ModelStackWithNoteRow*
InstrumentClip::getNoteRowForDrumName(ModelStackWithTimelineCounter* modelStack, char const* name) {
...
// *scaleAltered will not be set to false first - set it yourself. So that this
// can be called multiple times
ModelStackWithNoteRow*
InstrumentClip::getOrCreateNoteRowForYNote(int                            yNote,
    ...
    if (thisNoteRow) {
      ...
      if (!modelStack->song->isYNoteAllowed(yNote, inScaleMode)) {
        ...
        int newI = thisNoteRow->notes.insertAtKey(
            0); // Total hack - make it look like the NoteRow has a Note, so it
                // doesn't get discarded during setRootNote(). We set it back
                // (and then will soon give it a real note) really soon
        ...
        if (action) {
          ...
          memcpy(action->modeNotes[AFTER],
                 modelStack->song->modeNotes,
                 sizeof(modelStack->song->modeNotes));
        }
      }
...
// I think you need to check (playbackHandler.isEitherClockActive() &&
// song->isClipActive(thisClip)) before calling this.
...
void InstrumentClip::expectNoFurtherTicks(Song* song, bool actuallySoundChange) {
  ...
  ModelStackWithTimelineCounter* modelStack =
      setupModelStackWithTimelineCounter(modelStackMemory,
                                         song,
                                         this); // TODO: make caller supply this

  stopAllNotesPlaying(modelStack,
                      actuallySoundChange && !currentlyRecordingLinearly); // Stop all sound
      ...
      if (thisNoteRow->drum &&
        ...
        ModelStackWithThreeMainThings* modelStackWithThreeMainThingsForNoteRow =
            modelStack->addNoteRow(i, thisNoteRow)
                ->addOtherTwoThings(thisNoteRow->drum->toModControllable(),
...
// Check that we're allowed to make sound before you call this (nowhere does, is
// that bad?)
...
NoteRow* InstrumentClip::createNewNoteRowForKit(ModelStackWithTimelineCounter* modelStack,
  ...
  newNoteRow->setDrum(newDrum,
                      (Kit*)output,
                      modelStackWithNoteRow); // It might end up NULL. That's fine

  if (atStart) {
    yScroll++;

    // Adjust colour offset, because colour offset is relative to the lowest
    // NoteRow, and we just made a new lowest one
    colourOffset--;
  }
...
void InstrumentClip::musicalModeChanged(uint8_t                        yVisualWithinOctave,
  ...
  // Find all NoteRows which belong to this yVisualWithinOctave, and change
  // their note
  for (int i = 0; i < noteRows.getNumElements(); i++) {
    NoteRow* thisNoteRow = noteRows.getElement(i);
...
void InstrumentClip::seeWhatNotesWithinOctaveArePresent(bool  notesWithinOctavePresent[],
  ...
  song->rootNote = newRootNote; // Not ideal to be setting the global root note here... but
                                // as it happens, there's no scenario (currently) where this
  ...
  for (int i = 0; i < noteRows.getNumElements();) {
    ...
    if (!thisNoteRow->hasNoNotes()) {
      notesWithinOctavePresent[song->getYNoteWithinOctaveFromYNote(thisNoteRow->getNoteCode())] =
          true;
      i++;
    }

    // If this NoteRow has no notes, delete it, otherwise we'll have problems as
    // the musical mode is changed
    else {
      noteRows.deleteNoteRowAtIndex(i);
    }
  }
...
bool InstrumentClip::renderAsSingleRow(ModelStackWithTimelineCounter* modelStack,
  ...
  Clip::renderAsSingleRow(modelStack,
                          editorScreen,
                          xScroll,
                          xZoom,
                          image,
  ...
  for (int i = noteRowIndexStart; i < noteRowIndexEnd; i++) {
    ...
    thisNoteRow->renderRow(editorScreen,
                           mainColour,
                           tailColour,
                           blurColour,
                           image,
...
int InstrumentClip::guessRootNote(Song* song, int previousRoot) {
  ...
  seeWhatNotesWithinOctaveArePresent(notesPresent,
                                     0,
                                     song,
                                     false); // Don't delete anything yet, since we're still going
                                             // to make use of the noteRowsOnScreen!
...
int InstrumentClip::setNonAudioInstrument(Instrument*   newInstrument,
  ...
  // Maybe this function should have the ability to do something equivalent to
  // solicitParamManager(), for the purpose of getting bend ranges from other
  // Clips with same Instrument? Though it's an obscure requirement that's
  // probably hardly needed.
  ...
  if (newInstrument->type == INSTRUMENT_TYPE_MIDI_OUT) {
    ...
    ModelStackWithModControllable* modelStack =
        setupModelStackWithModControllable(modelStackMemory,
                                           song,
                                           this,
                                           newInstrument->toModControllable());
...
int InstrumentClip::setInstrument(Instrument*     newInstrument,
  ...
  // If MIDI or CV...
  if (newInstrument->type == INSTRUMENT_TYPE_MIDI_OUT ||
      newInstrument->type == INSTRUMENT_TYPE_CV) {
    return setNonAudioInstrument(newInstrument, song, newParamManager);
  }

  // Or if Synth or Kit...
  else {
    return setAudioInstrument(newInstrument,
                              song,
                              false,
                              newParamManager,
                              favourClipForCloningParamManager); // Tell it not to setup patching -
                                                                 // this will happen back here in
                                                                 // changeInstrumentPreset() after
                                                                 // all Drums matched up
  }
}

void InstrumentClip::prepareToEnterKitMode(Song* song) {
  // Make sure all rows on screen have a NoteRow. Any RAM problems and we'll
  // just quit
...
// Returns error code in theory - but in reality we're screwed if we get to that
// stage. newParamManager is optional - normally it's not supplied, and will be
// searched for
int InstrumentClip::changeInstrument(ModelStackWithTimelineCounter* modelStack,
  ...
  if (newInstrument->type == INSTRUMENT_TYPE_KIT) {
    ...
    if (output->type != INSTRUMENT_TYPE_KIT) {
      ...
      // Makes sure all NoteRows onscreen are populated, and deletes any empty
      // NoteRows not onscreen.
      prepareToEnterKitMode(modelStack->song);

      shouldBackUpExpressionParamsToo = true; // If switching from non-Kit to Kit, expression params
                                              // won't get used, so store them with the backup in
                                              // case the old MelodicInstrument gets used again
                                              // later. Actually is this ideal?
    }
  }
   ...
   * -- Save ParamManagers to a quick list to properly back up later. And if
   * we're deleting the Instrument, don't even end up doing that
   */

  if (isActiveOnOutput() && playbackHandler.isEitherClockActive())
    expectNoFurtherTicks(modelStack->song); // Still necessary? Probably.

  detachFromOutput(modelStack,
                   true,
                   (newInstrument->type == INSTRUMENT_TYPE_KIT),
                   false,
                   keepNoteRowsWithMIDIInput,
                   giveMidiAssignmentsToNewInstrument,
                   shouldBackUpExpressionParamsToo); // Will unassignAllNoteRowsFromDrums(),
                                                     // and remember Drum names

  int error = setInstrument(newInstrument,
                            modelStack->song,
                            newParamManager,
                            favourClipForCloningParamManager); // Tell it not to setup patching -
                                                               // this will happen back here in
                                                               // changeInstrumentPreset() after all
                                                               // Drums matched up
  if (error) {
  ...
  // If Clip (now) has a ParamManager (i.e. is not a CV Clip (wait, not
  // anymore?)), set its pos now. Don't do it for NoteRows yet - that happens as
  // Drums are set, below
  ...
  // If newInstrument has no activeClip, we must set that right now before the
  // audio routine is called - otherwise it won't be able to find its
  // ParamManager. This prevents a crash if we just navigated this Clip into
  // this Instrument and it already existed and had no Clips
  if (!newInstrument->activeClip) { newInstrument->setActiveClip(modelStack, false); }
  ...
  if (newInstrument->type == INSTRUMENT_TYPE_KIT) {
    ...
    for (int i = 0; i < noteRows.getNumElements(); i++) {
      ...
      for (DrumName* oldDrumName = thisNoteRow->firstOldDrumName; oldDrumName;
    ...
    int numNoteRowsDeletedFromBottom =
        (oldInstrument->type == INSTRUMENT_TYPE_KIT) ? oldYScroll - yScroll : 0;

    assignDrumsToNoteRows(modelStack,
                          true,
                          numNoteRowsDeletedFromBottom); // If any unassigned Drums, give them to
                                                         // any NoteRows without a Drum - or
                                                         // create them a new NoteRow. Sets up
                                                         // patching

    // If changing from a kit to a kit, we may have ended up with 0 NoteRows. We
    // do need to keep at least 1
    if (!noteRows.getNumElements()) { noteRows.insertNoteRowAtIndex(0); }
  }

  // Or if now a MelodicInstrument...
  else {

    // If the MelodicInstrument has an input MIDIDevice with bend range(s),
    // we'll often want to grab those. The same logic can be found in
    // View::noteOnReceivedForMidiLearn().
    ...
    if (midiInput->containsSomething() && midiInput->device) {
      ...
      if (zone >= 0) {
        ...
        newBendRanges[BEND_RANGE_FINGER_LEVEL] =
            device->mpeZoneBendRanges[zone][BEND_RANGE_FINGER_LEVEL];
  ...
  // Dispose of old Instrument down here, now that we can breathe (we've done
  // all the stuff above quickly because we couldn't call the audio routine
  // during it).
...
void InstrumentClip::deleteEmptyNoteRowsAtEitherEnd(bool                           onlyIfNoDrum,
                                                    ModelStackWithTimelineCounter* modelStack,
                                                    bool                           mustKeepLastOne,
  ...
  for (int i = noteRows.getNumElements() - 1; i >= mustKeepLastOne; i--) {
    ...
    if (!possiblyDeleteEmptyNoteRow(thisNoteRow,
                                    onlyIfNoDrum,
                                    modelStack->song,
                                    false,
                                    keepOnesWithMIDIInput))
      break;
...
bool InstrumentClip::possiblyDeleteEmptyNoteRow(NoteRow* noteRow,
                                                bool     onlyIfNoDrum,
                                                Song*    song,
  ...
  // If it has a drum, our work might be done, depending on what the caller
  // wanted
  if (drum) {
    if (onlyIfNoDrum) return false;

    if (onlyIfNonNumeric && drum->type == DRUM_TYPE_SOUND &&
        stringIsNumericChars(((SoundDrum*)drum)->name.get()))
      return false;
...
// Before calling this, you must ensure that each Drum's temp value represents
// whether it has a NoteRow assigned
void InstrumentClip::assignDrumsToNoteRows(ModelStackWithTimelineCounter* modelStack,
  ...
  // If at least one NoteRow already did have a Drum, then we want to put the
  // first unassigned drums (up til the first assigned one) and their new
  // NoteRows at the bottom of the screen
  if (firstNoteRowToHaveADrum >= 0) {

    // If first NoteRow already had a Drum, we can insert as many new ones below
    // it as we want
    if (firstNoteRowToHaveADrum == 0) maxNumNoteRowsToInsertAtBottom = 2147483647;

    // Otherwise, only allow enough new ones to be inserted that, combined with
    // the drum-less ones at the bottom, it'll take us up to the drum in
    // question
    else {
      maxNumNoteRowsToInsertAtBottom =
          kit->getDrumIndex(lowestDrumOnscreen) - firstNoteRowToHaveADrum;
    }

  insertSomeAtBottom:
    int numNoteRowsInsertedAtBottom = 0;

    while (nextPotentiallyUnassignedDrum &&
  ...
  // For any NoteRow without a Drum assigned, give it an unused Drum if there is
  // one
  for (int i = 0; i < noteRows.getNumElements(); i++) {
    NoteRow* thisNoteRow = noteRows.getElement(i);
    if (!thisNoteRow->drum) {
      ...
      if (shouldGiveMIDICommandsToDrums) { thisNoteRow->giveMidiCommandsToDrum(); }
    }
  }

  // If any NoteRows with no Drum remain (which means more NoteRows than Drums),
  // then delete them if they're at the end of the list and are empty (but not
  // if it's the last one left)
  if (anyNoteRowsRemainingWithoutDrum) {
    deleteEmptyNoteRowsAtEitherEnd(true, modelStack);
  }

  // Or, if all NoteRows which exist (possibly none) have a Drum, we'd better
  // check if there are any Drums with no NoteRow, and make them one
  else {

    for (; nextPotentiallyUnassignedDrum;
         nextPotentiallyUnassignedDrum = nextPotentiallyUnassignedDrum->next) {
...
void InstrumentClip::unassignAllNoteRowsFromDrums(ModelStackWithTimelineCounter* modelStack,
  ...
  for (int i = 0; i < noteRows.getNumElements(); i++) {
    ...
    if (thisNoteRow->drum) {
      ...
      // If we're retaining links to Sounds, like if we're undo-ably "deleting"
      // a Clip, just backup (and remove link to) the paramManager
      if (shouldRetainLinksToSounds) {
        if (thisNoteRow->paramManager.containsAnyMainParamCollections()) {
          modelStack->song->backUpParamManager((SoundDrum*)thisNoteRow->drum,
                                               this,
                                               &thisNoteRow->paramManager,
      ...
      // Or, the more normal thing...
        ...
        int                    noteRowId = getNoteRowId(thisNoteRow, i);
        ModelStackWithNoteRow* modelStackWithNoteRow =
            modelStack->addNoteRow(noteRowId, thisNoteRow);
...
int InstrumentClip::undoUnassignmentOfAllNoteRowsFromDrums(
    ModelStackWithTimelineCounter* modelStack) {
  for (int i = 0; i < noteRows.getNumElements(); i++) {
    NoteRow* noteRow = noteRows.getElement(i);
    if (noteRow->drum && noteRow->drum->type == DRUM_TYPE_SOUND) {
      ...
      bool success =
          modelStack->song->getBackedUpParamManagerPreferablyWithClip((SoundDrum*)noteRow->drum,
                                                                      this,
...
// Do *not* use this function to set it to NULL if you don't want to completely
// delete the old one I should make this "steal".
...
void InstrumentClip::restoreBackedUpParamManagerMIDI(ModelStackWithModControllable* modelStack) {
  ...
  char                           modelStackMemory[MODEL_STACK_MAX_SIZE];
  ModelStackWithThreeMainThings* modelStackWithThreeMainThings =
      modelStack->addParamManager(&paramManager);

  paramManager.trimToLength(loopLength,
                            modelStackWithThreeMainThings,
                            NULL,
...
void InstrumentClip::detachFromOutput(ModelStackWithTimelineCounter* modelStack,
                                      bool                           shouldRememberDrumNames,
                                      bool shouldDeleteEmptyNoteRowsAtEitherEnd,
                                      bool shouldRetainLinksToOutput,
                                      bool keepNoteRowsWithMIDIInput,
                                      bool shouldGrabMidiCommands,
                                      bool shouldBackUpExpressionParamsToo) {

  if (isActiveOnOutput()) { output->detachActiveClip(modelStack->song); }

  if (output->type == INSTRUMENT_TYPE_MIDI_OUT) {
    if (paramManager.containsAnyMainParamCollections()) { // Wouldn't this always be? Or
                                                          // is there some case where we
                                                          // might be calling this just
                                                          // after it's been created,
                                                          // and no paramManager yet?
    ...
    if (output->type == INSTRUMENT_TYPE_KIT) {
      ...
      if (shouldDeleteEmptyNoteRowsAtEitherEnd) { // Only true when called from
                                                  // changeInstrument()
        deleteEmptyNoteRowsAtEitherEnd(
            false,
            modelStack,
            false,
            keepNoteRowsWithMIDIInput); // Might call audio routine (?). Will
                                        // back up ParamManagers for any
                                        // NoteRows deleted with Drums
        // That does not enforce keeping the last NoteRow. This is ok because we
        // know if we're here that we're remaining a Kit
      }
      unassignAllNoteRowsFromDrums(modelStack,
                                   shouldRememberDrumNames,
                                   shouldRetainLinksToOutput,
                                   shouldGrabMidiCommands,
                                   shouldBackUpExpressionParamsToo);
    }

    modelStack->song->backUpParamManager((ModControllableAudio*)output->toModControllable(),
                                         this,
                                         &paramManager,
                                         shouldBackUpExpressionParamsToo);
  }
...
int InstrumentClip::setAudioInstrument(Instrument*     newInstrument,
                                       Song*           song,
                                       bool            shouldSetupPatching,
                                       ParamManager*   newParamManager,
                                       InstrumentClip* favourClipForCloningParamManager) {

  output = newInstrument;
  affectEntire =
      (newInstrument->type != INSTRUMENT_TYPE_KIT); // Moved here from changeInstrument, March 2021

  int error = solicitParamManager(song, newParamManager, favourClipForCloningParamManager);
  if (error) return error;

  // Arp stuff, so long as not a Kit (but remember, Sound/Synth is the only
  // other option in this function)
  if (newInstrument->type == INSTRUMENT_TYPE_SYNTH) {
  ...
  if (shouldSetupPatching) {
    char                           modelStackMemory[MODEL_STACK_MAX_SIZE];
    ModelStackWithTimelineCounter* modelStack =
        setupModelStackWithTimelineCounter(modelStackMemory, song, this);
...
void InstrumentClip::writeDataToFile(Song* song) {
  ...
  if (output->type == INSTRUMENT_TYPE_MIDI_OUT) {
    ...
    if (((MIDIInstrument*)instrument)->channelSuffix != -1) {
      storageManager.writeAttribute("midiChannelSuffix",
                                    ((MIDIInstrument*)instrument)->channelSuffix);
    }

    // MIDI PGM
    if (midiBank != 128) { storageManager.writeAttribute("midiBank", midiBank); }
    if (midiSub != 128) { storageManager.writeAttribute("midiSub", midiSub); }
    if (midiPGM != 128) { storageManager.writeAttribute("midiPGM", midiPGM); }
  } else if (output->type == INSTRUMENT_TYPE_CV) {
  ...
  if (output->type != INSTRUMENT_TYPE_KIT) {
    ...
    if (expressionParams) {
      ...
      if (output->type != INSTRUMENT_TYPE_MIDI_OUT) {
        ...
        storageManager.writeTag("bendRangeMPE",
                                expressionParams->bendRanges[BEND_RANGE_FINGER_LEVEL]);
      }
    }
  }
...
int InstrumentClip::readFromFile(Song* song) {
  ...
  someError:
    // Clear out all NoteRows of phony info stored where their drum pointer
    // would normally go
  ...
  while (*(tagName = storageManager.readNextTagOrAttributeName())) {
    ...
    if (!strcmp(tagName, "inKeyMode")) {
      ...
      // storageManager.createNewNonAudioInstrument(INSTRUMENT_TYPE_MIDI_OUT, 0,
      // -1);
      ...
      // if (!instrument) instrument =
      // storageManager.createNewNonAudioInstrument(INSTRUMENT_TYPE_CV, 0, -1);
      instrumentPresetSlot = storageManager.readTagOrAttributeValueInt();
    }
      ...
      affectEntire = storageManager.readTagOrAttributeValueInt();
                     ...
                     "soundMidiCommand")) { // Only for pre V2.0 song files
      soundMidiCommand.readChannelFromFile();
    }

    else if (!strcmp(tagName,
                     "modKnobs")) { // Pre V2.0 only - for compatibility

      instrumentTypeWhileLoading = INSTRUMENT_TYPE_MIDI_OUT;

      output = song->getInstrumentFromPresetSlot(INSTRUMENT_TYPE_MIDI_OUT,
                                                 ...
                                                 false);
      if (!output) {
        output = storageManager.createNewNonAudioInstrument(INSTRUMENT_TYPE_MIDI_OUT,
                                                            instrumentPresetSlot,
                                                            instrumentPresetSubSlot);
      ...
      error = ((MIDIInstrument*)output)
                  ->readModKnobAssignmentsFromFile(readAutomationUpToPos, &paramManager);
      if (error) return error;
    ...
    // For song files from before V2.0, where Instruments were stored within the
    // Clip. Loading Instrument from another Clip.
      ...
      if (!output) {
        {
          void* instrumentMemory =
              generalMemoryAllocator.alloc(sizeof(SoundInstrument), NULL, false, true);
    ...
    // Clip
      ...
      // Slight hack to fix crash with late-2016-ish songs
        ...
        ParamManager* otherParamManager = song->getBackedUpParamManagerPreferablyWithClip(
            (ModControllableAudio*)output->toModControllable(),
            this);
    ...
    // These next 3 - only created by alpha testers for a few weeks. Could
    // eventually remove.
    ...
    doReadExpressionParam:
      ...
      if (expressionParams) { expressionParams->readParam(summary, temp, readAutomationUpToPos); }
    }
      ...
      ParamCollectionSummary* summary          = paramManager.getExpressionParamSetSummary();
      ExpressionParamSet*     expressionParams = (ExpressionParamSet*)summary->paramCollection;
      if (expressionParams) { expressionParams->readFromFile(summary, readAutomationUpToPos); }
    }
  ...
  // Some stuff for song files before V2.0, where the Instrument would have been
  // loaded at this point

  // For song files from before V2.0, where Instruments were stored within the
  // Clip (which was called a Track back then)
  if (output) {
    if (!instrumentWasLoadedByReferenceFromClip) {
      switch (output->type) {
        case INSTRUMENT_TYPE_MIDI_OUT:
          ((MIDIInstrument*)output)->channelSuffix =
              getMin(25, getMax(-1, instrumentPresetSubSlot));
          // No break

        case INSTRUMENT_TYPE_CV:
          ((NonAudioInstrument*)output)->channel =
              getMin(numInstrumentSlots, getMax(0, instrumentPresetSlot));
          break;
    ...
    if (output->type != INSTRUMENT_TYPE_MIDI_OUT && output->type != INSTRUMENT_TYPE_CV) {
      ...
      if (!paramManager.containsAnyMainParamCollections()) {
        ...
        bool             success =
            song->getBackedUpParamManagerPreferablyWithClip((ModControllableAudio*)modControllable,
                                                            this,
                                                            &paramManager);
        if (success) {
          char                           modelStackMemory[MODEL_STACK_MAX_SIZE];
          ModelStackWithThreeMainThings* modelStackWithThreeMainThings =
              setupModelStackWithThreeMainThingsButNoNoteRow(modelStackMemory,
                                                             song,
                                                             modControllable,
                                                             this,
                                                             &paramManager);
        ...
        // If there wasn't one, that's because another Clip already took it.
        // Clone it from that Clip.
        else {

          // It can happen that a ParamManager was never created for a Kit (pre
          // V2.0, or perhaps only in 1.0?). Just create one now.
          if (!instrumentWasLoadedByReferenceFromClip && output->type == INSTRUMENT_TYPE_KIT) {
            ...
            error = paramManager.cloneParamCollectionsFrom(
                &instrumentWasLoadedByReferenceFromClip->paramManager,
                false); // No need to trim - param automation didn't exist back
                        // then
            if (error) goto someError;
          }
        }
  ...
  // Pre V3.2.0 (and also for some of 3.2's alpha phase), bend range wasn't
  // adjustable, wasn't written in the file, and was always 12.
  if (storageManager.firmwareVersionOfFileBeingRead <= FIRMWARE_3P2P0_ALPHA &&
      !paramManager.getExpressionParamSet()) {
    ExpressionParamSet* expressionParams = paramManager.getOrCreateExpressionParamSet();
    if (expressionParams) { expressionParams->bendRanges[BEND_RANGE_MAIN] = 12; }
  }
  ...
  switch (instrumentTypeWhileLoading) {
    ...
    case INSTRUMENT_TYPE_KIT:
      ...
      if (dirPathHasBeenSpecified) {
        ...
        // Where dir path has not been specified (i.e. before V4.0.0), go with the
        // default. The same has been done to the Instruments which this Clip will
        // get matched against.
...
int InstrumentClip::readMIDIParamsFromFile(int32_t readAutomationUpToPos) {
  ...
  while (*(tagName = storageManager.readNextTagOrAttributeName())) {
    if (!strcmp(tagName, "param")) {
      // int error = storageManager.readMIDIParamFromFile(readAutomationUpToPos,
      // this); if (error) return error;
      ...
      while (*(tagName = storageManager.readNextTagOrAttributeName())) {
        if (!strcmp(tagName, "cc")) {
          ...
          if (!strcasecmp(contents, "bend")) {
            paramId = 0;
          expressionParam:
            // If we're here, we're reading a pre-V3.2 file, and need to read
            // what we're now regarding as "expression".
            ...
            goto expressionParam;
                     ...
                     !strcmp(contents,
                             "120")) { // We used to write 120 for "none", pre V2.0
            ...
            if (expressionParams) {
              if (param->isAutomated())
                expressionParams->paramHasAutomationNow(
                    summary,
                    paramId); // Most other times you don't have to think about
                              // calling this. It's just because we didn't know
                              // which ParamCollection we were gonna load into,
                              // and MIDIParamCollection doesn't keep track of
                              // automation.

              // If channel pressure, gotta move and scale the values from how
              // they were in the pre-V3.2 firmware
              if (paramId) {
                param->transposeCCValuesToChannelPressureValues();
              }

              // Or if pitch bend, it'll no longer interpolate, so go place some
              // new nodes. Actually even without this step, you can only just
              // tell there's any problem.
              else {
                param->makeInterpolationGoodAgain(loopLength,
                                                  22); // 22 is picked somewhat arbitrarily - see
                                                       // comment for function itself.
              }
            }
          }
...
// This function also unassigns individual NoteRows from their "sound" MIDI
// commands
void InstrumentClip::prepNoteRowsForExitingKitMode(Song* song) {
  ...
  if (!chosenNoteRow) {
    ...
    for (int i = 0; i < noteRows.getNumElements(); i++) {
      ...
      if (thisNoteRow->y != -32768) {
        ...
        // But, if we're in key-mode, make sure this yNote fits within the
        // scale!
        if (inScaleMode) {
  ...
  // Occasionally we get a crazy scroll value. Not sure how. It happened to Jon
  // Hutton
  if (chosenNoteRow) {
    if (chosenNoteRow->y < -256 || chosenNoteRow->y >= 256)
      goto useRootNote; // Can't use isScrollWithinRange, cos that relies on
                        // existing note positions, which are messed up
  }
...
bool InstrumentClip::deleteSoundsWhichWontSound(Song* song) {
  ...
  if (output->type == INSTRUMENT_TYPE_KIT) {
    ...
    for (int i = 0; i < noteRows.getNumElements();) {
      ...
      if ((!clipIsActive || noteRow->muted || noteRow->hasNoNotes())
        ...
        // OI!! Don't nest any of those conditions inside other if statements.
        // We need the "else" below to take effect. Thanks

        // We'd ultimately love to just delete the Drum. But beware that
        // multiple NoteRows in different Clips may have the same Drum. We used
        // to just delete it, leading to a crash sometimes! Now, if we just do
        // this for the active Clip, it should be ok right, cos no other Clip is
        // going to be doing anything on its NoteRow?
  ...
  // For MelodicInstruments, we can delete the Clip (which we know is active on
  // the Instrument) if the Clip is inactive in the Song and the Instrument
  // isn't still rendering anything
  else {
...
void InstrumentClip::stopAllNotesForMIDIOrCV(ModelStackWithTimelineCounter* modelStack) {
  ...
  // This didn't use to be here but seems a good idea. Call this so that any
  // drone notes will restart next loop around. Also, not all synths necessarily
  // support the all-notes-off message, which the further below sends.
  stopAllNotesPlaying(modelStack);

  // And then we still need this but in case any notes have been sent out via
  // audition, or I guess being echoed thru
...
uint32_t InstrumentClip::getWrapEditLevel() {
  return wrapEditing ? wrapEditLevel : MAX_SEQUENCE_LENGTH; // Used to return the Clip length in
                                                            // this case, but that causes problems
                                                            // now that NoteRows may be longer.
}
...
// modelStack could contain a NULL noteRow if there isn't one - e.g. in a Synth
// Clip
bool InstrumentClip::allowNoteTails(ModelStackWithNoteRow* modelStack) {
  ...
  return noteRow->drum->allowNoteTails(
      modelStackWithSoundFlags); // Needs to survive a NULL noteRow, even if
                                 // this generally wouldn't happen (it might if
                                 // auditioning a Drum via MIDI or arranger
                                 // audition pad which doesn't have one)
}
...
// For the purpose of deciding a region length for parameter automation manual
// editing
...
NoteRow* InstrumentClip::getNoteRowFromId(int id) {
  if (output->type == INSTRUMENT_TYPE_KIT) {
    ...
    if (!noteRow) { noteRow = noteRows.insertNoteRowAtY(id); }
    return noteRow;
  }
}
...
void InstrumentClip::shiftOnlyOneNoteRowHorizontally(ModelStackWithNoteRow* modelStack,
  ...
  if (playbackHandler.isEitherClockActive() && modelStack->song->isClipActive(this)) {
    ...
    if (noteRow->paramManager.mightContainAutomation()) {
      ...
      noteRow->paramManager.setPlayPos(getLivePos(),
                                       modelStackWithThreeMainThings,
                                       modelStackWithThreeMainThings->isCurrentlyPlayingReversed());
    }
  }
...
void InstrumentClip::sendMIDIPGM() {
  ...
  if (midiBank != 128) { midiEngine.sendBank(masterChannel, midiBank, outputFilter); }
  if (midiSub != 128) { midiEngine.sendSubBank(masterChannel, midiSub, outputFilter); }
  if (midiPGM != 128) { midiEngine.sendPGMChange(masterChannel, midiPGM, outputFilter); }
}
...
void InstrumentClip::clearArea(ModelStackWithTimelineCounter* modelStack,
  ...
  for (int i = 0; i < noteRows.getNumElements(); i++) {
    ...
    thisNoteRow->clearArea(startPos,
                           endPos - startPos,
                           modelStackWithNoteRow,
                           action,
...
// Called when the user presses one of the instrument-type buttons
// (synth/kit/MIDI/CV). This function takes care of deciding what Instrument /
// preset to switch to.
Instrument* InstrumentClip::changeInstrumentType(ModelStackWithTimelineCounter* modelStack,
  ...
  bool canReplaceWholeInstrument =
      modelStack->song->canOldOutputBeReplaced(this, &availabilityRequirement);
  ...
  if (newInstrumentType == INSTRUMENT_TYPE_MIDI_OUT || newInstrumentType == INSTRUMENT_TYPE_CV) {
    newInstrument = modelStack->song->getNonAudioInstrumentToSwitchTo(newInstrumentType,
  ...
  // Synth / Kit
    ...
    result = Browser::confirmPresetOrNextUnlaunchedOne(newInstrumentType,
                                                       &newName,
                                                       availabilityRequirement);
    ...
    if (!newInstrument) {
      ...
      result.error = storageManager.loadInstrumentFromFile(modelStack->song,
                                                           NULL,
                                                           newInstrumentType,
                                                           false,
                                                           &newInstrument,
                                                           &result.fileItem->filePointer,
                                                           &newPresetName,
                                                           &Browser::currentDir);
    }
  ...
  if (shouldReplaceWholeInstrument) {
    ...
    int error = changeInstrument(modelStack,
                                 newInstrument,
                                 NULL,
  ...
  if (newInstrumentType == INSTRUMENT_TYPE_KIT) {
    // Make sure we're not scrolled too far up (this has to happen amongst this
    // code down here - NoteRows are deleted in the functions called above)
...
int InstrumentClip::claimOutput(ModelStackWithTimelineCounter* modelStack) {
  ...
  if (!output) { // Would only have an output already if file from before V2.0.0
                 // I think? So, this block normally does apply.
    int instrumentType = instrumentTypeWhileLoading;

    char const* instrumentName =
        (instrumentType < 2) ? backedUpInstrumentName[instrumentType].get() : NULL;
    char const* dirPath =
        (instrumentType < 2) ? backedUpInstrumentDirPath[instrumentType].get() : NULL;

    output =
        modelStack->song->getInstrumentFromPresetSlot(instrumentType,
    ...
    if (!output) {
      if (instrumentType == INSTRUMENT_TYPE_MIDI_OUT || instrumentType == INSTRUMENT_TYPE_CV) {
        output =
            storageManager.createNewNonAudioInstrument(instrumentType,
                                                       backedUpInstrumentSlot[instrumentType],
                                                       backedUpInstrumentSubSlot[instrumentType]);

        if (!output) { return ERROR_INSUFFICIENT_RAM; }
  ...
  if (output->type == INSTRUMENT_TYPE_KIT) {
    ...
    for (int i = 0; i < noteRows.getNumElements(); i++) {
      ...
      // Otherwise, we know we've sneakily put an integer index in place of the
      // pointer, so convert that back to an actual pointer now
      else {

        // Don't call setDrum(), because that would overwrite the NoteRow's
        // paramManager. It already has the right one, loaded from file
        Drum* drumFromIndex = kit->getDrumFromIndex((uint32_t)thisNoteRow->drum);

        ParamManagerForTimeline* otherParamManager;

        ModelStackWithNoteRow* modelStackWithNoteRow; // Defined up here so we can do the jump to
                                                      // haveNoDrum, below.

        int  error; // These declared here, to allow gotos
        bool success;

        // We need to see whether any other NoteRows *that we've assigned drums
        // so far* had this same drum. TODO: this could be waaaay more
        // efficient!
        for (int j = 0; j < i; j++) {
        ...
        // If we didn't get a paramManager (means pre-September-2016 song).
        // TODO: this whole section would lead to an ugly mess if the right
        // stuff wasn't in the file. Or if not enough RAM
        if (!thisNoteRow->paramManager.containsAnyMainParamCollections() &&
            thisNoteRow->drum->type == DRUM_TYPE_SOUND) {
          ...
          // If there wasn't one there, it means another Clip's NoteRow already
          // claimed it
          else {
            otherParamManager =
                modelStackWithNoteRow->song->findParamManagerForDrum(kit, thisNoteRow->drum, this);
            if (!otherParamManager) { return ERROR_UNSPECIFIED; }
              ...
              for (int s = 0; s < NUM_SOURCES; s++) {
                ...
                if (source->oscType == OSC_TYPE_SAMPLE) {
                  if (sound->transpose || source->transpose || source->cents ||
                      ...
                      //||
                      // thisNoteRow->paramManager->patchCableSet.doesParamHaveSomethingPatchedToIt(PARAM_LOCAL_PITCH_ADJUST)
                      //// No, can't call these cos patching isn't set up yet.
                      // Oh well
                      //||
                      // thisNoteRow->paramManager->patchCableSet.doesParamHaveSomethingPatchedToIt(PARAM_LOCAL_OSC_A_PITCH_ADJUST
                      //+ s)
                      ||
  ...
  // Otherwise, if not a Kit...
    ...
    // If we had a MIDI input channel for this Clip, as was the format pre V2.0,
    // move this to the Instrument
    ...
    // Occasionally we get a song file with a crazy scroll value. Not sure how.
    // It happened to Tia
    if (!isScrollWithinRange(0, yScroll)) { yScroll = 60; }
  }

  // Now the Instrument (and all Drums) are matched up, we can do the resonance
  // compensation crap.
  ...
  if (storageManager.firmwareVersionOfFileBeingRead < FIRMWARE_2P1P0_BETA) {
    ...
    // For songs saved before V2.0, ensure that non-square oscillators have PW
    // set to 0 (cos PW in this case didn't have an effect then but it will now)
      ...
      if (output->type == INSTRUMENT_TYPE_SYNTH) {
        ...
        for (int s = 0; s < NUM_SOURCES; s++) {
          if (sound->sources[s].oscType != OSC_TYPE_SQUARE) {
            ...
            patchedParams->params[PARAM_LOCAL_OSC_A_PHASE_WIDTH + s].setCurrentValueBasicForSetup(
                0);
...
void InstrumentClip::finishLinearRecording(ModelStackWithTimelineCounter* modelStack,
  ...
  // Notes may have been placed right at/past the end of the Clip, usually
  // because one was quantized forwards - and set to the exact end position -
  // and it wasn't yet known whether to extend the length of the Clip in case
  // the user cancelled linear recording. Trim them off, and move them to the
  // new Clip if there is one.
  for (int i = 0; i < noteRows.getNumElements(); i++) {
    NoteRow* thisNoteRow = noteRows.getElement(i);
    ...
    while (thisNoteRow->notes.getNumElements()) { // There's most likely only one offender,
                                                  // but you never really know
      Note* lastNote = thisNoteRow->notes.getLast();

      // If Note is past new end-point that we're setting now, then delete /
      // move the Note
      if (lastNote->pos >= loopLength) {
        ...
        if (newInstrumentClip) {
          ...
          if (newNoteRow) {
            newNoteRow->attemptNoteAdd(0,
                                       lastNote->length,
                                       lastNote->velocity,
                                       lastNote->probability,
                                       modelStackWithNoteRow,
                                       NULL); // I'm guessing I deliberately didn't send the Action
                                              // in here, cos didn't want to make this Note on the
                                              // new InstrumentClip undoable?
            newNoteRow->skipNextNote = true;
          }
        }

        // Delete the Note
        thisNoteRow->deleteNoteByIndex(thisNoteRow->notes.getNumElements() - 1,
                                       NULL,
                                       getNoteRowId(thisNoteRow, i),
                                       this);
      }
      ...
      // Or if Note not past end-point...
        ...
        if (mayStillLengthen &&
          ...
          // moveToFrontIfAlreadySnapshotted = true because we need to make the
          // Consequence closer to the front than any previous Clip-lengthening
          // that took place.
  ...
  // If we did create a new Clip, we want to leave currentlyRecordingLinearly
  // true just a bit longer so that when expectNoFurtherTicks() gets called as
  // the new Clip begins playing, it knows not to switch our currently
  // sounding/auditioning notes off. Otherwise, since that won't be happening,
  // we just want to ensure that recording stops now.
  currentlyRecordingLinearly = newInstrumentClip;

  if (isUnfinishedAutoOverdub) { isUnfinishedAutoOverdub = false; }

  if (getRootUI())
    getRootUI()->clipNeedsReRendering(
        this); // Notes might have been lengthened - we'd better render it.
}
...
Clip* InstrumentClip::cloneAsNewOverdub(ModelStackWithTimelineCounter* modelStack,
  ...
  newInstrumentClip->setupForRecordingAsAutoOverdub(
      this,
      modelStack->song,
      newOverdubNature); // Hopefully fine - I've moved this to after
                         // setInstrument in March 2021, so we can override the
                         // new affectEntire default value set there.

  char                           modelStackMemoryNewClip[MODEL_STACK_MAX_SIZE];
  ModelStackWithTimelineCounter* modelStackNewClip =
      setupModelStackWithTimelineCounter(modelStackMemoryNewClip,
                                         modelStack->song,
                                         newInstrumentClip);
  ...
  if (alternativeLongerLength) {
    for (int i = 0; i < noteRows.getNumElements(); i++) {
      ...
      if (numNotes) {
        ...
        if (lastNote->pos + lastNote->length > suggestedLength) { goto useAlternativeLength; }
      }
    }
...
bool InstrumentClip::currentlyScrollableAndZoomable() {
  return !onKeyboardScreen ||
         (getRootUI() == &sessionView && containsAnyNotes()); // Cheating a bit!
}

// Call this after setInstrument() / setAudioInstrument(). I forget exactly
// where setupPatching() fits into this picture... Arranger view calls that
// before this...
...
void InstrumentClip::getActiveModControllable(ModelStackWithTimelineCounter* modelStack) {
  if (output->type == INSTRUMENT_TYPE_KIT && !affectEntire && getRootUI() != &sessionView &&
    ...
    returnNull:
      ...
      // Ensure that the selected drum in fact has a NoteRow in this Clip. It
...
ModelStackWithNoteRow*
InstrumentClip::duplicateModelStackForClipBeingRecordedFrom(ModelStackWithNoteRow* modelStack,
  ...
  ModelStackWithNoteRow* otherModelStackWithNoteRow =
      otherModelStack->automaticallyAddNoteRowFromId();
...
void InstrumentClip::recordNoteOn(ModelStackWithNoteRow* modelStack,
  ...
  if (forcePos0) {
    ...
    if (FlashStorage::recordQuantizeLevel) {
      ...
      uint16_t     quantizeInterval = baseThing
                                  << (8 + modelStack->song->insideWorldTickMagnitude +
      ...
      if (quantizedLater != reversed) { // If need to quantize forwards (to later in time)...
        ...
        // If that's quantized it right to the end of the loop-length or maybe
        // beyond...
        if (quantizedPos >= effectiveLength) {

          // If recording to arrangement, go and extend the Clip/NoteRow early,
          // to create the place where we'll put the Note.
          if (playbackHandler.recording == RECORDING_ARRANGEMENT && isArrangementOnlyClip()) {

            int error;

            // If the NoteRow has independent *length* (not just independent
            // play-pos), then it needs to be treated individually.
            if (noteRow->loopLengthIfIndependent) {
              if (output->type == INSTRUMENT_TYPE_KIT &&
                ...
                if (otherNoteRow) { // It "should" always have it...
                  ...
                  noteRow->appendNoteRow(modelStack,
                                         otherModelStackWithNoteRow,
            ...
            // Otherwise, just extend the whole Clip.
              ...
              copyModelStack(thisModelStackMemory,
                             modelStack,
          ...
          // If recording linearly...
            ...
            // Don't do anything - let the note begin at or past (?) the Clip
            // length
          } else {
      ...
      // If we're quantized later to a pingpong-point, we have to consider the
      // play-direction to have changed.
    ...
    // If we quantized later, make sure that that note doesn't get played really
    // soon when the play-pos reaches it
  ...
  // Since recording usually involves creating lots of notes overall, we'll just
  // snapshot all the notes in bulk
  Action* action = actionLogger.getNewAction(ACTION_RECORD, true);
  if (action)
    action->recordNoteArrayChangeIfNotAlreadySnapshotted(this,
                                                         modelStack->noteRowId,
                                                         &noteRow->notes,
                                                         false,
                                                         true);
  // moveToFrontIfAlreadySnapshotted = true because we need to make the
  // Consequence closer to the front than any previous Clip-lengthening that
  // took place.
  ...
  if (reversed) {
    ...
    distanceToNextNote =
        noteRow->attemptNoteAdd(quantizedPos,
                                1,
                                velocity,
                                NUM_PROBABILITY_VALUES,
                                modelStack,
  ...
  // If that didn't work, get out - but not in the special case for linear
  // recording, discussed below.
  if (!distanceToNextNote && quantizedPos < effectiveLength) return;

  // If we're doing MPE, we'll want to place a node here at the Note's start, so
  // it's got the correct stuff to sound during its note-on when we play back.
  ...
  // Or if we haven't been supplied MPE values, just check if this NoteRow
  // already has MPE data, and only if so, go and overwrite it here.
  else {
  ...
  for (int m = 0; m < NUM_EXPRESSION_DIMENSIONS; m++) {
    ...
    // Special case for MPE - in this case where we're setting the value for the
    // whole length, we still do want to place one - just one - node at pos.
    // It'll be for the start of a note.
    ...
    doHomogenize:
      ...
      param->nodes.testSequentiality(
          "E442"); // drbourbon got, when check was inside homogenizeRegion().
                   // Now trying to work out where that came from. March 2022.
#endif

      param->homogenizeRegion(modelStackWithAutoParam,
                              quantizedPos,
                              distanceToNextNote,
                              value,
                              reversed,
                              ...
                              effectiveLength,
                              reversed,
                              posAtWhichClipWillCut);
    }

    else {

      // Special case for if linear recording, quantized later, right to end of
      // effectiveLength.
      if (quantizedPos >= effectiveLength) {
    ...
    // These manual sets are in case we quantized forwards and the region we
    // just created actually begins after "now"-time.
    param->currentValue              = value;
    param->valueIncrementPerHalfTick = 0;
    // TODO: and to make it perfect, we'd also want to ignore any further nodes
    // between now and the start of the region. Or, could probably get away with
    // just deleting them.
  }
}
...
// This function looks a bit weird... probably old... should it maybe instead
// call a function on the MelodicInstrument / Kit?
  ...
  if (output->type == INSTRUMENT_TYPE_KIT) {
    ...
    if (noteRowIndex >= 0 && noteRowIndex <= noteRows.getNumElements()) {
      ...
      if (noteRow->drum) {
        ...
        noteRow->drum->lastMIDIChannelAuditioned =
            MIDI_CHANNEL_NONE; // So it won't record any more MPE
      }
    }
  }
  ...
  if (ticksTilNextNoteRowEvent <= 0) {
    ...
    for (int i = 0; i < noteRows.getNumElements(); i++) {
      ...
      if (thisNoteRow->hasIndependentPlayPos()) {
        ...
        ModelStackWithNoteRow* modelStackWithNoteRow =
            modelStack->addNoteRow(getNoteRowId(thisNoteRow, i), thisNoteRow);
        if (modelStackWithNoteRow->isCurrentlyPlayingReversed()) { movement = -movement; }
