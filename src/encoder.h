...
public:
  ...
  int8_t encPos;    // Keeps track of knob's position relative to centre of closest
                    // detent
  int8_t detentPos; // Number of full detents offset since functions last dealt
                    // with
private:
  uint8_t portA;
  uint8_t pinA;
  ...
  bool    pinALastRead;
  bool    pinBLastRead;
  int8_t  encLastChange; // The "change" applied on the most recently detected
                         // action on this knob. Probably 1 or -1.
  bool doDetents;
  bool valuesNow[2];
};
