...
void NumericDriver::setText(char const* newText,
                            bool        alignRight,
                            uint8_t     drawDot,
                            bool        doBlink,
                            uint8_t*    newBlinkMask,
                            bool        blinkImmediately,
                            bool        shouldBlinkFast,
                            int         scrollPos,
                            uint8_t*    encodedAddition,
...
NumericDriver::setScrollingText(char const* newText, int startAtTextPos, int initialDelay) {
...
void NumericDriver::transitionToNewLayer(NumericLayer* newLayer) {
  ...
  if (!popupActive && nextTransitionDirection != 0 && topLayer != NULL) {
    ...
    if (layerSpace) {
      ...
      scrollTransition->transitionProgress =
          -NUMERIC_DISPLAY_LENGTH * scrollTransition->transitionDirection;
...
int NumericDriver::getEncodedPosFromLeft(int textPos, char const* text, bool* andAHalf) {
  ...
  int  encodedPos        = 0;
  bool lastSegmentHasDot = true; // Pretend this initially, cos the segment before the first one
  ...
  *andAHalf = false;
  ...
  for (int i = 0; true; i++) {
    ...
    // If dot here, and we haven't just had a dot previously, then this dot just
    // gets crammed into prev encoded char
...
int NumericDriver::encodeText(char const* newText,
                              uint8_t*    destination,
                              bool        alignRight,
                              uint8_t     drawDot,
  ...
  while (true) {
    ...
    // First, check if it's a dot, which we might want to add to a previous
    // position
    bool isDot = (thisChar == '.' || thisChar == '#' || thisChar == ',');
    ...
    if (isDot) {
      if (alignRight) {
        ...
        // If we're not the first character, and the previous character didn't
        // already have its dot illuminated, we'll just illuminate it
      ...
      if (alignRight && carryingDot) {
        ...
        // If this char already has some form of dot, we need to instead insert
        // the carried dot to its right
  ...
  if (limitToDisplayLength) {
    ...
    if (drawDot < NUMERIC_DISPLAY_LENGTH) {
      ...
      for (int i = 0; i < NUMERIC_DISPLAY_LENGTH; i++) {
        if ((drawDot >> (NUMERIC_DISPLAY_LENGTH - 1 - i)) & 1) { destination[i] |= 128; }
      }
    }
  }
  return writePos;
...
void NumericDriver::setTextAsSlot(int16_t currentSlot,
  ...
  if (blinkPos == -1) {
    ...
    blinkPos++; // Move an extra space left if we have a sub-slot / letter
                // suffix
...
void NumericDriver::displayPopup(char const* newText,
                                 int8_t      numFlashes,
                                 bool        alignRight,
                                 uint8_t     drawDot,
                                 int         blinkSpeed) {
...
extern "C" void displayPopupIfAllBootedUp(char const* text) {
  if (usbInitializationPeriodComplete) { numericDriver.displayPopup(text); }
}
