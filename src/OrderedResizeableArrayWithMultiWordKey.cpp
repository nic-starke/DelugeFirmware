...
OrderedResizeableArrayWithMultiWordKey::OrderedResizeableArrayWithMultiWordKey(int newElementSize,
                                                                               int newNumWordsInKey)
    : OrderedResizeableArrayWith32bitKey(newElementSize, 16, 15), numWordsInKey(newNumWordsInKey) {}

int OrderedResizeableArrayWithMultiWordKey::searchMultiWord(uint32_t* __restrict__ keyWords,
                                                            int comparison,
                                                            int rangeBegin,
                                                            int rangeEnd) {
...
int OrderedResizeableArrayWithMultiWordKey::insertAtKeyMultiWord(uint32_t* __restrict__ keyWords,
                                                                 int rangeBegin,
                                                                 int rangeEnd) {
  ...
  for (int w = 0; w < numWordsInKey; w++) { wordsHere[w] = keyWords[w]; }
