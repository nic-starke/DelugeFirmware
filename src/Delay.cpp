...
void Delay::informWhetherActive(bool newActive, int32_t userDelayRate) {
  ...
  // Or if no change...
    ...
    if (previouslyActive) {
      // If no writing has happened yet to this Delay, check that the buffer is
      // the right size. The delay time might have changed since it was set up,
      // and we could be better off making a new buffer, which is easily done
      // now, before anything's been written
...
void Delay::prepareToBeginWriting() {
  sizeLeftUntilBufferSwap = getAmountToWriteBeforeReadingBegins(); // If you change this, make sure
                                                                   // you
}
...
void Delay::setupWorkingState(DelayWorkingState* workingState, bool anySoundComingIn) {
  ...
  bool mightDoDelay =
      (workingState->delayFeedbackAmount >= 256 &&
       (anySoundComingIn || repeatsUntilAbandon)); // TODO: we want to be able to reduce the 256 to
                                                   // 1, but for some reason, the patching engine
                                                   // spits out 112 even when this should be 0...
  ...
  if (mightDoDelay) {
    ...
    if (sync != 0) {
      ...
      workingState->userDelayRate =
          multiply_32x32_rshift32_rounded(workingState->userDelayRate,
                                          playbackHandler.getTimePerInternalTickInverse(true));
  ...
  informWhetherActive(mightDoDelay,
                      workingState->userDelayRate); // Tell it to allocate memory if that hasn't
                                                    // already happened
  workingState->doDelay = isActive();               // Check that ram actually is allocated

  if (workingState->doDelay) {
    // If feedback has changed, or sound is coming in, reassess how long to
    // leave the delay sounding for
