...
public:
  ...
  void recordParamChangeIfNotAlreadySnapshotted(ModelStackWithAutoParam const* modelStack,
                                                bool                           stealData = false);
  void recordParamChangeDefinitely(ModelStackWithAutoParam const* modelStack, bool stealData);
  int  recordNoteArrayChangeIfNotAlreadySnapshotted(InstrumentClip* clip,
                                                    int             noteRowId,
                                                    NoteVector*     noteVector,
                                                    bool            stealData,
                                                    bool moveToFrontIfAlreadySnapshotted = false);
  int  recordNoteArrayChangeDefinitely(InstrumentClip* clip,
                                       int             noteRowId,
                                       NoteVector*     noteVector,
                                       bool            stealData);
  bool containsConsequenceNoteArrayChange(InstrumentClip* clip,
                                          int             noteRowId,
                                          bool            moveToFrontIfFound = false);
  void recordNoteExistenceChange(InstrumentClip* clip, int noteRowId, Note* note, int type);
  void recordNoteChange(InstrumentClip* clip,
                        int             noteRowId,
                        Note*           note,
                        int32_t         lengthAfter,
                        int             velocityAfter,
  ...
  // A bunch of snapshot-things here store their state both before or after the
  // action - because the action could have changed these
  int xScrollClip[2];
  ...
  // And a few more snapshot-things here only store one state - at the time of
  // the action, because the action could not change these things
  ...
  // We store these kinds of consequences separately because we need to be able
  // to search through them fast, when there may be a large number of other
  // kinds of consequences. Also, these don't need re-ordering each time we
  // revert
  ...
  int8_t offset; // Recorded for the purpose of knowing when we can do those
                 // "partial undos"
