...
void inputRoutine() {
  ...
  AudioEngine::renderInStereo = (AudioEngine::headphonesPluggedIn || outputPluggedInR ||
                                 AudioEngine::isAnyInternalRecordingHappening());
  ...
  if (ADC.ADCSR & (1 << 15)) {
    ...
    batteryMV = (voltageReadingLastTime) >> 15; // We only >> by 15 so that we intentionally double
                                                // the value, because the incoming voltage is halved
                                                // by a resistive divider already
    // uartPrint("batt mV: ");
...
bool readButtonsAndPads() {
  ...
  /*
  if (!inSDRoutine && !closedPeripheral && !anythingInitiallyAttachedAsUSBHost
  && AudioEngine::audioSampleTimer >= (44100 << 1)) { Uart::println("closing
  peripheral"); closeUSBPeripheral(); Uart::println("switching back to host");
      openUSBHost();
  ...
  */
  ...
  if (playbackHandler.playbackState &&
      (int32_t)(audioDriver.audioSampleTimer - timeNextSDTestAction) >= 0) {
...
void setupBlankSong() {
  void* songMemory = generalMemoryAllocator.alloc(sizeof(Song),
                                                  NULL,
                                                  false,
  ...
  setRootUILowLevel(&instrumentClipView); // Prevents crash. (Wait, I'd like to
                                          // know more about what crash...)
...
extern "C" int main2(void) {
  ...
  /*
   * For reasons not exactly known, globally declared instances of classes (so,
   * objects) will not get their constructors called automatically on boot-up as
   * is supposed to happen in C++. This will immediately cause problems, as
   * things don’t get initialized. And for classes with virtual functions (i.e.
   * using polymorphism), their vtable won’t even be set, causing an instant
   * crash as soon as any virtual function is called on them.
   *
   * This is why, right here in Deluge.cpp, every single globally declared
   * object gets manually set up with a “new” statement.
   *
   * See a more technical discussion of the problem here:
   * https://stackoverflow.com/questions/32807964/c-gcc-file-scope-objects-constructors-arent-being-called?noredirect=1#comment53452782_32807964
   */
  ...
  setOutputState(BATTERY_LED_1, BATTERY_LED_2,
                 1);                            // Switch it off (1 is off for open-drain)
...
#if HAVE_OLED
                10000000, // Higher than this would probably work... but let's stick to
                          // the OLED datasheet's spec of 100ns (10MHz).
#else
                30000000,
#endif
                0,
  ...
  // Wait for PIC Uart to flush out. Could this help Ron R with his Deluge
  // sometimes not booting? (No probably wasn't that.) Otherwise didn't seem
  // necessary.
  ...
  // Setup SPIBSC. Crucial that this only be done now once everything else is
  // running, because I've injected graphics and audio routines into the SPIBSC
  // wait routines, so that has to be running
  setPinMux(4, 2, 2);
  setPinMux(4, 3, 2);
  setPinMux(4, 4, 2);
  setPinMux(4, 5, 2);
  setPinMux(4, 6, 2);
  setPinMux(4, 7, 2);
  initSPIBSC(); // This will run the audio routine! Ideally, have external RAM
                // set up by now.
  ...
  while ((uint16_t)(*TCNT[TIMER_SYSTEM_FAST] - timeWaitBegan) <
    ...
    if (readingFirmwareVersion) {
      ...
      else { // If any hint of another button being held, don't do anything.
  ...
  // Ideally I'd like to repeatedly switch between host and peripheral mode
  // anytime there's no USB connection. To do that, I'd really need to know at
  // any point in time whether the user had just made a connection, just then,
  // that hadn't fully initialized yet. I think I sorta have that for host, but
  // not for peripheral yet.

  MIDIDeviceManager::readDevicesFromFile(); // Hopefully we can read this file
                                            // now.

  setupBlankSong(); // Can only happen after settings, which includes default
  ...
  while (1) {
    ...
    audioFileManager.slowRoutine(); // Only actually needs calling a couple of times per
                                    // second, but we can't put it in uiTimerManager cos
                                    // that gets called in card routine
...
extern "C" void routineForSD(void) {
  ...
  // We lock this to prevent multiple entry. Otherwise we could get SD ->
  // routineForSD() -> AudioEngine::routine() -> USB -> routineForSD()
...
void deleteOldSongBeforeLoadingNew() {
  ...
  AudioEngine::unassignAllVoices(true); // Need to do this now that we're not bothering getting the
                                        // old Song's Instruments detached and everything on delete
...
void spamMode() {
  ...
  while (true) {
    ...
    encoders[ENCODER_SELECT].detentPos = 0; // Reset. Crucial that this happens before we call
                                            // selectEncoderAction()
    ...
    encoders[ENCODER_SCROLL_Y].detentPos = 0; // Reset. Crucial that this happens before we call
                                              // selectEncoderAction()
    if (limitedDetentPos != 0) {
