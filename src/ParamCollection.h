...
public:
  ...
  virtual void generateRepeats(ModelStackWithParamCollection* modelStack,
                               uint32_t                       oldLength,
                               uint32_t                       newLength,
  ...
  virtual void trimToLength(uint32_t                       newLength,
                            ModelStackWithParamCollection* modelStack,
                            Action*                        action,
                            bool                           maySetupPatching)                                              = 0;
  virtual void shiftHorizontally(ModelStackWithParamCollection* modelStack,
                                 int32_t                        amount,
                                 int32_t                        effectiveLength)                                       = 0;
  virtual void processCurrentPos(ModelStackWithParamCollection* modelStack,
                                 int                            ticksSinceLast,
                                 bool                           reversed,
                                 bool                           didPingpong,
  ...
  virtual void nudgeNonInterpolatingNodesAtPos(int32_t                        pos,
                                               int                            offset,
                                               int32_t                        lengthBeforeLoop,
                                               Action*                        action,
                                               ModelStackWithParamCollection* modelStack)       = 0;
  virtual void notifyParamModifiedInSomeWay(ModelStackWithAutoParam const* modelStack,
                                            int32_t                        oldValue,
                                            bool                           automationChanged,
                                            bool                           automatedBefore,
                                            bool automatedNow); // Watch the heck out! This might
                                                                // delete the AutoParam in question.
  virtual ModelStackWithAutoParam*
  getAutoParamFromId(ModelStackWithParamId* modelStack,
                     bool allowCreation = false) = 0; // You must not pass this any child class
                                                      // of ModelStackWithThreeMoreThings (wait
                                                      // why again?). May return NULL
