...
void WaveformBasicNavigator::opened(SampleHolder* holder) { // Only if range is provided, grabs
                                                            // navigation from that if possible
...
bool WaveformBasicNavigator::zoom(int           offset,
    ...
    for (int m = 0; m < NUM_MARKER_TYPES; m++) {
      ...
      if (col >= 0 && col < displayWidth) {
        ...
        bestYet:
          ...
          if (thisMarkerDistanceFromCentre < pinMarkerDistanceFromCentre) { goto bestYet; }
        }
  ...
  if (pinMarkerCol == -1) { pinMarkerCol = (displayWidth >> 1); }
  ...
  memcpy(PadLEDs::imageStore[(offset > 0) ? displayHeight : 0],
         PadLEDs::image,
         (displayWidth + sideBarWidth) * displayHeight * 3);

  // Calculate pin squares
  int32_t zoomPinSquareBig =
      ((int64_t)(int32_t)(oldScroll - xScroll) << 16) / (int32_t)(newXZoom - oldZoom);
  ...
  PadLEDs::clearTickSquares(false); // We were mostly fine without this here, but putting it here
                                    // fixed weird problem where tick squares would appear when
                                    // zooming into waveform in SampleBrowser

  waveformRenderer.renderFullScreen(sample,
                                    xScroll,
                                    xZoom,
