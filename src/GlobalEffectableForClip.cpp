...
void GlobalEffectableForClip::renderOutput(ModelStackWithTimelineCounter* modelStack,
  ...
  // The >>1 is to make up for the fact that we've got the preset default to
  // effect a multiplication of 2 already (the maximum multiplication would be
  // 4)
  int32_t a = cableToLinearParamShortcut(
      unpatchedParams->getValue(PARAM_UNPATCHED_GLOBALEFFECTABLE_VOLUME));
  int32_t volumeAdjustment = getFinalParameterValueVolume(134217728, a) >> 1;

  int32_t volumePostFX = volumeAdjustment;

  // Make it a bit bigger so that default filter resonance doesn't reduce volume
  // overall. Unfortunately when I first implemented this for Kits, I just
  // fudged a number which didn't give the 100% accuracy that I need for
  // AudioOutputs, and I now have to maintain both for backwards compatibility
  ...
  int32_t reverbAmountAdjustForDrums =
      multiply_32x32_rshift32_rounded(reverbAmountAdjust, volumeAdjustment) << 5;

  int32_t pitchAdjust = getFinalParameterValueExp(
      16777216,
      unpatchedParams->getValue(PARAM_UNPATCHED_GLOBALEFFECTABLE_PITCH_ADJUST) >> 3);
  ...
  int32_t sidechainVolumeParam =
      unpatchedParams->getValue(PARAM_UNPATCHED_GLOBALEFFECTABLE_SIDECHAIN_VOLUME);
  ...
  if (sidechainVolumeParam != -2147483648) {
    ...
    int32_t positivePatchedValue =
        multiply_32x32_rshift32(compressorOutput,
                                getSidechainVolumeAmountAsPatchCableDepth(paramManagerForClip)) +
        536870912;
    postReverbVolume =
        (positivePatchedValue >> 15) *
        (positivePatchedValue >> 16); // This is tied to
                                      // getParamNeutralValue(PARAM_GLOBAL_VOLUME_POST_REVERB_SEND)
                                      // returning 134217728
  }

  static StereoSample globalEffectableBuffer[SSI_TX_BUFFER_NUM_SAMPLES]
      __attribute__((aligned(CACHE_LINE_SIZE)));

  bool canRenderDirectlyIntoSongBuffer =
      !isKit() && !filterSetConfig.doLPF && !filterSetConfig.doHPF && !delayWorkingState.doDelay &&
      (!pan || !AudioEngine::renderInStereo) && !clippingAmount &&
      !hasBassAdjusted(paramManagerForClip) && !hasTrebleAdjusted(paramManagerForClip) &&
      !reverbSendAmount && !isBitcrushingEnabled(paramManagerForClip) &&
  ...
  if (canRenderDirectlyIntoSongBuffer) {
    ...
    int32_t postFXAndReverbVolumeStart =
        (multiply_32x32_rshift32(postReverbVolumeLastTime, volumePostFX) << 5);
    if (postFXAndReverbVolumeStart > 134217728)
      goto doNormal; // If it's too loud, this optimized routine can't handle
                     // it. This is a design flaw...

    int32_t postFXAndReverbVolumeEnd =
        (multiply_32x32_rshift32(postReverbVolume, volumePostFX) << 5);
    if (postFXAndReverbVolumeEnd > 134217728) goto doNormal;

    // If it's a mono sample, that's going to have to get rendered into a mono
    // buffer first before it can be copied out to the stereo song-level buffer
    if (willRenderAsOneChannelOnlyWhichWillNeedCopying()) {
      memset(globalEffectableBuffer, 0, sizeof(int32_t) * numSamples);
      renderGlobalEffectableForClip(modelStack,
                                    globalEffectableBuffer,
                                    (int32_t*)outputBuffer,
                                    numSamples,
                                    reverbBuffer,
                                    reverbAmountAdjustForDrums,
                                    sideChainHitPending,
                                    shouldLimitDelayFeedback,
                                    isClipActive,
                                    pitchAdjust,
                                    postFXAndReverbVolumeStart,
  ...
  doNormal:
    ...
    // Render FX
    processSRRAndBitcrushing(globalEffectableBuffer,
                             numSamples,
                             &volumePostFX,
                             paramManagerForClip);
    processFXForGlobalEffectable(globalEffectableBuffer,
                                 numSamples,
                                 &volumePostFX,
                                 paramManagerForClip,
                                 &delayWorkingState,
    ...
    processReverbSendAndVolume(globalEffectableBuffer,
                               numSamples,
                               reverbBuffer,
                               volumePostFX,
...
int32_t
GlobalEffectableForClip::getSidechainVolumeAmountAsPatchCableDepth(ParamManager* paramManager) {
  int32_t sidechainVolumeParam = paramManager->getUnpatchedParamSet()->getValue(
...
// We pass activeClip into this because although each child of
// GlobalEffectableForClip inherits Output, one of them does so via Instrument,
// so we can't make GlobalEffectableForClip inherit directly from Output, so no
// access to activeClip
void GlobalEffectableForClip::getThingWithMostReverb(
    Clip*                     activeClip,
  ...
  if (activeClip) {
    ...
    if (!unpatchedParams->params[PARAM_UNPATCHED_GLOBALEFFECTABLE_REVERB_SEND_AMOUNT]
             .isAutomated() &&
        unpatchedParams->params[PARAM_UNPATCHED_GLOBALEFFECTABLE_REVERB_SEND_AMOUNT]
            .containsSomething(-2147483648)) {

      int32_t reverbHere =
          unpatchedParams->getValue(PARAM_UNPATCHED_GLOBALEFFECTABLE_REVERB_SEND_AMOUNT);
