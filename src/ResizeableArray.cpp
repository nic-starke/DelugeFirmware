...
#define LOCK_ENTRY                                     \
  if (lock) { numericDriver.freezeWithError("i008"); } \
  lock = true;
...
ResizeableArray::ResizeableArray(int newElementSize,
                                 ...
                                 int newNumExtrarSpacesToAllocate)
...
// Returns error
int ResizeableArray::copyElementsFromOldMemory(void* __restrict__ otherMemory,
                                               int otherMemorySize,
  ...
  if (!numElements) {
    ...
    memcpy((char* __restrict__)memory,
           (char* __restrict__)otherMemory + (otherMemoryStart * elementSize),
           numBytesBefore);
    ...
    if (elementsAfterWrap > 0) {
      memcpy((char* __restrict__)memory + (elementsBeforeWrap * elementSize),
             otherMemory,
  ...
  if (allocatedSize > (memorySize + maxNumEmptySpacesToKeep) * elementSize) {
    ...
    if (extraSpaceLeft > extraSpaceRight) {
      uint32_t amountShortened =
          generalMemoryAllocator.shortenLeft(memoryAllocationStart, extraSpaceLeft);
      memoryAllocationStart = (char* __restrict__)memoryAllocationStart + amountShortened;
    }

    else {
      generalMemoryAllocator.shortenRight(memoryAllocationStart,
...
void ResizeableArray::deleteAtIndex(int i, int numToDelete, bool mayShortenMemoryAfter) {
  ...
  mostBasicDelete:
    ...
    if ((i + (numToDelete >> 1)) < (numElements >> 1)) {
      // if (i) moveElementsRightNoWrap(0, i, numToDelete); // No - not sure
      // why, but this gave insane occasional crashes.
  ...
  // Or if yes wrap...
    ...
    // If our delete borders (or overlaps) the wrap point, our job is really
    // easy
    if (i <= elementsBeforeWrap && i + numToDelete >= elementsBeforeWrap) {
      ...
      // If closer to either start or end than wrap-point AND there isn't too
      // much wasted empty space between these...
      if (distanceFromEndPoint <= distanceFromWrapPoint) {

        // If we have a fixed memory allocation, then we don't need to worry
        // about wasted empty space, and we can just do the most basic delete.
        // Careful with this - it seemed to cause a crash when first introduced
        // in commit "Tidied up some stuff in ResizeableArray", though that
        // seems to have been because of the bugs fixed in commit "Fixed
        // horrendous problem with ResizeableArray[...]".
        if (staticMemoryAllocationSize || !mayShortenMemoryAfter) goto mostBasicDelete;

        int freeMemory = memorySize - newNum; // After deletion

        // If we're not going to end up with more free memory than we're
        // allowed, then the best option is to do the most basic delete
        if (freeMemory < maxNumEmptySpacesToKeep) goto mostBasicDelete;

        // Ok, if we're still here, we're looking to reduce memory usage

        // If easier to move bit between between deletion point and wrap
        // point...
...
// Currently this doesn't really support having a static memory allocation, so
// don't call this
bool ResizeableArray::ensureEnoughSpaceAllocated(int numAdditionalElementsNeeded) {
  ...
  if (!memory) {
    ...
    void* newMemory = generalMemoryAllocator.alloc(numAdditionalElementsNeeded * elementSize,
                                                   &allocatedMemorySize,
                                                   false,
                                                   true);
    if (!newMemory) {
...
tryAgain:
...
#endif
    generalMemoryAllocator.extend(memoryAllocationStart,
                                  (newNum)*elementSize - allocatedSize,
  ...
  getBrandNewMemory:
    ...
    newMemory = generalMemoryAllocator.alloc((newNum + numExtraSpacesToAllocate) * elementSize,
                                             &newMemoryAllocationSize,
                                             false,
                                             true);
    if (!newMemory) {
      newMemory =
          generalMemoryAllocator.alloc(newNum * elementSize, &newMemoryAllocationSize, false, true);
    }
    ...
    // Or if we're here, we got our new memory. Copy the stuff over. Before wrap
    // point...
    ...
    if (elementsAfterWrap > 0) {
      copyToNewMemory(newMemory,
                      elementsBeforeWrap,
                      getElementAddress(elementsBeforeWrap + memoryIncreasedBy),
                      elementsAfterWrap,
                      newMemorySize,
                      newMemoryStartIndex);
    }
...
bool ResizeableArray::attemptMemoryExpansion(int   minNumToExtend,
                                             int   idealNumToExtendIfExtendingAllocation,
...
startAgain:
  ...
  if (extraBytesLeft >= elementSize) {
    int extraElementsLeft = extraBytesLeft / elementSize; // See how many extra elements there are
  ...
  if (!potentialMemorySize)
    potentialMemorySize =
        generalMemoryAllocator.getAllocatedSize(memoryAllocationStart) - extraBytesLeft;
  uint32_t extraBytesRight = potentialMemorySize - (memorySize * elementSize);
  if (extraBytesRight >= elementSize) {
    int extraElementsRight = extraBytesRight / elementSize; // See how many extra elements there are
                                                            // space for on the right
...
*/
    // Try extending memory.
    // TODO: in a perfect world, we'd be able to specify a minimum amount as
    // well as a step-size for left and right, or something like that
    uint32_t amountExtendedLeft, amountExtendedRight;
    generalMemoryAllocator.extend(memoryAllocationStart,
...
void ResizeableArray::copyToNewMemory(void* __restrict__ newMemory,
                                      uint32_t destinationIndex,
                                      void* __restrict__ source,
                                      uint32_t numElementsToCopy,
  ...
  if (destinationIndex + numElementsToCopy > newMemorySize) {
    ...
    // Or if begins before wrap point...
      ...
      memcpy((char* __restrict__)newMemory + destinationIndex * elementSize,
             source,
             elementsBeforeWrap * elementSize);
      memcpy((char* __restrict__)newMemory,
             (char* __restrict__)source + elementsBeforeWrap * elementSize,
...
void ResizeableArray::moveElementsLeft(int oldStartIndex, int oldStopIndex, int distance) {
  ...
  // Otherwise, complicated
    ...
    int startPastWrapPoint =
        getMax(oldStartIndex - elementsBeforeWrap,
...
void ResizeableArray::moveElementsRight(int oldStartIndex, int oldStopIndex, int distance) {
  ...
  // Otherwise, complicated
    ...
    // Move the elements that need to move past the wrap point. TODO: this isn't
    // quite ideal...
...
int ResizeableArray::insertAtIndex(int i, int numToInsert, void* thingNotToStealFrom) {
  ...
  if (!memory) {
    ...
    int newMemorySize =
        (numExtraSpacesToAllocate >> 1) + numToInsert; // The >>1 is arbirtary - we just don't
                                                       // wanna be allocating lots

    uint32_t allocatedMemorySize;

    void* newMemory = generalMemoryAllocator.alloc(newMemorySize * elementSize,
                                                   &allocatedMemorySize,
                                                   false,
                                                   true,
                                                   false,
  ...
  // Otherwise, if some memory...
    ...
    if (elementsAfterWrap <= 0) {
      ...
      if (newNum > memorySize) {
        bool success = attemptMemoryExpansion(numToInsert,
                                              numToInsert + numExtraSpacesToAllocate,
    ...
    // Or, if wrap...
      ...
      if (distanceFromEndPoint <= distanceFromWrapPoint) {
        ...
        // If we have enough memory, then great - doing the "normal" thing is
        // the most efficient thing we can do. Sadly, if we've been inserting
        // lots, there won't be any extra memory within memorySize here - if
        // there's any, it'll be out to the sides (at the wrap point). It'd be
        // better if we had some in both places, like happens after a
        // get-brand-new-memory. Maybe I should make it so doing an extend puts
      ...
      // Or if closer to wrap-point...
        ...
        if (!attemptMemoryExpansion(numToInsert,
          ...
          // If we do actually have enough memory, working "normally" is still
          // an option, and it's now the best option
          if (newNum <= memorySize) goto workNormally;
      ...
      if (i < elementsBeforeWrap) {
        ...
        int otherOption = i + elementsAfterWrap; // The other option is to separately move the
                                                 // elements before the insertion, and the
                                                 // ones after the wrap
        if (elementsBetweenInsertionAndWrap <= otherOption) {
          ...
          moveElementsRight(i,
                            elementsBeforeWrap,
                            numToInsert); // No-wrap seems to work here, but I
                                          // can't quite see why
        }
          ...
          moveElementsLeft(elementsBeforeWrap + numToInsert,
                           numElements + numToInsert,
                           numToInsert);

          // Move elements before insertion point left. We call
          // moveElementsLeft() because that deals with if it passes over the
          // wrap point, which can definitely happen.
          memoryStart -= numToInsert;
          if (memoryStart < 0)
    ...
    getBrandNewMemory:
      ...
      // Otherwise, manually get some brand new memory and do a more complex
      // copying process
      uint32_t desiredSize = (newNum + numExtraSpacesToAllocate) * elementSize;

    getBrandNewMemoryAgain:
      uint32_t allocatedSize;
      void* __restrict__ newMemory = generalMemoryAllocator.alloc(desiredSize,
                                                                  &allocatedSize,
                                                                  false,
                                                                  true,
                                                                  false,
      ...
      if (!newMemory) {
        ...
        // If our expectations weren't already as low as they can go, then lower
        // our expectations and try again...
      ...
      // Set up the new memory so the wrap point is right in the middle. This
      // makes everything run faster, particularly if this brand-new-memory
      ...
      // If we didn't reach the wrap point because we reached the insertion
      // point, do some more, up until the wrap point
      if (firstElements < elementsBeforeWrap) {
        int secondElementsTotal = getMin(elementsBeforeWrap, numElements);
        if (secondElementsTotal > 0) {
          copyToNewMemory(newMemory,
                          firstElements + numToInsert,
                          getElementAddress(firstElements),
                          secondElementsTotal - firstElements,
                          newMemorySize,
                          newMemoryStartIndex);
        }
      }

      // We've now copied up until the wrap point, or to the end of the elements
      // if there weren't that many. See if we need to do any more before the
      // insertion point...
      ...
      if (elementsAfterWrapBeforeInsertion > 0) {
        copyToNewMemory(newMemory,
                        elementsBeforeWrap,
                        memory,
                        elementsAfterWrapBeforeInsertion,
                        newMemorySize,
                        newMemoryStartIndex);
      }

      // Ok, we've now passed the wrap point *and* the insertion point. See
      // what's left
      ...
      if (elementsLeft > 0) {
        copyToNewMemory(newMemory,
                        upTo + numToInsert,
                        getElementAddress(upTo),
                        elementsLeft,
                        newMemorySize,
                        newMemoryStartIndex);
      }

      // That copy may have taken ages, particularly in the case where they're
      // recording / resampling a sample and the array of clusters has grown.
