...
/*
 * Kits are made up of multiple Drums. Even when they are not drum sounds, the
 * class is called Drum, for better or worse. In most instructional material for
 * users, Synthstrom has referred to them often as “items within kits”, or
...
public:
  ...
  virtual void noteOn(ModelStackWithThreeMainThings* modelStack,
                      uint8_t                        velocity,
                      Kit*                           kit,
  ...
  virtual void noteOff(ModelStackWithThreeMainThings* modelStack,
                       int                            velocity = DEFAULT_LIFT_VALUE)       = 0;
  virtual bool allowNoteTails(ModelStackWithSoundFlags* modelStack,
                              bool                      disregardSampleLoop = false) = 0;
  ...
  virtual void getName(char* buffer) = 0; // May return up to 5 actual characters, so supply
                                          // at least a char[6]
  ...
  void         expressionEventPossiblyToRecord(ModelStackWithTimelineCounter* modelStack,
                                               int16_t                        newValue,
