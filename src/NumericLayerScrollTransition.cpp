...
bool NumericLayerScrollTransition::callBack() {
  ...
  if (progressFlipped > 0 && next) {
    int readingFrom = (transitionDirection == 1) ?
                          transitionProgress :
                          (NUMERIC_DISPLAY_LENGTH + transitionProgress - 1);
    ...
    // TODO: make this work proper! Fails if scrolling left to a display with
    // only the right-most character occupied
    ...
    if (allBlank) {
        ...
        if (transitionDirection == 1) {
            for (int8_t i = 0; i < NUMERIC_DISPLAY_LENGTH - 1; i++) {
                if (segmentsTransitioningTo[i] == 0) transitionProgress +=
    transitionDirection;
            }
        }
