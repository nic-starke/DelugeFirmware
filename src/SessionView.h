...
public:
  ...
  bool         renderRow(ModelStack* modelStack,
                         uint8_t     yDisplay,
  ...
  bool     calculateZoomPinSquares(uint32_t oldScroll,
                                   uint32_t newScroll,
                                   uint32_t newZoom,
                                   uint32_t oldZoom);
  ...
  uint8_t  selectedClipPressYDisplay; // Where the user's finger actually is on
                                      // screen
  uint8_t selectedClipPressXDisplay;
  bool    performActionOnPadRelease;
  bool    performActionOnSectionPadRelease; // Keep this separate from the above
                                            // one because we don't want a mod
