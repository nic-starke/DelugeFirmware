...
int ClipView::buttonAction(int x, int y, bool on, bool inCardRoutine) {
  ...
  // Horizontal encoder button press-down - don't let it do its zoom level thing
  // if zooming etc not currently accessible
  if (x == xEncButtonX && y == xEncButtonY && on &&
      !currentSong->currentClip->currentlyScrollableAndZoomable()) {
  }
...
Action* ClipView::lengthenClip(int32_t newLength) {
  ...
  bool undoing =
      (actionLogger.firstAction[BEFORE] && actionLogger.firstAction[BEFORE]->openForAdditions &&
  ...
  if (undoing) {
    allowResyncingDuringClipLengthChange = false; // Little bit of a hack. We don't want any
                                                  // resyncing to happen to this Clip
    actionLogger.revert(BEFORE, false, false);
    allowResyncingDuringClipLengthChange = true;
  }

  // Only if that didn't get us directly to the correct length, manually set
  // length. This will do a resync if playback active
  if (currentSong->currentClip->loopLength != newLength) {
    int actionType = (newLength < currentSong->currentClip->loopLength) ?
                         ACTION_CLIP_LENGTH_DECREASE :
...
Action* ClipView::shortenClip(int32_t newLength) {
  ...
  currentSong->setClipLength(
      ...
      newLength,
      action); // Subsequently shortening by more squares won't cause additional
               // Consequences to be added to the same Action - it checks, and
               // only stores the data (snapshots and original length) once
  return action;
}
...
int ClipView::getTickSquare() {
  ...
  if (currentSong->currentClip->getCurrentlyRecordingLinearly()) {
    ...
    if (newTickSquare == displayWidth && (!currentUIMode || currentUIMode == UI_MODE_AUDITIONING) &&
        ...
        (!currentSong->currentClip->armState ||
