...
bool SaveSongUI::opened() {
  ...
  doReturnFalse:
    renderingNeededRegardlessOfUI(); // Because unlike many UIs we've already gone and drawn the
                                     // QWERTY interface on the pads.
    return false;
  }
...
bool SaveSongUI::performSave(bool mayOverwrite) {
  ...
  if (ALPHA_OR_BETA_VERSION && currentlyAccessingCard) { numericDriver.freezeWithError("E316"); }
  ...
  // We might want to copy some samples around - either because we're "collecting" them to a folder,
  // or because they were loaded in from a collected folder and we now need to put them in the main
  // samples folder.
  ...
  error = audioFileManager.setupAlternateAudioFileDir(&newSongAlternatePath,
                                                      currentDir.get(),
    ...
    if (audioFile->numReasonsToBeLoaded) {
      ...
      if (audioFile->type == AUDIO_FILE_TYPE_SAMPLE) {
        ...
        if (!((Sample*)audioFile)->tempFilePathForRecording.isEmpty()) {
          FRESULT result = f_rename(((Sample*)audioFile)->tempFilePathForRecording.get(),
                                    audioFile->filePath.get());
          if (result == FR_OK) {
            ((Sample*)audioFile)->tempFilePathForRecording.clear();
          } else {
            anyErrorMovingTempFiles =
                true; // We at least need to warn the user that although the main file save was
                      // (hopefully soon to be) successful, something's gone wrong
            /*
      ...
      // Or if file needs copying either to or from an "alt" location - either because we're doing a
      // "collect media" or importing from such a folder. Crucial obscure combination - we could be
      // doing a "collect media" *AND ALSO* have moved (or even failed to move!) a recorded file
      // from its "temp" location above
      if (collectingSamples || !audioFile->loadedFromAlternatePath.isEmpty()) {

        // If saving as *same* song name/slot, collecting samples, and it already came from alt
        // location, no need to do it again
        if (collectingSamples && !audioFile->loadedFromAlternatePath.isEmpty()) {
        ...
        if (!audioFile->loadedFromAlternatePath.isEmpty()) {
          // If we loaded the file from an alternate path originally, well we saved that exact path
          // just so we can recall it here!
          ...
          sourceFilePath =
              (audioFile->type != AUDIO_FILE_TYPE_SAMPLE ||
               ((Sample*)audioFile)->tempFilePathForRecording.isEmpty()) ?
                  audioFile->filePath.get() :
                  ((Sample*)audioFile)
                      ->tempFilePathForRecording.get(); // It may still have a temp path if for some
                                                        // reason we failed to move it, above
        }

        // Note: we can't just use the clusters to write back to the card, cos these might contain
        // data that we converted
        ...
        if (collectingSamples) {
          ...
          // NOTE: I guess this would do this multiple times if we kept re-saving... probably not
          ...
          if (!memcasecmp(normalFilePath, "SAMPLES/RECORD/REC", 18) ||
            ...
            if (audioFile->filePath.getLength() - fileNamePos == 12 &&
              ...
              // And because the AudioFile in memory is now associated with a file name which only
              // exists in the "alternative location", we need to mark it as if it was loaded from
              // there, so any future copying of that file will treat it correctly
              // - particularly if the user does another collect-media save over this one, meaning
              // the file should not be copied again.
              needToPretendLoadedAlternate = true; // We don't have that alternate path yet, so just
                                                   // make a note to do it below.
            }
          }

          // Normally, the filePath will be in the SAMPLES folder, which our name-condensing system
          // was designed for...
          if (!memcasecmp(audioFile->filePath.get(), "SAMPLES/", 8)) {
            error = audioFileManager.setupAlternateAudioFilePath(&newSongAlternatePath,
                                                                 dirPathLengthNew,
                                                                 &audioFile->filePath);
            if (error) {
          ...
          // Or, if it wasn't in the SAMPLES folder, e.g. if it was in a dedicated SYNTH folder,
          // then we have to just use the original filename, and hope it doesn't clash with
          // anything.
          else {
        ...
        } // No problem - the audio file was already there from before, so we don't need to copy it
          // again now.
        else if (error)
        ...
        // Or if everything's fine and we're ready to write / copy...
          ...
          while (true) {
            UINT bytesRead;
            result = f_read(&fileSystemStuff.currentFile,
                            storageManager.fileClusterBuffer,
            ...
            UINT bytesWritten;
            result = f_write(&recorderFileSystemStuff.currentFile,
                             storageManager.fileClusterBuffer,
                             bytesRead,
                             &bytesWritten);
        ...
        // Write has succeeded. We can mark it as existing in its normal main location (e.g. in the
        // SAMPLES folder). Unless we were collection media, in which case it won't be there - it'll
        // be in the new alternate location we put it in.
  ...
  // If we're overwriting an existing file, we'll write to a temp file first. Find one that doesn't
  // already exist
  ...
  // (Sept 2019) - it seems a crash sometimes occurs sometime after this point. A 0-byte file gets
  // created. Could be for either overwriting or not.
  ...
  OLED::removeWorkingAnimation();
  char const* message =
      anyErrorMovingTempFiles ? "Song saved, but error moving temp files" : "Song saved";
