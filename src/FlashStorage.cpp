...
uint8_t recordQuantizeLevel; // Assumes insideWorldTickMagnitude==1, which is not default anymore,
...
uint8_t defaultBendRange[2] = {
    2,
    48}; // The 48 isn't editable. And the 2 actually should only apply to non-MPE MIDI, because
         // it's editable, whereas for MPE it's meant to always stay at 2.
...
void resetSettings() {
  ...
  for (int i = 0; i < NUM_GATE_CHANNELS; i++) { cvEngine.setGateType(i, GATE_MODE_V_TRIG); }
  ...
  for (int i = 0; i < NUM_GLOBAL_MIDI_COMMANDS; i++) { midiEngine.globalMIDICommands[i].clear(); }
...
void readSettings() {
  ...
  R_SFLASH_ByteRead(0x80000 - 0x1000,
                    buffer,
                    256,
                    SPIBSC_CH,
  ...
  for (int i = 0; i < NUM_GATE_CHANNELS; i++) { cvEngine.setGateType(i, buffer[22 + i]); }
  ...
  playbackHandler.midiInClockEnabled =
      (previouslySavedByFirmwareVersion < FIRMWARE_2P1P0_BETA) ? true : buffer[52];
  ...
  midiEngine.globalMIDICommands[GLOBAL_MIDI_COMMAND_PLAYBACK_RESTART].channelOrZone =
      buffer[38] - 1;
  ...
  midiEngine.globalMIDICommands[GLOBAL_MIDI_COMMAND_LOOP_CONTINUOUS_LAYERING].channelOrZone =
      buffer[70] - 1;
  midiEngine.globalMIDICommands[GLOBAL_MIDI_COMMAND_LOOP_CONTINUOUS_LAYERING].noteOrCC =
      buffer[71] - 1;

  if (previouslySavedByFirmwareVersion >= FIRMWARE_3P2P0_ALPHA) {
    MIDIDeviceManager::readDeviceReferenceFromFlash(GLOBAL_MIDI_COMMAND_PLAYBACK_RESTART,
                                                    &buffer[80]);
    ...
    MIDIDeviceManager::readDeviceReferenceFromFlash(GLOBAL_MIDI_COMMAND_LOOP_CONTINUOUS_LAYERING,
                                                    &buffer[100]);
  ...
  soundEditor.setShortcutsVersion(
      (previouslySavedByFirmwareVersion < FIRMWARE_2P1P3_BETA) ? SHORTCUTS_VERSION_1 : buffer[60]);
  ...
  if (previouslySavedByFirmwareVersion < FIRMWARE_3P1P0_ALPHA) {
    ...
    if (previouslySavedByFirmwareVersion ==
        FIRMWARE_3P1P0_ALPHA) { // Could surely delete this code?
...
void writeSettings() {
  ...
  for (int i = 0; i < NUM_GATE_CHANNELS; i++) { buffer[22 + i] = cvEngine.gateChannels[i].mode; }
  ...
  buffer[38] =
      midiEngine.globalMIDICommands[GLOBAL_MIDI_COMMAND_PLAYBACK_RESTART].channelOrZone + 1;
  ...
  buffer[70] =
      midiEngine.globalMIDICommands[GLOBAL_MIDI_COMMAND_LOOP_CONTINUOUS_LAYERING].channelOrZone + 1;
  buffer[71] =
      midiEngine.globalMIDICommands[GLOBAL_MIDI_COMMAND_LOOP_CONTINUOUS_LAYERING].noteOrCC + 1;
  ...
  MIDIDeviceManager::writeDeviceReferenceToFlash(GLOBAL_MIDI_COMMAND_LOOP_CONTINUOUS_LAYERING,
                                                 &buffer[100]);
  ...
  R_SFLASH_EraseSector(0x80000 - 0x1000,
                       SPIBSC_CH,
                       SPIBSC_CMNCR_BSZ_SINGLE,
                       1,
                       SPIBSC_OUTPUT_ADDR_24);
  R_SFLASH_ByteProgram(0x80000 - 0x1000,
                       buffer,
                       256,
                       SPIBSC_CH,
                       SPIBSC_CMNCR_BSZ_SINGLE,
