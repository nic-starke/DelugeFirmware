...
// If you're calling this direcly instead of calling writeToFile(), you'll need
// to check and possibly write a new tag for device - that can't be just an
// attribute. You should be sure that containsSomething() == true before calling
// this.
...
bool LearnedMIDI::equalsChannelAllowMPE(MIDIDevice* newDevice, int newChannel) {
  if (channelOrZone == MIDI_CHANNEL_NONE)
    return false; // 99% of the time, we'll get out here, because input isn't
                  // activated/learned.
  if (!equalsDevice(newDevice)) return false;
  if (channelOrZone < 16) return (channelOrZone == newChannel);
  if (!device)
    return false; // Could we actually be set to MPE but have no device? Maybe
                  // if loaded from weird song file?
  if (channelOrZone == MIDI_CHANNEL_MPE_LOWER_ZONE)
    return (newChannel <=
            device->ports[MIDI_DIRECTION_INPUT_TO_DELUGE].mpeLowerZoneLastMemberChannel);
  if (channelOrZone == MIDI_CHANNEL_MPE_UPPER_ZONE)
    return (newChannel >=
            device->ports[MIDI_DIRECTION_INPUT_TO_DELUGE].mpeUpperZoneLastMemberChannel);
