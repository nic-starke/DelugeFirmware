...
// A view is where the user can interact with the pads - song view, Clip view,
// and keyboard view. (Is that still a good description? This class is a bit of
// a mishmash of poorly organised code, sorry.)
...
public:
  ...
  void setActiveModControllableWithoutTimelineCounter(ModControllable* modControllable,
  ...
  void navigateThroughPresetsForInstrumentClip(int                            offset,
                                               ModelStackWithTimelineCounter* modelStack,
                                               bool                           doBlink = false);
  void navigateThroughAudioOutputsForAudioClip(int offset, AudioClip* clip, bool doBlink = false);
  bool changeInstrumentType(int                            newInstrumentType,
                            ModelStackWithTimelineCounter* modelStack,
                            bool                           doBlink = false);
  void drawOutputNameFromDetails(int         instrumentType,
                                 int         slot,
                                 int         subSlot,
                                 char const* name,
                                 bool        editedByUser,
  ...
  int      modNoteRowId;
  uint32_t modPos;
  uint32_t modLength; // 0 if not currently editing a region / step / holding a note.
                      // If you're gonna refer to this, you absolutely have to first
                      // check that the TimelineCounter you're thinking of setting
                      // some automation on == activeModControllableTimelineCounter
