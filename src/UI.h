...
// Non-exclusive UI modes, which can (if the code allows) occur at the same time
// as other ones, including the "exclusive" ones above.
...
public:
  ...
  virtual bool
  noteOnReceivedForMidiLearn(MIDIDevice* fromDevice, int channel, int note, int velocity) {
    return false;
  ...
  // When these return false it means they're transparent, showing what's
  // underneath. These *must* check whether image has been supplied - if not,
  // just return, saying whether opaque or not. Cos we need to be able to quiz
  // these without actually getting any rendering done.
  virtual bool renderMainPads(uint32_t whichRows                                    = 0,
                              uint8_t  image[][displayWidth + sideBarWidth][3]      = NULL,
                              uint8_t  occupancyMask[][displayWidth + sideBarWidth] = NULL,
                              bool     drawUndefinedArea                            = true) {
    return false;
  }
  virtual bool renderSidebar(uint32_t whichRows                                    = 0,
                             uint8_t  image[][displayWidth + sideBarWidth][3]      = NULL,
                             uint8_t  occupancyMask[][displayWidth + sideBarWidth] = NULL) {
    return false;
  }
...
void uiNeedsRendering(UI*      ui,
                      uint32_t whichMainRows = 0xFFFFFFFF,
                      uint32_t whichSideRows = 0xFFFFFFFF);
void renderingNeededRegardlessOfUI(uint32_t whichMainRows = 0xFFFFFFFF,
                                   uint32_t whichSideRows = 0xFFFFFFFF);
