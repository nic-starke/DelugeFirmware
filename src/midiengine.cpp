...
uint8_t currentDeviceNumWithSendPipe[USB_NUM_USBIP][2] = {
    ...
    MAX_NUM_USB_MIDI_DEVICES}; // One without, and one with, interrupt endpoints

// We now bypass calling this for successful as peripheral on A1 (see
// usb_pstd_bemp_pipe_process_rohan_midi())
void usbSendComplete(int ip) {
  ...
  connectedDevice->numBytesSendingNow =
      0; // We just do this instead from caller on A1 (see comment above)
  ...
  if (inHostMode) {
    ...
    // If that was the last device we were going to send to, that send's been
    // done, so we can just get out.
    ...
    while (true) {
      ...
      if (midiDeviceNum == stopSendingAfterDeviceNum[ip]) { // If reached end of devices and last
                                                            // one got disconnected in the
                                                            // interim (very rare)
...
void usbSendCompletePeripheralOrA1(usb_utr_t* p_mess, uint16_t data1, uint16_t data2) {
  ...
  // No actually don't - sometimes there'll be an error if another device
  // connected or disconnected from hub during fast MIDI sending. This seems to
  // happen even though I've stopped it from setting up or down the out-pipe as
  // it goes
...
void usbReceiveComplete(int ip, int deviceNum, int tranlen) {
  ...
  connectedDevice->numBytesReceived =
      64 - tranlen; // Seems wack, but yet, tranlen is now how many bytes didn't
                    // get received out of the original transfer size
  // Warning - sometimes (with a Teensy, e.g. my knob box), length will be 0.
  // Not sure why - but we need to cope with that case.

  connectedDevice->currentlyWaitingToReceive =
...
void usbReceiveCompletePeripheralOrA1(usb_utr_t* p_mess, uint16_t data1, uint16_t data2) {
  ...
  if (p_mess->status != USB_DATA_SHT) { // Are there actually any other possibilities that could
                                        // happen here? Can't remember.
    uartPrint("status: ");
...
extern "C" void flushUSBMIDIToHostedDevice(int ip, int d) {
  ...
  if (d != currentDeviceNumWithSendPipe[USB_CFG_USE_USBIP][isInterrupt]) {
    ...
    change_destination_of_send_pipe(&g_usb_midi_send_utr[USB_CFG_USE_USBIP],
                                    pipeNumber,
                                    g_usb_hmidi_tmp_ep_tbl[USB_CFG_USE_USBIP][d],
                                    connectedDevice->sq);
  }
  ...
  usb_send_start_rohan(&g_usb_midi_send_utr[USB_CFG_USE_USBIP],
                       pipeNumber,
                       connectedDevice->dataSendingNow,
                       connectedDevice->numBytesSendingNow);
}
...
void MidiEngine::flushUSBMIDIOutput() {
  ...
  for (int ip = 0; ip < USB_NUM_USBIP; ip++) {
    ...
    if (aPeripheral) {
      ...
      memcpy(connectedDevice->dataSendingNow,
             connectedDevice->preSendData,
             connectedDevice->numBytesSendingNow);
      ...
      // This next bit was written with multiple devices on hubs in mind, but
      // seems to work for a single MIDI device too
      ...
      while (true) {
        ConnectedUSBMIDIDevice* connectedDevice =
            &connectedUSBMIDIDevices[ip][midiDeviceNumToSendTo];
        ...
        if (midiDeviceNumToSendTo == newStopSendingAfter)
          goto getOut; // If back where we started, none are connected. Could
                       // this really happen? Probably.
...
void MidiEngine::sendNote(bool on, int note, uint8_t velocity, uint8_t channel, int filter) {
  ...
  // This is the only place where velocity is limited like this. In the internal
  // engine, it's allowed to go right between 0 and 128
...
void MidiEngine::sendClock(bool sendUSB, int howMany) {
  while (howMany--) { sendMidi(0x0F, 0x08, 0, 0, MIDI_OUTPUT_FILTER_NO_MPE, sendUSB); }
}
...
void MidiEngine::sendMidi(uint8_t statusType,
  ...
  // Send serial MIDI
  if (statusType == 0x0F ||
      MIDIDeviceManager::dinMIDIPorts.wantsToOutputMIDIOnChannel(channel, filter)) {
  ...
  for (int ip = 0; ip < USB_NUM_USBIP; ip++) {
    ...
    for (int d = 0; d < potentialNumDevices; d++) {
      ...
      if (connectedDevice->device && connectedDevice->canHaveMIDISent &&
          (statusType == 0x0F ||
           connectedDevice->device->wantsToOutputMIDIOnChannel(channel, filter))) {
...
void MidiEngine::sendSerialMidi(uint8_t statusType, uint8_t channel, uint8_t data1, uint8_t data2) {
  ...
  // If only 1 byte, we have to send it, and keep no record of last status byte
  // sent since we couldn't do running status anyway
  if (false && messageLength == 1) {
    bufferMIDIUart(statusByte);
    lastStatusByteSent = 0;
  }

  // Or, if message is longer, only send if status byte is different from last
  // time
  ...
  if (messageLength >= 2) {
    ...
    if (messageLength == 3) { bufferMIDIUart(data2); }
  }
}
  ...
  if (timer) {
    ...
    if (thisSerialByte & 0x80) {
      ...
      switch (thisSerialByte) {
        ...
        // If it's a realtime message, we have to obey it right now, separately
        // from any other message it was inserted into the middle of
        case 0xF8 ... 0xFF:
          midiMessageReceived(&MIDIDeviceManager::dinMIDIPorts,
                              thisSerialByte >> 4,
                              thisSerialByte & 0x0F,
                              0,
                              0,
        ...
        case 0xF0:
          ...
          // numSerialMidiInput = 0; // This would throw away any running status
          // stuff...
          return true;
      }

      // If we didn't return for any of those, then it's just a regular old
      // status message (or Sysex stop message). All these will end any ongoing
      // SysEx
      currentlyReceivingSysEx = false;
    ...
    if (getMidiMessageLength(serialMidiInput[0]) == numSerialMidiInput) {
      ...
      // If message was more than 1 byte long, and was a voice or mode message,
      // then allow for running status
...
void MidiEngine::checkIncomingUsbMidi() {
  ...
  if (!usbLockNow) {
    // Have to call this regularly, to do "callbacks" that will grab out the
    // received data
    usbLock = 1;
    usb_cstd_usb_task();
    usbLock = 0;
  ...
  for (int ip = 0; ip < USB_NUM_USBIP; ip++) {
    ...
    for (int d = 0; d < numDevicesNow; d++) {
      if (connectedUSBMIDIDevices[ip][d].device &&
          !connectedUSBMIDIDevices[ip][d].currentlyWaitingToReceive) {
        ...
        if (bytesReceivedHere) {
          ...
          for (; readPos < stopAt; readPos += 4) {
            ...
            midiMessageReceived(connectedUSBMIDIDevices[ip][d].device,
                                statusType,
                                channel,
                                data1,
                                data2,
        ...
        // Or as host
          ...
          // Only allowed to setup receive-transfer if not in the process of
          // sending to various devices. (Wait, still? Was this just because of
          // that insane bug that's now fixed?)
...
void MidiEngine::midiMessageReceived(MIDIDevice* fromDevice,
                                     uint8_t     statusType,
                                     uint8_t     channel,
                                     uint8_t     data1,
                                     uint8_t     data2,
  ...
  // Make copies of these, cos we might modify the variables, and we need the
  // originals to do MIDI thru at the end.
  ...
  if (statusType == 0x0F) {
    ...
    // All these messages, we should only interpret if there's definitely a song
    // loaded
    if (currentSong) {
      ...
      switch (statusType) {
        ...
        case 0x08: // Note off, and note on continues here too
          playbackHandler.noteMessageReceived(fromDevice,
                                              statusType & 1,
                                              channel,
                                              data1,
                                              data2,
        ...
        case 0x0A: // Polyphonic aftertouch
          playbackHandler.aftertouchReceived(fromDevice,
                                             channel,
                                             data2,
                                             data1,
                                             &shouldDoMidiThruNow);
        ...
        case 0x0B: // CC or channel mode message
          ...
          if (data1 < 120) {
            ...
            // Interpret RPN stuff, before we additionally try to process the CC
            // within the song, in case it means something different to the user.
            switch (data1) {
              case 100: // RPN LSB
        ...
        case 0x0E: // Pitch bend
          playbackHandler.pitchBendReceived(fromDevice,
                                            channel,
                                            data1,
                                            data2,
                                            &shouldDoMidiThruNow);
  ...
  // Do MIDI-thru if that's on and we didn't decide not to, above. This will let
  // clock messages through along with all other messages, rather than using our
  // special clock-specific system
  if (shouldDoMidiThruNow) {
    bool shouldSendUSB =
        (fromDevice == &MIDIDeviceManager::dinMIDIPorts); // Only send out on USB if it didn't
                                                          // originate from USB
    sendMidi(originalStatusType,
             channel,
             data1,
             originalData2,
