...
public:
  ...
  void doLaunch();
  void scheduleLaunchTiming(int64_t atTickCount,
                            int     numRepeatsUntil,
  ...
  void armingChanged();
  void userWantsToArmClipsToStartOrSolo(uint8_t section,
                                        Clip*   clip,
  ...
  int  investigateSyncedLaunch(Clip*     waitForClip,
  ...
  void armClipsToStartOrSoloWithQuantization(uint32_t pos,
                                             uint32_t quantization,
                                             uint8_t  section,
                                             bool     stopAllOtherClips,
                                             Clip*    clip,
                                             ...
                                             int      armState = ARM_STATE_ON_NORMAL);
  void armClipToStartOrSoloUsingQuantization(Clip*    thisClip,
                                             bool     doLateStart,
                                             uint32_t pos,
                                             int      armState = ARM_STATE_ON_NORMAL,
  ...
  void
  armClipLowLevel(Clip* loopableToArm, int armState, bool mustUnarmOtherClipsWithSameOutput = true);
  ...
  uint8_t lastSectionArmed; // 255 means none. 254 means the action was
...
private:
  ...
  void
  armSectionWhenNeitherClockActive(ModelStack* modelStack, int section, bool stopAllOtherClips);
};
