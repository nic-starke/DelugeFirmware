...
ConsequenceClipExistence::ConsequenceClipExistence(Clip*      newClip,
...
int ConsequenceClipExistence::revert(int time, ModelStack* modelStack) {
  ...
  if (time != type) { // (Re-)create
...
#endif
      return error; // Run away. This and the Clip(?) will get destructed, and
    ...
    if (!clip->output->activeClip) {
      clip->output->setActiveClip(modelStackWithTimelineCounter); // Must do this to avoid E170
                                                                  // error. If Instrument has no
                                                                  // backedUpParamManager, it must
                                                                  // have an activeClip
    }
  }
    ...
    // Make sure the currentClip isn't left pointing to this Clip. Most of the
    // time, ActionLogger::revertAction() reverts currentClip so we don't have
    // to worry about it - but not if action->currentClip is NULL!
    ...
    shouldBeActiveWhileExistent = session.deletingClipWhichCouldBeAbandonedOverdub(
        clip); // But should we really be calling this without checking the
               // Clip is a session one?
    ...
    if (clipArray == &modelStackWithTimelineCounter->song->sessionClips) {
      ...
      // be grabbed for another Clip - and also so overall soloing may be
      // cancelled if no others soloing
      ...
      clipArray->deleteAtIndex(clipIndex); // Deletes the array's pointer to the
                                           // Clip - not the Clip itself.
    }

    // This next call will back up all ParamManagers, including for Drums.
    // But it will (unusually) leave clip->output pointing to the Output, and
    // the same for any NoteRows' Drums. So that we can revert this stuff later.
    ...
    // ------------------------------------------------
    oldOutput->pickAnActiveClipIfPossible(modelStack); // Yup, we're required to call this after
                                                       // detachFromOutput().
  }
