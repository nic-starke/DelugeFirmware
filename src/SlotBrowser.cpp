...
// Todo: turn this into the open() function - which will need to also be able to
// return error codes?
int SlotBrowser::beginSlotSession(bool shouldDrawKeys, bool allowIfNoFolder) {

  currentFileHasSuffixFormatNameImplied = false;

  // We want to check the SD card is generally working here, so that if not, we
  // can exit out before drawing the QWERTY keyboard.
  int error = storageManager.initSD();
  if (error) return error;

  // But we won't try to open the folder yet, because we don't yet know what it
  // should be.
...
int SlotBrowser::horizontalEncoderAction(int offset) {
  ...
  if (currentFileItem) {
    // See if it's numeric. Here, filename has already had prefix removed if
    // it's numeric.
...
*/
// This gets called if you're gonna load the thing, or have turned the select
// knob to navigate, so these functions can treat it as a numeric format name
