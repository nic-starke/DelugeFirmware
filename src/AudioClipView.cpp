...
bool AudioClipView::renderMainPads(uint32_t whichRows,
  ...
  int endSquareDisplay =
      divide_round_negative(getClip()->loopLength - currentSong->xScroll[NAVIGATION_CLIP] - 1,
  ...
  // Or if yes Sample...
    ...
    getClip()->getScrollAndZoomInSamples(currentSong->xScroll[NAVIGATION_CLIP],
...
void AudioClipView::graphicsRoutine() {
  ...
  // Tempoless or arranger recording
           ...
           (currentPlaybackMode == &arrangement &&
            currentSong->currentClip->getCurrentlyRecordingLinearly())) {
    newTickSquare = displayWidth - 1;

    // Linearly recording
    if (currentSong->currentClip->getCurrentlyRecordingLinearly()) { // This would have to be true
                                                                     // if we got here, I think?
      ((AudioClip*)currentSong->currentClip)->renderData.xScroll =
          -1; // Make sure values are recalculated
    ...
    if (getClip()->getCurrentlyRecordingLinearly()) { needsRenderingDependingOnSubMode(); }
  ...
  if (PadLEDs::flashCursor != FLASH_CURSOR_OFF &&
    ...
    const uint8_t* colours =
        currentSong->currentClip->getCurrentlyRecordingLinearly() ? twos : zeroes;
...
void AudioClipView::transitionToSessionView() {
  ...
  if (!getClip() || !getClip()->sampleHolder.audioFile) { // !getClip() probably couldn't
    ...
    waveformRenderer.collapseAnimationToWhichRow =
        sessionView.getClipPlaceOnScreen(currentSong->currentClip);
    ...
    if (Buttons::isButtonPressed(tempoEncButtonX, tempoEncButtonY)) {
      ...
      if (soundEditorResult != ACTION_RESULT_NOT_DEALT_WITH) {
        ...
        int endSquareDisplay = divide_round_negative(
            ...
            currentSong->xZoom[NAVIGATION_CLIP]); // Rounds it well down, so we get the
                                                  // "final square" kinda...
        ...
        if (endMarkerVisible) {
          ...
          // Otherwise, move it
            ...
            if (sample) {
              ...
              // Ok, move the marker!
              int newLength = (x + 1) * currentSong->xZoom[NAVIGATION_CLIP] +
              ...
              uint64_t newLengthSamples =
                  (uint64_t)(oldLengthSamples * newLength + (oldLength >> 1)) /
              ...
              if (clip->sampleControls.reversed) {
                ...
                // If the end pos is very close to the end pos marked in the
                // audio file, assume some rounding happened along the way and
              ...
              int actionType = (newLength < oldLength) ? ACTION_CLIP_LENGTH_DECREASE :
                                                         ACTION_CLIP_LENGTH_INCREASE;

              // Change sample end-pos value. Must do this before calling
              // setClipLength(), which will end up reading this value.
...
void AudioClipView::playbackEnded() {
  ...
  // A few reasons we might want to redraw the waveform. If a Sample had only
  // partially recorded, it will have just been discarded. Or, if tempoless or
  // arrangement recording, zoom and everything will have just changed
  uiNeedsRendering(this, 0xFFFFFFFF, 0);
}
...
void AudioClipView::clipNeedsReRendering(Clip* clip) {
  if (clip == getClip()) {
    ...
    // Scroll back left if we need to - it's possible that the length just
    // reverted, if recording got aborted. Ok, coming back to this, it seems it
    // was a bit hacky that I put this in this function...
...
void AudioClipView::sampleNeedsReRendering(Sample* sample) {
  if (sample == getSample()) { uiNeedsRendering(this, 0xFFFFFFFF, 0); }
}
...
int AudioClipView::verticalEncoderAction(int offset, bool inCardRoutine) {
  if (!currentUIMode && Buttons::isShiftButtonPressed() &&
      !Buttons::isButtonPressed(yEncButtonX, yEncButtonY)) {
