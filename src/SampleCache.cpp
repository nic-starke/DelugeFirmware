...
SampleCache::SampleCache(Sample* newSample,
  ...
  for (int i = 0; i < numClusters; i++) {
      clusters[i] = NULL; // We don't actually have to initialize these, since
  writeBytePos tells us how many are "valid"
  }
  */
}
...
void SampleCache::clusterStolen(int clusterIndex) {
  ...
  unlinkClusters(clusterIndex + 1,
                 false); // Must do this before changing writeBytePos

  uint8_t bytesPerSample = sample->numChannels * CACHE_BYTE_DEPTH;

  writeBytePos = (uint32_t)((uint32_t)((clusterIndex << audioFileManager.clusterSizeMagnitude) +
                                       bytesPerSample - 1) /
                            bytesPerSample) *
                 bytesPerSample; // Make it a multiple of bytesPerSample - but round up.
                                 // If you try and simplify this, make sure it still works
                                 // for 0 and doesn't go negative or anything!
  ...
  if (numExistentClusters != clusterIndex) { numericDriver.freezeWithError("E295"); }
  clusters[clusterIndex] = NULL; // No need to remove this first Cluster from a queue or anything -
                                 // that's already all done by the thing that's stealing it
#endif
}
...
// You must be sure before calling this that newWriteBytePos is a multiple of
// (sample->numChannels * CACHE_BYTE_DEPTH)
void SampleCache::setWriteBytePos(int newWriteBytePos) {
  ...
  // Remember, a cache cluster actually gets (bytesPerSample - 1) extra usable
  // bytes after it.
...
// Does not move the new Cluster to the appropriate "availability queue",
// because it's expected that the caller is just about to call getCluster(), to
// get it, which will call prioritizeNotStealingCluster(), and that'll do it
bool SampleCache::setupNewCluster(int clusterIndex) {
  ...
  clusters[clusterIndex] = audioFileManager.allocateCluster(
      ...
      false,
      this);                     // Do not add reasons, and don't steal from this SampleCache
...
void SampleCache::prioritizeNotStealingCluster(int clusterIndex) {
  ...
  // This ensures, one Cluster at a time, that this Cache's Clusters are right
  // at the far end of their queue (so won't be stolen for a while), but in
  // reverse order so that the later-in-sample of those cache Clusters will be
  // stolen first

  // Remember, cache clusters never have "reasons", so we can assume these are
  // already in one of the stealableClusterQueues, ready to be "stolen".

  // First Cluster
  if (clusterIndex == 0) {
    if (clusters[clusterIndex]->list !=
            &generalMemoryAllocator.regions[MEMORY_REGION_SDRAM].stealableClusterQueues
                 [STEALABLE_QUEUE_CURRENT_SONG_SAMPLE_DATA_REPITCHED_CACHE] ||
        !clusters[clusterIndex]->isLast()) {

      clusters[clusterIndex]->remove(); // Remove from old list, if it was
                                        // already in one (might not have been).
      ...
      generalMemoryAllocator.regions[MEMORY_REGION_SDRAM].stealableClusterQueueLongestRuns
          [STEALABLE_QUEUE_CURRENT_SONG_SAMPLE_DATA_REPITCHED_CACHE] =
  ...
  // Later Clusters
    ...
    // In most cases, we'll want to do this thing to alter the ordering -
    // including if the Cluster in question hasn't actually been added to a
    // queue at all yet, because this functions serves the additional purpose of
...
int SampleCache::getNumExistentClusters(int32_t thisWriteBytePos) {
  ...
  // Remember, a cache Cluster actually gets (bytesPerSample - 1) extra usable
  // bytes after it.
  int numExistentClusters = (thisWriteBytePos + audioFileManager.clusterSize - bytesPerSample) >>
