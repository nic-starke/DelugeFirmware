...
public:
  ...
  bool init(Sample*              sample,
            ...
            int                  loopingType);
  void reInit(int64_t              newSamplePosBig,
              SamplePlaybackGuide* guide,
              VoiceSample*         voiceSample,
              Sample*              sample,
              int                  numChannels,
  ...
  bool hopEnd(SamplePlaybackGuide* guide,
              VoiceSample*         voiceSample,
              Sample*              sample,
              int                  numChannels,
              int32_t              timeStretchRatio,
  ...
  void readFromBuffer(int32_t* oscBufferPos,
                      int      numSamples,
                      int      numChannels,
  ...
  void reassessWhetherToBeFillingBuffer(int32_t phaseIncrement,
                                        int32_t timeStretchRatio,
  ...
  int64_t samplePosBig; // In whole samples including both channels. From
                        // audioDataStart. <<'d by 24
  ...
  Cluster* percCacheClustersNearby[2]; // Remembers and acts as a "reason" for
                                       // the two most recently needed /
                                       // accessed Clusters, basically

private:
  bool setupNewPlayHead(Sample*              sample,
                        VoiceSample*         voiceSample,
                        SamplePlaybackGuide* guide,
                        int                  newHeadBytePos,
