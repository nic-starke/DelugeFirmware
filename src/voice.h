...
public:
  ...
  VoiceUnisonPart unisonParts[maxNumUnison];    // Stores all oscillator positions and stuff,
                                                // for each Source within each Unison too
  VoiceSamplePlaybackGuide guides[NUM_SOURCES]; // Stores overall info on each Source (basically
                                                // just sample memory bounds), for the play-through
                                                // associated with this Voice right now.

  Sound* assignedToSound;

  int32_t paramFinalValues[FIRST_GLOBAL_PARAM]; // This is just for the *local*
                                                // params, specific to this
                                                // Voice only
  int32_t sourceValues[NUM_PATCH_SOURCES];      // At the start of this list are
                                                // local copies of the "global" ones.
                                                // It's cheaper to copy them here
                                                // than to pick and choose where the
                                                // Patcher looks for them
  ...
  int       inputCharacteristics[2]; // Contains what used to be called
                                     // noteCodeBeforeArpeggiation, and
                                     // fromMIDIChannel
  ...
  bool render(ModelStackWithVoice* modelStack,
              int32_t*             soundBuffer,
              int                  numSamples,
  ...
  bool     noteOn(ModelStackWithVoice* modelStack,
  ...
  void     changeNoteCode(ModelStackWithVoice* modelStack,
                          int                  newNoteCodeBeforeArpeggiation,
                          int                  newNoteCodeAfterArpeggiation,
...
private:
  // inline int32_t doFM(uint32_t *carrierPhase, uint32_t* lastShiftedPhase,
  // uint32_t carrierPhaseIncrement, uint32_t phaseShift);

  void renderOsc(int       s,
                 int       type,
                 int32_t   amplitude,
                 int32_t*  thisSample,
                 int32_t*  bufferEnd,
  ...
  void renderBasicSource(Sound*                   sound,
                         ParamManagerForTimeline* paramManager,
                         int                      s,
                         int32_t*                 oscBuffer,
                         int                      numSamples,
                         int32_t                  sourceAmplitude,
                         bool*                    unisonPartBecameInactive,
                         int32_t                  overallPitchAdjust,
                         bool                     doOscSync,
                         uint32_t*                oscSyncPos,
                         uint32_t*                oscSyncPhaseIncrements,
                         int32_t                  amplitudeIncrement,
  ...
  void    renderSineWaveWithFeedback(int32_t*  thisSample,
                                     int       numSamples,
                                     uint32_t* phase,
                                     int32_t   amplitude,
                                     ...
                                     bool      add,
                                     int32_t   amplitudeIncrement);
  void    renderFMWithFeedback(int32_t*  thisSample,
                               int       numSamples,
                               int32_t*  fmBuffer,
                               uint32_t* phase,
                               int32_t   amplitude,
                               ...
                               int32_t*  lastFeedbackValue,
                               int32_t   amplitudeIncrement);
  void    renderFMWithFeedbackAdd(int32_t*  thisSample,
                                  int       numSamples,
                                  int32_t*  fmBuffer,
                                  uint32_t* phase,
                                  int32_t   amplitude,
