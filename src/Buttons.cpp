...
int buttonAction(int x, int y, bool on, bool inCardRoutine) {
  ...
  buttonStates[x][y] = on; // Must happen up here before it's actioned, because if its action
                           // accesses SD card, we might multiple-enter this function, and don't
                           // want to then be setting this after that later action, erasing what
                           // it set
    ...
    if (on) {
      if (isShiftButtonPressed())
        ...
        if (getCurrentUI() != &loadSongUI) playbackHandler.displayTempoByCalculation();
           ...
           isButtonPressed(clipViewButtonX, clipViewButtonY) &&
...
void noPressesHappening(bool inCardRoutine) {
  for (int x = 0; x < NUM_BUTTON_COLS; x++) {
    for (int y = 0; y < NUM_BUTTON_ROWS; y++) {
      if (buttonStates[x][y]) { buttonAction(x, y, false, inCardRoutine); }
    }
  }
}
