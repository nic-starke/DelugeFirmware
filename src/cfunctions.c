...
void intToString(int32_t number, char* __restrict__ buffer, int minNumDigits) {
  ...
  if (isNegative)
    number = (uint32_t)0 - (uint32_t)number; // Can't just go "-number", cos somehow that
                                             // doesn't work for negative 2 billion!
...
void floatToString(float number,
  ...
  if (exponent >= -5) {
    ...
    fractionRemaining >>=
        (-5 - exponent); // Yes, in this case, we throw away bits and lose accuracy.
  }
  ...
  while (true) {
    ...
    if (decimalPlace >= maxNumDecimalPlaces) { break; }
  ...
  *writePos = 0;
  ...
  if (fractionRemaining >= (uint32_t)0x08000000) {
    oldEndPos = writePos;
  moveBackOneDigit:
    // If we reached left of string, oh no, we can't move back any further. So
    // move everything else instead.
  ...
  // Or if not rounding up, we still may have a string of zeros on the end,
  // which may go above our min decimal places, so we should lose those.
  else {
    while (true) {
      writePos--;
      if (decimalPlace <= minNumDecimalPlaces) {
        if (*writePos == '.')
          *writePos = 0; // If min decimal places was 0 and we got that far
                         // back, get rid of the decimal point.
        break;
      }
      decimalPlace--;
      if (*writePos != '0') break;
