...
// This function will be called repeatedly, at all times, to see if it's time to
// do a tick, and such
void PlaybackHandler::routine() {
  ...
  if (triggerClockRisingEdgesProcessed != triggerClockRisingEdgesReceived) {
    uint32_t time = triggerClockRisingEdgeTimes[triggerClockRisingEdgesProcessed &
                                                (TRIGGER_CLOCK_INPUT_NUM_TIMES_STORED - 1)];
    triggerClockRisingEdgesProcessed++;
    analogClockRisingEdge(time);
  }

  // If we're playing synced to analog clock input, auto-start mode, and there
  // hasn't been a rising edge for a while, then stop
  if ((playbackState & PLAYBACK_CLOCK_EXTERNAL_ACTIVE) && usingAnalogClockInput &&
      analogClockInputAutoStart &&
      (int32_t)(AudioEngine::audioSampleTimer - timeLastAnalogClockInputRisingEdge) >
          (44100 >> 1)) {
    endPlayback();
  }
}
...
void PlaybackHandler::slowRoutine() {
  // See if any MIDI commands are pending which couldn't be actioned before (see
  // comments in tryGlobalMIDICommands())
...
void PlaybackHandler::playButtonPressed(int buttonPressLatency) {
  ...
  // Or if currently playing...
    ...
    // Or, normal
      ...
      // If playing synced to analog clock input and it's on auto-start mode,
      // don't let them stop, because it'd just auto-start again
      if (playbackHandler.playbackState & PLAYBACK_CLOCK_EXTERNAL_ACTIVE) {
        if (usingAnalogClockInput && analogClockInputAutoStart) { return; }
...
static const uint32_t recordButtonUIModes[] = {UI_MODE_HORIZONTAL_ZOOM,
...
void PlaybackHandler::setupPlaybackUsingInternalClock(int buttonPressLatency, bool allowCountIn) {
  ...
  decideOnCurrentPlaybackMode(); // Must be done up here - we reference
                                 // currentPlaybackMode a bit below
  ...
  // Remember that the "next" tick (the one we're gonna do right now) is to
  // happen at now-time
  ...
  if (currentlySendingMIDIOutputClocks()) {
    if (newPos) {                                          // "continue"
      sendOutPositionViaMIDI(newPos, !ticksLeftInCountIn); // "Continue" message will only be sent
                                                           // if no count-in
    } else {                                               // "start"
      if (!doingTempolessRecord && !ticksLeftInCountIn) { midiEngine.sendStart(); }
    }
  }
  ...
  // The next timer tick will be 0, and we need to set this here because the
  // call to setupPlayback(), below, may well do things which need to look at
  // the current internal tick count (e.g. AudioClip::resume()), and that reads
  // from this.
  nextTimerTickScheduled = 0;

  setupPlayback(newPlaybackState, newPos, true, true, buttonPressLatency);

  timeNextTimerTickBig = (uint64_t)AudioEngine::audioSampleTimer
                         << 32; // Set this *after* calling setupPlayback, which will call the
                                // audio routine before we do the first tick

  swungTicksTilNextEvent = 0; // Need to ensure this, here, otherwise, it'll be set to some weird
                              // thing by some recent call to expectEvent()
}
...
void PlaybackHandler::setupPlayback(int     newPlaybackState,
                                    ...
                                    int     buttonPressLatencyForTempolessRecord) {

  actionLogger.closeAction(ACTION_RECORD);

  if (shouldShiftAccordingToClipInstance && currentPlaybackMode == &arrangement &&
  ...
  swungTicksTilNextEvent = 0; // Until recently (Aug 2019) I did not do this here for some reason I
                              // can't remember and insanely most stuff worked

  playbackState = newPlaybackState;
  cvEngine.playbackBegun(); // Call this *after* playbackState is set. If
                            // there's a count-in, nothing will happen

  if (getRootUI() && getCurrentUI() == getRootUI()) { getRootUI()->notifyPlaybackBegun(); }
  ...
  if (doOneLastAudioRoutineCall) {
    // Bit sneaky - we want to call the audio routine one last time, but need it
    // to think that playback's not happening so it doesn't do the first tick
    // yet
    playbackState = 0;
  ...
  // May not call audio routine during this, cos that'd try doing ticks before
  // everything's set up
  ...
  currentPlaybackMode->resetPlayPos(
      playFromPos,
      !ticksLeftInCountIn,
      buttonPressLatencyForTempolessRecord); // Have to do this after calling
                                             // AudioEngine::routine()
...
void PlaybackHandler::endPlayback() {
  ...
  bool shouldDoInstantSongSwap =
      currentPlaybackMode->endPlayback(); // Must happen after
                                          // currentSong->endInstancesOfActiveClips() is called
                                          // (ok I can't remember why I wrote that, and now it
                                          // needs to happen before so that
                                          // Clip::beingRecordedFrom is still set when playback
                                          // ends, so notes stop)

  playbackState = 0;        // Do this after calling currentPlaybackMode->endPlayback(), cos for
                            // arrangement that has to get the current tick, which needs to refer
                            // to which clock is active, which is stored in playbackState.
  ...
  // Or if not doing a song swap, some UI stuff to do
    ...
    if (currentSong && getRootUI()) { getRootUI()->playbackEnded(); }
  }
...
void PlaybackHandler::getMIDIClockOutTicksToInternalTicksRatio(uint32_t* internalTicksPer,
  ...
  *internalTicksPer = 1; // 12 rather than 24 so we get twice as many ticks,
                         // because only every 2nd one is rising-edge
...
void PlaybackHandler::scheduleNextTimerTick(uint32_t doubleSwingInterval) {
  ...
  uint64_t timeTilNextBig = currentSong->timePerTimerTickBig *
                            (uint64_t)doubleSwingInterval; // To improve: this can overflow if tempo
                                                           // down around 0.001 BPM
...
// 2nd part of timer tick action. We need to be able to call just this part if a
// song swap has occurred *not* at the same time as a timer tick
void PlaybackHandler::actionTimerTickPart2() {
  ...
  // But if count-in happening, limit it to one bar's length (or perhaps this
  // should be one beat, to avoid slight screwiness with very long swing
  // intervals?)
  ...
  if (!ticksLeftInCountIn) {
    ...
    maybeDoTriggerClockOutputTick:
      ...
      scheduleTriggerClockOutTickParamsKnown(analogOutTicksPer,
                                             fractionLastTimerTick,
    ...
    maybeDoMIDIClockOutputTick:
      ...
      scheduleMIDIClockOutTickParamsKnown(midiClockOutTicksPer,
                                          fractionLastTimerTick,
...
void PlaybackHandler::scheduleTriggerClockOutTickParamsKnown(uint32_t analogOutTicksPer,
                                                             uint64_t fractionLastTimerTick,
  ...
  if (fractionNextAnalogOutTick < nextTimerTickScheduled * analogOutTicksPer) {
    ...
    timeNextTriggerClockOutTick =
        (timeLastTimerTickBig + ((uint64_t)((fractionNextAnalogOutTick - fractionLastTimerTick) *
...
void PlaybackHandler::scheduleMIDIClockOutTickParamsKnown(uint32_t midiClockOutTicksPer,
                                                          uint64_t fractionLastTimerTick,
  ...
  if (fractionNextMIDIClockOutTick < nextTimerTickScheduled * midiClockOutTicksPer) {
    ...
    timeNextMIDIClockOutTick =
        (timeLastTimerTickBig + ((uint64_t)((fractionNextMIDIClockOutTick - fractionLastTimerTick) *
                                            currentSong->timePerTimerTickBig)) /
...
void PlaybackHandler::actionSwungTick() {
  ...
  if (ticksLeftInCountIn) {
    ...
    if (!ticksLeftInCountIn) {
      ...
      // May not call audio routine during this, cos that'd try doing ticks
      // before everything's set up
      currentPlaybackMode->resetPlayPos(
          posToNextContinuePlaybackFrom); // Have to do this after calling
                                          // AudioEngine::routine()
      ...
      if (currentlySendingMIDIOutputClocks()) {
        if (posToNextContinuePlaybackFrom) {
          midiEngine.sendContinue(); // Position pointer was already sent when
                                     // "play" pressed
        } else {
    ...
    // Or if there was still more count-in left...
      ...
      int newVisualCount =
          (uint32_t)(ticksLeftInCountIn - 1) / currentSong->getQuarterNoteLength() + 1;
  ...
  swungTickIncrement = swungTicksTilNextEvent; // This might have got reset to 0
                                               // if there was a song swap
  swungTicksTilNextEvent = 2147483647;

  if (isEitherClockActive()) { // Occasionally, considerLaunchEvent() will stop
                               // playback
    currentPlaybackMode->doTickForward(swungTickIncrement);

    if (isEitherClockActive()) { // Occasionally, doTickForward() will stop
                                 // playback

      // If we swapped song on just a swung tick that wasn't concurrent with a
      // timer tick (unusual), we need to go do some stuff that would normally
      // happen as part of the timer tick
      if (swappedSong && (playbackState & PLAYBACK_CLOCK_INTERNAL_ACTIVE) &&
          !currentlyActioningTimerTick) {
        actionTimerTickPart2();
      }
      ...
      doMetronome:
        ...
        if ((currentMetronomeTick % swungTicksPerQuarterNote) == 0) {
          uint32_t phaseIncrement =
              ((currentMetronomeTick % (swungTicksPerQuarterNote << 2)) == 0) ? 128411753 :
                                                                                50960238;
...
// This may be called when there already is one scheduled, to reschedule it, if
...
int PlaybackHandler::getNumSwungTicksInSinceLastTimerTick(uint32_t* timeRemainder) {
  ...
  // If first timer tick not actioned yet (currently the only function that
  // calls this function already separately deals with this though)...
  ...
  if (currentSong->hasAnySwing()) {
    ...
    uint32_t timePassedFiddledWith =
        (uint32_t)(timePassed * 50) / (uint32_t)(50 + currentSong->swingAmount);
    uint32_t ticksIn = timePassedFiddledWith / timePerTimerTick;
    if (ticksIn < swingInterval) { return ticksIn; }

    // Or if we're still here, it's in the second half
    uint32_t timeTilNextTimerTick =
        (uint32_t)(timeNextTimerTickBig >> 32) - AudioEngine::audioSampleTimer;
    ...
    int ticksTilEnd =
        (uint32_t)(timeTilNextFiddledWith - 1) / (uint32_t)timePerTimerTick + 1; // Rounds up.
...
int PlaybackHandler::getNumSwungTicksInSinceLastActionedSwungTick(uint32_t* timeRemainder) {
  if (currentlyActioningSwungTickOrResettingPlayPos) {
    if (timeRemainder) *timeRemainder = 0;
    return 0; // This will save some time, even though it was already returning
              // the correct result.
  }
...
int64_t PlaybackHandler::getActualSwungTickCount(uint32_t* timeRemainder) {
  ...
  if (playbackState & PLAYBACK_CLOCK_INTERNAL_ACTIVE) {
    ...
    // If first timer tick not actioned yet (not sure if we ever get called in
    // this case, though)...
    if (!nextTimerTickScheduled) {
      if (timeRemainder) *timeRemainder = 0;
      return 0;
  ...
  // External clock
    ...
    // Yes swing
      ...
      uint64_t startOfSwingBlock =
          (uint64_t)currentInternalTick / doubleSwingInterval * doubleSwingInterval;
      float posWithinSwingBlock = currentInternalTick - startOfSwingBlock;

      // First, see if we're in the first half still
      float swungTicksIn =
          (uint32_t)(posWithinSwingBlock * 50) / (uint32_t)(50 + currentSong->swingAmount);
      ...
      // Or, if we're in the second half
        ...
        actualSwungTick =
            startOfSwingBlock + doubleSwingInterval - (swungTicksTilEnd + 1); // Round that bit up
      }
    }
  }
  ...
  if (actualSwungTick < lastSwungTickActioned) {
    ...
    if (nextSwungTickToAction // Checking for special case when
                              // nextSwungTickToAction == 0, when playback first
                              // starts. Unchecked, that would sometimes lead to
                              // us returning -1 when following external clock.
...
int64_t PlaybackHandler::getCurrentInternalTickCount(uint32_t* timeRemainder) {
  ...
  if (playbackState & PLAYBACK_CLOCK_INTERNAL_ACTIVE) {
    ...
    // If no timer ticks have occurred yet, the answer is a resounding zero, and
    // we have to have this as a special case because timeLastTimerTickBig won't
    // have been set yet. This will happen all the time during playback setup,
    // e.g. in AudioClip::resumePlayback()
    if (!nextTimerTickScheduled) {
      if (timeRemainder) *timeRemainder = 0;
      internalTickCount = 0;
    }
    ...
    // Or, the normal case - calculate the answer
      ...
      uint32_t timeSinceLastTimerTick =
          AudioEngine::audioSampleTimer - (uint32_t)(timeLastTimerTickBig >> 32);
      ...
      // Safety against rounding errors - make sure we don't give an internal
      // tick count that's less than a swung tick we already actioned. I've seen
      // this happen (or at least become apparent) when at 6144 resolution with
      // very shortened audio clip.
  ...
  if (internalTickCount < 0)
    numericDriver.freezeWithError(
        "E429"); // Trying to narrow down "nofg" error, which Ron got most
                 // recently (Nov 2021). Wait no, he didn't have playback on!
#endif
...
float PlaybackHandler::getCurrentInternalTickFloatFollowingExternalClock() {
  ...
  // If we've only actually received one (or none - is that possible?) input
  // tick...
  if (lastInputTickReceived <= 0) {
    // We're before it, which won't make sense to the caller, so just say we're
    // at 0
    return 0;
  }
  ...
  // If that input tick hasn't "happened" yet and is currently just scheduled to
  // happen soon, then the current internal tick is before it
  ...
  goAgain:
    ...
    if (timeSincePreviousInputTick < 0) {
      ...
      if (t >= lastInputTickReceived)
        return 0; // If all the input ticks received so far have not yet
                  // "happened"
      if (t >= NUM_INPUT_TICKS_FOR_MOVING_AVERAGE - 1) { // If we just didn't remember that far back
    ...
    // Just see how far apart the last two received input ticks were (even
    // though we haven't actually actioned the most recent one yet)
    int32_t timeBetweenInputTicks = timeLastInputTicks[t] - timeLastInputTicks[t + 1];

    // Should now be impossible for them to be at the same time, since we should
    // be looking at one in the future and one not
    if (ALPHA_OR_BETA_VERSION && timeBetweenInputTicks <= 0) numericDriver.freezeWithError("E337");

    currentInputTick = (float)timeSincePreviousInputTick / (uint32_t)timeBetweenInputTicks +
                       lastInputTickReceived - t - 1;
  }
  ...
  // Or if it has happened...
    ...
    currentInputTick =
        (float)timeSinceLastInputTick / timePerInputTickMovingAverage + lastInputTickReceived;
  }
...
int32_t PlaybackHandler::getInternalTickTime(int64_t internalTickCount) {
  ...
  if (playbackState & PLAYBACK_CLOCK_INTERNAL_ACTIVE) {
    ...
    // If first timer tick hasn't even occurred yet, various values will not yet
    // be valid. The time of the first tick will not even have been decided. So,
    // use audioDriver.audioSampleTimer in its place, since that is what our
    // returned value will be compared to.
    if (!nextTimerTickScheduled) {
    ...
    return (int64_t)(timeLastTimerTickBig +
                     (int64_t)currentSong->timePerTimerTickBig * numTicksAfterLastTimerTick) >>
           32;
  }
  ...
  // External clock
    ...
    return (int32_t)timeLastInputTicks[0] + (int32_t)((inputTickCount - lastInputTickReceived) *
                                                      (int32_t)timePerInputTickMovingAverage);
  }
}
...
void PlaybackHandler::doSongSwap(bool preservePlayPosition) {
  ...
  if (currentSong) {
    ...
    if (isEitherClockActive()) {
      ...
      // If we're preserving the tempo (either cos we chose to or because we're
      // following external clock)
      if ((playbackHandler.playbackState & PLAYBACK_CLOCK_EXTERNAL_ACTIVE) ||
          songSwapShouldPreserveTempo) {

        // Since we're currentlyPlaying, we know that there still is a
        // currentSong

        // We want to keep the old song's tempo. We need to account for the fact
        // that the two songs might have different magnitudes though, before we
        // do anything like copy the timePerTimerTick. So we apply any
        // difference in magnitude to the timePerTimerTick before we copy /
        // compare it.
        ...
        // If tempo magnitude matching, we now need to look at how similar the
        // (potentially modified) timePerTimerTick's are, and shift magnitudes
      ...
      // Any MIDI or gate notes wouldn't be stopped by our unassignAllVoices()
      // call below
  ...
  // Some more "if we're playing" stuff - this needs to happen after currentSong
  // is swapped over, because resyncInternalTicksToInputTicks() references it
  if (isEitherClockActive()) {

    // If beginning in arranger, not allowed to preserve play position (the
    // caller never actually tries to do this anyway)
    if (currentSong->lastClipInstanceEnteredStartPos != -1) { preservePlayPosition = false; }
    ...
    // Otherwise, reset play position
      ...
      swungTicksTilNextEvent = 0; // Set it so the tick which we're gonna do right now, at the start
                                  // of the new song, has 0 increment
    }

    // And now, if switching to arranger (in which case, remember, we definitely
    // didn't preserve play position), do that
    ...
    // Or if we weren't switching to the arranger, the equivalent of that would
    // get called from Session::considerLaunchEvent()
  }
...
#else
  numericDriver.displayLoadingAnimation(); // More loading might need to happen
                                           // now, or still be happening
#endif
...
void PlaybackHandler::analogClockRisingEdge(uint32_t time) {
  ...
  // If not already playing, start now, if set to auto-start. But not if the
  // settings haven't yet been read
  if (!playbackState) {
  ...
  if (playbackState) { inputTick(true, time); }

  timeLastAnalogClockInputRisingEdge = AudioEngine::audioSampleTimer;
}

void PlaybackHandler::setupPlaybackUsingExternalClock(bool switchingFromInternalClock,
  ...
  if (switchingFromInternalClock) {
    ...
    lastInputTickReceived =
        round((uint64_t)getCurrentInternalTickCount() * inputTicksPer / internalTicksPer) - 1;
  ...
  // Tempo magnitude matching just wouldn't really make sense if playing from a
  // "continue" command, because each time we realise the tempo is actually
  // double or half, that'd mean we'd have to re-interpret the start-position at
  // the new magnitude, so completely jump the play-cursor to a different place,
  // whose first beat we already missed hearing. Also, Logic, and for that
  // matter the Deluge itself, when outputting a "continue", will output several
  // "clock" messages simultaneously to achieve a finer resolution start
  // position, and when hearing these, well, this looks like infinite tempo.
  ...
  slowpassedTimePerInternalTick = stickyTimePerInternalTick
                                  << slowpassedTimePerInternalTickSlowness;

  // Imagine the user had just hit the play button - how fast would we expect to
  // see internal ticks happening?
  uint64_t targetedTimePerInternalTick = stickyTimePerInternalTick;

  // Convert this to input tick time - and that's how often we're expecting to
  // see input ticks.
  uint32_t internalTicksPer;
  uint32_t inputTicksPer;
  getInternalTicksToInputTicksRatio(&inputTicksPer, &internalTicksPer);
  ...
  // If we don't yet know the time per input tick, go with the estimation we
  // just made
  ...
  if (!switchingFromInternalClock) {
    bool shouldShiftAccordingToClipInstance =
        (!fromContinueCommand && posToNextContinuePlaybackFrom == 0);
    decideOnCurrentPlaybackMode();
    setupPlayback(newPlaybackState,
                  posToNextContinuePlaybackFrom,
                  false,
                  shouldShiftAccordingToClipInstance);
    posToNextContinuePlaybackFrom = 0;
  } else {
    playbackState = newPlaybackState;
    setLedStates();
...
void PlaybackHandler::positionPointerReceived(uint8_t data1, uint8_t data2) {
  ...
  // If following external clock right now, jump to the position (actually 1
  // tick before the position, so the next "clock" message will play that pos)
  if (playbackState & PLAYBACK_CLOCK_EXTERNAL_ACTIVE) {

    // But, to get around a weird "bug" with Ableton and my Mbox, don't do this
    // if pos is 0 and we've only done the first input tick
    if (pos == 0 && lastInputTickReceived == 0) return;

    currentPlaybackMode->resetPlayPos((int32_t)pos);
  }

  // Otherwise, even if playing to internal clock, just store the position, to
  // use if we receive a "continue" message
  else {
...
void PlaybackHandler::startMessageReceived() {
  ...
  if (playbackState) {
    ...
    // If we received this message only just after having started playback
    // ourself, assume we're getting our output echoed back to us, and just
    // ignore it
    if (startIgnoringMidiClockInputIfNecessary()) return;

    // Otherwise, end our internal playback so we can start playing following an
    // external clock.
    endPlayback();
  }
...
void PlaybackHandler::continueMessageReceived() {
  ...
  if (playbackState) {
    ...
    // If already following external clock, there's nothing to do: if we already
    // received a song position pointer, that will have already taken effect
  ...
  usingAnalogClockInput = false;
  // posToNextContinuePlaybackFrom is left at whatever value it's at - playback
...
void PlaybackHandler::stopMessageReceived() {
  if (ignoringMidiClockInput || !midiInClockEnabled) return;
  if (playbackState & PLAYBACK_CLOCK_EXTERNAL_ACTIVE) { endPlayback(); }
}
...
void PlaybackHandler::clockMessageReceived(uint32_t time) {
  ...
  if (playbackState) { inputTick(false, time); }
}
...
void PlaybackHandler::scheduleSwungTickFromExternalClock() {
  ...
  if (!currentSong->hasAnySwing()) {
    ...
    internalTickPositionForNextSwungTickTimes50 =
        internalTickWithinIntervalTimes50 + (startOfSwingInterval * 50);
  }
  ...
  // If this next swung tick is gonna happen before we're going to receive the
  // *next* input tick (possibly right on this current input tick that we've
  // just received but haven't actually processed yet)...
  if (inputTickPositionForNextSwungTickTimes50TimesInternalTicksPer <
    ...
    // TODO: if we're scheduling a swung tick which corresponds to a couple of
    // input ticks back in time, which could happen if they haven't actually
    // "happened" yet and only been "received", I feel like the current method
    // would be quite inexact and we should be looking at
    // timeLastInputTicks[some_higher_number]. Though would that do less
    // smoothing out of jitter?

    // Another important note here:
    // inputTickFractionTimes50TimesInternalTicksPer may be negative because the
    // scheduled swung tick might be further back in time than the scheduled
    // time of the most recently received input tick - I think particularly for
    // very high resolution songs? This is fine, and even if we end up with a
    // time which has already passed, the swung tick will still get actioned on
    // the next audio routine call. The key point is just that the below
    // calculation must correctly deal with negative numbers.
    scheduledSwungTickTime =
...
void PlaybackHandler::inputTick(bool fromTriggerClock, uint32_t time) {
  ...
  // If we were using the internal clock, we want to start again using the
  // external clock, kinda
  if (playbackHandler.playbackState & PLAYBACK_CLOCK_INTERNAL_ACTIVE) {

    // If we're in our count-in, that's incompatible with following external
    // clock, so we'll just have to ignore the incoming clock for now. This has
    // the slightly weird effect that after the count-in is over, we'll switch
    // to following external clock, so tempo might abruptly change. But that's
    // probably better than just permanently ignoring incoming clock, which
    // could create more unclearness.
  ...
  // The 40 here is a fine-tuned amount to stop everything wrapping wrong when
  // CPU load heavy. 28 to 98 seemed to work correctly
  if (time)
    timeTilInputTick = (((uint32_t)(time - (uint32_t)AudioEngine::i2sTXBufferPos) >>
                         (2 + NUM_MONO_OUTPUT_CHANNELS_MAGNITUDE)) +
                        40) &
  ...
  if (tempoMagnitudeMatchingActiveNow) {
    if (lastInputTickReceived == -1)
      timeVeryFirstInputTick = timeThisInputTick; // Remember, lastInputTickReceived hasn't been
                                                  // increased yet for the input-tick we're
      ...
      uint32_t expectedTimeSinceVeryFirst =
          ...
          (lastInputTickReceived + 1); // Remember, lastInputTickReceived hasn't been increased yet
                                       // for the input-tick we're processing right now
      if (expectedTimeSinceVeryFirst < 1) expectedTimeSinceVeryFirst = 1;

      // If input ticks coming in too slow (and not about to overflow any
      // numbers)...
      ...
      // If input ticks coming in too fast (and not about to make any numbers
      // 0)...
      while (targetedTimePerInputTick > 1 &&
             timeSinceVeryFirst < expectedTimeSinceVeryFirst * 0.68) {
        currentSong->insideWorldTickMagnitude--;
        expectedTimeSinceVeryFirst >>= 1;
        targetedTimePerInputTick >>= 1;
      }
  ...
  // We do need an accurate measure of internal tick time immediately, for
  // syncing LFOs
  if (lastInputTickReceived != 0) {
    ...
    // Slowpass
    distanceToGo =
        thisTimePerInternalTick - (slowpassedTimePerInternalTick >>
                                   slowpassedTimePerInternalTickSlowness); // Ok this looks weird...
    slowpassedTimePerInternalTick += distanceToGo;

    // Sticky
    // Or, if new value is just 10% different than lowpassed value, we'll
    // change, too
    ...
    // 2% = 1095216660
    // 5% = 1127428915
    if ((lowpassedTimePerInternalTick >> 2) >
            multiply_32x32_rshift32(1127428915, stickyTimePerInternalTick) ||
      ...
      slowpassedTimePerInternalTick = lowpassedTimePerInternalTick
                                      << slowpassedTimePerInternalTickSlowness;
      ...
      stickyCurrentTimePerInternalTickInverse = 2147483647 / ((stickyTimePerInternalTick * 3) >> 1);
               ...
               (stickyTimePerInternalTick >> 2) >
                   multiply_32x32_rshift32(1084479242,
                                           slowpassedTimePerInternalTick >>
                                               slowpassedTimePerInternalTickSlowness)) {
  ...
  // Calculating time per input tick for the purpose of scheduling internal
  // ticks. TODO: this code largely mirrors what the tempo magnitude matching
  // code above does - could be combined. Or not?
  if (numInputTickTimesCounted) {
    timePerInputTickMovingAverage =
        (uint32_t)(timeThisInputTick - timeLastInputTicks[numInputTickTimesCounted - 1]) /
  ...
  // Schedule an upcoming swung tick. Can only do this after updating
  // timeLastInputTicks[0], just above.
  // TODO: would it be better, in the case where swungTickScheduled is already
  // true, to go and adjust / re-schedule it now we have more information?
  if (!swungTickScheduled) { scheduleSwungTickFromExternalClock(); }
}
...
void PlaybackHandler::resetTimePerInternalTickMovingAverage() {
  // Only do this if no tempo-targeting (that'd be a disaster!!), and if some
  // input ticks have actually been received
  if (!tempoMagnitudeMatchingActiveNow && lastInputTickReceived > 0) {
    ...
    timePerInternalTickMovingAverage =
        timePerInputTickMovingAverage * inputTicksPer / internalTicksPer;
  }
}

void PlaybackHandler::getAnalogOutTicksToInternalTicksRatio(uint32_t* internalTicksPer,
                                                            uint32_t* analogOutTicksPer) {
  *internalTicksPer = 12; // 12 rather than 24 so we get twice as many ticks,
                          // because only every 2nd one is rising-edge
...
void PlaybackHandler::getInternalTicksToInputTicksRatio(uint32_t* inputTicksPer,
                                                        uint32_t* internalTicksPer) {
  ...
  if (usingAnalogClockInput) {
    ...
    *internalTicksPer = 8; // Will usually get multiplied by 3 ( *=
                           // inputTickScale below ) to make 24
  } else {
    *inputTicksPer    = 3;
    *internalTicksPer = 1; // Will usually get multiplied by 3 ( *=
                           // inputTickScale below ) to make 3
  }
...
// To be called if we need to skip the analog-out tick count to a different
// place, because some scaling stuff has changed. At this point, whatever device
// is following our clock will cease to be correctly synced to us
void PlaybackHandler::resyncAnalogOutTicksToInternalTicks() {
  ...
  getAnalogOutTicksToInternalTicksRatio(&internalTicksPer, &analogOutTicksPer);
  lastTriggerClockOutTickDone =
      (uint64_t)getCurrentInternalTickCount() * analogOutTicksPer / internalTicksPer;
}
...
void PlaybackHandler::resyncMIDIClockOutTicksToInternalTicks() {
  ...
  getMIDIClockOutTicksToInternalTicksRatio(&internalTicksPer, &midiClockOutTicksPer);
  lastMIDIClockOutTickDone =
      (uint64_t)getCurrentInternalTickCount() * midiClockOutTicksPer / internalTicksPer;
}
...
void PlaybackHandler::tempoEncoderAction(int8_t offset,
  ...
  if (Buttons::isButtonPressed(xEncButtonX, xEncButtonY)) {
    ...
    if (playbackState & PLAYBACK_CLOCK_INTERNAL_ACTIVE) {
      if (currentlySendingMIDIOutputClocks()) {
        // TODO: these should also affect trigger clock output. Currently they
        // don't
        if (offset < 0) {
  ...
  // If MIDI learn button down, change clock out scale
    ...
    // If playing synced, double or halve our own playing speed relative to the
    // outside world
    if (playbackState & PLAYBACK_CLOCK_EXTERNAL_ACTIVE) {
      if (offset < 0 || currentSong->mayDoubleTempo()) { // Know when to disallow tempo doubling
        ...
        // We do not need to call resyncAnalogOutTicksToInternalTicks(). Yes the
        // insideWorldTickMagnitude has changed, but whatever effect this has on
        // the scaling of input ticks to internal ticks, it has the opposite
        // effect on the scaling of internal ticks to analog out ticks, so
        // nothing needs to change
      }
    }

    // Otherwise, change the output-tick scale - if we're actually sending
    // out-ticks
    else {
...
void PlaybackHandler::sendOutPositionViaMIDI(int32_t pos, bool sendContinueMessageToo) {
  ...
  if (positionPointer >= 16384) {
    ...
    if (currentPlaybackMode == &session) {
      ...
      // In rare case of no play-enabled Clip, we'll just have to send
      // position-0
      if (!longestClip) {
        positionPointer    = 0;
      ...
      // Or, more normally, get the position from our current play-position
      // within the longest Clip
      else {
        // Have to do a fair bit of working-backwards to get output clock ticks
        // from Song ticks
  ...
  surplusOutputTicks++; // Need one extra one to make the follower "play" the
                        // position the pointer points to, right now.

  midiEngine.sendPositionPointer(positionPointer);

  // If we've just switched output clocks back on and they were off, we'd better
  // send a continue message BUT this didn't work in Ableton! In Live 8, you
  // could instead put a "start" message *before* sending the position pointer.
  // But this doesn't work anymore in Live 9, so let's just do it the "proper"
  // MIDI way always.
  if (sendContinueMessageToo) midiEngine.sendContinue();

  for (int i = 0; i < surplusOutputTicks; i++) { midiEngine.sendClock(i != 0); }
}
...
void PlaybackHandler::displayTempoBPM(float tempoBPM) {
  ...
  if (roundedBigger != 0 &&
      !(playbackState & PLAYBACK_CLOCK_EXTERNAL_ACTIVE)) { // It might get supplied here as a 0,
                                                           // even if it's actually very
                                                           // slightly above that, but we don't
                                                           // want do display that as an integer
...
// Called when playing synced and sync scaling or magnitude have been changed -
// e.g. when user doubles or halves tempo, or sync scaling is activated
void PlaybackHandler::resyncInternalTicksToInputTicks(Song* song) {
  if (playbackState & PLAYBACK_CLOCK_EXTERNAL_ACTIVE) {

    lastSwungTickActioned = getCurrentInternalTickFloatFollowingExternalClock(); // This works.
                                                                                 // Although it
                                                                                 // doesn't do
                                                                                 // anything special
                                                                                 // to account for
    ...
    // In most cases, if we're here, we'll want to alter the "following"
    // internal tick tempo to "remember" what time-scaling stuff we changed.
    // This happens routinely every ~24 clocks anyway, but sometimes it makes
    // sense to store this change instantly, e.g. if we just changed
    // time-scaling like we probably just did
    resetTimePerInternalTickMovingAverage();
  }
}
...
void PlaybackHandler::grabTempoFromClip(Clip* clip) {
  ...
  uint64_t loopLengthSamples =
      ((AudioClip*)clip)->sampleHolder.getLengthInSamplesAtSystemSampleRate(true);
  ...
  // Record that change, ourselves. We sent false above because that mechanism
  // of recording it would do all this other stuff
  if (action) {

    void* consMemory = generalMemoryAllocator.alloc(sizeof(ConsequenceTempoChange));

    if (consMemory) {
      ConsequenceTempoChange* newConsequence = new (consMemory)
          ConsequenceTempoChange(timePerBigBefore, currentSong->timePerTimerTickBig);
...
void PlaybackHandler::finishTempolessRecording(bool shouldStartPlaybackAgain,
  ...
  for (int c = 0; c < currentSong->sessionClips.getNumElements(); c++) {
    ...
    if (clip->getCurrentlyRecordingLinearly()) {
      ...
      clip->finishLinearRecording(modelStackWithTimelineCounter,
                                  NULL,
                                  buttonLatencyForTempolessRecord); // nextPendingOverdub doesn't
                                                                    // actually get used in this
                                                                    // call, for audioClips
      ...
      clip->originalLength = ticksLong; // Reset this. After tempoless recording, actual original
  ...
  // Unless the user just hit the play button to stop playback, we probably just
  // want it to start again - so they hear the loop they just recorded
  if (shouldStartPlaybackAgain) {

    // And remember that this tempoless-record Action included beginning
    // playback, so undoing / redoing it later will stop and start playback
    // respectively
    if (action) {
    ...
    setupPlaybackUsingInternalClock(0, false); // Send 0 for buttonPressLatency - cos we know that
                                               // another tempoless record is not about to begin,
                                               // cos we just finished one
  }
}

static const uint32_t noteRecordingUIModes[] = {UI_MODE_HORIZONTAL_ZOOM,
                                                UI_MODE_HORIZONTAL_SCROLL,
                                                UI_MODE_AUDITIONING,
                                                UI_MODE_RECORD_COUNT_IN,
                                                0};
...
bool PlaybackHandler::shouldRecordNotesNow() {
  return (isEitherClockActive() && recording && isUIModeWithinRange(noteRecordingUIModes) &&
          (!playbackHandler.ticksLeftInCountIn ||
           getTimeLeftInCountIn() <=
               LINEAR_RECORDING_EARLY_FIRST_NOTE_ALLOWANCE) // If doing a count-in,
                                                            // only allow notes to
                                                            // be recorded up to
                                                            // 100mS early
  );
...
int32_t PlaybackHandler::getTimeLeftInCountIn() {
  uint32_t remainder;
  uint32_t ticks = playbackHandler.ticksLeftInCountIn -
                   playbackHandler.getNumSwungTicksInSinceLastActionedSwungTick(&remainder);
...
void PlaybackHandler::tapTempoButtonPress() {
  if (tapTempoNumPresses == 0)
    ...
    uint64_t totalTimeBetweenBig =
        (uint64_t)(uint32_t)(AudioEngine::audioSampleTimer - tapTempoFirstPressTime) << 32;
    ...
    currentSong->setTimePerTimerTick(timePerQuarterNoteBig / 3,
                                     true); // Put the fraction in the middle; it's more likely to
                                            // be accurate since we've been rounding down
    ...
    if (midiEngine.globalMIDICommands[c].equalsNoteOrCC(device, channel, note)) {
      switch (c) {
        ...
        case GLOBAL_MIDI_COMMAND_LOOP_CONTINUOUS_LAYERING:
          if (actionLogger.allowedToDoReversion() ||
              currentUIMode == UI_MODE_RECORD_COUNT_IN) { // Not quite sure if this describes
                                                          // exactly what we want but it'll
                                                          // do...
            int overdubNature =
                (c == GLOBAL_MIDI_COMMAND_LOOP) ? OVERDUB_NORMAL : OVERDUB_CONTINUOUS_LAYERING;
        ...
        case GLOBAL_MIDI_COMMAND_UNDO:
          if (actionLogger.allowedToDoReversion()) {
            ...
            // Firstly, we don't want to do it while we may be in some card access
            // routine - e.g. by a SampleRecorder. Secondly, reversion can take a
            // lot of time, and may want to call the audio routine - which is
            // locked cos we're in it!
        ...
        default:
          if (getCurrentUI() == getRootUI()) {
            if (currentUIMode == UI_MODE_NONE) { tapTempoButtonPress(); }
          }
          break;
      }
...
void PlaybackHandler::programChangeReceived(int channel, int program) {
  ...
  soundEditor.noteOnReceivedForMidiLearn(channel + 16, program, 127)) {} return;
  }
...
void PlaybackHandler::noteMessageReceived(MIDIDevice* fromDevice,
  ...
  if (currentUIMode == UI_MODE_MIDI_LEARN &&
      on) { // Checks velocity to let note-offs pass through, so no risk of
            // stuck note if they pressed learn while holding a note
  ...
  for (int c = currentSong->sessionClips.getNumElements() - 1; c >= 0; c--) {
    ...
    if (clip->muteMIDICommand.equalsNoteOrCC(fromDevice, channel, note)) {
      if (on) {
        ...
        session.toggleClipStatus(clip,
                                 &c,
                                 false,
                                 MIDI_KEY_INPUT_LATENCY); // Beware - calling this might insert or
                                                          // delete a Clip!
    ...
    if (!on || currentSong->isOutputActiveInArrangement(thisOutput)) {
      ...
      thisOutput->offerReceivedNote(
          ...
          shouldRecordNotesNowNow &&
              currentSong->isOutputActiveInArrangement(thisOutput), // Definitely don't record
                                                                    // if muted in arrangement
          doingMidiThru);
    }
  }
}
...
void PlaybackHandler::songSelectReceived(uint8_t songId) {
  // I've disabled this, people won't need it. If I was going to do it, I'd have
  // to have it check that the SD card isn't currently being accessed, and
  // nothing else crucial was happening
  /*
  if (storageManager.openXMLFile("SONGS", getThingFilename("SONG", songId, -1),
  "song")) { storageManager.loadSong(songId, -1);
  }
  */
}
...
void PlaybackHandler::switchToArrangement() {
  ...
  if (getCurrentUI() == &sessionView) { PadLEDs::reassessGreyout(); }
}
...
void PlaybackHandler::pitchBendReceived(MIDIDevice* fromDevice,
  ...
  for (Output* thisOutput = currentSong->firstOutput; thisOutput; thisOutput = thisOutput->next) {
    ...
    if (!isMPE &&
        modelStackWithTimelineCounter->timelineCounterIsSet()) { // Do we still need to check this?
      ...
      usedForParam = thisOutput->offerReceivedPitchBendToLearnedParams(
          ...
          modelStackWithTimelineCounter); // NOTE: this call may change
                                          // modelStackWithTimelineCounter->timelineCounter
                                          // etc!
    }

    if (!usedForParam) {
      thisOutput->offerReceivedPitchBend(modelStackWithTimelineCounter,
                                         fromDevice,
                                         channel,
                                         data1,
                                         data2,
...
void PlaybackHandler::midiCCReceived(MIDIDevice* fromDevice,
  ...
  for (Output* thisOutput = currentSong->firstOutput; thisOutput; thisOutput = thisOutput->next) {
    ...
    // If it has an activeClip... (Hmm, interesting, we don't allow MIDI control
    // of params when no activeClip? Yeah this checks out, as the various
    // offerReceivedCCToLearnedParams()'s require a timelineCounter, but this
    // seems restrictive for the user...)
    if (thisOutput->activeClip) {
      ...
      thisOutput->offerReceivedCC(modelStackWithTimelineCounter,
                                  fromDevice,
                                  channel,
                                  ccNumber,
                                  value,
...
// noteCode -1 means channel-wide, including for MPE input (which then means it
// could still then just apply to one note).
void PlaybackHandler::aftertouchReceived(MIDIDevice* fromDevice,
  ...
  bool isMPE =
      (noteCode == -1 &&
       fromDevice->ports[MIDI_DIRECTION_INPUT_TO_DELUGE].isChannelPartOfAnMPEZone(channel));

  if (isMPE) { fromDevice->defaultInputMPEValuesPerMIDIChannel[channel][2] = value << 8; }
  ...
  for (Output* thisOutput = currentSong->firstOutput; thisOutput; thisOutput = thisOutput->next) {
    ...
    thisOutput->offerReceivedAftertouch(modelStackWithTimelineCounter,
                                        fromDevice,
                                        channel,
                                        value,
                                        noteCode,
...
void PlaybackHandler::loopCommand(int overdubNature) {
  ...
  // Or if recording from session to arrangement, we're not allowed to do any
  // loopy stuff during that
  ...
  // If pending overdubs already exist, then delete those.
  else if (currentSong->deletePendingOverdubs()) { // TODO: this traverses all Clips.
                                                   // So does further down. This could
                                                   // be combined
    ...
    // Or if none were recording, or if it was the LAYERING command, then create
    // a new overdub (potentially in addition to having armed the old one to
    // stop
    ...
    doCreateNextOverdub:
      ...
      if (getRootUI() == &sessionView && currentUIMode == UI_MODE_CLIP_PRESSED_IN_SONG_VIEW) {
        clipToCreateOverdubFrom =
            sessionView.getClipOnScreen(sessionView.selectedClipPressYDisplay);
        clipIndexToCreateOverdubFrom =
            sessionView.selectedClipPressYDisplay + currentSong->songViewYScroll;
        sessionView.performActionOnPadRelease = false;
      }

      // Otherwise, prioritize the currentClip - so long as it's not
      // arrangement-only
        ...
        clipIndexToCreateOverdubFrom =
            currentSong->sessionClips.getIndexForClip(clipToCreateOverdubFrom);
      }
      ...
      if (clipToCreateOverdubFrom) {
        ...
        if (clipToCreateOverdubFrom->type != CLIP_TYPE_AUDIO ||
          ...
          if (!clipToCreateOverdubFrom->armedForRecording) {
            ...
            // Or if we weren't in record mode... heck, who knows, maybe just
            // switch it back on?
            else {
          ...
          // Or if that Clip was armed to record linearly...
            ...
            Clip* overdub =
                currentSong->createPendingNextOverdubBelowClip(clipToCreateOverdubFrom,
                                                               clipIndexToCreateOverdubFrom,
                                                               overdubNature);
            if (overdub) {
