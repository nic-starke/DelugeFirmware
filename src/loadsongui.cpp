...
bool LoadSongUI::opened() {
  ...
  gotError:
    ...
    // Oh no, we're unable to read a file representing the first song. Get out
    // quick!
    currentUIMode = UI_MODE_NONE;
    uiTimerManager.unsetTimer(TIMER_UI_SPECIFIC);
    renderingNeededRegardlessOfUI(); // Otherwise we may have left the
                                     // scrolling-in animation partially done
  ...
  if (currentUIMode != UI_MODE_VERTICAL_SCROLL) {
    currentUIMode = UI_MODE_VERTICAL_SCROLL; // Have to reset this again - it might have
                                             // finished the first bit of the scroll
    timerCallback();
  }
...
void LoadSongUI::enterKeyPress() {
  ...
  if (currentFileItem && currentFileItem->isFolder) {
    ...
    LoadUI::enterKeyPress(); // Converts name to numeric-only if it was typed as
                             // text
    performLoad();
  }
}
...
int LoadSongUI::buttonAction(int x, int y, bool on, bool inCardRoutine) {
  ...
  // Load button or select encoder press. Unlike most (all?) other children of
  // Browser, we override this and don't just call mainButtonAction(), because
  // unlike all the others, we need to action the load immediately on down-press
  // rather than waiting for press-release, because of that special action where
  // you hold the button down until you want to "launch" the new song.
...
void LoadSongUI::performLoad() {
  ...
  if (arrangement.hasPlaybackActive()) { playbackHandler.switchToSession(); }
  ...
  // If not currently playing, don't load both songs at once (this avoids any
  // RAM overfilling, fragmentation etc.)
  if (!playbackHandler.isEitherClockActive()) {
    uiTimerManager.unsetTimer(TIMER_PLAY_ENABLE_FLASH); // Otherwise, a timer might get called and
                                                        // try to access Clips that we may have
                                                        // deleted below (really?)
    deleteOldSongBeforeLoadingNew();
  } else {
    // Note: this is dodgy, but in this case we don't reset
    // view.activeControllableClip here - we let the user keep fiddling with it.
    // It won't get deleted.
    AudioEngine::logAction("a");
    AudioEngine::songSwapAboutToHappen();
    AudioEngine::logAction("b");
    playbackHandler.songSwapShouldPreserveTempo =
  ...
  fail:
    // If we already deleted the old song, make a new blank one. This will take
    // us back to InstrumentClipView.
    if (!currentSong) {
      // If we're here, it's most likely because of a file error. On paper, a
      // RAM error could be possible too.
      setupBlankSong();
  ...
  // Will return false if we ran out of RAM. This isn't currently detected for
  // while loading ParamNodes, but chances are, after failing on one of those,
  // it'd try to load something else and that would fail.
  ...
  // Search existing RAM for all samples, to lay a claim to any which will be
  // needed for this new Song. Do this before loading any new Samples from file,
  // in case we were in danger of discarding any from RAM that we might actually
  // want
  preLoadedSong->loadAllSamples(false);

  // Load samples from files, just for currently playing Sounds (or if not
  // playing, then all Sounds)
  if (playbackHandler.isEitherClockActive())
  ...
  if (playbackHandler.isEitherClockActive()) {
    ...
    // If load button was already released while that loading was happening, arm
    // for song-swap now
    if (!Buttons::isButtonPressed(loadButtonX, loadButtonY)) {
      bool result = session.armForSongSwap();

      // If arming couldn't really be done, e.g. because current song had no
      // Clips currently playing, swap has already occurred
      if (!result) { goto swapDone; }
    ...
    // Otherwise, set up so that the song-swap will be armed as soon as the user
    // releases the load button
    else {
#if HAVE_OLED
      OLED::removeWorkingAnimation();
    ...
    // We're now waiting, either for the user to arm, or for the arming to
    // launch the song-swap. Get loading all the rest of the samples which
    // weren't needed right away. (though we might run out of RAM cos we haven't
    // discarded all the old samples yet)
    AudioEngine::logAction("g");
    preLoadedSong->loadAllSamples(true);
    AudioEngine::logAction("h");

    // If any more waiting required before the song swap actually happens, do
    // that
...
swapDone:
#if HAVE_OLED
  OLED::displayWorkingAnimation(
      "Loading"); // To override our popup if we did one. (Still necessary?)
#endif
  // Ok, the swap's been done, the first tick of the new song has been done, and
  // there are potentially loads of samples wanting some data loaded. So do that
  // immediately
...
int LoadSongUI::timerCallback() {
  ...
  if (currentUIMode == UI_MODE_VERTICAL_SCROLL) {
    ...
    // matrixDriver.greyoutMinYDisplay = (scrollDirection > 0) ? displayHeight -
    // squaresScrolled : squaresScrolled;

    // Move the scrolling region
    memmove(PadLEDs::image[startSquare],
            PadLEDs::image[1 - startSquare],
    ...
    if (scrollingToNothing)
      ...
      memcpy(PadLEDs::image[endSquare],
             PadLEDs::imageStore[copyRow],
             (displayWidth + sideBarWidth) * 3);
    ...
    if (squaresScrolled >= displayHeight) {
      ...
      if (scrollDirection == -1) {
        exitThisUI(); // Ideally I don't think this should be allowed to be
                      // happen while in the card routine, which we're in right
                      // now...
      }
...
void LoadSongUI::scrollFinished() {
  // If we were scrolling out of one song and we got here, we just need to sit
  // back and wait for the next song to load
  if (!scrollingIntoSlot) {
    currentUIMode = UI_MODE_WAITING_FOR_NEXT_FILE_TO_LOAD;
  }
...
doSearch:
    ...
    int result = storageManager.findNextFile(offset,
            ...
            "SONG", currentDir.get(), &currentFilePointer, true, 255, NULL,
numberEditPos);
    ...
    enteredTextEditPos = getHowManyCharsAreTheSame(enteredText.get(),
newName.get()); enteredText.set(&newName); currentFilename.set(&newName); //
This will only get used in the case of a folder, so it's ok(ish) that we're
...
void LoadSongUI::currentFileChanged(int movementDirection) {
  ...
  if (movementDirection) {
    ...
    PadLEDs::setupScroll(movementDirection,
    ...
    scrollingIntoSlot = false;
    PadLEDs::renderScroll(); // The scrolling animation will begin while file is
                             // being found and loaded

    drawSongPreview(); // Scrolling continues as the file is read by this
                       // function
  ...
  if (currentUIMode == UI_MODE_LOADING_SONG_UNESSENTIAL_SAMPLES_ARMED) {
    ...
    if (currentUIMode == UI_MODE_NONE || currentUIMode == UI_MODE_HORIZONTAL_SCROLL) {
      ...
      goAgain:
                  ...
                  // Start horizontal scrolling
                  PadLEDs::setupScroll(offset, displayWidth + sideBarWidth,
      true, displayWidth + sideBarWidth); for (int i = 0; i < displayHeight;
      i++) PadLEDs::transitionTakingPlaceOnRow[i] = true; currentUIMode =
      UI_MODE_HORIZONTAL_SCROLL; scrollingIntoSlot = false;
                  PadLEDs::renderScroll(); // The scrolling animation will begin
      while file is being found and loaded
                  ...
                  // If user turned knob while finding file, get out so the new
      action can be done if
      (Encoders::encoders[ENCODER_THIS_CPU_SELECT].detentPos) { currentUIMode =
      UI_MODE_HORIZONTAL_SCROLL; // It might have been set to
      waitingForNextFileToLoad offset =
      Encoders::encoders[ENCODER_THIS_CPU_SELECT].getLimitedDetentPosAndReset();
                      goto goAgain;
                  }

                  drawSongPreview(); // Scrolling continues as the file is read
      by this function currentUIMode = UI_MODE_HORIZONTAL_SCROLL;
                  scrollingIntoSlot = true;

                  // Set up another horizontal scroll
                  PadLEDs::setupScroll(offset, displayWidth + sideBarWidth,
      false, displayWidth + sideBarWidth); for (int i = 0; i < displayHeight;
      i++) PadLEDs::transitionTakingPlaceOnRow[i] = true;
                  PadLEDs::renderScroll();
                  */
    }
  }
...
int LoadSongUI::verticalEncoderAction(int offset, bool inCardRoutine) {
  if (!currentUIMode && !Buttons::isButtonPressed(yEncButtonX, yEncButtonY) &&
...
void LoadSongUI::exitAction() {
  ...
  // If parts of the old song have been deleted, sorry, there's no way we can
  // exit without loading a new song
  if (deletedPartsOfOldSong) {
    numericDriver.displayPopup(
        HAVE_OLED ? "Can't return to current song, as parts have been unloaded" : "CANT");
    return;
  }
...
void LoadSongUI::drawSongPreview(bool toStore) {
  ...
  while (*(tagName = storageManager.readNextTagOrAttributeName())) {
    ...
    if (!strcmp(tagName, "previewNumPads")) {
      ...
      endY            = displayHeight;
      if (previewNumPads != 40) { skipNumCharsAfterRow = 48; }
#else
...
void LoadSongUI::displayText(bool blinkImmediately) {
  ...
  if (qwertyVisible) {
    ...
    if (currentFileItem && !currentFileItem->isFolder) {
      drawSongPreview(false); // Only draw this again so we can draw the
                              // keyboard on top of it. I think...
    } else {
