...
int32_t FilterSetConfig::init(int32_t  lpfFrequency,
                              ...
                              uint8_t  lpfMode,
                              int32_t  filterGain,
                              bool     adjustVolumeForHPFResonance,
                              int32_t* overallOscAmplitude) {

  hpfResonance = (hpfResonance >> 21) << 21; // Insanely, having changes happen in the small bytes
                                             // too often causes rustling
  ...
  if (doLPF) {
    ...
    if (lpfMode == LPF_MODE_TRANSISTOR_24DB_DRIVE) {
      ...
      processedResonance =
          2147483647 - resonance; // Always between 0 and 2. 1 represented as 1073741824
      ...
      if (AudioEngine::cpuDireness < 14 && (logFreq >> 24) > 51) {
        int32_t resonanceThreshold =
            interpolateTableSigned(logFreq, 30, resonanceThresholdsForOversampling, 6);
      ...
      if (doOversampling) {
        ...
        // Adjustment for how the oversampling shifts the frequency just
        // slightly
        lpfFrequency -= (multiply_32x32_rshift32_rounded(logFreq, lpfFrequency) >> 8) *
                        34; // + (lpfFrequency >> 8) * storageManager.devVarB;

        // Enforce a max frequency. Otherwise we'll generate stuff which will
        // cause problems for down-sampling again. But only if resonance is
        // high. If it's low, we need to be able to get the freq high, to let
        // all the HF through that we want to hear
    ...
    int32_t tannedFrequency = instantTan(
        lshiftAndSaturate(lpfFrequency,
                          5)); // Between 0 and 8, by my making. 1 represented by 268435456
    {

      // Cold transistor ladder
      if (lpfMode != LPF_MODE_TRANSISTOR_24DB_DRIVE) {
        // Some long-winded stuff to make it so if frequency goes really low,
        // resonance goes down. This is tuned a bit, but isn't perfect
        int32_t howMuchTooLow = 0;
        if (tannedFrequency < 6000000) { howMuchTooLow = 6000000 - tannedFrequency; }

        int32_t howMuchToKeep = 2147483647 - howMuchTooLow * 33;

        int32_t resonanceUpperLimit = 510000000; // Prone to feeding back lots
        tannedFrequency =
            getMax(tannedFrequency,
                   (int32_t)540817); // We really want to keep the frequency from going
                                     // lower than it has to - it causes problems

        int32_t resonance =
            2147483647 - (getMin(lpfResonance, resonanceUpperLimit) << 2); // Limits it

        resonance          = multiply_32x32_rshift32_rounded(resonance, resonance) << 1;
        processedResonance = 2147483647 - resonance; // 2147483647 - rawResonance2; // Always
                                                     // between 0 and 2. 1 represented as 1073741824
        processedResonance = multiply_32x32_rshift32_rounded(processedResonance, howMuchToKeep)
                             << 1;
      }
      ...
      moveability = multiply_32x32_rshift32_rounded(tannedFrequency,
                                                    divideBy1PlusTannedFrequency)
                    << 4; // Between 0 and 1. 1 represented by 2147483648 I'm pretty sure

      // Half ladder
      if (lpfMode == LPF_MODE_12DB) {
        int32_t moveabilityNegative =
            moveability - 1073741824; // Between -2 and 0. 1 represented as 1073741824
        lpf2Feedback =
        ...
        divideByTotalMoveabilityAndProcessedResonance =
            ...
            (67108864 + multiply_32x32_rshift32_rounded(
                            processedResonance,
                ...
                multiply_32x32_rshift32_rounded(
                    ...
                    multiply_32x32_rshift32_rounded(
                        ...
                        multiply_32x32_rshift32_rounded(moveability,
                                                        processedResonance))))); // 1 represented
                                                                                 // as 67108864
        divideByTotalMoveabilityAndProcessedResonance =
            (int64_t)67108864 * 1073741824 / onePlusThing;
      }

      if (lpfMode != LPF_MODE_TRANSISTOR_24DB_DRIVE) { // Cold transistor ladder only
        // Extra feedback - but only if freq isn't too high. Otherwise we get
        // aliasing
  ...
  filterGain = multiply_32x32_rshift32(filterGain, 1720000000)
               << 1; // This changes the overall amplitude so that, with resonance on
                     // 50%, the amplitude is the same as it was pre June 2017
  ...
  if (doHPF) {
    ...
    int32_t tannedFrequency = instantTan(
        lshiftAndSaturate(hpfFrequency,
                          5)); // Between 0 and 8, by my making. 1 represented by 268435456
    ...
    resonance             = multiply_32x32_rshift32_rounded(resonance, resonance) << 1;
    hpfProcessedResonance = 2147483647 - resonance; // 2147483647 - rawResonance2; // Always between
                                                    // 0 and 2. 1 represented as 1073741824

    hpfProcessedResonance = getMax(hpfProcessedResonance,
                                   (int32_t)134217728); // Set minimum resonance amount
    ...
    hpfMoveability =
        multiply_32x32_rshift32_rounded(tannedFrequency, hpfDivideBy1PlusTannedFrequency) << 4;
    ...
    int32_t moveabilitySquaredTimesProcessedResonance =
        multiply_32x32_rshift32(moveabilityTimesProcessedResonance,
                                hpfMoveability); // 1 = 268435456

    hpfHPF3Feedback =
        -multiply_32x32_rshift32_rounded(hpfMoveability, hpfDivideBy1PlusTannedFrequency);
