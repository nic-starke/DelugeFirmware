...
void Arrangement::setupPlayback() {
  ...
  playbackHandler.swungTicksTilNextEvent =
      0; // It seems strange that I ever put this here. It's now also in
         // PlaybackHandler::setupPlayback, so maybe extra unneeded here - but
         // that's not the only place that calls us, so have left it in for now
  ...
  currentSong->anyClipsSoloing = false; // Got to set this, since we just set
                                        // them all to not-soloing, above

  // Got to deactiveate all arrangement-only Clips too! Especially cos some of
  // them might be muted or something (this made stuff look very buggy!) For
  // each Clip in arrangement
...
bool Arrangement::endPlayback() {
  ...
  // Some Clips might have been reset to "disabled", so need their mute-square
  // redrawn if we're actually viewing the session view
  uiNeedsRendering(&sessionView, 0, 0xFFFFFFFF);

  playbackHandler.playbackState = 0; // Work-around. Our caller, PlaybackHandler::endPlayback(),
                                     // sets this next anyway, and can't do it earlier, but we need
                                     // it before reassessing sessionView's greyout.
...
void Arrangement::doTickForward(int posIncrement) {
  ...
  if (songParamManagerMightContainAutomation) {
    ModelStackWithThreeMainThings* modelStackStackWithThreeMainThing =
        modelStack->addTimelineCounter(modelStack->song)
            ->addOtherTwoThingsButNoNoteRow(&modelStack->song->globalEffectable,
                                            &modelStack->song->paramManager);

    currentSong->paramManager.processCurrentPos(modelStackStackWithThreeMainThing,
                                                posIncrement,
                                                false);
  }
  ...
  for (Output* output = currentSong->firstOutput; output; output = output->next) {
    ...
    if (output->recordingInArrangement) {
      ...
      if (!posIncrement)
        searchPos++; // If first, 0-length tick, don't look at the one that
                     // starts here.
    ...
    notRecording:
      // Or if inactive (and not recording), just check when final ClipInstance
      // ends
      if (!currentSong->isOutputActiveInArrangement(output)) {
        ...
        if (i >= 0 && i < output->clipInstances.getNumElements()) {
          ...
          if (thisClip) {
            ...
            if (endPos == lastProcessedPos) {
              ...
              if (posIncrement) { // Don't deactivate any Clips on the first,
                                  // 0-length tick, or else!
            ...
            // Or if it's still going...
              ...
              goto justDoArp; // No need to think about the next ClipInstance
                              // yet
            }
          }
        }
        ...
        if (!ticksTilStart) {
          ...
          if (posIncrement) { // If posIncrement is 0, it means this is the very
                              // first tick of playback, in which case this has
                              // just been set up already. But otherwise...
            thisClip->activeIfNoSolo = true;
            thisClip->setPos(modelStackWithTimelineCounter, 0);
            output->setActiveClip(modelStackWithTimelineCounter); // Used to call
                                                                  // assertActiveness(), but
                                                                  // that's actually unnecessary -
                                                                  // be because we're playing in
                                                                  // arrangement, setActiveClip()
                                                                  // is actually the only relevant
                                                                  // bit
          }
  ...
  if (anyChangeToSessionClipsPlaying) { uiNeedsRendering(&sessionView, 0, 0xFFFFFFFF); }
  ...
  if (playbackHandler.swungTicksTilNextEvent == 2147483647 &&
      ...
      (!playbackHandler.recording ||
       audioRecorder.recordingSource >=
           AUDIO_INPUT_CHANNEL_FIRST_INTERNAL_OPTION)) { // Only do this if not
                                                         // recording MIDI - but
                                                         // override that and do
                                                         // do it if we're
                                                         // "resampling"
  ...
  // Make sure we come back at right time for any song-level param automation.
  // Must only do these after checking above for
  // (playbackHandler.swungTicksTilNextEvent == 2147483647)
  playbackHandler.swungTicksTilNextEvent =
      getMin(playbackHandler.swungTicksTilNextEvent, nearestArpTickTime);
  if (songParamManagerMightContainAutomation) {
    playbackHandler.swungTicksTilNextEvent =
        getMin(playbackHandler.swungTicksTilNextEvent, currentSong->paramManager.ticksTilNextEvent);
    // Yes we could only do that if songParamManagerMightContainAutomation,
    // which means that we did call processCurrentPos() on that paramManager
    // above. Because otherwise, its ticksTilNextEvent would be an invalid value
    // - often 0, which causes a freeze / infinite loop.
  }
}
...
void Arrangement::resetPlayPos(int32_t newPos, bool doingComplete, int buttonPressLatency) {
  ...
  for (Output* output = currentSong->firstOutput; output; output = output->next) {
    ...
    // Or if no ClipInstance, shall we maybe make one and do a spot of linear
    // recording?
    else {
      if (doingComplete && playbackHandler.recording &&
...
// After unmuting, or creating, lengthening or changing the Clip of a
// ClipInstance
void Arrangement::resumeClipInstancePlayback(ClipInstance* clipInstance,
  ...
  if (thisClip) {
    int32_t clipPos =
        lastProcessedPos - clipInstance->pos; // Use just the currentPos, not the "actual" pos,
                                              // because a multi-tick-forward is probably coming

    thisClip->activeIfNoSolo = true; // Must set this before calling setPos, otherwise,
    ...
    currentSong->assertActiveness(modelStack); // Why exactly did I do this rather than just
                                               // setActiveClip()?

    if (doingComplete && mayActuallyResumeClip) { // Use thisClip->currentPos, not clipPos, cos
                                                  // it's got wrapped in setPos()
      thisClip->resumePlayback(modelStack);       // Do this even if the current pos is 0, otherwise
                                                  // AudioClips can fail to sound because that
      // non-"actual" pos can remain 0 for the whole thing
    }
  }
}

// Virtual function. Called after Clip length changed, which could have big
// effects if Clip repeats multiple times within an Instance. Don't have to yet
// know whether play-head is actually inside an associated ClipInstance. Don't
// worry about mustSetPosToSomething - the effects of that are only needed in
// Session. Check playbackHandler.isEitherClockActive() before calling this.
...
// easy list of just the Clips affected by each Action. Check
...
// For the purpose of stopping or starting sounds during playback if we edited
// the stuff
void Arrangement::rowEdited(Output*       output,
  ...
  if (hasPlaybackActive() && actualPos >= startPos && actualPos < endPos) {
    if (clipRemoved) { clipRemoved->expectNoFurtherTicks(currentSong); }

    if (clipInstanceAdded) { resumeClipInstancePlayback(clipInstanceAdded); }
  }
...
int32_t Arrangement::getLivePos(uint32_t* timeRemainder) {
  return lastProcessedPos +
         playbackHandler.getNumSwungTicksInSinceLastActionedSwungTick(timeRemainder);
}
...
int32_t Arrangement::getPosAtWhichClipWillCut(ModelStackWithTimelineCounter const* modelStack) {
  ...
  if (!clipInstance || clipInstance->clip != clip)
    return clip->currentlyPlayingReversed ? (-2147483648) :
                                            2147483647; // This shouldn't normally happen right?

  int32_t cutPos = clipInstance->length - clip->loopLength * clip->repeatCount;
  if (cutPos == clip->loopLength) { // If cutting right at the end of the
                                    // current repeat...

    if (clip->currentlyPlayingReversed) cutPos = 0; // Since we already knew it was loopLength.

    // See if the next ClipInstance has the same Clip and begins right as this
    // one ends
    ClipInstance* nextClipInstance = clip->output->clipInstances.getElement(i + 1);
    if (nextClipInstance) {
      if (nextClipInstance->clip == clip &&
          nextClipInstance->pos == clipInstance->pos + clipInstance->length) {
    ...
    if (clip->currentlyPlayingReversed) {
      ...
      if (cutPos < clip->loopLength &&
          cutPos <= clip->getLivePos()) { // We first check that cutPos is less
                                          // than the loopLength, so as not to
                                          // waste time on the calculations in
                                          // getLivePos if not necessary
        cutPos = 2147483647;
      }
    }
  }
...
// This includes it "looping" before the Clip's full length due to that
// ClipInstance ending, and there being another instance of the same Clip right
// after.
...
bool Arrangement::willClipLoopAtSomePoint(ModelStackWithTimelineCounter const* modelStack) {
  ...
  // See if the next ClipInstance has the same Clip and begins right as this one
  // ends
  ClipInstance* nextClipInstance = clip->output->clipInstances.getElement(i + 1);
  if (nextClipInstance) {
    if (nextClipInstance->clip == clip && nextClipInstance->pos == instanceEndPos) { return true; }
  }

  // Ok, we're near the end of the ClipInstance, but perhaps the last remaining
  // bit contains a Clip loop-end-point?
