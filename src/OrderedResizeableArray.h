...
public:
  OrderedResizeableArray(int newElementSize,
                         int keyNumBits,
                         int newKeyOffset                = 0,
...
protected:
  inline int32_t getKeyAtMemoryLocation(void* address) {
    ...
    return keyBig >> keyShiftAmount; // We use shifting instead of a mask so
                                     // negative numbers get treated directly.
  }
...
// The purpose of this is not so much that special functionality is required for
// 32-bit keys, but that some further child classes inherit from this, which
// require that the key be 32-bit.
class OrderedResizeableArrayWith32bitKey : public OrderedResizeableArray {
public:
  OrderedResizeableArrayWith32bitKey(int newElementSize,
                                     int newMaxNumEmptySpacesToKeep  = 16,
                                     int newNumExtraSpacesToAllocate = 15);
