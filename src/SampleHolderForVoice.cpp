...
SampleHolderForVoice::SampleHolderForVoice() {
  ...
  for (int l = 0; l < NUM_CLUSTERS_LOADED_AHEAD; l++) { clustersForLoopStart[l] = NULL; }
}

SampleHolderForVoice::~SampleHolderForVoice() {
  // We have to unassign reasons here, even though our parent destructor will
  // call unassignAllReasons() - our overriding of that virtual function won't
  // happen as we've already been destructed!
...
void SampleHolderForVoice::unassignAllClusterReasons(bool beingDestructed) {
  ...
  for (int l = 0; l < NUM_CLUSTERS_LOADED_AHEAD; l++) {
    if (clustersForLoopStart[l]) {
      audioFileManager.removeReasonFromCluster(
          clustersForLoopStart[l],
...
void SampleHolderForVoice::claimClusterReasons(bool reversed, int clusterLoadInstruction) {
  ...
  if (loopStartPlaybackAtSample) {
    ...
    claimClusterReasonsForMarker(clustersForLoopStart,
...
void SampleHolderForVoice::setTransposeAccordingToSamplePitch(bool minimizeOctaves,
                                                              bool doingSingleCycle,
                                                              bool rangeCoversJustOneNote,
                                                              bool thatOneNote) {
...
void SampleHolderForVoice::sampleBeenSet(bool reversed, bool manuallySelected) {
  ...
  // If we're here as a result of the user having manually selected a new file,
  // set up loop points
  if (manuallySelected) {
    loopStartPos = 0;
    loopEndPos   = 0;

    if (((Sample*)audioFile)->fileLoopEndSamples &&
        ((Sample*)audioFile)->fileLoopEndSamples <= lengthInSamples) {

      int loopLength =
          ((Sample*)audioFile)->fileLoopEndSamples - ((Sample*)audioFile)->fileLoopStartSamples;
      ...
      if (((Sample*)audioFile)->fileLoopStartSamples < lengthInSamples &&
        ...
        loopStartPos =
            ((Sample*)audioFile)->fileLoopStartSamples; // If it's 0, that'll translate to
                                                        // meaning no loop start pos, which is
  ...
  // Or if not manually selected
    ...
    // Prior to V2.1.x, sample markers were stored as milliseconds. Try loading
    // those now. Note - V2.1.x did still write these values in addition to the
    // new, sample-based ones, for backward compatibility. But we have to 100%
    ...
    if (storageManager.firmwareVersionOfFileBeingRead < FIRMWARE_2P1P0_BETA) {
      ...
      // Convert old MSec values, loaded from old song files, to samples. Can
      // only do this now that we know the sample rate
      if (startMSec) {
        if (!startPos) {
    ...
    // Check for illegal values - we could have a problem if an audio file got
