...
/*
 * An Instrument is the “Output” of a Clip - the thing which turns the sequence
 * or notes into sound (or MIDI or CV output). Instruments include Kit,
 * MIDIInstrument, and CVInsttrument. And then there’s SoundInstrument, which is
 * basically a synth.
 */
...
public:
  ...
  String              dirPath; // This needs to be initialized / defaulted to "SYNTHS" or
                               // "KITS" (for those Instrument types). The constructor does
                               // not do this, partly because I don't want it doing memory
                               // allocation, and also because in many cases, the function
  ...
  } // You must call this when an Instrument comes into existence or
    // something... for every Clip, not just for the activeClip
  void deleteAnyInstancesOfClip(InstrumentClip* clip);

  // virtual void writeInstrumentDataToFile(bool savingSong, char const*
  // slotName = "presetSlot", char const* subSlotName = "presetSubSlot");
