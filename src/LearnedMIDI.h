...
public:
  ...
  inline bool
  equalsNoteOrCCAllowMPEMasterChannels(MIDIDevice* newDevice, int newChannel, int newNoteOrCC) {
  ...
  // You must have determined that containsSomething() == true before calling
  // this.
  ...
  void writeToFile(char const* commandName,
                   int         midiMessageType); // Writes the actual tag in addition to
                                         // the attributes
  void readFromFile(int midiMessageType);

  inline void writeNoteToFile(char const* commandName) {
    writeToFile(commandName, MIDI_MESSAGE_NOTE);
  }
  inline void writeCCToFile(char const* commandName) {
    writeToFile(commandName, MIDI_MESSAGE_CC);
  }
  inline void writeChannelToFile(char const* commandName) {
    writeToFile(commandName, MIDI_MESSAGE_NONE);
  }

  inline void readNoteFromFile() {
    readFromFile(MIDI_MESSAGE_NOTE);
  }
  inline void readCCFromFile() {
    readFromFile(MIDI_MESSAGE_CC);
  }
  inline void readChannelFromFile() {
    readFromFile(MIDI_MESSAGE_NONE);
  }

  void readMPEZone();

  MIDIDevice* device;
  uint8_t     channelOrZone; // In addition to being set to channel 0 to 15, can
                             // also be MIDI_CHANNEL_MPE_LOWER_ZONE or
