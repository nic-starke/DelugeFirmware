...
void MenuItemSubmenu::beginSession(MenuItem* navigatedBackwardFrom) {
  ...
  while (!(*soundEditor.currentSubmenuItem)
              ->isRelevant(soundEditor.currentSound, soundEditor.currentSourceIndex)) {
    soundEditor.currentSubmenuItem++;
    if (!*soundEditor.currentSubmenuItem) { // Not sure we need this since we
                                            // don't wrap submenu items?
      soundEditor.currentSubmenuItem = items;
    }
  }
#if !HAVE_OLED
  ...
  for (int i = selectedRow + 1; i < OLED_MENU_NUM_OPTIONS_VISIBLE; i++) {
    do {
      ...
      if (!*thisSubmenuItem) goto searchBack;
        ...
        !(*thisSubmenuItem)->isRelevant(soundEditor.currentSound, soundEditor.currentSourceIndex));
  ...
  do {
    if (offset >= 0) {
      ...
      if (thisSubmenuItem == items) {
        ...
        return;
#else
        while (*(thisSubmenuItem + 1)) { thisSubmenuItem++; }
#endif
      } else {
        thisSubmenuItem--;
      }
...
void MenuItemSubmenu::learnKnob(MIDIDevice* fromDevice,
                                int         whichKnob,
                                int         modKnobMode,
                                int         midiChannel) {
  if (soundEditor.getCurrentMenuItem() == this)
