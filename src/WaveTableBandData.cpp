...
bool WaveTableBandData::mayBeStolen(void* thingNotToStealFrom) {
  return (waveTable && !waveTable->numReasonsToBeLoaded &&
          thingNotToStealFrom !=
              &audioFileManager.audioFiles); // Because stealing us would mean the WaveTable
                                             // being deleted too, we have to abide by this
                                             // rule as found in WaveTable::mayBeStolen().
}
...
void WaveTableBandData::steal(char const* errorCode) {
  ...
  // Tell the WaveTable that we're the BandData being stolen, so it won't
  // deallocate us - our caller will do that.
  waveTable->bandDataBeingStolen(this);

  // Delete the WaveTable from memory - deallocating all other BandDatas and
  // everything.
