...
Instrument::~Instrument() {
    // Arrangement-only Clips won't get deallocated here - that'll happen from
the Song.
    ...
    if (!name.isEmpty()) {
      ...
      if (type == INSTRUMENT_TYPE_MIDI_OUT && ((MIDIInstrument*)this)->sendsToMPE()) {
        storageManager.writeAttribute(
            slotXMLTag,
            (((NonAudioInstrument*)this)->channel == MIDI_CHANNEL_MPE_LOWER_ZONE) ? "lower" :
                                                                                    "upper");
      } else {
...
Clip* Instrument::createNewClipForArrangementRecording(ModelStack* modelStack) {
  ...
  // For synths and kits, there'll be an existing ParamManager, and we can clone
  // it. But for MIDI and CV, there might not be, and we don't want to clone it.
  // Instead, the call to setInstrument will create one.

  if (type == INSTRUMENT_TYPE_SYNTH || type == INSTRUMENT_TYPE_KIT) {

    int error =
        newParamManager.cloneParamCollectionsFrom(getParamManager(modelStack->song), false, true);
    ...
    if (activeClip) {
      newParamManager.cloneParamCollectionsFrom(&activeClip->paramManager,
                                                false,
  ...
  ModelStackWithTimelineCounter* modelStackWithTimelineCounter =
      modelStack->addTimelineCounter(newInstrumentClip);
  ...
  newInstrumentClip->setupAsNewKitClipIfNecessary(
      modelStackWithTimelineCounter); // Fix added Sept 2020 to stop Kits from
                                      // screwing up when recording in Arranger.
                                      // Michael B discovered. Also could cause
                                      // E314
...
int Instrument::setupDefaultAudioFileDir() {
  ...
  int         error =
      audioFileManager.setupAlternateAudioFileDir(&audioFileManager.alternateAudioFileLoadPath,
                                                  dirPathChars,
                                                  &name);
