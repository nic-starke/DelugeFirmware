...
void LiveInputBuffer::giveInput(int numSamples, uint32_t currentTime, int inputType) {
  ...
  do {
    ...
    for (int p = 0; p < DIFFERENCE_LPF_POLES; p++) {
      ...
      angleLPFMem[p] += multiply_32x32_rshift32_rounded(distanceToGo, 1 << 23); // distanceToGo >>
                                                                                // 9;
    ...
    if ((numRawSamplesProcessed & (PERC_BUFFER_REDUCTION_SIZE - 1)) == 0) {
      ...
      percBuffer[(numRawSamplesProcessed >> PERC_BUFFER_REDUCTION_MAGNITUDE) &
                 (INPUT_PERC_BUFFER_SIZE - 1)] = percussiveness;
    }
    lastAngle = angle;
...
bool LiveInputBuffer::getAveragesForCrossfade(int32_t* totals,
  ...
  for (int i = 0; i < TIME_STRETCH_CROSSFADE_NUM_MOVING_AVERAGES; i++) {
    ...
    do {
      ...
      if (numChannels == 2) { totals[i] += rawBuffer[currentPos * 2 + 1] >> 16; }
