...
SampleLowLevelReader::SampleLowLevelReader() {
  for (int l = 0; l < NUM_CLUSTERS_LOADED_AHEAD; l++) { clusters[l] = NULL; }
}

SampleLowLevelReader::~SampleLowLevelReader() {
  // unassignAllReasons(true); // We would want to call this, but we call it
  // manually instead, cos these often aren't actually destructed
}
...
// May return negative number - I think particularly if we're going in reversed
...
int32_t SampleLowLevelReader::getPlayByteLowLevel(Sample*              sample,
  ...
  if (clusters[0]) {
    ...
    if (compensateForInterpolationBuffer && interpolationBufferSizeLastTime) {
      ...
      // if (oscPos >= 8388608) extraSamples++; // This would be good, but we go
      // one better and just copy this to the new hop, in time stretching
      withinCluster +=
          extraSamples * sample->numChannels * sample->byteDepth * guide->playDirection;
    }
    ...
    return (int32_t)guide->endPlaybackAtByte +
           (int32_t)(uint32_t)currentPlayPos *
               guide->playDirection; // Hopefully this won't go negative, cos
                                     // we're returning as unsigned...
  }
}
...
void SampleLowLevelReader::setupForPlayPosMovedIntoNewCluster(SamplePlaybackGuide* guide,
  ...
  // Ok, now we've just moved the play-pos into a new Cluster, so do some
  // setting up for that
...
// finalClusterIndex cos we were reading cache before. There is no guarantee
// that this won't put the reassessmentLocation back before the currentPlayPos,
// which is not generally allowed (though it'd be harmless for "natively"
// playing Samples). Caller must ensure safety here.
bool SampleLowLevelReader::reassessReassessmentLocation(SamplePlaybackGuide* guide,
  ...
  if (!clusters[0])
    return true; // Is this for if we've gone past the end of the audio data,
                 // while re-pitching / interpolating?

  realignPlaybackParameters(sample);

  int clusterIndex = clusters[0]->clusterIndex;

  // We may have ended up past the finalClusterIndex if we've just switched from
  // using a cache. This needs correcting, so "looping" can occur at next
  // render. Must happen before setupReassessmentLocation() is called.
  ...
  unassignAllReasons(); // Can only do this after we've done the above stuff,
                        // which references clusters, which this will clear
...
// There is no guarantee that this won't put the reassessmentLocation back
// before the currentPlayPos, which is not generally allowed (though it'd be
// harmless for "natively" playing Samples). Caller must ensure safety here. I
// only discovered this bug / requirement in Sept 2020. Going to assume that
// only reassessReassessmentLocation() really needs to do this...
void SampleLowLevelReader::setupReassessmentLocation(SamplePlaybackGuide* guide, Sample* sample) {
  ...
  int     finalClusterIndex =
      guide->getFinalClusterIndex(sample, shouldObeyMarkers(), &endPlaybackAtByte);
  ...
  // Or if it's not the final Cluster...
    ...
    // Playing backwards
      ...
      uint32_t bytesBeforeCurrentClusterEnd =
          ...
          sample->audioDataStartPosBytes; // Well, it's really the "start" - the
                                          // left-most edge
      int excess = bytesBeforeCurrentClusterEnd % (uint8_t)bytesPerSample;
      if (excess == 0) excess = bytesPerSample;
  ...
  // Playing backwards
    ...
    int highestClusterIndex =
        audioDataStopPos >>
        audioFileManager.clusterSizeMagnitude; // There may actually be 1 less Cluster than
                                               // this if the audio data ends right at the
                                               // Cluster end, but that won't cause problems
    if (currentClusterIndex == highestClusterIndex)
      clusterStartLocation =
          &clusters[0]->data[(audioDataStopPos - 1) & (audioFileManager.clusterSize - 1)];
    else
...
bool SampleLowLevelReader::setupClusersForInitialPlay(SamplePlaybackGuide* guide,
  ...
  if (!success) { Uart::println("setupClustersForInitialPlay fail"); }
...
bool SampleLowLevelReader::setupClustersForPlayFromByte(SamplePlaybackGuide* guide,
  ...
  // Change in Aug 2019 - we return false if stuff is out of range. Seems right?
  // Previously we were constraining ClusterIndex to the range, but not changing
  // startPlaybackAtByte - didn't seem to make sense. Or, should it maybe do the
  // "outputting zeros" thing instead? Possibly not too important, as this only
  // gets called for an "initial play", and on time-stretch hop, which goes and
  // will try some alternative stuff if this fails
  ...
  int32_t bytePosWithinNewCluster =
      startPlaybackAtByte - clusterIndex * audioFileManager.clusterSize;

  setupForPlayPosMovedIntoNewCluster(guide, sample, bytePosWithinNewCluster, sample->byteDepth);

  // No check has been made that currentPlayPos is not already later than the
  // new reassessmentLocation. If caller isn't sure about this, call
  // changeClustersIfNecessary(). changeClustersIfNecessary() itself calls this
  // function when it changes current Cluster, so we absolutely couldn't call it
  // from here.
  return true;
}

// Unassign the old ones before you call this.
bool SampleLowLevelReader::assignClusters(SamplePlaybackGuide* guide,
                                          Sample*              sample,
                                          int                  clusterIndex,
                                          int                  priorityRating) {
  int finalClusterIndex = guide->getFinalClusterIndex(sample, shouldObeyMarkers());
  ...
  for (int l = 0; l < NUM_CLUSTERS_LOADED_AHEAD; l++) {
    ...
    // The first one is required to not only have returned an object to us
    // (which it might not have if insufficient RAM or maybe other reasons), but
    // also to be fully loaded.
    if (l == 0) {

      if (!clusters[l] || !clusters[l]->loaded) { return false; }
    }
...
bool SampleLowLevelReader::moveOnToNextCluster(SamplePlaybackGuide* guide,
  ...
  for (int l = 0; l < NUM_CLUSTERS_LOADED_AHEAD - 1; l++) { clusters[l] = clusters[l + 1]; }
  ...
  // Remove the compensation we'd done on the play pos relating to the byte
  // depth of samples
  ...
  setupForPlayPosMovedIntoNewCluster(guide,
                                     sample,
                                     bytePosWithinOldCluster -
                                         audioFileManager.clusterSize * guide->playDirection,
                                     sample->byteDepth);

  return true;
}

// Returns false if stopping deliberately or clusters weren't loaded in time. In
// that case, caller may wish to output some zeros to work through the
// interpolation buffer. All reasons (e.g. clusters[0]) will be unassigned / set
  ...
  while (true) {
    ...
    if (reassessmentAction == REASSESSMENT_ACTION_NEXT_CLUSTER) {
      ...
      if (loopingAtLowLevel) {
        bool success =
            setupClusersForInitialPlay(guide, sample, byteOvershoot, true, priorityRating);
        if (!success) {
    ...
    count++;
    if (count >= 1024)
      numericDriver.freezeWithError("E227"); // This happened one time! When stopping AudioClips
                                             // from playing back, after recording and mucking
                                             // around with SD card reaching full
#endif
  }
  return true;
}
...
void SampleLowLevelReader::fillInterpolationBufferRetrospectively(Sample* sample,
  ...
  for (int i = startI; i < bufferSize; i++) {
    ...
    justWriteZeros:
      ...
      if (sample->numChannels == 2) { interpolationBuffer[1][0][i] = 0; }
    }
...
bool SampleLowLevelReader::fillInterpolationBufferForward(SamplePlaybackGuide* guide,
                                                          Sample*              sample,
  ...
  for (int i = numSpacesToFill - 1; i >= 0; i--) {
    ...
    doZeroesFillingBuffer:
      ...
      if (!stillGoing) { goto doZeroesFillingBuffer; }
...
void SampleLowLevelReader::jumpBackSamples(Sample* sample, int numToJumpBack, int playDirection) {
  while (numToJumpBack--) { // Could probably be more efficient, but why bother
                            // - this whole function is rare
...
// This sets up for the reading of some samples. If interpolating, it jumps the
// play-head forward one output-sample, in place of this happening for the first
// one in the fast rendering routine (it still happens there for all but the
// first). This is because the rendering of the first output-sample may require
// multiple source-samples to be fed into the interpolation buffer, and they
// might not all be in the same cluster - they might span across a cluster
// boundary. This function is equipped to deal with this, whereas the fast
// rendering routine is not. This function will be called again whenever any
// such cluster-changing situation arises.
//
// Non-interpolating playback, too, checks in this function (so at the start of
// the render) whether we need to change to the next cluster.
bool SampleLowLevelReader::considerUpcomingWindow(SamplePlaybackGuide* guide,
  ...
  if (phaseIncrement != 16777216) {
    ...
    if (!interpolationBufferSizeLastTime) {
      ...
      // Fill up the furthest-back end of the interpolation buffer
      fillInterpolationBufferRetrospectively(sample,
                                             interpolationBufferSize,
                                             halfBufferSize,
                                             guide->playDirection);

      // And fill up to end of interpolation buffer
      bool success = fillInterpolationBufferForward(guide,
                                                    sample,
                                                    interpolationBufferSize,
                                                    loopingAtLowLevel,
                                                    halfBufferSize,
      ...
      if (ALPHA_OR_BETA_VERSION && clusters[0]) {
        int32_t bytesLeftWhichMayBeRead =
            (int32_t)((uint32_t)reassessmentLocation - (uint32_t)currentPlayPos) *
            guide->playDirection;
    ...
    if (numSamplesToJumpForward) {
      ...
      if (clusters[0]) {
        // If by more than INTERPOLATION_BUFFER_SIZE, we need to do a pre-jump
        // to a buffer's-length before we're jumping forward to, to fill up the
        // buffer
        if (numSamplesToJumpForward > interpolationBufferSize) {
          currentPlayPos += (numSamplesToJumpForward - interpolationBufferSize) * bytesPerSample *
                            guide->playDirection;
          numSamplesToJumpForward =
              interpolationBufferSize; // That's how much jumping is left to do
        }
      }
      ...
      while (numSamplesToJumpForward--) {
        ...
        doZeroes:
          ...
          bool stillGoing =
              changeClusterIfNecessary(guide, sample, loopingAtLowLevel, priorityRating);
          if (!stillGoing) {
          ...
          bufferIndividualSampleForInterpolation(sample->bitMask,
                                                 sample->numChannels,
                                                 sample->byteDepth,
                                                 currentPlayPos);
    ...
    // Or if not jumping forward any samples...
      ...
      if (ALPHA_OR_BETA_VERSION && clusters[0]) {
        ...
        // That should mean we've already read this one, so we definitely
        // shouldn't be beyond the reassessmentLocation...
        int32_t bytesLeftWhichMayBeRead =
            (int32_t)((uint32_t)reassessmentLocation - (uint32_t)currentPlayPos) *
            guide->playDirection;
    ...
    // currentPlayPos may now actually be at or beyond the reassessmentLocation
    // - that's ok Wait, I can see how it could be at it, but surely it couldn't
    // be beyond it anymore?

    // The rest of this window is going to require that we jump forward
    // (*numSamples - 1) times
    if (*numSamples >= 2) {

      int samplesWeWantToReadThisWindow =
          ((uint64_t)phaseIncrement * (*numSamples - 1) + oscPos) >> 24;
      ...
      // Or if still going on waveform
        ...
        if (shouldShorten) {
          samplesLeftWhichMayBeRead =
              ...
              (uint8_t)bytesPerSample; // If we're here, we know
                                       // bytesLeftWhichMayBeRead is quite small
        }
      }
      ...
      if (shouldShorten) {
        ...
        // We add 1 because remember, we were just considering (numSamples - 1)
        // the whole time - because we've already done a jump-forward for the
  ...
  // No interpolating
    ...
    // Check if we already ended up at the end of the Cluster after the last
    // window
    ...
    // If there are actually less bytes remaining than we ideally wanted for
    // this window...
    if (*numSamples * bytesPerSample > bytesLeftWhichMayBeRead) {
      *numSamples = (uint32_t)bytesLeftWhichMayBeRead / (uint8_t)bytesPerSample;
      ...
      if (ALPHA_OR_BETA_VERSION && *numSamples <= 0) {
        ...
        numericDriver.freezeWithError("E147"); // Crazily, Michael B got in Nov 2022, when "closing"
                                               // a recorded loop.
      }
    }
  }
...
void SampleLowLevelReader::bufferIndividualSampleForInterpolation(uint32_t bitMask,
  ...
  // This works better than using memmoves. Ideally we'd switch this off if not
  // smoothly interpolating - check that that's actually more efficient though
  for (int i = INTERPOLATION_MAX_NUM_SAMPLES - 1; i >= 1; i--) {
    interpolationBuffer[0][0][i] = interpolationBuffer[0][0][i - 1];
    if (numChannels == 2) { interpolationBuffer[1][0][i] = interpolationBuffer[1][0][i - 1]; }
  }

  interpolationBuffer[0][0][0] = *(int16_t*)(playPosNow + 2);

  if (numChannels == 2) { interpolationBuffer[1][0][0] = *(int16_t*)(playPosNow + 2 + byteDepth); }
}
...
void SampleLowLevelReader::bufferZeroForInterpolation(int numChannels) {
  ...
  if (numChannels == 2) { interpolationBuffer[1][0][0] = 0; }
...
void SampleLowLevelReader::jumpForwardLinear(int      numChannels,
  ...
  if (numSamplesToJumpForward) {
    ...
    // If jumping forward by more than INTERPOLATION_BUFFER_SIZE, we first need
    // to jump to the one before we're jumping forward to, to grab its value
...
void SampleLowLevelReader::interpolate(int32_t* __restrict__ sampleRead,
                                       int numChannelsNow,
                                       int whichKernel) {
#include "interpolate.h"
}

void SampleLowLevelReader::interpolateLinear(int32_t* __restrict__ sampleRead,
                                             int numChannelsNow,
                                             int whichKernel) {
#include "interpolatelinear.h"
}

// This stuff is in its own function here rather than in Voice because for some
// reason it's faster
void SampleLowLevelReader::readSamplesResampled(int32_t** __restrict__ oscBufferPos,
                                                ...
                                                bool* __restrict__ doneAnySamplesYet,
  ...
  int32_t const* const oscBufferEnd =
      oscBufferPosNow + numSamplesTotal * numChannelsAfterCondensing;
  ...
  if (interpolationBufferSize > 2) {
    ...
    do {
      ...
      if (__builtin_expect(stillGotActualData, 1)) {
        ...
        if (numSamplesToJumpForward) {
          ...
          // If jumping forward by more than INTERPOLATION_MAX_NUM_SAMPLES, we
          // first need to jump to the one before we're jumping forward to, to
          // grab its value
          if (numSamplesToJumpForward > INTERPOLATION_MAX_NUM_SAMPLES) {
            currentPlayPosNow +=
                (numSamplesToJumpForward - INTERPOLATION_MAX_NUM_SAMPLES) * jumpAmount;
    ...
    skipFirstSmooth:
      ...
      *oscBufferPosNow = multiply_accumulate_32x32_rshift32_rounded(
          existingValueL,
          sampleRead[0],
          *amplitude); // sourceAmplitude is modified above; using accumulate
                       // made no difference
      oscBufferPosNow++;
...
void SampleLowLevelReader::readSamplesNative(int32_t** __restrict__ bufferPos,
                                             int     numSamplesTotal,
                                             Sample* sample,
                                             int     jumpAmount,
                                             int     numChannels,
                                             int     numChannelsAfterCondensing,
                                             int32_t* __restrict__ amplitude,
                                             int32_t        amplitudeIncrement,
                                             TimeStretcher* timeStretcher,
                                             bool           bufferingToTimeStretcher) {
  ...
  do {
    ...
    *bufferPosNow = multiply_accumulate_32x32_rshift32_rounded(
        existingValueL,
        sampleReadL,
        *amplitude); // *amplitude is modified above; using accumulate made no
                     // difference
    bufferPosNow++;
    ...
    if (numChannelsAfterCondensing == 2) {
      ...
      *bufferPosNow =
          multiply_accumulate_32x32_rshift32_rounded(existingValueR, sampleReadR, *amplitude);
      bufferPosNow++;
    }
...
// Returns false if actual error. Not if it just reached the end. In that case
// it just sets timeStretcher->playHeadStillActive[whichPlayHead] to false
bool SampleLowLevelReader::readSamplesForTimeStretching(int32_t*             outputBuffer,
                                                        SamplePlaybackGuide* guide,
                                                        Sample*              sample,
                                                        int                  numSamples,
                                                        int                  numChannels,
  ...
  do {
    ...
    timeStretcher->playHeadStillActive[whichPlayHead] = considerUpcomingWindow(guide,
                                                                               sample,
                                                                               &samplesNow,
                                                                               phaseIncrement,
                                                                               loopingAtLowLevel,
    ...
    if (!timeStretcher->playHeadStillActive[whichPlayHead]) {
      ...
      // If we got false, that can just mean end of waveform. But if clusters[0]
      // has been set to NULL too, that means (SD card) error
      if (clusters[0]) return false;
    ...
    // No resampling
    if (phaseIncrement == 16777216) {
      readSamplesNative(&outputBuffer,
                        samplesNow,
                        sample,
                        jumpAmount,
                        numChannels,
                        ...
                        amplitudeIncrement,
                        timeStretcher,
                        bufferingToTimeStretcher);
    }

    // Resampling
    else {
      bool doneAnySamplesYet = false;
      readSamplesResampled(&outputBuffer,
                           samplesNow,
                           sample,
                           jumpAmount,
                           numChannels,
...
void SampleLowLevelReader::cloneFrom(SampleLowLevelReader* other, bool stealReasons) {
  ...
  for (int l = 0; l < NUM_CLUSTERS_LOADED_AHEAD; l++) {
    if (clusters[l]) { audioFileManager.removeReasonFromCluster(clusters[l], "E131"); }
