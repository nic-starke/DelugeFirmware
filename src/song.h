...
public:
  ...
  ParamManagerForTimeline*
       findParamManagerForDrum(Kit* kit, Drum* drum, Clip* stopTraversalAtClip = NULL);
  ...
  void grabVelocityToLevelFromMIDIDeviceAndSetupPatchingForAllParamManagersForInstrument(
      MIDIDevice*      device,
      SoundInstrument* instrument);
  void
  grabVelocityToLevelFromMIDIDeviceAndSetupPatchingForAllParamManagersForDrum(MIDIDevice* device,
                                                                              SoundDrum*  drum,
                                                                              Kit*        kit);
  ...
  Instrument*  getInstrumentFromPresetSlot(int         instrumentType,
                                           int         presetNumber,
  ...
  void         replaceInstrument(Instrument* oldInstrument,
  ...
  Instrument*  getNonAudioInstrumentToSwitchTo(int     newInstrumentType,
  ...
  Output*     firstOutput;
  Instrument* firstHibernatingInstrument; // All Instruments have inValidState set to
                                          // false when they're added to this list
  ...
  // How many orders of magnitude faster internal ticks are going than input
  // ticks. Used in combination with inputTickScale, which is usually 1, but is
  // different if there's an inputTickScaleClip. So, e.g. if
  // insideWorldTickMagnitude is 1, this means the inside world is spinning
  // twice as fast as the external world, so MIDI sync coming in representing an
  // 8th-note would be interpreted internally as a quarter-note (because two
  // internal 8th-notes would have happened, twice as fast, making a
  // quarter-note)
  int insideWorldTickMagnitude;

  // Sometimes, we'll do weird stuff to insideWorldTickMagnitude for
  // sync-scaling, which would make BPM values look weird. So, we keep
  ...
  int32_t lastClipInstanceEnteredStartPos; // -1 means we are not "inside" an
                                           // arrangement. While we're in the
  ...
  bool outputClipInstanceListIsCurrentlyInvalid; // Set to true during scenarios
                                                 // like replaceInstrument(), to
                                                 // warn other functions not to
                                                 // look at Output::clipInstances
  ...
  void     renderAudio(StereoSample* outputBuffer,
                       int           numSamples,
  ...
  ParamManager* getBackedUpParamManagerForExactClip(ModControllableAudio* modControllable,
                                                    Clip*                 clip,
                                                    ParamManager*         stealInto = NULL);
  ParamManager* getBackedUpParamManagerPreferablyWithClip(ModControllableAudio* modControllable,
                                                          Clip*                 clip,
                                                          ParamManager*         stealInto = NULL);
  void          backUpParamManager(ModControllableAudio*    modControllable,
                                   Clip*                    clip,
  ...
  Clip* getSessionClipWithOutput(Output* output,
  ...
  void  sortOutWhichClipsAreActiveWithoutSendingPGMs(ModelStack* modelStack,
  ...
  Clip* getLongestActiveClipWithMultipleOrFactorLength(int32_t targetLength,
  ...
  void    deletingClipInstanceForClip(Output* output,
                                      Clip*   clip,
                                      Action* action,
  ...
  bool  deletePendingOverdubs(Output* onlyWithOutput                                   = NULL,
  ...
  void    midiDeviceBendRangeUpdatedViaMessage(ModelStack* modelStack,
                                               MIDIDevice* device,
                                               int         channelOrZone,
                                               int         whichBendRange,
                                               int         bendSemitones);
  ...
  // Reverb params to be stored here between loading and song being made the
  // active one
