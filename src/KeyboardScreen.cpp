...
static const uint32_t padActionUIModes[] = {UI_MODE_AUDITIONING,
...
int KeyboardScreen::padAction(int x, int y, int velocity) {
  ...
  if (sdRoutineLock && !allowSomeUserActionsEvenWhenInCardRoutine)
    return ACTION_RESULT_REMIND_ME_OUTSIDE_CARD_ROUTINE; // Allow some of the
                                                         // time when in card
                                                         // routine.
  ...
  if (currentUIMode == UI_MODE_SCALE_MODE_BUTTON_PRESSED) {
    ...
    if (velocity && currentSong->currentClip->output->type !=
                        INSTRUMENT_TYPE_KIT) { // We probably couldn't have got this far if
                                               // it was a Kit, but let's just check
    ...
    // NOTE: Most of this refers to the Instrument's activeClip - *not* the Clip
    // we're viewing, which might not be the activeClip, even though we did call
    // makeClipActiveOnInstrumentIfPossible() above
    ...
    if (velocity) {
      ...
      // Look for an empty pad press - or an existing press of the same physical
      // pad - which could be left if we missed a press-off event somehow (think
      // this was happening when switching presets sometimes)
      ...
      if (instrument->type == INSTRUMENT_TYPE_SYNTH) {
        if (velocity) {
          if (getCurrentUI() == &soundEditor &&
            ...
            multiRangeMenu.noteOnToChangeRange(noteCode +
      ...
      // Only now that we know we're not going to return prematurely can we mark
      // the pad as pressed
      {
      ...
      // Begin resampling - yup this is even allowed if we're in the card
      // routine!
    ...
    // Press-up
      ...
      // There were no presses. Just check we're not still stuck in
      // "auditioning" mode, as users have still been reporting problems with
      // this. (That comment from around 2021?)
    ...
    foundIt:
      ...
      // We need to check that we had actually switched the note on here - it
      // might have already been sounding, from the sequence
      if (!yDisplayActive[yDisplay]) return ACTION_RESULT_DEALT_WITH;

      // If any other of the same note is being held down, then don't switch it
      // off. Also, see if we're still "auditioning" any notes at all
      ...
      for (p = 0; p < MAX_NUM_KEYBOARD_PAD_PRESSES; p++) {
        if (padPresses[p].x != 255) {
          ...
          // If the same note is still being held down (on a different pad),
          // then we don't want to switch it off either
          if (getNoteCodeFromCoords(padPresses[p].x, padPresses[p].y) == noteCode)
            return ACTION_RESULT_DEALT_WITH;
        }
      }

      // If we had indeed sounded the note via audition (as opposed to it being
      // on in the sequence), switch it off. If that was not the case, well, we
      // still did want to potentially exit audition mode above, cos users been
      // reporting stuck note problems, even though I can't see quite how we'd
      // get stuck there
    ...
    // Recording - this only works *if* the Clip that we're viewing right now is
    // the Instrument's activeClip
    if (clipIsActiveOnInstrument && playbackHandler.shouldRecordNotesNow() &&
      ...
      if (velocity) {
        ...
        // couple more cases - see noteMessageReceived() in MelodicInstrument
        // and Kit
        if (isUIModeActive(UI_MODE_RECORD_COUNT_IN)) { // It definitely will be auditioning
                                                       // if we're here
          ...
          ModelStackWithNoteRow* modelStackWithNoteRow =
              getCurrentClip()->getOrCreateNoteRowForYNote(noteCode,
...
int KeyboardScreen::buttonAction(int x, int y, bool on, bool inCardRoutine) {
  ...
  if (x == scaleModeButtonX && y == scaleModeButtonY) {
    ...
    if (on) {
      ...
      // If user is auditioning just one note, we can go directly into Scale
  ...
  // Song view button
    ...
    if (on && currentUIMode == UI_MODE_NONE) {
      ...
      if (currentSong->lastClipInstanceEnteredStartPos != -1 ||
          currentSong->currentClip->section == 255) {
      ...
      doOther:
        ...
        memcpy(&PadLEDs::occupancyMaskStore,
               PadLEDs::occupancyMask,
               sizeof(PadLEDs::occupancyMask));
        // memset(PadLEDs::occupancyMaskStore, 16, sizeof(uint8_t) *
        // displayHeight * (displayWidth + sideBarWidth));
  ...
  for (int p = 0; p < MAX_NUM_KEYBOARD_PAD_PRESSES; p++) {
    if (padPresses[p].x != 255) {
      ...
      ((MelodicInstrument*)currentSong->currentClip->output)
          ->endAuditioningForNote(modelStack, noteCode);
...
void KeyboardScreen::recalculateColours() {
  for (int i = 0; i < displayHeight * KEYBOARD_ROW_INTERVAL + displayWidth; i++) {
    getCurrentClip()->getMainColourFromY(getCurrentClip()->yScrollKeyboardScreen + i,
                                         0,
                                         noteColours[i]);
  }
}
...
bool KeyboardScreen::renderMainPads(uint32_t whichRows,
  ...
  // First, piece together a picture of all notes-within-an-octave which are
  // active
    ...
    if (flashDefaultRootNoteOn) {
      for (int y = 0; y < displayHeight; y++) {
        ...
        for (int x = 0; x < displayWidth; x++) {
          ...
          if (!noteWithinOctave) { memcpy(image[y][x], noteColours[yDisplay], 3); }
  ...
  // Or normal
    ...
    for (int y = 0; y < displayHeight; y++) {
      ...
      for (int x = 0; x < displayWidth; x++) {
        ...
        // Otherwise, square will just get left black, from its having been
        // wiped above
...
bool KeyboardScreen::renderSidebar(uint32_t whichRows,
  ...
  for (int y = 0; y < displayHeight; y++) { memset(image[y][displayWidth], 0, sideBarWidth * 3); }
  ...
  if (isUIModeWithinRange(padActionUIModes)) {
    ...
    if (offset >= 0) {
      newYNote = getCurrentClip()->yScrollKeyboardScreen +
                 (displayHeight - 1) * KEYBOARD_ROW_INTERVAL + displayWidth - 1;
    } else
      newYNote = getCurrentClip()->yScrollKeyboardScreen;
