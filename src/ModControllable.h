...
public:
  ModControllable();
  virtual bool modEncoderButtonAction(uint8_t                        whichModEncoder,
                                      bool                           on,
                                      ModelStackWithThreeMainThings* modelStack) {
    return false;
  } // Returns whether Instrument was changed
  virtual void
  modButtonAction(uint8_t whichModButton, bool on, ParamManagerForTimeline* paramManager){};
  ...
  virtual ModelStackWithAutoParam*
                   getParamFromMIDIKnob(MIDIKnob* knob,
                                        ModelStackWithThreeMainThings*
                                            modelStack); // Check that autoParam isn't NULL, after calling this
  virtual uint8_t* getModKnobMode(); // Return NULL if different modes not supported
  virtual bool     isKit() {
    return false;
  }
  virtual int
  getKnobPosForNonExistentParam(int                      whichModEncoder,
                                ModelStackWithAutoParam* modelStack); // modelStack->autoParam will
                                                                      // be NULL in this rare case!!
  virtual int modEncoderActionForNonExistentParam(int                      offset,
                                                  int                      whichModEncoder,
