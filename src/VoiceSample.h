...
public:
  ...
  int  attemptLateSampleStart(SamplePlaybackGuide* voiceSource,
                              Sample*              sample,
                              int64_t              rawSamplesLate,
                              int                  numSamples = 0);
  void endTimeStretching();
  bool render(SamplePlaybackGuide* guide,
              int32_t*             oscBuffer,
              int                  numSamples,
              Sample*              sample,
              int                  numChannels,
  ...
  void readSamplesResampledPossiblyCaching(int32_t** oscBufferPos,
  ...
  bool    sampleZoneChanged(SamplePlaybackGuide* voiceSource,
                            Sample*              sample,
                            int                  markerType,
                            int                  loopingType,
                            int                  priorityRating,
  ...
  bool    stopUsingCache(SamplePlaybackGuide* guide,
                         Sample*              sample,
                         int                  priorityRating,
                         bool                 loopingAtLowLevel);
  bool    possiblySetUpCache(SampleControls*      sampleControls,
                             SamplePlaybackGuide* guide,
                             int32_t              phaseIncrement,
                             int32_t              timeStretchRatio,
                             int                  priorityRating,
                             int                  loopingType);
  bool    fudgeTimeStretchingToAvoidClick(Sample*              sample,
                                          SamplePlaybackGuide* guide,
                                          int32_t              phaseIncrement,
  ...
  uint32_t pendingSamplesLate; // This isn't used for AudioClips. And for
                               // samples in STRETCH mode, the exact number
  ...
  bool         fudging;
  bool         forAudioClip;      // This is a wee bit of a hack - but we need to be able to
                                  // know this
  bool    writingToCache;         // Value is only valid if cache assigned
  int32_t cacheLoopEndPointBytes; // 2147483647 means no looping. Will be set to
                                  // sample end-point if looping there. Gets
                                  // re-set to 2147483647 when note "released"
  int32_t cacheEndPointBytes;     // Will sometimes be the whole length of the sample.
                                  // Wherever the red marker is. Or a little further if
                                  // it's the full length of the sample, to allow for
                                  // timestretch / interpolation ring-out
  uint32_t cacheLoopLengthBytes;

private:
  bool weShouldBeTimeStretchingNow(Sample*              sample,
                                   SamplePlaybackGuide* guide,
                                   int                  numSamples,
                                   int32_t              phaseIncrement,
                                   int32_t              timeStretchRatio,
