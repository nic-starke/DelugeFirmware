...
public:
  MemoryRegion();
  void     setup(void*    emptySpacesMemory,
  ...
  uint32_t                stealableClusterQueueLongestRuns
      [NUM_STEALABLE_QUEUES]; // Keeps track, semi-accurately, of biggest runs
                              // of memory that could be stolen. In a perfect
                              // world, we'd have a second index on
                              // stealableClusterQueues[q], for run length.
                              // Although even that wouldn't automatically
                              // reflect changes to run lengths as neighbouring
                              // memory is allocated.
...
private:
  uint32_t freeSomeStealableMemory(int   totalSizeNeeded,
                                   void* thingNotToStealFrom,
                                   int* __restrict__ foundSpaceSize);
  void     markSpaceAsEmpty(uint32_t spaceStart,
                            uint32_t spaceSize,
                            bool     mayLookLeft  = true,
                            bool     mayLookRight = true);
  NeighbouringMemoryGrabAttemptResult
       attemptToGrabNeighbouringMemory(void*    originalSpaceAddress,
                                       int      originalSpaceSize,
                                       int      minAmountToExtend,
                                       int      idealAmountToExtend,
                                       void*    thingNotToStealFrom,
                                       uint32_t markWithTraversalNo        = 0,
                                       bool     originalSpaceNeedsStealing = false);
