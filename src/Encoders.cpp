...
void init() {
  ...
  for (int i = 0; i < NUM_ENCODERS; i++) { new (&encoders[i]) Encoder; }
...
void readEncoders() {
  for (int i = 0; i < NUM_ENCODERS; i++) { encoders[i].read(); }
}
...
bool interpretEncoders(bool inCardRoutine) {
  ...
  for (int e = 0; e < NUM_FUNCTION_ENCODERS; e++) {
    ...
    if (encoders[e].detentPos != 0) {
      ...
      // Limit. Some functions can break if they receive bigger numbers, e.g.
      // LoadSongUI::selectEncoderAction()
      int limitedDetentPos  = encoders[e].detentPos;
      encoders[e].detentPos = 0; // Reset. Crucial that this happens before we
                                 // call selectEncoderAction()
      ...
      switch (e) {
        ...
        case ENCODER_SCROLL_X:
          ...
          // Actually, after coding this up, I realise I actually have it above
          // stopping the X encoder from even getting here during the SD routine.
          // Ok so we'll leave it that way, in addition to me having made all the
          // horizontalEncoderAction() calls SD-routine-safe
        checkResult:
  ...
  if (!inCardRoutine || currentUIMode == UI_MODE_LOADING_SONG_UNESSENTIAL_SAMPLES_ARMED) {
    ...
    for (int e = 0; e < 2; e++) {
      ...
      if (encoders[ENCODER_MOD_0 - e].encPos != 0) {
        ...
        bool turnedRecently =
            (AudioEngine::audioSampleTimer - timeModEncoderLastTurned[e] < (44100 >> 1));

        // If it was turned recently...
        if (turnedRecently) {

          // Mark as turned recently again. Must do this before the
          // encoder-action gets invoked below, because that might want to reset
          // this
        ...
        // Or if it wasn't turned recently, it's going to get marked as turned
        // recently now, but remember what direction we came, so that if we go
        // back that direction again we can write it off as an accidental wiggle
        else {
