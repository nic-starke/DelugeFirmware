...
ArpeggiatorSettings::ArpeggiatorSettings() {
  ...
  // I'm so sorry, this is incredibly ugly, but in order to decide the default sync level, we have
  // to look at the current song, or even better the one being preloaded. Default sync level is used
  // obviously for the default synth sound if no SD card inserted, but also some synth presets,
  // possibly just older ones, are saved without this so it can be set to the default at the time of
  // loading.
...
void ArpeggiatorForDrum::noteOn(ArpeggiatorSettings*  settings,
  ...
  arpNote.outputMemberChannel =
      MIDI_CHANNEL_NONE; // MIDIInstrument might set this later, but it needs to be
                         // MIDI_CHANNEL_NONE until then so it doesn't get included in the survey
                         // that will happen of existing output member channels.

  for (int m = 0; m < NUM_EXPRESSION_DIMENSIONS; m++) { arpNote.mpeValues[m] = mpeValues[m]; }
...
// May return the instruction for a note-on, or no instruction. The noteCode instructed might be
// some octaves up from that provided here.
void Arpeggiator::noteOn(ArpeggiatorSettings*  settings,
  ...
  arpNote->outputMemberChannel =
      MIDI_CHANNEL_NONE; // MIDIInstrument might set this, but it needs to be MIDI_CHANNEL_NONE
                         // until then so it doesn't get included in the survey that will happen of
                         // existing output member channels.

  for (int m = 0; m < NUM_EXPRESSION_DIMENSIONS; m++) { arpNote->mpeValues[m] = mpeValues[m]; }

noteInserted:
  arpNote->inputCharacteristics[MIDI_CHARACTERISTIC_CHANNEL] =
      fromMIDIChannel; // This is here so that "stealing" a note being edited can then replace its
                       // MPE data during editing. Kind of a hacky solution, but it works for now.
  ...
  if (settings && settings->mode) {
    ...
    // Or if the arpeggiator was already sounding
      ...
      if (whichNoteCurrentlyOnPostArp >= n) { whichNoteCurrentlyOnPostArp++; }
    }
    // Don't do the note-on now, it'll happen automatically at next render
  } else {
    instruction->noteCodeOnPostArp = noteCode;
...
void Arpeggiator::noteOff(ArpeggiatorSettings*  settings,
  ...
  if (n < notes.getNumElements()) {
    ...
    if (arpNote->inputCharacteristics[MIDI_CHARACTERISTIC_NOTE] == noteCodePreArp) {
      ...
      // Or if yes arpeggiation, we'll only stop right now if that was the last note to switch off.
      // Otherwise, it'll turn off soon with the arpeggiation.
      else {
        if (notes.getNumElements() == 1) {
...
void Arpeggiator::switchNoteOn(ArpeggiatorSettings* settings, ArpReturnInstruction* instruction) {
  ...
  if (settings->mode == ARP_MODE_RANDOM) {
    ...
    currentOctave               = getRandom255() % settings->numOctaves;
    currentDirection = 1; // Must set a currentDirection here, even though RANDOM doesn't use it, in
                          // case user changes arp mode.
  }
  ...
  noteCodeCurrentlyOnPostArp =
      arpNote->inputCharacteristics[MIDI_CHARACTERISTIC_NOTE] + (int)currentOctave * 12;
...
void ArpeggiatorBase::render(ArpeggiatorSettings*  settings,
  ...
  if (gatePos >= gateThresholdSmall) {
    ...
    // And maybe (if not syncing) the gatePos is also far enough along that we also want to switch a
    // note on?
    if (!syncedNow && gatePos >= 16777216) { switchNoteOn(settings, instruction); }
  }
...
int32_t ArpeggiatorBase::doTickForward(ArpeggiatorSettings*  settings,
  ...
  return howFarIntoPeriod; // Normally we will have modified this variable above, and it no longer
                           // represents what its name says.
}
