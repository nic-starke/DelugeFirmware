...
public:
  ...
  int  cloneParamCollectionsFrom(ParamManager* other,
                                 bool          copyAutomation,
  ...
  ExpressionParamSet*
  getOrCreateExpressionParamSet(bool forDrum = false); // Will return NULL if can't create

  inline ParamCollectionSummary* getExpressionParamSetSummary() { // Will return one containing NULL
                                                                  // if didn't exist
  ...
  ParamCollectionSummary summaries[PARAM_COLLECTIONS_STORAGE_NUM]; // This list should be
                                                                   // terminated by an object whose
                                                                   // values are all zero. Yes, all
                                                                   // of them must be zero, because
                                                                   // if we know this, we can check
                                                                   // for stuff faster.
};

class ParamManagerForTimeline final
    : public ParamManager { // I want to rename this one to "with automation"
public:
  ...
  void generateRepeats(ModelStackWithThreeMainThings* modelStack,
                       uint32_t                       oldLength,
                       uint32_t                       newLength,
                       bool                           shouldPingpong);
  void appendParamManager(ModelStackWithThreeMainThings* modelStack,
  ...
  void processCurrentPos(ModelStackWithThreeMainThings* modelStack,
                         int                            ticksSinceLast,
                         bool                           reversed,
  ...
  void shiftHorizontally(ModelStackWithThreeMainThings* modelStack,
                         int32_t                        amount,
                         int32_t                        effectiveLength);
  void nudgeAutomationHorizontallyAtPos(int32_t                        pos,
                                        int                            offset,
