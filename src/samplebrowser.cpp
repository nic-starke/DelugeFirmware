...
bool SampleBrowser::opened() {
  ...
  sdError:
    ...
    numericDriver.setNextTransitionDirection(
        0); // Cancel the transition that we'll now not be doing
    return false;
  }
  ...
  // If currentPath is blank, or is somewhere outside of the SAMPLES folder,
  // then default to previously manually loaded sample
...
void SampleBrowser::focusRegained() {
  ...
  IndicatorLEDs::setLedState(saveLedX,
                             saveLedY,
                             false); // In case returning from delete-file context menu
}
...
void SampleBrowser::exitAction() {
  ...
  if (!isUIOpen(&soundEditor)) {
    // If no file was selected, the user wanted to get out of creating this
    // Drum.
    if (soundEditor.editingKit() &&
        ((Kit*)currentSong->currentClip->output)
            ->getFirstUnassignedDrum(
                (InstrumentClip*)currentSong->currentClip) // Only if some unassigned Drums
  ...
  if (currentFileItem) {
    ...
    if (currentFileItem->isFolder) {
      ...
      char const* filenameChars =
          currentFileItem->filename.get(); // Extremely weirdly, if we try to just put this inside
                                           // the parentheses in the next line, it returns an empty
                                           // string (&nothing). Surely this is a compiler error??

      int error = goIntoFolder(filenameChars);

      if (error) {
        numericDriver.displayError(error);
        close(); // Don't use goBackToSoundEditor() because that would do a
                 // left-scroll
        return;
      }
    }
    ...
    // Or if it's an audio file...
      ...
      if (Buttons::isShiftButtonPressed()) {
        ...
        // Can only do this for Kit Clips, and for source 0, not 1, AND there
        // has to be only one drum present, which is assigned to the first
        // NoteRow
...
int SampleBrowser::buttonAction(int x, int y, bool on, bool inCardRoutine) {
  ...
  if (x == saveButtonX && y == saveButtonY && Buttons::isShiftButtonPressed()) {
    if (!currentUIMode && on) {
      FileItem* currentFileItem = getCurrentFileItem();
      if (currentFileItem) {
        if (!currentFileItem->isFolder) { // This is an additional requirement
                                          // only present in this class
  ...
  // Record button
    ...
    bool success =
        changeUISideways(&audioRecorder); // If this fails, we will become the current UI again
    if (success) {
...
void SampleBrowser::previewIfPossible(int movementDirection) {
  ...
  if (movementDirection && movementDirection *
  Encoders::encoders[ENCODER_THIS_CPU_SELECT].detentPos > 0 &&
  numFilesFoundInRightDirection > 1) { Uart::println("returned 1"); return;
  }
  */
  ...
  if (currentFileItem && !currentFileItem->isFolder) {
    ...
    // This more formally does the thing that actually was happening
    // accidentally for ages, as found by Michael B.
    ...
    if (movementDirection && movementDirection *
    Encoders::encoders[ENCODER_THIS_CPU_SELECT].detentPos > 0 &&
    numFilesFoundInRightDirection > 1) { Uart::println("returned 2"); return;
    }
    */

    // If the Sample at least loaded, even if we didn't sound it, then try to
    // render its waveform.
    if (AudioEngine::sampleForPreview->sources[0].ranges.getNumElements() >= 1) {
      AudioFile* sample =
          ((MultisampleRange*)AudioEngine::sampleForPreview->sources[0].ranges.getElement(0))
      ...
      if (sample) {
        ...
        if (movementDirection) {
          waveformRenderer.renderFullScreen(waveformBasicNavigator.sample,
                                            waveformBasicNavigator.xScroll,
                                            waveformBasicNavigator.xZoom,
                                            PadLEDs::imageStore,
                                            &waveformBasicNavigator.renderData);
          memset(PadLEDs::transitionTakingPlaceOnRow,
                 1,
                 sizeof(PadLEDs::transitionTakingPlaceOnRow));
...
int SampleBrowser::padAction(int x, int y, int on) {
  ...
  // Allow auditioning
  if (x == displayWidth + 1) {
    if (getRootUI() == &instrumentClipView) { return instrumentClipView.padAction(x, y, on); }
  }
...
int SampleBrowser::claimAudioFileForInstrument(bool makeWaveTableWorkAtAllCosts) {
  ...
  return holder->loadFile(soundEditor.currentSource->sampleControls.reversed,
                          true,
                          true,
                          CLUSTER_ENQUEUE,
                          0,
...
// This displays any (rare) specific errors generated, then spits out just a
// boolean success. For the "may" arguments, 0 means no; 1 means auto; 2 means
// do definitely as the user has specifically requested it.
bool SampleBrowser::claimCurrentFile(int mayDoPitchDetection,
                                     int mayDoSingleCycle,
                                     int mayDoWaveTable) {
  ...
  // Otherwise, we're something to do with an Instrument...
    ...
    soundEditor.currentSound->unassignAllVoices(); // We used to only do this if osc type wasn't
                                                   // already SAMPLE...
    ...
    doLoadAsWaveTable:
      ...
      */
      ...
      if (error) {
        // If word has come back that this file isn't wanting to load as a
        // WaveTable...
        if (error == ERROR_FILE_NOT_LOADABLE_AS_WAVETABLE ||
            error == ERROR_FILE_NOT_LOADABLE_AS_WAVETABLE_BECAUSE_STEREO) {

          // If that was what the user really specified they wanted, and we
          // couldn't do it, then we have to tell them no.
      ...
      // Alright, if we're still here, it was successfully loaded as a
      // WaveTable!
    ...
    doLoadAsSample:
      ...
      */
      ...
      // If the file was actually clearly a wavetable file, and we're allowed to
      // load one, then go do that instead.
      ...
      // If 20ms or less, and we're not a kit, then we'd like to be a
      // single-cycle waveform.
      if (!soundEditor.editingKit() &&
          (mayDoSingleCycle == 2 || (mayDoSingleCycle == 1 && mSec <= 20))) {

        // Ideally, we'd like to use the wavetable engine for this
        // single-cycle-ness
        if (mayDoWaveTable && numTypesTried <= 1 && sample->numChannels == 1 &&
          ...
          makeWaveTableWorkAtAllCosts = true; // So that the loading functions don't just chicken
                                              // out when it doesn't look all that wavetabley.
          goto doLoadAsWaveTable;
        }

        // Otherwise, set play mode to LOOP, and we'll just do single-cycle as a
        // sample. (This is now pretty rare.)
        soundEditor.currentSource->repeatMode = SAMPLE_REPEAT_LOOP;
        doingSingleCycleNow                   = true;
      }

      // If time stretching or looping on (or we just decided to do
      // single-cycle, above), leave that the case
      ...
      // Otherwise...
        ...
        if (sample->fileLoopEndSamples) {
          ...
          // If this led to an actual loop end pos, with more waveform after it,
          // and the sample's not too long, we can do a ONCE.
          if (((MultisampleRange*)soundEditor.currentMultiRange)->sampleHolder.loopEndPos &&
              mSec < 2002) {
          ...
          soundEditor.currentSource->repeatMode =
              (mSec < 2002) ? SAMPLE_REPEAT_ONCE : SAMPLE_REPEAT_CUT;
        }
      }
      ...
      if (soundEditor.editingKit()) {
        ...
        // Give Drum no name, momentarily. We don't want it to show up when
        // we're searching for duplicates
        drum->name.clear();
        ...
        // Ensure Drum name isn't a duplicate, and if need be, make a new name
        // from the fileNamePostPrefix.
      ...
      // If a synth...
        ...
        if (mayDoPitchDetection) {
          ...
          ((MultisampleRange*)soundEditor.currentMultiRange)
              ->sampleHolder.setTransposeAccordingToSamplePitch(shouldMinimizeOctaves,
                                                                doingSingleCycleNow);
        }
      ...
      if (soundEditor.currentSound->modKnobs[7][0].paramDescriptor.getJustTheParam() == p) {
        soundEditor.currentSound->modKnobs[7][0].paramDescriptor.setToHaveParamOnly(
            PARAM_UNPATCHED_BITCRUSHING + PARAM_UNPATCHED_SECTION);
        anyChange = true;
      }
    ...
    // If there was only one MultiRange, don't go back to the range menu (that's
    // the BOT-TOP thing).
...
void SampleBrowser::autoDetectSideChainSending(SoundDrum*  drum,
                                               Source*     source,
                                               char const* fileName) {
...
void SampleBrowser::audioFileIsNowSet() {
  ...
  // Reset osc volume, if it's not automated and was at 0. Wait but that will
  // only do it for the current ParamManager... there could be other ones...
  if (!modelStackWithParam->autoParam->containsSomething(-2147483648)) {
    ...
    //((ParamManagerBase*)soundEditor.currentParamManager)->setPatchedParamValue(PARAM_LOCAL_OSC_A_VOLUME
    //+ soundEditor.currentSourceIndex, 2147483647, 0xFFFFFFFF, 0,
    // soundEditor.currentSound, currentSong, currentSong->currentClip, false);
  }
}
...
void sortSamples(bool (*sortFunction)(Sample*, Sample*),
                 int       numSamples,
                 Sample*** sortAreas,
                 int*      readArea,
                 int*      writeArea) {
  ...
  while (numComparing < numSamples) {
    ...
    for (int whichComparison = 0; whichComparison * numComparing * 2 < numSamples;
         whichComparison++) {
      ...
      for (int writeI = numComparing * whichComparison * 2;
           writeI < numComparing * (whichComparison + 1) * 2 && writeI < numSamples;
           writeI++) {
...
int getNumTimesIncorrectSampleOrderSeen(int numSamples, Sample** samples) {
  ...
  for (int s = 1; s < numSamples; s++) {
    ...
    if (sampleB->midiNote < sampleA->midiNote) { timesIncorrectOrderSeen++; }
  }
...
bool SampleBrowser::loadAllSamplesInFolder(bool      detectPitch,
                                           int*      getNumSamples,
                                           Sample*** getSortArea,
                                           bool*     getDoingSingleCycle,
                                           int*      getPrefixAndDirLength) {
        ...
        if (thisSample->partOfFolderBeingLoaded) {
          ...
          if (thisSample->numReasonsToBeLoaded <= 0)
            numericDriver.freezeWithError(
                "E213"); // I put this here to try and catch an E004 Luc got
#endif
  ...
  while (true) {
    ...
    result = f_readdir_get_filepointer(&staticDIR,
                                       &staticFNO,
    ...
    // Keep investigating if there's a common prefix to all files in this
    // folder. currentFilename will be set to the name of the first file in the
    // folder, or another one we looked at more recently
    if (numSamples > 0) {
    ...
    Sample* newSample = (Sample*)audioFileManager.getAudioFileFromFilename(
        ...
        &error,
        &thisFilePointer,
        AUDIO_FILE_TYPE_SAMPLE); // We really want to be able to pass a file
                                 // pointer in here
    if (error || !newSample) {
    ...
    if (commonMIDINote == -2) {
      ...
      if (commonMIDINote != newSample->midiNoteFromFile) { commonMIDINote = -3; }
    }
  ...
  if (getPrefixAndDirLength) {
    // If just one file, there's no prefix.
    if (numSamples <= 1) { numCharsInPrefixForFolderLoad = 0; }
  ...
  // If all samples were tagged with the same MIDI note, we get suspicious and
  // delete them.
  bool discardingMIDINoteFromFile = (numSamples > 1 && commonMIDINote >= 0);

  Sample** sortArea =
      (Sample**)generalMemoryAllocator.alloc(numSamples * sizeof(Sample*) * 2, NULL, false, true);
  if (!sortArea) {
  ...
  // Go through each sample in memory that was from the folder in question,
  // adding them to our pointer list
  int sampleI = 0;
  for (int e = 0; e < audioFileManager.audioFiles.getNumElements(); e++) {
    AudioFile* audioFile = (AudioFile*)audioFileManager.audioFiles.getElement(e);
    ...
    if (audioFile->type == AUDIO_FILE_TYPE_SAMPLE) {
      ...
      if (thisSample->partOfFolderBeingLoaded) {
        ...
        if (detectPitch) { thisSample->workOutMIDINote(doingSingleCycle); }
  ...
  numSamples = sampleI; // In case it's lower now, e.g. due to some samples'
...
#define NOTE_CHECK_ERROR_MARGIN 0.75
    ...
    // If the Samples are in precisely the wrong order, something's happened
    // like we've been interpretting a dash (-) in the filenames as a minus
    // sign. Just reverse the order.
    ...
    sortSamples(filenameGreaterOrEqualOctaveStartingFromA, numSamples,
    sortAreas, &readArea, &writeArea); int badnessRatingFromA =
    getNumTimesIncorrectSampleOrderSeen(numSamples, sortAreas[readArea]); if
    (!badnessRatingFromA) goto allSorted; // If that's all fine, we're done

    // If from A was actually worse than C, go back
    if (badnessRatingFromA >= badnessRatingFromC) {

        // But if C is actually bad enough, we might conclude that the filenames
    are irrelevant if ((badnessRatingFromC * 3) > numSamples) goto
    justSortByPitch;

        Uart::println("going back to ordering from C");
        sortSamples(filenameGreaterOrEqual, numSamples, sortAreas, &readArea,
    &writeArea);
    }
    ...
    // Or if A is better...
        ...
        // But if A is still actually bad enough, we might conclude that the
    filenames are irrelevant if ((badnessRatingFromA * 3) > numSamples) goto
    justSortByPitch;
    }
    */

    // Ok, we're here, the samples are optimally ordered by file, but, the pitch
    // is out.
    ...
    for (int s = 1; s < numSamples; s++) {
      ...
      if (noteHere < prevNote - NOTE_CHECK_ERROR_MARGIN) {
        ...
        for (int t = s - 1; t >= 0; t--) {
          ...
          numIncorrectBackwards++; // If we're here, this note would have to be
                                   // marked as incorrect
        }
    ...
    // Ok, we've now marked a bunch of samples as having the incorrect pitch,
    // which we know because it doesn't match the filename order. So go through
    // and correct them, now that we've got a better idea of the range they
    // should fit in.
    for (int s = 0; s < numSamples; s++) {
      ...
      // If maxFreqHz is too low, it's likely no use to us, and the whole
      // mission's probably messed up, so just call this one an error. Hopefully
      // I can improve this one day. See Michael B's Mellotron samples
      ...
      // If didn't work, see if we can pretend we're looking for 1 octave
      // higher, where there'd probably be a harmonic too. This can help if the
      // fundamental isn't visible - it worked on Leo's piano samples before I
      // realised that those harmonics had just been deleted by my aggressive
      // use of a threshold
      if (thisSample->midiNote == MIDI_NOTE_ERROR) {
        minFreqHz *= 2;
        maxFreqHz *= 2;
...
allSorted:
  // All sorted! If the sorted values have ended up in the secondary area, move
  // them back to the first
  if (readArea == 1) { memcpy(sortArea, &sortArea[numSamples], numSamples * sizeof(Sample*)); }
...
bool SampleBrowser::importFolderAsMultisamples() {
  ...
  // If we now want more than one range, be efficient by getting our array of
  // ranges to pre-allocate all the memory it's going to use
  if (numSamples > 1) {
  ...
  // If we've ended up with some samples a whole octave higher than the others,
  ...
  if (numSamples) {
    ...
    for (int s = 1; s < numSamples; s++) {
      ...
      if (noteHere >= prevNote + 12.5 && noteHere <= prevNote + 13.5) {
        ...
        // If there are other intervals of more than a semitone, we can't really
        // take it for granted what's going on, so get out
    ...
    if (whichSampleIsAnOctaveUp) {
      ...
      if (whichSampleIsAnOctaveUp * 2 < numSamples) {
        for (int s = 0; s < whichSampleIsAnOctaveUp; s++) { sortArea[s]->midiNote += 12; }
      }

      // Or correct later ones?
      else {
        for (int s = whichSampleIsAnOctaveUp; s < numSamples; s++) { sortArea[s]->midiNote -= 12; }
      }
    }
  }
...
skipOctaveCorrection:
  ...
  int rangeIndex = 0; // Keep this different to the sample index, just in case
                      // we need to skip a sample because it has the same pitch
                      // as a previous one. Skipping a range would leave our
                      // rangeArray with unused space allocated, but that's ok
  ...
  for (int s = 0; s < numSamples; s++) {
    ...
    range->sampleHolder.setAudioFile(thisSample,
                                     soundEditor.currentSource->sampleControls.reversed,
                                     true);
    bool rangeCoversJustOneNote = (topNote == lastTopNote + 1);
    range->sampleHolder.setTransposeAccordingToSamplePitch(false,
                                                           doingSingleCycle,
  ...
  if (numWithFileLoopPoints * 2 >= numSamples) {
    ...
    // If this led to an actual loop end pos, with more waveform after it, and
    // the sample's not too long, we can do a ONCE
  ...
  // Or if no loop points set...
    ...
    // If 2 seconds or less, set play mode to ONCE. Otherwise, cut
    soundEditor.currentSource->repeatMode =
        (averageMSec < 2002) ? SAMPLE_REPEAT_ONCE : SAMPLE_REPEAT_CUT;
  }
  ...
  char modelStackMemory[MODEL_STACK_MAX_SIZE];
  {
    ...
    for (int s = 0; s < numSamples; s++) {
      ...
      if (s == 0) {
        ...
        // Ensure osc type is "sample". For the later drums, calling
        // setupAsSample() does this same thing
        if (soundEditor.currentSource->oscType != OSC_TYPE_SAMPLE) {
          soundEditor.currentSound->unassignAllVoices();
          soundEditor.currentSource->setOscType(OSC_TYPE_SAMPLE);
        }
        ...
        if (!modelStackWithParam->autoParam->isAutomated()) {
          modelStackWithParam->autoParam->setCurrentValueWithNoReversionOrRecording(
              modelStackWithParam,
              2147483647);
          //((ParamManagerBase*)soundEditor.currentParamManager)->setPatchedParamValue(PARAM_LOCAL_OSC_A_VOLUME
          //+ soundEditor.currentSourceIndex, 2147483647, 0xFFFFFFFF, 0,
          // firstDrum, currentSong, currentSong->currentClip, false);
        }
    ...
    skipNameStuff:
      ...
      source->repeatMode =
          (thisSample->getLengthInMSec() < 2002) ? SAMPLE_REPEAT_ONCE : SAMPLE_REPEAT_CUT;
...
static const uint32_t zoomUIModes[] = {UI_MODE_HOLDING_HORIZONTAL_ENCODER_BUTTON,
                                       UI_MODE_AUDITIONING,
                                       0};
  ...
  doNormal:
    ...
    if (currentlyShowingSamplePreview &&
        (isUIModeActive(UI_MODE_HOLDING_HORIZONTAL_ENCODER_BUTTON) ||
         waveformBasicNavigator.isZoomedIn())) {

      // We're quite likely going to need to read the SD card to do either
      // scrolling or zooming
      if (sdRoutineLock) return ACTION_RESULT_REMIND_ME_OUTSIDE_CARD_ROUTINE;

      // Zoom
      if (isUIModeActive(UI_MODE_HOLDING_HORIZONTAL_ENCODER_BUTTON)) {
        if (isUIModeWithinRange(zoomUIModes)) { waveformBasicNavigator.zoom(offset); }
      }
    ...
    { uiTimerManager.unsetTimer(TIMER_DISPLAY); scrollingText->currentPos +=
    offset;

            int maxScroll = scrollingText->length - NUMERIC_DISPLAY_LENGTH;

            if (scrollingText->currentPos < 0) scrollingText->currentPos = 0;
            if (scrollingText->currentPos > maxScroll) scrollingText->currentPos
    = maxScroll;
