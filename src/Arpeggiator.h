...
struct ArpNote {
  int16_t inputCharacteristics[2]; // Before arpeggiation. And applying to MIDI
                                   // input if that's happening. Or, channel
...
public:
  ...
  uint32_t sampleSyncLengthOn; // This defaults to zero, or may be overwritten
                               // by the caller to the Arp - and then the Arp
                               // itself may override that.
  ArpNote* arpNoteOn;
...
public:
  virtual void noteOn(ArpeggiatorSettings*  settings,
                      int                   noteCode,
                      int                   velocity,
                      ArpReturnInstruction* instruction,
                      int                   fromMIDIChannel,
                      int16_t const*        mpeValues) = 0;
  void         render(ArpeggiatorSettings*  settings,
                      int                   numSamples,
                      uint32_t              gateThreshold,
                      uint32_t              phaseIncrement,
                      ArpReturnInstruction* instruction);
  int32_t      doTickForward(ArpeggiatorSettings*  settings,
                             ArpReturnInstruction* instruction,
...
public:
  ArpeggiatorForDrum();
  void    noteOn(ArpeggiatorSettings*  settings,
                 int                   noteCode,
                 int                   velocity,
                 ArpReturnInstruction* instruction,
                 int                   fromMIDIChannel,
  ...
  void    reset();
  ArpNote arpNote; // For the one note. noteCode will always be 60. velocity
                   // will be 0 if off.
...
public:
  ...
  void noteOn(ArpeggiatorSettings*  settings,
              ...
              int16_t const*        mpeValues);
  void
  noteOff(ArpeggiatorSettings* settings, int noteCodePreArp, ArpReturnInstruction* instruction);
  bool hasAnyInputNotesActive();
