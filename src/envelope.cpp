...
considerEnvelopeStage:
  switch (state) {
    case ENVELOPE_STAGE_ATTACK:
      pos += attack * numSamples; // Increment the pos *before* taking a value, so we can skip
                                  // the attack section entirely with a high posIncrease
      if (pos >= 8388608) {
        pos = 0;
    ...
    case ENVELOPE_STAGE_DECAY:
      lastValue =
          sustain + (uint32_t)multiply_32x32_rshift32(getDecay8(pos, 23), 2147483647 - sustain) * 2;
    ...
    case ENVELOPE_STAGE_RELEASE:
      ...
      // int32_t thisValue = multiply_32x32_rshift32(getDecay8(pos, 23),
      // lastValue) << 1; int32_t negativePos = (8388608 - pos) >> 13; int32_t
      // thisValue = multiply_32x32_rshift32(negativePos * negativePos *
      // negativePos, lastValue) << 2;
      lastValue =
          multiply_32x32_rshift32(interpolateTable(pos, 23, releaseTable), lastValuePreCurrentStage)
          << 1;
      break;
    ...
    case ENVELOPE_STAGE_FAST_RELEASE:
      ...
      // lastValue = multiply_32x32_rshift32((8388608 - pos) << 8,
      // lastValuePreRelease) << 1;

      // This alternative line does the release in a sine shape, which you'd think
      // would cause less high-frequency content than the above kinda "triangle"
      // one, but it sounds about the same somehow Actually no it does sound a bit
      // better for Kody's deep bass sample
      lastValue = multiply_32x32_rshift32((getSine(pos + (8388608 >> 1), 24) >> 1) + 1073741824,
...
void Envelope::noteOff(uint8_t envelopeIndex, Sound* sound, ParamManagerForTimeline* paramManager) {
  ...
  if (!sound->envelopeHasSustainCurrently(envelopeIndex, paramManager)) {
    ignoredNoteOff = true;
  } else if (state < ENVELOPE_STAGE_RELEASE) { // Could we ever have already
                                               // been in release state?
