...
/*
int8_t NonAudioDrum::getModKnobLevel(uint8_t whichModEncoder, ParamManagerBase*
paramManager, uint32_t pos, TimelineCounter* playPositionCounter) { if
(whichModEncoder == 0) { channelEncoderCurrentOffset = 0;
    }
    return -64;
}
*/
...
void NonAudioDrum::modChange(ModelStackWithThreeMainThings* modelStack,
  ...
  *value = newValue;
  ...
  if (wasOn) { noteOn(modelStack, lastVelocity, NULL, zeroMPEValues); }
}
