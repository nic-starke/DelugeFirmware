...
void MenuItemIntegerContinuous::drawBar(int yTop, int marginL, int marginR) {
  ...
  /*
      ...
      OLED::drawVerticalLine(leftMost, y, y + endLineHalfHeight,
     OLED::oledMainImage); OLED::drawVerticalLine(rightMost, y, y +
     endLineHalfHeight, OLED::oledMainImage);
  */
  int          minValue          = getMinValue();
  int          maxValue          = getMaxValue();
  ...
  if (posHorizontal <= zeroPosHorizontal) {
    ...
    OLED::invertArea(xMin,
                     zeroPosHorizontal - posHorizontal + 1,
                     yTop,
                     yTop + height,
                     OLED::oledMainImage);
  } else {
