...
OrderedResizeableArray::OrderedResizeableArray(int newElementSize,
                                               int keyNumBits,
                                               int newKeyOffset,
    ...
    : ResizeableArray(newElementSize, newMaxNumEmptySpacesToKeep, newNumExtraSpacesToAllocate),
      keyMask(0xFFFFFFFF >> (32 - keyNumBits)), keyOffset(newKeyOffset),
      keyShiftAmount(32 - keyNumBits) {}

// With duplicate keys, this will work correctly, returning the leftmost
// matching (or greater) one if doing GREATER_OR_EQUAL, or the rightmost less
// one if doing LESS. This behaviour for duplicates can be tested with
// TEST_VECTOR_DUPLICATES / testDuplicates().
int OrderedResizeableArray::search(int32_t searchKey,
                                   int     comparison,
                                   int     rangeBegin,
                                   int     rangeEnd) {
...
// Like searchMultiple(), but much less complex as we know it's only doing 2
// search terms.
void OrderedResizeableArrayWith32bitKey::searchDual(int32_t const* __restrict__ searchTerms,
  ...
  while (rangeBegin != rangeEnd) {
    ...
    if (keyHere < searchTerms[0])
      ...
      // And we can also narrow down our next search, if that's going to be left
      // of our proposedIndex too.
...
// Returns results as if GREATER_OR_EQUAL had been supplied to search. To turn
// this into LESS, subtract 1 Results are written back into the searchTerms
// array
void OrderedResizeableArrayWith32bitKey::searchMultiple(int32_t* __restrict__ searchTerms,
  ...
  for (int t = 0; t < numSearchTerms; t++) {
    ...
    // Solve for this search term, putting aside valuable data as we narrow down
    // the range of items we're investigating. And we're also already making use
    // of any data we previously put aside - our rangeBegin and rangeEnd are
    // probably already fairly tight.
    while (rangeBegin != rangeEnd) {
      ...
      if (examiningElementPos >= searchTerms[t]) {
        ...
        // But we also want to make a note for any further search terms for
        // which their "defaultRangeEnd" could also be tightened to the same
        // point. How many more search terms to the right can we do this for?
        // We'll figure that out quickly by (ironically) searching through the
        // search terms to find the first one >= the element pos we're currently
        // looking at. Thankfully, we've already tightened the
        ...
        // If this tightened defaultRangeEnd is going to apply beyond just this
        // search term here, make a note by putting it on the stack.
        if (searchTermsRangeEnd > t + 1) {

          // Well, we only need a new stack entry if this new run ends before
          // the previous one did. Otherwise, we'll just overwrite that, so no
          // need to
          if (searchTermsRangeEnd < searchRecords[currentSearchRecord].lastsUntilSearchTerm) {

            // But, if the stack was going to overflow, the beauty is, we can
            // just not put our new item on the stack at all. Everything will
            // still work fine, though not quite as efficiently as it could have
            if (currentSearchRecord == maxNumSearchRecords - 1) continue;

            currentSearchRecord++;
            // if (currentSearchRecord > maxSearchRecord) maxSearchRecord =
            // currentSearchRecord;
          }
...
bool OrderedResizeableArrayWith32bitKey::generateRepeats(int32_t wrapPoint, int32_t endPos) {
  ...
  int oldNum = search(wrapPoint,
                      GREATER_OR_EQUAL); // Do this rather than just copying numElements
                                         // - this is better because it ensures we ignore
                                         // / chop off any elements >= wrapPoint
...
void OrderedResizeableArrayWith32bitKey::testSearchMultiple() {
  ...
  int32_t* searchPos = (int32_t*)generalMemoryAllocator.alloc(
      TEST_SEARCH_MULTIPLE_NUM_SEARCH_TERMS * sizeof(int32_t),
      NULL,
      false,
      false);
...
void OrderedResizeableArray::test() {
  ...
  while (true) {
    ...
    startAgain:
      int32_t value = ((uint32_t)getRandom255() << 16) | ((uint32_t)getRandom255() << 8) |
                      (uint32_t)getRandom255();
...
void OrderedResizeableArray::testDuplicates() {
  int count = 0;
  while (1) {
    if (!(count & 31)) { Uart::println("testing duplicate search..."); }
    count++;

    for (int i = 0; i < NUM_DUPLICATES_TO_TEST; i++) {
      int key = (getNoise() >> 16) & 1023;
      int num = getRandom255() % 7;
      for (int n = 0; n < num; n++) { insertAtKey(key); }
    }
    ...
    for (int j = 0; j < 1000; j++) {
      ...
      // If here, we got a key higher than our search key, so check the next key
      // to the left is lower
      if (i) {
...
// 32-bit key
OrderedResizeableArrayWith32bitKey::OrderedResizeableArrayWith32bitKey(
    int newElementSize,
    int newMaxNumEmptySpacesToKeep,
    int newNumExtraSpacesToAllocate)
    : OrderedResizeableArray(newElementSize,
                             32,
                             0,
                             newMaxNumEmptySpacesToKeep,
                             newNumExtraSpacesToAllocate) {}

void OrderedResizeableArrayWith32bitKey::shiftHorizontal(int32_t shiftAmount,
  ...
  int cutoffIndex = search(cutoffPos,
                           GREATER_OR_EQUAL); // This relates to the key/position cutoff - nothing
                                              // to do with the memory location wrap point!

  // Update the elements' keys (positions) - starting with those left of the
  // cutoffIndex.
  int i      = 0;
...
updateKeys:
  ...
  // If that was us just doing the ones left of the cutoffIndex, go back and do
  ...
  // If the leftmost element (in terms of key/position, *not* physical memory
  ...
  if (cutoffIndex && cutoffIndex < numElements) {
    ...
    if (memoryTooBigBy) {
      ...
      // If wrap, then do the smallest amount of memory moving possible to make
      // the ends touch
      ...
      if (numElementsBeforeWrap < numElements) {
        ...
        // Or, vice versa
          ...
          memmove((char*)memory + newMemoryStart * elementSize,
                  (char*)memory + memoryStart * elementSize,
                  numElementsBeforeWrap * elementSize);
          memoryStart = newMemoryStart;
        }
      }

      // Or if no wrap, just alter some parameters. (Remember, we will have
      // introduced a wrap, which we'll set up below.)
      else {
    ...
    // Finally, actually move the first-element-index along to reflect the new
    // leftmost element
