...
public:
  ...
  static int       readFileItemsForFolder(char const*  filePrefixHere,
                                          bool         allowFolders,
  ...
  static void
  deleteFolderAndDuplicateItems(int instrumentAvailabilityRequirement = AVAILABILITY_ANY);
  static PresetNavigationResult doPresetNavigation(int         offset,
  ...
  confirmPresetOrNextUnlaunchedOne(int     instrumentType,
                                   String* searchName,
                                   int     availabilityRequirement);
  static ReturnOfConfirmPresetOrNextUnlaunchedOne
              findAnUnlaunchedPresetIncludingWithinSubfolders(Song* song,
                                                              int   instrumentType,
                                                              int   availabilityRequirement);
  bool        opened();
...
protected:
  ...
  virtual int  arrivedInNewFolder(int         direction,
                                  char const* filenameToStartAt = NULL,
  ...
  static int   readFileItemsFromFolderAndMemory(Song*       song,
                                                int         instrumentType,
                                                char const* filePrefixHere,
                                                char const* filenameToStartAt,
                                                char const* defaultDirToAlsoTry,
                                                bool        allowFoldersint,
                                                int availabilityRequirement = AVAILABILITY_ANY,
                                                int newCatalogSearchDirection = CATALOG_SEARCH_RIGHT);

  static int fileIndexSelected; // If -1, we have not selected any real
                                // file/folder. Maybe there are no files, or
                                // maybe we're typing a new name.
  static int scrollPosVertical;
  static int numCharsInPrefix; // Only used for deciding Drum names within Kit.
                               // Oh and initial text scroll position.
