  ...
  // Or if reached 75% full, try getting more
    ...
    if (secondaryMemory) {
      ...
      while (secondaryMemoryFunctionCurrentIteration < secondaryMemoryNumBuckets) {
        void* sourceBucketAddress =
            secondaryMemoryGetBucketAddress(secondaryMemoryFunctionCurrentIteration);
...
void* OpenAddressingHashTable::lookup(uint32_t key) {
  ...
  while (true) {
    ...
    // If we've wrapped all the way around (which could only happen if table
    // 100% full)
    if (b == bInitial) break;
  }
...
bool OpenAddressingHashTable::remove(uint32_t key) {
  ...
  // If we've hit zero elements, and it's worth getting rid of the memory, just
  // do that
  if (!numElements && numBuckets > initialNumBuckets) {
    ...
    while (true) {
      ...
      // Bucket contains an element. What bucket did this element ideally want
      // to be in?
...
void OpenAddressingHashTable::test() {
  ...
  while (true) {
      ...
      do {
        elementsAdded[numElementsAdded] = getNoise() & 0xFF;
               ...
               (uint8_t)elementsAdded[numElementsAdded] ==
                   0xFF); // Don't allow 0 - we'll use that for special test. Or
                          // 0xFF, cos that means empty
    ...
    result = remove(0);
    if (result) {
      Uart::println("reported successful removal of element when there are no "
                    "elements at all");
      while (1)
        ;
    }
