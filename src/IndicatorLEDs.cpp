...
void setKnobIndicatorLevel(uint8_t whichKnob, uint8_t level) {
  ...
  // Without this here, pads turn on glitchily. Problem can also be solved by
  // turning the baud rate down, but it's a bit late for that! It's very weird
  // that this is the only command which causes this problem.
...
void clearKnobIndicatorLevels() {
  for (int i = 0; i < NUM_LEVEL_INDICATORS; i++) { setKnobIndicatorLevel(i, 0); }
}
