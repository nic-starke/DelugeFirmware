...
void MenuItemMIDIDevices::selectEncoderAction(int offset) {
  ...
  // Don't show devices which aren't connected. Sometimes we won't even have a
  // name to display for them.
...
#if HAVE_OLED
  midiDeviceMenu.basicTitle = soundEditor.currentMIDIDevice
                                  ->getDisplayName(); // A bit ugly, but saves us extending a class.
#endif
...
void MenuItemMIDIDevices::drawPixelsForOled() {
  ...
  while (r < OLED_MENU_NUM_OPTIONS_VISIBLE &&
         d < MIDIDeviceManager::hostedMIDIDevices.getNumElements()) {
