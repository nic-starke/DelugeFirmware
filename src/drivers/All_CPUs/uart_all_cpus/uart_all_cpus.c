...
#define DMA_SCIM_TX_CONFIG              \
  (0b00000000001000000000000000101000 | \
   DMA_AM_FOR_SCIM) // LVL is 0 for SCIM despite the datasheet saying it should
                    // be 1 - a misprint, I believe
...
int uartFlush(int item) {
  ...
  if (num < 0) {
    ...
    // If there are also some further bytes starting from the left of the
    // circular buffer that we want to send as well, set that up to happen
    // automatically
    if (numLeft) {
      DMACn(txDmaChannels[item]).N1TB_n = numLeft;
      newConfig |= DMAC_CHCFG_0S_REN | DMAC_CHCFG_0S_RSW |
                   DMAC_CHCFG_0S_DEM; // Set to switch to next "register" after first
                                      // transaction, and also mask interrupt
    }
  }
...
void uartFlushIfNotSending(int item) {
  ...
  if (!uartItems[item].txSending) {
    // There should be no way the DMA TX-complete interrupt could occur in this
    // bit, cos we could only be here if it'd already completed and set
    // txSending to 0...
  ...
  // Applies to MIDI only - if sending was already happening, take note that we
  // want to send additional stuff as soon as that's done. For PIC, this always
  // happens anyway
  else {
    // WARNING - it'd be a problem if the DMA TX-finished interrupt occurred
    // right here... And when not using volatiles, we also could end up here if
    // that interrupt occurred shortly before the if statement above at the
    // start
...
int uartGetTxBufferFullnessByItem(int item) {
  return (uartItems[item].txBufferWritePos - uartItems[item].txBufferReadPos) &
         (txBufferSizes[item] - 1);
}
...
uint8_t uartGetChar(int item, char_t* readData) {
  ...
  char_t const* currentWritePos =
      (char_t*)DMACnNonVolatile(rxDmaChannels[item]).CRDA_n; // We deliberately don't go (volatile
                                                             // uint32_t*) here, for speed
...
uint32_t* uartGetCharWithTiming(int timingCaptureItem, char_t* readData) {
  ...
  *readData = *(rxBufferReadAddr[item] + UNCACHED_MIRROR_OFFSET);
  ...
  uint32_t* timer = (uint32_t*)((uint32_t)&timingCaptureBuffers
                                    [timingCaptureItem]
...
// This is the function which seems to cause a crash if called via interrupt
// during the above one
void tx_interrupt(int item) {
  ...
  // Clear Transfer End Bit Status - but hey we actually don't need to do this
  // to clear the interrupt - provided we're not going to be reading these flags
  // later
  //*g_dma_reg_tbl[txDmaChannels[item]].chctrl |= (DMAC0_CHCTRL_n_CLREND |
  // DMAC0_CHCTRL_n_CLRTC);
}

// (I think) this has to be called after Uart initialized, otherwise problem
