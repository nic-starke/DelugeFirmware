...
void oledMainInit() {
  ...
  */
  ...
  MainOLED_WCom(0x40); // SET DISPLAY START LINE (040H-07FH) // Moves entire
                       // graphics vertically
  ...
  /*
 R_INTC_RegistIntFunc(INTC_ID_RSPI_SPTI0 + SPI_CHANNEL_OLED_MAIN * 3,
 oledTransferComplete); R_INTC_SetPriority(INTC_ID_RSPI_SPTI0 +
 SPI_CHANNEL_OLED_MAIN * 3, 13); // Not very important
...
void enqueueSPITransfer(int destinationId, uint8_t const* image) {
  ...
  while (readPosNow != spiTransferQueueWritePos) {
      if (spiTransferQueue[readPosNow].destinationId == destinationId &&
  spiTransferQueue[readPosNow].dataAddress == image) return; readPosNow =
  (readPosNow + 1) & (SPI_TRANSFER_QUEUE_SIZE - 1);
  }
  */
  ...
  // If DMA not currently sending, and our new entry is still in the queue (it
  // didn't get sent inside an interrupt just now), then send it now.
...
void oledDMAInit() {
  ...
  DMACn(OLED_SPI_DMA_CHANNEL).CHCFG_n =
      0b00000000001000000000001001101000 | (OLED_SPI_DMA_CHANNEL & 7);
  ...
  R_INTC_SetPriority(DMA_INTERRUPT_0 + OLED_SPI_DMA_CHANNEL,
                     13); // Priority is not very important
