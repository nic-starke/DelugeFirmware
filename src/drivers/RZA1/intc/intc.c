 ...
 * Copyright (C) 2012 - 2014 Renesas Electronics Corporation. All rights
 *reserved.
 *******************************************************************************/
/*******************************************************************************
 * File Name   : intc.c
...
#define INTC_ICDISR_REG_TOTAL                                                 \
  (((uint16_t)INTC_ID_TOTAL / 32) + 1)                              /* ICDISR \
                                                                     */
...
#define INTC_ICDIPTR_REG_TOTAL                                                 \
  (((uint16_t)INTC_ID_TOTAL / 4) + 1)                               /* ICDIPTR \
                                                                     */
...
******************************************************************************/
/* ==== Global variable ==== */
static uint32_t intc_icdicfrn_table[] = /* Initial value table of Interrupt
...
int32_t R_INTC_RegistIntFunc(uint16_t int_id, void (*func)(uint32_t int_sense)) {
  /* ==== Argument check ==== */
  if (int_id >= INTC_ID_TOTAL) { return DEVDRV_ERROR; /* Argument error */ }
...
void R_INTC_Init(void) {
  ...
  for (offset = 8; offset < INTC_ICDIPTR_REG_TOTAL;
       offset++) /* Do not initialize ICDIPTR0 to ICDIPTR7 */
  {
  ...
  INTC.ICCBPR = 0x00000002uL; /* Group priority field [7:3], Subpriority field
                                 [2:0](Do not use) */
 ...
 * Return Value : DEVDRV_SUCCESS    : Success of INTC interrupt priority level
 *setting : DEVDRV_ERROR      : Failure of INTC interrupt priority level setting
 ******************************************************************************/
int32_t R_INTC_SetPriority(uint16_t int_id, uint8_t priority) {
  ...
  /* ==== Argument check ==== */
  if ((int_id >= INTC_ID_TOTAL) || priority >= 32) { return DEVDRV_ERROR; /* Argument error */ }
 ...
 * Return Value : DEVDRV_SUCCESS     : Success of INTC interrupt mask level
 *setting : DEVDRV_ERROR       : Failure of INTC interrupt mask level setting
 ******************************************************************************/
int32_t R_INTC_SetMaskLevel(uint8_t mask_level) {
  volatile uint8_t dummy_buf_8b;

  /* ==== Argument check ==== */
  if (mask_level >= 32) { return DEVDRV_ERROR; /* Argument error */ }
 ...
 * Arguments    : uint16_t int_id    : Interrupt ID (INTC_ID_TINT0 to
 *INTC_ID_TINT170) : uint32_t int_sense : Interrupt detection : :
 *INTC_LEVEL_SENSITIVE : Level sense :                    :   INTC_EDGE_TRIGGER
 *: Edge trigger Return Value : DEVDRV_SUCCESS : Success of INTC interrupt
 *configuration setting : DEVDRV_ERROR   : Failure of INTC interrupt
