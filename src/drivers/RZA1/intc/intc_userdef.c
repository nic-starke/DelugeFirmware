 ...
 *function : registered as an element of the int_id in the INTC interrupt
 *handler : function table intc_func_table[] is executed. Arguments    :
 *uint16_t int_id    : Interrupt ID : uint32_t int_sense : Interrupt detection
 *              :                    :   INTC_LEVEL_SENSITIVE : Level sense
 *              :                    :   INTC_EDGE_TRIGGER    : Edge trigger
 * Return Value : none
 ******************************************************************************/
...
void Userdef_FIQ_HandlerExe(void) {
  /* Add processing when executing the processing at the time FIQ interrupt was
   * generated */
  while (1) { /* Do Nothing */
  }
}
