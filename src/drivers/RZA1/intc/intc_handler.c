 ...
 *IRQ : multiple interrupts are enabled. The processing for unsupported :
 *interrupt ID is executed by calling Userdef_INTC_UndefId function. : In the
 *interrupt handler processing, when the int_sense shows :
 *"INTC_LEVEL_SENSITIVE", clear the interrupt source because it : means a level
 *sense interrupt. Arguments    : uint32_t icciar : Interrupt ID (value of
 *ICCIAR register) Return Value : none
 ******************************************************************************/
void INTC_Handler_Interrupt(uint32_t icciar) {
  /* Stacks are restored by ASM with the top level to correspond to multiple
   * interrupts */
  uint32_t           mask;
  uint32_t           int_sense;
  uint16_t           int_id;
  ...
  {
    // Insane thing that keeps happening - we get here somehow, with int_id
    // 1023.
    // freezeWithError("i029");
    uartPrintln("i029 ----------------------------------------------------!!");
    return; // Just keep running - it seems to work at least most of the time?
    // Userdef_INTC_UndefId(int_id); // Previously, it'd just go in here and
    // freeze.
  }
 ...
 * Description  : This function is the INTC interrupt handler processing called
 *by : the fiq_handler. Arguments    : none Return Value : none
 *******************************************************************************/
