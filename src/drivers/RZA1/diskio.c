...
DRESULT
disk_read(BYTE  pdrv,   /* Physical drive nmuber (0) */
  ...
  loadAnyEnqueuedClustersRoutine(); // Always ensure SD streaming is fulfilled
...
#define FCLK_SLOW() \
  RSPI0.SPBR =      \
      ceil((float)66666666 / (400000 * 2) - 1); /* Set slow clock, 100k-400k (Nothing to do) */
// #define   FCLK_FAST() RSPI0.SPBR = ceil((float)66666666 / (25000000 * 2) -
// 1);        /* Set fast clock, depends on the CSD (Nothing to do) */
...
static void setDataLength(BYTE newLength) {
  ...
  if (currentDataLength == 32) {
    ...
    RSPIx.SPBFCR = 0b00110010; // Transmit trigger number set to 0 bytes (only trigger when
                               // it's completely empty). You'd think setting it to 4 bytes
                               // would be faster, but somehow that doesn't work with DMA.
                               // RSPIx.SPBFCR = 0b00100010; // 4 bytes

    //*g_dma_reg_tbl[send_dma_channel].chcfg =    0b0001100100010001001100000 |
    //(send_dma_channel % 8u); *g_dma_reg_tbl[receive_dma_channel].chcfg =
    // 0b0000100100010001001100000 | (receive_dma_channel % 8u);
    ...
    //*g_dma_reg_tbl[send_dma_channel].chcfg =  0b0001100000000001001100000 |
    //(send_dma_channel % 8u); *g_dma_reg_tbl[receive_dma_channel].chcfg =
    // 0b0000100000000001001100000 | (receive_dma_channel % 8u);
  }
}
...
static void xmit_spi_multi(const BYTE* buff, /* Data to be sent */
  ...
  do {
    ...
    // RSPIx.SPDR.UINT32 = ((uint32_t)(*buff++)) | ((uint32_t)(*buff++) << 8) |
    // ((uint32_t)(*buff++) << 16) | ((uint32_t)(*buff++) << 24);
    ...
    if (doneAny) {
      ...
      // Receive data. Note that even if we didn't want the receive data, we
      // still have to read it back, because SPI transmission halts once the RX
      // buffer is full
      RSPIx.SPDR.UINT32;
    } else
      doneAny = true;
  ...
  // Receive data. Note that even if we didn't want the receive data, we still
  // have to read it back, because SPI transmission halts once the RX buffer is
  // full
  RSPIx.SPDR.UINT32;
}
...
static void spiDMARx(BYTE* buff, UINT cnt) {
  ...
  // Invalidate - before doing the read. For some weird reason it doesn't work
  // unless we do this. We invalidate after, also - I suspect that that helps
  // too
...
static void mmcDMAInterrupt(uint32_t int_sense) {
  ...
  if (sdDirection == SD_DIRECTION_READING) {
    ...
    if (transferStatus == DMA_STATUS_WAIT_DATA_TOKEN) {
      ...
      if (dmaTokenBuffer[2] == 0xFF) {
        ...
        // Otherwise, try again...
          ...
          // Add a delay before each SPI transaction - we're expecting to have
          // to keep quizzing for a while, and we want to avoid doing this very
          // often.
    ...
    // Or if we were waiting for a sector to transfer...
      ...
      // Invalidate cache. For some reason I can't work out, this has to be done
      // here, once per sector. Trying to do it for the whole transfer at the
      // end leads to CORR errors when flicking through presets fast. Update -
      // currently doing it before read, too. That in conjunction with after
      // whole read would probably be enough.
    ...
    if (transferStatus == DMA_STATUS_WAIT_SECTOR) {
      ...
      if (transferStatus == DMA_STATUS_WAIT_READY) {
        ...
        if ((dmaTokenBuffer[2] & 0x1F) != 0x05) {
          ...
          // Do a wait-ready-for-next-command. Even if we've now finished all
          // sectors, we'll need to wait to be ready to send the STOP command
...
static void rcvr_spi_multi(BYTE* buff, /* Buffer to store received data */
  ...
  // Non-DMA code - weirdly runs a bit faster. Loading 512 bytes on the max
  // clock setting only takes 0.123 mS anyway. Biggest problem is that calling
  // the audio routine is likely to take longer than this (at least at heavy
  // load when it matters), so we're slowed way down.
  setDataLength(32);
...
void spiDMAWriteSectors(BYTE* buff, UINT* count) {
  ...
  while (transferStatus < DMA_STATUS_COMPLETE) { routineForSD(); }
...
static BYTE send_cmd(BYTE  cmd, /* Command byte */
  ...
  RSPIx.SPDR.UINT8[0] = 0xFF; // Extra one we're not going to add to the count, because we want to
                              // read it back after separately after reading back the counted ones
  ...
  while ((res & 0x80) && --n) { res = xchg_spi(0xFF); }
...
void spiDMAReadSectors(BYTE* buff, UINT* count) {
  ...
  Timer1                           = 1000;
  transferStatus                   = DMA_STATUS_WAIT_DATA_TOKEN;
  DMACn(send_dma_channel).CHITVL_n = 8192; // It'll take ages to respond to this first one, so don't
                                           // bother quizzing it often
  spiDMARx(dmaTokenBuffer + 2, 1);

  do {
    routineForSD();

    // Nadia was getting timeouts somehow - the interrupt wasn't getting called.
    // Will catching them here work, in the absence of a proper solution?
    if (!Timer1) { transferStatus = DMA_STATUS_ERROR; }
  } while (transferStatus < DMA_STATUS_COMPLETE);

  // If no error and we just finished the whole transfer, we'll have a CRC to
  // discard at the end of the final block
  if (transferStatus == DMA_STATUS_COMPLETE) {
    xchg_spi(0xFF); // Discard CRC
    xchg_spi(0xFF);
    *count = 0;

    // Ideally we'd expect to just invalidate the cache here. But for some
    // reason I can't work out, that doesn't work. Instead, it's done per sector
    // in the interrupt function - see there. Update - now it's done before,
    // too.
...
DRESULT
disk_read_without_streaming_first(BYTE  pdrv,   /* Physical drive nmuber (0) */
                                  BYTE* buff,   /* Pointer to the data buffer to store read data */
                                  LBA_t sector, /* Start sector number (LBA) */
                                  UINT  count   /* Sector count (1..128) */
) {
  ...
  if (commandResult == 0) {
    ...
    if (count == 1) {
      ...
      // Theoretically, I should be able to replace the above with this
      // function, which does DMA. It doesn't work for some reason. It's ok -
      // since this would only be for 1 sector, I have this idea that this might
      // actually be faster.
      // spiDMAReadSectors(buff, &count);
    }

    else {
      // Have deactivated DMA here, because Nadia's card was occasionally
      // locking these up.
  ...
  if (send_cmd(CMD0, 0) == 1) {                        /* Enter Idle state */
    ...
    if (send_cmd(CMD8, 0x1AA) == 1) {                  /* SDv2? */
      ...
      if (ocr[2] == 0x01 && ocr[3] == 0xAA) { /* The card can work at vdd range of 2.7-3.6V */
        while (Timer1 && send_cmd(ACMD41, 0x40000000)) { /* Wait for leaving idle state
                                                            (ACMD41 with HCS bit) */
          routineForSD();
        }
...
// Actually this buffer never gets used, so I've removed it! - Rohan
...
disk_initialize(BYTE pdrv /* Physical drive nmuber to identify the drive */
  ...
  // sd_set_buffer(SD_PORT, &test_sd_rw_buff[0], SD_RW_BUFF_SIZE); // Actually
  // this buffer never gets used, so I've removed it! - Rohan
...
DRESULT
disk_read_without_streaming_first(BYTE  pdrv,   /* Physical drive nmuber to identify the drive */
  ...
  */
  ...
  // My good 16gb card gave about 150 per read. Bad card gave ~250, and
  // occasionally up to 30,000!
...
DRESULT disk_ioctl(BYTE  pdrv, /* Physical drive nmuber (0..) */
  ...
  switch (cmd) {
    case CTRL_SYNC:  /* Flush write-back cache, Wait for end of internal process */
      return RES_OK; // I think this just means "yes, you can do some writing
                     // now", which you always can. Rohan
