...
uint32_t midiRxTimingBuffer[MIDI_RX_TIMING_BUFFER_SIZE]
    __attribute__((aligned(CACHE_LINE_SIZE))); // I'd like to store just 16 bits per entry for this,
                                               // but the DMA wouldn't do it - whether or not I also
                                               // set its source data size to 16 bits
...
void Userdef_SCIF_UART_Init(uint8_t channel, uint8_t mode, uint16_t cks, uint32_t baudRate) {
  ...
  // I've deactivated this because I think all clocks are switched on in stb.c,
  // and we don't want just channel 2 anymore
  /*
...
*/
  ...
  b7 BGDM - Baud rate generator double-speed mode  : Double speed mode
  (increased by Rohan) b0 ABCS - Base clock select in asynchronous mode : Base
  clock is 16 times the bit rate */
...
const uint8_t txInterruptPriorities[] = {
    5,
    13 // This has to be higher number / lower priority than the MIDI-send timer
       // interrupt
};
...
const uint32_t midiUartDmaRxTimingLinkDescriptor[] __attribute__((aligned(CACHE_LINE_SIZE))) = {
    ...
    0b10000001000100100010000000100000 | DMA_AM_FOR_SCIF |
        (MIDI_RX_TIMING_DMA_CHANNEL & 7),       // Config. LVL has to be set to 0 for some reason,
                                                // otherwise we get duplicates of each value
    0,                                          // Interval
    0,                                          // Extension
    (uint32_t)midiUartDmaRxTimingLinkDescriptor // Next link address (this one again)
};

const uint32_t* const uartRxLinkDescriptors[] = {picUartDmaRxLinkDescriptor,
                                                 midiUartDmaRxLinkDescriptor};
