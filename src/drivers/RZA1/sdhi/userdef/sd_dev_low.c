...
static int sddev_init_0(void) {
  ...
  while (1) {
    if (sddev_check_timer() == SD_ERR) { break; }
  }
  sddev_end_timer();
...
static int sddev_init_1(void) {
  ...
  /* ---- wait card detect ---- */
  // I've commented this out. It makes it wait one second, regardless of
  // anything. Is this ever necessary? Part of the standard or something? But
  // it'd already have been getting power... - Rohan
...
/******************************************************************************
 * Function Name: int sddev_read_data(int sd_port, unsigned char *buff,unsigned
 *long reg_addr,long num); Description  : read from SDHI buffer FIFO Arguments
 *: unsigned char *buff    : buffer addrees to store reading datas : unsigned
 *long reg_addr : SDIP FIFO address : long num               : counts to
 *read(unit:byte) Return Value : success : SD_OK : fail    : SD_ERR
 ******************************************************************************/
...
/******************************************************************************
 * Function Name: int sddev_write_data(int sd_port, unsigned char *buff,unsigned
 *long reg_addr,long num); Description  : write to SDHI buffer FIFO Arguments :
 *unsigned char *buff    : buffer addrees to store writting datas : unsigned
 *long reg_addr : SDIP FIFO address : long num               : counts to
 *write(unit:byte) Return Value : success : SD_OK : fail    : SD_ERR
 ******************************************************************************/
...
/******************************************************************************
 * Function Name: int sddev_init_dma(unsigned long buff,unsigned long reg,long
 *cnt,int dir); Description  : Initialize DMAC to transfer data from SDHI FIFO
...
/******************************************************************************
 * Function Name: static int sddev_init_dma_0(unsigned long buff,unsigned long
 *reg,long cnt,int dir); Description  : Initialize DMAC to transfer data from
 *SDHI FIFO Arguments    : unsigned long buff : buffer addrees to transfer datas
 *              : unsigned long reg  : SDIP FIFO address
 *              : long cnt           : counts to transfer(unit:byte)
 *              : int dir            : direction to transfer
 *              :                    :   0 : FIFO -> buffer
...
static int sddev_init_dma_0(unsigned long buff, unsigned long reg, long cnt, int dir) {
  ...
  sd_DMAC_PeriReqInit((const dmac_transinfo_t*)&trans_info,
                      ...
                      request_factor,
                      0,
                      SD0_DMA_CHANNEL); /* Dont care DMAC_REQ_REQD is setting in
...
/******************************************************************************
 * Function Name: static int sddev_init_dma_1(unsigned long buff,unsigned long
 *reg,long cnt,int dir); Description  : Initialize DMAC to transfer data from
 *SDHI FIFO Arguments    : unsigned long buff : buffer addrees to transfer datas
 *              : unsigned long reg  : SDIP FIFO address
 *              : long cnt           : counts to transfer(unit:byte)
...
static int sddev_init_dma_1(unsigned long buff, unsigned long reg, long cnt, int dir) {
  ...
  sd_DMAC_PeriReqInit((const dmac_transinfo_t*)&trans_info,
                      ...
                      request_factor,
                      0,
                      SD1_DMA_CHANNEL); /* Dont care DMAC_REQ_REQD is setting in
                                           usb0_host_DMAC1_PeriReqInit() */
...
static int sddev_wait_dma_end_1(long cnt) {
  ...
  if (time < 2000)
    time = 2000; // I've seen block write operations sometimes just randomly
                 // take as long as 1250ms, despite it normally being 2ms
 ...
 * Description  : Setting Interrupt function for
 *SDHI(INTC_ID_SDHI0_0,INTC_ID_SDHI0_3) Arguments    : Interrupt mode Return
 *Value : none
 ******************************************************************************/
static void sddev_sd_int_handler_0(uint32_t int_sense) {
  sd_int_handler(0);
