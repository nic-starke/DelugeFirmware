...
#define SD_RESP0                                                                   \
  ((0x0cu << SD_REG_SHIFT) + SD_BYTE_OFFSET)                     /* Response R23-8 \
                                                                  */
...
#define SD_OPTION                                                                   \
  ((0x28u << SD_REG_SHIFT) + SD_BYTE_OFFSET)                       /* Access Option \
                                                                    */
...
#define SIZE_CARD_1008MB                               \
  (1008 * 1024 * 1024 / 512) /* 1008*1MB/(sector size) \
                              */
#define SIZE_CARD_1024MB                               \
  (1024 * 1024 * 1024 / 512) /* 1024*1MB/(sector size) \
                              */
#define SIZE_CARD_2016MB                                                              \
  (2016 * 1024 * 1024 / 512)                                /* 2016*1MB/(sector size) \
                                                             */
...
#define SIZE_CARD_16384MB                            \
  (16384ul * 1024ul * 2ul) /* 2048*(1MB/sector size) \
                            */
#define SIZE_CARD_32768MB                                                         \
  (32768ul * 1024ul * 2ul)                              /* 2048*(1MB/sector size) \
                                                         */
...
typedef struct __sdhndl {          /* SD handle */
  ...
  unsigned short status_data[18 / sizeof(unsigned short)]; /* STATUS DATA value (phys spec
                                                              ver1.10) */
  unsigned short io_len[8]; /* io block length common:0 func:more than 1 */
  unsigned char  io_reg[8][SDIO_INTERNAL_REG_SIZE /
                          sizeof(unsigned char)]; /* CCCR(=0) and FBR(1 to 7) value */
  unsigned char  cis[8]
                   [SDIO_INTERNAL_CIS_SIZE / sizeof(unsigned char)]; /* CIS value (to be fixed) */
...
typedef struct _sd_format_param {
  ...
  int (*write)(SDHNDL*                  hndl,
               struct _sd_format_param* sdfmt,
               unsigned char            fill,
               unsigned long            secno,
               long                     seccnt,
               int (*callback)(unsigned long, unsigned long));
  /* format erase function  */
  int (*erase)(SDHNDL*                  hndl,
               struct _sd_format_param* sdfmt,
               unsigned long            secno,
               long                     seccnt,
               int (*callback)(unsigned long, unsigned long));
...
/* ---- sdio_trns.c ---- */
int _sdio_software_trans(SDHNDL*        hndl,
                         unsigned char* buff,
                         long           cnt,
                         int            dir,
...
int  _sd_card_send_cmd_arg(SDHNDL*        hndl,
                           unsigned short cmd,
                           int            resp,
                           unsigned short h_arg,
                           unsigned short l_arg);
