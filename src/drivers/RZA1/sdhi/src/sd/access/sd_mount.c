 ...
 *__CC_ARM - #endif) :                         Modified
 *_sd_card_switch_func_access_mode1()
 *******************************************************************************/
#include "../../../inc/sdif.h"
 ...
 * Include      :
 * Declaration  : int sd_mount(int sd_port, unsigned long mode,unsigned long
 *voltage); Functions    : mount SD memory card user area : can be access user
 *area after this function is finished : without errors : turn on power
 *              :
...
int sd_mount(int sd_port, unsigned long mode, unsigned long voltage) {
  ...
  // This buffer is actually never used! Perhaps I removed whatever used it. -
  // Rohan
  ...
  if (_sd_card_init(hndl) != SD_OK) { goto ERR_EXIT; /* failed card initialize */ }

  if (hndl->media_type & SD_MEDIA_MEM) { /* with memory part */
    /* ==== check card registers ==== */
    /* ---- check CSD register ---- */
    if (_sd_check_csd(hndl) != SD_OK) { goto ERR_EXIT; }

    /* ---- no check other registers (to be create) ---- */

    /* get user area size */
    if (_sd_get_size(hndl, SD_USER_AREA) != SD_OK) { goto ERR_EXIT; }
  ...
  if (hndl->media_type & SD_MEDIA_IO) { /* with IO part */
    if (_sd_io_mount(hndl) != SD_OK) { goto ERR_EXIT; }
  }

  if (hndl->media_type & SD_MEDIA_MEM) { /* with memory part */
    if (_sd_mem_mount(hndl) != SD_OK) { goto ERR_EXIT; }
  ...
  if (hndl->media_type & SD_MEDIA_SD) {
    if (_sd_card_get_scr(hndl) != SD_OK) { goto ERR_EXIT; }
    ...
    if (spec) { /* ---- more than phys spec ver1.10 ---- */
      ...
      if (hndl->sup_speed == SD_MODE_HS) {
        ...
        if (_sd_set_mem_speed(hndl) != SD_OK) { goto ERR_EXIT; }
      }
  ...
  if (hndl->media_type & SD_MEDIA_IO) {
    if (hndl->sup_speed == SD_MODE_HS) {
      if (_sd_set_io_speed(hndl) != SD_OK) { goto ERR_EXIT; }
    }
...
int _sd_card_init(SDHNDL* hndl) {
  ...
  if (hndl->sup_card & SD_MODE_IO) {
    ...
    if (ret == SD_OK) {
      /* set OCR (issue CMD5) */
      if (_sd_card_send_ocr(hndl, SD_MEDIA_IO) != SD_OK) { return SD_ERR; }
  ...
  if (hndl->media_type == SD_MEDIA_UNKNOWN) {
    ...
    if (ret != SD_OK) { return SD_ERR; /* error for CMD0 */ }
  ...
  /* ---- get CID (issue CMD2) ---- */
  if (_sd_card_send_cmd_arg(hndl, CMD2, SD_RESP_R2_CID, 0, 0) != SD_OK) { return SD_ERR; }

GET_RCA:
  /* ---- get RCA (issue CMD3) ---- */
  if (hndl->media_type & SD_MEDIA_COMBO) { /* IO or SD */
    for (i = 0; i < 3; i++) {
      if (_sd_card_send_cmd_arg(hndl, CMD3, SD_RESP_R6, 0, 0) != SD_OK) { return SD_ERR; }
      if (hndl->rca[0] != 0x00) {
        if (hndl->media_type & SD_MEDIA_IO) { hndl->io_flag |= SD_IO_POWER_INIT; }
        break;
      }
    }
    /* illegal RCA */
  ...
  if (hndl->media_type == SD_MEDIA_IO) { return SD_OK; }
  ...
  if (hndl->media_type & SD_MEDIA_MEM) {
    ...
    if (hndl->csd[3] & 0x0010u) { /* implimented */
      /* set DSR (issue CMD4) */
      hndl->dsr[0] = 0x0404;
      if (_sd_card_send_cmd_arg(hndl, CMD4, SD_RESP_NON, hndl->dsr[0], 0x0000) != SD_OK) {
        return SD_ERR;
      }
    }
  }
...
int _sd_io_mount(SDHNDL* hndl) {
  ...
  if (_sdio_direct(hndl, &io_buff, 0, 0x08, 0, 0) != SD_OK) { goto ERR_EXIT; }
  ...
  /* ---- get CCCR value ---- */
  if (_sdio_read_byte(hndl, hndl->io_reg[0], 0, 0, SDIO_INTERNAL_REG_SIZE, SD_IO_INCREMENT_ADDR) !=
      SD_OK) {
    goto ERR_EXIT;
  }
  ...
  if ((hndl->io_reg[0][0x08] & 0x02) != 0) {
    ...
    if (hndl->trans_mode & SD_MODE_DMA) {
      if (hndl->trans_mode & SD_MODE_DMA_64) {
        if (len == 32) { hndl->io_len[0] = 0xffff; /* already but not supported */ }
      }
    }
#endif

    for (i = 1; i < 8; i++) { hndl->io_len[i] = 0; /* not yet */ }
  } else {
    for (i = 0; i < 8; i++) { hndl->io_len[i] = 0xffff; /* already but not supported */ }
  }
...
int _sd_mem_mount(SDHNDL* hndl) {
  ...
  if ((hndl->media_type & SD_MEDIA_IO) == 0) {
    ...
    if (_sd_set_clock(hndl, (int)hndl->csd_tran_speed, SD_CLOCK_ENABLE) != SD_OK) { goto ERR_EXIT; }
  }
  ...
  if (_sd_card_send_cmd_arg(hndl, CMD16, SD_RESP_R1, 0x0000, 0x0200) != SD_OK) { goto ERR_EXIT; }

  /* if 4bits transfer supported (SD memory card mandatory), change bus width
   * 4bits */
  if (hndl->media_type & SD_MEDIA_SD) { _sd_set_port(hndl, hndl->sup_if_mode); }

  /* clear pull-up DAT3 */
  if (hndl->media_type & SD_MEDIA_SD) {
    if (_sd_send_acmd(hndl, ACMD42, 0, 0) != SD_OK) { goto ERR_EXIT; }
    /* check R1 resp */
    if (_sd_get_resp(hndl, SD_RESP_R1) != SD_OK) { goto ERR_EXIT; }
  }

  /* if SD memory card, get SD Status */
  if (hndl->media_type & SD_MEDIA_SD) {
    if (_sd_card_get_status(hndl) != SD_OK) { goto ERR_EXIT; }
    /* get protect area size */
    if (_sd_get_size(hndl, SD_PROT_AREA) != SD_OK) { goto ERR_EXIT; }
  }
...
int _sd_set_io_speed(SDHNDL* hndl) {
  ...
  if ((hndl->io_reg[0][0] & 0x0F) == 0x02) {
    ...
    if (hndl->io_reg[0][0x13] & 0x01) {
      ...
      io_buff = 0x02;
      if (_sdio_direct(hndl, &io_buff, 0, 0x13, 1, SD_IO_VERIFY_WRITE) != SD_OK) { return SD_ERR; }
...
int _sd_set_mem_speed(SDHNDL* hndl) {
  /* query func */
  if (_sd_card_switch_func(hndl, 0x00FF, 0xFF00) != SD_OK) { goto ERR_EXIT; }
  if (hndl->speed_mode & SD_SUP_SPEED) { /* high speed supported */

    /* make transfer card high speed mode */
    if (_sd_card_switch_func_access_mode1(hndl, 0x80FF, 0xFF01) != SD_OK) { goto ERR_EXIT; }

    /* case of combo card, set clock frequency high speed after transfering  io
     * part high speed */
    if (hndl->media_type == SD_MEDIA_SD) {
      hndl->csd_tran_speed = SD_CLK_50MHz;
      hndl->speed_mode |= SD_CUR_SPEED;
...
int sd_unmount(int sd_port) {
  ...
  hndl = _sd_get_hndls(sd_port);
  if (hndl == 0) { return SD_ERR; /* not initialized */ }
...
int _sd_card_switch_func_access_mode0(SDHNDL* hndl, unsigned short h_arg, unsigned short l_arg) {
  ...
  for (loop = 0; loop < 3; loop++) {
    ...
    for (i = 0; i < 9; i++) {
      hndl->status_data[i] =
          (stat_buff[hndl->sd_port][i] << 8) | (stat_buff[hndl->sd_port][i] >> 8);
    }
...
int _sd_card_get_status(SDHNDL* hndl) {
  ...
  if (_sd_read_byte(hndl, ACMD13, 0, 0, rw_buff, SD_STATUS_BYTE) != SD_OK) { return SD_ERR; }

  /* ---- distinguish SD ROM card ---- */
  if ((rw_buff[2] & 0xffu) == 0x00) { /* [495:488] = 0x00 */
    ret = SD_OK;
    if ((rw_buff[3] & 0xffu) == 0x01) { hndl->write_protect |= SD_WP_ROM; }
  } else {
    ret = SD_ERR;
...
int _sd_card_get_scr(SDHNDL* hndl) {
  ...
  if (_sd_read_byte(hndl, ACMD51, 0, 0, rw_buff, SD_SCR_REGISTER_BYTE) != SD_OK) { return SD_ERR; }
 ...
 *****************************************************************************/
int _sd_read_byte(SDHNDL*        hndl,
                  unsigned short cmd,
                  unsigned short h_arg,
                  unsigned short l_arg,
  ...
  if (cmd & 0x0040u) { /* ACMD13, ACMD22 and ACMD51 */
    ...
    if (_sd_send_cmd(hndl, cmd) != SD_OK) { return SD_ERR; }
  }
  /* ---- check R1 response ---- */
  if (_sd_get_resp(hndl, SD_RESP_R1) != SD_OK) { goto ErrExit; }
  ...
  /* transfer data */
  if (sddev_read_data(hndl->sd_port,
                      readbuff,
                      (unsigned long)(hndl->reg_base + SD_BUF0),
                      (long)byte) != SD_OK) {
    _sd_set_err(hndl, SD_ERR_CPU_IF);
    goto ErrExit;
  }
 ...
 * Remark       : transfer type is PIO
 *****************************************************************************/
int _sd_write_byte(SDHNDL*        hndl,
                   unsigned short cmd,
                   unsigned short h_arg,
                   unsigned short l_arg,
  ...
  /* transfer data */
  if (sddev_write_data(hndl->sd_port,
                       writebuff,
                       (unsigned long)(hndl->reg_base + SD_BUF0),
                       (long)byte) != SD_OK) {
    _sd_set_err(hndl, SD_ERR_CPU_IF);
  ...
  if (hndl->int_info2 & SD_INFO2_MASK_ERR) {
    ...
    if (time_out == SD_TIMEOUT_ERASE_CMD) {
      ...
      if (hndl->error == SD_ERR_CARD_TOE) {
        ...
        if (_sd_wait_rbusy(hndl, 10000000) != SD_OK) { goto ErrExit; }
      } else {
        goto ErrExit;
      }
    } else {
...
int _sd_calc_erase_sector(SDHNDL* hndl) {
  ...
  if ((hndl->scr[0] & 0x0f00) == 0x0200) {
    ...
    if ((au > 0) && (au < 0x0a)) {
      ...
      if (erase_size != 0) { hndl->erase_sect *= erase_size; }
    }
...
static int _sd_card_query_partitions(SDHNDL* hndl, int sub, unsigned char* rw_buff) {
  ...
  if (_sd_read_byte(hndl, CMD45, (unsigned short)sub, 0, rw_buff, SD_QUERY_PARTITION_LIST_BYTE) ==
      SD_OK) {
...
int esd_select_partition(int sd_port, int id) {
  ...
  if (!hndl->mount) { return SD_ERR; /* not mounted yet */ }
  /* ---- supply clock (data-transfer ratio) ---- */
 ...
 * Include      :
 * Declaration  : int esd_query_partition(int sd_port, int sub, unsigned char
 **data); Functions    : Argument     : int sub       : sub-command : unsigned
 *char *rw_buff   : 512byte area Return       : SD_OK : end of succeed Remark :
 *****************************************************************************/
