 ...
 *              : Jun.30.2014 ver.4.01.00 Added compliler swtich (#ifdef
 *__CC_ARM - #endif)
 ******************************************************************************/
#include "../../../inc/sdif.h"
...
int _sd_set_clock(SDHNDL* hndl, int clock, int enable) {
  ...
  if (enable == SD_CLOCK_ENABLE) {
    ...
    for (i = 0; i < SCLKDIVEN_LOOP_COUNT; i++) {
      ...
      if ((sd_inp(hndl, SD_INFO2) & SD_INFO2_MASK_CBSY) == 0) { break; }
#else
      if (sd_inp(hndl, SD_INFO2) & SD_INFO2_MASK_SCLKDIVEN) { break; }
#endif
    }
    if (i == SCLKDIVEN_LOOP_COUNT) { hndl->error = SD_ERR_CBSY_ERROR; }
...
int _sd_set_port(SDHNDL* hndl, int port) {
  ...
  if (hndl->media_type == SD_MEDIA_IO) { /* IO */
    ...
    if (_sdio_direct(hndl, &io_buff, 0, 0x07, 1, SD_IO_VERIFY_WRITE) != SD_OK) { return SD_ERR; }
    ...
    if ((hndl->resp_status & RES_STATE) == STATE_TRAN) { /* transfer state */
      ...
      if (_sd_send_acmd(hndl, ACMD6, 0, arg) != SD_OK) { return SD_ERR; }
      if (_sd_get_resp(hndl, SD_RESP_R1) != SD_OK) { return SD_ERR; }
    }
  }
...
int sd_iswp(int sd_port) {
  ...
  if ((sd_port != 0) && (sd_port != 1)) { return SD_ERR; }

  hndl = _sd_get_hndls(sd_port);
  if (hndl == 0) { return SD_ERR; /* not initilized */ }
...
int _sd_set_err(SDHNDL* hndl, int error) {
  if (hndl->error == SD_OK) { hndl->error = error; }
...
void sd_stop(int sd_port) {
  ...
  if ((sd_port != 0) && (sd_port != 1)) { return; }

  hndl = _sd_get_hndls(sd_port);
  if (hndl == 0) { return; /* not initilized */ }
 ...
 * Include      :
 * Declaration  : int sd_get_type(int sd_port, unsigned char *type,unsigned char
 **speed,unsigned char *capa); Functions    : get mounting card type, current
 *and supported speed mode : and capacity type : (if SD memory card) : following
 *card types are defined : SD_MEDIA_UNKNOWN : unknown media : SD_MEDIA_MMC : MMC
 *card : SD_MEDIA_SD : SD Memory card : SD_MEDIA_IO : SD IO card (IO spec
 *ver1.10) : SD_MEDIA_COMBO : SD COMBO card (IO spec ver1.10) Argument     :
 *unsigned char *type : mounting card type : unsigned char *speed : speed mode
...
int sd_get_type(int sd_port, unsigned char* type, unsigned char* speed, unsigned char* capa) {
  ...
  if (type) { *type = hndl->media_type; }
  if (hndl->partition_id > 0) { *type |= SD_MEDIA_EMBEDDED; }
  if (speed) { *speed = hndl->speed_mode; }
  if (capa) { *capa = hndl->csd_structure; }
  return SD_OK;
}
 ...
 * Include      :
 * Declaration  : int sd_get_size(int sd_port, unsigned long *user,unsigned long
 **protect); Functions    : get total sectors of user area calculated from CSD
...
int sd_get_size(int sd_port, unsigned long* user, unsigned long* protect) {
  ...
  if (user) { *user = hndl->card_sector_size; }
  if (protect) { *protect = hndl->prot_sector_size; }
  return SD_OK;
}
...
int _sd_get_size(SDHNDL* hndl, unsigned int area) {
  ...
  if (area & SD_PROT_AREA) {
    ...
    if ((hndl->sup_ver == SD_MODE_VER2X) && (hndl->csd_structure == 0x01)) {
      hndl->prot_sector_size =
          (((unsigned long)hndl->sdstatus[2] << 16u) | ((unsigned long)hndl->sdstatus[3])) / 512;
    } else {
      hndl->prot_sector_size = hndl->sdstatus[3] * ((unsigned long)1 << (c_mult + 2)) *
  ...
  if (area & SD_USER_AREA) {
    if ((hndl->sup_ver == SD_MODE_VER2X) && (hndl->csd_structure == 0x01)) {
      ...
      /* ---- C_SIZE ---- */
      c_size = ((hndl->csd[3] & 0x0003u) << 10) | ((hndl->csd[4] & 0xffc0u) >> 6);

      /* calculate the number of all sectors */
      hndl->card_sector_size = ((unsigned long)(c_size + 1) * ((unsigned long)1 << (c_mult + 2)) *
                                ((unsigned long)1 << read_bl_len)) /
                               512;
    }
  }
 ...
 * Include      :
 * Declaration  : int sd_get_reg(int sd_port, unsigned char *ocr,unsigned char
 **cid,unsigned char *csd, unsigned char *dsr,unsigned char *scr); Functions :
 *get card register value Argument     : unsigned char *ocr : OCR register
 *address : unsigned char *cid : CID register address : unsigned char *csd : CSD
 *register address : unsigned char *dsr : DSR register address : unsigned char
 **scr : SCR register address Return       : SD_OK : end of succeed : SD_ERR:
 *end of error Remark       : if pointer has NULL ,the register value isn't
 *returned
 *****************************************************************************/
int sd_get_reg(int            sd_port,
               unsigned char* ocr,
               unsigned char* cid,
               unsigned char* csd,
               unsigned char* dsr,
 ...
 * Include      :
 * Declaration  : int sd_get_speed(int sd_port, unsigned char *clss,unsigned
 *char *move); Functions    : get card speed class and performance move Argument
 *: unsigned char *class : speed class : unsigned char *move : performance move
 * Return       : SD_OK : end of succeed
 *              : SD_ERR: end of error
 * Remark       : if pointer has NULL ,the register value isn't returned
 *****************************************************************************/
int sd_get_speed(int sd_port, unsigned char* clss, unsigned char* move) {
  ...
  if (clss) { *clss = hndl->speed_class; }

  if (move) { *move = hndl->perform_move; }
 ...
 * Include      :
 * Declaration  : int sd_get_ver(int sd_port, unsigned short *sdhi_ver,char
 **sddrv_ver); Functions    : get SDHI version from VERSION register : get SD
 *Driver version from constant DRIVER NAME Argument     : unsigned short
 **sdhi_ver : SDHI version : char *sddrv_ver : SD Driver version Return       :
 *SD_OK : end of succeed : SD_ERR: end of error Remark       : if pointer has
 *NULL ,the value isn't returned
 *****************************************************************************/
int sd_get_ver(int sd_port, unsigned short* sdhi_ver, char* sddrv_ver) {
  ...
  if (sdhi_ver) { *sdhi_ver = sd_inp(hndl, VERSION); }

  if (sddrv_ver) {
    for (i = 0; i < 32; ++i) { *sddrv_ver++ = *name++; }
  }
...
int sd_set_cdtime(int sd_port, unsigned short cdtime) {
  ...
  if (cdtime >= 0x000f) { return SD_ERR; /* undefined value */ }
 ...
 * Include      :
 * Declaration  : int sd_set_responsetime(int sd_port, unsigned short
 *responsetime); Functions    : set response timeout equal to
 *IMCLK*2^(13+cdtime) Argument     : unsigned short responsetime : response
 *timeout interval Return       : SD_OK : end of succeed : SD_ERR: end of error
 * Remark       :
 *****************************************************************************/
int sd_set_responsetime(int sd_port, unsigned short responsetime) {
  ...
  if (responsetime >= 0x000f) { return SD_ERR; /* undefined value */ }
...
int sd_set_buffer(int sd_port, void* buff, unsigned long size) {
  ...
  if (((unsigned long)buff & 0x00000003u) != 0) { return SD_ERR; }
...
int _sd_standby(SDHNDL* hndl) {
  ...
  if (_sd_set_clock(hndl, (int)hndl->csd_tran_speed, SD_CLOCK_ENABLE) != SD_OK) { goto ErrExit; }
  ...
  if ((ret != SD_OK) && (hndl->error != SD_ERR_RES_TOE)) { goto ErrExit; }
...
int _sd_inactive(SDHNDL* hndl) {
  ...
  /* ==== state transfer (transfer to stand-by) ==== */
  if (_sd_card_send_cmd_arg(hndl, CMD15, SD_RESP_NON, hndl->rca[0], 0x0000) != SD_OK) {
    goto ErrExit;
  }
...
int sd_reget_reg(int sd_port, unsigned char* reg, int is_csd) {
  ...
  if (_sd_standby(hndl) != SD_OK) { goto ErrExit; }
  ...
  if (_sd_card_send_cmd_arg(hndl, cmd, SD_RESP_R2_CID, hndl->rca[0], 0x0000) != SD_OK) {
    goto ErrExit;
  }
  ...
  if (_sd_active(hndl) != SD_OK) { goto ErrExit; }
...
int sd_set_softwp(int sd_port, int is_set, unsigned long data) {
  ...
  ret = _sd_card_send_cmd_arg(hndl,
                              cmd,
                              SD_RESP_R1b,
  ...
  if (ret != SD_OK) {
    ...
    if (hndl->error == SD_ERR_CARD_TOE) {
      if (_sd_wait_rbusy(hndl, 100000) != SD_OK) { goto ErrExit; }
    } else {
      goto ErrExit;
    }
  }
  ...
  if (_sd_read_byte(hndl, CMD30, (unsigned short)(data >> 16), (unsigned short)data, is_wp, 4) !=
      SD_OK) {
    goto ErrExit;
  }
 ...
 * Summary      : lock/unlock
 * Include      :
 * Declaration  : int sd_lock_unlock(int sd_port, unsigned char code,unsigned
 *char *pwd,unsigned char len); Functions    : lock/unlock operation : passward
 *length is up to 16 bytes : case of cahnge passward, total length is 32
 *bytes,that is : old and new passward maximum length Argument     : unsigned
 *char code : operation code : unsigned char *pwd : passward : unsigned char len
 *: passward length Return       : SD_OK : end of succeed : SD_ERR: end of error
 * Remark       :
 *****************************************************************************/
int sd_lock_unlock(int sd_port, unsigned char code, unsigned char* pwd, unsigned char len) {
  ...
  if (_sd_write_byte(hndl, CMD42, 0x0000, 0x0000, (unsigned char*)data, cmd_len) != SD_OK) {
    goto ErrExit;
  }
...
ErrExit:
  ...
  for (loop = 0; loop < 3; loop++) {
    ...
    if (_sd_card_send_cmd_arg(hndl, CMD16, SD_RESP_R1, 0x0000, 0x0200) == SD_OK) { break; }
  }
...
int sd_set_tmpwp(int sd_port, int is_set) {
  ...
  if (_sd_write_byte(hndl, CMD27, 0x0000, 0x0000, w_csd, sizeof(w_csd)) != SD_OK) { goto ErrExit; }
...
unsigned char _sd_calc_crc(unsigned char* data, int len) {
  ...
  for (i = len; i > 0; i--, data++) { /* byte loop */
    for (j = 8; j > 0; j--) {         /* bit loop */
      ...
      for (k = 0; k < sizeof(p_crc); k++) { p_crc[k] = t_crc[k]; }
    }
  }

  /* convert bit to byte form */
  for (i = 0; i < sizeof(p_crc); i++) { crc7 |= (unsigned char)((p_crc[i] << (6 - i))); }
...
int _sd_wait_rbusy(SDHNDL* hndl, int time) {
  ...
  for (i = 0; i < time; ++i) {
    ...
    if (_sd_check_media(hndl) != SD_OK) { break; }
