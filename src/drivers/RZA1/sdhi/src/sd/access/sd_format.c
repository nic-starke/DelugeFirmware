...
/* CHS parameter table */
static const CHS_RECOM chs_tbl[] = {{SIZE_CARD_2MB, NUM_HEAD_2, SEC_PER_TRACK_16},
...
/* SC,BU table */
static const SCBU_RECOM scbu_tbl[] = {{SIZE_CARD_8MB, SEC_PER_CLUSTER_16, SIZE_OF_BU_16},
...
/* ==== prototype ==== */
static int _sd_format(SDHNDL*      hndl,
                      SD_FMT_WORK* sdfmt,
                      int          format_mode,
...
static int _sd_format_write(SDHNDL*       hndl,
                            SD_FMT_WORK*  sdfmt,
                            unsigned char fill,
                            unsigned long secno,
                            long          seccnt,
                            int (*callback)(unsigned long, unsigned long));
static int _sd_format_erase(SDHNDL*       hndl,
                            SD_FMT_WORK*  sdfmt,
                            unsigned long secno,
                            long          seccnt,
                            int (*callback)(unsigned long, unsigned long));
 ...
 * Include      :
 * Declaration  : int sd_format(int sd_port, int mode,int (*callback)(unsigned
 *long,unsigned long)); Functions    : format SD memory card based on SD apec
 *part2
 *              :
...
int sd_format(int sd_port, int mode, int (*callback)(unsigned long, unsigned long)) {
  ...
  if (mode == SD_FORMAT_QUICK) { callback = 0; /* not used callback function at quick format */ }
  /* ---- is card existed? ---- */
  ...
  /* ---- check card type ---- */
  if (hndl->media_type != SD_MEDIA_SD && hndl->media_type != SD_MEDIA_MMC &&
      hndl->media_type != SD_MEDIA_IO && hndl->media_type != SD_MEDIA_COMBO) {
 ...
 * Include      :
 * Declaration  : int sd_format2(int sd_port, int mode,unsigned long
 *volserial,int (*callback)(unsigned long,unsigned long)); Functions    : format
 *SD memory card based on SD apec part2
 *              :
 * Argument     : int mode : format mode SD_FORMAT_QUICK or SD_FORMAT_FULL
 ...
 *              :   int format_mode,int (*callback)(unsigned long,unsigned
 *long)) Functions    : format SD memory card based on SD apec part2
 *              :
...
static int _sd_format(SDHNDL*      hndl,
  ...
  if (sdfmt->fmt_exfat == 1) {
    ...
    if (callback) { (*callback)(0, sdfmt->format_size); }
    ...
    for (i = 0; i < 2; i++) {
      ...
      for (k = 0; k < 512; k++) {
        if ((k == 106) || (k == 107) || (k == 112)) { continue; }
        /* calculate checksum */
        checksum =
            ((checksum & 1) ? 0x80000000 : 0) + (checksum >> 1) + (unsigned long)sdfmt->pbuff[k];
      }
  ...
  if (sdfmt->fmt_fatbit == 32) { /* FAT32 */
    ...
    ret = (*sdfmt->write)(hndl,
                          sdfmt,
                          0,
                          offset,
                          (long)sdfmt->fmt_sc,
                          callback); /* already buffer data are cleared zero due
                                        to previous FAT writing */
    if (ret != SD_OK) { return ret; }
    erase_offset += 64;
  } else { /* FAT16 */
    /* ---- PBR ---- */
...
static int _sd_get_fmt_parm(SDHNDL* hndl, SD_FMT_WORK* sdfmt) {
  ...
  /* size�́ANumbr of Heads, Sectors per Track, Sectors per Cluster, Boundary
   * Unit�̌v�Z�Ɏg�p����B*/
  /* size��0x100000000sectors(2TB)�̏ꍇ�A0xFFFFFFFFsectors�Ƃ��Čv�Z���Ă������l�ɂȂ邽�߁A
   */
  /* SIZE_CARD_2TB �� 0xFFFFFFFF(unsigned long�̍ő�l) �ƒ�`���Ă��� */
  if (hndl->card_sector_size > 0 && size < hndl->card_sector_size) { size = SIZE_CARD_2TB; }
...
static int _sd_calc_parameter(SD_FMT_WORK* sdfmt) {
  ...
  /* since max cluster number of FAT12 is 0xFF5, the number of max clusters is
   * 0xFF4 */
  ...
  if (fatbit == 32) { /* FAT32 */
    ...
    while (1) {                /* iterative calculation for number of sectors in MBR */
      ...
      if (nom != bu) { nom += bu; /* if different each other, add */ }
  ...
  *ptr++ = 0x00;
  ...
  if (sdfmt->fmt_nom > 16450560) {
    ...
    *ptr++ = 0xff;
    ...
    /* start head */
    *ptr++ = (unsigned char)(sdfmt->fmt_nom % (sdfmt->fmt_spt * sdfmt->fmt_hn) / sdfmt->fmt_spt);
 ...
 * Include      :
 * Declaration  : static int _sd_init_br_chksum(SD_FMT_WORK *sdfmt, unsigned
 *long checksum) Functions    : set Checksum to image buffer
 *              :
 * Argument     : SD_FMT_WORK *sdfmt : format parameter
 * Return       : SD_OK : end of succeed
 *              : SD_ERR: end of error
 ...
 * Include      :
 * Declaration  : static int _sd_init_upcase_table(SD_FMT_WORK *sdfmt, unsigned
 *long secoffset) Functions    : Argument     : SD_FMT_WORK *sdfmt : format
 *parameter : unsigned long secoffset : sector offset Return       : SD_OK : end
...
static int _sd_init_upcase_table(SD_FMT_WORK* sdfmt, unsigned long secoffset) {
  ...
  /* copy up-case table */
  for (i = secoffset * 512 / sizeof(unsigned short);
       i < (secoffset + 1) * 512 / sizeof(unsigned short);
       i++) {
    if (i >= sizeof(sd_upcase_tbl) / sizeof(unsigned short)) { break; }
...
static int _sd_init_root_directory(SD_FMT_WORK* sdfmt) {
  ...
  *ptr++ = 0;
  /* data length */
  clustercnt =
      (sdfmt->area_size - sdfmt->fmt_nom - sdfmt->fmt_ssa) / sdfmt->fmt_sc; /* cluster count */
  size = clustercnt / 8; /* size = ceil(cluster count/8) */
  if (clustercnt % 8) { size++; }
  *ptr++ = (unsigned char)size;
  *ptr++ = (unsigned char)(size >> 8);
  *ptr++ = (unsigned char)(size >> 16);
  *ptr++ = (unsigned char)(size >> 24);
...
static int _sd_format_write(SDHNDL*       hndl,
  ...
  while (seccnt) { /* loop for transfer sector size */
    ...
    /* call callback function */
    if (callback) { (*callback)(secno, sdfmt->format_size); }
...
static int _sd_format_erase(SDHNDL*       hndl,
  ...
  while (seccnt) {
    ...
    ret = _sd_card_send_cmd_arg(hndl,
                                startcom,
                                SD_RESP_R1,
                                (unsigned short)(startaddres >> 16),
                                (unsigned short)startaddres);
    ...
    ret = _sd_card_send_cmd_arg(hndl,
                                endcom,
                                SD_RESP_R1,
                                (unsigned short)(endaddres >> 16),
                                (unsigned short)endaddres);
    if (ret != SD_OK) { goto ErrExit; }
