 ...
 * Include      :
 * Declaration  : int sd_init(int sd_port, unsigned long base, void *workarea,
 *int cd_port); Functions    : initialize SD Driver work memory started from
 *SDHI register : base : address specified by argument (base) : initialize port
 *specified by argument (cd_port) : work memory is allocated quadlet boundary
...
int sd_init(int sd_port, unsigned long base, void* workarea, int cd_port) {
  ...
  for (i = sizeof(SDHNDL); i > 0; i--) { *ptr++ = 0; }
  ...
  sd_outp(hndl,
          SD_OPTION,
          0x00BD); // Changed by Rohan to NCycle=SDCLK*2^23. NCycle=SDCLK*2^24 would
                   // be 0x00be, but this was too long and meant it sometimes
                   // couldn't access the card when first trying, on boot
#else
...
int sd_finalize(int sd_port) {
  ...
  if (SDHandle[sd_port] == 0) { return SD_ERR; /* not initilized */ }
...
int _sd_init_hndl(SDHNDL* hndl, unsigned long mode, unsigned long voltage) {
  ...
  for (i = 0; i < 4 / sizeof(unsigned short); ++i) { hndl->ocr[i] = 0; }
  for (i = 0; i < 16 / sizeof(unsigned short); ++i) { hndl->cid[i] = 0; }
  for (i = 0; i < 16 / sizeof(unsigned short); ++i) { hndl->csd[i] = 0; }
  for (i = 0; i < 2 / sizeof(unsigned short); ++i) { hndl->dsr[i] = 0; }
  for (i = 0; i < 4 / sizeof(unsigned short); ++i) { hndl->rca[i] = 0; }
  for (i = 0; i < 8 / sizeof(unsigned short); ++i) { hndl->scr[i] = 0; }
  for (i = 0; i < 14 / sizeof(unsigned short); ++i) { hndl->sdstatus[i] = 0; }
  for (i = 0; i < 18 / sizeof(unsigned short); ++i) { hndl->status_data[i] = 0; }
  for (i = 0; i < 4 / sizeof(unsigned short); ++i) { hndl->if_cond[i] = 0; }
  ...
  if (hndl->sup_card & SD_MODE_IO) {
    ...
    for (i = 0; i < 4 / sizeof(unsigned short); ++i) { hndl->io_ocr[i] = 0; }
  ...
  if (hndl->sup_ver == SD_MODE_VER2X) {
    ...
    if (hndl->voltage & 0x00FF8000) { hndl->if_cond[1] |= 0x0100; /* high volatege : 2.7V-3.6V */ }
    if (hndl->voltage & 0x00000F00) { hndl->if_cond[1] |= 0x0200; /* low volatege : 1.65V-1.95V */ }
  }
