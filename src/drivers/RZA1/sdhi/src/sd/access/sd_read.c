...
static int _sd_single_read(SDHNDL* hndl, unsigned char* buff, unsigned long psn, int mode);

int doActualReadRohan(int            sd_port,
                      SDHNDL*        hndl,
                      unsigned char* buff,
                      long           cnt,
                      int            mode,
  ...
  if (mode == SD_MODE_SW) { /* ==== PIO ==== */
    ...
    // If seems we have to invalidate RAM before as well as after the DMA
    // transfer. Otherwise, there can be crackles in loaded sample data if the
    // same small number of RAM clusters are being reused lots. Also seen a
    // problem with some incorrect values (so a "click") coming in on a
    // single-cycle waveform being loaded - and this was not really a
    // reused-lots scenario - although this was using the main sector-read
    // buffer, so maybe that counts as reuse. Guessing problem occurs because if
    // not invalidated, there might be a cache for this memory which hasn't been
    // written/flushed back out yet, and that happens during the DMA transfer,
    // overwriting the audio data in actual RAM.
    // https://support.xilinx.com/s/article/64839?language=en_US - seems to
    // concur with this, and actually suggests that it is normal and necessary
    // to invalidate both before and after transfer.
    ...
    int result =
        sddev_init_dma(sd_port, (unsigned long)buff, reg_base_here, cnt * 512, SD_TRANS_READ);
 ...
 * Include      :
 * Declaration  : int sd_read_sect(int sd_port, unsigned char *buff,unsigned
 *long psn,long cnt); Functions    : read sector data from physical sector
 *number (=psn) by the : number of sectors (=cnt) : if SD Driver mode is
 *SD_MODE_SW, data transfer by : sddev_read_data function : if SD Driver mode is
 *SD_MODE_DMA, data transfer by DMAC
 *              :
...
int sd_read_sect(int sd_port, unsigned char* buff, unsigned long psn, long cnt) {
  ...
  for (i = cnt; i > 0; i -= TRANS_SECTORS, psn += TRANS_SECTORS, buff += TRANS_SECTORS * 512) {
    ...
    if (mode == SD_MODE_DMA) { /* ==== DMA ==== */
      ...
      sd_outp(hndl,
              CC_EXT_MODE,
              (unsigned short)(sd_inp(hndl, CC_EXT_MODE) | CC_EXT_MODE_DMASDRW)); /* enable DMA */
    }

    /* issue CMD18 (READ_MULTIPLE_BLOCK) */
    if (mmc_lastsect) { /* MMC last sector access */
      if (_sd_send_mcmd(hndl, CMD18 | 0x7c00u, SET_ACC_ADDR) != SD_OK) { goto ErrExit_DMA; }
    } else {
      if (_sd_send_mcmd(hndl, CMD18, SET_ACC_ADDR) != SD_OK) { goto ErrExit_DMA; }
    }

    ret = doActualReadRohan(sd_port, hndl, buff, cnt, mode, dma_64);

    if (ret != SD_OK) { goto ErrExit; }
    /* ---- wait All end interrupt ---- */
    logAudioAction("0a");

    // TODO: what if we didn't wait for this interrupt? It can't be that
    // necessary, since we've already read the data... how much time would we
    // save? Or is this just customary as the interrupt's already happened?
    ...
    if (mmc_lastsect) {
      if (_sd_card_send_cmd_arg(hndl, 12, SD_RESP_R1b, 0, 0) != SD_OK) {
        ...
        if (hndl->resp_status & 0xffffe008ul) {
          if (psn + cnt != hndl->card_sector_size) { goto ErrExit; /* but for last block */ }
          if (hndl->resp_status & 0x7fffe008ul) { goto ErrExit; /* not OUT_OF_RANGE error */ }
...
ErrExit_DMA:
ErrExit:
  if (mode == SD_MODE_DMA) { sddev_disable_dma(sd_port); /* disable DMA */ }
  sd_outp(hndl,
          CC_EXT_MODE,
          (unsigned short)(sd_inp(hndl, CC_EXT_MODE) & ~CC_EXT_MODE_DMASDRW)); /* disable DMA */
...
static int _sd_single_read(SDHNDL* hndl, unsigned char* buff, unsigned long psn, int mode) {
  ...
  // Kinda need this because, reading just one sector, we're not gonna be
  // sitting waiting for interrupts for very long, so might not do any audio
  // routine calls later
  // routineForSD();
  ...
  if (_sd_send_mcmd(hndl, CMD17, SET_ACC_ADDR) != SD_OK) { goto ErrExit_DMA; }
  ...
  if (_sd_card_send_cmd_arg(hndl, CMD13, SD_RESP_R1, hndl->rca[0], 0x0000) != SD_OK) {
    ...
    /* ignore errors during last block access */
    if (hndl->resp_status & 0xffffe008ul) {
      if (psn + 1 != hndl->card_sector_size) { goto ErrExit; /* but for last block */ }
      if (hndl->resp_status & 0x7fffe008ul) { goto ErrExit; /* not OUT_OF_RANGE error */ }
      /* clear OUT_OF_RANGE error */
      hndl->resp_status &= 0x1f00u;
      hndl->error = SD_OK;
...
ErrExit_DMA:
ErrExit:
  if (mode == SD_MODE_DMA) { sddev_disable_dma(hndl->sd_port); /* disable DMA */ }
