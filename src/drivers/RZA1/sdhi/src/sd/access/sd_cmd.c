...
int _sd_send_acmd(SDHNDL* hndl, unsigned short cmd, unsigned short h_arg, unsigned short l_arg) {
  ...
  logAudioAction("1a");
  if (_sd_send_cmd(hndl, CMD55) != SD_OK) { return SD_ERR; }
...
int _sd_send_mcmd(SDHNDL* hndl, unsigned short cmd, unsigned long startaddr) {
  ...
  if (hndl->int_info1 & SD_INFO1_MASK_RESP) {
    if (!hndl->error) { _sd_get_resp(hndl, SD_RESP_R1); /* check R1 resp */ }
  } else {
    _sd_set_err(hndl, SD_ERR_NO_RESP_ERROR); /* no response */
  }
...
int _sd_send_iocmd(SDHNDL* hndl, unsigned short cmd, unsigned long arg) {
  ...
  if (hndl->int_info1 & SD_INFO1_MASK_RESP) {
    if (!hndl->error) { _sd_get_resp(hndl, SD_RESP_R5); /* check R5 resp */ }
  } else {
...
int _sd_card_send_ocr(SDHNDL* hndl, int type) {
  ...
  for (i = 0; i < 200; i++) {
    switch (type) {
      ...
      case SD_MEDIA_IO:
        /* ---- issue CMD5 ---- */
        _sd_set_arg(hndl, (unsigned short)(hndl->voltage >> 16), (unsigned short)hndl->voltage);
        logAudioAction("1e");
        ret = _sd_send_cmd(hndl, CMD5);
        break;
    ...
    if (ret == SD_OK) {
      if (type == SD_MEDIA_IO) {        /* IO */
        ...
        if (hndl->io_ocr[0] & 0x8000) { /* busy cleared */
          ...
          ret = SD_ERR; /* busy */
          sddev_int_wait(hndl->sd_port,
                         5); /* add wait function because retry interval is too short */
        }
      } else {                          /* memory */
    ...
    /* if more than 3 times response timeout occured, retry stop quick
     * distinction to MMC */
...
int _sd_check_csd(SDHNDL* hndl) {
  ...
  /* ---- CCC  ---- */
  hndl->csd_ccc =
      (unsigned short)(((hndl->csd[2] & 0x00ffu) << 4u) | ((hndl->csd[3] & 0xf000u) >> 12u));
  ...
  if (hndl->media_type & SD_MEDIA_SD) {
    erase_sector_size = ((hndl->csd[5] & 0x003fu) << 1u) | ((hndl->csd[6] & 0x8000) >> 15);
    erase_group_size  = (hndl->csd[6] & 0x7f00u) >> 8u;
  } else {
    erase_sector_size = (hndl->csd[5] & 0x007cu) >> 2u;
    erase_group_size  = ((hndl->csd[5] & 0x0003u) << 3u) | ((hndl->csd[6] & 0xe000u) >> 13u);
  }
