 ...
 * Include      :
 * Declaration  : int sd_cd_int(int sd_port, int enable,int (*callback)(int,
 *int)); Functions    : set card detect interrupt : if select SD_CD_INT_ENABLE,
 *detect interrupt is enbled and : it is possible register callback function :
...
int sd_cd_int(int sd_port, int enable, int (*callback)(int, int)) {
  ...
  if ((hndl->int_info1_mask & (SD_INFO1_MASK_DET_DAT3 | SD_INFO1_MASK_DET_CD)) == 0) {
