 ...
 * Include      :
 * Declaration  : int sd_write_sect(int sd_port, unsigned char *buff,unsigned
 *long psn,long cnt,int writemode); Functions    : write sector data from
 *physical sector number (=psn) by the : number of sectors (=cnt) : if SD Driver
 *mode is SD_MODE_SW, data transfer by : sddev_read_data function : if SD Driver
 *mode is SD_MODE_DMA, data transfer by DMAC Argument     : unsigned char *buff
 *: write data buffer : unsigned long psn : write physical sector number : long
 *cnt : number of write sectors : int writemode : memory card write mode :
 *SD_WRITE_WITH_PREERASE : pre-erease write :   SD_WRITE_OVERWRITE : overwrite
 ...
 * Remark       :
 *****************************************************************************/
int _sd_write_sect(SDHNDL* hndl, unsigned char* buff, unsigned long psn, long cnt, int writemode) {
  long           i, j;
  ...
  for (i = cnt; i > 0; i -= TRANS_SECTORS, psn += TRANS_SECTORS, buff += TRANS_SECTORS * 512) {
    ...
    if ((hndl->media_type & SD_MEDIA_SD) && (writemode == SD_WRITE_WITH_PREERASE)) {
      if (_sd_send_acmd(hndl, ACMD23, 0, (unsigned short)cnt) != SD_OK) { goto ErrExit; }
    ...
    if (mode == SD_MODE_SW) { /* ==== PIO ==== */
      ...
      info1_back = (unsigned short)(hndl->int_info1_mask & SD_INFO1_MASK_DET_CD);
...
/* ---- initialize DMAC ---- */
      ...
      if (dma_64 != SD_MODE_DMA_64)
        reg_base_now += SD_BUF0; /* SD_CMD Address for 64byte transfer */

      if (sddev_init_dma(hndl->sd_port,
                         (unsigned long)buff,
                         reg_base_now,
                         cnt * 512,
    ...
    if (trans_ret != SD_OK) { goto ErrExit; }
    ...
    if (hndl->media_type & SD_MEDIA_SD) {
      if (_sd_get_resp(hndl, SD_RESP_R1) != SD_OK) {
        ...
        if (_sd_read_byte(hndl, ACMD22, 0, 0, wb, 4) != SD_OK) { goto ErrExit; }
        /* type cast (unsigned long) remove compiler dependence */
        writeblock = ((unsigned long)wb[0] << 24) | ((unsigned long)wb[1] << 16) |
                     ((unsigned long)wb[2] << 8) | (unsigned long)wb[3];
...
/*
    ...
    When the SDHI does not execute the command sequence and the SD card is not
   in the transfer state, CMD12 must be issued.
    ...
    3.DMA transfer processing is failed.
      The sequence is suspended by the implementation failure of the
   user-defined function or INFO2 error. 4.Interrupt for access end or response
   end is not generated. Implementation failure of the user-defined function
   caused timeout during the command sequence execution. 5.Error bit of INFO2 is
   set. 6.ACMD22 is in error. 7.The sector count for writing obtained by ACMD22
   and the transmitted sector count are different. 8.Error bit of CMD13 response
   is set. 9.CMD13 issuing process is failed. 10�DThe SD card is not in the
   transfer state when CMD13 is issued.
*/
ErrExit:
  error = hndl->error;
  /* ---- clear error bits ---- */
...
static int _sd_single_write(SDHNDL* hndl, unsigned char* buff, unsigned long psn, int mode) {
  ...
  if (_sd_send_mcmd(hndl, CMD24, SET_ACC_ADDR) != SD_OK) { goto ErrExit; }
  /* ---- disable RespEnd and ILA ---- */
  _sd_clear_int_mask(hndl, SD_INFO1_MASK_RESP, SD_INFO2_MASK_ILA);
...
/* ---- initialize DMAC ---- */
    ...
    if (dma_64 == SD_MODE_DMA_64) {
      ...
      if (sddev_init_dma(hndl->sd_port, (unsigned long)buff, hndl->reg_base, 512, SD_TRANS_WRITE) !=
          SD_OK) {
        _sd_set_err(hndl, SD_ERR_CPU_IF);
        goto ErrExit;
      }
