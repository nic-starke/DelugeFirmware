 ...
 * Include      :
 * Declaration  : int sdio_read(int sd_port, unsigned char *buff,unsigned long
 *func,unsigned long adr,long cnt,unsigned long op_code); Functions    : read io
 *register from specified address (=adr) by the : number of bytes or blocks
 *(=cnt) : if SD Driver mode is SD_MODE_SW, data transfer by : sddev_read_data
 *function : if SD Driver mode is SD_MODE_DMA, data transfer by DMAC Argument :
 *unsigned char *buff : read data buffer : unsigned long func : access function
 *number : unsigned long adr : read register address : long cnt : number of read
 *registers (byte) : unsigned long op_code : operation code :   SD_IO_FIXED_ADDR
 *: R/W fixed address :   SD_IO_INCREMENT_ADDR : R/W increment address Return :
 *SD_OK : end of succeed : SD_ERR: end of error Remark       :
 *****************************************************************************/
int sdio_read(int            sd_port,
              unsigned char* buff,
              unsigned long  func,
              unsigned long  adr,
              long           cnt,
  ...
  if ((func > 7) || (adr > 0x1ffff)) { return SD_ERR; }
  ...
  if ((op_code & SD_IO_FORCE_BYTE) == 0) {
    if (hndl->io_len[func] == 0) { /* not yet */
      if (sdio_get_blocklen(sd_port, &blocklen, func) != SD_OK) { return hndl->error; }
    }
...
int _sdio_read(SDHNDL*        hndl,
  ...
  if (sec) { /* more than io block size (=blocklen bytes) */
    ...
    for (i = sec; i > 0; i -= TRANS_BLOCKS) {
      ...
      /* set argument */
      arg = ((func << 28) | 0x08000000u | (op_code << 26) | (adr << 9) | (unsigned long)sec);
...
/* ---- initialize DMAC ---- */
        ...
        if (dma_64 == SD_MODE_DMA_64) {
          if (sddev_init_dma(hndl->sd_port,
                             (unsigned long)buff,
                             hndl->reg_base, /* SD_CMD Address for 64byte transfer */
                             (sec * blocklen),
                             SD_TRANS_READ) != SD_OK) {
        ...
        sd_outp(hndl,
                CC_EXT_MODE,
                (unsigned short)(sd_inp(hndl, CC_EXT_MODE) & ~CC_EXT_MODE_DMASDRW));
      ...
      if (op_code == SD_IO_INCREMENT_ADDR) { adr += (sec * blocklen); }
      buff += (sec * blocklen);
  ...
  if (rem) { /* remaining bytes are less than io block size or not support multi
                block transfer */
    ...
    /* set argument */
    arg = ((func << 28) | (op_code << 26) | (adr << 9) | (unsigned long)rem);
 ...
 * Remark       :
 *****************************************************************************/
int _sdio_read_byte(SDHNDL*        hndl,
                    unsigned char* buff,
                    unsigned long  func,
                    unsigned long  adr,
                    long           cnt,
  ...
  if (sec) { /* more than io block size (=512 bytes) */
    ...
    for (i = sec; i > 0; i--) {
      ...
      /* set argument */
      arg = ((func << 28) | 0x00000000u | (op_code << 26) | (adr << 9) | (unsigned long)0);
...
/* ---- initialize DMAC ---- */
      ...
      if (op_code == SD_IO_INCREMENT_ADDR) { adr += 512; }
      buff += 512;

      /* ---- is io_abort compulsory? ---- */
      if (hndl->io_abort[func]) { i = 0; /* set zero to break loop */ }
    }
  }
