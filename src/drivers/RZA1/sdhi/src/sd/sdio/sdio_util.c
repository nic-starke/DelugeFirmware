 ...
 * Include      :
 * Declaration  : int sdio_get_cia(int sd_port, unsigned char *reg, unsigned
 *char *cis, unsigned long func_num, long cnt); Functions    : get cia value
 * Argument     : unsigned char *reg : CCCR or FBR register address
 *              : unsigned char *cis : CIS register address
 *              : unsigned long func_num : function number (0 to 7, 0 means
 *Common) : long cnt : size of CIS to read Return       : SD_OK : end of succeed
 *              : SD_ERR: end of error
 * Remark       : if pointer has NULL ,the register value isn't returned
 *****************************************************************************/
int sdio_get_cia(int            sd_port,
                 unsigned char* reg,
                 unsigned char* cis,
  ...
  if (func_num > 7) { return SD_ERR; }

  hndl = _sd_get_hndls(sd_port);
  if (hndl == 0) { return SD_ERR; /* not initilized */ }
  hndl->error = SD_OK;

  if (sdio_read(sd_port,
                hndl->io_reg[func_num],
                0,
                (unsigned long)(0x100 * func_num),
                SDIO_INTERNAL_REG_SIZE,
                (SD_IO_INCREMENT_ADDR | SD_IO_FORCE_BYTE)) != SD_OK) {
    return hndl->error;
  }

  if (reg) { /* CCCR or FBR */
    for (i = 0; i < SDIO_INTERNAL_REG_SIZE; ++i) { *reg++ = hndl->io_reg[func_num][i]; }
  }
  ...
  if (adr != 0) {
    if (cis) {
      if (sdio_read(sd_port, cis, 0, adr, cnt, (SD_IO_INCREMENT_ADDR | SD_IO_FORCE_BYTE)) !=
          SD_OK) {
        return hndl->error;
      }

      for (i = 0; i < cnt; ++i) { hndl->cis[func_num][i] = *cis++; }
    }
  }
...
int sdio_set_enable(int sd_port, unsigned char func_bit) {
  ...
  if (_sdio_direct(hndl, &func_bit, 0, 0x02, 1, SD_IO_VERIFY_WRITE) != SD_OK) { goto ErrExit; }
...
int sdio_get_ready(int sd_port, unsigned char* func_bit) {
  ...
  if (_sdio_direct(hndl, func_bit, 0, 0x03, 0, 0) != SD_OK) { goto ErrExit; }
 ...
 * Include      :
 * Declaration  : int sdio_set_int(int sd_port, unsigned char func_bit, int
 *enab); Functions    : enable or disable interrupt from io functions Argument
 *: unsigned char func_bit : specify enable function by bit map : unsigned char
 *enab : control enable or disable Return       : SD_OK : end of succeed :
 *SD_ERR: end of error Remark       :
 *****************************************************************************/
int sdio_set_int(int sd_port, unsigned char func_bit, int enab) {
  ...
  if (_sdio_direct(hndl, &func_bit, 0, 0x04, 1, SD_IO_VERIFY_WRITE) != SD_OK) { goto ErrExit; }
 ...
 * Include      :
 * Declaration  : int sdio_get_int(int sd_port, unsigned char *func_bit,int
 **enab); Functions    : inquire io functions's interrupt status Argument     :
 *unsigned char *func_bit : ready function by bit map Return       : SD_OK : end
 *of succeed : SD_ERR: end of error Remark       :
 *****************************************************************************/
int sdio_get_int(int sd_port, unsigned char* func_bit, int* enab) {
  ...
  if (_sdio_direct(hndl, func_bit, 0, 0x04, 0, 0) != SD_OK) { goto ErrExit; }
 ...
 * Include      :
 * Declaration  : int sdio_set_blocklen(int sd_port, unsigned short len,
 *unsigned long func_num); Functions    : set io function's block length
 * Argument     : unsigned short len : block length
 *              : unsigned long func_num : specify function by number (from 0 to
 *7) Return       : SD_OK : end of succeed : SD_ERR: end of error Remark       :
 *****************************************************************************/
int sdio_set_blocklen(int sd_port, unsigned short len, unsigned long func_num) {
  ...
  if (_sdio_set_blocklen(hndl, len, func_num) != SD_OK) { goto ErrExit; }
 ...
 * Include      :
 * Declaration  : int sdio_get_blocklen(int sd_port, unsigned short
 **len,unsigned long func_num); Functions    : inquire io function's block
 *length Argument     : unsigned short *len : block length : unsigned long
 *func_num : specify function by number (from 0 to 7) Return       : SD_OK : end
 *of succeed : SD_ERR: end of error Input        : Output       : Call functions
 *: Remark       :
 *****************************************************************************/
int sdio_get_blocklen(int sd_port, unsigned short* len, unsigned long func_num) {
  ...
  if (hndl->trans_mode & SD_MODE_DMA) {
    if (hndl->trans_mode & SD_MODE_DMA_64) {
      if (len_t == 32) { hndl->io_len[func_num] = 0xffff; /* already but not supported */ }
    }
  }
#endif
...
int sdio_set_blkcnt(int sd_port, short blocks) {
  ...
  if (!(hndl->media_type & SD_MEDIA_IO)) { return SD_ERR_IO_CAPAB; }
