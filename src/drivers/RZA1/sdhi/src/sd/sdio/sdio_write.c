 ...
 * Include      :
 * Declaration  : int sdio_write(int sd_port, unsigned char *buff,unsigned long
 *func,unsigned long adr,long cnt,unsigned long op_code); Functions    : write
 *io register from (or at) the address (=adr) by the : number of bytes or blocks
 ...
 *unsigned char *buff : write data buffer : unsigned long func : access function
 *number : unsigned long adr : read register address : long cnt : number of
 *write registers (byte) : unsigned long op_code : operation code :
 *SD_IO_FIXED_ADDR : R/W fixed address :   SD_IO_INCREMENT_ADDR : R/W increment
 *address Return       : SD_OK : end of succeed : SD_ERR: end of error Remark :
 *****************************************************************************/
int sdio_write(int            sd_port,
               unsigned char* buff,
               unsigned long  func,
               unsigned long  adr,
               long           cnt,
...
int _sdio_write(SDHNDL*        hndl,
  ...
  if (sec) { /* more than io block size (=blocklen bytes) */
    ...
    for (i = sec; i > 0; i -= TRANS_BLOCKS) {
      ...
      /* set argument */
      arg = ((func << 28) | 0x88000000u | (op_code << 26) | (adr << 9) | (unsigned long)sec);
...
/* ---- initialize DMAC ---- */
  ...
  if (rem) { /* remaining bytes are less than io block size or not support multi
    ...
    /* set argument */
    arg = ((func << 28) | 0x80000000u | (op_code << 26) | (adr << 9) | (unsigned long)rem);
 ...
 * Remark       :
 *****************************************************************************/
int _sdio_write_byte(SDHNDL*        hndl,
                     unsigned char* buff,
                     unsigned long  func,
                     unsigned long  adr,
                     long           cnt,
  ...
  if (sec) { /* more than io block size (=512 bytes) */
    ...
    for (i = sec; i > 0; i--) {
      ...
      /* set argument */
      arg = ((func << 28) | 0x80000000u | (op_code << 26) | (adr << 9) | (unsigned long)0);
