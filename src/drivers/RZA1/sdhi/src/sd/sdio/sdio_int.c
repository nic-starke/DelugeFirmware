...
int _sdio_get_int(SDHNDL* hndl) {
  ...
  if (info) { return SD_OK; /* any interrupt occured */ }
...
void sdio_int_handler(int sd_port) {
  ...
  if (_sdio_get_int(hndl) == SD_OK) {
    if (hndl->int_io_callback) { (*hndl->int_io_callback)(sd_port); }
  } else {
...
int sdio_enable_int(int sd_port) {
  ...
  sd_outp(hndl,
          SDIO_INFO1_MASK,
          ((unsigned short)(sd_inp(hndl, SDIO_INFO1_MASK) & ~SDIO_MODE_IOMOD)) | 0x06);
