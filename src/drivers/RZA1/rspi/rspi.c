...
void R_RSPI_Create(uint8_t channel, uint32_t bitRate, uint8_t phase, uint8_t dataSize) {
  ...
  if (dataSize == 32)
    ...
    RSPI(channel).SPBFCR.BYTE = 0b00100000; // Receive buffer data triggering number is 1 byte. TX
                                            // buffer declared "empty" as soon as it has 4 bytes of
                                            // "space" in it (remember, it has 8 bytes total)
...
#else
  RSPI(channel).SPCR |= 0b00101000; // Just TX interrupt (for DMA). We'll manually enable the RX
                                    // one when we need it.
#endif
 ...
 * Arguments    : tx_buf -
 *                 transfer buffer pointer (not used when data is handled by
 *DTC) tx_num - buffer size rx_buf - receive buffer pointer (not used when data
 *is handled by DTC) Return Value : status - MD_OK or MD_ARGERROR
 *******************************************************************************/
...
void R_RSPI_SendAndDontWait(uint8_t channel, uint8_t data) {
  ...
  while (RZA_IO_RegRead_8(&(RSPI(channel).SPSR.BYTE), RSPIn_SPSR_SPRF_SHIFT, RSPIn_SPSR_SPRF)) {
    ...
    // Receive data. Note that even if we didn't want the receive data, we still
    // have to read it back, because SPI transmission halts once the RX buffer
    // is full
