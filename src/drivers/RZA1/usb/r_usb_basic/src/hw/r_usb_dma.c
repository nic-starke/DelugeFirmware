...
uint16_t g_usb_cstd_dma_fifo[USB_NUM_USBIP][USB_DMA_USE_CH_MAX]; /* DMA0 and DMA1 FIFO buffer
                                                                    size */
...
uint8_t g_usb_cstd_dma_ch[USB_NUM_USBIP][USB_FIFO_ACCESS_NUM_MAX] =
    {/* DMA ch no. table */
...
void usb_cstd_buf2dxfifo_start_dma(usb_utr_t* ptr, uint16_t pipe, uint16_t useport) {
  ...
  if (0u != DmaSize) {
    ...
    usb_cstd_Buf2Fifo_DMAX(ch_no,
                           ptr,
                           useport,
                           (uint32_t)(g_p_usb_hstd_data[ip][pipe]),
                           DmaSize,
                           pipe,
                           TransDataBlockSize);
#endif
    ...
    usb_cstd_Buf2Fifo_DMAX(ch_no,
                           ptr,
                           useport,
                           (uint32_t)(g_p_usb_pstd_data[pipe]),
                           DmaSize,
                           pipe,
                           TransDataBlockSize);
#endif
...
/***********************************************************************************************************************
 ...
 Return value    : none
 ***********************************************************************************************************************/
void usb_cstd_dxfifo2buf_start_dma(usb_utr_t* ptr,
                                   uint16_t   pipe,
                                   uint16_t   useport,
                                   uint32_t   length) {
  uint8_t* p_data_ptr;
...
void usb_dma_driver(void) {
  ...
  if (g_usb_cstd_dma_int.wp != g_usb_cstd_dma_int.rp) {
    if (USB_HOST == g_usb_usbmode) {
      ...
      usb_dma_stop_dxfifo(
          utr.ip,
          g_usb_cstd_dma_int.buf[g_usb_cstd_dma_int.rp].fifo_type); /* Stop DMA,FIFO access */
      ...
      usb_dma_buf2dxfifo_complete(USB_NULL,
                                  g_usb_cstd_dma_int.buf[g_usb_cstd_dma_int.rp].fifo_type);
...
/***********************************************************************************************************************
 ...
 Description     : Set end of DMA transfer. Set to restart DMA trans-
                 : fer according to data size of remaining functions to be
 processed. Arguments       : usb_utr_t    *ptr         : Pointer to usb_utr_t
 structure. : uint16_t     useport      : FIFO select Return value    : none
 ***********************************************************************************************************************/
...
/***********************************************************************************************************************
 ...
 Arguments       : uint16_t     ip_no        : USB module number
 (USB_IP0/USB_IP1) : uint16_t     useport      : FIFO select : uint8_t dma_ch_no
 : DMA channel no. Return value    : none
 ***********************************************************************************************************************/
...
/***********************************************************************************************************************
 ...
 Arguments       : uint16_t     ip_no        : USB module number
 (USB_IP0/USB_IP1) : uint16_t     useport      : FIFO select Return value    :
 DMA channel no.
 ***********************************************************************************************************************/
...
/***********************************************************************************************************************
 ...
 Description     : Get DMA channel no.
 Arguments       : uint16_t     ip_no        : USB module number
 (USB_IP0/USB_IP1) : uint16_t     ch_no        : FIFO select Return value    :
 DMA channel no.
 ***********************************************************************************************************************/
uint16_t usb_dma_ip_ch_no2useport(uint16_t ip_no, uint16_t ch_no) {
  ...
  for (port_cnt = 0; port_cnt < USB_FIFO_ACCESS_NUM_MAX; port_cnt++) {
    if (g_usb_cstd_dma_ch[ip][port_cnt] == ch_no) { return port_cnt; }
  }
  return 0xffff;
...
uint16_t usb_dma_def_tx_ch_no2ip_no(uint16_t ch_no) {
  ...
  for (ip_cnt = 0; ip_cnt < USB_NUM_USBIP; ip_cnt++) {
    if (g_usb_dma_setting_ch[ip_cnt][USB_DMA_TX] == ch_no) { return ip_cnt; }
  }
  return 0xffff;
#endif /* (USB_CFG_MODE & USB_CFG_PERI) == USB_CFG_PERI */
...
/***********************************************************************************************************************
 ...
 Return value    : none
 ***********************************************************************************************************************/
void usb_cstd_DXfifo2BufStartDma(usb_utr_t* ptr,
                                 uint16_t   pipe,
                                 uint16_t   useport,
                                 uint32_t   length,
                                 uint32_t   dest_addr) {
  ...
  if (0u != dma_size) {
    ...
    usb_cstd_set_transaction_counter(ptr,
                                     pipe,
                                     (uint16_t)(((length - (uint32_t)1u) / mxps) + (uint32_t)1u));
    ...
    //  We ignore NRDY interrupts anyway, as there are tons of them
    //  continuously, and enabling them at all was causing freezes (or was it
    //  UART / SD lockups?), right since we first added this "new" (2016) USB
    //  driver (in 2019).
...
/***********************************************************************************************************************
 ...
 Return value    : void
 ***********************************************************************************************************************/
void usb_cpu_dxfifo2buf_start_dma0(usb_utr_t* ptr,
                                   uint32_t   SourceAddr,
                                   uint16_t   useport,
                                   uint32_t   destAddr,
                                   uint32_t   transfer_size) {
  ...
  /* Wait for RZA1 check stasus register Channel Status Register EN==0 and
   * TACT==0 */
  if ((DMAC0.CHSTAT_n & 0x05) != 0u) {
    DMAC0.CHCTRL_n |= USB_DMA_CHCTRL_CLREN;
    DMAC0.CHCTRL_n |= USB_DMA_CHCTRL_CLRRQ;
    while (1) {
      if ((DMAC0.CHSTAT_n & 0x05) == 0u) { break; }
    }
  }
  /* stop DMARS */
  ...
  chcfg_data = USB_DMA_CHCFG_AM_BCM | USB_DMA_CHCFG_LVL | USB_DMA_CHCFG_DEM | USB_DMA_CHCFG_HIEN |
               USB_DMA_CHCFG_SEL_0_8 | USB_DMA_CHCFG_DDS_256 | USB_DMA_CHCFG_SAD |
               USB_DMA_CHCFG_SDS_256;
#else
  chcfg_data = USB_DMA_CHCFG_AM_BCM | USB_DMA_CHCFG_LVL | USB_DMA_CHCFG_DEM | USB_DMA_CHCFG_HIEN |
               USB_DMA_CHCFG_SEL_0_8 | USB_DMA_CHCFG_DDS_8 | USB_DMA_CHCFG_SAD |
               USB_DMA_CHCFG_SDS_8;
#endif
  ...
  if (USB_CFG_IP0 == ip) {
    if (USB_D0DMA == useport) { DMAC.DMARS0 |= 0x00000083; }
    if (USB_D1DMA == useport) { DMAC.DMARS0 |= 0x00000087; }
  }
  if (USB_CFG_IP1 == ip) {
    if (USB_D0DMA == useport) { DMAC.DMARS0 |= 0x0000008B; }
    if (USB_D1DMA == useport) { DMAC.DMARS0 |= 0x0000008F; }
  }
...
/***********************************************************************************************************************
 ...
 Return value    : void
 ***********************************************************************************************************************/
void usb_cpu_dxfifo2buf_start_dma1(usb_utr_t* ptr,
                                   uint32_t   SourceAddr,
                                   uint16_t   useport,
                                   uint32_t   destAddr,
                                   uint32_t   transfer_size) {
  ...
  if ((DMAC1.CHSTAT_n & 0x05) != 0u) {
    ...
    while (1) {
      if ((DMAC1.CHSTAT_n & 0x05) == 0u) { break; }
    }
  }
...
#if USE_DMA32
  chcfg_data = USB_DMA_CHCFG_AM_BCM | USB_DMA_CHCFG_LVL | USB_DMA_CHCFG_DEM | USB_DMA_CHCFG_HIEN |
               USB_DMA_CHCFG_SEL_1_9 | USB_DMA_CHCFG_DDS_256 | USB_DMA_CHCFG_SAD |
               USB_DMA_CHCFG_SDS_256;
#else
  chcfg_data = USB_DMA_CHCFG_AM_BCM | USB_DMA_CHCFG_LVL | USB_DMA_CHCFG_DEM | USB_DMA_CHCFG_HIEN |
               USB_DMA_CHCFG_SEL_1_9 | USB_DMA_CHCFG_DDS_8 | USB_DMA_CHCFG_SAD |
               USB_DMA_CHCFG_SDS_8;
#endif
  ...
  if (USB_CFG_IP0 == ip) {
    if (USB_D0DMA == useport) { DMAC.DMARS0 |= 0x00830000; }
    if (USB_D1DMA == useport) { DMAC.DMARS0 |= 0x00870000; }
  }
  if (USB_CFG_IP1 == ip) {
    if (USB_D0DMA == useport) { DMAC.DMARS0 |= 0x008B0000; }
    if (USB_D1DMA == useport) { DMAC.DMARS0 |= 0x008F0000; }
  }
  DMAC1.CHCTRL_n |= USB_DMA_CHCTRL_SWRST;
...
/***********************************************************************************************************************
 ...
 Return value    : void
 ***********************************************************************************************************************/
void usb_cpu_dxfifo2buf_start_dma2(usb_utr_t* ptr,
                                   uint32_t   SourceAddr,
                                   uint16_t   useport,
                                   uint32_t   destAddr,
                                   uint32_t   transfer_size) {
  ...
  if ((DMAC2.CHSTAT_n & 0x05) != 0u) {
    ...
    while (1) {
      if ((DMAC2.CHSTAT_n & 0x05) == 0u) { break; }
    }
  }
...
#if USE_DMA32
  chcfg_data = USB_DMA_CHCFG_AM_BCM | USB_DMA_CHCFG_LVL | USB_DMA_CHCFG_DEM | USB_DMA_CHCFG_HIEN |
               USB_DMA_CHCFG_SEL_2_10 | USB_DMA_CHCFG_DDS_256 | USB_DMA_CHCFG_SAD |
               USB_DMA_CHCFG_SDS_256;
#else
  chcfg_data = USB_DMA_CHCFG_AM_BCM | USB_DMA_CHCFG_LVL | USB_DMA_CHCFG_DEM | USB_DMA_CHCFG_HIEN |
               USB_DMA_CHCFG_SEL_2_10 | USB_DMA_CHCFG_DDS_8 | USB_DMA_CHCFG_SAD |
               USB_DMA_CHCFG_SDS_8;
#endif
  ...
  if (USB_CFG_IP0 == ip) {
    if (USB_D0DMA == useport) { DMAC.DMARS1 |= 0x00000083; }
    if (USB_D1DMA == useport) { DMAC.DMARS1 |= 0x00000087; }
  }
  if (USB_CFG_IP1 == ip) {
    if (USB_D0DMA == useport) { DMAC.DMARS1 |= 0x0000008B; }
    if (USB_D1DMA == useport) { DMAC.DMARS1 |= 0x0000008F; }
  }
...
/***********************************************************************************************************************
 ...
 Return value    : void
 ***********************************************************************************************************************/
void usb_cpu_dxfifo2buf_start_dma3(usb_utr_t* ptr,
                                   uint32_t   SourceAddr,
                                   uint16_t   useport,
                                   uint32_t   destAddr,
                                   uint32_t   transfer_size) {
  ...
  if ((DMAC3.CHSTAT_n & 0x05) != 0u) {
    ...
    while (1) {
      if ((DMAC3.CHSTAT_n & 0x05) == 0u) { break; }
    }
  }
...
#if USE_DMA32
  chcfg_data = USB_DMA_CHCFG_AM_BCM | USB_DMA_CHCFG_LVL | USB_DMA_CHCFG_DEM | USB_DMA_CHCFG_HIEN |
               USB_DMA_CHCFG_SEL_3_11 | USB_DMA_CHCFG_DDS_256 | USB_DMA_CHCFG_SAD |
               USB_DMA_CHCFG_SDS_256;
#else
  chcfg_data = USB_DMA_CHCFG_AM_BCM | USB_DMA_CHCFG_LVL | USB_DMA_CHCFG_DEM | USB_DMA_CHCFG_HIEN |
               USB_DMA_CHCFG_SEL_3_11 | USB_DMA_CHCFG_DDS_8 | USB_DMA_CHCFG_SAD |
               USB_DMA_CHCFG_SDS_8;
#endif
  ...
  if (USB_CFG_IP0 == ip) {
    if (USB_D0DMA == useport) { DMAC.DMARS1 |= 0x00830000; }
    if (USB_D1DMA == useport) { DMAC.DMARS1 |= 0x00870000; }
  }
  if (USB_CFG_IP1 == ip) {
    if (USB_D0DMA == useport) { DMAC.DMARS1 |= 0x008B0000; }
    if (USB_D1DMA == useport) { DMAC.DMARS1 |= 0x008F0000; }
  }
  DMAC3.CHCTRL_n |= USB_DMA_CHCTRL_SWRST;
...
/***********************************************************************************************************************
 ...
 Return value    : void
 ***********************************************************************************************************************/
void usb_cpu_dxfifo2buf_start_dmaX(usb_utr_t* ptr,
                                   uint16_t   ch_no,
                                   uint32_t   SourceAddr,
                                   uint16_t   useport,
                                   uint32_t   destAddr,
...
/***********************************************************************************************************************
 ...
 Return value    : void
 ***********************************************************************************************************************/
void usb_cpu_buf2dxfifo_start_dma0(uint16_t useport,
                                   uint32_t src_adr,
                                   uint16_t ip,
                                   uint32_t transfer_size,
  ...
  if (1u == trans_block_size) {
    DMAC0.CHCFG_n = USB_DMA_CHCFG_AM_BCM | USB_DMA_CHCFG_LVL | USB_DMA_CHCFG_HIEN |
                    USB_DMA_CHCFG_SEL_0_8 | USB_DMA_CHCFG_DDS_8 | USB_DMA_CHCFG_DAD |
                    USB_DMA_CHCFG_SDS_8 | USB_DMA_CHCFG_REQD;
  } else if (4u == trans_block_size) {
    DMAC0.CHCFG_n = USB_DMA_CHCFG_AM_BCM | USB_DMA_CHCFG_LVL | USB_DMA_CHCFG_HIEN |
                    USB_DMA_CHCFG_SEL_0_8 | USB_DMA_CHCFG_DDS_32 | USB_DMA_CHCFG_DAD |
                    USB_DMA_CHCFG_SDS_32 | USB_DMA_CHCFG_REQD;
  } else if (32u == trans_block_size) {
    DMAC0.CHCFG_n = USB_DMA_CHCFG_AM_BCM | USB_DMA_CHCFG_LVL | USB_DMA_CHCFG_HIEN |
                    USB_DMA_CHCFG_SEL_0_8 | USB_DMA_CHCFG_DDS_256 | USB_DMA_CHCFG_DAD |
                    USB_DMA_CHCFG_SDS_256 | USB_DMA_CHCFG_REQD;
  }
...
/***********************************************************************************************************************
 ...
 Return value    : void
 ***********************************************************************************************************************/
void usb_cpu_buf2dxfifo_start_dma1(uint16_t useport,
                                   uint32_t src_adr,
                                   uint16_t ip,
                                   uint32_t transfer_size,
                                   uint8_t  trans_block_size) {
  ...
  if (1u == trans_block_size) {
    DMAC1.CHCFG_n = USB_DMA_CHCFG_AM_BCM | USB_DMA_CHCFG_LVL | USB_DMA_CHCFG_HIEN |
                    USB_DMA_CHCFG_SEL_1_9 | USB_DMA_CHCFG_DDS_8 | USB_DMA_CHCFG_DAD |
                    USB_DMA_CHCFG_SDS_8 | USB_DMA_CHCFG_REQD;
  } else if (4u == trans_block_size) {
    DMAC1.CHCFG_n = USB_DMA_CHCFG_AM_BCM | USB_DMA_CHCFG_LVL | USB_DMA_CHCFG_HIEN |
                    USB_DMA_CHCFG_SEL_1_9 | USB_DMA_CHCFG_DDS_32 | USB_DMA_CHCFG_DAD |
                    USB_DMA_CHCFG_SDS_32 | USB_DMA_CHCFG_REQD;
  } else if (32u == trans_block_size) {
    DMAC1.CHCFG_n = USB_DMA_CHCFG_AM_BCM | USB_DMA_CHCFG_LVL | USB_DMA_CHCFG_HIEN |
                    USB_DMA_CHCFG_SEL_1_9 | USB_DMA_CHCFG_DDS_256 | USB_DMA_CHCFG_DAD |
                    USB_DMA_CHCFG_SDS_256 | USB_DMA_CHCFG_REQD;
  }
...
/***********************************************************************************************************************
 ...
 Return value    : void
 ***********************************************************************************************************************/
void usb_cpu_buf2d1fifo_start_dma2(uint16_t useport,
                                   uint32_t src_adr,
                                   uint16_t ip,
                                   uint32_t transfer_size,
                                   uint8_t  trans_block_size) {
  ...
  if (1u == trans_block_size) {
    DMAC2.CHCFG_n = USB_DMA_CHCFG_AM_BCM | USB_DMA_CHCFG_LVL | USB_DMA_CHCFG_HIEN |
                    USB_DMA_CHCFG_SEL_2_10 | USB_DMA_CHCFG_DDS_8 | USB_DMA_CHCFG_DAD |
                    USB_DMA_CHCFG_SDS_8 | USB_DMA_CHCFG_REQD;
  } else if (4u == trans_block_size) {
    DMAC2.CHCFG_n = USB_DMA_CHCFG_AM_BCM | USB_DMA_CHCFG_LVL | USB_DMA_CHCFG_HIEN |
                    USB_DMA_CHCFG_SEL_2_10 | USB_DMA_CHCFG_DDS_32 | USB_DMA_CHCFG_DAD |
                    USB_DMA_CHCFG_SDS_32 | USB_DMA_CHCFG_REQD;
  } else if (32u == trans_block_size) {
    DMAC2.CHCFG_n = USB_DMA_CHCFG_AM_BCM | USB_DMA_CHCFG_LVL | USB_DMA_CHCFG_HIEN |
                    USB_DMA_CHCFG_SEL_2_10 | USB_DMA_CHCFG_DDS_256 | USB_DMA_CHCFG_DAD |
                    USB_DMA_CHCFG_SDS_256 | USB_DMA_CHCFG_REQD;
  }
...
/***********************************************************************************************************************
 ...
 Return value    : void
 ***********************************************************************************************************************/
void usb_cpu_buf2d1fifo_start_dma3(uint16_t useport,
                                   uint32_t src_adr,
                                   uint16_t ip,
                                   uint32_t transfer_size,
                                   uint8_t  trans_block_size) {
  ...
  if (1u == trans_block_size) {
    DMAC3.CHCFG_n = USB_DMA_CHCFG_AM_BCM | USB_DMA_CHCFG_LVL | USB_DMA_CHCFG_HIEN |
                    USB_DMA_CHCFG_SEL_3_11 | USB_DMA_CHCFG_DDS_8 | USB_DMA_CHCFG_DAD |
                    USB_DMA_CHCFG_SDS_8 | USB_DMA_CHCFG_REQD;
  } else if (4u == trans_block_size) {
    DMAC3.CHCFG_n = USB_DMA_CHCFG_AM_BCM | USB_DMA_CHCFG_LVL | USB_DMA_CHCFG_HIEN |
                    USB_DMA_CHCFG_SEL_3_11 | USB_DMA_CHCFG_DDS_32 | USB_DMA_CHCFG_DAD |
                    USB_DMA_CHCFG_SDS_32 | USB_DMA_CHCFG_REQD;
  } else if (32u == trans_block_size) {
    DMAC3.CHCFG_n = USB_DMA_CHCFG_AM_BCM | USB_DMA_CHCFG_LVL | USB_DMA_CHCFG_HIEN |
                    USB_DMA_CHCFG_SEL_3_11 | USB_DMA_CHCFG_DDS_256 | USB_DMA_CHCFG_DAD |
                    USB_DMA_CHCFG_SDS_256 | USB_DMA_CHCFG_REQD;
  }
...
/***********************************************************************************************************************
 ...
 Arguments       : uint32_t     src                 : transfer data pointer
                 ...
                 : uint8_t      trans_block_size    : DMA trans block size (1,
 4, 32byte) Return value    : none
 ***********************************************************************************************************************/
void usb_cstd_Buf2Fifo_DMAX(uint16_t   ch_no,
                            usb_utr_t* ptr,
                            uint16_t   useport,
                            uint32_t   src,
                            uint32_t   data_size,
...
/***********************************************************************************************************************
 ...
 4, 32byte) Return value    : none
 ***********************************************************************************************************************/
void usb_cstd_Buf2Fifo_DMA0(usb_utr_t* ptr,
                            uint16_t   useport,
                            uint32_t   src,
                            uint32_t   data_size,
                            uint8_t    pipe,
...
/***********************************************************************************************************************
 ...
 Arguments       : uint32_t     src                 : transfer data pointer
                 ...
                 : uint16_t     pipe                : Pipe nr.
                 : uint8_t      trans_block_size    : DMA trans block size (1,
 32byte) Return value    : none
 ***********************************************************************************************************************/
void usb_cstd_Buf2Fifo_DMA1(usb_utr_t* ptr,
                            uint16_t   useport,
                            uint32_t   src,
                            uint32_t   data_size,
                            uint8_t    pipe,
  ...
  /* Enable Not Ready Interrupt */
  // usb_cstd_nrdy_enable(ptr, pipe);
  //  We ignore NRDY interrupts anyway, as there are tons of them continuously,
  //  and enabling them at all was causing freezes (or was it UART / SD
  //  lockups?), right since we first added this "new" (2016) USB driver (in
  //  2019).
...
/***********************************************************************************************************************
 ...
 32byte) Return value    : none
 ***********************************************************************************************************************/
void usb_cstd_Buf2Fifo_DMA2(usb_utr_t* ptr,
                            uint16_t   useport,
                            uint32_t   src,
                            uint32_t   data_size,
                            uint8_t    pipe,
...
/***********************************************************************************************************************
 ...
 32byte) Return value    : none
 ***********************************************************************************************************************/
void usb_cstd_Buf2Fifo_DMA3(usb_utr_t* ptr,
                            uint16_t   useport,
                            uint32_t   src,
                            uint32_t   data_size,
                            uint8_t    pipe,
...
uint32_t usb_cstd_GetDXfifoYAdr(uint16_t ip, uint16_t trans_size, uint16_t pipemode) {
  ...
  switch (trans_size) {
    case 1u:
      ...
      if (USB_IP0 == ip) {
        if (USB_D0DMA == pipemode) { ret_address = (uint32_t) & (USB200.D0FIFO.UINT8[3]); }
        if (USB_D1DMA == pipemode) { ret_address = (uint32_t) & (USB200.D1FIFO.UINT8[3]); }
      }
      if (USB_IP1 == ip) {
        if (USB_D0DMA == pipemode) { ret_address = (uint32_t) & (USB201.D0FIFO.UINT8[3]); }
        if (USB_D1DMA == pipemode) { ret_address = (uint32_t) & (USB201.D1FIFO.UINT8[3]); }
      }
    ...
    case 4u:
      ...
      if (USB_IP0 == ip) {
        if (USB_D0DMA == pipemode) { ret_address = (uint32_t) & (USB200.D0FIFO); }
        if (USB_D1DMA == pipemode) { ret_address = (uint32_t) & (USB200.D1FIFO); }
      }
      if (USB_IP1 == ip) {
        if (USB_D0DMA == pipemode) { ret_address = (uint32_t) & (USB201.D0FIFO); }
        if (USB_D1DMA == pipemode) { ret_address = (uint32_t) & (USB201.D1FIFO); }
      }
    ...
    case 32u:
      ...
      if (USB_IP0 == ip) {
        if (USB_D0DMA == pipemode) { ret_address = (uint32_t) & (USB200.D0FIFOB0); }
        if (USB_D1DMA == pipemode) { ret_address = (uint32_t) & (USB200.D1FIFOB0); }
      }
      if (USB_IP1 == ip) {
        if (USB_D0DMA == pipemode) { ret_address = (uint32_t) & (USB201.D0FIFOB0); }
        if (USB_D1DMA == pipemode) { ret_address = (uint32_t) & (USB201.D1FIFOB0); }
      }
...
void usb_creg_write_dxfbcfg(uint16_t ip, uint16_t pipemode, uint16_t data) {
  if (USB_IP0 == ip) {
    if (USB_D0DMA == pipemode) { USB200.D0FBCFG = data; }
    if (USB_D1DMA == pipemode) { USB200.D1FBCFG = data; }
  }
  if (USB_IP1 == ip) {
    if (USB_D0DMA == pipemode) { USB201.D0FBCFG = data; }
    if (USB_D1DMA == pipemode) { USB201.D1FBCFG = data; }
  }
...
/***********************************************************************************************************************
 ...
 Description     : Returns the *address* of the FIFOSEL register corresponding
 to : specified PIPEMODE. Arguments       : uint16_t     ip           : ip port
 no (port0 = 0, ...) : uint16_t     pipemode     : CUSE/D0DMA/D1DMA Return value
 : none
 ***********************************************************************************************************************/
static void* usb_creg_get_fifosel_adr(uint16_t ip, uint16_t pipemode) {
  ...
  switch (pipemode) {
    case USB_CUSE:
      ...
      if (USB_IP0 == ip) { reg_p = (void*)&(USB200.CFIFOSEL); }
      if (USB_IP1 == ip) { reg_p = (void*)&(USB201.CFIFOSEL); }
    ...
    case USB_D0DMA:
      ...
      if (USB_IP0 == ip) { reg_p = (void*)&(USB200.D0FIFOSEL); }
      if (USB_IP1 == ip) { reg_p = (void*)&(USB201.D0FIFOSEL); }

      break;

    case USB_D1DMA:

      if (USB_IP0 == ip) { reg_p = (void*)&(USB200.D1FIFOSEL); }
      if (USB_IP1 == ip) { reg_p = (void*)&(USB201.D1FIFOSEL); }
...
/***********************************************************************************************************************
 ...
 Description     : Reset DCLRM-bits (FIFO buffer not auto-cleared) of the
 FIFOSEL : corresponding to the specified PIPEMODE. Arguments       : uint16_t
 ip           : ip Portno (port0 = 0...) : uint16_t     pipemode     :
 CUSE/D0DMA/D1DMA. Return value    : none
 ***********************************************************************************************************************/
...
/***********************************************************************************************************************
 ...
 Description     : Set DREQE-bits (to output signal DxREQ_Na) of the FIFOSEL
 cor- : responding to specified PIPEMODE. Arguments       : uint16_t     ip : ip
 Port no (port0 = 0...) : uint16_t     pipemode     : CUSE/D0DMA/D1DMA. Return
 value    : none
 ***********************************************************************************************************************/
...
/***********************************************************************************************************************
 ...
 Description     : Clear DREQE-bits (To prohibit the output of the signal
 DxREQ_N) : of the FIFOSEL corresponding to the specified PIPEMODE. Arguments :
 uint16_t     ip           : ip Port no (port0 = 0...) : uint16_t     pipemode
 : CUSE/D0DMA/D1DMA Return value    : none
 ***********************************************************************************************************************/
...
/***********************************************************************************************************************
 ...
 Description     : Set MBW-bits (CFIFO Port Access Bit Width) of the FIFOSEL
 cor- : responding to the specified PIPEMODE, to select 8 or 16-bit : wide FIFO
 port access. Arguments       : uint16_t      ip           : ip Port no(port0 =
 0...) : uint16_t      pipemode     : CUSE/D0DMA/D1DMA. : uint16_t      data :
 Defined value of 8 (data = 0x0000) or 16 bit : (data = 0x0400), 32 bit (data =
 0x0800) access mode. Return value    : none
 ***********************************************************************************************************************/
  ...
  *reg_p = reg_data;
  ...
  switch (pipemode) {
    ...
    case USB_CUSE:
      ...
      if (USB_IP0 == ip) { dummy_rd_w = USB200.CFIFO.UINT32; }
      if (USB_IP1 == ip) { dummy_rd_w = USB201.CFIFO.UINT32; }

      break;

    case USB_D0DMA:

      if (USB_IP0 == ip) { dummy_rd_w = USB200.D0FIFO.UINT32; }
      if (USB_IP1 == ip) { dummy_rd_w = USB201.D0FIFO.UINT32; }

      break;

    case USB_D1DMA:

      if (USB_IP0 == ip) { dummy_rd_w = USB200.D1FIFO.UINT32; }
      if (USB_IP1 == ip) { dummy_rd_w = USB201.D1FIFO.UINT32; }
...
/***********************************************************************************************************************
 ...
 the FIFOCTR corresponding : to the specified PIPEMODE. Arguments       :
 usb_utr_t    *ptr         : Pointer to usb_utr_t structure. : uint16_t pipemode
 : CUSE/D0DMA/D1DMA. Return value    : none
 ***********************************************************************************************************************/
...
void usb_creg_set_brdyenb(uint16_t ip, uint16_t pipeno) {
  if (USB_IP0 == ip) { USB200.BRDYENB |= (1 << pipeno); }
  if (USB_IP1 == ip) { USB201.BRDYENB |= (1 << pipeno); }
...
void usb_creg_set_bempenb(uint16_t ip, uint16_t pipeno) {
  if (USB_IP0 == ip) { USB200.BEMPENB |= (1 << pipeno); }
  if (USB_IP1 == ip) { USB201.BEMPENB |= (1 << pipeno); }
...
void usb_creg_clr_sts_bemp(uint16_t ip, uint16_t pipeno) {
  if (USB_IP0 == ip) { USB200.BEMPSTS = (uint16_t) ~(1 << pipeno); }
  if (USB_IP1 == ip) { USB201.BEMPSTS = (uint16_t) ~(1 << pipeno); }
...
/***********************************************************************************************************************
 ...
 content. : The Pipe Control Register returned is determined by the speci- :
 fied pipe number. Arguments       : uint16_t     ip           : ip Port
 no(port0 = 0...) : uint16_t     pipeno       : Pipe number. Return value    :
 PIPExCTR content
 ***********************************************************************************************************************/
uint16_t usb_creg_read_pipectr(uint16_t ip, uint16_t pipeno) {
  ...
  if (USB_PIPE0 == pipeno) {
    if (USB_IP0 == ip) { reg_p = (uint16_t*)&(USB200.DCPCTR); }
    if (USB_IP1 == ip) { reg_p = (uint16_t*)&(USB201.DCPCTR); }
  }
  if (USB_PIPE1 == pipeno) {
    if (USB_IP0 == ip) { reg_p = (uint16_t*)&(USB200.PIPE1CTR); }
    if (USB_IP1 == ip) { reg_p = (uint16_t*)&(USB201.PIPE1CTR); }
  }
  if (USB_PIPE2 == pipeno) {
    if (USB_IP0 == ip) { reg_p = (uint16_t*)&(USB200.PIPE2CTR); }
    if (USB_IP1 == ip) { reg_p = (uint16_t*)&(USB201.PIPE2CTR); }
  }
  if (USB_PIPE3 == pipeno) {
    if (USB_IP0 == ip) { reg_p = (uint16_t*)&(USB200.PIPE3CTR); }
    if (USB_IP1 == ip) { reg_p = (uint16_t*)&(USB201.PIPE3CTR); }
  }
