...
uint16_t g_usb_hstd_use_pipe[USB_NUM_USBIP]; // Keeps a record of which pipes
                                             // are in use, with one bit
                                             // representing each pipe
...
/***********************************************************************************************************************
 ...
 Return value    : uint16_t                  : FIFO buffer size or max packet
 size.
 ***********************************************************************************************************************/
...
void usb_cstd_pipe_init(usb_utr_t* ptr, uint16_t pipe, uint16_t* tbl, uint16_t ofs) {
  ...
  int      ip; // Bugfix added by Rohan - we need this in case ptr is supplied as
               // NULL, which happens always in peripheral mode

  if (USB_NULL == ptr) {
    ip = USB_CFG_USE_USBIP; // Bugfix added by Rohan - we need this in case ptr
                            // is supplied as NULL, which happens always in
                            // peripheral mode
    ...
    ip = ptr->ip; // Bugfix added by Rohan - we need this in case ptr is
  ...
  if (USB_NULL != tbl[ofs + 1]) {
    g_usb_hstd_use_pipe[ip] |=
        ((uint16_t)1 << pipe); // Bugfix added by Rohan - we need this in case ptr is
                               // supplied as NULL, which happens always in peripheral mode
  } else {
    g_usb_hstd_use_pipe[ip] &=
        (~((uint16_t)1 << pipe)); // Bugfix added by Rohan - we need this in case ptr is
  ...
  if ((USB_D0DMA == useport) || (USB_D1DMA == useport)) { tbl[ofs + 1] |= USB_BFREON; }
  ...
  // hw_usb_set_bempenb(USB_NULL, pipe); // Added by Rohan - we now just enable
  // this here once, rather than at the start of every transfer. Actually no,
  // doesn't work... though I thought it did for a bit...
...
void change_destination_of_send_pipe(usb_utr_t* ptr, uint16_t pipe, uint16_t* tbl, int sq) {
  ...
  if ((USB_D0DMA == useport) || (USB_D1DMA == useport)) { tbl[1] |= USB_BFREON; }
#endif

  hw_usb_write_pipecfg(NULL, tbl[1], pipe);
  // We won't set the PIPEBUF, cos I (Rohan) have made it so each pipe number
  // always has the same buffer number, and we're still on the same pipe.
  hw_usb_write_pipemaxp(NULL,
                        tbl[3],
                        pipe); // This sets the destination USB peripheral
                               // address, so yes we need to change this.
  hw_usb_write_pipeperi(NULL,
                        tbl[4]); // Probably worth doing. This sets stuff relating to "flushing"
                                 // and "intervals" - possibly more relevant to ISO or interrupt
                                 // endpoints, but this driver does indeed work out a value to put
                                 // for this in the table, so we'd better grab it
...
void usb_cstd_clr_pipe_cnfg(usb_utr_t* ptr, uint16_t pipe_no) {
  ...
  // if (pipe_no == USB_CFG_HMIDI_BULK_SEND || pipe_no ==
  // USB_CFG_HMIDI_INT_SEND) return; // Don't deconfigure the send-pipe cos it's
  // shared - Rohan
...
#if ((USB_CFG_MODE & USB_CFG_HOST) == USB_CFG_HOST)
  /* Clear use pipe no info. Added by Rohan - I can't see why this wouldn't have
   * already been done, and the USB library doesn't even make use of this info
   * anyway, so modification should be fine. */
  g_usb_hstd_use_pipe[USB_CFG_USE_USBIP] &=
      (~((uint16_t)1 << pipe_no)); // Uses "hard-coded" USB_CFG_USE_USBIP
                                   // because ptr is sometimes NULL
#endif                             /* (USB_CFG_MODE & USB_CFG_HOST) == USB_CFG_HOST */
...
void usb_cstd_set_nak(usb_utr_t* ptr, uint16_t pipe) {
  ...
  /* The state of PBUSY continues while transmitting the packet when it is a
   * detach. */
  /* 1ms comes off when leaving because the packet duration might not exceed
   * 1ms.  */
  /* Whether it is PBUSY release or 1ms passage can be judged. */
  for (n = 0; n < 0xFFFFu; ++n) {
    /* PIPE control reg read */
    buf = hw_usb_read_pipectr(ptr, pipe);
    if ((uint16_t)(buf & USB_PBUSY) == 0) { n = 0xFFFEu; }
  }
...
void usb_cstd_set_nak_fast_rohan(uint16_t pipe) {
  ...
  // A bit of a weird one... my copying and simplifying all this stuff into one
  // function caused i029 (unknown interrupt) error when sending MIDI. It seems
  // that we actually need a tiny ~1uS delay here. I couldn't work out just why
  // that would be required. And the kinda cryptic original comment in Renesas's
  // code didn't really help.
#if 0
...
uint16_t usb_cstd_is_set_frdy_rohan(uint16_t pipe) {
  ...
  // I drastically rearranged the code from usb_cstd_is_set_frdy(), here.
  // Basically, it's meant to time out with error if not FRDY within 100ns.
  uint16_t startTime = *TCNT[TIMER_SYSTEM_SUPERFAST];
  ...
  while (true) {
    buffer = *p_reg;
    if (buffer & USB_FRDY) { return buffer; }
    uint16_t timeNow    = *TCNT[TIMER_SYSTEM_SUPERFAST];
    uint16_t timePassed = timeNow - startTime;
    if (timePassed >= 5) { // Definitely more than 100ns then. (It's 29.5928
...
uint16_t usb_cstd_is_set_frdy(usb_utr_t* ptr, uint16_t pipe, uint16_t fifosel, uint16_t isel) {
  ...
  for (i = 0; i < 4; i++) {
    ...
    if ((uint16_t)(buffer & USB_FRDY) == USB_FRDY) { return (buffer); }
...
/***********************************************************************************************************************
 ...
 Arguments       : usb_utr_t    *ptr         : Pointer to usb_utr_t structure.
                 ...
                 : uint16_t     isel         : CFIFO Port Access
 Direction.(Pipe1 to 9:Set to 0) Return value    : none
 ***********************************************************************************************************************/
