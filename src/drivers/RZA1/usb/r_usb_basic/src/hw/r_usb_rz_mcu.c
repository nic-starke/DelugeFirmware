...
usb_err_t usb_module_start(uint8_t ip_type) {
  ...
  if (USB_IP0 == ip_type) {
    if (true == g_usb_is_opened[USB_IP0]) { return USB_ERR_BUSY; /* USB_ERR_OPENED */ }
    ...
    if (true == g_usb_is_opened[USB_IP1]) { return USB_ERR_BUSY; /* USB_ERR_OPENED */ }
...
usb_err_t usb_module_stop(uint8_t ip_type) {
  ...
  if (USB_IP0 == ip_type) {
    if (g_usb_is_opened[ip_type] == false) { return USB_ERR_NOT_OPEN; }
 ...
 * Return Value : none
 ************************************************************************************************************************/
void usb_cpu_usb_int_hand(uint32_t int_sense) // This function actually gets called as a CPU
...
void usb_cpu_delay_xms(uint16_t time) // Modified by Rohan
{
  ...
  do { routineForSD(); } while ((int16_t)(*TCNT[TIMER_SYSTEM_SLOW] - stopTime) < 0);
...
uint16_t usb_chattaring(volatile uint16_t* syssts) {
  ...
  while (1) {
    ...
    if ((lnst[0] == lnst[1]) && (lnst[0] == lnst[2])) { break; }
  }
  return lnst[0];
