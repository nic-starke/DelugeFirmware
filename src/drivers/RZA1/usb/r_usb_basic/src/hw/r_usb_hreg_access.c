...
void hw_usb_hset_rwupe(usb_utr_t* ptr, uint16_t port) {
  if (USB_PORT0 == port) { ptr->ipp->DVSTCTR0 |= USB_RWUPE; }
...
/***********************************************************************************************************************
 ...
 Description     : Clear the RWUPE-bit specified port's DVSTCTR0 reg-
                 : ister. When host. To prohibit detection of remote wake-up
 from : a USB Function. Arguments       : usb_utr_t    *ptr         : Pointer to
 usb_utr_t structure. : uint16_t     port         : Port number. Return value :
 none
 ***********************************************************************************************************************/
...
/***********************************************************************************************************************
 ...
 Description     : Set the RESUME-bit specified port's DVSTCTR0 register
                 : When host. To allow output of resume signal to a USB
 Function. Arguments       : usb_utr_t    *ptr         : Pointer to usb_utr_t
 structure. : uint16_t     port         : Port number. Return value    : none
 ***********************************************************************************************************************/
void hw_usb_hset_resume(usb_utr_t* ptr, uint16_t port) {
  if (USB_PORT0 == port) { ptr->ipp->DVSTCTR0 |= USB_RESUME; }
...
void hw_usb_hset_uact(usb_utr_t* ptr, uint16_t port) {
  if (USB_PORT0 == port) { ptr->ipp->DVSTCTR0 |= USB_UACT; }
...
/***********************************************************************************************************************
 ...
 Description     : Write the specified data to the specified port's INTENB
 register. Arguments       : usb_utr_t    *ptr         : Pointer to usb_utr_t
 structure. : uint16_t     port         : Port number. : uint16_t  data : The
 value to write. Return value    : none
 ***********************************************************************************************************************/
void hw_usb_hwrite_intenb(usb_utr_t* ptr, uint16_t port, uint16_t data) {
  if (USB_PORT0 == port) { ptr->ipp->INTENB1 = data; }
} /* End of function hw_usb_hwrite_intenb() */

/***********************************************************************************************************************
 Function Name   : hw_usb_hset_enb_ovrcre
 Description     : Set specified port's OVRCRE-bit (Overcurrent Input Change
 Int- : errupt Status Enable) in the INTENB1 register. Arguments       :
 usb_utr_t    *ptr         : Pointer to usb_utr_t structure. : uint16_t     port
 : Port number. Return value    : none
 ***********************************************************************************************************************/
...
/***********************************************************************************************************************
 ...
 register, : to prohibit VBUS interrupts. Arguments       : usb_utr_t    *ptr :
 Pointer to usb_utr_t structure. : uint16_t     port         : Port number.
 Return value    : none
 ***********************************************************************************************************************/
...
void hw_usb_hset_enb_bchge(usb_utr_t* ptr, uint16_t port) {
  if (USB_PORT0 == port) { ptr->ipp->INTENB1 |= USB_BCHGE; }
...
void hw_usb_hset_enb_dtche(usb_utr_t* ptr, uint16_t port) {
  if (USB_PORT0 == port) { ptr->ipp->INTENB1 |= USB_DTCHE; }
...
void hw_usb_hset_enb_attche(usb_utr_t* ptr, uint16_t port) {
  if (USB_PORT0 == port) { ptr->ipp->INTENB1 |= USB_ATTCHE; }
...
/***********************************************************************************************************************
 ...
 Return value    : none
 ***********************************************************************************************************************/
void hw_usb_hset_enb_pddetinte(usb_utr_t* ptr) {} /* End of function hw_usb_hset_enb_pddetinte() */
...
void hw_usb_hwrite_intsts(usb_utr_t* ptr, uint16_t port, uint16_t data) {
  if (USB_PORT0 == port) { ptr->ipp->INTSTS1 = data; }
...
void hw_usb_hclear_sts_bchg(usb_utr_t* ptr, uint16_t port) {
  if (USB_PORT0 == port) { ptr->ipp->INTSTS1 = (uint16_t)(~USB_BCHG & INTSTS1_MASK); }
...
/***********************************************************************************************************************
 ...
 Detec- : tion Interrupt Status". Arguments       : usb_utr_t    *ptr         :
 Pointer to usb_utr_t structure. : uint16_t     port         : Port number.
 Return value    : none
 ***********************************************************************************************************************/
void hw_usb_hclear_sts_dtch(usb_utr_t* ptr, uint16_t port) {
  if (USB_PORT0 == port) { ptr->ipp->INTSTS1 = (uint16_t)(~USB_DTCH & INTSTS1_MASK); }
...
/***********************************************************************************************************************
 ...
 Status". Arguments       : usb_utr_t    *ptr         : Pointer to usb_utr_t
 structure. : uint16_t     port         : Port number. Return value    : none
 ***********************************************************************************************************************/
void hw_usb_hclear_sts_attch(usb_utr_t* ptr, uint16_t port) {
  if (USB_PORT0 == port) { ptr->ipp->INTSTS1 = (uint16_t)(~USB_ATTCH & INTSTS1_MASK); }
...
/***********************************************************************************************************************
 ...
 Description     : Set te SUREQ-bit in the DCPCTR register
                 : (Set SETUP packet send when HostController function is
 selected) Arguments       : usb_utr_t    *ptr         : Pointer to usb_utr_t
...
/***********************************************************************************************************************
 ...
 address. Arguments       : usb_utr_t    *ptr         : Pointer to usb_utr_t
 structure. : uint16_t     devsel       ; USB device address value Return value
 : DEVADDx content
 ***********************************************************************************************************************/
...
/***********************************************************************************************************************
 ...
 Description     : Set the DEVADD register's USBSPD for the specified device
 address. Arguments       : usb_utr_t    *ptr         : Pointer to usb_utr_t
 structure. : uint16_t     devsel       : USB device address value : uint16_t
 data         : Setting value. Return value    : none
 ***********************************************************************************************************************/
...
/***********************************************************************************************************************
 ...
 Return value    : none
 ***********************************************************************************************************************/
void hw_usb_hset_dcpmode(usb_utr_t* ptr) {} /* End of function hw_usb_hset_dcpmode() */
...
void hw_usb_hmodule_init(usb_ctrl_t* p_ctrl) {
  ...
  if (USB_IP0 == p_ctrl->module) /* USB200 */
  {
    ...
    USB200.SYSCFG0 &= ~USB_DPRPU; // Added by Rohan. Otherwise, incorrectly thinks a device is
                                  // already connected if we'd previously initialized the
                                  // peripheral module
    ...
    switch (sts) {
      ...
      case USB_LS_JSTS:
        ...
        if (USB_FS_JSTS == sts) { USB200.SYSCFG0 |= USB_HSE; }
        ...
        if (USB_LSMODE == (USB200.DVSTCTR0 & USB_RHST)) { USB200.SOFCFG |= USB_TRNENSEL; }
  ...
  } else if (USB_IP1 == p_ctrl->module) /* USB201 */
  {
    ...
    switch (sts) {
      ...
      case USB_LS_JSTS:
        ...
        if (USB_FS_JSTS == sts) { USB201.SYSCFG0 |= USB_HSE; }
        ...
        if (USB_LSMODE == (USB201.DVSTCTR0 & USB_RHST)) { USB201.SOFCFG |= USB_TRNENSEL; }
