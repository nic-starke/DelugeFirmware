...
int usb_pstd_interrupt_handler(uint16_t* type, uint16_t* status) {
  ...
  *status = 0;
  ...
  if ((intsts0 & (USB_VBINT | USB_RESM | USB_SOFR | USB_DVST | USB_CTRT | USB_BEMP | USB_NRDY |
                  USB_BRDY)) == 0u) {
  ...
  // BEMP interrupt - is this buffer empty, as in we've finished sending a
  // transfer - I think? Rohan
  if (ists0 & USB_BEMP) {
  ...
  // NRDY interrupt. I think this gets generated each time the host requests
  // some data, but we just happen not to have any to send. Rohan
    ...
    // Ok, disregard everything else below - NRDY interrupts never actually need
    // dealing with - unless we have ISO endpoints, which MIDI never does. We'll
    // just clear the interrupt and get out. I've actually disabled NRDY
    // interrupts for my own code, but it seems many other parts of the USB
    // library enable them.
#if USB_CFG_USE_USBIP == USB_CFG_IP0
...
#endif        /* USB_CFG_USE_USBIP == USB_CFG_IP1 */
    return 1; // SOFR interrupts don't result in any action when processed.
              // Rohan
    *type = USB_INT_SOFR;
  }
...
/***********************************************************************************************************************
 ...
 R8A66597(ASSP). Arguments       : none Return value    : none
 ***********************************************************************************************************************/
