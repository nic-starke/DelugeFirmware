...
uint16_t g_usb_pstd_eptbl_index[2][USB_MAX_EP_NO + 1u]; /* Index of endpoint
                                                           information table */
...
/***********************************************************************************************************************
 ...
 Arguments       : uint16_t     type         : Interrupt type.
                 : uint16_t     status       : BRDYSTS register & BRDYENB
...
static void usb_pstd_interrupt(uint16_t type, uint16_t status) {
  ...
  switch (type) {
    ...
    // NRDY interrupts don't need processing (I filter them out during the
    // interrupt) unless we use ISO endpoints.
    /*
    case USB_INT_NRDY:
...
/***********************************************************************************************************************
 ...
 Return value    : uint16_t                  : Value used to select this
...
uint16_t usb_pstd_get_alternate_num(uint16_t con_num, uint16_t int_num) {
  ...
  length |=
      (uint16_t)((uint16_t)(*(uint8_t*)((uint32_t)g_usb_pstd_driver.p_configtbl + (uint16_t)3u))
                 << 8u);
...
uint16_t usb_pstd_chk_remote(void) {
  ...
  if (0 == g_usb_pstd_config_num) { return USB_FALSE; }
  ...
  if ((atr & USB_CF_RWUPON) == USB_CF_RWUPON) { return USB_TRUE; }
  return USB_FALSE;
  ...
  switch (pipe_number) {
    ...
    default:
      ...
      for (i = 0; USB_PDTBLEND != tbl[i]; i += USB_EPL) {
        /* Pipe number */
        pipe = (uint16_t)(tbl[i + 0] & USB_CURPIPE);
        if (pipe == pipe_number) { usb_cstd_pipe_init(USB_NULL, pipe, tbl, i); }
      }
...
uint16_t usb_pstd_chk_pipe_info(uint16_t speed, uint16_t* ep_tbl, uint8_t* descriptor) {
  ...
  if (USB_DT_ENDPOINT == descriptor[1]) {
    ...
    if ((descriptor[2] & USB_EP_DIRMASK) == USB_EP_OUT) {
      ...
      /* IN(send) */
      pipe_cfg |=
          (uint16_t)((uint16_t)(ep_tbl[1] & (USB_DBLBFIELD | USB_CNTMDFIELD)) | USB_DIR_P_IN);
      retval = USB_DIR_P_IN;
    }
...
/***********************************************************************************************************************
 ...
 Arguments       : uint16_t     pipe         : Pipe no.
                 : uint16_t     useport      : FIFO
 select(USB_CUSE,USB_DMA0,....) Return value    : none
 ***********************************************************************************************************************/
...
/***********************************************************************************************************************
 ...
 Description     : Force termination of data transfer of specified pipe. Request
                 : PCD to force termination data transfer, and the PCD
 forced-terminates data transfer. Arguments       : uint16_t     pipe         :
 Pipe number. Return value    : usb_er_t                  : Error info.
 ***********************************************************************************************************************/
...
/***********************************************************************************************************************
 ...
 Description     : Register pipe information table, descriptor information
 table, : callback function, etc. This info is specified by the data in : the
 structure usb_pcdreg_t. Arguments       : usb_pcdreg_t *registinfo  : Class
 driver structure. Return value    : none
 ***********************************************************************************************************************/
...
/***********************************************************************************************************************
 ...
 Description     : Clear the information registered in the structure
 usb_pcdreg_t. Arguments       : none Return value    : none
 ***********************************************************************************************************************/
...
/***********************************************************************************************************************
 ...
 Description     : Get USB Device information such as USB Device status and con-
                 : figuration No. etc.
 Arguments       : usb_utr_t    *ptr         : Pointer to usb_utr_t structure.
                 : uint16_t      *tbl        : Device information storage
pointer TBL. :                           : This pointer is used to provide the
caller with :                           : the device's status, speed,
configuration and :                           : interface number, and the value
of the remote wakeup flag. Return value    : none
***********************************************************************************************************************/
...
void usb_peri_registration(usb_ctrl_t* ctrl, usb_cfg_t* cfg) {
  ...
  pdriver.p_othertbl =
      cfg->p_usb_reg->p_config_h;                 /* Other configuration descriptor Table address */
...
uint16_t usb_peri_pipe_info(uint8_t* table, uint16_t speed, uint16_t length) {
  ...
  while (ofdsc < length) {
    ...
    if (USB_DT_ENDPOINT == table[ofdsc + USB_EP_B_DESCRIPTORTYPE]) {
      ...
      if (USB_ERROR == retval) { return USB_ERROR; }
      pipe_offset += USB_EPL;
    }
    ofdsc += table[ofdsc];
  }
...
void usb_peri_suspended(usb_utr_t* ptr, uint16_t data1, uint16_t data2) {
  ...
  // displayPopupIfAllBootedUp("DETACHED");    // Added by Rohan. Putting this
  // in the "detach" func didn't have the desired effect, maybe because Deluge's
  // VBUS stays on as the MCU sees it?
