...
// extern usb_utr_t        *g_p_usb_hstd_pipe[][USB_MAX_PIPE_NO + 1u];     /*
// Message pipe */ extern uint8_t          *g_p_usb_hstd_data[][USB_MAX_PIPE_NO
// + 1u];     /* PIPEn Buffer pointer(8bit) */ extern uint32_t
// g_usb_hstd_data_cnt[][USB_MAX_PIPE_NO + 1u];    /* PIPEn Buffer counter */
...
extern uint16_t
    g_usb_hstd_dcp_register[][USB_MAXDEVADDR + 1u]; /* DEVSEL & DCPMAXP (Multiple device) */
...
extern uint16_t
    g_usb_pstd_eptbl_index[2][USB_MAX_EP_NO + 1u]; /* Index of Endpoint Information table */
...
inline static void usb_cstd_chg_curpipe_rohan_fast(uint16_t pipe) {
  if ((fifoSels[USB_CUSE] & (USB_ISEL | USB_CURPIPE)) != pipe) {
    ...
    do {
      fifoSels[USB_CUSE] = buf; // This function only changed some of the bits,
                                // as specified by bitptn
      *p_reg = buf;
      //}
      buffer = *p_reg;
    } while ((buffer & (USB_ISEL | USB_CURPIPE)) !=
             pipe); // I've got it getting stuck here sometimes! Rohan
  }
}
...
/***********************************************************************************************************************
 ...
 clear it. Argument        : none Return          : flg                       :
 usb_scheduler_schedule_flag
 ***********************************************************************************************************************/
...
// void                usb_registration(usb_ctrl_t *ctrl, usb_cfg_t *cfg); //
// Commented out by Rohan
