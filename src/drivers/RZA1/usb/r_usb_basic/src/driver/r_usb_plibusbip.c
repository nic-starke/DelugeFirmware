...
/***********************************************************************************************************************
 ...
 Description     : Set USB registers as required to write from data buffer to
 USB : FIFO, to have USB FIFO to write data to bus. Arguments       : uint16_t
 pipe         : Pipe number. : uint16_t     useport      : Port number. Return
 value    : none
 ***********************************************************************************************************************/
void usb_pstd_buf2fifo(uint16_t pipe,
                       uint16_t useport) // pipe is never 0. Rohan
{
  uint16_t end_flag;
    ...
    case USB_CUSE:
      ...
      if ((uint32_t)0u != length) {
        ...
        if ((length % mxps) == (uint32_t)0u) {
          ...
          /* Set Transaction counter */
          usb_cstd_set_transaction_counter(USB_NULL,
                                           pipe,
...
uint16_t usb_read_data_fast_rohan(uint16_t pipe) {
  ...
  if (!numBytesReceived) {
    ...
    *p_reg = USB_BCLR;
    ...
    // If received more data than we asked for - that's an error basically. This
    // should really never happen with MIDI. The original code still went
    // through with reading the data in this case, but it's easier for us not
    // to, and what use would it do. It really should never happen, it'd be a
    // weird thing to see.
    if (g_usb_data_cnt[pipe] < numBytesReceived) return USB_READOVER;

    // MIDI will always be in multiples of 4 bytes, so can simplify things a
    // little
    uint8_t* __restrict__ readPos = g_p_usb_data[pipe];
    const uint8_t* const stopAt   = g_p_usb_data[pipe] + numBytesReceived;

    do { // We know there's at least one byte to read - we checked for
         // !numBytesReceived above
      /* 32bit FIFO access */
  ...
  return USB_READEND; // It might also have been a READSHRT, but the distinction
...
/***********************************************************************************************************************
 ...
 Description     : Request to read data from USB FIFO, and manage the size of
                 : the data read.
 Arguments       : uint16_t     pipe         : Pipe number.
                 : uint16_t     pipemode     : FIFO
 select(USB_CUSE,USB_DMA0,....) Return value    : uint16_t     end_flag
 ***********************************************************************************************************************/

// I now call this instead of the HSTD equivalent, everywhere. It does the same
// thing - after slight modification.
uint16_t usb_pstd_read_data(uint16_t pipe, uint16_t pipemode) {
  ...
  if (g_usb_data_cnt[pipe] < dtln) {
    ...
    if ((g_usb_usbmode == USB_PERI) && (USB_PIPE0 == pipe) &&
        ((dtln % mxps) == 0)) // Now that I cheekily use this function for both host and
                              // peripheral, we have to do a check for USB_PERI in this one place
                              // where it matters. Otherwise, Targus USB 1.1 hub doesn't work.
    {
      /* Just Receive Size */
      /* Peripheral Function */
...
/***********************************************************************************************************************
 ...
 transmission/recep- : tion is complete. Arguments       : uint16_t     pipe :
 Pipe number. : uint16_t     status       : Transfer status type. Return value
 : none
 ***********************************************************************************************************************/
void usb_pstd_data_end(uint16_t pipe, uint16_t status) {
  ...
  if (USB_NULL != g_p_usb_pipe[pipe]) {
    /* Check PIPE TYPE */
    if (usb_cstd_get_pipe_type(USB_NULL, pipe) !=
      ...
      // usb_pstd_fifo_to_buf(pipe, USB_CUSE);
      {
        ...
        if (end_flag == USB_READEND) { // I condensed USB_READSHRT into this too
          // usb_pstd_data_end(pipe, newStatus);
          {
            ...
            if (true || USB_NULL != g_p_usb_pipe[pipe]) {
              /* Check PIPE TYPE */
              if (true || usb_cstd_get_pipe_type(USB_NULL, pipe) !=
                              USB_TYPFIELD_ISO) // Corrected by Rohan from USB_ISO
              {
                /* Transfer information set */
                ...
                g_p_usb_pstd_pipe[pipe]->pipectr    =
                hw_usb_read_pipectr(USB_NULL,pipe);
                g_p_usb_pstd_pipe[pipe]->keyword    = pipe;
                */
                //((usb_cb_t)g_p_usb_pstd_pipe[pipe]->complete)(g_p_usb_pstd_pipe[pipe],
                // USB_NULL, USB_NULL); usbReceiveComplete(0, 0,
                // g_usb_pstd_data_cnt[pipe]);
                g_p_usb_pipe[pipe] =
                    (usb_utr_t*)USB_NULL; // Is this necessary? Doesn't look like it

                // I've just pasted the relevant contents of
                // usbReceiveComplete() in here
                connectedUSBMIDIDevices[0][0].numBytesReceived =
                    64 - g_usb_data_cnt[pipe]; // Seems wack, but yet, tranlen is now
                                               // how many bytes didn't get received
                                               // out of the original transfer size
                // Warning - sometimes (with a Teensy, e.g. my knob box), length
                // will be 0. Not sure why - but we need to cope with that case.
    ...
    if ((bitsts & USB_BITSET(i)) != 0u) {
      ...
      if (USB_NULL != g_p_usb_pipe[i]) {
        ...
        if ((USB_D0DMA == useport) || (USB_D1DMA == useport)) {
          ...
          set_dtc_brock_cnt =
              (uint16_t)((g_usb_data_cnt[g_usb_cstd_dma_pipe[USB_CFG_USE_USBIP][dma_ch]] - 1) /
...
/***********************************************************************************************************************
 ...
 Arguments       : uint16_t     bitsts       : NRDYSTS Register & NRDYENB
 Register Return value    : none
 ***********************************************************************************************************************/
void usb_pstd_nrdy_pipe_process(uint16_t bitsts) {
  ...
  for (i = USB_MIN_PIPE_NO; i <= USB_MAX_PIPE_NO; i++) {
    if ((bitsts & USB_BITSET(i)) != 0) {
      /* Interrupt check */
      if (USB_NULL != g_p_usb_pipe[i]) {
        if (usb_cstd_get_pipe_type(USB_NULL, i) ==
            USB_TYPFIELD_ISO) // Corrected by Rohan from USB_ISO
        {
          /* Wait for About 60ns */
...
// Heavily chopped down function by Rohan, from the one below it. We already
// know the only pipe it could be is USB_CFG_PMIDI_BULK_OUT
void usb_pstd_bemp_pipe_process_rohan_midi(uint16_t bitsts) {
  ...
  {
    /* Interrupt check */
    if (true || USB_NULL != g_p_usb_pipe[pipe]) // Don't bother with this check now we
                                                // don't actually need to dereference
                                                // this pointer. Rohan
    {
      uint16_t pid;
      ...
      if (pipeCtrRead & USB_PID_STALL) {
        ...
        if (true || !(pipeCtrRead & USB_INBUFM)) // Ensure "there is no transmissible data in
                                                 // the buffer memory". Well we shouldn't be
                                                 // here if there still was.
        {
          // usb_pstd_data_end(pipe, (uint16_t)USB_DATA_NONE);
          {
            ...
            /*
                                    uint16_t startTime =
               *TCNT[TIMER_SYSTEM_SUPERFAST]; while (true) { uint16_t stopTime =
               *TCNT[TIMER_SYSTEM_SUPERFAST]; uint16_t duration = stopTime -
               startTime; if (duration >= 60) break; // 10 seems fine with Og
               and no LTO. 40 one time was not fine with LTO - 60 seems fine
                                    }
            */

            // Turns out we just didn't need any of this stuff! And getting rid
            // of it stopped the i029 errors!!
#if 0
                        /* Disable Interrupt */
                        /* Disable Ready Interrupt */
                        hw_usb_clear_brdyenb(USB_NULL,pipe);
...
/***********************************************************************************************************************
 ...
 complete data transmission for the PIPE Arguments       : uint16_t     bitsts
 : BEMPSTS Register & BEMPENB Register Return value    : none
 ***********************************************************************************************************************/
