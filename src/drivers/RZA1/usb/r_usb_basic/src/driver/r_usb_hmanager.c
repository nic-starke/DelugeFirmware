 ...
 ***********************************************************************************************************************/
/* Enumeration Table */
extern void (*g_usb_hstd_enumaration_process[8])(usb_utr_t*,
                                                 uint16_t,
                                                 uint16_t); /* Enumeration Table */
...
static void usb_hstd_mgr_rel_mpl(usb_utr_t* ptr, uint16_t n) {
  ...
  if (USB_OK != err) { USB_PRINTF1("### USB MGR rel_blk error: %d\n", n); }
...
static uint16_t usb_hstd_enumeration(usb_utr_t* ptr) {
  ...
  switch (p_usb_shstd_mgr_msg[ptr->ip]->result) {
            ...
            case USB_OK:
...
#if USB_CFG_COMPLIANCE == USB_CFG_ENABLE
              if (USB_ON == g_usb_disp_param_set) { usb_compliance_disp((void*)&g_usb_disp_param); }
#endif /* USB_CFG_COMPLIANCE == USB_CFG_ENABLE */
              ...
              if (1 != flg) {
                ...
                displayPopupIfAllBootedUp(
                    "UNKN"); // By Rohan. Means couldn't find an available "driver"
                             // for this device. It could be a 2nd hub.
#endif
          ...
          if (usb_hstd_enum_function2(&enume_mode) == USB_TRUE) {
            ...
            for (md = 0; md < g_usb_hstd_device_num[ptr->ip]; md++) {
              ...
              if (g_usb_hstd_device_addr[ptr->ip] == driver->devaddr) {
                /* Device state */
                g_usb_hstd_device_info[ptr->ip][g_usb_hstd_device_addr[ptr->ip]][1] =
                    USB_CONFIGURED;
                ...
                (*driver->devconfig)(ptr,
                                     g_usb_hstd_device_addr[ptr->ip],
                                     (uint16_t)USB_NO_ARG); /* Call Back */
      ...
      if (USB_DEVICEENUMERATION == enume_mode) {
        switch (g_usb_hstd_enum_seq[ptr->ip]) {
          ...
          case 6:
            ...
            /* Device state */
            g_usb_hstd_device_info[ptr->ip][g_usb_hstd_device_addr[ptr->ip]][2] =
                descriptor_table[5];
    ...
    case USB_DATA_STALL:
      ...
      /* Device enumeration function */
      usb_hstd_enum_function4(&g_usb_hstd_enum_seq[ptr->ip],
                              &enume_mode,
                              g_usb_hstd_device_addr[ptr->ip]);
...
static uint16_t usb_hstd_chk_device_class(usb_utr_t* ptr, usb_hcdreg_t* driver, uint16_t port) {
  ...
  for (i = 0; i < driver->p_tpl[0]; i++) {
    ...
    if ((driver->p_tpl[(i * 2) + 2] == USB_NOVENDOR) || (driver->p_tpl[(i * 2) + 2] == vendor_id)) {
      ...
      if ((driver->p_tpl[(i * 2) + 3] == USB_NOPRODUCT) ||
          (driver->p_tpl[(i * 2) + 3] == product_id)) {
        id_check = USB_OK;
#if USB_CFG_COMPLIANCE == USB_CFG_ENABLE
  ...
  if (id_check == USB_ERROR) {
    ...
    //        ctrl.address    = g_usb_hstd_device_addr[ptr->ip];  /* USB Device
    //        address */ ctrl.module     = ptr->ip;                          /*
    //        Module number setting */ usb_set_event(USB_STS_NOT_SUPPORT,
    //        &ctrl);          /* Set Event()  */
  ...
  if (total_length2 > USB_CONFIGSIZE) { total_length2 = USB_CONFIGSIZE; }
  ...
  while (total_length1 < total_length2) {
    ...
    if (0 == descriptor_table[total_length1]) { break; }
  }
  return USB_ERROR;
...
void usb_hstd_enum_get_descriptor(usb_utr_t* ptr, uint16_t addr, uint16_t cnt_value) {
  ...
  switch (cnt_value) {
    ...
    case 5:
      ...
      usb_shstd_std_request[ptr->ip][0] =
    ...
    case 4:
      ...
      usb_shstd_std_request[ptr->ip][3] =
          (uint16_t)(((uint16_t)data_table[3] << 8) + (uint16_t)data_table[2]);
...
void usb_hstd_enum_set_address(usb_utr_t* ptr, uint16_t addr, uint16_t setaddr) {
  usb_shstd_std_request[ptr->ip][0] =
      (USB_SET_ADDRESS | USB_HOST_TO_DEV | USB_STANDARD | USB_DEVICE);
...
/***********************************************************************************************************************
 ...
 Description     : Setup a call to the function usb_hstd_change_device_state to
 re- : quest the connected USB Device to change status. Arguments       :
 usb_utr_t    *ptr         : Pointer to usb_utr_t structure. : uint16_t devaddr
 : Device Address : uint16_t     msginfo      : Request type. Return          :
 none
 ***********************************************************************************************************************/
...
/***********************************************************************************************************************
 ...
 Description     : Setup a call to the function usb_hstd_change_device_state to
 re- : quest the connected USB Device to change status. Arguments       :
 usb_utr_t    *ptr         : Pointer to usb_utr_t structure. : usb_cb_t complete
 : Callback function Pointer : uint16_t     devaddr      : Device address :
 uint16_t     msginfo      : Request type for HCD : uint16_t     mgr_msginfo  :
 Request type for MGR Return          : none
 ***********************************************************************************************************************/
 ...
 ***********************************************************************************************************************/
static uint16_t usb_hstd_chk_remote(usb_utr_t* ptr) {
  if ((g_usb_hstd_class_data[ptr->ip][7] & USB_CF_RWUPON) != (uint8_t)0) { return USB_TRUE; }
  return USB_FALSE;
...
uint16_t usb_hstd_set_feature(usb_utr_t* ptr, uint16_t addr, uint16_t epnum, usb_cb_t complete) {
  if (0xFF == epnum) {
    /* SetFeature(Device) */
    g_usb_hstd_class_request[ptr->ip][0] =
        USB_SET_FEATURE | USB_HOST_TO_DEV | USB_STANDARD | USB_DEVICE;
    g_usb_hstd_class_request[ptr->ip][1] = USB_DEV_REMOTE_WAKEUP;
    g_usb_hstd_class_request[ptr->ip][2] = (uint16_t)0x0000;
  } else {
    /* SetFeature(endpoint) */
    g_usb_hstd_class_request[ptr->ip][0] =
        USB_SET_FEATURE | USB_HOST_TO_DEV | USB_STANDARD | USB_ENDPOINT;
 ...
 ***********************************************************************************************************************/
uint16_t
usb_hstd_get_config_desc(usb_utr_t* ptr, uint16_t addr, uint16_t length, usb_cb_t complete) {
  uint16_t i;
 ...
 ***********************************************************************************************************************/
uint16_t
usb_hstd_get_string_desc(usb_utr_t* ptr, uint16_t addr, uint16_t string, usb_cb_t complete) {
  uint16_t i;
  ...
  if (0 == string) {
    ...
    g_usb_hstd_class_request[ptr->ip][2] |=
        (uint16_t)((uint16_t)(g_usb_hstd_class_data[ptr->ip][3]) << 8);
    g_usb_hstd_class_request[ptr->ip][3] = (uint16_t)CLSDATASIZE;
  }
  g_usb_hstd_class_request[ptr->ip][0] =
      USB_GET_DESCRIPTOR | USB_DEV_TO_HOST | USB_STANDARD | USB_DEVICE;
...
void usb_hstd_electrical_test_mode(usb_utr_t* ptr, uint16_t product_id, uint16_t port) {
  ...
  switch (product_id) {
    ...
    case 0x0108: /* SINGLE_STEP_GET_DEV_DESC_DATA */
      ...
      do { brdysts = hw_usb_read_brdysts(ptr); } while (!(brdysts & USB_BRDY0));
      usb_cstd_set_nak(ptr, USB_PIPE0);
