...
/**********************************************************************************************************************
 ...
 pipe. Arguments       : usb_utr_t    *ptr         : Pointer to usb_utr_t
 structure. : uint16_t     pipe         : Pipe number. Return value    : none
 *********************************************************************************************************************/
...
void usb_set_event(uint16_t event, usb_ctrl_t* ctrl) {
  ...
  if (g_usb_cstd_event.write_pointer >= USB_EVENT_MAX) { g_usb_cstd_event.write_pointer = 0; }
} /* End of function usb_pstd_DriverRelease() */
...
void usb_cstd_usb_task(void) {
  if (USB_HOST == g_usb_usbmode) {
    ...
    doMoreIfAnyMore : {}
      ...
      if (USB_FLGSET == usb_cstd_check_schedule()) /* Check for any task processing requests
                                                      flags. */
      {
        ...
        */
        goto doMoreIfAnyMore; // By Rohan. Want to process everything
                              // immediately - especially cos of that bug which
                              // means all data reads must be done before any
                              // data writes. (Wait, does that still exist?)
      }
#if defined(USB_CFG_HMSC_USE)
...
void usb_class_task(void) {
  ...
  if (USB_FALSE == g_drive_search_lock) {
    if (g_drive_search_que_cnt > 0) {
      ...
      utr.ip = USB_IP0;
      if ((g_drive_search_lock & USB_IP_MASK) == USBA_ADDRESS_OFFSET) { utr.ip = USB_IP1; }
