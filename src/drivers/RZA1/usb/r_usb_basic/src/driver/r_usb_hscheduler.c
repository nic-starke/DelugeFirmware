...
/* Schedule Set Flag  */
uint8_t usb_scheduler_schedule_flag; // Whether something's scheduled to happen
                                     // right now
...
usb_msg_t* p_usb_scheduler_add_use; // This seems to be the pointer to the
                                    // actual message that's been scheduled.
uint8_t usb_scheduler_id_use;       // Ok, this is the ID (e.g. USB_HCD_MBX) of what
                                    // message the scheduler has decided we should use.
...
usb_er_t usb_cstd_isnd_msg(uint8_t id, usb_msg_t* mess) {
  ...
  if (id < USB_IDMAX) {
    ...
    if (usb_pri < USB_PRIMAX) {
      ...
      usb_write++;
      if (usb_write >= USB_TABLEMAX) { usb_write = USB_TBLCLR; }

      /* Check pointer */
      if (usb_write == usb_scheduler_pri_r[usb_pri]) { return USB_ERROR; }
    ...
    for (i = 0; i < USB_WAIT_EVENT_MAX; i++) {
      if (0 != usb_scheduler_wait_counter[id][i]) {
        ...
        if (0 == usb_scheduler_wait_counter[id][i]) {
          ...
          if (USB_OK != err) { usb_scheduler_wait_counter[id][i]++; }
        }
      }
    }
  }
...
void usb_cstd_sche_init(void) {
  ...
  for (i = 0; USB_PRIMAX != i; i++) {
    ...
    for (j = 0; USB_TABLEMAX != j; j++) { usb_scheduler_table_id[i][j] = USB_IDMAX; }
  }

  /* Initialize block table */
  for (i = 0; USB_BLKMAX != i; i++) { usb_scheduler_blk_flg[i] = USB_NULL; }
...
void usb_cstd_scheduler(void) {
  ...
  /* wait msg */
  usb_cstd_wait_scheduler(); // I think this is some horribly inefficient way of
                             // delaying messages that are meant to have a delay
                             // time. Rohan

  /* Priority Table reading */
  usb_pri = USB_CNTCLR;
  while (usb_pri < USB_PRIMAX) // I think this is cycling through all the priorities, from
                               // highest priority, til we find a message to action.
  {
    ...
    if (usb_read != usb_scheduler_pri_w[usb_pri]) {
      ...
      usb_read++;
      if (usb_read >= USB_TABLEMAX) { usb_read = USB_TBLCLR; }
