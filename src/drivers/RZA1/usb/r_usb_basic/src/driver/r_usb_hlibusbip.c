...
void usb_hstd_set_dev_addr(usb_utr_t* ptr, uint16_t addr, uint16_t speed, uint16_t port) {
  if (USB_DEVICE_0 == addr) { hw_usb_write_dcpmxps(ptr, (uint16_t)(USB_DEFPACKET + USB_DEVICE_0)); }
...
#if ((USB_CFG_DTC == USB_CFG_ENABLE) || (USB_CFG_DMA == USB_CFG_ENABLE))
  if (USB_PIPE1 == pipe) { fifo_mode = USB_D0DMA; }
  if (USB_PIPE2 == pipe) { fifo_mode = USB_D1DMA; }
#endif /* ((USB_CFG_DTC == USB_CFG_ENABLE) || (USB_CFG_DMA == USB_CFG_ENABLE)) \
        */
...
void usb_receive_start_rohan_midi(uint16_t pipe) {
  ...
  /* Ignore count clear */
  g_usb_hstd_ignore_cnt[USB_CFG_USE_USBIP][pipe] = (uint16_t)0u; // Was only included in host code
                                                                 // originally - not peripheral
  ...
  // Corner cut here vs original code - it'll always be less than 1 packet, just
  // go.
  ...
  hw_usb_set_pid_nonzero_pipe_rohan(pipe, USB_PID_BUF); // I guess this is how it says "request me
                                                        // an incoming data transfer"?
  ...
  // Commenting that out seems to work fine now, I've tested and that's good,
  // but previous comment was:
  //  "Generally not needed and removing this works fine, except one case -
  //  after reconnecting a device to a hub (the Targus USB2 one at least), MIDI
  //  won't get to us, every 2nd reconnect!"
}
...
/***********************************************************************************************************************
 ...
 Description     : Set USB registers as appropriate after data transmission/re-
                 : ception, and call the callback function as
 transmission/recep- : tion is complete. Arguments       : usb_utr_t    *ptr :
 Pointer to usb_utr_t structure. : uint16_t     pipe         : Pipe number. :
 uint16_t     status       : Transfer status type. Return value    : none
 ***********************************************************************************************************************/
void usb_hstd_data_end(usb_utr_t* ptr, uint16_t pipe, uint16_t status) {
  ...
  if (USB_NULL != g_p_usb_pipe[pipe]) {
    ...
    if (USB_TYPFIELD_ISO != usb_cstd_get_pipe_type(ptr, pipe)) {
      ...
      // g_usb_hstd_ignore_cnt[ip][pipe];
      g_p_usb_pipe[pipe]->ipp = usb_hstd_get_usb_ip_adr(ip);
      g_p_usb_pipe[pipe]->ip  = ip;

      usb_utr_t* temp    = g_p_usb_pipe[pipe];
      g_p_usb_pipe[pipe] = (usb_utr_t*)USB_NULL; // Moved by Rohan - set this to NULL before the
                                                 // callback so the callback can do another
                                                 // transfer, which sets this to something else
      (temp->complete)(temp, 0,
                       0); // This does our callback on our outgoing transfers. Rohan
    } else {
      /* Transfer information set */
      g_p_usb_pipe[pipe]->tranlen = g_usb_data_cnt[pipe];
      g_p_usb_pipe[pipe]->pipectr = hw_usb_read_pipectr(ptr, pipe);
...
void usb_hstd_brdy_pipe_process_rohan_midi_and_hub(usb_utr_t* ptr, uint16_t bitsts) {
  ...
  // ip = ptr->ip;
  for (pipe = USB_CFG_HMIDI_BULK_RECV_MIN; pipe <= USB_PIPE9;
       pipe++) { // We'll just do the receive pipes we actually sometimes use
                 // for MIDI, plus PIPE9 for hubs. We could also do it by USB
                 // device that we know is connected...
    ...
    if (bitsts & USB_BITSET(pipe)) {
      ...
      if (true || USB_NULL != g_p_usb_pipe[pipe]) { // I think this'll be fine now
        ...
        // usb_hstd_fifo_to_buf(ptr, pipe, useport);
        {
          ...
          end_flag = usb_read_data_fast_rohan(pipe); // Modified by Rohan to call the PSTD one,
                                                     // which does the exact same now

          if (end_flag == USB_READEND) { // I condensed USB_READSHRT into this too
#if 1
            if (pipe == USB_PIPE9) { // For hub - use original function. Without this,
                                     // reconnecting devices to hub doesn't work, even
                                     // though they do if initially connected.
            ...
            { // For MIDI - use this optimized code
              ...
              if (true || USB_NULL != g_p_usb_pipe[pipe]) // Checked above already
              {
                ...
                if (true || USB_TYPFIELD_ISO != usb_cstd_get_pipe_type(NULL, pipe)) {
                  ...
                  g_p_usb_pipe[pipe]->pipectr    = hw_usb_read_pipectr(NULL,
                  pipe); g_p_usb_pipe[pipe]->errcnt     = (uint8_t)
                  g_usb_hstd_ignore_cnt[USB_CFG_USE_USBIP][pipe];
                  g_p_usb_pipe[pipe]->ipp        =
                  usb_hstd_get_usb_ip_adr(USB_CFG_USE_USBIP);
                  g_p_usb_pipe[pipe]->ip         = USB_CFG_USE_USBIP;
                  */
                  usb_utr_t* temp = g_p_usb_pipe[pipe];
                  // g_p_usb_pipe[pipe] = (usb_utr_t*) USB_NULL; // Moved by
                  // Rohan - set this to NULL before the callback so the
                  // callback can do another transfer, which sets this to
                  // something else (temp->complete)(temp, 0, 0); // This does
                  // our callback on our outgoing transfers. Rohan

                  unsigned int deviceNum = temp - &g_usb_midi_recv_utr[0][0];

                  if (deviceNum < MAX_NUM_USB_MIDI_DEVICES) { // Gotta check this - I can
                                                              // totally see something going
                                                              // wrong, with all the other
                                                              // checks I'm now skipping!
                    connectedUSBMIDIDevices[0][deviceNum].numBytesReceived =
                        64 - g_usb_data_cnt[pipe];
                    // Warning - sometimes (with a Teensy, e.g. my knob box),
                    // length will be 0. Not sure why - but we need to cope with
                    // that case.
...
/***********************************************************************************************************************
 ...
 Arguments       : usb_utr_t    *ptr         : Pointer to usb_utr_t structure.
                 : uint16_t     bitsts       : BRDYSTS Register & BRDYENB
    ...
    if ((bitsts & USB_BITSET(i)) != 0) {
      ...
      if (USB_NULL != g_p_usb_pipe[i]) {
        ...
        if ((USB_D0DMA == useport) || (USB_D1DMA == useport)) {
          ...
          set_dtc_brock_cnt =
              (uint16_t)((g_usb_hstd_data_cnt[ip][g_usb_cstd_dma_pipe[ip][dma_ch]] - 1) /
...
/***********************************************************************************************************************
 ...
 Arguments       : usb_utr_t    *ptr         : Pointer to usb_utr_t structure.
                 : uint16_t     bitsts       : NRDYSTS Register & NRDYENB
 Register Return value    : none
 ***********************************************************************************************************************/
...
void usb_hstd_bemp_pipe_process_rohan_midi(usb_utr_t* ptr, uint16_t bitsts) {
  ...
  // We only have one SEND pipe for BULK endpoints (standard for MIDI), and one
  // for INTERRUPT endpoints (which our dumb Actition pedal uses). It's not
  // necessary to check PIPE9 for the hub - I guess we must only receive, not
  // send, on that pipe.
    ...
    if (true || USB_NULL != g_p_usb_pipe[pipe]) // No real harm if this somehow was NULL
    {
      ...
      if (buffer & USB_PID_STALL) // Probably couldn't happen...
      {
        ...
        if (pipe == USB_CFG_HMIDI_INT_SEND ||
            !(buffer & USB_INBUFM)) // For some reason, the original code only
                                    // needs this check for BULK, not INTERRUPT
        {
          /* End of data transfer */
          // usb_hstd_data_end(ptr, pipe, (uint16_t) USB_DATA_NONE);
          {

            // This stuff not needed after all. We were getting occasional i029
            // when disconnecting / reconnecting many devices with hub, even
            // before this change, and still now.
#if 0
                        /* PID = NAK */
                        /* Set NAK */
                        //usb_cstd_select_nak(ptr, pipe);
            ...
            if (true || USB_NULL != g_p_usb_pipe[pipe]) // Already checked above
            {
              ...
              if (true || USB_TYPFIELD_ISO != usb_cstd_get_pipe_type(ptr, pipe)) {
                ...
                */
                ...
                usb_utr_t* temp = g_p_usb_pipe[pipe];
                g_p_usb_pipe[pipe] =
                    (usb_utr_t*)USB_NULL; // Moved by Rohan - set this to NULL before the
                                          // callback so the callback can do another
                                          // transfer, which sets this to something else
                //(temp->complete)(temp, 0, 0); // This does our callback on our
...
/***********************************************************************************************************************
 ...
 complete data transmission for the PIPE Arguments       : usb_utr_t    *ptr :
 Pointer to usb_utr_t structure. : uint16_t     bitsts       : BEMPSTS Register
 & BEMPENB Register Return value    : none
 ***********************************************************************************************************************/
