...
void usb_hstd_brdy_pipe(usb_utr_t* ptr) {
  ...
  /* only one BRDY message is issued even when the demand of PIPE0 and PIPEx has
   * been generated at the same time. */
  if (true || (bitsts & USB_BRDY0) == USB_BRDY0) // Changed by Rohan - now we only come
                                                 // to this function for pipe0
  {
    ...
    switch (g_usb_hstd_ctsq[ptr->ip]) {
      ...
      case USB_DATARD:
        ...
        switch (usb_pstd_read_data(USB_PIPE0, USB_CUSE)) // Modified by Rohan to call the PSTD
...
void usb_hstd_nrdy_pipe(usb_utr_t* ptr) {
  ...
  /* When operating by the host function, usb_hstd_nrdy_pipe() is executed
   * without fail because */
  /* only one NRDY message is issued even when the demand of PIPE0 and PIPEx has
   * been generated at the same time. */
...
void usb_hstd_bemp_pipe(usb_utr_t* ptr) {
  ...
  /* When operating by the host function, usb_hstd_bemp_pipe() is executed
   * without fail because */
  /* only one BEMP message is issued even when the demand of PIPE0 and PIPEx has
   * been generated at the same time. */
  if (true ||
      (bitsts & USB_BEMP0) == USB_BEMP0) // Modified by Rohan - now we only come here for BEMP0
  {
        ...
        case USB_DATAWR:
          ...
          switch (usb_hstd_write_data(ptr, USB_PIPE0, USB_CUSE)) {
            ...
            case USB_WRITESHRT:
              ...
              g_usb_hstd_ctsq[ptr->ip] =
                  USB_STATUSWR; /* Next stage is Control write status stage */
              hw_usb_set_bempenb(ptr, (uint16_t)USB_PIPE0); /* Enable Empty Interrupt */

              // usb_cstd_nrdy_enable(ptr, (uint16_t)USB_PIPE0); /* Enable Not Ready
              // Interrupt */
              //  We ignore NRDY interrupts anyway, as there are tons of them
              //  continuously, and enabling them at all was causing freezes (or was
              //  it UART / SD lockups?), right since we first added this "new"
              //  (2016) USB driver (in 2019).
