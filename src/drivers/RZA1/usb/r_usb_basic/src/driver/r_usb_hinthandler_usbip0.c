...
/***********************************************************************************************************************
 ...
 Return value    : none
 ***********************************************************************************************************************/
void usb_hstd_usb_handler(uint32_t sense) // This function actually gets called as part of a CPU
                                          // interrupt, for real! Rohan
{
  usb_utr_t* ptr;
  usb_er_t   err;
  ...
  */
  ...
  if (USB_OK != err) { USB_PRINTF1("### lib_UsbHandler DEF2 isnd_msg error (%ld)\n", err); }

  /* Renewal Message count  */
  g_usb_cstd_int_msg_t_cnt[0]++;
  if (USB_INTMSGMAX == g_usb_cstd_int_msg_t_cnt[0]) { g_usb_cstd_int_msg_t_cnt[0] = 0; }
...
/***********************************************************************************************************************
 ...
 Return value    : none
 ***********************************************************************************************************************/
void usb_hstd_dma_handler(void) {} /* End of function usb_hstd_dma_handler() */

/***********************************************************************************************************************
 Function Name   : usb_hstd_init_usb_message
 Description     : Initialization of message to be used at time of USB
 interrupt. Arguments       : usb_utr_t    *ptr         : Pointer to usb_utr_t
...
void usb_hstd_init_usb_message(usb_utr_t* ptr) {
  ...
  for (i = 0; i < USB_INTMSGMAX; i++) { g_usb_cstd_int_msg_t[ptr->ip][i].msginfo = msg_info; }
