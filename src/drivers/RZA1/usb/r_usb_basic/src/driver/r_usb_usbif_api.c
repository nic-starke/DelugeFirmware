 ...
 * Description  : USB Host and Peripheral Driver API code. HCD(Host Control
 *Driver) PCD (Peripheral Control Driver)
 ***********************************************************************************************************************/
/**********************************************************************************************************************
...
uint16_t R_USB_GetEvent(usb_ctrl_t* p_ctrl) {
  ...
  if (g_usb_cstd_event.write_pointer != g_usb_cstd_event.read_pointer) {
    *p_ctrl = g_usb_cstd_event.ctrl[g_usb_cstd_event.read_pointer];
    ...
    if (g_usb_cstd_event.read_pointer >= USB_EVENT_MAX) { g_usb_cstd_event.read_pointer = 0; }
  }
  return event;
...
usb_err_t R_USB_Open(usb_ctrl_t* ctrl, usb_cfg_t* cfg) {
  ...
  /* Argument Checking */
  if ((USB_IP0 != ctrl->module) && (USB_IP1 != ctrl->module)) { return USB_ERR_PARA; }
  ...
  switch (ctrl->type) {
    ...
    case USB_PMSC:
      ...
      if (USB_PERI != cfg->usb_mode) { return USB_ERR_PARA; }
    ...
    case USB_HMSC:
      ...
      if (USB_HOST != cfg->usb_mode) { return USB_ERR_PARA; }
  ...
  if (USB_HOST == g_usb_usbmode) {
...
#if ((USB_CFG_MODE & USB_CFG_PERI) == USB_CFG_PERI)
#if USB_CFG_PARAM_CHECKING == USB_CFG_ENABLE
    if ((USB_LS == cfg->usb_speed) || (USB_NULL == cfg->p_usb_reg)) { return USB_ERR_PARA; }
    ...
    if (USB_SUCCESS == err) {
      ...
      if (USB_HS == cfg->usb_speed) { hw_usb_set_hse(USB_NULL, USB_NULL); }
...
#if USB_CFG_PARAM_CHECKING == USB_CFG_ENABLE
  if (USB_NULL == p_ctrl) { return USB_ERR_PARA; }

  /* Argument Checking */
  if ((USB_IP0 != p_ctrl->module) && (USB_IP1 != p_ctrl->module)) { return USB_ERR_PARA; }
#endif /* USB_CFG_PARAM_CHECKING == USB_CFG_ENABLE */
...
#if ((USB_CFG_MODE & USB_CFG_PERI) == USB_CFG_PERI)
      usb_pstd_driver_release(); /* Clear the information registered in the
...
/***********************************************************************************************************************
 ...
 areatransfer data address : uint32_t     size         : Read size Return value
 : usb_err_t    error code   : USB_SUCCESS,USB_ERR_NG etc.
 ***********************************************************************************************************************/
usb_err_t R_USB_Read(usb_ctrl_t* p_ctrl, uint8_t* buf, uint32_t size) {
...
#if USB_CFG_PARAM_CHECKING == USB_CFG_ENABLE
  if ((USB_NULL == p_ctrl) || (USB_NULL == buf) || (USB_NULL == size)) { return USB_ERR_PARA; }
  ...
  if ((USB_SUCCESS != err) || (USB_STS_CONFIGURED != info.status)) { return USB_ERR_NG; }
...
/***********************************************************************************************************************
 ...
 Arguments       : usb_ctrl_t   *p_ctrl      : Pointer to usb_ctrl_t structure.
                 : uint8_t      *buf         : Pointer to the data buffer
 areatransfer data address : uint32_t     size         : Write size Return value
 : usb_err_t    error code   : USB_SUCCESS,USB_ERR_NG etc.
 ***********************************************************************************************************************/
...
#if USB_CFG_PARAM_CHECKING == USB_CFG_ENABLE
  if ((USB_NULL == p_ctrl) || ((USB_NULL == buf) && (0 != size))) { return USB_ERR_PARA; }
...
usb_err_t R_USB_Stop(usb_ctrl_t* p_ctrl, uint16_t type) {
  ...
  if ((USB_WRITE != type) && (USB_READ != type)) { return USB_ERR_PARA; }

  /* Argument Checking */
  if ((USB_IP0 != p_ctrl->module) && (USB_IP1 != p_ctrl->module)) { return USB_ERR_PARA; }

  if ((USB_PVND == p_ctrl->type) || (USB_HVND == p_ctrl->type)) { return USB_ERR_NG; }

  if ((USB_PMSC == p_ctrl->type) || (USB_HMSC == p_ctrl->type)) { return USB_ERR_PARA; }
...
usb_err_t R_USB_Suspend(usb_ctrl_t* p_ctrl) {
...
#if USB_CFG_PARAM_CHECKING == USB_CFG_ENABLE
  if (USB_PERI == g_usb_usbmode) { return USB_ERR_NG; /* Support Host only. */ }
  ...
  if ((0 == p_ctrl->address) || (USB_MAXDEVADDR < p_ctrl->address)) { return USB_ERR_PARA; }
#endif /* USB_CFG_PARAM_CHECKING == USB_CFG_ENABLE */

  ret_code = R_USB_GetInformation(p_ctrl, &info);
  if (USB_SUCCESS != ret_code) { return USB_ERR_NG; }
...
usb_err_t R_USB_Resume(usb_ctrl_t* p_ctrl) {
  ...
  if (USB_HOST == g_usb_usbmode) {
    ...
    if (USB_PHID != p_ctrl->type) { return USB_ERR_PARA; }
  }
  ...
  if (USB_STS_SUSPEND != info.status) { return USB_ERR_NOT_SUSPEND; }
...
#if USB_CFG_PARAM_CHECKING == USB_CFG_ENABLE
    if (USB_NULL == p_info) { return USB_ERR_PARA; }
#endif /* USB_CFG_PARAM_CHECKING == USB_CFG_ENABLE */
...
usb_err_t R_USB_PipeRead(usb_ctrl_t* p_ctrl, uint8_t* buf, uint32_t size) {
  ...
  if ((USB_NULL == buf) || (USB_NULL == size)) { return USB_ERR_PARA; }
  ...
  ret_code = R_USB_GetInformation(p_ctrl, &info);
  if ((USB_SUCCESS != ret_code) || (USB_STS_CONFIGURED != info.status)) { return USB_ERR_NG; }
  ...
  if (USB_HOST == g_usb_usbmode) {
...
#if defined(USB_CFG_HVND_USE)
    g_usb_hdata[p_ctrl->module][p_ctrl->pipe].complete =
        usb_hvendor_read_complete; /* Callback function */
#endif                             /* defined(USB_CFG_HVND_USE) */
    ...
    *(uint32_t*)g_usb_hdata[p_ctrl->module][p_ctrl->pipe].p_usr_data = p_ctrl->address;
    ...
    err = usb_hstd_transfer_start(
        &g_usb_hdata[p_ctrl->module][p_ctrl->pipe]); /* USB Transfer Start */
#endif /* (USB_CFG_MODE & USB_CFG_HOST) == USB_CFG_HOST */
  } else {
#if ((USB_CFG_MODE & USB_CFG_PERI) == USB_CFG_PERI)
    if (USB_NULL != g_p_usb_pipe[p_ctrl->pipe]) { ret_code = USB_ERR_BUSY; }
...
#if defined(USB_CFG_HVND_USE)
    g_usb_hdata[p_ctrl->module][p_ctrl->pipe].complete =
        usb_hvendor_write_complete; /* Callback function */
#endif                              /* defined(USB_CFG_HVND_USE) */
    g_usb_hdata[p_ctrl->module][p_ctrl->pipe].segment                = USB_TRAN_END;
    *(uint32_t*)g_usb_hdata[p_ctrl->module][p_ctrl->pipe].p_usr_data = p_ctrl->address;
...
usb_err_t R_USB_PipeStop(usb_ctrl_t* p_ctrl) {
  ...
  if (USB_OK == err) { ret_code = USB_SUCCESS; }
...
/***********************************************************************************************************************
 ...
 Arguments       : usb_ctrl_t   *p_ctrl      : Pointer to usb_ctrl_t structure.
                 : uint16_t     *p_pipe      : Pointer to the area to store the
 used pipe bitmap. Return value    : usb_err_t    error code   :
 USB_SUCCESS,USB_ERR_NG etc.
 ***********************************************************************************************************************/
usb_err_t R_USB_GetUsePipe(usb_ctrl_t* p_ctrl, uint16_t* p_pipe) {
...
#if USB_CFG_PARAM_CHECKING == USB_CFG_ENABLE
  if ((USB_NULL == p_ctrl) && (USB_HOST == g_usb_usbmode)) { return USB_ERR_PARA; }
#endif /* USB_CFG_PARAM_CHECKING == USB_CFG_ENABLE */
  ...
  if (USB_NULL == p_pipe) { return USB_ERR_PARA; }
...
/***********************************************************************************************************************
 ...
 Arguments       : usb_ctrl_t   *p_ctrl      : Pointer to usb_ctrl_t structure.
                 : usb_pipe_t   *p_info      : Pointer to the area to store the
 pipe information Return value    : usb_err_t error code      :
 USB_SUCCESS,USB_ERR_NG etc.
 ***********************************************************************************************************************/
  ...
  if (USB_HOST == g_usb_usbmode) {
    ...
    for (i = 0; USB_PDTBLEND != p_table[i]; i += USB_EPL) /* EP table loop */
    {
      if (p_table[i] == p_ctrl->pipe) /* EP table pipe no check */
      {
        ...
        p_info->mxps =
            usb_cstd_get_maxpacket_size(USB_NULL, p_ctrl->pipe); /* Set Max packet size */
        return USB_SUCCESS;
      }
    }
#endif /* (USB_CFG_MODE & USB_CFG_PERI) == USB_CFG_PERI */
