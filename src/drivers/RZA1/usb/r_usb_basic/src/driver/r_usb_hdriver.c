...
// This is only used by some of the sample device classes. Hosting MIDI doesn't
// use it. Rohan
...
/* IN pipe */   /* OUT pipe */
    ...
    USB_NULL,
#else
    USB_NULL, USB_NULL, USB_NULL, USB_NULL, USB_NULL, USB_NULL, USB_NULL, USB_NULL,
#endif
...
usb_er_t usb_hstd_transfer_start(usb_utr_t* ptr) {
  ...
  if (USB_MAX_PIPE_NO < pipenum) { return USB_ERROR; }

  if (USB_ON == g_usb_hstd_pipe_request[ptr->ip][pipenum]) { return USB_QOVR; }
...
/***********************************************************************************************************************
 ...
 Description     : Send request for RESUME signal output to USB device to MGR
 task. Arguments       : usb_utr_t    *ptr         : Pointer to usb_utr_t
 structure. : uint16_t     devaddr      : Device Address Return          : none
 ***********************************************************************************************************************/
 ...
 ***********************************************************************************************************************/
usb_er_t usb_hstd_hcd_snd_mbx(usb_utr_t* ptr,
                              uint16_t   msginfo,
                              uint16_t   dat,
                              uint16_t*  adr,
                              usb_cb_t   callback) {
  ...
  if (USB_OK == err) {
    ...
    if (USB_OK != err) {
      ...
      if (USB_OK != err) { USB_PRINTF1("### hHcdSndMbx rel_blk error (%ld)\n", err); }
    }
  } else {
...
void usb_hstd_mgr_snd_mbx(usb_utr_t* ptr, uint16_t msginfo, uint16_t dat, uint16_t res) {
  ...
  if (USB_OK == err) {
    ...
    if (USB_OK != err) {
      ...
      if (USB_OK != err) { USB_PRINTF1("### hMgrSndMbx rel_blk error (%ld)\n", err); }
    }
  } else {
...
void usb_hstd_hcd_rel_mpl(usb_utr_t* ptr, uint16_t n) {
  ...
  if (USB_OK != err) { USB_PRINTF1("### USB HCD rel_blk error: %d\n", n); }
...
/***********************************************************************************************************************
 ...
 Description     : Submit utr: Get the device address via the specified pipe
 num- : ber and do a USB transfer. Arguments       : usb_utr_t    *ptr         :
 Pointer to usb_utr_t structure. Return          : usb_er_t                  :
 USB_OK
 ***********************************************************************************************************************/
static usb_er_t usb_hstd_set_submitutr(usb_utr_t* ptr) {
  uint16_t   pipenum;
  ...
  if (USB_PIPE0 == pipenum) {
    ...
    /* Control Write Data */
      ...
      pp       = g_p_usb_pipe[USB_PIPE0];
      end_flag = usb_hstd_ctrl_write_start(ptr,
                                           pp->tranlen,
                                           (uint8_t*)pp->p_tranadr); /* Control write start */
...
static void usb_hstd_interrupt(usb_utr_t* ptr) {
  ...
  switch (intsts) {
    ...
    case USB_INT_NRDY:
      usb_hstd_nrdy_pipe(ptr); // I've seen this happen when disconnecting a
                               // device from a hub... Rohan
      break;
...
usb_er_t usb_hstd_clr_feature(usb_utr_t* ptr, uint16_t addr, uint16_t epnum, usb_cb_t complete) {
  ...
  if (0xFF == epnum) {
    /* ClearFeature(Device) */
    usb_shstd_clr_stall_request[0] =
        USB_CLEAR_FEATURE | USB_HOST_TO_DEV | USB_STANDARD | USB_DEVICE;
    usb_shstd_clr_stall_request[1] = USB_DEV_REMOTE_WAKEUP;
    usb_shstd_clr_stall_request[2] = (uint16_t)0x0000;
  } else {
    /* ClearFeature(endpoint) */
    usb_shstd_clr_stall_request[0] =
        USB_CLEAR_FEATURE | USB_HOST_TO_DEV | USB_STANDARD | USB_ENDPOINT;
...
/***********************************************************************************************************************
 ...
 function is completed Arguments       : usb_utr_t    *ptr         : Pointer to
 usb_utr_t structure. : uint16_t     data1        : Not Use : uint16_t     data2
 : Not Use Return value    : none
 ***********************************************************************************************************************/
static void usb_hstd_clr_stall_result(usb_utr_t* ptr, uint16_t data1, uint16_t data2) {
  usb_mh_t   p_blf;
  usb_er_t   err;
...
void usb_hstd_hcd_task(usb_vp_int_t stacd) {
  ...
  switch (msginfo) {
    ...
    case USB_MSG_HCD_PCUTINT: // Not sure if this can actually happen - no other
                              // reference to this macro

      ptr = (usb_utr_t*)p_usb_shstd_hcd_msg;
      /* USB interrupt Handler */
      uartPrintln("going into usb_hstd_interrupt_handler() from the routine task - "
                  "that's kinda bad I think!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
...
// Heavily modified function by Rohan. Works for setting up a send, for both
// host and peripheral. Supply a NULL ptr for peripheral.
void usb_send_start_rohan(usb_utr_t* ptr, uint16_t pipe, uint8_t const* data, int size) {
  ...
  // Select NAK */
  usb_cstd_set_nak_fast_rohan(pipe); // Ok somehow even this wasn't necessary!! Rohan. (Trying
                                     // re-enabling now to fix that freeze I keep getting in
  ...
  /* Ignore count clear */ // Only the original host function - not
                           // peripheral - had this.

  hw_usb_clear_status_bemp(NULL, pipe);
  /* BEMP Status Clear */ // Seems not necessary. Rohan. (Trying re-enabling
                          // now to fix that freeze I keep getting in
                          // usb_cstd_chg_curpipe_rohan_fast()...)
  hw_usb_clear_sts_brdy(NULL, pipe);
  /* BRDY Status Clear */ // Seems not necessary. Rohan. (Trying re-enabling
                          // now to fix that freeze I keep getting in
                          // usb_cstd_chg_curpipe_rohan_fast()...)

  // usb_hstd_buf2fifo(ptr, pipe, useport);  /* Buffer to FIFO data write */
  {
    hw_usb_clear_brdyenb(NULL, pipe); // Seems not necessary. Rohan. (Trying re-enabling now to
                                      // fix that freeze I keep getting in
                                      // usb_cstd_chg_curpipe_rohan_fast()...)
    ...
    // end_flag = usb_hstd_write_data(ptr, pipe, useport);
    {
      ...
      // buffer = usb_cstd_is_set_frdy(ptr, pipe, (uint16_t)USB_CUSE,
      // USB_FALSE);
      uint16_t result = usb_cstd_is_set_frdy_rohan(pipe);

      /* Check error */
      if (USB_FIFOERROR == result) { goto gotFifoError; }

      // Simplifications here by Rohan, since we always send 64 bytes or
      // smaller, and that always fits within one transfer (max 512 bytes).
      // Also, we always send in multiples of 4 bytes.

      // g_p_usb_data[pipe] = usb_hstd_write_fifo(ptr, count, USB_CUSE,
      // g_p_usb_data[pipe]);
      {
      ...
      // Simplification by Rohan, since we always send 64 bytes or smaller, and
      // that always fits within one transfer (max 512 bytes).
    ...
    hw_usb_set_bempenb(USB_NULL, pipe); // It'd be cool if we could only need to set this up
                                        // once, on pipe setup... but that didn't seem to work.

    /* Enable Not Ready Interrupt */
    hw_usb_set_nrdyenb(USB_NULL,
                       pipe); // Seems not necessary. Rohan. (Trying re-enabling
                              // now to fix that freeze I keep getting in
                              // usb_cstd_chg_curpipe_rohan_fast()...)
  }
    ...
    case USB_D1DMA:
      ...
      if (g_usb_hstd_data_cnt[ptr->ip][pipe] <= g_usb_cstd_dma_fifo[ptr->ip][dma_ch]) {
        g_usb_cstd_dma_size[ptr->ip][dma_ch] =
            g_usb_hstd_data_cnt[ptr->ip][pipe]; /* Transfer data size */
...
void usb_hstd_fifo_to_buf(usb_utr_t* ptr, uint16_t pipe, uint16_t useport) {
  ...
  end_flag = usb_pstd_read_data(pipe, useport); // Modified by Rohan to call the PSTD
                                                // function, since they're the same now
...
usb_er_t usb_hstd_transfer_end(usb_utr_t* ptr, uint16_t pipe, uint16_t status) {
  ...
  if (USB_ON == g_usb_hstd_pipe_request[ptr->ip][pipe]) { return USB_ERROR; }
...
usb_er_t usb_hstd_mgr_open(usb_utr_t* ptr) {
  ...
  for (i = USB_PIPE0; i <= USB_MAX_PIPE_NO; i++) { g_usb_hstd_suspend_pipe[ptr->ip][i] = USB_OK; }
  ...
  for (i = 0; i < (USB_MAXDEVADDR + 1u); i++) {
    ...
    g_usb_hstd_device_info[ptr->ip][i][3] =
        (uint16_t)USB_IFCLS_NOT; /* Interface Class : NO class */
...
uint16_t usb_hstd_chk_pipe_info(uint16_t speed, uint16_t* ep_tbl, uint8_t* descriptor) {
  ...
  if (USB_DT_ENDPOINT == descriptor[1]) {
    ...
    if ((descriptor[2] & USB_EP_DIRMASK) == USB_EP_IN) {
      ...
      /* OUT(send) */
      pipe_cfg |=
          (uint16_t)((uint16_t)(ep_tbl[1] & (USB_DBLBFIELD | USB_CNTMDFIELD)) | USB_DIR_H_OUT);
    ...
    if (0 != descriptor[6]) {
      ...
      if (((uint16_t)(pipe_cfg & USB_TYPFIELD) == USB_TYPFIELD_INT) && (USB_HSCONNECT != speed)) {
        work1 = descriptor[6];
        work2 = 0;
        for (; 0 != work1; work2++) { work1 = (uint16_t)(work1 >> 1); }
        if (0 != work2) {
          /* Interval time */
...
void usb_hstd_set_pipe_info(uint16_t* dst_ep_tbl, uint16_t* src_ep_tbl, uint16_t length) {
  ...
  for (i = 0; i < length; i++) { dst_ep_tbl[i] = src_ep_tbl[i]; }
} /* End of function usb_hstd_set_pipe_info() */

/***********************************************************************************************************************
 Function Name   : usb_hstd_return_enu_mgr
 Description     : Continuous enumeration is requested to MGR task (API for
 nonOS) Arguments       : usb_utr_t    *ptr         : Pointer to usb_utr_t
 structure. : uint16_t     cls_result   : Class check result Return          :
 none
 ***********************************************************************************************************************/
 ...
 ***********************************************************************************************************************/
usb_er_t
usb_hstd_change_device_state(usb_utr_t* ptr, usb_cb_t complete, uint16_t msginfo, uint16_t member) {
  usb_mh_t       p_blf;
  usb_er_t       err;
  usb_er_t       err2;
  usb_hcdinfo_t* hp;
...
usb_err_t usb_hstd_hcd_open(usb_utr_t* ptr) {
  ...
  for (i = USB_PIPE0; i <= USB_MAX_PIPE_NO; i++) { g_p_usb_pipe[i] = (usb_utr_t*)USB_NULL; }

#if USB_CFG_BC == USB_CFG_ENABLE
  if (USB_IP1 == ptr->ip) { g_usb_hstd_bc[ptr->ip].state = USB_BC_STATE_INIT; }
...
void usb_hstd_device_information(usb_utr_t* ptr, uint16_t devaddr, uint16_t* tbl) {
  ...
  if (devaddr == 0) {
    for (p = tbl, i = 0; i < 8; ++i) { *p++ = USB_NOPORT; }
    ...
    for (i = 0; i < 8; ++i) { tbl[i] = g_usb_hstd_device_info[ptr->ip][devaddr][i]; }
  }
