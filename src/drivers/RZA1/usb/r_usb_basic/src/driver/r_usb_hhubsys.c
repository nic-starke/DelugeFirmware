...
#define USB_HUBDOWNPORT \
  (127u) /* HUB downport (MAX15) */ // Set by Rohan. Having a high number here
                                    // doesn't use any extra memory (possibly it
                                    // might have before I did my other code
                                    // edits).
...
static uint16_t usb_hhub_pipe_info(usb_utr_t* ptr,
                                   uint8_t*   table,
                                   uint16_t   offset,
                                   uint16_t   speed,
                                   uint16_t   length);
...
static void
usb_hhub_new_connect(usb_utr_t* ptr, uint16_t hubaddr, uint16_t portnum, usb_clsinfo_t* mess);
static uint16_t usb_hhub_port_attach(uint16_t hubaddr, uint16_t portnum, usb_clsinfo_t* mess);
static void
usb_hhub_port_reset(usb_utr_t* ptr, uint16_t hubaddr, uint16_t portnum, usb_clsinfo_t* mess);
...
uint16_t g_usb_hhub_def_ep_tbl[USB_NUM_USBIP][USB_EPL + 1] = {
    {
        /* PIPE9 Definition */
        USB_PIPE9,
        USB_NULL | USB_BFREOFF | USB_CFG_DBLBOFF | USB_CFG_CNTMDOFF | USB_CFG_SHTNAKOFF | USB_NULL |
            USB_NULL,
        (uint16_t)USB_BUF_SIZE(64u) |
            USB_BUF_NUMB(7u), // PIPE9 must use buffer 7, according to manual!
        USB_NULL,
        USB_NULL,
        USB_CUSE,
  ...
  if (USB_MAXHUB != g_usb_shhub_number[ptr->ip]) {
    ...
    if (USB_OK == err) {
      ...
      if (USB_OK != err) {
        ...
        if (USB_OK != err2) { USB_PRINTF1("### hHubOpen rel_blk error (%ld)\n", err2); }
      }
    } else {
    ...
    usb_hstd_set_pipe_info(&g_usb_hhub_def_ep_tbl[ptr->ip][index],
                           &g_usb_hhub_tmp_ep_tbl[ptr->ip][index],
                           (uint16_t)USB_EPL);

    g_usb_shhub_process[ptr->ip] = USB_MSG_CLS_INIT;
    usb_hstd_set_pipe_registration(ptr,
                                   (uint16_t*)&g_usb_hhub_def_ep_tbl[ptr->ip],
...
/***********************************************************************************************************************
 ...
 Arguments       : usb_utr_t    *ptr         : Pointer to usb_utr_t structure.
                 : usb_hcdreg_t *callback    : Pointer ot usb_hcdreg_t
 structure. Return value    : none
 ***********************************************************************************************************************/
void usb_hhub_registration(usb_utr_t* ptr, usb_hcdreg_t* callback) {
  ...
  if (USB_NULL == callback) {
    ...
    driver.p_pipetbl = (uint16_t*)&g_usb_hhub_def_ep_tbl[ptr->ip]; /* Pipe Information Table
                                                                      Define address */
  } else {
...
uint16_t usb_hhub_get_hub_information(usb_utr_t* ptr, uint16_t hubaddr, usb_cb_t complete) {
  ...
  /* Request */
  g_usb_shhub_class_request[ptr->ip][0] =
      USB_GET_DESCRIPTOR | USB_DEV_TO_HOST | USB_CLASS | USB_DEVICE;
  ...
  if (USB_QOVR == qerr) { return USB_HUB_QOVR; }
 ...
 ***********************************************************************************************************************/
uint16_t
usb_hhub_get_port_information(usb_utr_t* ptr, uint16_t hubaddr, uint16_t port, usb_cb_t complete) {
  usb_er_t qerr;
...
void usb_hhub_task(usb_vp_int_t stacd) {
  ...
  switch (mess->msginfo) {
    ...
    default:
      ...
      err = USB_REL_BLK(USB_HUB_MPL, (usb_mh_t)mess);
      if (USB_OK != err) { USB_PRINTF0("### USB HUB rel_blk error\n"); }
...
/* Condition compilation by the difference of useful function */
  ...
  switch (g_usb_shhub_class_seq[ptr->ip]) {
    ...
    case USB_SEQ_3:
      ...
      /* Get HUB descriptor */
      g_usb_shhub_process[ptr->ip] = USB_MSG_CLS_CHECKREQUEST;
      checkerr                     = usb_hhub_get_hub_information(ptr,
                                              g_usb_shhub_dev_addr[ptr->ip],
                                              usb_hhub_class_request_complete);
      /* Submit overlap error */
      if (USB_HUB_QOVR == checkerr) { usb_hhub_specified_path_wait(ptr, (uint16_t)10); }
    ...
    case USB_SEQ_4:
      ...
      // By Rohan: It seems some hubs don't send that descriptor back. If it
      // stalled, just make a guess about how many ports it had - it'll still
      // work.
      ...
      if (USB_OK == retval) { usb_hhub_check_request(ptr, checkerr); /* Next sequence */ }
    ...
    case USB_SEQ_5:
      ...
      /* Get HUB descriptor Check */
      if (USB_OK == checkerr) {
        retval = usb_hhub_check_descriptor(g_usb_hhub_descriptor[ptr->ip],
                                           (uint16_t)USB_DT_HUBDESCRIPTOR);
        if (USB_ERROR == retval) {
          ...
          // Edited by Rohan: If more hub ports than we support, just use the
          // first few - don't render the whole hub useless
          g_usb_hhub_descriptor[ptr->ip][2] = USB_HUBDOWNPORT;
          // checkerr = USB_ERROR;
        } else {
      ...
      /* Port number set */
      g_usb_shhub_info_data[ptr->ip][g_usb_shhub_dev_addr[ptr->ip]].port_num =
          g_usb_hhub_descriptor[ptr->ip][2];
...
static void usb_hhub_init_down_port(usb_utr_t* ptr, uint16_t hubaddr, usb_clsinfo_t* mess) {
  ...
  if (USB_MSG_CLS_INIT != g_usb_shhub_process[ptr->ip]) /* Check Hub Process */
  {
    ...
    if (USB_OK != err) { USB_PRINTF0("### HUB snd_msg error\n"); /* Send Message failure */ }
  } else {
    switch (g_usb_shhub_init_seq[ptr->ip]) {
      ...
      case USB_SEQ_2: /* Request Result Check */
        ...
        // By Rohan: If stall, then that port number doesn't exist. The need to do
        // this is a side-effect of my other fix that just assumes the max number
        // of ports if the hub won't tell us
...
static uint16_t usb_hhub_port_attach(uint16_t hubaddr, uint16_t portnum, usb_clsinfo_t* mess) {
  ...
  if (USB_MSG_HUB_ATTACH != g_usb_shhub_process[ptr->ip]) {
      ...
      case USB_SEQ_2:
        ...
        if ((USB_CONFIGURED == g_usb_hstd_mgr_mode[ptr->ip][rootport]) ||
          ...
          usb_hhub_specified_path_wait(
              mess,
              30u); // Modified by Rohan from 3u - when too low, we get a freeze
                    // on attaching a device to a hub. I did some more accurate
                    // testing, and it seems that a delay is needed, and the
    ...
    err = USB_REL_BLK(USB_HUB_MPL, (usb_mh_t)mess);
    if (USB_OK != err) { USB_PRINTF0("### USB HostHubClass rel_blk error\n"); }
  }
  return USB_OK;
  ...
  if (USB_MSG_HUB_EVENT != g_usb_shhub_process[ptr->ip]) {
    ...
    switch (g_usb_shhub_event_seq[ptr->ip]) {
      case USB_SEQ_0: /* Request */
        ...
        {
          /* Hub and Port Status Change Bitmap(b0:Hub,b1:DownPort1change
           * detected,b2:DownPort2,...) */
          if (0 != (g_usb_hhub_data[ptr->ip][hubaddr][0] &
        ...
        if (USB_OK == retval) {
          ...
          if (0 != (port_status & USB_BIT_PORT_CONNECTION)) /* PORT_CONNECTION */
          {
            if (USB_MSG_HUB_SUBMITRESULT == g_usb_shhub_info[ptr->ip]) {
              if (0 == (g_usb_shhub_down_port[ptr->ip][hubaddr] &
                        USB_BITSET(g_usb_shhub_event_port[ptr->ip]))) {
                ...
                if (g_usb_shhub_process[ptr->ip] != USB_MSG_HUB_ATTACH)
                  goto noPortConnection; // Error checking added by Rohan -
                                         // otherwise USB stops function when too
...
/***********************************************************************************************************************
 ...
 Arguments       : usb_utr_t    *ptr         : Pointer to usb_utr_t structure.
                 : uint16_t     hubaddr      : Hub address
                 : uint16_t     portnum      : Down port number
                 : usb_clsinfo_t *mess       : Pointer to usb_clsinfo_t
 structure. Return value    : none
 ***********************************************************************************************************************/
static void
usb_hhub_port_reset(usb_utr_t* ptr, uint16_t hubaddr, uint16_t portnum, usb_clsinfo_t* mess) {
    ...
    switch (g_usb_shhub_reset_seq[ptr->ip]) {
      ...
      case USB_SEQ_1:
        ...
        if (USB_OK == retval) {
          ...
          /* Get Status */
          retval =
              usb_hhub_get_port_information(ptr, hubaddr, portnum, usb_hhub_class_request_complete);
...
static void usb_hhub_trans_start(usb_utr_t* ptr,
  ...
  g_usb_shhub_data_mess[ptr->ip][hubaddr].keyword =
      g_usb_shhub_info_data[ptr->ip][hubaddr].pipe_num;
...
static uint16_t usb_hhub_port_clr_feature(usb_utr_t* ptr,
  ...
  /* Request */
  g_usb_shhub_class_request[ptr->ip][0] =
      USB_CLEAR_FEATURE | USB_HOST_TO_DEV | USB_CLASS | USB_OTHER;
  g_usb_shhub_class_request[ptr->ip][1] = command;
  g_usb_shhub_class_request[ptr->ip][2] = port; /* Port number */
  g_usb_shhub_class_request[ptr->ip][3] = 0;
  g_usb_shhub_class_request[ptr->ip][4] = hubaddr; /* Device address */
...
static void usb_hhub_trans_complete(usb_utr_t* mess, uint16_t data1, uint16_t data2) {
  ...
  if ((USB_MSG_HUB_SUBMITRESULT != g_usb_shhub_process[ptr->ip]) &&
      (USB_NULL != g_usb_shhub_process[ptr->ip])) {
    err = USB_SND_MSG(USB_HUB_MBX, (usb_msg_t*)mess);
    if (USB_OK != err) {
      /* Send Message failure */
      USB_PRINTF0("### HUB snd_msg error\n");
    ...
    switch (mess->status) {
      ...
      case USB_DATA_OK:
        ...
        if ((USB_DEFAULT == g_usb_hstd_mgr_mode[ptr->ip][0]) ||
          ...
          err = USB_SND_MSG(USB_HUB_MBX, (usb_msg_t*)mess);
          if (USB_OK != err) { USB_PRINTF0("### HUB task snd_msg error\n"); }
        } else {
          /* HUB port connection */
    ...
    switch (table[ofdsc + 1]) {
      ...
      case USB_DT_ENDPOINT: /* Endpoint Descriptor */
        ...
        if (USB_EP_INT == (table[ofdsc + 3] & USB_EP_TRNSMASK)) {
          retval =
              usb_hstd_chk_pipe_info(speed, &g_usb_hhub_tmp_ep_tbl[ptr->ip][offset], &table[ofdsc]);
...
/***********************************************************************************************************************
 ...
 Arguments       : uint16_t     **table      : Pointer to Configuration
 Descriptor area : uint16_t     spec         : HUB specification Return value :
 uint16_t                  : DONE/ERROR
 ***********************************************************************************************************************/
static uint16_t usb_hhub_chk_config(uint16_t** table, uint16_t spec) {
...
/***********************************************************************************************************************
 ...
 Arguments       : uint16_t     **table      : Pointer to Interface Descriptor
 area : uint16_t     spec         : HUB specification Return value    : uint16_t
 : DONE/ERROR
 ***********************************************************************************************************************/
static uint16_t usb_hhub_chk_interface(uint16_t** table, uint16_t spec) {
  uint8_t* descriptor_Table;
...
static uint16_t usb_hhub_chk_tbl_indx1(usb_utr_t* ptr, uint16_t hubaddr) {
  ...
  for (i = USB_MAX_PIPE_NO; 0 != i; i--) {
    if (0 == pipecheck[i - 1]) { return (uint16_t)((USB_MAX_PIPE_NO - i) * USB_EPL); }
  }
  return (USB_ERROR);
...
static void usb_hhub_port_detach(usb_utr_t* ptr, uint16_t hubaddr, uint16_t portnum) {
  ...
  /* Selective detach */
  usb_hhub_selective_detach(ptr, devaddr); // Moved by Rohan. Was previously below the below for
                                           // loop, but this function needs some of the data which
                                           // gets nullified below.
    ...
    for (i = USB_MIN_PIPE_NO; i <= USB_MAX_PIPE_NO; i++) {
      ...
      if (usb_hstd_get_device_address(ptr, i) == addr) {
        ...
        if (i == USB_CFG_HMIDI_BULK_SEND || i == USB_CFG_HMIDI_INT_SEND)
          continue; // Don't deconfigure the send-pipe or end its transfer, cos
                    // it's shared - Rohan
 ...
 ***********************************************************************************************************************/
uint16_t usb_hhub_get_string_descriptor1(usb_utr_t* ptr,
                                         uint16_t   devaddr,
                                         uint16_t   index,
                                         usb_cb_t   complete) {
...
/***********************************************************************************************************************
 ...
 Return value    : uint16_t                  : Error info
 ***********************************************************************************************************************/
uint16_t usb_hhub_get_string_descriptor2(usb_utr_t* ptr,
                                         uint16_t   devaddr,
                                         uint16_t   index,
                                         usb_cb_t   complete) {
...
/***********************************************************************************************************************
 ...
 Arguments       : usb_clsinfo_t *ptr        : Pointer to usb_clsinfo_t
 structure. Return value    : none
 ***********************************************************************************************************************/
...
/***********************************************************************************************************************
 ...
 Description     : Next Process Selector
 Arguments       : usb_clsinfo_t *mess       : Pointer to usb_clsinfo_t
 structure. : uint16_t     times        : Timeout value. Return value    : none
 ***********************************************************************************************************************/
