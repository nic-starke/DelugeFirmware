 ...
 */
// I can only get DMA working for one pipe. Look at usb_pstd_pipe2fport(). This
// sets the DMA channel per pipe, for up to two pipes. So long as only one of
// these pipes is used, it'll work. Something's stopping both from working
// together. But, even if I fixed it, using DMA for just 2 pipes still wouldn't
// be all that helpful.
 ...
 * USB_CFG_TPL          : Set the VID and PID pairs for the USB device to be
 * connected.
 * */
