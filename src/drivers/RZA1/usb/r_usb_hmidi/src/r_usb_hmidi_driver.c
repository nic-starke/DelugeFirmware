 ...
 *
 * Copyright (C) 2014(2015) Renesas Electronics Corporation. All rights
 *reserved.
 ***********************************************************************************************************************/
/***********************************************************************************************************************
 ...
 ******************************************************************************/
/* variables */
static uint16_t usb_shmidi_class_request_setup[USB_NUM_USBIP]
                                              [5];     /* Control Transfer Request field */
...
static usb_er_t usb_hhid_data_trans(usb_utr_t* ptr,
                                    uint16_t   pipe,
                                    uint32_t   size,
                                    uint8_t*   table,
                                    usb_cb_t   complete);
...
uint8_t*  g_p_usb_hmidi_config_table[USB_NUM_USBIP];   /* Configuration Descriptor
                                                          Table */
uint8_t* g_p_usb_hmidi_device_table[USB_NUM_USBIP];    /* Device Descriptor Table */
uint8_t* g_p_usb_hmidi_interface_table[USB_NUM_USBIP]; /* Interface Descriptor
                                                          Table */
...
/******************************************************************************
 ...
 Argument        : usb_utr_t    *ptr         : USB internal structure. Selects
 USB channel. : uint16_t     unused       : Not use : uint16_t     status : Not
 use Return          : none
 ******************************************************************************/
static void usb_hmidi_check_result(usb_utr_t* ptr, uint16_t unused, uint16_t status) {
  usb_mh_t       p_blf;
...
static void usb_hmidi_enumeration_sequence(usb_utr_t* mess) {
  ...
  switch (g_usb_hmidi_enum_seq[mess->ip]) {
    ...
    case USB_HHID_ENUM_STR_DT0_REQ:
      ...
      usb_hmidi_get_string_desc(mess,
                                g_usb_hmidi_devaddr[mess->ip],
                                (uint16_t)0,
                                (usb_cb_t)&usb_hmidi_check_result);
    ...
    case USB_HHID_ENUM_STR_IPRODUCT_WAIT: {
      ...
      if ((usb_er_t)USB_CTRL_END == mess->status) {
        ...
        for (j = 0; j < table[0]; j++) { pdata[j] = table[j * 2 + 2]; }
        pdata[table[0]] = 0;
...
void usb_hmidi_task(usb_vp_int_t stacd) {
  ...
  if (p_mess->msginfo == USB_HHID_TCMD_OPEN) { usb_hmidi_enumeration_sequence(p_mess); }

  err = USB_REL_BLK(USB_HMIDI_MPL, (usb_mh_t)p_mess);
  if (USB_OK != err) { USB_PRINTF0("### USB HHID Task rel_blk error\n"); }
...
/******************************************************************************
 ...
 Argument        : usb_utr_t    *ptr         : USB internal structure. Selects
 USB channel. : uint8_t      *table       : Descriptor store address : uint16_t
 speed        : Conect Dpeed(Hi/Full/...) : uint16_t     length       :
 Descriptor total lenght : uint16_t     count        : Pipe tabel index Return
 value    : uint16_t                  : Error info
 ******************************************************************************/
  ...
  while (ofdsc < length) {
    ...
    switch (table[ofdsc + 1]) {
      ...
      case USB_DT_ENDPOINT:
        ...
        // Depending whether it's an in or out pipe, decide where to put its
        // details "Receive" pipe
        ...
        // We now have to set the buffer number for the pipe. I wasn't doing this
        // for ages, resulting in the insane bug where data sent out would be seen
        // coming back in. I haven't double-checked, but I think that even the
        // unmodified USB host driver from Renesas was failing to set this.

        // Some pipe numbers mandate a particular buffer number - see the manual.
        // Others I've picked arbitrarily. So long as every pipe has a different
        // buffer number than every other pipe, we won't get that crazy bug as
        // before.
        ...
        retval = usb_hstd_chk_pipe_info(
            speed,
            (uint16_t*)&pipetbl[pipeTableOffset],
            &table[ofdsc]); // We could check the return for error if we wanted...

        if ((USB_ON == detect_in_pipe) && (USB_ON == detect_out_pipe)) { return USB_OK; }
      ...
      moveOnToNextDescriptor:
        if (!table[ofdsc])
          return USB_ERROR;    // Prevent infinite loop if there's somehow a zero
                               // there. Got this while quickly plugging and
                               // unplugging lots of devices / hub, to test.
...
/******************************************************************************
 ...
 Arguments       : usb_utr_t    *ptr         : USB internal structure. Selects
 USB channel. : uint16_t     addr         : device address : uint16_t     string
 : descriptor index : usb_cb_t     complete     : callback function Return value
 : uint16_t                  : error info
 ******************************************************************************/
uint16_t
usb_hmidi_get_string_desc(usb_utr_t* ptr, uint16_t addr, uint16_t string, usb_cb_t complete) {
  uint16_t i;
  ...
  usb_shmidi_class_request_setup[ptr->ip][0] =
      (USB_GET_DESCRIPTOR | USB_DEV_TO_HOST | USB_STANDARD | USB_DEVICE);
...
/******************************************************************************
 ...
 Arguments       : usb_utr_t    *ptr         : USB internal structure. Selects
 USB channel. : usb_cb_t     complete     : callback info Return value    :
 uint16_t                  : USB_DONE
 ******************************************************************************/
static uint16_t usb_hmidi_cmd_submit(usb_utr_t* ptr, usb_cb_t complete) {
  usb_er_t err;
...
/******************************************************************************
 ...
 Arguments       : usb_utr_t    *ptr         : USB internal structure. Selects
 e.g. channel. : uint16_t     devadr       : Device Adrress : uint16_t     data2
...
void hmidi_configured(usb_utr_t* ptr, uint16_t devadr, uint16_t data2) {
  ...
  // Totally hackish way of getting the device number from our offset from the
  // start of the "endpoint table", but it's the only way the "driver" actually
  // keeps track of this
  ...
  if (currentDeviceNumWithSendPipe[isInterrupt] == midiDeviceNum)
    currentDeviceNumWithSendPipe[isInterrupt] =
        MAX_NUM_USB_MIDI_DEVICES; // Force a reset of pipe setup if it was
                                  // already set to this deviceNum
...
void hmidi_detach(usb_utr_t* ptr, uint16_t devadr, uint16_t data2) {
  // Have to get the MIDI device num by searching through the info left in the
  // endpoint table
  int d;
  ...
  g_usb_hmidi_tmp_ep_tbl[USB_CFG_USE_USBIP][d][1] &=
      (uint16_t)(USB_BFREON | USB_CFG_SHTNAKON);                          /* PIPECFG */
...
void usb_registration(usb_utr_t* ptr) {
  ...
  {
    driver.p_pipetbl =
        (uint16_t*)&g_usb_hmidi_tmp_ep_tbl[ptr->ip][i]; /* Pipe def. table address. */
