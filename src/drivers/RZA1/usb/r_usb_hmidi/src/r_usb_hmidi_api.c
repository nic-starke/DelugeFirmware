...
/******************************************************************************
 ...
 Arguments       : usb_utr_t    *ptr         : IP info (mode, IP no., reg
 address). Return          : none
 ******************************************************************************/
...
/******************************************************************************
 ...
 Arguments       : usb_utr_t    *ptr         : IP info (mode, IP no., reg
 address). : uint16_t     devadr       : Device address Return          : none
 ******************************************************************************/
void R_USB_HmidiSetPipeRegistration(usb_utr_t* ptr, uint16_t devadr) {
  ...
  // Or, if we are on a hub...
    ...
    // Otherwise, if the send-pipe has already been set up, we only have to set
    // up the receive-pipe
...
void R_USB_HmidiClassCheck(usb_utr_t* ptr, uint16_t** table) {
  ...
  g_p_usb_hmidi_device_table[ptr->ip] =
      (uint8_t*)((uint16_t*)table[0]); /* Device Descriptor Table */
  g_p_usb_hmidi_config_table[ptr->ip] =
      (uint8_t*)((uint16_t*)table[1]);                       /* Configuration Descriptor Table */
  ...
  g_p_usb_hmidi_interface_table[ptr->ip] =
      (uint8_t*)((uint16_t*)table[2]); /* Interface Descriptor Table */
...
/******************************************************************************
 ...
 Arguments       : usb_utr_t    *ptr         : IP info (mode, IP no., reg
 address). : uint16_t     devadr       : Device address Return value    : Pipe
 table address.
 ******************************************************************************/
