 ...
 * Device(s)    : RZ/A1H
 * Tool-Chain   : ARM Development Studio�iDS-5TM�j Version 5.26, KPIT GNUARM-RZ
 *v14.01 Description  : USB Peripheral Mass-Storage Sample Code.
 ***********************************************************************************************************************/
/**********************************************************************************************************************
...
static usb_descriptor_t usb_descriptor = {
    ...
    (uint8_t*)g_midi_configuration, /* Pointer to the configuration descriptor
                                       for Full-speed */
    (uint8_t*)0,
    /* Pointer to the configuration descriptor for High-speed */ // Won't work
                                                                 // if this is
                                                                 // set to the
                                                                 // same memory
                                                                 // as the
                                                                 ...
                                                                 // NULL seems
                                                                 // fine
    (uint8_t*)0,                   // Don't think we need this /* Pointer to the qualifier
                                   // descriptor */
...
void openUSBHost(void) {
  ...
  if (USB_SUCCESS != err) { uartPrintln("Failed to open USB0\n"); }
...
void closeUSBPeripheral(void) // You can't immediately close this after opening
                              // it - you'd better wait a little while.
{
  usb_ctrl_t ctrl;
  ctrl.module = WHICH_USB_MODULE;
  ctrl.type   = USB_PMSC;
  ...
  if (USB_SUCCESS != ret_code) { uartPrintln("Failed to close peripheral"); }
