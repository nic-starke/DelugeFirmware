...
int oledPendingMessageToSend =
    0; // 0 means none. The purpose of this variable is to ensure thread safety.
...
void oledRoutine() {
  ...
  sendMessageToPIC:
    ...
    if (howLate >= 0) {
      // oledLowLevelTimerCallback(); // No need to actually set the delay timer
      // - we've already waited for ages - any further delay would be pointless.
...
void sendCVTransfer() {
  ...
  RSPI(SPI_CHANNEL_OLED_MAIN).SPBFCR.BYTE = 0b00110010; // 0b11110010;//0b01100010;//0b00100010; //
                                                        // Now we do *not* reset the RX buffer.
  ...
  spiTransferQueueReadPos =
      ...
      (SPI_TRANSFER_QUEUE_SIZE - 1); // Must do this before actually sending SPI, cos the interrupt
                                     // could come before we do this otherwise. True story.
...
void initiateSelectingOled() {
  ...
  // Actual queue position gets moved along in oledSelectingComplete() when that
  // gets called.
}
...
void oledTransferComplete(uint32_t int_sense) {
  ...
  // Otherwise, deselect OLED. And when that's finished, we might send some more
  // if there is more.
  else {
...
void cvSPITransferComplete(uint32_t sense) {
  ...
  setOutputState(6, 1, true); // Deselect CV DAC. We do it here, nice and early, since we
                              // might be re-selecting it very soon in sendCVTransfer(),
                              // and a real pulse does need to be sent. This should be safe
                              // - I even tried only deselecting it in the instruction
                              // preceding the re-selection, and it did work.

  RSPI(SPI_CHANNEL_OLED_MAIN).SPBFCR.BYTE |=
      1 << 6; // Reset RX buffer. Slightly weird that we have to do it here,
              // after the transfer? Or not weird?
...
void oledDeselectionComplete() {
  ...
  if (spiTransferQueueWritePos != spiTransferQueueReadPos) { sendSPITransferFromQueue(); }
}
