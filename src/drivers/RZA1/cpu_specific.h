...
#define PIC_TX_BUFFER_SIZE \
  1024 // Theoretically I should be able to do just 512, and it almost works,
       // but occasionally the cursor pos won't update - at the end of an audio
       // clip record
...
#define UI_MS_PER_REFRESH_SCROLLING \
  7 // Any faster than this is faster than the UART bus can do, so there's a
    // lag, and if we're sending out multiple scrolls fast, the buffer can build
    // up quite a lag
#endif
