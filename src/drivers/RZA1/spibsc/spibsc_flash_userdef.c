...
******************************************************************************/
static int32_t
set_mode(uint32_t ch_no, uint32_t dual, en_sf_req_t req, uint8_t data_width, uint8_t addr_mode);
static int32_t
read_status(uint8_t* status1, uint8_t* status2, uint32_t ch_no, uint32_t dual, uint8_t data_width);
static int32_t
read_config(uint8_t* config1, uint8_t* config2, uint32_t ch_no, uint32_t dual, uint8_t data_width);
static int32_t
read_bank(uint8_t* bank1, uint8_t* bank2, uint32_t ch_no, uint32_t dual, uint8_t data_width);
static int32_t write_only_status(uint8_t status, uint32_t ch_no, uint32_t dual, uint8_t data_width);
static int32_t
write_status(uint8_t status, uint8_t config, uint32_t ch_no, uint32_t dual, uint8_t data_width);
static int32_t write_bank(uint8_t bank, uint32_t ch_no, uint32_t dual, uint8_t data_width);
static int32_t read_autoboot(uint8_t* autoboot1,
                             uint8_t* autoboot2,
                             uint32_t ch_no,
                             uint32_t dual,
                             uint8_t  data_width);
 ...
 *                -1 : error
 ******************************************************************************/
int32_t Userdef_SFLASH_Set_Mode(uint32_t    ch_no,
                                uint32_t    dual,
                                en_sf_req_t req,
                                uint8_t     data_width,
                                uint8_t     addr_mode) {
...
int32_t Userdef_SFLASH_Busy_Wait(uint32_t ch_no, uint32_t dual, uint8_t data_width) {
  ...
  while (1) {
    ...
    if (dual == SPIBSC_CMNCR_BSZ_DUAL) {
      /* s-flash x 2  */
      if (((st_reg1 & 0x01) == 0) && ((st_reg2 & 0x01) == 0)) { break; }
    } else if (dual == SPIBSC_CMNCR_BSZ_SINGLE) {
      /* s-flash x 1  */
      if ((st_reg1 & 0x01) == 0) { break; }
    } else {
      ret = -1;
      break;
    }
 ...
 ******************************************************************************/
int32_t
Userdef_SFLASH_Ctrl_Protect(en_sf_req_t req, uint32_t ch_no, uint32_t dual, uint8_t data_width) {
  uint8_t st_reg1;
  uint8_t st_reg2;
  uint8_t cf_reg1;
  uint8_t cf_reg2;
 ...
 * Description  : Programs the status and configuration of the serial flash
 *memory. Arguments    : uint8_t status : status register value uint32_t ch_no :
 *use channel No uint32_t dual uint8_t data_width Return Value :  0 : success -1
 *: error
 ******************************************************************************/
static int32_t
write_only_status(uint8_t status, uint32_t ch_no, uint32_t dual, uint8_t data_width) {
  int32_t ret;
 ...
 * Description  : Programs the status and configuration of the serial flash
 *memory. Arguments    : uint8_t status : status register value uint8_t config :
 *configuration register value uint32_t ch_no : use channel No uint32_t dual
 *                uint8_t data_width
 * Return Value :  0 : success
 *                -1 : error
 ******************************************************************************/
 ...
 *                locked out.It is good practice to check for the errors
 *evertime and specially the first time the status is checked.
 *
 * Arguments    : uint32_t ch_no : use channel No
 *                uint32_t dual
 *                uint8_t data_width
