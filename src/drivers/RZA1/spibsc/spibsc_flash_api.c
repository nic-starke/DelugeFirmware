...
******************************************************************************/
static int32_t read_data_quad(uint32_t addr,
                              uint8_t* buf,
                              int32_t  unit,
                              uint32_t ch_no,
                              uint32_t dual,
                              uint8_t  addr_mode);
static int32_t read_data_single(uint32_t addr,
                                uint8_t* buf,
                                int32_t  unit,
                                uint32_t ch_no,
                                uint32_t dual,
 ...
 *                -1 : error
 ******************************************************************************/
int32_t R_SFLASH_EraseSector(uint32_t addr,
                             uint32_t ch_no,
                             uint32_t dual,
                             uint8_t  data_width,
                             uint8_t  addr_mode) {
 ...
 *                When 1 was designated by SPI_QUAD macro, the page program
 *command corresponding to Quad is used. When 0 was designated by SPI_QUAD
 *macro, The page program command corresponding to Single is used. Arguments :
 *uint32_t addr : write address(address of serial flash memory) uint8_t *buf  :
 *write data(Start address in a buffer) int32_t size  : The number of data byte
 *                uint32_t ch_no : use channel No
 *                uint32_t dual
 *                uint32_t data_width
 *                uint8_t  addr_mode
 ...
 *                -1 : error
 ******************************************************************************/
int32_t R_SFLASH_ByteProgram(uint32_t addr,
                             uint8_t* buf,
                             int32_t  size,
                             uint32_t ch_no,
                             uint32_t dual,
  ...
  while (size > 0) {
    ...
    if (size <= 0) { g_spibsc_spimd_reg.sslkp = SPIBSC_SPISSL_NEGATE; }
 ...
 *                When 1 was designated by SPI_QUAD macro, read command
 *corresponding to Quad is used. When 0 was designated by SPI_QUAD macro, read
 *command corresponding to Single is used. Arguments    : uint32_t addr  : read
 *address(address of serial flash memory) uint8_t *buf   : Start address in a
 *buffer int32_t  size  : The number of data byte uint32_t ch_no : use channel
 *No uint32_t dual  : dual mode uint32_t data_width uint8_t  addr_mode Return
 *Value :  0 : success -1 : error
 ******************************************************************************/
int32_t R_SFLASH_ByteRead(uint32_t addr,
                          uint8_t* buf,
                          int32_t  size,
                          uint32_t ch_no,
                          uint32_t dual,
