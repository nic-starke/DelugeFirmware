...
void View::setTripletsLedState() {
  ...
  IndicatorLEDs::setLedState(tripletsLedX,
                             tripletsLedY,
...
int View::buttonAction(int x, int y, bool on, bool inCardRoutine) {
  ...
  // Tap tempo button. Shouldn't move this to MatrixDriver, because this code
  // can put us in tapTempo mode, and other UIs aren't built to handle this
  ...
  // MIDI learn button
    ...
    if (on) {
      ...
      if (currentUIMode == UI_MODE_MIDI_LEARN) { endMIDILearn(); }
    }
  }
      ...
      if (on) {
        if (currentUIMode == UI_MODE_NONE) {
          ...
          if (Buttons::isShiftButtonPressed()) {
            ...
            if (available) { openUI(&contextMenuClearSong); }
          }
      ...
      // Press release
        ...
        if (currentUIMode == UI_MODE_HOLDING_LOAD_BUTTON) {
          ...
          if ((int32_t)(AudioEngine::audioSampleTimer - timeSaveButtonPressed) < (44100 >> 1)) {
            ...
            // Need to redraw everything if no success, because the LoadSongUI
            // does some drawing before even determining whether it can start
            // successfully
            if (!success) {
  ...
  // Back button
    ...
    if (on) {
      ...
      if (actionLogger.allowedToDoReversion()) {
        ...
        // Here we'll take advantage of the pending command system which has to
        // exist for these commands for their MIDI-triggered case anyway. In the
        // future, maybe a lot more commands should pend in the same way?
...
#endif
        // If we were in tap tempo mode (i.e. waiting for the second tap), stop
        // it!
        if (currentUIMode == UI_MODE_TAP_TEMPO) { playbackHandler.tapTempoAutoSwitchOff(); }
    }
  }
...
void View::endMIDILearn() {
  if (shouldSaveSettingsAfterMidiLearn) {
    if (!AudioEngine::audioRoutineLocked)
      FlashStorage::writeSettings(); // Rare case where we could have been
                                     // called during audio routine
  }
...
void View::setTimeBaseScaleLedState() {
  ...
  if (getCurrentUI()->toClipMinder() &&
      currentSong->currentClip == currentSong->getSyncScalingClip()) {
    IndicatorLEDs::blinkLed(syncScalingLedX, syncScalingLedY);
  }

  // Otherwise, just light it solidly on or off
  else {
    IndicatorLEDs::setLedState(syncScalingLedX,
                               syncScalingLedY,
                               currentSong->getSyncScalingClip() != NULL);
  }
}
...
void View::sectionMidiLearnPadPressed(bool on, uint8_t section) {
  if (on) {
    ...
    if (deleteMidiCommandOnRelease) { learnedThing->clear(); }
    endMidiLearnPressSession(0);
  }
}
...
void View::drumMidiLearnPadPressed(bool on, Drum* drum, Kit* kit) {
  if (on) {
    ...
    kitPressedForMIDILearn =
        kit; // Having this makes it possible to search much faster when we call
...
void View::melodicInstrumentMidiLearnPadPressed(bool on, MelodicInstrument* instrument) {
  if (on) {
    ...
    if (deleteMidiCommandOnRelease) {
      clearMelodicInstrumentMonoExpressionIfPossible(); // In case it gets
                                                        // "stuck".
      learnedThing->clear();
...
void View::endMidiLearnPressSession(uint8_t newThingPressed) {
  // Depending on which thing was previously pressed, we might have to do some
  // admin
  ...
  if (thingPressedForMidiLearn) {
    ...
    switch (thingPressedForMidiLearn) {
      ...
      case MIDI_LEARN_DRUM_INPUT: {
        // For a Drum, we can assume that the user must be viewing a Clip, as the
        // currentClip.
        ((Instrument*)currentSong->currentClip->output)->beenEdited(false);

        // Copy bend ranges if appropriate. This logic is duplicated in
        // NoteRow::setDrum().
        int newBendRange;
        ...
        if (newBendRange) {
          NoteRow* noteRow = ((InstrumentClip*)currentSong->currentClip)
                                 ->getNoteRowForDrum(drumPressedForMIDILearn);
          if (noteRow) {
            ExpressionParamSet* expressionParams =
                noteRow->paramManager.getOrCreateExpressionParamSet(true);
            if (expressionParams) {
        ...
        if (drumPressedForMIDILearn->type == DRUM_TYPE_SOUND) {
          currentSong->grabVelocityToLevelFromMIDIDeviceAndSetupPatchingForAllParamManagersForDrum(
              fromDevice,
              (SoundDrum*)drumPressedForMIDILearn,
              kitPressedForMIDILearn);
        }
      ...
      case MIDI_LEARN_MELODIC_INSTRUMENT_INPUT:
        ...
        ParamManager* paramManager = melodicInstrumentPressedForMIDILearn->getParamManager(
            currentSong); // Could be NULL, e.g. for CVInstruments with no
                          // Clips
        ...
        isMPEZone:
          // Now that we've just learned a MIDI input, update bend ranges from the
          // input device, if they were set, and no automation in activeClip. Same
          // logic can be found in InstrumentClip::changeInstrument().
          int zone = channelOrZone - MIDI_CHANNEL_MPE_LOWER_ZONE;

          newBendRanges[BEND_RANGE_MAIN] = fromDevice->mpeZoneBendRanges[zone][BEND_RANGE_MAIN];
          newBendRanges[BEND_RANGE_FINGER_LEVEL] =
              fromDevice->mpeZoneBendRanges[zone][BEND_RANGE_FINGER_LEVEL];

          if (newBendRanges[BEND_RANGE_FINGER_LEVEL]) {
            InstrumentClip* clip =
                (InstrumentClip*)melodicInstrumentPressedForMIDILearn->activeClip;
            if (!clip || !clip->hasAnyPitchExpressionAutomationOnNoteRows()) {
              if (paramManager) { // Could be NULL, e.g. for CVInstruments with no
                                  // Clips
                ExpressionParamSet* expressionParams =
                    paramManager->getOrCreateExpressionParamSet();
                if (expressionParams) {
        ...
        // Or if we don't already know this is an MPE zone...
          ...
          if (learnedThing->device == fromDevice) {
            ...
            // If multiple channels seen, that's a shortcut for setting up MPE
            // zones for the device in question
            if (highestMIDIChannelSeenWhileLearning != lowestMIDIChannelSeenWhileLearning) {
              if (lowestMIDIChannelSeenWhileLearning == 1) {
                ...
                fromDevice->ports[MIDI_DIRECTION_INPUT_TO_DELUGE]
          ...
          // If still here, it's not MPE. Now that we know that, see if we want to
          // apply a stored bend range for the input MIDI channel of the device
        ...
        // In a perfect world, we'd also update CVInstrument::cachedBendRanges[].
        // But that'd only make a difference if it had no Clips.

        // We need to reset the expression params, in case they've gotten stuck.
        // This was mostly prone to happening when doing the "learn MPE input"
        // multi-finger trick.
        ...
        if (melodicInstrumentPressedForMIDILearn->type == INSTRUMENT_TYPE_SYNTH) {
          currentSong
              ->grabVelocityToLevelFromMIDIDeviceAndSetupPatchingForAllParamManagersForInstrument(
                  fromDevice,
                  (SoundInstrument*)melodicInstrumentPressedForMIDILearn);
        }
...
void View::clearMelodicInstrumentMonoExpressionIfPossible() {
  ...
  if (paramManager) {
    ...
    ExpressionParamSet*     expressionParams =
        (ExpressionParamSet*)expressionParamsSummary->paramCollection;
    if (expressionParams) {
      ...
      ModelStackWithParamCollection* modelStack =
          setupModelStackWithSong(modelStackMemory, currentSong)
              ->addTimelineCounter(
                  melodicInstrumentPressedForMIDILearn->activeClip) // Could be NULL
              ->addOtherTwoThingsButNoNoteRow(
                  melodicInstrumentPressedForMIDILearn->toModControllable(),
                  paramManager)
  ...
  if (thingPressedForMidiLearn) {
    ...
    if (thingPressedForMidiLearn == MIDI_LEARN_MELODIC_INSTRUMENT_INPUT) {
      ...
      if (currentSong->currentClip->output->type == INSTRUMENT_TYPE_MIDI_OUT) {
        ...
        // But only if user hasn't already started learning MPE stuff... Or
        // regular note-ons...
        if (highestMIDIChannelSeenWhileLearning < lowestMIDIChannelSeenWhileLearning) {
          learnedThing->device        = fromDevice;
    ...
    // Or, for all other types of things the user might be holding down...
      ...
      // So long as the value wasn't 0, pretend it was a note-on for
      // command-learn purposes
      if (value) { noteOnReceivedForMidiLearn(channelOrZone + 16, cc, 127); }
    }
#endif
  }
}
...
void View::modEncoderAction(int whichModEncoder, int offset) {
  ...
  if (activeModControllableModelStack.modControllable) {
    ...
    copyModelStack(newModelStackMemory, &activeModControllableModelStack,
    sizeof(ModelStackWithThreeMainThings)); ModelStackWithThreeMainThings*
    localModelStack = (ModelStackWithThreeMainThings*)newModelStackMemory;
    */
    ...
    if (activeModControllableModelStack.timelineCounterIsSet()) {
      ...
      copyModelStack(modelStackTempMemory,
      ...
      noteTailsAllowedBefore = activeModControllableModelStack.modControllable->allowNoteTails(
          tempModelStack->addSoundFlags());

      bool timelineCounterChanged =
          activeModControllableModelStack.getTimelineCounter()
...

    {
      ...
      // Or, if normal case - an actual param
        ...
        if (modelStackWithParam->timelineCounterIsSet() &&
            !modelStackWithParam->paramCollection->doesParamIdAllowAutomation(
                modelStackWithParam)) {
        ...
        if (!newKnobPos && modelStackWithParam->paramCollection->shouldParamIndicateMiddleValue(
                               modelStackWithParam)) {
          IndicatorLEDs::blinkKnobIndicator(whichModEncoder);

          // Make it harder to turn that knob away from its centred position
          Encoders::timeModEncoderLastTurned[whichModEncoder] =
...
void View::modEncoderButtonAction(uint8_t whichModEncoder, bool on) {
  ...
  // If the learn button is pressed, user is trying to copy or paste, and the
  // fact that we've ended up here means they can't
  if (Buttons::isButtonPressed(learnButtonX, learnButtonY)) {
#if !HAVE_OLED
    if (on) { numericDriver.displayPopup("CANT"); }
#endif
    return;
  }
  ...
  if (activeModControllableModelStack.modControllable) {
    ...
    char modelStackMemory[MODEL_STACK_MAX_SIZE];
    copyModelStack(modelStackMemory,
                   &activeModControllableModelStack,
                   sizeof(ModelStackWithThreeMainThings));
    ModelStackWithThreeMainThings* modelStack = (ModelStackWithThreeMainThings*)modelStackMemory;

    bool anyEditingDone =
        activeModControllableModelStack.modControllable->modEncoderButtonAction(whichModEncoder,
                                                                                on,
                                                                                modelStack);
...
void View::setKnobIndicatorLevel(uint8_t whichModEncoder) {
  ...
  // timelineCounter and paramManager could be NULL - if the user is holding
  // down an audition pad in Arranger, and that Output has no Clips. Especially
  // if it's a MIDIInstrument (no ParamManager).
  ModelStackWithAutoParam* modelStackWithParam =
      activeModControllableModelStack.modControllable->getParamFromModEncoder(
          whichModEncoder,
          &activeModControllableModelStack,
  ...
  if (modelStackWithParam->autoParam) {
    ...
    knobPos =
        modelStackWithParam->modControllable->getKnobPosForNonExistentParam(whichModEncoder,
                                                                            modelStackWithParam);
  }
    ...
    if (on) {
      ...
      if (isUIModeWithinRange(modButtonUIModes)) {
        activeModControllableModelStack.modControllable->modButtonAction(
            whichButton,
            true,
            (ParamManagerForTimeline*)activeModControllableModelStack.paramManager);
...
void View::setModLedStates() {
  ...
  if (!itsTheSong) {
    ...
    if (getRootUI() == &instrumentClipView) {
      ...
      shouldBlink =
          (clip->output->type == INSTRUMENT_TYPE_KIT) ? clip->affectEntire : clip->onKeyboardScreen;
    }
...
void View::displayAutomation() {
  if (pendingParamAutomationUpdatesModLevels) { setKnobIndicatorLevels(); }
  if (getCurrentUI() == &soundEditor) soundEditor.getCurrentMenuItem()->readValueAgain();
}
...
void View::setModRegion(uint32_t pos, uint32_t length, int noteRowId) {
  ...
  if (length && activeModControllableModelStack.timelineCounterIsSet() &&
      ...
      activeModControllableModelStack.paramManager && !playbackHandler.isEitherClockActive() &&
    ...
    activeModControllableModelStack.paramManager->toForTimeline()->grabValuesFromPos(
        pos,
        &activeModControllableModelStack);
    // activeModControllable might not be a Sound, but in that case, the
    // pointer's not going to get used
  }
  setKnobIndicatorLevels();
}
...
void View::cycleThroughReverbPresets() {
  ...
  for (int p = 0; p < NUM_PRESET_REVERBS; p++) {
    int differentness = std::abs(currentRoomSize - presetReverbRoomSize[p]) +
                        std::abs(currentDampening - presetReverbDampening[p]);
...
void View::displayOutputName(Output* output, bool doBlink, Clip* clip) {
  ...
  drawOutputNameFromDetails(output->type,
                            channel,
                            channelSuffix,
                            output->name.get(),
                            editedByUser,
...
// If HAVE_OLED, must make sure OLED::sendMainImage() gets called after this.
void View::drawOutputNameFromDetails(int         outputType,
                                     int         channel,
                                     int         channelSuffix,
                                     char const* name,
                                     bool        editedByUser,
...
#endif
  OLED::drawStringCentred(outputTypeText,
                          yPos,
  ...
  oledDrawString:
    ...
    if (stringLengthPixels <= OLED_MAIN_WIDTH_PIXELS) {
      OLED::drawStringCentred(nameToDraw,
                              yPos,
                              OLED::oledMainImage[0],
                              OLED_MAIN_WIDTH_PIXELS,
                              textSpacingX,
...
#else
    bool andAHalf;
    if (numericDriver.getEncodedPosFromLeft(99999, name, &andAHalf) >
        NUMERIC_DISPLAY_LENGTH) { // doBlink &&
      ...
      if (*charPos == '0') { // If first digit is 0, then no more digits allowed.
        ...
        while (*charPos >= '0' && *charPos <= '9' && charPos < (name + 3)) { charPos++; }
      }
...
void View::navigateThroughAudioOutputsForAudioClip(int offset, AudioClip* clip, bool doBlink) {
  ...
  // Work out availabilityRequirement. But we don't in this case need to think
  // about whether the Output can be "replaced" - that's for InstrumentClips
  ...
  Output* newOutput =
      currentSong->getNextAudioOutput(offset, clip->output, availabilityRequirement);
...
void View::navigateThroughPresetsForInstrumentClip(int                            offset,
  ...
  // Work out availabilityRequirement. This can't change as presets are
  // navigated through... I don't think?
  int  availabilityRequirement;
  bool oldInstrumentCanBeReplaced =
      modelStack->song->canOldOutputBeReplaced(clip, &availabilityRequirement);
  ...
  if (instrumentType == INSTRUMENT_TYPE_MIDI_OUT || instrumentType == INSTRUMENT_TYPE_CV) {
    ...
    // TODO: the contents of these badly wants to be replaced with how I did it
    // in changeInstrumentType()!
    ...
    if (instrumentType == INSTRUMENT_TYPE_CV) {
      while (true) {
        ...
        if (availabilityRequirement == AVAILABILITY_ANY)
          ...
          if (!modelStack->song->doesNonAudioSlotHaveActiveClipInSession(instrumentType,
                                                                         newChannel))
            break;
    ...
    // Or MIDI
      ...
      while (true) {
        ...
        if (availabilityRequirement == AVAILABILITY_ANY) {
          ...
          if (!modelStack->song->getInstrumentFromPresetSlot(instrumentType,
                                                             newChannel,
                                                             newChannelSuffix,
                                                             NULL,
                                                             NULL,
    ...
    newInstrument = modelStack->song->getInstrumentFromPresetSlot(instrumentType,
                                                                  newChannel,
                                                                  newChannelSuffix,
                                                                  NULL,
                                                                  NULL,
    ...
    // If we want to "replace" the old Instrument, we can instead sneakily just
    // modify its channel
    if (shouldReplaceWholeInstrument) {
      ...
      // Because these are just MIDI / CV instruments and we're changing them
      // for all Clips, we can just change the existing Instrument object!
    ...
    // Otherwise...
      ...
      // If an Instrument doesn't yet exist for the new channel we're gonna
      // use...
      if (!newInstrument) {
        if (instrumentType == INSTRUMENT_TYPE_MIDI_OUT) {
          newInstrument =
              modelStack->song->grabHibernatingMIDIInstrument(newChannel, newChannelSuffix);
          if (newInstrument) goto gotAnInstrument;
        }
        newInstrument = storageManager.createNewNonAudioInstrument(instrumentType,
                                                                   newChannel,
                                                                   newChannelSuffix);
        if (!newInstrument) {
        ...
        // We just allocated a brand new Instrument in RAM. If MIDI, copy knob
        // assignments from old Instrument
        if (instrumentType == INSTRUMENT_TYPE_MIDI_OUT) {
          MIDIInstrument* newMIDIInstrument = (MIDIInstrument*)newInstrument;
          MIDIInstrument* oldMIDIInstrument = (MIDIInstrument*)clip->output;
          memcpy(newMIDIInstrument->modKnobCCAssignments,
                 oldMIDIInstrument->modKnobCCAssignments,
                 sizeof(oldMIDIInstrument->modKnobCCAssignments));
          newInstrument->editedByUser =
              oldNonAudioInstrument->editedByUser; // This keeps a record of "whether there are
                                                   // any CC assignments", so must be copied
                                                   // across
        }
    ...
    gotAnInstrument:
      ...
      int error = clip->changeInstrument(modelStack,
                                         newInstrument,
                                         NULL,
  ...
  // Or if we're on a Kit or Synth...
      ...
      for (Drum* thisDrum = kit->firstDrum; thisDrum; thisDrum = thisDrum->next) {
        if (thisDrum->type == DRUM_TYPE_SOUND) {
          ...
          if (!modelStack->song->getBackedUpParamManagerPreferablyWithClip(
            ...
            if (!modelStack->song->findParamManagerForDrum(kit,
                                                           soundDrum)) { // If no ParamManager with
                                                                         // a NoteRow somewhere...
              ...
              if (results.loadedFromFile)
                ...
                numericDriver.freezeWithError("E105"); // Sven got - very rare! This means Kit was
                                                       // hibernating, I guess.
            }
          }
        }
    ...
    if (shouldReplaceWholeInstrument) {
      ...
      // We know the Instrument hasn't been added to the Song, and this call
      // will do it
    ...
    // Otherwise, just changeInstrument() for this one Clip
      ...
      // If that Instrument wasn't already in use in the Song, copy default
      // velocity over
      newInstrument->defaultVelocity = oldInstrument->defaultVelocity;

      // If we're here, we know the Clip is not playing in the arranger (and
      // doesn't even have an instance in there)
  ...
  modelStack->song->ensureAllInstrumentsHaveAClipOrBackedUpParamManager(
      "E058",
      "H058"); // I got this during limited-RAM testing. Maybe there wasn't
               // enough RAM to create the ParamManager or store its backup?
}
...
void View::instrumentChanged(ModelStackWithTimelineCounter* modelStack, Instrument* newInstrument) {
  ...
  setActiveModControllableTimelineCounter(
      modelStack->getTimelineCounter()); // Do a redraw. Obviously the Clip is
                                         // the same
}
...
void View::getClipMuteSquareColour(Clip* clip, uint8_t thisColour[]) {
  ...
  if (currentUIMode == UI_MODE_VIEWING_RECORD_ARMING && clip && clip->armedForRecording) {
    if (blinkOn) {
      bool shouldGoPurple =
          clip->type == CLIP_TYPE_AUDIO && ((AudioClip*)clip)->overdubsShouldCloneOutput;
      ...
      // Dull colour, cos can't actually begin linear recording despite being
      // armed
      else {
        if (shouldGoPurple) {
          thisColour[0] = 60;
  ...
  // If user assigning MIDI controls and this Clip has a command assigned, flash
  // pink
  ...
  // Or if not soloing...
    ...
    if (currentSong->getAnyClipsSoloing()) { dimColour(thisColour); }
  }
...
int View::clipStatusPadAction(Clip* clip, bool on, int yDisplayIfInSessionView) {
  ...
  switch (currentUIMode) {
    case UI_MODE_MIDI_LEARN:
      ...
      if (!on) { uiNeedsRendering(&sessionView, 0, 1 << yDisplayIfInSessionView); }
      break;
    ...
    case UI_MODE_NONE:
      // If the user was just quick and is actually holding the record button but
      // the submode just hasn't changed yet...
      if (on && Buttons::isButtonPressed(recordButtonX, recordButtonY)) {
        clip->armedForRecording = !clip->armedForRecording;
        sessionView.timerCallback(); // Get into UI_MODE_VIEWING_RECORD_ARMING.
                                     // TODO: this needs doing properly - what if
                                     // we're in a Clip view?
        break;
      }
      // No break
    ...
    case UI_MODE_STUTTERING:
      if (on) {
        sessionView.performActionOnPadRelease =
