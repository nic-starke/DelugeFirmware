...
int32_t reverbCompressorVolumeInEffect; // Active right now - possibly overridden by
                                        // the sound with the most reverb
...
// Let's keep these grouped - the stuff we're gonna access regularly during
// audio rendering
int      cpuDireness = 0;
...
Voice staticVoices[NUM_VOICES_STATIC]; // Hmm, I forgot this was still being
                                       // used. It's not a great way of doing
                                       // things... wait does this still
                                       // actually get used? No?
Voice* firstUnassignedVoice;

// You must set up dynamic memory allocation before calling this, because of its
// call to setupWithPatching()
void init() {
  ...
  paramManagerForSamplePreview =
      new ((void*)paramManagerForSamplePreviewMemory) ParamManagerForTimeline();
  ...
  for (int i = 0; i < NUM_VOICE_SAMPLES_STATIC; i++) {
    ...
    voiceSamples[i].nextUnassigned =
        (i == NUM_VOICE_SAMPLES_STATIC - 1) ? NULL : &voiceSamples[i + 1];
  }

  for (int i = 0; i < NUM_TIME_STRETCHERS_STATIC; i++) {
    new (&timeStretchers[i]) TimeStretcher();
    timeStretchers[i].nextUnassigned =
        (i == NUM_TIME_STRETCHERS_STATIC - 1) ? NULL : &timeStretchers[i + 1];
  }
  ...
  i2sRXBufferPos = (uint32_t)getRxBufferStart() +
                   ((SSI_RX_BUFFER_NUM_SAMPLES - SSI_TX_BUFFER_NUM_SAMPLES - 16)
                    << (2 + NUM_MONO_INPUT_CHANNELS_MAGNITUDE)); // Subtracting 5 or more
                                                                 // seems fine
#endif
}
...
void unassignAllVoices(bool deletingSong) {
  ...
  // Because we unfortunately don't have a master list of VoiceSamples or
  // actively sounding AudioClips, we have to unassign all of those by going
  // through all AudioOutputs. But if there's no currentSong, that's fine - it's
  // already been deleted, and this has already been called for it before then.
  if (currentSong) {
    for (Output* output = currentSong->firstOutput; output; output = output->next) {
      if (output->type == OUTPUT_TYPE_AUDIO) { ((AudioOutput*)output)->cutAllSound(); }
    }
  }
}
...
void songSwapAboutToHappen() {
  ...
  uiTimerManager.unsetTimer(TIMER_PLAY_ENABLE_FLASH); // Otherwise, a timer might get called and try
                                                      // to access Clips that we may have deleted
                                                      // below
  logAction("a1");
...
// To be called when CPU is overloaded and we need to free it up. This stops the
// voice which has been releasing longest, or if none, the voice playing
// longest.
  ...
  if (bestVoice) {
    ...
    if (justDoFastRelease) {
      if (bestVoice->envelopes[0].state < ENVELOPE_STAGE_FAST_RELEASE) {
        ...
        if (!stillGoing) { unassignVoice(bestVoice, bestVoice->assignedToSound); }
...
void routine() {
  ...
  if (audioRoutineLocked) {
    ...
    return; // Prevents this from being called again from inside any e.g. memory
            // allocation routines that get called from within this!
  }
  ...
  int      numSamples =
      ((uint32_t)(saddr - i2sTXBufferPos) >> (2 + NUM_MONO_OUTPUT_CHANNELS_MAGNITUDE)) &
  ...
  // Flush everything out of the MIDI buffer now. At this stage, it would only
  // really have live user-triggered output and MIDI THRU in it. We want any
  // messages like "start" to go out before we send any clocks below, and also
  // want to give them a head-start being sent and out of the way so the clock
  // messages can be sent on-time
  ...
  if (anythingInMidiOutputBufferNow || anythingInGateOutputBufferNow) {
    ...
    // We're only allowed to do this if the timer ISR isn't pending (i.e. we
    // haven't enabled to timer to trigger it) - otherwise this will all get
    // called soon anyway. I thiiiink this is 100% immune to any synchronization
    // problems?
  ...
  if (numSamples >= direnessThreshold) { // 20
    ...
    if (!bypassCulling) {
      ...
      if (numSamplesOverLimit >= 0) {
        ...
        for (int i = 0; i < numToCull; i++) { cullVoice(); }
    ...
    if ((int32_t)(audioSampleTimer - timeDirenessChanged) >=
        (44100 >> 3)) { // Only if it's been long enough
      timeDirenessChanged = audioSampleTimer;
      cpuDireness--;
      if (cpuDireness < 0)
        cpuDireness = 0;
  ...
  // Want to round to be doing a multiple of 4 samples, so the NEON functions
  // can be utilized most efficiently. Note - this can take numSamples up as
  // high as SSI_TX_BUFFER_NUM_SAMPLES (currently 128).
  if (numSamples >= 3) { numSamples = (numSamples + 2) & ~3; }

#endif

  int timeWithinWindowAtWhichMIDIOrGateOccurs = -1; // -1 means none

  // If a timer-tick is due during or directly after this window of audio
  // samples...
  ...
  startAgain:
    ...
    // If tick occurs right now at the start of this window - which is fairly
    // likely, cos we would have cut the previous window short if it had such a
    // tick during it...
    if (timeTilNextTick <= 0) {
      if (nextTickType == TICK_TYPE_TIMER) {
        ...
        playbackHandler.scheduleSwungTick(); // If that swung tick action just did a song
                                             // swap, this will have already been called,
                                             // but fortunately this doesn't break anything
      }

      // Those could have outputted clock or other MIDI / gate
      if (midiEngine.anythingInOutputBuffer() || cvEngine.clockOutputPending ||
          cvEngine.gateOutputPending) { // Not asapGateOutputPending. That probably
                                        // actually couldn't have been generated by
                                        // a actionSwungTick() anyway I think?
    ...
    // If the tick is during this window, shorten the window so we stop right at
    // the tick
    if (timeTilNextTick < numSamples) { numSamples = timeTilNextTick; }

    // And now we know how long the window's definitely going to be, see if we
    // want to do any trigger clock or MIDI clock out ticks during it

    if (playbackHandler.triggerClockOutTickScheduled) {
      int32_t timeTilTriggerClockOutTick =
          playbackHandler.timeNextTriggerClockOutTick - audioSampleTimer;
  ...
  if (reverbOn) {
    ...
    // Reverb panning
    bool thisDoPanning =
        (renderInStereo && shouldDoPanning(reverbPan, &reverbAmplitudeL, &reverbAmplitudeR));
    ...
    // HPF on reverb send, cos if it has DC offset, the reverb magnifies that,
    // and the sound farts out
    {
  ...
  if (getCurrentUI() == &sampleBrowser || getCurrentUI() == &contextMenuFileBrowserKit ||
    ...
    sampleForPreview->render(modelStack,
                             renderingBuffer,
                             numSamples,
                             reverbBuffer,
                             sideChainHitPending);
  ...
  // LPF and stutter for song (must happen after reverb mixed in, which is why
  // it's happening all the way out here
  ...
  // 167763968 is 134217728 made a bit bigger so that default filter resonance
  // doesn't reduce volume overall

  if (currentSong) {
    FilterSetConfig filterSetConfig;
    currentSong->globalEffectable.setupFilterSetConfig(&filterSetConfig,
                                                       &masterVolumeAdjustmentL,
    ...
    masterVolumeAdjustmentR = masterVolumeAdjustmentL; // This might have changed in the above
                                                       // function calls

    currentSong->globalEffectable.processStutter(renderingBuffer,
                                                 numSamples,
                                                 &currentSong->paramManager);

    // And we do panning for song here too - must be post reverb, and we had to
    // do a volume adjustment below anyway
    int32_t pan = currentSong->paramManager.getUnpatchedParamSet()->getValue(
  ...
  */
  ...
  if ((midiEngine.anythingInOutputBuffer() || anyGateOutputPending) &&
    ...
    // I don't think this actually could still get left at -1, but just in
    // case...
    if (timeWithinWindowAtWhichMIDIOrGateOccurs == -1) {
      timeWithinWindowAtWhichMIDIOrGateOccurs = 0;
    }
    ...
    uint32_t saddrMovementSinceStart =
        saddrPosAtEnd - saddrPosAtStart; // You'll need to &(SSI_TX_BUFFER_NUM_SAMPLES - 1) this
                                         // anytime it's used

    int32_t samplesTilMIDIOrGate =
        (timeWithinWindowAtWhichMIDIOrGateOccurs - saddrMovementSinceStart -
         unadjustedNumSamplesBeforeLappingPlayHead) &
        (SSI_TX_BUFFER_NUM_SAMPLES - 1);

    if (!samplesTilMIDIOrGate) samplesTilMIDIOrGate = SSI_TX_BUFFER_NUM_SAMPLES;

    // samplesTilMIDI += 10; This gets the start of stuff perfectly lined up.
    // About 10 for MIDI, 12 for gate

    if (anyGateOutputPending) {
      // If a gate note-on was processed at the same time as a gate note-off,
      // the note-off will have already been sent, but we need to make sure that
      // the note-on now doesn't happen until a set amount of time after the
      // note-off.
      int32_t gateMinDelayInSamples =
          ((uint32_t)cvEngine.minGateOffTime * 289014) >> 16; // That's *4.41 (derived from 44100)
      int32_t samplesTilAllowedToSend = cvEngine.mostRecentSwitchOffTimeOfPendingNoteOn +
                                        gateMinDelayInSamples - audioSampleTimer;
    ...
    *TGRA[TIMER_MIDI_GATE_OUTPUT] =
        ((uint32_t)samplesTilMIDIOrGate * 766245) >>
...
bool doSomeOutputting() {
  ...
  while (renderingBufferOutputPosNow != renderingBufferOutputEnd) {
    ...
    if (!(((uint32_t)((uint32_t)i2sTXBufferPosNow - saddr) >>
    ...
    // Here we're going to do something equivalent to a
    // multiply_32x32_rshift32(), but add dithering. Because dithering is good,
    // and also because the annoying codec chip freaks out if it sees the same
    // 0s for too long.
    int64_t lAdjustedBig =
        (int64_t)renderingBufferOutputPosNow->l * (int64_t)masterVolumeAdjustmentL +
        (int64_t)getNoise();
    int64_t rAdjustedBig =
        (int64_t)renderingBufferOutputPosNow->r * (int64_t)masterVolumeAdjustmentR +
        (int64_t)getNoise();
    ...
    if (anythingProbablyPressed) {
      ...
      if ((audioSampleTimer >> 6) & 1) { outputSample = -outputSample; }
  ...
  renderingBufferOutputPos = renderingBufferOutputPosNow; // Write back from __restrict__ pointer to
                                                          // permanent pointer
  ...
  if (numSamplesOutputted) {
    ...
    for (SampleRecorder* recorder = firstRecorder; recorder; recorder = recorder->next) {
      ...
      // Recording from an input source
        ...
        // We'll feed a bunch of samples to the SampleRecorder - normally all of
        // what we just advanced, but if the buffer wrapped around, we'll just
        // go to the end of the buffer, and not worry about the extra bit at the
        // start - that'll get done next time.
        uint32_t stopPos = (i2sRXBufferPos < (uint32_t)recorder->sourcePos) ?
                               (uint32_t)getRxBufferEnd() :
                               i2sRXBufferPos;
        ...
        // We also enforce a firm limit on how much to feed, to keep things
        // sane. Any remaining will get done next time.
...
void updateReverbParams() {
  ...
  if (reverbCompressorVolume < 0) {
    ...
    // Set the initial "highest amount found" to that of the song itself, which
    // can't be affected by sidechain. If nothing found with more reverb, then
    // we don't want the reverb affected by sidechain
    ...
    for (Output* thisOutput = currentSong->firstOutput; thisOutput; thisOutput = thisOutput->next) {
      thisOutput->getThingWithMostReverb(&soundWithMostReverb,
    ...
    if (soundWithMostReverb) {
      ...
      if (whichCable != 255) {
        reverbCompressorVolumeInEffect =
            patchCableSet->getModifiedPatchCableAmount(whichCable,
      ...
      reverbCompressorVolumeInEffect =
          globalEffectableWithMostReverb->getSidechainVolumeAmountAsPatchCableDepth(
              paramManagerWithMostReverb);

    compressorFound:
      reverbCompressorShapeInEffect = paramManagerWithMostReverb->getUnpatchedParamSet()->getValue(
...
void previewSample(String* path, FilePointer* filePointer, bool shouldActuallySound) {
  ...
  int error =
      range->sampleHolder.loadFile(false, true, true, CLUSTER_LOAD_IMMEDIATELY, filePointer);
  ...
  if (shouldActuallySound) {
    ...
    ModelStackWithThreeMainThings* modelStack =
        setupModelStackWithThreeMainThingsButNoNoteRow(modelStackMemory,
                                                       ...
                                                       NULL,
                                                       paramManagerForSamplePreview);
    sampleForPreview->Sound::noteOn(modelStack,
                                    &sampleForPreview->arpeggiator,
                                    NOTE_FOR_DRUM,
                                    zeroMPEValues);
    bypassCulling = true; // Needed - Dec 2021. I think it's during
                          // SampleBrowser::selectEncoderAction() that we may
...
Voice* solicitVoice(Sound* forSound) {
  ...
  if (numSamplesLastTime >= 100 && activeVoices.getNumElements()) {
    ...
    numSamplesLastTime -= 10; // Stop this triggering for lots of new voices. We just don't know
                              // how they'll weigh up to the ones being culled
  ...
  if (i == -1) {
    // if (ALPHA_OR_BETA_VERSION) numericDriver.freezeWithError("E193"); // No,
    // having run out of RAM here isn't a reason to not continue.
...
void unassignVoice(Voice*                    voice,
                   Sound*                    sound,
                   ModelStackWithSoundFlags* modelStack,
                   bool                      removeFromVector,
                   bool                      shouldDispose) {
...
void timeStretcherUnassigned(TimeStretcher* timeStretcher) {
  if (timeStretcher >= timeStretchers &&
      timeStretcher < &timeStretchers[NUM_TIME_STRETCHERS_STATIC]) {
...
void doRecorderCardRoutines() {
  ...
  while (true) {
    ...
    // If, while in the card routine, a new Recorder was added, then our linked
    // list traversal state thing will be out of wack, so let's just get out and
    // come back later
  ...
  if (ALPHA_OR_BETA_VERSION && ENABLE_CLIP_CUTTING_DIAGNOSTICS && count >= 10 &&
      !numericDriver.popupActive) {
...
void slowRoutine() {
  ...
  // The RX buffer is much bigger than the TX, and we get our timing from the TX
  // buffer's sending. However, if there's an audio glitch, and also at
  // start-up, it's possible we might have missed an entire cycle of the TX
  // buffer. That would cause the RX buffer's latency to increase. So here, we
  // check for that and correct it. The correct latency for the RX buffer is
  // between (SSI_TX_BUFFER_NUM_SAMPLES) and (2 * SSI_TX_BUFFER_NUM_SAMPLES).
...
SampleRecorder* getNewRecorder(int  numChannels,
                               int  folderID,
                               int  mode,
                               bool keepFirstReasons,
  ...
  if (!recorderMemory) { return NULL; }

  SampleRecorder* newRecorder = new (recorderMemory) SampleRecorder();

  error = newRecorder->setup(numChannels,
                             mode,
                             keepFirstReasons,
                             writeLoopPoints,
                             folderID,
  ...
  if (mode >= AUDIO_INPUT_CHANNEL_FIRST_INTERNAL_OPTION) {
    renderInStereo = true; // Quickly set it to true. It'll keep getting reset
                           // to true now every time inputRoutine() is called
  }

  return newRecorder;
}

// PLEASE don't call this if there's any chance you might be in the SD card
// routine...
...
//     for (Voice* thisVoice = voices; thisVoice != &voices[numVoices];
//     thisVoice++) {
