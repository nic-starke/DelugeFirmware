...
// This class extends OrderedResizeableArrayWith32bitKey so that if we wish, we
// can still use that class's single-word search functions, where we only care
// about the first word.

class OrderedResizeableArrayWithMultiWordKey : public OrderedResizeableArrayWith32bitKey {
public:
  OrderedResizeableArrayWithMultiWordKey(int newElementSize   = sizeof(uint32_t) * 2,
                                         int newNumWordsInKey = 2);
  int  searchMultiWord(uint32_t* __restrict__ keyWords,
                       int comparison,
                       int rangeBegin = 0,
                       int rangeEnd   = -1);
  int  searchMultiWordExact(uint32_t* __restrict__ keyWords,
                            int* getIndexToInsertAt = NULL,
                            int  rangeBegin         = 0);
