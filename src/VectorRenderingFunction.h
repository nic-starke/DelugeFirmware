...
#define waveRenderingFunctionGeneral()                                                          \
  {                                                                                             \
    ...
    uint16x4_t strength2;                                                                       \
                                                                                                \
    /* Need to use a macro rather than a for loop here, otherwise won't                         \
     * compile with less than O2. */                                                            \
...
// Renders 4 wave values (a "vector") together in one go - special case for
...
#define waveRenderingFunctionPulse()                                                   \
  {                                                                                    \
    ...
    /* Need to use a macro rather than a for loop here, otherwise won't                \
     * compile with less than O2. */                                                   \
    waveRenderingFunctionPulseForLoop(0) waveRenderingFunctionPulseForLoop(1)          \
        waveRenderingFunctionPulseForLoop(2) waveRenderingFunctionPulseForLoop(3)      \
                                                                                       \
    ...
    int16x4_t valueB2 = vreinterpret_s16_u16(vshrn_n_u32(readValueB, 16));             \
                                                                                       \
    /* Sneakily do this backwards to flip the polarity of the output, which we         \
     * need to do anyway */                                                            \
