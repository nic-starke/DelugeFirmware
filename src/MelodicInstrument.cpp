...
bool MelodicInstrument::writeMelodicInstrumentAttributesToFile(Clip* clipForSavingOutputOnly,
  ...
  if (!clipForSavingOutputOnly) {
    ...
    // Annoyingly, I used one-off tag names here, rather than it conforming to
    // what the LearnedMIDI class now uses. Channel gets written here as an
    // attribute. Device, gets written below, as a tag.
    if (midiInput.containsSomething()) {
      if (midiInput.isForMPEZone()) {
        char const* zoneText =
            (midiInput.channelOrZone == MIDI_CHANNEL_MPE_LOWER_ZONE) ? "lower" : "upper";
...
void MelodicInstrument::writeMelodicInstrumentTagsToFile(Clip* clipForSavingOutputOnly,
  ...
  if (!clipForSavingOutputOnly) {
    // Annoyingly, I used one-off tag names here, rather than it conforming to
    // what the LearnedMIDI class now uses.
    if (midiInput.containsSomething()) {
      // Device gets written here as a tag. Channel got written above, as an
      // attribute.
...
void MelodicInstrument::offerReceivedNote(ModelStackWithTimelineCounter* modelStack,
  ...
  if (MIDIDeviceManager::differentiatingInputsByDevice && midiInput.device &&
      fromDevice != midiInput.device)
    return;
  ...
  yupItsForUs:
    ...
    ModelStackWithNoteRow* modelStackWithNoteRow =
        instrumentClip ? instrumentClip->getNoteRowForYNote(note, modelStack) :
    ...
    if (on) {
      ...
      // MPE stuff - if editing note, we need to record the initial values which
      // might have been sent before this note-on.
      instrumentClipView.reportMPEInitialValuesForNoteEditing(
          modelStackWithNoteRow,
          mpeValues); // Hmm, should we really be going in here even when it's
                      // not MPE input?
      ...
      if (!noteRow || !noteRow->soundingStatus) {
        ...
        if (instrumentClip) {
          ...
          if (shouldRecordNotes) {
            ...
            // Special case - if recording session to arrangement, then yes we
            // do want to record to the Clip always (even if not designated as
            // "active")
            ...
            // And another special case - if there's a linear recording
            // beginning really soon, and activeClip is not linearly recording
            // (and maybe not even active)...
              ...
              if (samplesTilLaunch <= LINEAR_RECORDING_EARLY_FIRST_NOTE_ALLOWANCE) {
                ...
                if (clipAboutToRecord) { goto recordingEarly; }
              }
            }
            ...
            doRecord:
              instrumentClip->possiblyCloneForArrangementRecording(
                  modelStack); // Will have to re-get modelStackWithNoteRow
                               // after this call
              instrumentClip =
                  (InstrumentClip*)modelStack->getTimelineCounter(); // Re-get it, cos it
                                                                     // might have changed
              ...
              if (noteRow) {
                instrumentClip->recordNoteOn(modelStackWithNoteRow,
                                             velocity,
                                             forcePos0,
                                             mpeValuesOrNull,
                                             midiChannel);
      ...
      justAuditionNote:
        /*
                        ...
                        // If it was not MPE, then get the pitch bend and
           channel pressure values from the ParamManager, so e.g. the synth can
           make the correct sound on
                        // the note-on it's going to do now.
                        if (mpeValues == zeroMPEValues) {
                            ParamManager* paramManager =
           getParamManager(modelStackWithNoteRow->song); MPEParamSet*
           expressionParams = paramManager->getMPEParamSet(); if
           (expressionParams) { for (int m = 0; m < NUM_MPE_SOURCES; m++) {
                                    tempExpressionParams[m] =
           expressionParams->params[m].getCurrentValue() >> 16;
                                }
    ...
    // Note-off
      ...
      if (notesAuditioned.searchExact(note) != -1) {
        ...
        if (noteRow) {
          ...
          if (shouldRecordNotes && ((playbackHandler.recording == RECORDING_ARRANGEMENT &&
                                     instrumentClip->isArrangementOnlyClip()) ||
      ...
      if (noteRow) {
        // MPE-controlled params are a bit special in that we can see (via this
        // note-off) when the user has removed their finger and won't be sending
        // more values. So, let's unlatch those params now.
        ExpressionParamSet* mpeParams = noteRow->paramManager.getExpressionParamSet();
        if (mpeParams) { mpeParams->cancelAllOverriding(); }
      }

      // We want to make sure we sent the note-off even if it didn't think
      // auditioning was happening. This is to stop a stuck note if MIDI thru
      // was on and they're releasing the note while still holding learn to
...
void MelodicInstrument::offerReceivedPitchBend(
  ...
  if (midiInput.equalsDevice(fromDevice)) {
    ...
    forMasterChannel:
      ...
      if (type == INSTRUMENT_TYPE_MIDI_OUT) {
        // .. and it's outputting on the same channel as this MIDI message came
        // in, don't do MIDI thru!
        if (doingMidiThru && ((MIDIInstrument*)this)->channel == channel) {
          *doingMidiThru = false;
        }
      }

      // Still send the pitch-bend even if the Output is muted. MidiInstruments
      // will check for and block this themselves

      int32_t newValue = (int32_t)(((uint32_t)data1 | ((uint32_t)data2 << 7)) - 8192)
                         << 18; // Was 16... why?
      processParamFromInputMIDIChannel(CC_NUMBER_PITCH_BEND,
                                       newValue,
      ...
      mpeX:
        ...
        int32_t value32 = (int32_t)value16 << 16; // Unlike for whole-Instrument pitch bend, this
                                                  // per-note kind is a modulation *source*, not
                                                  // the "preset" value for the parameter!
        polyphonicExpressionEventPossiblyToRecord(modelStackWithTimelineCounter,
                                                  value32,
                                                  0,
                                                  channel,
...
void MelodicInstrument::offerReceivedCC(
    ModelStackWithTimelineCounter* modelStackWithTimelineCounter,
    MIDIDevice*                    fromDevice,
    uint8_t                        channel,
    uint8_t                        ccNumber,
  ...
  if (midiInput.equalsDevice(fromDevice)) {
    ...
    forMasterChannel:
      ...
      // Still send the cc even if the Output is muted. MidiInstruments will
      // check for and block this themselves
      ...
      mpeY:
        if (ccNumber == 74) { // All other CCs are not supposed to be used for
                              // Member Channels, for anything.
          int32_t value32 = (value - 64) << 25;
          polyphonicExpressionEventPossiblyToRecord(modelStackWithTimelineCounter,
                                                    value32,
                                                    1,
...
void MelodicInstrument::offerReceivedAftertouch(
    ModelStackWithTimelineCounter* modelStackWithTimelineCounter,
    MIDIDevice*                    fromDevice,
    int                            channel,
    int                            value,
  ...
  if (midiInput.equalsDevice(fromDevice)) {
    ...
    forMasterChannel:
      ...
      // Still send the aftertouch even if the Output is muted. MidiInstruments
      // will check for and block this themselves
      ...
      if (noteCode != -1) {
        ...
        // We wouldn't be here if this was MPE input, so we know this incoming
        // polyphonic aftertouch message is allowed
      }

      // Or, channel pressure
      else {
        processParamFromInputMIDIChannel(CC_NUMBER_AFTERTOUCH,
                                         valueBig,
                                         modelStackWithTimelineCounter);
      }
    }
    ...
    // Or if MPE enabled...
      ...
      // Only if a "channel pressure" message (which with MPE of course refers
      // to ideally just one key). Non-MPE "polyphonic key pressure" messages
      // are not allowed in MPE currently.
      if (noteCode == -1) {
...
bool MelodicInstrument::isNoteRowStillAuditioningAsLinearRecordingEnded(NoteRow* noteRow) {
  return (notesAuditioned.searchExact(noteRow->y) != -1) &&
         (earlyNotes.searchExact(noteRow->y) == -1);
}
...
void MelodicInstrument::stopAnyAuditioning(ModelStack* modelStack) {
  ...
  earlyNotes.empty(); // This is fine, though in a perfect world we'd prefer to
                      // just mark the notes as no longer active
  if (activeClip)
...
void MelodicInstrument::beginAuditioningForNote(ModelStack*    modelStack,
  ...
  ModelStackWithNoteRow* modelStackWithNoteRow =
      modelStack->addTimelineCounter(activeClip)->addNoteRow(0, NULL);
...
ModelStackWithAutoParam* MelodicInstrument::getParamToControlFromInputMIDIChannel(
    int                            cc,
  ...
  if (!mpeParams) {
    return modelStack->addParam(NULL, NULL, 0,
                                NULL); // Crude way of saying "none".
  }
...
// Big part of this function is that it can decide to call
// possiblyCloneForArrangementRecording().
void MelodicInstrument::processParamFromInputMIDIChannel(
    int                            cc,
    int32_t                        newValue,
  ...
  if (modelStack->timelineCounterIsSet()) {
    ...
    if (view.modLength && modelStack->getTimelineCounter() ==
  ...
  ModelStackWithThreeMainThings* modelStackWithThreeMainThings =
      modelStackWithNoteRow->addOtherTwoThings(toModControllable(),
  ...
  if (modelStackWithParam->autoParam) {
    modelStackWithParam->autoParam->setValuePossiblyForRegion(
        ...
        modPos,
        modLength,
        false); // Don't delete nodes in linear run, cos this might need to be
                // outputted as MIDI again
  }
}
...
// Ok this is similar to processParamFromInputMIDIChannel(), above, but for MPE.
// It's different because one input message might have multiple AutoParams it
// applies to (i.e. because the member channel might have multiple notes /
// NoteRows). And also because the AutoParam is allowed to not exist at all -
// e.g. if there's no NoteRow for the note
// - but we still want to cause a sound change in response to the message.
void MelodicInstrument::polyphonicExpressionEventPossiblyToRecord(
    ModelStackWithTimelineCounter* modelStack,
    int32_t                        newValue,
  ...
  // If recording, we send the new value to the AutoParam, which will also sound
  ...
  if (modelStack->timelineCounterIsSet()) { // &&
    ...
    for (int n = 0; n < arpeggiator.notes.getNumElements(); n++) {
      ...
      if (arpNote->inputCharacteristics[whichCharacteristic] ==
          channelOrNoteNumber) { // If we're actually identifying by
                                 // MIDI_CHARACTERISTIC_NOTE, we could do a much
                                 // faster search, but let's not bother - that's
                                 // only done when we're receiving MIDI
                                 // polyphonic aftertouch messages, and there's
                                 // hardly much to search through.
        ModelStackWithNoteRow* modelStackWithNoteRow =
            ((InstrumentClip*)modelStack->getTimelineCounter())
                ->getNoteRowForYNote(arpNote->inputCharacteristics[MIDI_CHARACTERISTIC_NOTE],
                                     modelStack); // No need to create - it should already exist
                                                  // if they're recording a note here.
        NoteRow* noteRow = modelStackWithNoteRow->getNoteRowAllowNull();
        if (noteRow) {
          bool success = noteRow->recordPolyphonicExpressionEvent(modelStackWithNoteRow,
                                                                  newValue,
  ...
  // Or if not recording, just sound the change ourselves here (as opposed to
  // the AutoParam doing it).
  else {
    polyphonicExpressionEventOnChannelOrNote(newValue,
                                             whichExpressionDimension,
                                             channelOrNoteNumber,
                                             whichCharacteristic);
  }
