  ...
  if (currentUIMode == UI_MODE_VIEWING_RECORD_ARMING) {
    ...
    *rows = 0;
    for (int yDisplay = 0; yDisplay < displayHeight; yDisplay++) {
      ...
      if (clip && !clip->armedForRecording) { *rows |= (1 << yDisplay); }
    }
    return true;
...
void SessionView::focusRegained() {
  ...
  redrawClipsOnScreen(doingRender); // We want this here, not just in opened(), because after
                                    // coming back from loadInstrumentPresetUI, need to at least
                                    // redraw, and also really need to re-render stuff in case
                                    // note-tails-being-allowed has changed
...
int SessionView::buttonAction(int x, int y, bool on, bool inCardRoutine) {
    ...
    if (on) {
      ...
      if (Buttons::isButtonPressed(recordButtonX, recordButtonY)) {
        ...
        if (!playbackHandler.playbackState) {
          ...
          currentSong->clearArrangementBeyondPos(arrangerView.xScrollWhenPlaybackStarted,
                                                 action); // Want to do this before setting up
                                                          // playback or place new instances
          int error = currentSong->placeFirstInstancesOfActiveClips(
          ...
          arrangement.playbackStartedAtPos =
              arrangerView.xScrollWhenPlaybackStarted; // Have to do this after
                                                       // setting up playback
        ...
        if (playbackHandler.recording == RECORDING_ARRANGEMENT) {
          ...
          currentSong->resumeClipsClonedForArrangementRecording(); // Must call before
                                                                   // calling
                                                                   // getArrangementRecordPos(),
                                                                   // cos that detaches
                                                                   // the cloned Clip
        ...
        if (proposedStartPos > MAX_SEQUENCE_LENGTH - clip->loopLength) {
          numericDriver.displayPopup(HAVE_OLED ? "Clip would breach max arrangement length" :
                                                 "CANT");
          return ACTION_RESULT_DEALT_WITH;
        }
        ...
        arrangement.rowEdited(output,
                              proposedStartPos,
                              proposedStartPos + clip->loopLength,
                              NULL,
                              newInstance);
  ...
  changeInstrumentType:
    if (on && currentUIMode == UI_MODE_CLIP_PRESSED_IN_SONG_VIEW &&
  ...
  if (xDisplay < displayWidth) {
      ...
      if (!Buttons::isShiftButtonPressed()) {
        ...
        if (currentUIMode == UI_MODE_NONE) {
          ...
          holdingRecord:
            ...
            // We may or may not be doing a tempoless record and need to finish
            // that up.
            if (playbackHandler.playbackState && currentPlaybackMode == &session) {
              ...
              // If source clip currently recording, arm it to stop (but not if
              // tempoless recording)
              if (playbackHandler.isEitherClockActive() &&
                  sourceClip->getCurrentlyRecordingLinearly() && !sourceClip->armState) {
                session.toggleClipStatus(sourceClip,
                                         &clipIndex,
                                         false,
              ...
              Clip* overdub = currentSong->createPendingNextOverdubBelowClip(sourceClip,
                                                                             clipIndex,
                                                                             newOverdubNature);
              if (overdub) {
                ...
                // Since that was all effective, let's exit out of
                // UI_MODE_VIEWING_RECORD_ARMING too
                if (currentUIMode == UI_MODE_VIEWING_RECORD_ARMING) {
                  uiTimerManager.unsetTimer(TIMER_UI_SPECIFIC);
                  currentUIMode = UI_MODE_NONE;
                  PadLEDs::reassessGreyout(false);
                ...
                // If we were doing a tempoless record, now's the time to stop
                // that and restart playback
                if (!playbackHandler.isEitherClockActive()) {
                  playbackHandler.finishTempolessRecording(true,
                                                           INTERNAL_BUTTON_PRESS_LATENCY,
                                                           false);
                }
          ...
          // If Clip present here...
            ...
            removePendingOverdub:
              if (sdRoutineLock)
                return ACTION_RESULT_REMIND_ME_OUTSIDE_CARD_ROUTINE; // Possibly
                                                                     // not
                                                                     // quite
                                                                     // necessary...
              removeClip(yDisplay);
          ...
          // Otherwise, try and create one
            ...
            // if (possiblyCreatePendingNextOverdub(clipIndex,
            // OVERDUB_EXTENDING)) return ACTION_RESULT_DEALT_WITH;
    ...
    // Release
      ...
      if (isUIModeActive(UI_MODE_CLIP_PRESSED_IN_SONG_VIEW)) {
        ...
        // If doing nothing, at least exit the submode - if this was that
        // initial press
        else {
          if (yDisplay == selectedClipPressYDisplay && xDisplay == selectedClipPressXDisplay) {
          justEndClipPress:
            if (sdRoutineLock)
              return ACTION_RESULT_REMIND_ME_OUTSIDE_CARD_ROUTINE; // If in card
                                                                   // routine,
                                                                   // might mean
                                                                   // it's still
                                                                   // loading an
      ...
      // In all other cases, then if also inside card routine, do get it to
      // remind us after. Especially important because it could be that the user
      // has actually pressed down on a pad, that's caused a new clip to be
      // created and preset to load, which is still loading right now, but the
      // uiMode hasn't been set to "holding down" yet and control hasn't been
      // released back to the user, and this is the user releasing their press,
      // so we definitely want to be reminded of this later after the above has
      // happened.
      else {
        if (sdRoutineLock) return ACTION_RESULT_REMIND_ME_OUTSIDE_CARD_ROUTINE;
      }
  ...
  // Or, status or section (aka audition) pads
    ...
    if (playbackHandler.playbackState && currentPlaybackMode == &arrangement) {
      ...
      if (xDisplay == displayWidth) {
        ...
        if (clip) { return view.clipStatusPadAction(clip, on, yDisplay); }
      }
  ...
  if (on) {
    ...
    if (isNoUIModeActive()) {
      ...
      if (Buttons::isShiftButtonPressed()) {
        ...
        do { oldSection = (oldSection + 1) % MAX_NUM_SECTIONS; } while (!sectionUsed[oldSection]);
...
void SessionView::drawSectionRepeatNumber() {
  ...
  char buffer[21];
  if (number == -1) {
    outputText = "Launch non-\nexclusively"; // Need line break cos line splitter doesn't
                                             // deal with hyphens.
  } else {
    outputText = buffer;
...
void SessionView::drawSectionSquare(uint8_t yDisplay, uint8_t thisImage[][3]) {
  ...
  if (!clip)
    ...
    if (view.midiLearnFlashOn &&
      ...
      // If user assigning MIDI controls and has this section selected, flash to
      // half brightness
...
int setPresetOrNextUnlaunchedOne(InstrumentClip* clip,
                                 int             instrumentType,
  ...
  *instrumentAlreadyInSong  = newInstrument && result.fileItem->instrumentAlreadyInSong;
  ...
  result.error = clip->setAudioInstrument(newInstrument,
                                          currentSong,
                                          true,
                                          NULL); // Does a setupPatching()
  if (result.error) {
...
doGetInstrument:
  ...
  ModelStackWithTimelineCounter* modelStackWithTimelineCounter =
      modelStack->addTimelineCounter(newClip);
...
void SessionView::replaceAudioClipWithInstrumentClip(int instrumentType) {
  ...
  if (instrumentType == INSTRUMENT_TYPE_SYNTH || instrumentType == INSTRUMENT_TYPE_KIT) {
    ...
    Instrument* newInstrument =
        currentSong->getNonAudioInstrumentToSwitchTo(instrumentType,
                                                     AVAILABILITY_INSTRUMENT_UNUSED,
                                                     0,
                                                     -1,
...
void SessionView::replaceInstrumentClipWithAudioClip() {
  ...
  Clip* newClip = currentSong->replaceInstrumentClipWithAudioClip(oldClip,
  ...
  currentSong->arrangementYScroll--; // Is our best bet to avoid the scroll
                                     // appearing to change visually
  ...
  uiNeedsRendering(this,
                   1 << selectedClipYDisplay,
                   1 << selectedClipYDisplay); // If Clip was in keyboard view,
                                               // need to redraw that
}
...
void SessionView::removeClip(uint8_t yDisplay) {
  currentSong->ensureAllInstrumentsHaveAClipOrBackedUpParamManager(
      "E373",
      "H373"); // Trying to narrow down H067 that Leo got, below.
  ...
  currentSong->ensureAllInstrumentsHaveAClipOrBackedUpParamManager("E067",
                                                                   "H067"); // Leo got a H067!!!!
}
...
void SessionView::redrawNumericDisplay() {
  ...
  if (playbackHandler.isEitherClockActive()) {
    ...
    if (currentPlaybackMode == &session) {
      ...
      if (getCurrentUI() == &loadSongUI) {
        ...
        if (currentUIMode != UI_MODE_HOLDING_SECTION_PAD) { goto yesDoIt; }
      }
    }
  ...
  nothingToDisplay:
    if (getCurrentUI() == this || getCurrentUI() == &arrangerView) {
      if (currentUIMode != UI_MODE_HOLDING_SECTION_PAD) { numericDriver.setText(""); }
    }
  }
...
// This gets called by redrawNumericDisplay() - or, if HAVE_OLED, it gets called
// instead, because this still needs to happen.
...
void SessionView::cloneClip(uint8_t yDisplayFrom, uint8_t yDisplayTo) {
  ...
  currentSong->sessionClips.insertClipAtIndex(
      newClip,
...
void SessionView::graphicsRoutine() {
  ...
  for (int yDisplay = 0; yDisplay < displayHeight; yDisplay++) {
    ...
    // Tempoless recording
      ...
      if (clip->getCurrentlyRecordingLinearly()) { // This would have to be true
                                                   // if we got here, I think?
        if (clip->type == CLIP_TYPE_AUDIO) {
      ...
      int     localScroll    = getClipLocalScroll(clip,
      ...
      if (clip->getCurrentlyRecordingLinearly()) {
        ...
        if (newTickSquare >= 0 &&
            (!clip->armState || xScrollBeforeFollowingAutoExtendingLinearRecording !=
  ...
  if (!anyLinearRecordingOnThisScreen && currentUIMode != UI_MODE_HORIZONTAL_SCROLL) {
    ...
    if (anyLinearRecordingOnNextScreen) {
      ...
      if (!currentUIMode && getCurrentUI() == this) {
        ...
        int32_t newXScroll = currentSong->xScroll[NAVIGATION_CLIP] +
                             currentSong->xZoom[NAVIGATION_CLIP] * displayWidth;
        horizontalScrollForLinearRecording(newXScroll);
      }
    }

    // Or if not, cancel following scrolling along, and go back to where we
    // started
    else {
      if (xScrollBeforeFollowingAutoExtendingLinearRecording != -1) {
...
bool SessionView::setupScroll(uint32_t oldScroll) {
  ...
  pendingUIRenderingLock = true; // Ok I'm sorta pretending that this is definitely previously
                                 // false, though only one caller of this function actually checks
                                 // for that. Should be ok-ish though...
  ...
  for (int yDisplay = 0; yDisplay < displayHeight; yDisplay++) {
    ...
    if (clip && clip->currentlyScrollableAndZoomable()) {
      ...
      if (moved) {
        ...
        clip->renderAsSingleRow(modelStackWithTimelineCounter,
                                this,
                                newLocalPos,
                                xZoom,
                                PadLEDs::imageStore[yDisplay][0],
                                PadLEDs::occupancyMaskStore[yDisplay]);
        anyMoved = true;
      }
...
uint32_t SessionView::getClipLocalScroll(Clip* clip, uint32_t overviewScroll, uint32_t xZoom) {
  return getMin((clip->loopLength - 1) / (xZoom * displayWidth) * xZoom * displayWidth,
                overviewScroll);
}
...
void SessionView::flashPlayRoutine() {
  ...
  for (int yDisplay = 0; yDisplay < displayHeight; yDisplay++) {
    Clip* clip = getClipOnScreen(yDisplay);
    if (clip && clip->armState) { whichRowsNeedReRendering |= (1 << yDisplay); }
  }
...
void SessionView::noteRowChanged(InstrumentClip* instrumentClip, NoteRow* noteRow) {
  ...
  if (currentUIMode == UI_MODE_HORIZONTAL_SCROLL)
    return; // Is this 100% correct? What if that one Clip isn't visually
            // scrolling?
...
uint32_t SessionView::getGreyedOutRowsNotRepresentingOutput(Output* output) {
  ...
  for (int yDisplay = 0; yDisplay < displayHeight; yDisplay++) {
    Clip* clip = getClipOnScreen(yDisplay);
    if (clip && clip->output == output) { rows &= ~(1 << yDisplay); }
  }
  return rows;
}
...
bool SessionView::renderMainPads(uint32_t whichRows,
  ...
  for (int yDisplay = 0; yDisplay < displayHeight; yDisplay++) {
    if (whichRows & (1 << yDisplay)) {
      bool success = renderRow(modelStack,
                               yDisplay,
                               image[yDisplay],
                               occupancyMask[yDisplay],
                               drawUndefinedArea);
...
bool SessionView::renderRow(ModelStack* modelStack,
  ...
  if (clip) {
    ...
    // If user assigning MIDI controls and this Clip has a command assigned,
    // flash pink
    if (view.midiLearnFlashOn &&
      ...
      for (int xDisplay = 0; xDisplay < displayWidth; xDisplay++) {
        // We halve the intensity of the brightness in this case, because a lot
        // of pads will be lit, it looks mental, and I think one user was having
        // it cause his Deluge to freeze due to underpowering.
      ...
      if (view.thingPressedForMidiLearn == MIDI_LEARN_MELODIC_INSTRUMENT_INPUT &&
          ...
          view.learnedThing ==
              &((MelodicInstrument*)clip->output)->midiInput) { // Should be fine even if output
                                                                // isn't a MelodicInstrument
...
void SessionView::transitionToViewForClip(Clip* clip) {
  ...
  if (!clip) {
    ...
    // If there was no previous one (e.g. because we just loaded the Song), do
    // nothing.
    if (!clip || clip->section == 255) { return; }
  }
  currentSong->currentClip = clip;
  int clipPlaceOnScreen =
      getMax((int16_t)-1, getMin((int16_t)displayHeight, getClipPlaceOnScreen(clip)));

  currentSong->xScroll[NAVIGATION_CLIP] = getClipLocalScroll(clip,
                                                             currentSong->xScroll[NAVIGATION_CLIP],
                                                             currentSong->xZoom[NAVIGATION_CLIP]);
  ...
  if (clip->type == CLIP_TYPE_INSTRUMENT) {
    ...
    if (((InstrumentClip*)clip)->onKeyboardScreen) {
      ...
      instrumentClipView.recalculateColours(); // Won't have happened automatically because we
                                               // haven't begun the "session"
      instrumentClipView.renderMainPads(0xFFFFFFFF,
                                        &PadLEDs::imageStore[1],
                                        &PadLEDs::occupancyMaskStore[1],
                                        false);
      instrumentClipView.renderSidebar(0xFFFFFFFF,
                                       &PadLEDs::imageStore[1],
                                       &PadLEDs::occupancyMaskStore[1]);

      instrumentClipView.fillOffScreenImageStores(); // Important that this is done after
                                                     // currentSong->xScroll is changed, above
...
void SessionView::finishedTransitioningHere() {
  ...
  renderSidebar(0xFFFFFFFF,
                &PadLEDs::imageStore[displayHeight],
                &PadLEDs::occupancyMaskStore[displayHeight]);
  PadLEDs::timerRoutine(); // What... why? This would normally get called from
                           // that...
}
...
void SessionView::playbackEnded() {
  ...
  for (int yDisplay = 0; yDisplay < displayHeight; yDisplay++) {
    ...
    if (clip && clip->type == CLIP_TYPE_AUDIO) {
      ...
      if (!audioClip->sampleHolder.audioFile) { whichRowsToReRender |= (1 << yDisplay); }
    }
  }
...
void SessionView::sampleNeedsReRendering(Sample* sample) {
  ...
  for (int c = bottomIndex; c < topIndex; c++) {
    Clip* thisClip = currentSong->sessionClips.getClipAtIndex(c);
    if (thisClip->type == CLIP_TYPE_AUDIO &&
        ((AudioClip*)thisClip)->sampleHolder.audioFile == sample) {
      int yDisplay = c - currentSong->songViewYScroll;
      uiNeedsRendering(this, (1 << yDisplay), 0);
    }
  }
  ...
  for (int yDisplay = 0; yDisplay < displayHeight; yDisplay++) {
    ...
    if (clip) {
      ...
      if (clip->muteMIDICommand.containsSomething() ||
          (view.thingPressedForMidiLearn == MIDI_LEARN_CLIP &&
           &clip->muteMIDICommand == view.learnedThing) ||
          currentSong->sections[clip->section].launchMIDICommand.containsSomething() ||
