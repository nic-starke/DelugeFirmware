...
static void clearIRQInterrupt(int irqNumber) {
  ...
  if (flagRead & (1 << irqNumber)) { INTC.IRQRR.WORD = flagRead & ~(1 << irqNumber); }
}
...
static void int_irq6(uint32_t sense) {
  ...
  triggerClockRisingEdgeTimes[triggerClockRisingEdgesReceived &
...
int_t main1(void) {
  ...
  *TCNT[TIMER_MIDI_GATE_OUTPUT] = 0u;
  ...
  // Original comment regarding above priority: "Must be greater than 9, so less
  // prioritized than USB interrupt, so that can still happen while this
  // happening. But must be lower number / more prioritized than MIDI UART TX
  // DMA interrupt! Or else random crash occasionally." But, I've now undone the
  // change in "USB sending as host now done in ISR too!" commit, which set it
  // to 11. That was causing the SD / UART lockups (checked and observed again
  // around V4.0.0-beta2), and was possibly only actually done in the first
  // place to help with my hack fix for what I thought was that USB "hardware
  // bug", which I ended up resolving later anyway.

  // Set up slow system timer - 33 ticks per millisecond (30.30303 microseconds
  // per tick) on A1
  disableTimer(TIMER_SYSTEM_SLOW);
  timerControlSetup(TIMER_SYSTEM_SLOW, 0, 1024);
  enableTimer(TIMER_SYSTEM_SLOW);

  // Set up fast system timer - 528 ticks per millisecond (1.893939 microseconds
  // per tick) on A1
  disableTimer(TIMER_SYSTEM_FAST);
  timerControlSetup(TIMER_SYSTEM_FAST, 0, 64);
  enableTimer(TIMER_SYSTEM_FAST);

  // Set up super-fast system timer - 33.792 ticks per microsecond (29.5928
  // nanoseconds per tick) on A1
  ...
  /* Configure IRQs detections on falling edge. Due to the presence of a
   * transistor, we want to read falling edges on the trigger clock rather than
   * rising. */
