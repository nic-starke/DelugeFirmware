...
SampleHolder::SampleHolder() {
  ...
  for (int l = 0; l < NUM_CLUSTERS_LOADED_AHEAD; l++) { clustersForStart[l] = NULL; }
}

SampleHolder::~SampleHolder() {

  // Don't call setSample() - that does writing to variables which isn't
  // necessary
...
void SampleHolder::setAudioFile(AudioFile* newSample,
                                bool       reversed,
  ...
  if (audioFile) {
    ...
    // If we're here as a result of the user having manually selected a new
    // file, set the zone to its actual length.
    if (manuallySelected) {
      startPos = 0;
      endPos   = lengthInSamples;
    }

    // Otherwise, simply make sure that the zone doesn't exceed the length of
    // the sample
    else {
...
void SampleHolder::claimClusterReasons(bool reversed, int clusterLoadInstruction) {
  ...
  // unassignAllReasons(); // This now happens as part of
  // reassessPosForMarker(), called below
  ...
  int startPlaybackAtByte =
      ((Sample*)audioFile)->audioDataStartPosBytes + startPlaybackAtSample * bytesPerSample;

  claimClusterReasonsForMarker(clustersForStart,
                               startPlaybackAtByte,
                               playDirection,
...
void SampleHolder::claimClusterReasonsForMarker(Cluster** clusters,
  ...
  for (int l = 0; l < NUM_CLUSTERS_LOADED_AHEAD; l++) { newClusters[l] = NULL; }

  // Populate new list
  for (int l = 0; l < NUM_CLUSTERS_LOADED_AHEAD; l++) {

    /*
    // If final one, only load it if posWithinCluster is at least a quarter of
    the way in if (l == NUM_SAMPLE_CLUSTERS_LOADED_AHEAD - 1) { if
    (playDirection == 1) { if (posWithinCluster < (sampleManager.clusterSize >>
    2)) break;
        }
        else {
            if (posWithinCluster > sampleManager.clusterSize -
    (sampleManager.clusterSize >> 2)) break;
        }
    }
    */
    ...
    newClusters[l] =
        sampleCluster->getCluster(((Sample*)audioFile), clusterIndex, clusterLoadInstruction);

    if (!newClusters[l])
      Uart::println("NULL!!");
    else if (clusterLoadInstruction == CLUSTER_LOAD_IMMEDIATELY_OR_ENQUEUE &&
             !newClusters[l]->loaded)
  ...
  // Replace old list
  for (int l = 0; l < NUM_CLUSTERS_LOADED_AHEAD; l++) {
    if (clusters[l]) { audioFileManager.removeReasonFromCluster(clusters[l], "E146"); }
