...
public:
  ...
  ~InstrumentClip();
  void increaseLengthWithRepeats(ModelStackWithTimelineCounter* modelStack,
                                 int32_t                        newLength,
  ...
  bool renderAsSingleRow(ModelStackWithTimelineCounter* modelStack,
  ...
  void stopAllNotesPlaying(ModelStackWithTimelineCounter* modelStack,
                           bool                           actuallySoundChange = true);
  void resumePlayback(ModelStackWithTimelineCounter* modelStack, bool mayMakeSound = true);
  void setPos(ModelStackWithTimelineCounter* modelStack,
              int32_t                        newPos,
  ...
  int      getNumNoteRows();
  void
  ensureInaccessibleParamPresetValuesWithoutKnobsAreZero(ModelStackWithTimelineCounter* modelStack,
                                                         Sound*                         sound);
  ...
  bool inScaleMode; // Probably don't quiz this directly - call
                    // isScaleModeClip() instead
  ...
  LearnedMIDI soundMidiCommand; // This is now handled by the Instrument, but for
                                // loading old songs, we need to capture and store this
  ...
  // These *only* store a valid preset number for the instrument-types that the
  // Clip is not currently on
  ...
  void                   lengthChanged(ModelStackWithTimelineCounter* modelStack,
                                       int32_t                        oldLength,
                                       Action*                        action = NULL);
  NoteRow*               createNewNoteRowForKit(ModelStackWithTimelineCounter* modelStack,
                                                bool                           atStart,
                                                int*                           getIndex = NULL);
  int                    changeInstrument(ModelStackWithTimelineCounter* modelStack,
  ...
  void                   detachFromOutput(ModelStackWithTimelineCounter* modelStack,
  ...
  void                   assignDrumsToNoteRows(ModelStackWithTimelineCounter* modelStack,
                                               bool                           shouldGiveMIDICommandsToDrums = false,
                                               int numNoteRowsPreviouslyDeletedFromBottom = 0);
  void                   unassignAllNoteRowsFromDrums(ModelStackWithTimelineCounter* modelStack,
  ...
  NoteRow*               getOrCreateNoteRowForYNote(int     yNote,
                                                    Song*   song,
                                                    Action* action          = NULL,
                                                    bool*   scaleAltered    = NULL,
                                                    int*    getNoteRowIndex = NULL);
  ModelStackWithNoteRow* getOrCreateNoteRowForYNote(int                            yNote,
                                                    ModelStackWithTimelineCounter* modelStack,
                                                    Action*                        action = NULL,
                                                    bool* scaleAltered                    = NULL);
  ...
  int  setAudioInstrument(Instrument*     newInstrument,
                          Song*           song,
  ...
  void    clearArea(ModelStackWithTimelineCounter* modelStack,
                    int32_t                        startPos,
                    int32_t                        endPos,
                    Action*                        action);
  int     getScaleType();
  ...
  ModelStackWithNoteRow* getNoteRowForDrumName(ModelStackWithTimelineCounter* modelStack,
                                               char const*                    name);
  ...
  int                    setNonAudioInstrument(Instrument*   newInstrument,
                                               Song*         song,
                                               ParamManager* newParamManager = NULL);
  int                    setInstrument(Instrument*     newInstrument,
                                       Song*           song,
                                       ParamManager*   newParamManager,
                                       InstrumentClip* favourClipForCloningParamManager = NULL);
  ...
  int                    appendClip(ModelStackWithTimelineCounter* thisModelStack,
                                    ModelStackWithTimelineCounter* otherModelStack);
  void                   instrumentBeenEdited();
  Instrument*            changeInstrumentType(ModelStackWithTimelineCounter* modelStack,
  ...
  void getSuggestedParamManager(Clip*                     newClip,
  ...
  void    finishLinearRecording(ModelStackWithTimelineCounter* modelStack,
  ...
  void    quantizeLengthForArrangementRecording(ModelStackWithTimelineCounter* modelStack,
                                                int32_t                        lengthSoFar,
  ...
  ModelStackWithNoteRow*
       duplicateModelStackForClipBeingRecordedFrom(ModelStackWithNoteRow* modelStack,
...
private:
  ...
  void deleteEmptyNoteRowsAtEitherEnd(bool                           onlyIfNoDrum,
                                      ModelStackWithTimelineCounter* modelStack,
                                      bool                           mustKeepLastOne       = true,
                                      bool                           keepOnesWithMIDIInput = true);
  ...
  bool possiblyDeleteEmptyNoteRow(NoteRow* noteRow,
                                  bool     onlyIfNoDrum,
                                  Song*    song,
                                  bool     onlyIfNonNumeric   = false,
