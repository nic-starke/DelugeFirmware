...
void MenuItemMidiCommand::drawPixelsForOled() {
  ...
  if (!command->containsSomething()) {
    OLED::drawString("Command unassigned",
                     0,
                     yPixel,
    ...
    if (command->device) { deviceString = command->device->getDisplayName(); }
    OLED::drawString(deviceString,
                     0,
                     yPixel,
                     OLED::oledMainImage[0],
                     OLED_MAIN_WIDTH_PIXELS,
