...
public:
  ...
  virtual bool
  performSave(bool mayOverwrite = false) = 0; // Returns true if success, or if otherwise dealt
                                              // with (e.g. "overwrite" context menu brought up)
  void focusRegained();
  ...
  bool canSeeViewUnderneath() final {
    return (DELUGE_MODEL == DELUGE_MODEL_40_PAD);
  }
  int timerCallback();
