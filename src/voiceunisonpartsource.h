...
public:
  ...
  bool     noteOn(Voice*                    voice,
                  ...
                  uint8_t                   synthMode);
  void     unassign();
  bool     getPitchAndSpeedParams(Source*                   source,
                                  VoiceSamplePlaybackGuide* voiceSource,
                                  uint32_t*                 phaseIncrement,
                                  uint32_t*                 timeStretchRatio,
                                  uint32_t*                 noteLengthInSamples);
  uint32_t getSpeedParamForNoSyncing(Source* source,
                                     int32_t phaseIncrement,
                                     int32_t pitchAdjustNeutralValue);

  uint32_t oscPos; // FKA phase. No longer used for Sample playback / rate
                   // conversion position. Only waves, including wavetable.
