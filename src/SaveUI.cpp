...
/*
// TODO: in the future, there may be a case to be made for moving this to
LoadOrSaveUI.
...
void SaveUI::displayText(bool blinkImmediately) {
    ...
    if (enteredText.isEmpty() && !currentFolderIsEmpty) {
        ...
        numericDriver.setTextAsSlot(currentSlot, currentSubSlot,
currentFileExists, true, numberEditPos);
    }
...
void SaveUI::enterKeyPress() {
  ...
  } // Previously had &&currentFolderIsEmpty ... why?
...
int SaveUI::buttonAction(int x, int y, bool on, bool inCardRoutine) {
  ...
  // Select encoder button - we want to override default behaviour here and
  // potentially do nothing, so user doesn't save over something by accident.
  else if (x == selectEncButtonX && y == selectEncButtonY && currentFileItem &&
