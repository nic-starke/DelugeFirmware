...
public:
  ...
  uint32_t pos;
  uint8_t  state; // You may not set this directly, even from this class. Call
                  // setState()
  int32_t  lastValue;
  ...
  int32_t  render(uint32_t        numSamples,
                  uint32_t        attack,
                  uint32_t        decay,
                  uint32_t        sustain,
                  uint32_t        release,
                  const uint16_t* releaseTable);
  void     unconditionalRelease(uint8_t  typeOfRelease           = ENVELOPE_STAGE_RELEASE,
