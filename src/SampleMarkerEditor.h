...
// This is just for when we're editing a Sample's loop points etc. It mostly
// makes use of WaveformBasicNavigator, which itself makes use of
// WaveformRenderer. However, I've implemented this without any class
// inheritance
...
private:
  ...
  void renderForOneCol(int           xDisplay,
                       uint8_t       thisImage[displayHeight][displayWidth + sideBarWidth][3],
                       MarkerColumn* cols);
  void renderMarkersForOneCol(int     xDisplay,
                              uint8_t thisImage[displayHeight][displayWidth + sideBarWidth][3],
                              MarkerColumn* cols);
};
