...
public:
  ...
  // It's much easier to store local copies of the most recent of these, so we
  // never have to go doing complex quizzing of the arp, or MPE params, which we
  // otherwise would have to do regularly.
  int32_t monophonicPitchBendValue;
  int32_t polyPitchBendValue;

  uint8_t cachedBendRanges[2]; // Cache these here just in case there's no
                               // ParamManager - because CVInstruments don't do
