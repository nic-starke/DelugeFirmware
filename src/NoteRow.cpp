...
int NoteRow::beenCloned(ModelStackWithNoteRow* modelStack, bool shouldFlattenReversing) {
  // No need to clone much stuff - it's been automatically copied already as a
  // block of memory.
  ...
  bool flatteningReversingNow =
      (shouldFlattenReversing &&
       getEffectiveSequenceDirectionMode(modelStack) == SEQUENCE_DIRECTION_REVERSE);
  ...
  if (flatteningReversingNow && numNotes) {
    NoteVector oldNotes = notes; // Sneakily and temporarily clone this - still
                                 // pointing to the old NoteRow's notes' memory.
    notes.init();
    ...
    oldNotes.init(); // Because this is about to get destructed, we need to stop
                     // it pointing to the old NoteRow's notes' memory, cos we
                     // don't want that getting deallocated.
  }
  ...
  if (shouldFlattenReversing && sequenceDirectionMode != SEQUENCE_DIRECTION_PINGPONG)
    sequenceDirectionMode = SEQUENCE_DIRECTION_OBEY_PARENT;
  // Pingponging won't have been flattened by a single clone. And we may be
  // about to flatten it with a generateRepeats(), so need to keep this
  // designation for now.
...
uint8_t NoteRow::getSquareType(int32_t                squareStart,
  ...
  addNewNote:
    ...
    newNote->setVelocity(
        ((Instrument*)((Clip*)modelStack->getTimelineCounter())->output)->defaultVelocity);
    ...
    if (i + 1 < notes.getNumElements()) {
      ...
      newNote->setLength(
          getMin(desiredNoteLength, notes.getElement(0)->pos + effectiveLength - newNote->pos));
    }

    // Record consequence
    if (action)
      action->recordNoteExistenceChange((InstrumentClip*)modelStack->getTimelineCounter(),
    ...
    if (clipCurrentlyPlaying && !muted) {
      ...
      // If the play-pos is inside this note, see if we'd like to attempt a
      // late-start of it
  ...
  // Start by finding the last note to begin *before the right-edge* of this
  // square
  ...
  // Or if the note starts left of this square, or there's no note there which
  // means we'll look at the final one wrapping around...
  else {
    bool wrapping = (i == -1);
    ...
    if (noteEnd > squareStart) {
      ...
      // We have a tail! But if tails aren't allowed, cut it off and make a new
      // note here instead
      if (!allowNoteTails) {
        if (action)
    ...
    // Or if note's tail does not overlap this square, making this square
    // completely empty...
    else {
      ...
      if (extendPreviousNoteIfPossible && allowNoteTails) {
        ...
        // If previously extended into this square, cut it to go to just the
        // start of the square
...
int NoteRow::addCorrespondingNotes(int32_t                targetPos,
                                   int32_t                newNotesLength,
                                   uint8_t                velocity,
                                   ModelStackWithNoteRow* modelStack,
                                   bool                   allowNoteTails,
                                   Action*                action) {
  ...
  int numScreensToAddNoteOn =
      (uint32_t)(effectiveLength + wrapEditLevel - posWithinEachScreen - 1) / wrapEditLevel;

  // Allocate all the working memory we're going to need for this operation -
  // that's arrays for searchPos and resultingIndexes
  int32_t* __restrict__ searchTerms =
      (int32_t*)
          generalMemoryAllocator.alloc(numScreensToAddNoteOn * sizeof(int), NULL, false, true);
  if (!searchTerms) return ERROR_INSUFFICIENT_RAM;

  // Make new NoteVector to copy into as we go through each screen - and
  // pre-allocate the max amount of memory we might need
  NoteVector newNotes;
  ...
  // Search for all those positions. Will return the indexes of the next note at
  // GREATER_OR_EQUAL to that position
  ...
  // For each screen we're going to add a note on, copy all notes prior to (and
  ...
  for (int screenIndex = 0; screenIndex < numScreensToAddNoteOn; screenIndex++) {
    ...
    // Copy all notes before this one (which is to the right of the insertion
    // pos, so if there is already one at the insertion pos, we'll be copying
    // that too.
    ...
    // If haven't copied any Notes since last insertion (and remember, there
    // won't always be an insertion because there might already be a note
    // there), then we do want to insert a new one, but not modify any old ones
    if (!sourceNote) goto addNewNote;

    // If we're here cos there's a sourceNote, that always means there's also a
    // destNote, and the two refer to the same note from the user's perspective
    // (though in different arrays).

    // Or if the last thing we did was copy some Notes (including possibly the
    // last one being already *at* the insertion pos for the previous screen)...
    prevNoteMaxLength = posThisScreen - destNote->pos;
    if (prevNoteMaxLength > 0) {

      // Make sure that Note (in its copied destination) doesn't extend past the
      // insertion pos...
      if (destNote->length > prevNoteMaxLength) { destNote->setLength(prevNoteMaxLength); }

    addNewNote:
      sourceNote = NULL; // Reset it - we now (briefly) will not have copied any
                         // notes since inserting this one
      ...
      // If there are more notes coming up to the right, just make sure we're
      // not gonna eat into em
      ...
      // Otherwise, make sure we don't eat into the first note when we wrap back
      // around
      else {
        Note* firstNote = notes.getElement(0);
        newLength =
            getMin(newNotesLength, (firstNote->pos + effectiveLength - posWithinEachScreen));
      }
  ...
  // For the final note (which could be either one we copied or one we inserted)
  // make sure it doesn't wrap around and eat into the first note (which also
  // could have been either copied or inserted)
  if (destNote && nextIndexToCopyTo >= 2) {
    ...
    if (destNote->length > maxLengthThisNote) { destNote->setLength(maxLengthThisNote); }
  }

  // Delete any Notes we initially created (we create in bulk for efficiency)
  // but then ended up not using
  int numToDelete = newNotesInitialSize - nextIndexToCopyTo;
  if (numToDelete > 0) { newNotes.deleteAtIndex(nextIndexToCopyTo, numToDelete); }
...
// This gets called after we've scrolled and attempted to drag notes. And for
// recording. If you supply an Action, it'll add an individual
// ConsequenceNoteExistenceChange. Or, you can supply NULL and do something else
// yourself. Returns distanceToNextNote, or 0 on fail.
int NoteRow::attemptNoteAdd(int32_t                pos,
  ...
  if (length <= 0)
    length = 1; // Special case where note added at the end of linear record
                // must temporarily be allowed to eat into note at position 0
  ...
  if (action)
    action->recordNoteExistenceChange((InstrumentClip*)modelStack->getTimelineCounter(),
                                      modelStack->noteRowId,
                                      newNote,
                                      CREATE); // This only gets called (action is only supplied)
                                               // when drag-scrolling Notes
...
int NoteRow::clearArea(int32_t                areaStart,
  ...
  // It'd also be tempting to just abort if there were no notes within the
  // area-to-clear, but remember, we also might need to shorten note tails
  ...
  // Populate big list of all the positions we want to search. There's one each
  // for the start *and* the end of each area
  int32_t areaStartThisScreen =
      areaStart + actuallyExtendNoteAtStartOfArea; // If actuallyExtendNoteAtStartOfArea,
                                                   // then only clear from 1 tick later
  ...
  for (int screenIndex = 0; screenIndex < numScreens; screenIndex++) {
    ...
    // If we copied some Notes just then, before the start of this area but
    // after the previous one...
    if (destNote) {

      int32_t areaBeginPosThisScreen = screenIndex * wrapEditLevel + areaStart;

      // If we're actually looking for a Note right at the area-start, to extend
      // to the full width...
      if (actuallyExtendNoteAtStartOfArea) {

        // If the Note does actually begin right at the area-start, make it the
        // full area length
        if (destNote->pos == areaBeginPosThisScreen) {
          destNote->setLength(areaWidth);
        }

        // Or if it started earlier, then we'll flag everything to do with this
        // area, including any clearing
        else {
          continue; // Crucially skip the final line of the loop, below, which
                    // says don't copy any more til after the area
        }
      }

      // Or, if we're just doing a plain old clear, then ensure the last Note
      // doesn't extend into the area
      else {
    ...
    // Or if we didn't just copy any Notes, then only in the case that we're
    // doing note-extending, don't bother clearing the area
    else {
  ...
  // If we copied some Notes after the final area, ensure the final Note doesn't
  // wrap around into the first area
  if (destNote) {
...
void NoteRow::recordNoteOff(uint32_t               noteOffPos,
                            ModelStackWithNoteRow* modelStack,
                            Action*                action,
                            int                    velocity) {
  ...
  if (wrapping) {
    ...
    if (getEffectiveSequenceDirectionMode(modelStack) == SEQUENCE_DIRECTION_PINGPONG) {
      note           = notes.getElement((notes.getNumElements() - 1) *
                              reversed); // Will be 0 if playing forwards
...
modifyNote:
  // Don't allow a note to be created that's the length of the whole Clip.
  // Because there's a scenario that can lead to this happening erroneously
  if (newLength < effectiveLength) {
    ...
    // Doing a wrap while reversed is unique because we have to move our note
  ...
  if (((InstrumentClip*)modelStack->getTimelineCounter())->wrapEditing &&
    ...
    if (newLength != thisNote->length) {
      ...
      clearArea(areaStart,
                ...
                modelStack,
                action,
                ((InstrumentClip*)modelStack->getTimelineCounter())->getWrapEditLevel(),
                true);
    }
  }

  // If not wrap-editing, it's easy - we can just set the length of this one
  // Note
  else {
    if (action)
      action->recordNoteArrayChangeIfNotAlreadySnapshotted(
...
int NoteRow::editNoteRepeatAcrossAllScreens(int32_t                editPos,
  ...
  // The "area" is the notes that'll get deleted as a result of the nudge *if*
  // there's a note in each place
  ...
  for (int screenIndex = 0; screenIndex < numScreens; screenIndex++) {
    ...
    // If on the final screen, we need to make sure our "area" doesn't overlap
    // that
    if (screenIndex == numScreens - 1) {
      ...
      if (areaEndPosThisScreen > effectiveLength) {
        ...
        // If that's ended up 0 or negative, there's nothing for us to do.
        // Though there'd probably be no harm if this check wasn't here, and in
        // a perfect world maybe we'd check this before deciding how many search
        // terms?
    ...
    if (oldNumNotesThisScreen) {
      ...
      // Now go over all those repeating notes and set their positions so
      // they're spaced out
  ...
  // Record change, stealing the old note data
  if (action) {
    // We "definitely" store the change, because unusually, we may want to
    // revert individual Consequences in the Action one by one
...
int NoteRow::nudgeNotesAcrossAllScreens(int32_t                editPos,
  ...
  if (nudgeOffset >= 0 && (numScreens - 1) * wrapEditLevel + editPos + 1 == effectiveLength) {
    ...
    if (lastSourceNote->pos == effectiveLength - 1) {
      ...
      *destNote     = *lastSourceNote;
      ...
      // Check whether that's chopped off any Note at pos 0 which wouldn't
      // itself get nudged to the right
      Note* __restrict__ firstSourceNote = notes.getElement(0);
      if (firstSourceNote->pos == 0 && editPos != 0) { nextIndexToCopyFrom = 1; }
      ...
      destNote = NULL;     // We don't want to do anything further to this Note
                           // below like we normally would after copying a nudge one
      numSourceNotes--;    // Don't copy the last one when we get to it - we've
                           // just done it
  ...
  for (int screenIndex = 0; screenIndex < numScreens; screenIndex++) {
    ...
    // If nudging right...
      ...
      if (destNote) {
        ...
        // If there was a nudge note, it will be the last one we copied. If
        // so...
        if (destNote->pos == preNudgeNotePos) {
          ...
          nextIndexToCopyFrom =
              searchTerms[(screenIndex << 1) + 1]; // Skip any Notes that'll get deleted (only now
                                                   // that we know we actually have nudged a Note)
          ...
          if (nextIndexToCopyFrom < numSourceNotes) { // If there's more Notes
            ...
            maxLength = nextNote->pos - postNudgeNotePos; // If it's too close, restrict
                                                          // the nudged note's length
  ...
  if (wrappingLeft) {
    ...
    if (nextIndexToCopyTo > 0) {
      ...
      // Perhaps that's right where we want our Note - in which case overwrite
      // it
  ...
  // Or a less extreme case where we just nudged the very first Note left and it
  // didn't wrap - but we still need to check the final Note's length
...
// note is usually supplied as NULL, and that means you don't get the
// lift-velocity
...
void NoteRow::renderRow(TimelineView* editorScreen,
  ...
  do {
    // Start by presuming we'll do all the squares now... though we may decide
    // in a second to do a smaller number now, and come back for more batches
    xEndNow = xEnd;
    ...
    for (int square = xStartNow; square < xEndNow; square++) {
      ...
      // Work out its endPos...
      int32_t thisSquareEndPos = editorScreen->getPosFromSquare(square + 1, xScroll, xZoom) -
                                 effectiveRowLength * whichRepeat;

      // If we're drawing repeats and the square ends beyond end of Clip...
      if (drawRepeats && thisSquareEndPos > effectiveRowLength) {

        // If this is the first square we're doing now, we can take a shortcut
        // to skip forward some repeats
        if (square == xStartNow) {
        ...
        // Otherwise, we'll come back in a moment to do the rest of the repeats
        // - just record that we want to stop rendering before this square until
        // then
        else {
          xEndNow = square;
          break;
    ...
    int squareStartPos = editorScreen->getPosFromSquare(xStartNow, xScroll, xZoom) -
                         effectiveRowLength * whichRepeat;
    ...
    xStartNow = xEndNow;
    whichRepeat++;
    // TODO: if we're gonna repeat, we probably don't need to do the multi
    // search again...
  } while (xStartNow != xEnd); // This will only do another repeat if we'd modified xEndNow,
                               // which can only happen if drawRepeats
}
...
int32_t NoteRow::processCurrentPos(ModelStackWithNoteRow* modelStack,
  ...
  // If we have an independent play-pos, we need to check if we've reached the
  // end (right of left) of this NoteRow.
  if (hasIndependentPlayPos()) {

    // Remember, lastProcessedPosIfIndependent has already been incremented
    // during call to Clip::incrementPos().

    // Deal with recording from session to arrangement.
    if (loopLengthIfIndependent // Only if independent *length* - which isn't
                                // always the case when we have independent play
                                // pos.
      ...
      if (otherNoteRow) {
        ...
        int whichRepeatThisIs =
            (uint32_t)effectiveLength / (uint32_t)otherNoteRow->loopLengthIfIndependent;

        appendNoteRow(modelStack,
    ...
    if (playingReversedNow) {
      ...
      // If actually got left of zero, it's time to loop, and normally this
      // wouldn't happen if pingponging because direction changes right when we
      // hit zero.
      ...
      // Normally we do the pingpong when we hit pos 0, so the direction will
      // change and we'll start going right again now, in time for NoteRows and
      // stuff to know the direction as they're processed and predict what notes
      // we're going to hit next etc.
      if (!lastProcessedPosIfIndependent) { // Possibly only just became the
                                            // case
        repeatCountIfIndependent++;
        if (getEffectiveSequenceDirectionMode(modelStack) == SEQUENCE_DIRECTION_PINGPONG) {
          lastProcessedPosIfIndependent = -lastProcessedPosIfIndependent; // In case it did get left
                                                                          // of zero.
      ...
      if (ticksTilEnd <= 0) { // Yes, note it might not always arrive directly at the end. When
                              // (Audio) Clip length is shortened, the lastProcessedPos is
                              // altered, but it could be that many swung ticks have actually
                              // passed since we last processed, so there might be a big jump
                              // forward and we end up past the loop point.

        lastProcessedPosIfIndependent -= effectiveLength;
        repeatCountIfIndependent++;

        if (getEffectiveSequenceDirectionMode(modelStack) == SEQUENCE_DIRECTION_PINGPONG) {
          // Normally we'll have hit the exact loop point, meaning
          // lastProcessedPos will have wrapped to 0, above. But just in case we
          // went further, and need to wrap back to somewhere nearish the
  ...
  if (paramManager.mightContainAutomation()) {
    ...
    if (didPingpong) { paramManager.notifyPingpongOccurred(modelStackWithThreeMainThings); }

    bool mayInterpolate =
        drum ? drum->type == DRUM_TYPE_SOUND : (clip->output->type == INSTRUMENT_TYPE_SYNTH);
    // We'll not interpolate for CV, just for efficiency. Since our CV output
    // steps are limited anyway, this is probably reasonably reasonable.

    paramManager.processCurrentPos(modelStackWithThreeMainThings,
                                   ticksSinceLast,
                                   playingReversedNow,
                                   didPingpong,
                                   mayInterpolate);
  ...
  if (muted) {
  noFurtherNotes:
    // If this NoteRow has independent length set, make sure we come back at its
    // end. Otherwise, the Clip handles this.
    if (loopLengthIfIndependent) {
      ...
      if (!playingReversedNow)
        ...
        ticksTilNextNoteEvent = loopLengthIfIndependent; // Wrap from 0 up to length. Need to do
                                                         // this when playing reversed.
    }
  }
    ...
    if (isAuditioning(modelStack)) {
      ...
      // If they've also just recorded a note and it was quantized later, we do
      // need to keep an eye out for it, despite the fact that we're
      // auditioning.
    ...
    if (soundingStatus == STATUS_SEQUENCED_NOTE) {
      ...
      if (!notes.getNumElements()) {
      stopNote:
        stopCurrentlyPlayingNote(modelStack, true, thisNote); // Ideally (but optionally) supply the
                                                              // note, so lift-velocity can be used
      } else {

        // First search for the (should-be-existent) note which begins "earlier"
        // than our current pos.
        int32_t searchLessThan = effectiveCurrentPos;

        // If we're playing backwards, we want to include notes whose pos is
        // *equal* to the current pos, which would mean ending right now (cos
        // we're playing backwards).
        searchLessThan += (bool)playingReversedNow;
        // Buuut, a special condition for pingponging to allow notes touching
        // the right-end of this Clip / NoteRow to just keep sounding as the
        // direction changes. Nah actually don't do that.
        //&& (effectiveCurrentPos ||
        // getEffectiveSequenceDirectionMode(modelStack) !=
        ...
        // If playing reversed, we have to check that we've even reached this
        // note yet. Maybe we haven't, and there's actually no note that should
        // be currently playing (e.g. after an undo).
        if (playingReversedNow) {
        ...
        int noteLateEdgePos = thisNote->pos; // Depending on play direction, this will be either
                                             // the left or right edge of the Note.
        ...
        // If note ends right now (or even earlier, which shouldn't normally
        ...
        if (ticksTilNextNoteEvent <= 0) {
          ...
          if (thisNote->pos == 0 && thisNote->length == effectiveLength) {
            ...
            // If it's a cut-mode sample, though, we want it to stop, so it can
            // get retriggered again from the start. Same for time-stretching -
            // although those can loop themselves, caching comes along and
            // stuffs that up, so let's just stop em.
    ...
    // Now, if no note is playing (even if that only became the case just now as
    // one ended, above)...
    ...
    currentlyOff:
      ...
      // Or, more normally, when some Notes do exist...
        ...
        // We want to search for the next note which will sound (which depends
        // on our play direction)...
        int32_t searchPos = effectiveCurrentPos;
        int     nextNoteI;

        bool allowingNoteTailsNow; // We'll only bother populating this if
                                   // playingReversedNow
        ...
        if (playingReversedNow) {
          ...
          if (searchPos == 0) {
            if (!allowingNoteTailsNow) {
              if (!alreadySearchedBackwards && notes.getNumElements() &&
                  !notes.getElement(0)->pos) {
                nextNoteI = 0;
          ...
          // Or normal case, for all other currentPos's...
            ...
            // Basic case for say synth sounds, when playing reversed, is we
            // want to stop sounding each Note at its "pos". So it's appropriate
            // to just search further-left than that. But for one-shot sounds,
            // we do still sound the Note at the "pos", so we need to include
            // that in the search so we'll see that Note now. But, not if we've
            // actually already sounded it just before (on previous loop of this
            // code) and are now wanting to search past it.
        ...
        if (nextNoteI < 0 || nextNoteI >= notes.getNumElements()) {
          ...
          // If playing reversed, we might still want to try again, taking
          // notice of a "wrapping" note at the right-hand end of this NoteRow.
      ...
      gotValidNoteIndex:
        ...
        int32_t newTicksTil =
            nextNote->pos - effectiveCurrentPos; // Assumes we're playing forwards - it'll get
        ...
        if (playingReversedNow) {
          ...
          // So long as notes have tails (e.g. they're synth sounds, not
          // one-shot), we of course want to start sounding the note at its
          // right-most edge, which we'll hit a while sooner.
          if (allowingNoteTailsNow) { newTicksTil -= nextNote->length; }

          // If it's past the wrap-point, we don't care where the Note is
          // anymore. But we might care where the wrap-point is.
        ...
        if (newTicksTil <= 0) {
          ...
          // If playing reversed and not allowing note tails (i.e. doing
          // one-shot drums), we're already at the left-most edge of the note,
          // so immediately stop it again
          // - and we'll then come back near here and look at the *next* Note
          ...
          newTicksTil = nextNote->length; // Want this even if we're "skipping"
                                          // playing the note (why exactly?)
        }
...
void NoteRow::attemptLateStartOfNextNoteToPlay(ModelStackWithNoteRow* modelStack, Note* note) {
  ...
  if (currentlyPlayingReversed &&
      !((InstrumentClip*)modelStack->getTimelineCounter())->allowNoteTails(modelStack))
    return; // Not supported yet - would be tricky cos by the time it's "late",
            // we'd be left of note->pos

  int32_t numSwungTicksInSinceLastActionedOne =
      playbackHandler.getNumSwungTicksInSinceLastActionedSwungTick();

  int32_t noteBeginsSoundingAtPos = note->pos;
  if (currentlyPlayingReversed)
    noteBeginsSoundingAtPos += note->length; // We already know that allowNoteTails() is true, from
                                             // check above
  ...
  // We also have to check and wrap if swungTicksAgo is >= length, which can
  // happen when doing an instant-(late)-start because the last actioned swung
  // tick might have been a while ago, and so clip->currentPos is sitting at
  // some fairly high number (leading to a big swungTicksAgo), because the
  // expectation is it's about to wrap around when it gets a big increment when
  // we do action a tick very shortly. Seems to happen when instant-launching
  // long Clips with short quantization, e.g. the one-bar when no other Clips
  // active. This obviously isn't really ideal - could we do something like
  // "actioning" the tick before we get here, to keep the numbers making more
  // sense? Fix only applied 1 Mar 2021, for V3.1.6
  else if (swungTicksAgo >= effectiveLoopLength)
    swungTicksAgo -= effectiveLoopLength;

  int32_t swungTicksBeforeLastActionedOne =
      swungTicksAgo - numSwungTicksInSinceLastActionedOne; // This may end up negative, meaning
                                                           // after; that's ok

  int32_t noteOnTime =
      playbackHandler.getInternalTickTime(playbackHandler.lastSwungTickActioned -
  ...
  if (playbackHandler.playbackState & PLAYBACK_CLOCK_INTERNAL_ACTIVE)
      timeAgo += (((uint64_t)ticksAgo * currentSong->timePerTimerTickFraction +
  4294967296 - playbackHandler.lastTimerTickDoneFraction) >> 32); // This
  sometimes ends up 1 sample off, I think - not quite sure why

  // TODO: that logic won't deliver a perfect result if the tempo has changed
  though, or there's an imperfect external clock source, or swing stuff...

  if (playbackHandler.lastInternalTickDone > 0) {
      timeAgo += AudioEngine::audioSampleTimer -
  playbackHandler.timeLastInternalTick;
  }
  */
  ...
  if ((sound &&
       (allows = sound->allowsVeryLateNoteStart(((InstrumentClip*)modelStack->getTimelineCounter()),
                                                thisParamManager))) ||
    ...
    if (!allows) {
      ...
      timeAgo = 0; // For one-shot samples, we don't want the sample to start
                   // part-way through.
    }
...
// Note may be NULL if it's a note-off, in which case you don't get
// lift-velocity
void NoteRow::playNote(bool                   on,
                       ModelStackWithNoteRow* modelStack,
                       Note*                  thisNote,
                       int32_t                ticksLate,
    ...
    if (on) {
        ...
        if (output->type == INSTRUMENT_TYPE_SYNTH) {
          if (((SoundInstrument*)output)->noteIsOn(getNoteCode()) &&
              ((SoundInstrument*)output)
                  ->allowNoteTails(
                      modelStack
                          ->addOtherTwoThings(((Clip*)modelStack->getTimelineCounter())
                                                  ->output->toModControllable(),
                                              &modelStack->getTimelineCounter()->paramManager)
                          ->addSoundFlags())) {
            // Alright yup the note's still sounding from before - no need to do
            // anything
          }

          // Or if those conditions failed, then yes we need to send the note
          // again
          else
      ...
      doSentNoteForMelodicInstrument:
        ...
        storePendingNoteOn:
          ...
          pendingNoteOnList->pendingNoteOns[pendingNoteOnList->count].noteRowId =
              modelStack->noteRowId;
          pendingNoteOnList->pendingNoteOns[pendingNoteOnList->count].velocity =
              thisNote->getVelocity();
  ...
  // And for all cases of a note-on, remember that that's what's happening, for
  // later.
  if (on) {
...
void NoteRow::setLength(ModelStackWithNoteRow* modelStack,
  ...
  if (playbackHandler.isEitherClockActive() && modelStack->song->isClipActive(clip)) {
    ...
    currentlyPlayingReversedIfIndependent = playingReversedBefore; // Will have no effect if is/was
                                                                   // in fact obeying parent Clip

    if (!hadIndependentPlayPosBefore) { repeatCountIfIndependent = clip->repeatCount; }

    if (shouldResumePlaybackOnNoteRowLengthSet) { resumePlayback(modelStack, true); }
  }
}
...
void NoteRow::trimToLength(uint32_t newLength, ModelStackWithNoteRow* modelStack, Action* action) {
  ...
  trimNoteDataToNewClipLength(newLength,
                              (InstrumentClip*)modelStack->getTimelineCounter(),
                              action,
                              modelStack->noteRowId);
...
void NoteRow::trimNoteDataToNewClipLength(uint32_t        newLength,
  ...
  if (newNumNotes) {
    ...
    basicTrim:
      ...
      if (numToDelete >= 0) { notes.deleteAtIndex(newNumNotes, numToDelete); }
    ...
    // Or if action...
      ...
      // If action already has a backed up snapshot for this param, can still
      // just do a basic trim
  ...
  // Or if no notes afterwards...
    ...
    if (action)
      action->recordNoteArrayChangeIfNotAlreadySnapshotted(clip,
                                                           noteRowId,
                                                           &notes,
                                                           true); // Steal them
    notes.empty(); // Delete them - in case no action, or the above chose not to
                   // steal them
  }
}
...
bool NoteRow::generateRepeats(ModelStackWithNoteRow* modelStack,
  ...
  paramManager.generateRepeats(modelStackWithThreeMainThings,
                               oldLoopLength,
                               newLoopLength,
                               pingponging);
  ...
  if (sequenceDirectionMode == SEQUENCE_DIRECTION_PINGPONG) {
    // Pingponging is being flattened out, and although there are arguments
  ...
  // Deal with single droning note case - but don't do this for samples in CUT
  // or STRETCH mode
  if (numNotesBefore == 1 && notes.getElement(0)->length == oldLoopLength) {
    ...
    if (!sound || (!sound->hasCutModeSamples(paramManagerNow) &&
                   !sound->hasAnyTimeStretchSyncing(paramManagerNow))) {
  ...
  if (pingponging) {
    ...
    // This is crude and lazy, but the amount of elements I'll create is rounded
    // way up, and we'll delete any extras, below.
    ...
    if (noteTailsAllowed) {
      ...
      // If there is, we need to edit its length now, to reflect how it would
      // have played in a pingpong scenario.
      if (anyWrapping) {
        lastNote->length = lengthBeforeWrap
                           << 1; // TODO: make sure this doesn't eat past the end-point...
      }
      ...
      for (int r = 1; r < numRepeatsRoundedUp; r++) { // For each repeat
        ...
        for (int iNewWithinRepeat = 0; iNewWithinRepeat < numNotesBefore; iNewWithinRepeat++) {
          ...
          if (newPos >= newLoopLength)
            break; // Crude way of stopping part-way through the final repeat if
  ...
  // Largely because this function generates multiple repeats in one go, unlike
  // clone() and appendNoteRow() which only do one, we aim for efficiency by
  // looking at each of the "source" notes in turn, and for each of those, look
  // at all its copies. The result may be that iteration dependence gets
  // flattened entirely (and you can force this by supplying numRepeatsRounded
  // as 0), or if the Clip/NoteRow is still not going to be as long as all of a
  // note's iterations, then its iteration dependence will be modified to keep
  // it sounding the same, to the extent possible.
  ...
  for (int i = 0; i < numNotesBefore; i++) {
    ...
    if (probability > NUM_PROBABILITY_VALUES) {
      ...
      int newNumFullLoops =
          numRepeatsRounded ? newLoopLength / (uint32_t)(oldLoopLength * divisor) : 1;

      int whichFullLoop         = 0; // If newNumFullLoops is 0, this will never get above 0
      int whichRepeatWithinLoop = 0;

      // Go through each copy of that original note (i.e. each repeat of
      // oldLength)
      while (true) {
        ...
        if (pingponging && (whichRepeatTotal & 1)) {
          ...
          if (noteTailsAllowed) {
            ...
            if (thisRepeatedNotePos < 0)
              thisRepeatedNotePos =
                  oldLoopLength - lengthAfterWrap; // This means we've got the wrapped note,
                                                   // while reversing.
          } else {
        ...
        if (whichRepeatWithinLoop != iterationWithinDivisorWithinRepeat) {
          ...
          // If deleting the original one, those are what our for loop is
          // currently iterating through, so alter that process
          ...
          if (newNumFullLoops == 0) {
            // I think this bit is for, like, if we had a note doing 1of6, and
            // we're doing two repeats total on this Clip, well then to keep it
            // sounding the same, we'd now need the note to be a 1of3.
            int newDivisor = (uint32_t)divisor / (uint32_t)numRepeatsRounded;
            if (newDivisor <= 1) goto switchOff;
            int newIterationWithinDivisor =
...
void NoteRow::resumePlayback(ModelStackWithNoteRow* modelStack, bool clipMayMakeSound) {
  if (noteRowMayMakeSound(clipMayMakeSound) && soundingStatus == STATUS_OFF &&
      !isAuditioning(modelStack)) {

    if (!notes.getNumElements()) return;

    int32_t effectiveActualCurrentPos = getLivePos(modelStack);

    // See if our play-pos is inside of a note, which we might want to try
    // playing...
    ...
    if (noteEnd > effectiveActualCurrentPos) { attemptLateStartOfNextNoteToPlay(modelStack, note); }
  }
...
void NoteRow::silentlyResumePlayback(ModelStackWithNoteRow* modelStack) {
  ...
  int32_t effectiveCurrentPos =
      modelStack->getLastProcessedPos(); // Why did we not opt for the "actual" one?
  ...
  if (noteEnd > effectiveCurrentPos) { soundingStatus = STATUS_SEQUENCED_NOTE; }
}
...
int NoteRow::readFromFile(int*            minY,
  ...
  drum = (Drum*)0xFFFFFFFF; // Code for "no drum". We swap this for a real
                            // value soon

  int newBendRange = -1; // Temp variable for this because we can't actually create the
                         // expressionParams before we know what kind of Drum (if any) we have.
  ...
  while (*(tagName = storageManager.readNextTagOrAttributeName())) {
    ...
    if (!strcmp(tagName, "muted")) {
      ...
      drum = (Drum*)storageManager.readTagOrAttributeValueInt(); // Sneaky - we store an integer
                                                                 // in place of this pointer,
                                                                 // then swap it back to
      ...
      readAutomationUpToPos = loopLengthIfIndependent; // So we can read automation right up to the
                                                       // actual length of this NoteRow.
    }

    else if (!strcmp(tagName, "sequenceDirection")) {
      sequenceDirectionMode =
          stringToSequenceDirectionMode(storageManager.readTagOrAttributeValue());
    }
      ...
      // Sneaky sorta hack for 2016 files - allow more params to be loaded into
      // a ParamManager that already had some loading done by the Drum
    ...
    doReadNoteData:
      ...
      while (true) {
        ...
        if (numElementsToAllocateFor <= 0) {
          ...
          // See how many more chars before the end of the cluster. If there are
          // any...
          uint32_t charsRemaining = storageManager.getNumCharsRemainingInValue();
          if (charsRemaining) {

            // Allocate space for the right number of notes, and remember how
            // long it'll be before we need to do this check again
            numElementsToAllocateFor = (uint32_t)(charsRemaining - 1) / noteHexLength + 1;
            notes.ensureEnoughSpaceAllocated(numElementsToAllocateFor); // If it returns false... oh
                                                                        // well. We'll fail later
          }
        }
  ...
  *minY = y + 1;
  ...
  if (newBendRange != -1) {
    ...
    if (expressionParams) { expressionParams->bendRanges[BEND_RANGE_FINGER_LEVEL] = newBendRange; }
  }
...
void NoteRow::writeToFile(int drumIndex, InstrumentClip* clip) {
  ...
  if (forKit) { storageManager.writeAttribute("colourOffset", getColourOffset(clip)); }

  if (loopLengthIfIndependent) storageManager.writeAttribute("length", loopLengthIfIndependent);
  if (sequenceDirectionMode != SEQUENCE_DIRECTION_OBEY_PARENT)
    storageManager.writeAttribute("sequenceDirection",
                                  sequenceDirectionModeToString(sequenceDirectionMode));
  ...
  if (expressionParams && forKit) {
    storageManager.writeAttribute("bendRange",
...
void NoteRow::setDrumToNull(ModelStackWithTimelineCounter const* modelStack) {
    if (paramManager.containsAnyParamCollections()) {
        modelStack->song->backUpParamManager((SoundDrum*)drum,
(Clip*)modelStack->getTimelineCounter(), &paramManager);
...
// Can handle NULL newParamManager. In fact, only one caller sends an actual
// one.
void NoteRow::setDrum(Drum*                  newDrum,
  ...
  if (paramManager.containsAnyMainParamCollections()) {
    modelStack->song->backUpParamManager((SoundDrum*)drum,
                                         ...
                                         &paramManager,
                                         false); // Don't steal expression params - we'll keep
                                                 // them here with this NoteRow.

    paramManager.forgetParamCollections();
  }

  drum = (SoundDrum*)newDrum; // Better set this temporarily for this call. See comment above
                              // for why we can't set it permanently yet

  if (newParamManager) { // Only one caller actually sends this.
    paramManager.stealParamCollectionsFrom(
        newParamManager); // Don't bother stealing expression/MPE params -
                          // newParamManager is actually literally brand new in
                          // the one case that it gets supplied.
    if (paramManager.containsAnyParamCollectionsIncludingExpression()) {
      trimParamManager(modelStack);
    }
  }

  // Set drum to NULL first - it's crucial because we're about to call the
  // following functions:
  // - currentSong->findParamManagerForDrum() looks for NoteRows with this Drum.
  // We don't want to be one just yet, or we'll be the one that's found
  drum = NULL;
  ...
  if (newDrum && newDrum->type == DRUM_TYPE_SOUND) {
    ...
    if (!paramManager.containsAnyMainParamCollections()) {
      if (favourClipForCloningParamManager) {
        ...
        if (noteRow) {
          ...
          // That might not work if there was insufficient RAM, but we'll still
      ...
      if (!paramManager.containsAnyMainParamCollections()) {
        ...
        drum = soundDrum; // Better set this temporarily for this call. See comment
                          // above for why we can't set it permanently yet
        ...
        if (!paramManager.containsAnyMainParamCollections()) {
          ...
          // If there is no NoteRow for this Drum... Oh dear. Make a blank
          // ParamManager and pray?
          else {
            int error = paramManager.setupWithPatching();
            if (error)
              numericDriver.freezeWithError(
                  "E010"); // If there also was no RAM, we're really in trouble.
            Sound::initParams(&paramManager);
    ...
    drum = soundDrum;
    soundDrum->ensureInaccessibleParamPresetValuesWithoutKnobsAreZero(
    ...
    if (playbackHandler.isEitherClockActive() && paramManager.mightContainAutomation() &&
      ...
      paramManager.setPlayPos(clip->getLivePos(),
                              modelStackWithThreeMainThings,
  ...
  if (drum) {
    ...
    // Copy bend range, if appropriate. This logic is duplicated in
...
void NoteRow::rememberDrumName() {
  ...
  if (drum && drum->type == DRUM_TYPE_SOUND) {
    ...
    while (*prevPointer) {
      ...
      // If we'd already stored the name we were gonna store now, no need to do
      // anything
      if ((*prevPointer)->name.equalsCaseIrrespective(&soundDrum->name)) { return; }

      prevPointer = &((*prevPointer)->next);
    }

    // If we're here, we're at the end of the list, didn't find an instance of
    // the name, and want to add it to the end of the list now
    void* drumNameMemory = generalMemoryAllocator.alloc(sizeof(DrumName));
    if (drumNameMemory) { *prevPointer = new (drumNameMemory) DrumName(&soundDrum->name); }
  }
}
...
int32_t NoteRow::getDistanceToNextNote(int32_t                      pos,
  ...
  goAgain:
    if (i == -1) { i = notes.getNumElements() - 1; }
  ...
  // If reversed, we need to actually consider the note's rightmost edge, where
  // its "length" ends.
  if (reversed) {
    ...
    if (distance <= 0) {
      ...
      // If there only is one note, there's no point looking to the next one -
      // just think about the next time we'll come around.
...
// Caller must call expectEvent() on the Clip, and paramManager->setPlayPos on
// this NoteRow, if (and only if) playbackHandler.isEitherClockActive().
void NoteRow::shiftHorizontally(int amount, ModelStackWithNoteRow* modelStack) {

  int32_t effectiveLength = modelStack->getLoopLength();

  if (paramManager.containsAnyParamCollectionsIncludingExpression()) {
    paramManager.shiftHorizontally(modelStack->addOtherTwoThingsAutomaticallyGivenNoteRow(),
                                   amount,
                                   effectiveLength);
  }
...
bool NoteRow::doesProbabilityExist(int32_t apartFromPos, int probability, int secondProbability) {
  ...
  for (int n = 0; n < notes.getNumElements(); n++) {
    ...
    if (note->pos != apartFromPos) {
      if (note->getProbability() == probability) { return true; }
      if (secondProbability != -1 && note->getProbability() == secondProbability) { return true; }
    }
  }
...
bool NoteRow::paste(ModelStackWithNoteRow* modelStack,
  ...
  // TODO: this could be done without all these many inserts, and could be
  // improved further by "stealing" the data into the action, above
  for (int n = 0; n < copiedNoteRow->numNotes; n++) {

    Note* noteSource = &copiedNoteRow->notes[n];

    int32_t newPos = modelStack->song->xScroll[NAVIGATION_CLIP] +
                     (int32_t)roundf((float)noteSource->pos * scaleFactor);

    // Make sure that with dividing and rounding, we're not overlapping the
    // previous note - or past the end of the screen / Clip
...
void NoteRow::giveMidiCommandsToDrum() {
  // Prior to V2.0, NoteRows had MIDI sound / mute commands. Now this belongs to
  // the Drum, so copy that over
  if (muteMIDICommand.containsSomething()) {
    if (!drum->muteMIDICommand.containsSomething()) { drum->muteMIDICommand = muteMIDICommand; }
    muteMIDICommand.clear();
  }

  if (midiInput.containsSomething()) {
    if (!drum->midiInput.containsSomething()) { drum->midiInput = midiInput; }
    midiInput.clear();
  }
}
...
int NoteRow::appendNoteRow(ModelStackWithNoteRow* thisModelStack,
  ...
  if (reversingNow) {
    ...
    if (clip->allowNoteTails(thisModelStack)) {
      ...
      // If there is a wrapped note, we need to edit the length of the copy of
      // it that already exists at the end of this NoteRow *before* we do the
      // appending.
  ...
  // We may not have ended up using all the elements we inserted, due to
  // iteration dependence, so delete any extra.
  int numExtraToDelete = notes.getNumElements() - insertIndex;
  if (numExtraToDelete) { notes.deleteAtIndex(insertIndex, numExtraToDelete); }
...
void NoteRow::resumeOriginalNoteRowFromThisClone(ModelStackWithNoteRow* modelStackOriginal,
  ...
  NoteRow* originalNoteRow = modelStackOriginal->getNoteRowAllowNull(); // It might be NULL - we'll
  ...
  bool stillSounding = (originalNoteRow && !originalNoteRow->muted &&
...
uint32_t NoteRow::getLivePos(ModelStackWithNoteRow const* modelStack) {
  ...
  if (!hasIndependentPlayPos()) { return clip->getLivePos(); }
  ...
  int32_t numSwungTicksInSinceLastActioned =
      playbackHandler.getNumSwungTicksInSinceLastActionedSwungTick() +
      clip->noteRowsNumTicksBehindClip;
  ...
  if (livePos < 0)
    livePos += modelStack->getLoopLength(); // Could happen if reversing and
                                            // currentPosHere is 0.
...
void NoteRow::getMPEValues(ModelStackWithNoteRow* modelStack, int16_t* mpeValues) {
  ...
  if (!mpeParams) {
    for (int m = 0; m < NUM_EXPRESSION_DIMENSIONS; m++) { mpeValues[m] = 0; }
    return;
  }
...
// This is obviously very inefficient, doing deletions for every "screen",
// sequentially. Also, pos is provided as the squareStart, but in a perfect
// world, we'd actually use the pos of the first note within that square - for
// each "screen"! Or something like that...
...
bool NoteRow::recordPolyphonicExpressionEvent(ModelStackWithNoteRow* modelStack,
                                              int32_t                newValueBig,
  ...
  ModelStackWithAutoParam* modelStackWithAutoParam =
      modelStack->addOtherTwoThingsAutomaticallyGivenNoteRow()->addParam(mpeParams,
                                                                         mpeParamsSummary,
                                                                         whichExpressionDimension,
                                                                         param);

  // Only if this exact TimelineCounter and NoteRow is having automation
  // step-edited, we can set the value for just a region.
  if (view.modLength && modelStackWithAutoParam->noteRowId == view.modNoteRowId &&
    ...
    // As well as just setting values now, InstrumentClipView keeps a record,
    // for in case the user then releases the note, in which case we'll want the
    // values from when they pressed hardest etc.
    instrumentClipView.reportMPEValueForNoteEditing(whichExpressionDimension, newValueBig);

    // And also, set the values now, for in case they're instead gonna stop
    // editing the note before releasing this MIDI note.
    ...
    int32_t distanceToNextNote = getDistanceToNextNote(livePos,
                                                       modelStackWithAutoParam,
                                                       modelStack->isCurrentlyPlayingReversed());
    int32_t distanceToNextNode =
        param->getDistanceToNextNode(modelStackWithAutoParam,
                                     livePos,
                                     modelStackWithAutoParam->isCurrentlyPlayingReversed());
    ...
    param->setCurrentValueInResponseToUserInput(
        ...
        livePos,
        false,
        doMPEMode); // Don't allow deletion of nodes in linear run. See comments
                    // above that function
  }
...
void NoteRow::setSequenceDirectionMode(ModelStackWithNoteRow* modelStack, int newMode) {
  ...
  // Or if now *not* pingponging...
    ...
    currentlyPlayingReversedIfIndependent =
        (newMode == SEQUENCE_DIRECTION_REVERSE); // Won't necessarily have an effect - if
                                                 // we're now set to obey-parent.

    // If we just changed direction...
    if (reversedBefore != modelStack->isCurrentlyPlayingReversed()) {
      lastProcessedPosIfIndependent = modelStack->getLoopLength() -
                                      lastProcessedPosIfIndependent; // Again, might have no effect.
