...
public:
  ...
  void setTextAsSlot(int16_t currentSlot,
  ...
  NumericLayerScrollingText*
       setScrollingText(char const* newText, int startAtPos = 0, int initialDelay = 600);
...
#if !HAVE_OLED
  int  encodeText(char const* newText,
                  uint8_t*    destination,
                  bool        alignRight,
                  uint8_t     drawDot              = 255,
                  bool        limitToDisplayLength = true,
                  int         scrollPos            = 0);
