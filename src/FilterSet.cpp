...
void FilterSet::renderHPF(int32_t*         outputSample,
  ...
  int32_t a = multiply_32x32_rshift32_rounded(filterSetConfig->divideByTotalMoveability,
                                              firstHPFOutput + feedbacksValue)
              << (4 + 1);
  ...
  hpfLPF1.doFilter(a - hpfHPF3.doFilter(a, filterSetConfig->hpfMoveability),
                   filterSetConfig->hpfMoveability);

  a = multiply_32x32_rshift32_rounded(a, filterSetConfig->hpfDivideByProcessedResonance)
      << (8 - 1); // Normalization
...
void FilterSet::renderHPFLong(int32_t*         outputSample,
  ...
  int32_t hpfDivideByTotalMoveabilityIncrement =
      (int32_t)(filterSetConfig->divideByTotalMoveability - hpfDivideByTotalMoveabilityNow) /
      (int32_t)numSamples;
  hpfDivideByTotalMoveabilityLastTime = filterSetConfig->divideByTotalMoveability;
  ...
  do {
    ...
    int32_t a = multiply_32x32_rshift32_rounded(hpfDivideByTotalMoveabilityNow,
                                                firstHPFOutput + feedbacksValue)
                << (4 + 1);
    ...
    a = multiply_32x32_rshift32_rounded(a, hpfDivideByProcessedResonanceNow)
        << (8 - 1); // Normalization
...
inline int32_t
FilterSet::do24dBLPFOnSample(int32_t input, FilterSetConfig* filterSetConfig, int saturationLevel) {

  // For drive filter, apply some heavily lowpassed noise to the filter
  // frequency, to add analog-ness
  ...
  int32_t moveability = filterSetConfig->moveability +
                        multiply_32x32_rshift32(filterSetConfig->moveability, noiseLastValue);

  int32_t feedbacksSum =
      (lpfLPF1.getFeedbackOutputWithoutLshift(filterSetConfig->lpf1Feedback) +
  ...
  // Note: in the line above, we "should" halve filterSetConfig->divideBy1plusg
  // to get it into the 1=1073741824 range. But it doesn't sound as good.
  // Primarily it stops us getting to full resonance. But even if we allow
  // further resonance increase, the sound just doesn't quite compare. Lucky I
  // discovered this by mistake

  int32_t x =
      multiply_32x32_rshift32_rounded(
          (input -
           (multiply_32x32_rshift32_rounded(feedbacksSum, filterSetConfig->processedResonance)
            << 3)),
          filterSetConfig->divideByTotalMoveabilityAndProcessedResonance)
      << 2;

  // Only saturate if resonance is high enough. Surprisingly, saturation makes
  // no audible difference until very near the point of feedback
  if (saturationLevel) { x = getTanHUnknown(x, saturationLevel); }

  return lpfLPF4.doFilter(
             lpfLPF3.doFilter(lpfLPF2.doFilter(lpfLPF1.doFilter(x, moveability), moveability),
                              moveability),
             moveability)
         << 1;
}

inline int32_t FilterSet::doDriveLPFOnSample(int32_t          input,
                                             FilterSetConfig* filterSetConfig,
                                             int              extraSaturation) {
  ...
  // We don't saturate the input anymore, because that's the place where we'd
  // get the most aliasing!
  int32_t x =
      multiply_32x32_rshift32_rounded(
          (input -
           (multiply_32x32_rshift32_rounded(feedbacksSum, filterSetConfig->processedResonance)
...
void FilterSet::renderLPFLong(int32_t*         startSample,
                              int32_t*         endSample,
                              FilterSetConfig* filterSetConfig,
                              uint8_t          lpfMode,
                              int              sampleIncrement,
  ...
  if (lpfMode == LPF_MODE_12DB) {
    ...
    do {
      ...
      int32_t feedbacksSum =
          lpfLPF1.getFeedbackOutput(filterSetConfig->lpf1Feedback) +
      ...
      // Only saturate if resonance is high enough. Surprisingly, saturation
      // makes no audible difference until very near the point of feedback
      if (true || filterSetConfig->processedResonance > 510000000) { // Re-check this?
        x = getTanHUnknown(x, 1 + extraSaturation);                  // Saturation
      }

      *currentSample =
          lpfLPF3.doAPF(lpfLPF2.doFilter(lpfLPF1.doFilter(x, moveability), moveability),
                        moveability)
          << 1;
  ...
  // Full ladder (regular)
    ...
    if (filterSetConfig->processedResonance >
        900000000) { // Careful - pushing this too high leads to crackling, only
                     // at the highest frequencies, and at the top of the
                     // non-saturating resonance range
      int32_t* currentSample = startSample;
      do {
  ...
  // Full ladder (drive)
    ...
    if (filterSetConfig->doOversampling) {
      int32_t* currentSample = startSample;
      do {
        // Linear interpolation works surprisingly well here - it doesn't lead
        // to audible aliasing. But its big problem is that it kills the highest
        // frequencies, which is especially noticeable when resonance is low.
        // This is because it'll turn all your high sine waves into triangles
        // whose fundamental is lower in amplitude. Now, if we immediately
        // downsampled that again with no filtering, no problem, because those
        // new really high harmonics that make it triangular are still there.
        // But after running it through the ladder filter, those get doubly
        // filtered out, leaving just the reduced-amplitude (fundamental) sine
        // waves where our old ones were. So instead, we need to do just a
        // little bit more and take one extra, previous sample into account in
        // our interpolation. This is enough to make the HF loss inaudible
        // - although we can still see it on the spectrum analysis.

        // Insanely just doubling up our input values to oversample works better
        // than fancy 3-sample interpolation. And even, making our
        // "interpolated" sample just a 0 and doubling the amplitude of the
        // actual sample works very nearly as well as this, but gives a little
        // bit more aliasing on high notes fed in.

        doDriveLPFOnSample(*currentSample, filterSetConfig, extraSaturationDrive);

        // Crude downsampling - just take every second sample, with no
        // anti-aliasing filter. Works fine cos the ladder LPF filter takes care
        // of lots of those high harmonics!
        int32_t outputSampleToKeep =
            doDriveLPFOnSample(*currentSample, filterSetConfig, extraSaturationDrive);

        // Only perform the final saturation stage on this one sample, which we
        // want to keep
