...
void MenuItemPatchCableStrength::renderOLED() {
  ...
  OLED::drawString(getSourceDisplayNameForOLED(s),
                   0,
                   yPixel,
  ...
  if (!destinationDescriptor.isJustAParam()) {
    // OLED::drawGraphicMultiLine(OLED::downArrowIcon, 0, yPixel, 8,
    // OLED::oledMainImage[0]);
    ...
    OLED::drawGraphicMultiLine(OLED::rightArrowIcon,
                               rightArrowX,
                               horizontalLineY - 2,
                               3,
                               OLED::oledMainImage[0]);
    ...
    OLED::drawString(getSourceDisplayNameForOLED(s2),
                     TEXT_SPACING_X * 2,
                     yPixel - 3,
                     OLED::oledMainImage[0],
                     OLED_MAIN_WIDTH_PIXELS,
  ...
  OLED::drawGraphicMultiLine(OLED::downArrowIcon,
                             destinationDescriptor.isJustAParam() ? 2 : (TEXT_SPACING_X * 2 + 2),
                             yPixel,
                             5,
                             OLED::oledMainImage[0]);
  yPixel += ySpacing;
  ...
  OLED::drawString(getPatchedParamDisplayNameForOled(p),
                   0,
                   yPixel,
                   OLED::oledMainImage[0],
                   OLED_MAIN_WIDTH_PIXELS,
  ...
  OLED::drawStringAlignRight(buffer,
                             extraY + OLED_MAIN_TOPMOST_PIXEL + 4 +
                                 destinationDescriptor.isJustAParam(),
                             OLED::oledMainImage[0],
                             OLED_MAIN_WIDTH_PIXELS,
                             18,
                             20);
...
ModelStackWithAutoParam* MenuItemPatchCableStrength::getModelStack(void* memory,
                                                                   bool  allowCreation) {
  ...
  ModelStackWithParamId* ModelStackWithParamId =
      modelStackWithParamCollection->addParamId(getLearningThing().data);
...
int MenuItemPatchCableStrength::checkPermissionToBeginSession(Sound*       sound,
                                                              int          whichThing,
  ...
  if (!sound->maySourcePatchToParam(
          ...
          ((ParamManagerForTimeline*)
               soundEditor.currentParamManager))) { // Note, that requires
...
MenuItem* MenuItemPatchCableStrength::selectButtonPress() {
  ...
  if (Buttons::isShiftButtonPressed()) {
    ...
    if (modelStack->autoParam) { modelStack->autoParam->deleteAutomation(action, modelStack); }
...
void MenuItemPatchCableStrengthRegular::drawValue() {
  ...
  uint8_t drawDot =
      patchCableSet->doesDestinationDescriptorHaveAnyCables(getLearningThing()) ? 3 : 255;
...
ParamDescriptor MenuItemPatchCableStrengthRegular::getLearningThing() {
  ...
  paramDescriptor.setToHaveParamAndSource(soundEditor.patchingParamSelected,
                                          sourceSelectionMenuRegular.s);
...
int MenuItemPatchCableStrengthRegular::checkPermissionToBeginSession(Sound*       sound,
  ...
  if (soundEditor.patchingParamSelected == PARAM_GLOBAL_VOLUME_POST_FX) {
    if (!sound->maySourcePatchToParam(
            getS(),
            soundEditor.patchingParamSelected,
...
uint8_t MenuItemPatchCableStrengthRegular::shouldBlinkPatchingSourceShortcut(int      s,
  ...
  // Or, if it's the source controlling the range of the source we're editing
  // for...
...
MenuItem*
MenuItemPatchCableStrengthRegular::patchingSourceShortcutPress(int  s,
...
ParamDescriptor MenuItemPatchCableStrengthRange::getLearningThing() {
  ParamDescriptor paramDescriptor;
  paramDescriptor.setToHaveParamAndTwoSources(soundEditor.patchingParamSelected,
                                              sourceSelectionMenuRegular.s,
...
MenuItem*
MenuItemPatchCableStrengthRange::patchingSourceShortcutPress(int  newS,
                                                             bool previousPressStillActive) {
  return (MenuItem*)0xFFFFFFFF;
}
...
int MenuItemFixedPatchCableStrength::checkPermissionToBeginSession(Sound*       sound,
                                                                   int          whichThing,
                                                                   MultiRange** currentRange) {
...
uint8_t MenuItemFixedPatchCableStrength::shouldBlinkPatchingSourceShortcut(
    int      s,
    uint8_t* sourceShortcutBlinkColours) {
...
MenuItem*
MenuItemFixedPatchCableStrength::patchingSourceShortcutPress(int s, bool previousPressStillActive) {
  sourceSelectionMenuRange.s = s;
  return &patchCableStrengthMenuRange;
}
