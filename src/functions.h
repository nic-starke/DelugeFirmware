...
inline int32_t signed_saturate_operand_unknown(int32_t val, int bits) {
  ...
  // Despite having this switch at the per-audio-sample level, it doesn't
  // introduce any slowdown compared to just always saturating by the same
  // amount!
  switch (bits) {
    case 31:
...
inline void renderRingmodSample(int32_t* __restrict__ thisSample,
                                int32_t amplitude,
                                int32_t waveValueA,
                                int32_t waveValueB) {
  *thisSample +=
      multiply_32x32_rshift32_rounded(multiply_32x32_rshift32(waveValueA, waveValueB), amplitude);
}
...
static void convertFloatToIntAtMemoryLocation(uint32_t* pos) {
  ...
  int32_t outputValue =
      (exponent >= 0) ? 2147483647 : (uint32_t)((readValue << 8) | 0x80000000) >> (-exponent);
...
inline int32_t interpolateTableSigned(uint32_t       input,
                                      int            numBitsInInput,
                                      const int16_t* table,
...
inline int32_t interpolateTableSigned2d(uint32_t       inputX,
                                        uint32_t       inputY,
                                        int            numBitsInInputX,
                                        int            numBitsInInputY,
                                        const int16_t* table,
  ...
  int32_t value1 = interpolateTableSigned(inputX,
                                          numBitsInInputX,
                                          &table[whichValue * tableSizeOneRow],
                                          numBitsInTableSizeX);
  int32_t value2 = interpolateTableSigned(inputX,
                                          numBitsInInputX,
                                          &table[(whichValue + 1) * tableSizeOneRow],
                                          numBitsInTableSizeX);
...
inline int32_t
getTanHAntialiased(int32_t input, uint32_t* lastWorkingValue, unsigned int saturationAmount) {

  uint32_t workingValue = (uint32_t)lshiftAndSaturateUnknown(input, saturationAmount) + 2147483648u;

  int32_t toReturn =
      interpolateTableSigned2d(workingValue, *lastWorkingValue, 32, 32, &tanH2d[0][0], 7, 6) >>
      (saturationAmount + 1);
...
inline void drawSquare(uint8_t  squareColour[],
                       int      intensity,
                       uint8_t  square[],
                       uint8_t* occupancyMask,
                       int      occupancyFromWhichColourCame) {

  int modifiedIntensity = intensity; //(intensity * occupancyFromWhichColourCame) >> 6; // Out of
                                     // 65536

  // Make new colour being drawn into this square marginalise the colour already
  // in the square
  int colourRemainingAmount = 65536;

  // We know how much colour we want to add to this square, so constrain any
  // existing colour to the remaining "space" that it may still occupy
  int maxOldOccupancy = (65536 - modifiedIntensity) >> 10;

  // If the square has more colour in it than it's allowed to retain, then plan
  // to reduce it
