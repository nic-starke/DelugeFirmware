...
void Slicer::doSlice() {
  ...
  char modelStackMemory[MODEL_STACK_MAX_SIZE];
  {
    ...
    if (firstDrum->nameIsDiscardable) { firstDrum->name.set("1"); }
    ...
    bool doEnvelopes = (lengthMSPerSlice >= 90); // Only do fades in and out if we've got at
                                                 // least 100ms to play with
    ...
    firstDrum->sources[0].repeatMode =
        (lengthMSPerSlice < 2002) ? SAMPLE_REPEAT_ONCE : SAMPLE_REPEAT_CUT;
    ...
    firstRange->sampleHolder.claimClusterReasons(firstDrum->sources[0].sampleControls.reversed,
                                                 CLUSTER_ENQUEUE);
    if (doEnvelopes) {
      ParamCollectionSummary* summary = modelStack->paramManager->getPatchedParamSetSummary();
      ModelStackWithParamId*  modelStackWithParamId =
          modelStack->addParamCollectionAndId(summary->paramCollection,
                                              summary,
    ...
    for (int i = 1; i < numClips; i++) {
      ...
      if (doEnvelopes) {
        paramManager.getPatchedParamSet()
            ...
            .setCurrentValueBasicForSetup(getParamFromUserValue(PARAM_LOCAL_ENV_0_ATTACK, 1));
        if (i != numClips - 1) {
          paramManager.getPatchedParamSet()
              ->params[PARAM_LOCAL_ENV_0_RELEASE]
              .setCurrentValueBasicForSetup(getParamFromUserValue(PARAM_LOCAL_ENV_0_RELEASE, 1));
        }
      }
  ...
  // New NoteRows have probably been created, whose colours haven't been grabbed
  // yet.
