...
void ParamSet::copyOverridingFrom(ParamSet* otherParamSet) {
  ...
  int numParams = getNumParams();
  for (int p = 0; p < numParams; p++) { params[p].copyOverridingFrom(&otherParamSet->params[p]); }
}
...
inline void
ParamSet::checkWhetherParamHasInterpolationNow(ModelStackWithParamCollection const* modelStack,
                                               int                                  p) {
...
void ParamSet::setPlayPos(uint32_t pos, ModelStackWithParamCollection* modelStack, bool reversed) {
  ...
  int32_t oldValue = param->getCurrentValue();
  param->setPlayPos(pos, modelStackWithAutoParam,
                    reversed); // May change interpolation state
...
void ParamSet::appendParamCollection(ModelStackWithParamCollection* modelStack,
  ...
  params[p].appendParam(&otherParamSet->params[p],
                        oldLength,
...
void ParamSet::trimToLength(uint32_t                       newLength,
  ...
  if (!params[p].isAutomated()) { paramHasNoAutomationNow(modelStack, p); }
...
void ParamSet::backUpParamToAction(int                            p,
                                   Action*                        action,
                                   ModelStackWithParamCollection* modelStack) {
  AutoParam*               param                   = &params[p];
  ModelStackWithAutoParam* modelStackWithAutoParam = modelStack->addAutoParam(p, param);
...
void PatchedParamSet::notifyParamModifiedInSomeWay(ModelStackWithAutoParam const* modelStack,
                                                   int32_t                        oldValue,
                                                   bool                           automationChanged,
                                                   bool                           automatedBefore,
                                                   bool                           automatedNow) {
  ParamSet::notifyParamModifiedInSomeWay(modelStack,
                                         oldValue,
                                         automationChanged,
                                         automatedBefore,
                                         automatedNow);
  ...
  if (!modelStack->timelineCounterIsSet() ||
    ...
    if (currentValueChanged) {
      ((Sound*)modelStack->modControllable)
          ->notifyValueChangeViaLPF(modelStack->paramId,
                                    true,
                                    modelStack,
                                    oldValue,
                                    currentValue,
  ...
  // Because some patch cables are marked as "unusable" under certain
  // circumstances, see if those circumstances have changed
  switch (modelStack->paramId) {
    ...
    case PARAM_LOCAL_MODULATOR_1_FEEDBACK:
      ...
      bool containedSomethingBefore =
          AutoParam::containedSomethingBefore(automatedBefore, oldValue, -2147483648);
      if (containedSomethingBefore != containsSomethingNow) {
        ...
        ((PatchCableSet*)modelStackWithParamCollection->paramCollection)
            ->setupPatching(modelStackWithParamCollection); // Only need to setupPatching on
                                                            // this one ParamManager because
                                                            // this is the only one for which
                                                            // the param preset value has just
                                                            // changed
      }
  }
}
...
int PatchedParamSet::paramValueToKnobPos(int32_t paramValue, ModelStackWithAutoParam* modelStack) {
  if (modelStack->paramId == PARAM_LOCAL_OSC_A_PHASE_WIDTH ||
      modelStack->paramId == PARAM_LOCAL_OSC_B_PHASE_WIDTH) {
...
int32_t PatchedParamSet::knobPosToParamValue(int knobPos, ModelStackWithAutoParam* modelStack) {
  if (modelStack->paramId == PARAM_LOCAL_OSC_A_PHASE_WIDTH ||
    ...
    if (knobPos < 64) { paramValue = (knobPos + 64) << 24; }
    return paramValue;
  } else
...
// Displays text number. This will only actually end up getting used/seen on
// MIDI Clips, at channel/Clip level - not MPE/polyphonic.
...
int ExpressionParamSet::paramValueToKnobPos(int32_t                  paramValue,
...
void ExpressionParamSet::readFromFile(ParamCollectionSummary* summary,
  ...
  while (*(tagName = storageManager.readNextTagOrAttributeName())) {
    ...
    if (!strcmp(tagName, "channelPressure")) { // Alpha testers had 2 weeks or
                                               // so to create files like this -
                                               // not sure if anyone even did.
      p = 2;
      goto doReadParam;
    }
...
void ExpressionParamSet::moveRegionHorizontally(ModelStackWithParamCollection* modelStack,
                                                int32_t                        pos,
                                                int32_t                        length,
                                                int                            offset,
                                                int32_t                        lengthBeforeLoop,
  ...
  // Because this is just for ExpressionParamSet, which only has 3 params, let's
  // just do it for all of them rather than our other optimization.
  for (int p = 0; p < NUM_EXPRESSION_DIMENSIONS; p++) {
    AutoParam*               param                   = &params[p];
    ModelStackWithAutoParam* modelStackWithAutoParam = modelStack->addAutoParam(p, param);
    param->moveRegionHorizontally(modelStackWithAutoParam,
                                  pos,
                                  length,
                                  offset,
                                  lengthBeforeLoop,
