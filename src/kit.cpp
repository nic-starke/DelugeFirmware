...
bool Kit::writeDataToFile(Clip* clipForSavingOutputOnly, Song* song) {
  ...
  if (clipForSavingOutputOnly)
    ...
    // If no activeClip, that means no Clip has this Instrument, so there should
    // be a backedUpParamManager that we should use
  ...
  if (clipToTakeDrumOrderFrom) {
    // First, write Drums in the order of their NoteRows. Remove these drums
    // from our list - we'll re-add them in a moment, at the start, i.e. in the
    // same order they appear in the file
    for (int i = 0; i < ((InstrumentClip*)clipToTakeDrumOrderFrom)->noteRows.getNumElements();
         i++) {
      ...
      if (thisNoteRow->drum) {
        ...
        if (clipForSavingOutputOnly) { paramManagerForDrum = &thisNoteRow->paramManager; }
        // Or if saving Song, we know there's a NoteRow, so no need to save the
        // ParamManager

        writeDrumToFile(drum,
                        paramManagerForDrum,
                        (clipForSavingOutputOnly == NULL),
                        &selectedDrumIndex,
                        &drumIndex,
                        song);
  ...
  // Then, write remaining Drums (or all Drums in the case of saving Song) whose
  // order we didn't take from a NoteRow
  ...
  while (true) {
    ...
    // If saving Kit (not song), only save Drums if some other NoteRow in the
    // song has it - in which case, save as "default" the params from that
    // NoteRow
    if (clipForSavingOutputOnly) {
      Uart::println("yup, clipForSavingOutputOnly");
      NoteRow* noteRow = song->findNoteRowForDrum(this, thisDrum);
      if (!noteRow) goto moveOn;
      paramManagerForDrum = &noteRow->paramManager; // Of course there won't be one if it's a
                                                    // NonAudioDrum, but that's fine
    }

    // Or if saving song...
    else {
      // If no activeClip, this means we want to store all Drums
      // - and for SoundDrums, save as "default" any backedUpParamManagers (if
      // none for a SoundDrum, definitely skip it)
      if (!activeClip) {
        Uart::println("nah, !activeClip");
        if (thisDrum->type == DRUM_TYPE_SOUND) {
          paramManagerForDrum =
              song->getBackedUpParamManagerPreferablyWithClip((SoundDrum*)thisDrum, NULL);
          if (!paramManagerForDrum) goto moveOn;
        }
      }

      // Otherwise, if some Clip does have this Kit, then yes do save this Drum
      // - with no ParamManager though
      else {

        // ... but, if no NoteRow has this Drum, we actually want to delete it
        // now, so that its existence doesn't affect drumIndexes!
...
void Kit::writeDrumToFile(Drum*         thisDrum,
...
int Kit::readFromFile(Song* song, Clip* clip, int32_t readAutomationUpToPos) {
  ...
  while (*(tagName = storageManager.readNextTagOrAttributeName())) {
    ...
    if (!strcmp(tagName, "soundSources")) {
      ...
      int result = GlobalEffectableForClip::readTagFromFile(tagName,
                                                            &paramManager,
  ...
  if (selectedDrumIndex != -1) { selectedDrum = getDrumFromIndex(selectedDrumIndex); }
...
int Kit::readDrumFromFile(Song* song, Clip* clip, int drumType, int32_t readAutomationUpToPos) {
  ...
  int error =
      newDrum->readFromFile(song,
                            clip,
                            readAutomationUpToPos); // Will create and "back up" a new ParamManager
                                                    // if anything to read into it
  if (error) {
...
int Kit::loadAllAudioFiles(bool mayActuallyReadFiles) {
  ...
  bool doingAlternatePath = mayActuallyReadFiles && (audioFileManager.alternateLoadDirStatus ==
                                                     ALTERNATE_LOAD_DIR_NONE_SET);
...
getOut:
  if (doingAlternatePath) { audioFileManager.thingFinishedLoading(); }
...
void Kit::loadCrucialAudioFilesOnly() {
  ...
  bool doingAlternatePath =
      (audioFileManager.alternateLoadDirStatus == ALTERNATE_LOAD_DIR_NONE_SET);
  if (doingAlternatePath) {
...
void Kit::drumRemoved(Drum* drum) {
  ...
  if (i != -1) { numericDriver.freezeWithError("E321"); }
#endif
}
...
int Kit::getDrumIndex(Drum* drum) {
  int index = 0;
  for (Drum* thisDrum = firstDrum; thisDrum != drum; thisDrum = thisDrum->next) { index++; }
  return index;
}
...
SoundDrum* Kit::getDrumFromName(char const* name, bool onlyIfNoNoteRow) {
  for (Drum* thisDrum = firstDrum; thisDrum; thisDrum = thisDrum->next) {
    ...
    if (thisDrum->type == DRUM_TYPE_SOUND &&
        ((SoundDrum*)thisDrum)->name.equalsCaseIrrespective(name))
...
// Beware - unlike usual, modelStack, a ModelStackWithThreeMainThings*,  might
// have a NULL timelineCounter
void Kit::renderGlobalEffectableForClip(ModelStackWithTimelineCounter* modelStack,
  ...
  // Render Drums. Traverse backwards, in case one stops rendering (removing
  // itself from the list) as we render it
  for (int d = drumsWithRenderingActive.getNumElements() - 1; d >= 0; d--) {
    ...
    if (activeClip) {
      ...
      // If a new Clip had just launched on this Kit, but an old Drum was still
      // sounding which isn't present in the new Clip. In a perfect world, maybe
      // we'd instead have it check and cut the voice / Drum on switch. This
      // used to be E255
      if (!thisNoteRow) {
      ...
      drumParamManager =
          modelStack->song->getBackedUpParamManagerPreferablyWithClip(soundDrum, NULL);
    }

    ModelStackWithThreeMainThings* modelStackWithThreeMainThings =
        modelStack->addNoteRow(noteRowIndex, thisNoteRow)
            ->addOtherTwoThings(soundDrum, drumParamManager);

    soundDrum->render(modelStackWithThreeMainThings,
                      globalEffectableBuffer,
                      numSamples,
                      reverbBuffer,
                      sideChainHitPending,
                      reverbAmountAdjust,
                      shouldLimitDelayFeedback,
                      pitchAdjust); // According to our volume, we tell Drums to
                                    // send less reverb
  }
    ...
    for (int i = 0; i < noteRows->getNumElements(); i++) {
      ...
      if (thisNoteRow->drum &&
          thisNoteRow->drum->type ==
              DRUM_TYPE_SOUND) { // Just don't bother ticking other ones for now
                                 // - their MPE doesn't need to interpolate.

        ParamCollectionSummary* patchedParamsSummary =
            &thisNoteRow->paramManager.summaries[1]; // No time to call the proper function and do
        ...
        ParamCollectionSummary* unpatchedParamsSummary =
            &thisNoteRow->paramManager.summaries[0]; // No time to call the proper function and do
                                                     // error checking, sorry.
        ...
        ParamCollectionSummary* patchCablesSummary =
            &thisNoteRow->paramManager.summaries[2]; // No time to call the proper function and do
                                                     // error checking, sorry.
        ...
        ParamCollectionSummary* expressionParamsSummary =
            &thisNoteRow->paramManager.summaries[3]; // No time to call the proper function and do
                                                     // error checking, sorry.
        ...
        // Was that right? Until Jan 2022 I didn't have it checking for
        // expression params automation here for some reason...
      }
    }
  }
}
...
void Kit::renderOutput(ModelStack*   modelStack,
  ...
  GlobalEffectableForClip::renderOutput(modelStackWithTimelineCounter,
                                        paramManager,
                                        outputBuffer,
                                        numSamples,
                                        reverbBuffer,
...
void Kit::offerReceivedCCToLearnedParams(MIDIDevice*                    fromDevice,
  ...
  ModControllableAudio::offerReceivedCCToLearnedParams(
      ...
      ccNumber,
      value,
      modelStack); // NOTE: this call may change modelStack->timelineCounter
                   // etc!

  // Now do it for each NoteRow / Drum
  if (modelStack->timelineCounterIsSet()) { // This is always actually true currently
                                            // for calls to this function, but let's
                                            // make this safe and future proof.
    InstrumentClip* clip =
    ...
    for (int i = 0; i < clip->noteRows.getNumElements(); i++) {
      ...
      if (thisDrum && thisDrum->type == DRUM_TYPE_SOUND) {
        if (((SoundDrum*)thisDrum)
                ->offerReceivedPitchBendToLearnedParams(fromDevice,
                                                        channel,
                                                        data1,
                                                        data2,
                                                        modelStack)) {
...
void Kit::choke() {
  for (Drum* thisDrum = firstDrum; thisDrum; thisDrum = thisDrum->next) { thisDrum->choke(NULL); }
}
...
void Kit::setupWithoutActiveClip(ModelStack* modelStack) {
  ...
  for (Drum* thisDrum = firstDrum; thisDrum; thisDrum = thisDrum->next) {
    if (thisDrum->type == DRUM_TYPE_SOUND) {
      ...
      ParamManager* paramManager =
          modelStackWithTimelineCounter->song->getBackedUpParamManagerPreferablyWithClip(
              (ModControllableAudio*)soundDrum,
              NULL);
  ...
  if (clip) {
    for (int i = 0; i < clip->noteRows.getNumElements(); i++) {
      ...
      if (thisNoteRow->drum && thisNoteRow->drum->type == DRUM_TYPE_SOUND) {
        ...
        ModelStackWithThreeMainThings* modelStackWithThreeMainThings =
            modelStack->addNoteRow(i, thisNoteRow)
                ->addOtherTwoThings(soundDrum, &thisNoteRow->paramManager);
    ...
    for (Drum* thisDrum = firstDrum; thisDrum; thisDrum = thisDrum->next) {
      if (thisDrum->type == DRUM_TYPE_SOUND) {
        ...
        ModelStackWithParamCollection* modelStackWithParamCollection =
            modelStackWithThreeMainThings->addParamCollectionSummary(
                paramManager->getPatchCableSetSummary());
...
bool Kit::setActiveClip(ModelStackWithTimelineCounter* modelStack, int maySendMIDIPGMs) {
  ...
  if (clipChanged) {
    ...
    for (int i = 0; i < noteRows->getNumElements(); i++) {
      ...
      if (thisNoteRow->drum) { // In a perfect world we'd do this for every
                               // Drum, even any without NoteRows in new Clip,
                               // but meh this'll be fine
        ...
        if (thisNoteRow->drum->type == DRUM_TYPE_SOUND) {
          ...
          if (!(count & 7)) {
            AudioEngine::routineWithClusterLoading(); // -----------------------------------
                                                      // I guess very often this
                                                      // wouldn't work cos the
                                                      // audio routine would be
                                                      // locked
          }
          count++;
    ...
    for (Drum* thisDrum = firstDrum; thisDrum; thisDrum = thisDrum->next) {
      if (!thisDrum->noteRowAssignedTemp) { thisDrum->drumWontBeRenderedForAWhile(); }
    }
...
void Kit::compensateInstrumentVolumeForResonance(ParamManagerForTimeline* paramManager,
  ...
  if (storageManager.firmwareVersionOfFileBeingRead < FIRMWARE_1P2P0 &&
    ...
    int32_t compensation = interpolateTableSigned(
        unpatchedParams->getValue(PARAM_UNPATCHED_GLOBALEFFECTABLE_LPF_RES) + 2147483648,
        32,
    ...
    if (compensationDB > 0.1)
      unpatchedParams->shiftParamVolumeByDB(PARAM_UNPATCHED_GLOBALEFFECTABLE_VOLUME,
                                            compensationDB);

    // The SoundDrums, like all Sounds, will have already had resonance
    // compensation done on their default ParamManagers if and when any were in
    // fact loaded. Or, if we're going through a Song doing this to all
    // ParamManagers within Clips, the Clip will automatically do all NoteRows /
    // Drums next.
...
int32_t Kit::doTickForwardForArp(ModelStack* modelStack, int32_t currentPos) {
  ...
  for (int i = 0; i < ((InstrumentClip*)activeClip)->noteRows.getNumElements(); i++) {
    ...
    if (thisNoteRow->drum &&
        thisNoteRow->drum->type ==
            DRUM_TYPE_SOUND) { // For now, only SoundDrums have Arps, but that's
                               // actually a kinda pointless restriction...
      SoundDrum* soundDrum = (SoundDrum*)thisNoteRow->drum;

      ArpReturnInstruction instruction;

      ModelStackWithNoteRow* modelStackWithNoteRow =
          modelStackWithTimelineCounter->addNoteRow(i, thisNoteRow);
      ...
      int32_t ticksTilNextArpEventThisDrum =
          soundDrum->arpeggiator.doTickForward(&soundDrum->arpSettings,
                                               &instruction,
                                               currentPosThisRow,
                                               reversed);

      ModelStackWithSoundFlags* modelStackWithSoundFlags =
          modelStackWithNoteRow->addOtherTwoThings(soundDrum, &thisNoteRow->paramManager)
              ->addSoundFlags();
...
void Kit::getThingWithMostReverb(Sound**                   soundWithMostReverb,
  ...
  GlobalEffectableForClip::getThingWithMostReverb(activeClip,
                                                  soundWithMostReverb,
                                                  paramManagerWithMostReverb,
                                                  globalEffectableWithMostReverb,
                                                  highestReverbAmountFound);
  ...
  if (activeClip) {
    ...
    for (int i = 0; i < ((InstrumentClip*)activeClip)->noteRows.getNumElements(); i++) {
      ...
      ((SoundDrum*)thisNoteRow->drum)
          ->getThingWithMostReverb(soundWithMostReverb,
                                   paramManagerWithMostReverb,
                                   globalEffectableWithMostReverb,
                                   highestReverbAmountFound,
...
void Kit::offerReceivedNote(ModelStackWithTimelineCounter* modelStack,
  ...
  InstrumentClip* instrumentClip =
      (InstrumentClip*)modelStack->getTimelineCounterAllowNull(); // Yup it might be NULL
  ...
  bool shouldRecordNoteOn =
      ...
      currentSong->isClipActive(instrumentClip); // Even if this comes out as false here, there are
                                                 // some special cases below where we might insist
                                                 // on making it true
  for (Drum* thisDrum = firstDrum; thisDrum; thisDrum = thisDrum->next) {
    ...
    if (thisDrum->midiInput.equalsNoteOrCCAllowMPE(fromDevice, channel, note)) {
      ...
      if (doingMidiThru && thisDrum->type == DRUM_TYPE_MIDI &&
          ((MIDIDrum*)thisDrum)->channel == channel && ((MIDIDrum*)thisDrum)->note == note) {
        *doingMidiThru = false;
      }

      // Just once, for first Drum we're doing a note-on on, see if we want to
      // switch to a different InstrumentClip, for a couple of reasons
      if (lookingForFirstDrumForNoteOn && instrumentClip && shouldRecordNotes) {
        ...
        if (playbackHandler.recording == RECORDING_ARRANGEMENT) {
          ...
          instrumentClip = (InstrumentClip*)modelStack->getTimelineCounter(); // Re-get it, cos it
                                                                              // might have changed
        ...
        // And another special case - if there's a pending overdub beginning
        // really soon, and activeClip is not linearly recording (and maybe not
        // even active)...
          ...
          if (samplesTilLaunch <= LINEAR_RECORDING_EARLY_FIRST_NOTE_ALLOWANCE) {
            ...
            if (clipAboutToRecord) { goto goingToRecordNoteOnEarly; }
          }
        }
      }
      ...
      if (recordingNoteOnEarly) {
        bool allowingNoteTails =
            instrumentClip && instrumentClip->allowNoteTails(modelStackWithNoteRow);
      ...
      if (on) {
        ...
        // If input is MPE, we need to give the Drum the most recent MPE
        // expression values received on the channel on the Device. It doesn't
        // keep track of these when a note isn't on, and even if it did, this
        // new note might be on a different channel (just same notecode).
        ...
        // And if non-MPE input, just set those finger-level MPE values to 0. If
        // an MPE instrument had been used just before, it could have left them
        // set to something.
        else {
          for (int i = 0; i < NUM_EXPRESSION_DIMENSIONS; i++) {
        ...
        // MPE stuff - if editing note, we need to take note of the initial
        // values which might have been sent before this note-on.
        ...
        if (!thisNoteRow || !thisNoteRow->soundingStatus) {
          ...
          if (thisNoteRow && shouldRecordNoteOn) {
            ...
            if (fromDevice->ports[MIDI_DIRECTION_INPUT_TO_DELUGE].isChannelPartOfAnMPEZone(
                    channel)) {
          ...
          // TODO: possibly should change the MPE params' currentValue to the
          // initial values, since that usually does get updated by the
      ...
      // Note-off
        ...
        if (thisNoteRow) {
          ...
          // MPE-controlled params are a bit special in that we can see (via
          // this note-off) when the user has removed their finger and won't be
          // sending more values. So, let's unlatch those params now.
          ExpressionParamSet* mpeParams = thisNoteRow->paramManager.getExpressionParamSet();
          if (mpeParams) { mpeParams->cancelAllOverriding(); }
        }
        endAuditioningForDrum(modelStackWithNoteRow,
                              thisDrum,
                              velocity); // Do this even if not marked as auditioned, to avoid
                                         // stuck notes in cases like if two note-ons were sent
      }
    }

    // Or if this is the Drum's mute command...
    if (instrumentClip && on &&
        thisDrum->muteMIDICommand.equalsNoteOrCC(fromDevice, channel, note)) {

      ModelStackWithNoteRow* modelStackWithNoteRow =
          instrumentClip->getNoteRowForDrum(modelStack, thisDrum);
  ...
  for (Drum* thisDrum = firstDrum; thisDrum; thisDrum = thisDrum->next) {
    if (thisDrum->midiInput.equalsChannelAllowMPE(fromDevice, channel)) {
      ...
      if (thisDrum->midiInput.isForMPEZone()) { // If Drum has MPE input.
        if (channel ==
            thisDrum->midiInput.getMasterChannel()) { // Message coming in on master channel -
                                                      // that's "main"/zone-level, too.
          goto yesThisDrum;
        }
        if (channel == thisDrum->lastMIDIChannelAuditioned) { // Or if per-finger level,
                                                              // check the member channel
                                                              // of the message matches the
                                                              // one sounding on the Drum
                                                              // right now.
          level = BEND_RANGE_FINGER_LEVEL;
          goto yesThisDrum;
        }
      } else { // Or, if Drum does not have MPE input, then this is a
    ...
    if (thisDrum->midiInput.equalsChannelAllowMPE(fromDevice, channel)) {
      ...
      if (thisDrum->midiInput.isForMPEZone()) { // If Drum has MPE input.
        ...
        yesThisDrum:
          ...
          thisDrum->expressionEventPossiblyToRecord(modelStackWithTimelineCounter,
                                                    value16,
                                                    1,
                                                    level);
        }
      ...
      // If not an MPE input, we don't want to respond to this CC74 at all (for
      // this Drum).
    }
  }
}
...
// could still then just apply to one note). This function could be optimized a
// bit better, there are lots of calls to similar functions.
void Kit::offerReceivedAftertouch(ModelStackWithTimelineCounter* modelStackWithTimelineCounter,
  ...
  for (Drum* thisDrum = firstDrum; thisDrum; thisDrum = thisDrum->next) {
    ...
    // Or a polyphonic aftertouch message - these aren't allowed for MPE except
...
void Kit::offerBendRangeUpdate(ModelStack* modelStack,
  ...
  if (whichBendRange == BEND_RANGE_MAIN)
    return; // This is not used in Kits for Drums. Drums use their
            // BEND_RANGE_FINGER_LEVEL for both kinds of bend.
            // TODO: Hmm, for non-MPE instruments we'd want to use this kind of
            // bend range update and just paste it into BEND_RANGE_FINGER_LEVEL
            // though...
      ...
      if (activeClip) {
        ...
        if (noteRow) {
          ExpressionParamSet* expressionParams =
              noteRow->paramManager.getOrCreateExpressionParamSet();
          if (expressionParams) {
        ...
        // ParamManager* paramManager =
        // modelStack->song->getBackedUpParamManagerPreferablyWithClip(thisDrum,
        // NULL); // TODO...
      }
    }
  }
...
void Kit::stopAnyAuditioning(ModelStack* modelStack) {
  ...
  for (Drum* thisDrum = firstDrum; thisDrum; thisDrum = thisDrum->next) {
    if (thisDrum->auditioned) {
      ModelStackWithNoteRow* modelStackWithNoteRow =
          activeClip ? ((InstrumentClip*)activeClip)
                           ->getNoteRowForDrum(modelStackWithTimelineCounter, thisDrum) :
...
bool Kit::isAnyAuditioningHappening() {
  for (Drum* thisDrum = firstDrum; thisDrum; thisDrum = thisDrum->next) {
    if (thisDrum->auditioned) { return true; }
  }
  return false;
}

// You must supply noteRow if there is an activeClip with a NoteRow for that
// Drum. The TimelineCounter should be the activeClip. Drum must not be NULL -
// check first if not sure!
void Kit::beginAuditioningforDrum(ModelStackWithNoteRow* modelStack,
  ...
  if (modelStack->getNoteRowAllowNull()) {
    ...
    if (drum->type == DRUM_TYPE_SOUND) {
      paramManagerForDrum =
          modelStack->song->getBackedUpParamManagerPreferablyWithClip((SoundDrum*)drum, NULL);
      if (!paramManagerForDrum)
        numericDriver.freezeWithError("E314"); // Ron got this, June 2020, while "dragging" a row
                                               // vertically in arranger
    }
  }
...
// Check that it's auditioned before calling this if you don't want it
// potentially sending an extra note-off in some rare cases. You must supply
// noteRow if there is an activeClip with a NoteRow for that Drum. The
// TimelineCounter should be the activeClip.
  ...
  if (drum->type == DRUM_TYPE_SOUND) {
    ...
    if (!paramManagerForDrum)
      numericDriver.freezeWithError(
          "E312"); // Should make ALPHA_OR_BETA_VERSION after V3.0.0 release
  }
...
gotParamManager:
  ...
  if (activeClip)
    activeClip->expectEvent(); // Because the absence of auditioning here means
                               // sequenced notes may play
}
