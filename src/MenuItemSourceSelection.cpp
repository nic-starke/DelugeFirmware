...
void MenuItemSourceSelection::drawPixelsForOled() {
  ...
  for (int i = 0; i < OLED_MENU_NUM_OPTIONS_VISIBLE; i++) { itemNames[i] = NULL; }
  ...
  while (i < OLED_MENU_NUM_OPTIONS_VISIBLE) {
    ...
    if (sourceIsAllowed(sHere)) {
      ...
      if (thisOption == soundEditor.currentValue) { selectedRowOnScreen = i; }
      i++;
    } else {
...
void MenuItemSourceSelection::beginSession(MenuItem* navigatedBackwardFrom) {
  ...
  if (navigatedBackwardFrom) {
    ...
    while (true) {
      ...
      // If patching already exists on this source, we use this as the initial
      // one to show to the user
      if (soundEditor.currentParamManager->getPatchCableSet()
              ->isSourcePatchedToDestinationDescriptorVolumeInspecific(s,
                                                                       getDestinationDescriptor()))
        break;
...
MenuItem*
MenuItemSourceSelectionRegular::patchingSourceShortcutPress(int  newS,
...
ParamDescriptor MenuItemSourceSelectionRange::getDestinationDescriptor() {
  ParamDescriptor descriptor;
  descriptor.setToHaveParamAndSource(soundEditor.patchingParamSelected,
...
MenuItem* MenuItemSourceSelectionRange::patchingSourceShortcutPress(int  newS,
                                                                    bool previousPressStillActive) {
