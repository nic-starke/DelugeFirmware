...
public:
  ...
  virtual int     clone(ModelStackWithTimelineCounter* modelStack,
                        bool                           shouldFlattenReversing = false) = 0;
  ...
  virtual void    setPos(ModelStackWithTimelineCounter* modelStack,
                         int32_t                        newPos,
                         bool                           useActualPosForParamManagers = true);
  virtual void    setPosForParamManagers(ModelStackWithTimelineCounter* modelStack,
                                         bool                           useActualPos = true);
  virtual void    expectNoFurtherTicks(Song* song, bool actuallySoundChange = true) = 0;
  virtual void    resumePlayback(ModelStackWithTimelineCounter* modelStack,
                                 bool                           mayMakeSound = true)                          = 0;
  virtual void    reGetParameterAutomation(ModelStackWithTimelineCounter* modelStack);
  virtual void    processCurrentPos(ModelStackWithTimelineCounter* modelStack,
  ...
  virtual int
  transferVoicesToOriginalClipFromThisClone(ModelStackWithTimelineCounter* modelStackOriginal,
                                            ModelStackWithTimelineCounter* modelStackClone) {
    return NO_ERROR;
  }
  virtual void increaseLengthWithRepeats(ModelStackWithTimelineCounter* modelStack,
                                         int32_t                        newLength,
                                         int     independentNoteRowInstruction,
                                         bool    completelyRenderOutIterationDependence = false,
                                         Action* action                                 = NULL) {
  } // This is not implemented for AudioClips - because in the cases where we
    // call this, we don't want it to happen for AudioClips
  virtual void lengthChanged(ModelStackWithTimelineCounter* modelStack,
                             int32_t                        oldLength,
                             Action*                        action = NULL);
  virtual void getSuggestedParamManager(Clip*                     newClip,
                                        ParamManagerForTimeline** suggestedParamManager,
                                        Sound*                    sound);

  virtual void detachFromOutput(
      ...
      bool shouldBackUpExpressionParamsToo = true) = 0; // You're likely to want to call
                                                        // pickAnActiveClipIfPossible() after this

  virtual int  undoDetachmentFromOutput(ModelStackWithTimelineCounter* modelStack);
  virtual bool renderAsSingleRow(ModelStackWithTimelineCounter* modelStack,
  ...
  virtual int
  claimOutput(ModelStackWithTimelineCounter* modelStack) = 0; // To be called after Song loaded, to
                                                              // link to the relevant Output object
  virtual void  finishLinearRecording(ModelStackWithTimelineCounter* modelStack,
                                      Clip*                          nextPendingLoop = NULL,
                                      int buttonLatencyForTempolessRecord            = 0) = 0;
  virtual int   beginLinearRecording(ModelStackWithTimelineCounter* modelStack,
                                     int                            buttonPressLatency)                   = 0;
  void          drawUndefinedArea(int32_t localScroll,
                                  uint32_t,
                                  ...
                                  TimelineView* editorScreen,
                                  bool          tripletsOnHere);
  bool          opportunityToBeginSessionLinearRecording(ModelStackWithTimelineCounter* modelStack,
                                                         bool*                          newOutputCreated,
                                                         int                            buttonPressLatency);
  virtual Clip* cloneAsNewOverdub(ModelStackWithTimelineCounter* modelStack,
                                  int                            newOverdubNature) = 0;
  ...
  virtual void quantizeLengthForArrangementRecording(ModelStackWithTimelineCounter* modelStack,
                                                     int32_t                        lengthSoFar,
  ...
  void         setSequenceDirectionMode(ModelStackWithTimelineCounter* modelStack,
  ...
  bool wasWantingToDoLinearRecordingBeforeCountIn; // Only valid during a
                                                   // count-in
  ...
  int32_t loopLength;
  int32_t originalLength; // Before linear recording of this Clip began, and this
                          // Clip started getting extended to multiples of this
...
protected:
  virtual void
  posReachedEnd(ModelStackWithTimelineCounter* modelStack); // May change the TimelineCounter in the
                                                            // modelStack if new Clip got created
  virtual bool cloneOutput(ModelStackWithTimelineCounter*
                               modelStack) = 0; // Returns whether a new Output was in fact created
  int          solicitParamManager(Song*         song,
