...
ParamCollection::ParamCollection(int newObjectSize, ParamCollectionSummary* summary)
  ...
  for (int i = 0; i < MAX_NUM_UINTS_TO_REP_ALL_PARAMS;
       i++) { // Just do both even if we're only using one.
...
void ParamCollection::notifyParamModifiedInSomeWay(ModelStackWithAutoParam const* modelStack,
  ...
  if (currentValueChanged || automationChanged)
    modelStack->paramManager->notifyParamModifiedInSomeWay(modelStack,
                                                           currentValueChanged,
                                                           automationChanged,
                                                           automatedNow);

  if (automationChanged && automatedNow) { ticksTilNextEvent = 0; }
}
