...
void ArrangerView::moveClipToSession() {
  ...
  if (!clip) {
    numericDriver.displayPopup(HAVE_OLED ? "Empty clip instances can't be moved to the session" :
                                           "EMPTY");
  }
...
int ArrangerView::buttonAction(int x, int y, bool on, bool inCardRoutine) {
  ...
  // Cross-screen button
    ...
    if (on && currentUIMode == UI_MODE_NONE) {
      ...
      IndicatorLEDs::setLedState(crossScreenEditLedX,
                                 crossScreenEditLedY,
  ...
  // Record button - adds to what MatrixDriver does with it
    ...
    return ACTION_RESULT_NOT_DEALT_WITH; // Make the MatrixDriver do its normal
                                         // thing with it too
  }
...
void ArrangerView::clearArrangement() {
  ...
  // We go through deleting the ClipInstances one by one. This is actually quite
  // inefficient, but complicated to improve on because the deletion of the
  // Clips themselves, where there are arrangement-only ones, causes the calling
  // of output->pickAnActiveClipIfPossible. So we have to ensure that extra
  // ClipInstances don't exist at any instant in time, or else it'll look at
  // those to pick the new activeClip, which might not exist anymore.
...
void ArrangerView::repopulateOutputsOnScreen(bool doRender) {
  ...
  while (output) {
    ...
    if (row >= 0) { outputsOnScreen[row] = output; }
    row++;
    output = output->next;
  }
  ...
  if (doRender) { uiNeedsRendering(this); }
}
...
void ArrangerView::drawMuteSquare(int yDisplay, uint8_t thisImage[][3]) {
  ...
  doBlack:
    memset(thisColour, 0, 3);
           ...
           outputsOnScreen[yDisplay]->armedForRecording) {
    if (blinkOn) {
  ...
  // Or if not soloing...
    ...
    if (currentSong->getAnyOutputsSoloingInArrangement()) { dimColour(thisColour); }
  }
}
...
ModelStackWithNoteRow* ArrangerView::getNoteRowForAudition(ModelStack* modelStack, Kit* kit) {
  ...
  if (kit->activeClip) {
    ...
    modelStackWithNoteRow =
        instrumentClip->getNoteRowForDrumName(modelStackWithTimelineCounter, "SNAR");
...
void ArrangerView::beginAudition(Output* output) {
  ...
  if (!playbackHandler.playbackState && !Buttons::isShiftButtonPressed()) {
    ...
    if (instrument->type == INSTRUMENT_TYPE_KIT) {
      ...
      if (drum)
        kit->beginAuditioningforDrum(modelStackWithNoteRow,
                                     drum,
                                     kit->defaultVelocity,
                                     zeroMPEValues);
    } else {
...
void ArrangerView::changeOutputToInstrument(int newInstrumentType) {
  ...
  if (currentSong->getClipWithOutput(oldOutput)) {
    numericDriver.displayPopup(
        HAVE_OLED ? "Audio tracks with clips can't be turned into an instrument" : "CANT");
    return;
  }
...
Instrument* ArrangerView::createNewInstrument(int   newInstrumentType,
  ...
  result = Browser::findAnUnlaunchedPresetIncludingWithinSubfolders(currentSong,
                                                                    newInstrumentType,
  ...
  *instrumentAlreadyInSong  = newInstrument && result.fileItem->instrumentAlreadyInSong;
  ...
  if (!newInstrument) {
    ...
    result.error = storageManager.loadInstrumentFromFile(currentSong,
                                                         NULL,
                                                         newInstrumentType,
                                                         false,
                                                         &newInstrument,
...
void ArrangerView::auditionPadAction(bool on, int y) {
    ...
    doNewPress:
      ...
      if (!output) {
        ...
        if (!instrumentAlreadyInSong)
          currentSong->addOutput(output,
                                 (yPressedEffective == -currentSong->arrangementYScroll -
                                                           1)); // This should always be triggered
  ...
  // Release press
    ...
    if (y == yPressedActual) { exitSubModeWithoutAction(); }
  }
}
      ...
      case UI_MODE_MIDI_LEARN:
        if (output) {
          if (output->type == OUTPUT_TYPE_AUDIO) {
            ...
            if (velocity)
              numericDriver.displayPopup(
                  HAVE_OLED ? "MIDI must be learned to kit items individually" : "CANT");
          } else {
    ...
    if (velocity) {
      ...
      switch (currentUIMode) {
        ...
        case UI_MODE_HOLDING_HORIZONTAL_ENCODER_BUTTON:
          ...
          if (!output->soloingInArrangementMode) {
            ...
            if (arrangement.hasPlaybackActive()) {
              ...
              // If other Instruments were already soloing, or if they weren't but
              // this instrument was muted, we'll need to tell it to start playing
            ...
            // If we're the first Instrument to be soloing, need to tell others
            // they've been inadvertedly deactivated
            if (!currentSong->getAnyOutputsSoloingInArrangement()) {
          ...
          doUnsolo:
            ...
            if (!currentSong->anyOutputsSoloingInArrangement) { rowsToRedraw = 0xFFFFFFFF; }

            // If any other Instruments still soloing, or if we're "muted",
            // deactivate us
            if (currentSong->getAnyOutputsSoloingInArrangement() ||
                output->mutedInArrangementMode) {
            ...
            if (arrangement.hasPlaybackActive()) {
              ...
              // If no other Instruments still soloing, re-activate all the other
              // ones
              if (!currentSong->getAnyOutputsSoloingInArrangement()) {
                for (Output* thisOutput = currentSong->firstOutput; thisOutput;
                     thisOutput         = thisOutput->next) {
        ...
        case UI_MODE_HOLDING_ARRANGEMENT_ROW_AUDITION:
          // If it's the mute pad for the same row we're auditioning, don't do
          // anything. User might be subconsciously repeating the "drag row"
          // action for Kits in InstrumentClipView.
          if (y == yPressedEffective) break;
          goto regularMutePadPress; // Otherwise, do normal.

        case UI_MODE_NONE:
          // If the user was just quick and is actually holding the record button
          // but the submode just hasn't changed yet...
        ...
        regularMutePadPress:
          ...
          if (output->mutedInArrangementMode) {
            ...
            if (arrangement.hasPlaybackActive() &&
                !currentSong->getAnyOutputsSoloingInArrangement()) {
              outputActivated(output);
            }
          }
          ...
          // Muting
            ...
            if (!currentSong->getAnyOutputsSoloingInArrangement()) { outputDeactivated(output); }
  ...
  // Edit pad
    ...
    if (currentUIMode == UI_MODE_HOLDING_ARRANGEMENT_ROW_AUDITION) {
      ...
      if (velocity) {
        ...
        if (x == 11 && y == 5) {
          Output* output = outputsOnScreen[yPressedEffective];
          if (output && output->type != INSTRUMENT_TYPE_MIDI_OUT &&
              output->type != INSTRUMENT_TYPE_CV) {
            endAudition(output);
            currentUIMode         = UI_MODE_NONE;
            renameOutputUI.output = output;
            openUI(&renameOutputUI);
            uiNeedsRendering(this, 0,
                             0xFFFFFFFF); // Stop audition pad being illuminated
          }
        }
      }
#endif
    } else {
      if (output) { editPadAction(x, y, velocity); }
    }
  }
...
// For now, we're always supplying clearingWholeArrangement as false, even when
// we are doing that
void ArrangerView::deleteClipInstance(Output*       output,
                                      int           clipInstanceIndex,
    ...
    if (on && currentUIMode == UI_MODE_NONE) {
      ...
      if (clipInstance && clipInstance->pos + clipInstance->length >= squareStart) {
        ...
        if (oldClip && !oldClip->isArrangementOnlyClip() &&
          ...
          int error =
              arrangement.doUniqueCloneOnClipInstance(clipInstance, clipInstance->length, true);
          if (error) {
            numericDriver.displayError(error);
          } else {
    ...
    if (on) {
      ...
      doNewPress:
        ...
        if (clipInstance) {
          ...
          // finishes to our left, we can still go ahead and make a new Instance
          // here
        ...
        makeNewInstance:
          ...
          Output* lastOutputInteractedWith =
              currentSong->getOutputFromIndex(lastInteractedOutputIndex);
          ...
          // Test thing
          {
            ...
            if (nextClipInstance && nextClipInstance->pos == squareStart) {
              numericDriver.freezeWithError(
                  "E233"); // Yes, this happened to someone. Including me!!
            }
          }
          ...
          getItFromSection:
            if (lastInteractedSection == 255) {
              ...
              // If that section had none, just get any old one (still might
              // return NULL - that's fine)
              if (!newClip) { newClip = currentSong->getSessionClipWithOutput(output); }
            }
          }

          // Make the actual new ClipInstance. Do it now, after potentially
          // looking at existing ones above, so that we don't look at this new
          // one above
          pressedClipInstanceIndex = output->clipInstances.insertAtKey(squareStart);

          // Test thing
          {
            ClipInstance* nextInstance =
                output->clipInstances.getElement(pressedClipInstanceIndex + 1);
            if (nextInstance) {
          ...
          arrangement.rowEdited(output,
                                clipInstance->pos,
                                clipInstance->pos + clipInstance->length,
                                NULL,
                                clipInstance);
      ...
      // Already pressing - length edit
        ...
        if (x > xPressed) {
          ...
          if (clipInstance->length > lengthTilNewSquareStart) {
            ...
            if (clipInstance->clip)
              arrangement.rowEdited(output,
                                    clipInstance->pos + lengthTilNewSquareStart,
                                    clipInstance->pos + clipInstance->length,
                                    clipInstance->clip,
                                    NULL);
            clipInstance->change(action,
                                 output,
                                 clipInstance->pos,
                                 lengthTilNewSquareStart,
                                 clipInstance->clip);
          }
    ...
    // Release press
      ...
      if (isUIModeActive(UI_MODE_HOLDING_ARRANGEMENT_ROW)) {
        ...
        if (x == xPressed && y == yPressedEffective) {
          ...
          if (!actionOnDepress ||
              (int32_t)(AudioEngine::audioSampleTimer - pressTime) >= (44100 >> 1)) {
          justGetOut:
          ...
          // Or if yes we do want to do some action...
            ...
            // Otherwise, go into Clip
              ...
              // If Clip wasn't created yet, create it first. This does both
              // AudioClips and InstrumentClips
              if (!clipInstance->clip) {

                if (!currentSong->arrangementOnlyClips.ensureEnoughSpaceAllocated(1))
                  goto justGetOut;

                int size = (output->type == OUTPUT_TYPE_AUDIO) ? sizeof(AudioClip) :
                ...
                newClip->section        = 255;
                newClip->activeIfNoSolo = false; // Always need to set arrangement-only Clips like
                                                 // this on create
                ...
                // Possibly want to set this as the activeClip, if Instrument
                // didn't have one yet. Crucial that we do this not long after
                // calling setInstrument, in case this is the first Clip with
                // the Instrument and we just grabbed the backedUpParamManager
                // for it, which it might go and look for again if the audio
                // routine was called in the interim
                if (!output->activeClip) { output->setActiveClip(modelStack); }

                currentSong->arrangementOnlyClips.insertClipAtIndex(newClip, 0);

                Action* action = actionLogger.getNewAction(ACTION_CLIP_INSTANCE_EDIT, false);
                if (action)
                  action->recordClipExistenceChange(currentSong,
                                                    &currentSong->arrangementOnlyClips,
                                                    newClip,
                                                    CREATE);
...
// Only call if this is the currentUI. May be called during audio / playback
// routine
void ArrangerView::exitSubModeWithoutAction() {

  // First, stop any stuttering. This may then put us back in one of the
  // subModes dealt with below
  if (isUIModeActive(UI_MODE_STUTTERING)) {
    ((ModControllableAudio*)view.activeModControllableModelStack.modControllable)
  ...
  if (isUIModeActive(UI_MODE_MIDI_LEARN)) { view.endMIDILearn(); }
}
...
void ArrangerView::transitionToClipView(ClipInstance* clipInstance) {
  ...
  if (clip->type == CLIP_TYPE_AUDIO) {
    ...
    ((AudioClip*)clip)
        ->getScrollAndZoomInSamples(currentSong->xScroll[NAVIGATION_CLIP],
                                    currentSong->xZoom[NAVIGATION_CLIP],
                                    &xScrollSamples,
                                    &xZoomSamples);

    waveformRenderer.findPeaksPerCol((Sample*)((AudioClip*)clip)->sampleHolder.audioFile,
                                     xScrollSamples,
                                     xZoomSamples,
    ...
    if (((InstrumentClip*)clip)->onKeyboardScreen) {
      ...
      keyboardScreen.renderMainPads(0xFFFFFFFF,
                                    &PadLEDs::imageStore[1],
    ...
    // Or if just regular old InstrumentClipView
      ...
      instrumentClipView.renderMainPads(0xFFFFFFFF,
                                        &PadLEDs::imageStore[1],
                                        &PadLEDs::occupancyMaskStore[1],
                                        false);
  ...
  int64_t clipLengthBig =
      ((uint64_t)clip->loopLength << 16) / currentSong->xZoom[NAVIGATION_ARRANGEMENT];
  ...
  PadLEDs::explodeAnimationXWidthBig =
      ((uint32_t)(end - start) / currentSong->xZoom[NAVIGATION_ARRANGEMENT]) << 16;
  ...
  PadLEDs::sendOutSidebarColours(); // They'll have been cleared by the first
                                    // explode render
}
...
bool ArrangerView::transitionToArrangementEditor() {
  ...
  int     i =
      output->clipInstances.search(currentSong->lastClipInstanceEnteredStartPos, GREATER_OR_EQUAL);
  ClipInstance* clipInstance = output->clipInstances.getElement(i);
  ...
  memcpy(PadLEDs::occupancyMaskStore[1],
         PadLEDs::occupancyMask,
         (displayWidth + sideBarWidth) * displayHeight);
  ...
  int64_t clipLengthBig = ((uint64_t)currentSong->currentClip->loopLength << 16) /
  ...
  int numExtraRepeats =
      (uint32_t)(clipInstance->length - 1) / (uint32_t)currentSong->currentClip->loopLength;
  ...
  PadLEDs::explodeAnimationXStartBig = xStartBig;
  PadLEDs::explodeAnimationXWidthBig = ((end - start) / currentSong->xZoom[NAVIGATION_ARRANGEMENT])
                                       << 16;
...
// Wait.... kinda extreme that this seems to be able to happen during card
// routine even... is that dangerous? Seems to return whether it managed to put
// it in a new, valid position.
bool ArrangerView::putDraggedClipInstanceInNewPosition(Output* newOutputToDragInto) {
  ...
  ClipInstance* clipInstance =
      pressedClipInstanceOutput->clipInstances.getElement(pressedClipInstanceIndex);
  Clip* clip = clipInstance->clip;
  ...
  if (clip)
    arrangement.rowEdited(pressedClipInstanceOutput,
                          clipInstance->pos,
                          clipInstance->pos + length,
                          clip,
                          NULL);
  ...
  if (clip)
    arrangement.rowEdited(newOutputToDragInto,
                          clipInstance->pos,
                          clipInstance->pos + length,
                          NULL,
                          clipInstance);
...
uint32_t ArrangerView::doActualRender(int32_t  xScroll,
                                      uint32_t xZoom,
                                      uint32_t whichRows,
                                      uint8_t* image,
                                      uint8_t  occupancyMask[][displayWidth + sideBarWidth],
  ...
  for (int yDisplay = 0; yDisplay < displayHeight; yDisplay++) {
    if (whichRows & (1 << yDisplay)) {
      ...
      bool success =
          renderRow(modelStack, yDisplay, xScroll, xZoom, image, occupancyMaskThisRow, renderWidth);
...
bool ArrangerView::renderRow(ModelStack* modelStack,
  ...
  if (!output) {
    ...
    while (imageThisRow < imageEnd) { *(imageThisRow++) = 0; }
  ...
  if (currentUIMode == UI_MODE_HOLDING_ARRANGEMENT_ROW && !pressedClipInstanceIsInValidPosition) {
    if (yPressedEffective == yDisplay) { drawGhostClipInstanceHere = true; }
    if (output == pressedClipInstanceOutput) { ignoreI = pressedClipInstanceIndex; }
  }

  bool success = renderRowForOutput(modelStack,
                                    output,
                                    xScroll,
                                    xZoom,
                                    imageThisRow,
  ...
  if (drawGhostClipInstanceHere) {
    ...
    int xMovement = currentSong->xScroll[NAVIGATION_ARRANGEMENT] -
...
// Returns false if can't because in card routine
// occupancyMask can be NULL
bool ArrangerView::renderRowForOutput(ModelStack* modelStack,
                                      Output*     output,
                                      int32_t     xScroll,
                                      uint32_t    xZoom,
                                      uint8_t*    image,
  ...
  if (!output->clipInstances.getNumElements()) {
    while (imageNow < imageEnd) { *(imageNow++) = 0; }
    return true;
  }
  ...
  memcpy(&searchTerms[firstXDisplayNotLeftOf0],
         &squareEndPos[firstXDisplayNotLeftOf0],
         (renderWidth - firstXDisplayNotLeftOf0) * sizeof(int32_t));

  output->clipInstances.searchMultiple(&searchTerms[firstXDisplayNotLeftOf0],
                                       renderWidth - firstXDisplayNotLeftOf0);
  ...
  squareStartPosSet:
    ...
    if (clipInstance) {
      ...
      // If Instance starts exactly on square or somewhere within square, draw
      // "head". We don't do the "blur" colour in arranger - it looks too white
      // and would be confused with white/unique instances
      ...
      // Otherwise...
        ...
        // See if we want to do a "tail" - which might be several squares long
        // from here
        if (instanceEnd > squareStartPos) {
          ...
          // Draw either the blank, non-existent Clip if this Instance doesn't
          // have one...
          if (!clipInstance->clip) {
  ...
  if (instrumentType == INSTRUMENT_TYPE_MIDI_OUT || instrumentType == INSTRUMENT_TYPE_CV) {
    ...
    if (instrumentType == INSTRUMENT_TYPE_CV) {
      do {
        ...
        cantDoIt:
          numericDriver.displayPopup(HAVE_OLED ? "No free channel slots available in song" :
                                                 "CANT");
          return;
        }
          ...
          currentSong
              ->getInstrumentFromPresetSlot(instrumentType, newChannel, -1, NULL, NULL, false));
    }
    ...
    // Or MIDI
      ...
      } while (currentSong->getInstrumentFromPresetSlot(instrumentType,
                                                        newChannel,
                                                        newChannelSuffix,
                                                        NULL,
                                                        NULL,
    ...
    if (oldNonAudioInstrument->activeClip &&
    ...
    // Because these are just MIDI / CV instruments and we're changing them for
    // all Clips, we can just change the existing Instrument object!
...
void ArrangerView::changeOutputToAudio() {
  ...
  cant:
    numericDriver.displayPopup(
        HAVE_OLED ? "Instruments with clips can't be turned into audio tracks" : "CANT");
    return;
  }
  ...
  // Or if no old Clip, we just simply make a new Output here and don't worry
  // about Clips
  else {
...
int ArrangerView::horizontalEncoderAction(int offset) {
  ...
  // Or shift presssed - extend or delete time
    ...
    // Disallow while arranger playback active - we'd be battling autoscroll and
    // stuff
    if (isNoUIModeActive()) {
      ...
      if (arrangement.hasPlaybackActive()) {
        ...
        int actionType =
            (offset >= 0) ? ACTION_ARRANGEMENT_TIME_EXPAND : ACTION_ARRANGEMENT_TIME_CONTRACT;
        ...
        if (offset >= 0) {
          void* consMemory =
              generalMemoryAllocator.alloc(sizeof(ConsequenceArrangerParamsTimeInserted));
          if (consMemory) {
          ...
          if (action) {
            unpatchedParams->backUpAllAutomatedParamsToAction(action,
                                                              modelStackWithUnpatchedParams);
          }
        ...
        for (Output* thisOutput = currentSong->firstOutput; thisOutput;
          ...
          while (i < thisOutput->clipInstances.getNumElements()) {
            ...
            if (offset < 0 &&
                instance->pos + scrollAmount < currentSong->xScroll[NAVIGATION_ARRANGEMENT]) {
            ...
            // Otherwise, just move it
              ...
              // If contracting time, shorten the previous ClipInstance only if
              // the ClipInstances we're moving will eat into its tail.
              // Otherwise, leave the tail there. Perhaps it'd make more sense
              // to cut the tail off regardless, but possibly just due to me not
              // thinking about it, this was not done in pre-V4 firmware, and
              // actually having it this way probably helps users.
              if (!movedOneYet && offset < 0 && i > 0) {
                movedOneYet = true;
                ClipInstance* prevInstance =
                    (ClipInstance*)thisOutput->clipInstances.getElement(i - 1);
                int32_t maxLength = newPos - prevInstance->pos;
                if (prevInstance->length > maxLength) {
                  prevInstance->change(action,
                                       thisOutput,
                                       prevInstance->pos,
                                       maxLength,
                                       prevInstance->clip);
  ...
  // Encoder button not pressed - we'll just scroll (and possibly drag a
  // ClipInstance horizontally)
...
int ArrangerView::horizontalScrollOneSquare(int direction) {
  ...
  // Make sure we don't scroll too far left, though I'm pretty sure we already
  // did above?
...
// No need to check whether playback active before calling - we check for that
// here.
void ArrangerView::reassessWhetherDoingAutoScroll(int32_t pos) {
  ...
  if (doingAutoScrollNow) { autoScrollNumSquaresBehind = getSquareFromPos(pos); }
}
...
int ArrangerView::verticalScrollOneSquare(int direction) {
  ...
  // Or if dragging ClipInstance vertically
    ...
    Output* newOutput =
        currentSong->getOutputFromIndex(yPressedEffective + currentSong->arrangementYScroll);
...
void ArrangerView::setNoSubMode() {
  ...
  if (doingAutoScrollNow)
    reassessWhetherDoingAutoScroll(); // Maybe stop auto-scrolling. But don't
                                      // start.
}
  ...
  if (PadLEDs::flashCursor != FLASH_CURSOR_OFF) {
    ...
    if (!arrangement.hasPlaybackActive() || currentUIMode == UI_MODE_EXPLODE_ANIMATION ||
      ...
      if (doingAutoScrollNow && isUIModeWithinRange(autoScrollUIModes)) {
        ...
        int32_t newScrollPos = (actualCurrentPos / currentSong->xZoom[NAVIGATION_ARRANGEMENT] -
                                autoScrollNumSquaresBehind) *
                               currentSong->xZoom[NAVIGATION_ARRANGEMENT];

        // If now is the time to scroll to a different position (usually one
        // square)...
        if (newScrollPos != currentSong->xScroll[NAVIGATION_ARRANGEMENT]) {
          ...
          if (wasLessThanZero) { autoScrollNumSquaresBehind = getSquareFromPos(actualCurrentPos); }

          if (PadLEDs::flashCursor == FLASH_CURSOR_FAST) {
            PadLEDs::clearTickSquares();  // Make sure new fast flashes get sent
                                          // out
            mustRedrawTickSquares = true; // Make sure this gets sent below here
          }
          if (currentUIMode != UI_MODE_HORIZONTAL_ZOOM) { uiNeedsRendering(this, 0xFFFFFFFF, 0); }
        }
      }
    ...
    if (newTickSquare != lastTickSquare || mustRedrawTickSquares) {
      ...
      for (int yDisplay = 0; yDisplay < displayHeight; yDisplay++) {
        ...
        tickSquares[yDisplay] = (currentSong->getAnyOutputsSoloingInArrangement() &&
                                 (!output || !output->soloingInArrangementMode)) ?
                                    255 :
                                    newTickSquare;
        colours[yDisplay]     = (output && output->recordingInArrangement) ?
                                    2 :
        ...
        if (arrangement.hasPlaybackActive() && currentUIMode != UI_MODE_EXPLODE_ANIMATION) {
          ...
          if (output->recordingInArrangement) { uiNeedsRendering(this, (1 << yDisplay), 0); }
        }
      }
...
void ArrangerView::autoScrollOnPlaybackEnd() {
  ...
  if (doingAutoScrollNow && isUIModeWithinRange(autoScrollPlaybackEndUIModes) &&
      !Buttons::isButtonPressed(xEncButtonX, xEncButtonY)) { // Don't do it if they're instantly
                                                             // restarting playback again

    uint32_t xZoom        = currentSong->xZoom[NAVIGATION_ARRANGEMENT];
    int32_t  newScrollPos = ((uint32_t)(arrangement.playbackStartedAtPos + (xZoom >> 1)) / xZoom -
                            autoScrollNumSquaresBehind) *
                           xZoom;

    if (newScrollPos < 0) newScrollPos = 0;

    // If that actually puts us back to near where we were scrolled to when
    // playback began (which it usually will), just go back there exactly. Added
    // in response to Michael noting that if you do an UNDO and then also stop
    // playback while recording e.g. MIDI to arranger, it scrolls backwards
    // twice (if you have "follow" on). Actually it seems that in that
    // situation, undoing (probably due to other mechanics that get enacted)
    // won't let it take you further than 1 screen back from the play-cursor
...
void ArrangerView::playbackEnded() {
  if (currentPlaybackMode == &arrangement) { autoScrollOnPlaybackEnd(); }
...
void ArrangerView::clipNeedsReRendering(Clip* clip) {
  ...
  for (int yDisplay = 0; yDisplay < displayHeight; yDisplay++) {
    ...
    if (output == clip->output) {
      // In a perfect world we'd see if the Clip is actually horizontally
      // scrolled on-screen
