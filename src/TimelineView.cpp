...
bool TimelineView::calculateZoomPinSquares(uint32_t oldScroll,
  ...
  int32_t zoomPinSquareBig =
      ((int64_t)(int32_t)(oldScroll - newScroll) << 16) / (int32_t)(newZoom - oldZoom);

  for (int i = 0; i < displayHeight; i++) { PadLEDs::zoomPinSquare[i] = zoomPinSquareBig; }
...
int TimelineView::horizontalEncoderAction(int offset) {
  ...
  // These next two, I had here before adding the actual SD lock check /
  // remind-later above. Maybe they're not still necessary? If either was true,
  // wouldn't sdRoutineLock be true also for us to have gotten here?
  if (pendingUIRenderingLock)
    return ACTION_RESULT_REMIND_ME_OUTSIDE_CARD_ROUTINE; // Would possibly
                                                         // prefer to have this
                                                         // case cause it to
                                                         // still come back
                                                         // later and do it, but
  ...
  if (horizontalEncoderActionLock)
    return ACTION_RESULT_REMIND_ME_OUTSIDE_CARD_ROUTINE; // Really wouldn't want
                                                         // to get in here
                                                         // multiple times,
                                                         // while pre-rendering
                                                         // the waveforms for
  ...
  if (isUIModeActive(UI_MODE_HOLDING_HORIZONTAL_ENCODER_BUTTON)) {
    ...
    if (isUIModeActiveExclusively(UI_MODE_HOLDING_HORIZONTAL_ENCODER_BUTTON)) {
      ...
      int32_t  newScroll =
          currentSong->xScroll[navSysId] / (newZoom * displayWidth) * (newZoom * displayWidth);
  ...
  // Encoder button not pressed, scroll
    ...
    if (!Buttons::isShiftButtonPressed()) {
      ...
      int32_t newXScroll =
          currentSong->xScroll[navSysId] + offset * currentSong->xZoom[navSysId] * displayWidth;
      ...
      if (newXScroll < getMaxLength() || offset < 0) {
        if (newXScroll != currentSong->xScroll[navSysId]) { initiateXScroll(newXScroll); }
      }
      displayScrollPos();
    }
  }
...
void TimelineView::initiateXScroll(uint32_t newXScroll, int numSquaresToScroll) {
  ...
  if (anyAnimationToDo) {
    currentUIMode |= UI_MODE_HORIZONTAL_SCROLL; // Must set this before calling
                                                // PadLEDs::setupScroll(), which
                                                // might then unset it
...
bool TimelineView::zoomToMax(bool inOnly) {
  ...
  if (maxZoom != oldZoom && (!inOnly || maxZoom < oldZoom)) {
    ...
    int32_t newScroll =
        currentSong->xScroll[getNavSysId()] / (maxZoom * displayWidth) * (maxZoom * displayWidth);
...
void TimelineView::initiateXZoom(int zoomMagnitude, int32_t newScroll, uint32_t oldZoom) {
  ...
  memcpy(PadLEDs::imageStore[(zoomMagnitude < 0) ? displayHeight : 0],
         PadLEDs::image,
  ...
  bool anyToAnimate =
      calculateZoomPinSquares(oldScroll, newScroll, currentSong->xZoom[getNavSysId()], oldZoom);
  ...
  if (anyToAnimate) {
    ...
    renderMainPads(0xFFFFFFFF,
                   &PadLEDs::imageStore[storeOffset],
                   &PadLEDs::occupancyMaskStore[storeOffset],
                   true);
...
bool TimelineView::scrollLeftIfTooFarRight(int32_t maxLength) {
  ...
  if (getPosFromSquare(0) >= maxLength) {
    initiateXScroll(currentSong->xScroll[getNavSysId()] -
                    currentSong->xZoom[getNavSysId()] * displayWidth);
    // displayScrollPos();
    return true;
  }
  return false;
...
int32_t TimelineView::getPosFromSquare(int32_t square, int32_t xScroll, uint32_t xZoom) {
  ...
  if (inTripletsView()) {
    ...
    if (xZoom < currentSong->tripletsLevel) {
      int32_t prevBlockStart =
          xScroll / (currentSong->tripletsLevel * 3) * (currentSong->tripletsLevel * 3);
      int32_t blockStartRelativeToScroll = (prevBlockStart - xScroll) / 3 * 4; // Negative or 0

      int32_t posRelativeToBlock = ((int32_t)square) * (xZoom * 4 / 3) -
                                   blockStartRelativeToScroll; // Relative to block start pos
      uint32_t numBlocksIn = posRelativeToBlock / (currentSong->tripletsLevel * 4);

      // These two lines affect the resulting "pos" of cols which are
      // "undefined" so they can be detected as such
...
int32_t
TimelineView::getSquareFromPos(int32_t pos, bool* rightOnSquare, int32_t xScroll, uint32_t xZoom) {

  int32_t posRelativeToScroll = pos - xScroll;

  if (inTripletsView()) {
    if (xZoom < currentSong->tripletsLevel) {
      int32_t blockStartPos =
          xScroll / (currentSong->tripletsLevel * 3) * (currentSong->tripletsLevel * 3);
      ...
      if (rightOnSquare)
        *rightOnSquare = (posRelativeToBlockStart % (xZoom * 4 / 3) ==
                          0); // Will the % be ok if it's negative? No! :O

      int32_t numBlocksIn = divide_round_negative(
          posRelativeToBlockStart,
      ...
      int32_t semiFinal = posRelativeToBlockStart + blockStartRelativeToScroll * 4 / 3 +
                          numBlocksIn * currentSong->tripletsLevel;
      int32_t final = divide_round_negative(semiFinal, xZoom * 4 / 3);
      return final;
    } else if (xZoom < currentSong->tripletsLevel * 2) {
      int posRelativeToTripletsStart =
          posRelativeToScroll %
          (currentSong->tripletsLevel * 3); // Will the % be ok if it's negative? No! :O
      if (rightOnSquare)
  ...
  if (rightOnSquare)
    *rightOnSquare =
        (posRelativeToScroll >= 0 &&
         (posRelativeToScroll % xZoom) == 0); // Will the % be ok if it's negative? No! :O

  // Have to divide the two things separately before subtracting, otherwise
  // negative results get rounded the wrong way!
