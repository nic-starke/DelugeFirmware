...
bool SoundInstrument::writeDataToFile(Clip* clipForSavingOutputOnly, Song* song) {
  ...
  // MelodicInstrument::writeDataToFile(clipForSavingOutputOnly, song); // Nope,
  // this gets called within the below call
  ...
  // Or if saving Song...
    ...
    // If no activeClip, that means no Clip has this Output, so there should be
    // a backedUpParamManager that we should use
    if (!activeClip)
...
// arpSettings optional - no need if you're loading a new V2.0 song where
// Instruments are all separate from Clips and won't store any arp stuff
int SoundInstrument::readFromFile(Song* song, Clip* clip, int32_t readAutomationUpToPos) {

  char                           modelStackMemory[MODEL_STACK_MAX_SIZE];
  ModelStackWithModControllable* modelStack = setupModelStackWithSong(modelStackMemory, song)
                                                  ->addTimelineCounter(clip)
                                                  ->addModControllableButNoNoteRow(this);
...
void SoundInstrument::renderOutput(ModelStack*   modelStack,
  ...
  if (!skippingRendering) {
    Sound::render(modelStackWithThreeMainThings,
                  startPos,
                  numSamples,
                  reverbBuffer,
                  sideChainHitPending,
  ...
  if (playbackHandler.isEitherClockActive() && !playbackHandler.ticksLeftInCountIn &&
    ...
    ParamCollectionSummary* patchedParamsSummary =
        &modelStackWithThreeMainThings->paramManager
             ->summaries[1]; // No time to call the proper function and do error
                             // checking, sorry.
    ...
    yesTickParamManagerForClip:
      ...
      ParamCollectionSummary* unpatchedParamsSummary =
          &modelStackWithThreeMainThings->paramManager
               ->summaries[0]; // No time to call the proper function and do
      ...
      ParamCollectionSummary* patchCablesSummary =
          &modelStackWithThreeMainThings->paramManager
               ->summaries[2]; // No time to call the proper function and do
                               // error checking, sorry.
      ...
      ParamCollectionSummary* expressionParamsSummary =
          &modelStackWithThreeMainThings->paramManager
               ->summaries[3]; // No time to call the proper function and do
                               // error checking, sorry.
...
void SoundInstrument::setupPatching(ModelStackWithTimelineCounter* modelStack) {
  ...
  if (clip) {
    ...
    paramManager =
        (ParamManagerForTimeline*)modelStack->song->getBackedUpParamManagerPreferablyWithClip(this,
                                                                                              NULL);
...
bool SoundInstrument::setActiveClip(ModelStackWithTimelineCounter* modelStack,
                                    int                            maySendMIDIPGMs) {
...
// Alternative to what's in the NonAudioInstrument:: implementation, which would
// almost work here, but we cut corner for Sound by avoiding going through the
// Arp and just talk directly to the Voices. (Despite my having made it now
// actually need to talk to the Arp too, as below...) Note, this virtual
// function actually overrides/implements from two base classes -
// MelodicInstrument and ModControllable.
void SoundInstrument::polyphonicExpressionEventOnChannelOrNote(int newValue,
                                                               int whichExpressionDimension,
                                                               int channelOrNoteNumber,
                                                               int whichCharacteristic) {
  int s = whichExpressionDimension + PATCH_SOURCE_X;

  // sourcesChanged |= 1 << s; // We'd ideally not want to apply this to all
  // voices though...
  ...
  // Must update MPE values in Arp too - useful either if it's on, or if we're
  // in true monophonic mode - in either case, we could need to suddenly do a
  // note-on for a different note that the Arp knows about, and need these MPE
  // values.
  int n, nEnd;
...
void SoundInstrument::sendNote(ModelStackWithThreeMainThings* modelStack,
                               bool                           isOn,
                               int                            noteCode,
  ...
  if (isOn) {
    ...
    if (instruction.noteCodeOffPostArp != ARP_NOTE_NONE) {
      ...
      if (!modelStack->paramManager)
        numericDriver.freezeWithError("E402"); // Previously we were allowed to receive a NULL
                                               // paramManager, then would just crudely do an
                                               // unassignAllVoices(). But I'm pretty sure this
...
void SoundInstrument::compensateInstrumentVolumeForResonance(
...
// Any time it gets edited, we want to grab the default arp settings from the
// activeClip
...
int32_t SoundInstrument::doTickForwardForArp(ModelStack* modelStack, int32_t currentPos) {
  ...
  int32_t ticksTilNextArpEvent =
      arpeggiator.doTickForward(&((InstrumentClip*)activeClip)->arpSettings,
                                &instruction,
                                currentPos,
                                activeClip->currentlyPlayingReversed);

  ModelStackWithSoundFlags* modelStackWithSoundFlags =
      modelStackWithThreeMainThings->addSoundFlags();
...
bool SoundInstrument::noteIsOn(int noteCode) {
  ...
  if (arpSettings) {
    if (arpSettings->mode != ARP_MODE_OFF || polyphonic == POLYPHONY_LEGATO ||
        polyphonic == POLYPHONY_MONO) {
  ...
  for (int v = ends[0]; v < ends[1]; v++) {
    ...
    if ((thisVoice->noteCodeAfterArpeggiation == noteCode) &&
        thisVoice->envelopes[0].state <
            ENVELOPE_STAGE_RELEASE) { // Ignore releasing notes. Is this right?
      return true;
    }
  }
  return false;
