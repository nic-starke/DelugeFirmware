...
#define RENDER_OSC_SYNC(storageFunctionName,                                                       \
  ...
  uint32_t samplesIncludingNextCrossoverSample =                                                   \
      1; /* A starting value that'll be added to. It's 1 because we want to                        \
            include the 1 extra sample at the end - the crossover sample. */                       \
  startRenderingASyncLabel:                                                                        \
  uint32_t distanceTilNextCrossoverSample = -resetterPhase - (resetterPhaseIncrement >> 1);        \
  samplesIncludingNextCrossoverSample +=                                                           \
      (uint32_t)(distanceTilNextCrossoverSample - 1) / resetterPhaseIncrement;                     \
  bool shouldBeginNextSyncAfter =                                                                  \
  ...
  storageFunctionName(valueFunctionName);                                                          \
                                                                                                   \
  /* Sort out the crossover sample at the *start* of that window we just did,                      \
   * if there was one. */                                                                          \
  ...
  if (shouldBeginNextSyncAfter) {                                                                  \
    /* We've just done a crossover (i.e. hit a sync point) at the end of that                      \
     * window, so start thinking about that and planning the next window. */                       \
    ...
    extraInstructionsForCrossoverSampleRedo;                                                       \
                                                                                                   \
    resetterPhase +=                                                                               \
        resetterPhaseIncrement * (samplesIncludingNextCrossoverSample -                            \
                                  renderedASyncFromItsStartYet); /* We want this to always show    \
                                                                    one sample late at this point  \
                                                                    (why again?). */               \
    /* The first time we get here, it won't yet be, so make it so. */                              \
                                                                                                   \
    fadeBetweenSyncs =                                                                             \
        multiply_32x32_rshift32((int32_t)resetterPhase,                                            \
                                resetterDivideByPhaseIncrement) /* The result of that comes out as \
                                                                   between "-0.5 and 0.5",         \
                                                                   represented as +-(1<<14) */     \
        << 17; /* And this makes it "full-scale", so "1" is 1<<32. */                              \
    phase = multiply_32x32_rshift32(fadeBetweenSyncs, phaseIncrement) + retriggerPhase;            \
                                                                                                   \
    phase -= phaseIncrement; /* Because we're going back and redoing the last                      \
                                sample. */                                                         \
    renderedASyncFromItsStartYet = true;                                                           \
    samplesIncludingNextCrossoverSample =                                                          \
        2; /* Make this 1 higher now, because resetterPhase's value is 1                           \
              sample later than what it "is in reality". */                                        \
    goto startRenderingASyncLabel;                                                                 \
  }                                                                                                \
                                                                                                   \
  /* We're not beginning a next sync, so are not going to reset phase, so need                     \
   * to update (increment) it to keep it valid. */                                                 \
  phase += phaseIncrement * numSamplesThisSyncRender;

#define RENDER_WAVETABLE_LOOP(dontCare)           \
  {                                               \
    doRenderingLoop(bufferStartThisSync,          \
                    bufferEndThisSyncRender,      \
                    firstCycleNumber,             \
                    bandHere,                     \
                    phaseTemp,                    \
                    phaseIncrement,               \
...
#define RENDER_SINGLE_CYCLE_WAVEFORM_LOOP(dontCare)     \
  {                                                     \
    doRenderingLoopSingleCycle(bufferStartThisSync,     \
                               bufferEndThisSyncRender, \
                               bandHere,                \
                               phaseTemp,               \
                               phaseIncrement,          \
...
#define WAVETABLE_EXTRA_INSTRUCTIONS_FOR_CROSSOVER_SAMPLE_REDO                    \
  {                                                                               \
    crossCycleStrength2 +=                                                        \
        crossCycleStrength2Increment * (samplesIncludingNextCrossoverSample - 1); \
  }
