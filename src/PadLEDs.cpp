...
// We stash these here for during UI-transition animation, because if that's
// happening as part of an undo, the Sample might not be there anymore
...
bool shouldNotRenderDuringTimerRoutine() {
  return (renderingLock || currentUIMode == UI_MODE_EXPLODE_ANIMATION ||
...
void clearMainPadsWithoutSending() {
  for (int y = 0; y < displayHeight; y++) { memset(image[y], 0, displayWidth * 3); }
}

void clearSideBar() {
  for (int y = 0; y < displayHeight; y++) { memset(&image[y][displayWidth], 0, 6); }
...
inline void sendRGBForOneCol(int x) {
  for (int y = 0; y < displayHeight; y++) { sendRGBForOnePadFast(x, y, image[y][x]); }
}
...
void sendRGBForOnePadFast(int x, int y, const uint8_t* colourSource) {
  ...
  if (flashCursor == FLASH_CURSOR_SLOW && slowFlashSquares[y] == x &&
      currentUIMode != UI_MODE_HORIZONTAL_SCROLL) {
...
void renderInstrumentClipCollapseAnimation(int xStart, int xEndOverall, int progress) {
  ...
  uint16_t* intensity2Array =
      (uint16_t*)&miscStringBuffer[MAX_NUM_ANIMATED_ROWS * sizeof(uint16_t)];
  int8_t newRowPosition1Array[MAX_NUM_ANIMATED_ROWS];

  for (int i = 0; i < numAnimatedRows; i++) {
    int newRowPosition =
        (int)animatedRowGoingFrom[i] * 65536 +
  ...
  int greyStart = instrumentClipView.getSquareFromPos(clipLength - 1,
                                                      NULL,
  ...
  if (currentUIMode == UI_MODE_NOTEROWS_EXPANDING_OR_COLLAPSING) {
    ...
    greyTop = animatedRowGoingTo[0] + 1 +
              (((displayHeight - animatedRowGoingTo[0]) * progress + 32768) >> 16);
  ...
  for (int col = xStart; col < xEndOverall; col++) {
    ...
    if (col < displayWidth) {
      if (col >= xEnd)
        continue; // It's beyond the end of the Clip, and it's already been
                  // filled in grey
    ...
    for (int i = 0; i < numAnimatedRows; i++) {
      ...
      if (isUIModeActive(UI_MODE_INSTRUMENT_CLIP_COLLAPSING) ||
        ...
        if (col == displayWidth) {
          ...
          for (int colour = 0; colour < 3; colour++) {
            int newColour =
                rshift_round((int)squareColours[colour] * progress, 16) +
      ...
      if (newRowPosition1Array[i] >= 0 && newRowPosition1Array[i] < displayHeight) {
        drawSquare(squareColours,
                   intensity1,
                   PadLEDs::image[newRowPosition1Array[i]][col],
                   &occupancyMask[newRowPosition1Array[i]][col],
                   occupancyMaskStore[i][col]);
      }
      ...
      if (newRowPosition1Array[i] >= -1 && newRowPosition1Array[i] < displayHeight - 1) {
        drawSquare(squareColours,
                   ...
                   &occupancyMask[newRowPosition1Array[i] + 1][col],
                   occupancyMaskStore[i][col]);
      }
    }
  }
...
void renderAudioClipCollapseAnimation(int progress) {
  ...
  int endSquareDisplay =
      divide_round_negative(clipLength - currentSong->xScroll[NAVIGATION_CLIP] - 1,
                            currentSong->xZoom[NAVIGATION_CLIP]); // Rounds it well down, so we get
                                                                  // the "final square" kinda...
  int greyStart = endSquareDisplay + 1;
  int xEnd      = getMin(displayWidth, greyStart);

  for (int col = 0; col < xEnd; col++) {
    waveformRenderer.renderOneColForCollapseAnimation(col,
                                                      col,
...
void renderAudioClipExplodeAnimation(int explodedness, bool shouldSendOut) {
  ...
  for (int xDestSquareRightEdge = 0; xDestSquareRightEdge <= displayWidth; xDestSquareRightEdge++) {
    ...
    int xSourceLeftEdge = xSourceRightEdge; // What was the last square's right edge is now the
                                            // current square's left edge
    ...
    // For first iteration, we just wanted that value, to use next time - and we
    // should get out now
    ...
    waveformRenderer.renderOneColForCollapseAnimationZoomedOut(xSourceLeftEdgeLimited,
                                                               xSourceRightEdgeLimited,
                                                               xDest,
                                                               sampleMaxPeakFromZero,
                                                               explodedness,
                                                               PadLEDs::image,
    ...
    if (xSourceRightEdge >= displayWidth)
      break; // If we got to the right edge of everything we want to draw
             // onscreen
  }
...
void renderExplodeAnimation(int explodedness, bool shouldSendOut) {
  ...
  // Set up some stuff for each x-pos that we don't want to be constantly
  // re-calculating
  ...
  for (int xSource = 0; xSource < displayWidth; xSource++) {
    ...
    // xOriginBig = getMin(xOriginBig, explodeAnimationXStartBig +
    // explodeAnimationXWidthBig - 65536);

    xOriginBig &= ~(uint32_t)65535; // Make sure each pixel's "origin-point" is right on
                                    // an exact square - rounded to the left. That'll
                                    // match what we'll see in the arranger
    ...
    // Ok, so we're gonna squish this source square amongst 4 destination
    // squares
    xDestArray[xSource] = xDestBig >> 16;

    // May as well narrow things down if we know now that some xSources won't
    // end up onscreen
  ...
  for (int ySource = -1; ySource < displayHeight + 1; ySource++) {
    ...
    int yDestBig =
        explodeAnimationYOriginBig + (((int64_t)ySourceBigRelativeToOrigin * explodedness) >> 16);
    ...
    for (int xSource = xStart; xSource < xEnd; xSource++) {
      ...
      if (occupancyMaskStore[ySource + 1][xSource]) { // If there's actually anything in this
                                                      // source square...
        ...
        for (int xOffset = 0; xOffset < 2; xOffset++) {
          ...
          for (int yOffset = 0; yOffset < 2; yOffset++) {
            ...
            drawSquare(imageStore[ySource + 1][xSource],
                       intensityNow,
                       PadLEDs::image[yNow][xNow],
                       &occupancyMask[yNow][xNow],
                       occupancyMaskStore[ySource + 1][xSource]);
          }
        }
      }
    }
  ...
  if (shouldSendOut) {
    ...
    uiTimerManager.setTimer(TIMER_MATRIX_DRIVER,
                            35); // Nice small number of milliseconds here. This
                                 // animation is prone to looking jerky
  }
}
...
void timerRoutine() {
  ...
  if (uartGetTxBufferSpace(UART_ITEM_PIC_PADS) <=
      NUM_BYTES_IN_MAIN_PAD_REDRAW + NUM_BYTES_IN_SIDEBAR_REDRAW) {
    setTimerForSoon();
    return;
  }
  ...
  if (isUIModeActive(UI_MODE_HORIZONTAL_ZOOM)) {
  ...
  else if (isUIModeActive(UI_MODE_AUDIO_CLIP_EXPANDING) ||
           isUIModeActive(UI_MODE_AUDIO_CLIP_COLLAPSING)) {
    renderAudioClipExpandOrCollapse();
  }

  else if (isUIModeActive(UI_MODE_INSTRUMENT_CLIP_COLLAPSING) ||
           isUIModeActive(UI_MODE_INSTRUMENT_CLIP_EXPANDING)) {
      ...
      // Otherwise, there's stuff we want to fade in / to
        ...
        if (animationDirection == 1) {
          if (currentSong->currentClip->type == CLIP_TYPE_INSTRUMENT) {
            ...
            goto stopFade; // No need for fade since no sidebar, and also if we
                           // tried it'd get glitchy cos we're not set up for it
          }
        } else {
          changeRootUI(&arrangerView);

          if (arrangerView.doingAutoScrollNow)
            goto stopFade; // If we suddenly just started doing an auto-scroll,
                           // there's no time to fade
        }
    ...
    stopFade:
      currentUIMode = UI_MODE_NONE;
      renderingNeededRegardlessOfUI(); // Just in case some waveforms couldn't
                                       // be rendered when the store was written
                                       // to, we want to re-render everything
                                       // now
    } else {
      renderFade(progress);
    }
    ...
    if (greyoutChangeDirection != 0) {
      float amountDone =
          (float)(AudioEngine::audioSampleTimer - greyoutChangeStartTime) / greyoutSpeed;
      if (greyoutChangeDirection > 0) {
        if (amountDone > 1) {
          greyoutChangeDirection = 0;
          setGreyoutAmount(1);
...
#else
  for (int col = 0; col < sideBarWidth; col++) { sortLedsForCol(col + displayWidth); }
#endif
...
void renderZoom() {
  ...
  // int32_t negativeFactorProgress = pow(zoomFactor, transitionProgress - 1) *
  // 134217728; // Sorry, the purpose of this variable has got a bit cryptic, it
  // exists after much simplification
  ...
  // inImageWidthComparedToNormal and outImageWidthComparedToNormal show how
  // much bigger than "normal" those two images are to appear. E.g. when fully
  // zoomed out, the out-image would be "1" (65536), and the in-image would be
  // "0.5" (32768). And so on.

  uint32_t inImageTimesBiggerThanNormal =
      interpolateTable(fine, 10, expTableSmall); // This could be changed to run on a bigger
                                                 // number of bits in input
  inImageTimesBiggerThanNormal = increaseMagnitude(inImageTimesBiggerThanNormal, coarse - 14);

  renderZoomWithProgress(inImageTimesBiggerThanNormal,
                         sineValue,
                         &imageStore[0][0][0],
                         &imageStore[displayHeight][0][0],
                         0,
                         0,
                         displayWidth,
                         displayWidth,
...
void renderZoomWithProgress(int      inImageTimesBiggerThanNative,
  ...
  uint32_t inImageTimesSmallerThanNative =
      4294967295u / inImageTimesBiggerThanNative; // How many squares of the zoomed-in image
                                                  // fit into each square of our output image,
                                                  // at current zoom level
  uint32_t outImageTimesSmallerThanNative =
      4294967295u / outImageTimesBiggerThanNative; // How many squares of the zoomed-out image
                                                   // fit into each square of our output
  ...
  int32_t* outputSquareStartOnInImage =
      (int32_t*)&miscStringBuffer[displayWidth * sizeof(int32_t) * 0];
  int32_t* outputSquareEndOnInImage =
      (int32_t*)&miscStringBuffer[displayWidth * sizeof(int32_t) * 1];
  int32_t* outputSquareStartOnOutImage =
      (int32_t*)&miscStringBuffer[displayWidth * sizeof(int32_t) * 2];
  int32_t* outputSquareEndOnOutImage =
      (int32_t*)&miscStringBuffer[displayWidth * sizeof(int32_t) * 3];
  uint16_t* inImageFadePerCol =
      (uint16_t*)shortStringBuffer; // 0 means show none. 65536 means show all, only
...
#define zoomPinSquareOuter zoomPinSquare
  ...
  for (int yDisplay = 0; yDisplay < displayHeight; yDisplay++) {
    if (transitionTakingPlaceOnRow[yDisplay]) {
      ...
      // If this row doesn't have the same pin-square as the last, we have to
      // calculate some stuff. Otherwise, this can be reused.
      if (zoomPinSquare[yDisplay] != lastZoomPinSquareDone) {
        lastZoomPinSquareDone = zoomPinSquare[yDisplay];

        // Work out what square the thinner image begins at (i.e. its left-most
        // edge)
        int32_t inImagePos0Onscreen =
            zoomPinSquare[yDisplay] -
        ...
        // Do some pre-figuring-out for each column of the final-rendered image
        // - which we can hopefully refer to for each row
        for (int xDisplay = 0; xDisplay < displayWidth; xDisplay++) {
          ...
          // Work out how much of this square will be covered by the "in"
          // (thinner) image (often it'll be all of it, or none)
          ...
          // Convert that into knowing what proportion of colour from each image
          // we want to grab
          ...
          int32_t outputSquareLeftEdgePositionOnInImageRelativeToPinSquare =
              ((int64_t)outputSquareLeftEdgePositionRelativeToPinSquare *
               inImageTimesSmallerThanNative) >>
              16;
          int32_t outputSquareLeftEdgePositionOnOutImageRelativeToPinSquare =
              ((int64_t)outputSquareLeftEdgePositionRelativeToPinSquare *
               outImageTimesSmallerThanNative) >>
              16;

          // Work out, for this square/col/pixel, the corresponding local
          // coordinate for both the in- and out-images. Do that for both the
          // leftmost and rightmost edge of this square
          outputSquareStartOnOutImage[xDisplay] =
              zoomPinSquareOuter[yDisplay] -
              outputSquareLeftEdgePositionOnOutImageRelativeToPinSquare;
          outputSquareStartOnInImage[xDisplay] =
              zoomPinSquareInner[yDisplay] -
              outputSquareLeftEdgePositionOnInImageRelativeToPinSquare;
      ...
      for (int xDisplay = 0; xDisplay < displayWidth; xDisplay++) {
        ...
        if (inImageFadePerCol[xDisplay]) {
          ...
          renderZoomedSquare(outputSquareStartOnInImage[xDisplay],
                             outputSquareEndOnInImage[xDisplay],
                             inImageTimesBiggerThanNative,
                             inImageFadePerCol[xDisplay],
                             outValue,
                             innerImage,
                             innerImageRightEdge,
                             &drawingAnything);
        ...
        {
          renderZoomedSquare(outputSquareStartOnOutImage[xDisplay],
                             outputSquareEndOnOutImage[xDisplay],
                             outImageTimesBiggerThanNative,
...
void renderZoomedSquare(int32_t      outputSquareStartOnSourceImage,
  ...
  int32_t outImageStartSquareLeftEdge =
      (uint32_t)(outputSquareStartOnSourceImage) & ~(uint32_t)65535;
  for (int32_t sourceSquareLeftEdge = getMax((int32_t)0, outImageStartSquareLeftEdge); true;
    ...
    *drawingAnything = true;
    ...
    uint32_t intensity             = getMin(sourceSquareRightEdge, outputSquareEndOnSourceImage) -
                         getMax(sourceSquareLeftEdge,
                                outputSquareStartOnSourceImage); // Will end up at max 65536
...
#if DELUGE_MODEL == DELUGE_MODEL_40_PAD
  sendOutMainPadColours();
  if (areaToScroll > displayWidth) { sendOutSidebarColours(); }
#else
...
void setupScroll(int8_t  thisScrollDirection,
...
int getTransitionProgress() {
  return ((uint64_t)(AudioEngine::audioSampleTimer - transitionStartTime) * 65536) /
         transitionLength;
}

void copyBetweenImageStores(uint8_t* __restrict__ dest,
...
void moveBetweenImageStores(uint8_t* dest,
                            uint8_t* source,
                            int      destWidth,
                            int      sourceWidth,
                            int      copyWidth) {
