...
int32_t spareRenderingBuffer[3][SSI_TX_BUFFER_NUM_SAMPLES]
    __attribute__((aligned(CACHE_LINE_SIZE)));

int32_t oscSyncRenderingBuffer[SSI_TX_BUFFER_NUM_SAMPLES + 4]
    __attribute__((aligned(CACHE_LINE_SIZE))); // Hopefully I could make this use the
                                               // spareRenderingBuffer instead...
...
// Unusually, modelStack may be supplied as NULL, because when unassigning all
// voices e.g. on song swap, we won't have it. You'll normally want to call
// audioDriver.voiceUnassigned() after this.
...
void Voice::unassignStuff() {
  for (int s = 0; s < NUM_SOURCES; s++) {
    for (int u = 0; u < assignedToSound->numUnison; u++) { unisonParts[u].sources[s].unassign(); }
  }
}
...
bool Voice::noteOn(ModelStackWithVoice* modelStack,
  ...
  // Setup "half-baked" envelope output values. These need to exist before we do
  // the initial patching below - and it's only after that that we can render
  // the "actual" envelope output values, taking their own input patching into
  // account.
  for (int e = 0; e < numEnvelopes; e++) {

    // If no attack-stage...
    if (paramManager->getPatchedParamSet()->getValue(PARAM_LOCAL_ENV_0_ATTACK + e) ==
        -2147483648u) {
  ...
  sourceValues[PATCH_SOURCE_VELOCITY] =
  ...
  // Patch all sources to exclusive params, to give them an initial value.
  // Exclusive params (params with just 1 source) aren't continuously
  // recalculated, so they need that initial value. Remember, calculating that
  // initial value also takes into account the "preset value". This probably
  // isn't strictly necessary for sources which we know will be constantly
  // changing, because that would make patching constantly calculate too. But
  // that's only really the envelopes, plus the LFOs (just the local one?) if
  // they're not square
  for (int s = 0; s < FIRST_LOCAL_SOURCE; s++) { sourceValues[s] = sound->globalSourceValues[s]; }
  patcher.performInitialPatching(sound, paramManager);

  // Setup and render envelopes - again. Because they're local params (since
  // mid-late 2017), we really need to render them *after* initial patching is
  // performed.
  for (int e = 0; e < numEnvelopes; e++) {
  ...
  for (int s = 0; s < NUM_SOURCES; s++) {
    ...
    // Various stuff in this block is only relevant for OSC_TYPE_SAMPLE, but no
    // real harm in it just happening in other cases.
    ...
    if (sourceEverActive) {
      ...
      if (sound->getSynthMode() != SYNTH_MODE_FM &&
        ...
        // Set up MultiRange
        MultiRange* range =
            sound->sources[s].getRange(noteCodeAfterArpeggiation + sound->transpose);
        if (!range) { // There could be no Range for a SAMPLE or WAVETABLE
                      // Source that just hasn't had a file loaded, like how
                      // OSC2 very often would be sitting
          goto gotInactive;
        }

        AudioFileHolder* holder = range->getAudioFileHolder();
        // Only actually set the Range as ours if it has an AudioFile - so that
        // we'll always know that any VoiceSource's range definitely has a
        // sample
        if (!holder->audioFile) { goto gotInactive; }
        ...
        if (sound->sources[s].oscType == OSC_TYPE_SAMPLE) {
          ...
          if (sound->sources[s].repeatMode == SAMPLE_REPEAT_STRETCH) {
            ...
            guides[s].sequenceSyncStartedAtTick =
                ...
                ticksLate; // No harm setting this even if it's not valid or
                           // needed. And yes, this is supposed to use
                           // lastSwungTickActioned, not
                           // getActualSwungTickCount(). ticksLate is relative
                           // to that.
          }
        }
      }
  ...
  for (int s = 0; s < NUM_SOURCES; s++) {
    ...
    // int samplesLateHere = samplesLate; // Make our own copy of this - we're
    // going to deactivate it if we're in STRETCH mode, cos that works
    // differently
    ...
    for (int u = 0; u < sound->numUnison; u++) {
      ...
      // Check that we already marked this unison-part-source as active. Among
      // other things, this ensures that if the osc is set to SAMPLE, there
      // actually is a sample loaded.
      if (unisonParts[u].sources[s].active) {
        bool success = unisonParts[u].sources[s].noteOn(this,
                                                        source,
                                                        &guides[s],
                                                        samplesLate,
                                                        ...
                                                        sound->synthMode);
        if (!success)
          return false; // This shouldn't really ever happen I don't think
                        // really...
      }
    }
  }
...
void Voice::changeNoteCode(ModelStackWithVoice* modelStack,
  ...
  // We definitely want to go to these values smoothly. Probably wish it was
  // even smoother... Actually nah this sounds / feels great!
  for (int m = 0; m < NUM_EXPRESSION_DIMENSIONS; m++) {
    localExpressionSourceValuesBeforeSmoothing[m] = newMPEValues[m] << 16;
    // TODO: what if there's just channel aftertouch, and it's still held
    // down...
  }
...
void Voice::randomizeOscPhases(Sound* sound) {
  for (int u = 0; u < sound->numUnison; u++) {
    ...
    if (sound->getSynthMode() == SYNTH_MODE_FM) {
      for (int m = 0; m < numModulators; m++) { unisonParts[u].modulatorPhase[m] = getNoise(); }
    }
  }
}
...
void Voice::calculatePhaseIncrements(ModelStackWithVoice* modelStack) {
  ...
  for (int s = 0; s < NUM_SOURCES; s++) {
    ...
    makeInactive: // Frequency too high to render! (Higher than 22.05kHz)
      for (int u = 0; u < sound->numUnison; u++) { unisonParts[u].sources[s].active = false; }
      continue;
    }
    ...
    if (source->oscType == OSC_TYPE_SAMPLE &&
        guides[s].audioFileHolder) { // Do not do this for WaveTables
    ...
    if (sound->getSynthMode() != SYNTH_MODE_FM &&
      ...
      phaseIncrement =
          multiply_32x32_rshift32(noteIntervalTable[noteWithinOctave], pitchAdjustNeutralValue);
      ...
      // If shifting left...
        ...
        // (which means one semitone below the limit, because osc-cent + unison
        // could push it up a semitone)
    ...
    if (source->oscType == OSC_TYPE_SAMPLE) { // guides[s].sampleHolder
      phaseIncrement =
          ((SampleHolderForVoice*)guides[s].audioFileHolder)->fineTuner.detune(phaseIncrement);
    } else {
    ...
    // Or if multiple unison
      ...
      for (int u = 0; u < sound->numUnison; u++) {
        unisonParts[u].sources[s].phaseIncrementStoredValue =
  ...
  if (sound->getSynthMode() == SYNTH_MODE_FM) {
    for (int m = 0; m < numModulators; m++) {
      ...
      if (sound->getSmoothedPatchedParamValue(PARAM_LOCAL_MODULATOR_0_VOLUME + m, paramManager) ==
          -2147483648)
      ...
      // Or if multiple unison
        ...
        for (int u = 0; u < sound->numUnison; u++) {
          unisonParts[u].modulatorPhaseIncrement[m] =
              sound->unisonDetuners[u].detune(phaseIncrement);
        }
      }
    }
...
void Voice::noteOff(ModelStackWithVoice* modelStack, bool allowReleaseStage) {
  ...
  for (int s = 0; s < NUM_SOURCES; s++) { guides[s].noteOffReceived = true; }

  ParamManagerForTimeline* paramManager = (ParamManagerForTimeline*)modelStack->paramManager;
  Sound*                   sound        = (Sound*)modelStack->modControllable;

  // Only do it if note-offs are meant to be processed for this sound. Otherwise
  // ignore it.
      ...
      if (sound->sources[s].oscType == OSC_TYPE_SAMPLE && guides[s].loopEndPlaybackAtByte) {
        for (int u = 0; u < sound->numUnison; u++) {
          if (unisonParts[u].sources[s].active) {
            ...
            bool success =
                unisonParts[u].sources[s].voiceSample->noteOffWhenLoopEndPointExists(this,
                                                                                     &guides[s]);

            if (!success) { unisonParts[u].sources[s].unassign(); }
          }
        }
      }
    }
...
bool Voice::sampleZoneChanged(ModelStackWithVoice* modelStack, int s, int markerType) {
  ...
  if (!holder)
    return true; // If no holder, that means this Source/Sample is not currently
                 // playing, e.g. because its volume was set to 0.
  ...
  // Well, that is, make sure we're not past the new end. Being before the start
  // is ok, because we'll come back into the still-remaining part soon enough.
  ...
  for (int u = 0; u < sound->numUnison; u++) {
    ...
    if (voiceUnisonPartSource->active) {
      bool stillActive = voiceUnisonPartSource->voiceSample->sampleZoneChanged(&guides[s],
                                                                               sample,
                                                                               markerType,
                                                                               loopingType,
                                                                               getPriorityRating());
  ...
  // If none of this source still active, and no noise, see if the other source
  // still has any...
  if (!anyStillActive &&
      !paramManager->getPatchedParamSet()->params[PARAM_LOCAL_NOISE_VOLUME].containsSomething(
          -2147483648)) {
...
// Before calling this, you must set the filterSetConfig's doLPF and doHPF to
// default values
...
bool Voice::render(ModelStackWithVoice* modelStack,
  ...
  // If we've previously ignored a note-off, we need to check that the user
  // hasn't changed the preset so that we're now waiting for a note-off again
  if (previouslyIgnoredNoteOff && sound->allowNoteTails(modelStack, true)) { noteOff(modelStack); }

  // Do envelopes - if they're patched to something (always do the first one
  // though)
  for (int e = 0; e < numEnvelopes; e++) {
    if (e == 0 || (paramManager->getPatchCableSet()->sourcesPatchedToAnything[GLOBALITY_LOCAL] &
  ...
  bool unassignVoiceAfter =
      (envelopes[0].state == ENVELOPE_STAGE_OFF); //(envelopes[0].state >= ENVELOPE_STAGE_DECAY &&
                                                  // localSourceValues[PATCH_SOURCE_ENVELOPE_0 -
                                                  // FIRST_LOCAL_SOURCE] == -2147483648);
  // Local LFO
  if (paramManager->getPatchCableSet()->sourcesPatchedToAnything[GLOBALITY_LOCAL] &
      (1 << PATCH_SOURCE_LFO_LOCAL)) {
    int32_t old                          = sourceValues[PATCH_SOURCE_LFO_LOCAL];
    sourceValues[PATCH_SOURCE_LFO_LOCAL] = lfo.render(numSamples,
                                                      sound->lfoLocalWaveType,
                                                      paramFinalValues[PARAM_LOCAL_LFO_LOCAL_FREQ]);
  ...
  if (expressionParams) { bendRanges = expressionParams->bendRanges; }

  int32_t totalBendAmount = // Comes out as a 32-bit number representing a range
                            // of +-192 semitones
  ...
  if (portaEnvelopePos < 8388608) {
    ...
    // Move envelope on. Using the "release rate" lookup table gives by far the
    // best range of speed values
  ...
  // Decide whether to do an auto-release for sample. Despite this being
  // envelope-related, meaning we'd ideally prefer to do it before patching, we
  // can only do it after because we need to know pitch

  // If not already releasing and some release is set, and no noise-source...
  if (sound->getSynthMode() != SYNTH_MODE_FM && envelopes[0].state < ENVELOPE_STAGE_RELEASE &&
      hasReleaseStage() &&
      !paramManager->getPatchedParamSet()->params[PARAM_LOCAL_NOISE_VOLUME].containsSomething(
          -2147483648)) {
    ...
    for (int s = 0; s < NUM_SOURCES; s++) {
      ...
      // If it's not a sample, or it's not a play-once, or it has a loop-end
      // point but we haven't received the note-off, then we don't want the
      // auto-release feature for it
      if (source->oscType != OSC_TYPE_SAMPLE ||
          source->repeatMode != SAMPLE_REPEAT_ONCE // Don't do it for anything else. STRETCH is
                                                   // too hard to calculate
          || !guides[s].audioFileHolder ||
          (((SampleHolderForVoice*)guides[s].audioFileHolder)->loopEndPos &&
    ...
    if (whichSourcesNeedAttention) {
      ...
      int releaseStageLengthSamples =
          (uint32_t)8388608 / (uint32_t)paramFinalValues[PARAM_LOCAL_ENV_0_RELEASE];
      ...
      for (int s = 0; s < NUM_SOURCES; s++) {
        ...
        if (whichSourcesNeedAttention & (1 << s)) {
          ...
          // This Source needs an auto release applied. Calculate for the last
          // unison, because that'll have the higher pitch, so will be ending
          // soonest
          VoiceUnisonPartSource* voiceUnisonPartSource =
              &unisonParts[sound->numUnison - 1].sources[s];
          ...
          int bytesLeft = (int32_t)((uint32_t)guides[s].endPlaybackAtByte - (uint32_t)bytePos) *
          ...
          // Work out the actual sample read rate, from the "native" read rate
          // for the last unison, combined with the "pitch adjust" amount, and
          // the pitch adjust for this source alone. If the pitch goes
          // crazy-high, this will fall through and prevent auto-release from
          // happening
          ...
          if (!adjustPitch(&actualSampleReadRate,
                           paramFinalValues[PARAM_LOCAL_OSC_A_PITCH_ADJUST + s]))
            continue;
          // TODO: actualSampleReadRate should probably be affected by time
          // stretching, too. BUT that'd stuff up some existing users' songs -
          ...
          if (actualSampleReadRate != 16777216) {
            releaseStageLengthBytes =
                ((int64_t)releaseStageLengthBytes * actualSampleReadRate) >> 24;
          }

          // If this sample says it's not time to do auto-release yet, then we
          // don't want to do it yet, so get out
          if (bytesLeft >= releaseStageLengthBytes) goto skipAutoRelease;

          // And also see how many audio samples were left for this source. Only
          // do this in here because it involves time-consuming division
      ...
      // If we're still here, then *all* sources which needed attention say yes
      // do it now, so do it Just do this for the amplitude envelope
...
skipAutoRelease : {}
  ...
  // Apply envelope 0 to volume. This takes effect as a cut only; when the
  // envelope is at max height, volume is unaffected. Important that we use
  // lshiftAndSaturate here - otherwise, number can overflow if combining high
  // velocity patching with big LFO
  int32_t overallOscAmplitude = lshiftAndSaturate(
      multiply_32x32_rshift32(paramFinalValues[PARAM_LOCAL_VOLUME],
                              (sourceValues[PATCH_SOURCE_ENVELOPE_0] >> 1) + 1073741824),
      2);

  int32_t filterGain; // This is the gain which gets applied to compensate for
                      // any change in gain that the filter is going to cause

  // Prepare the filters
  if (sound->hasFilters()) {

    filterGain = filterSetConfig->init(
        paramFinalValues[PARAM_LOCAL_LPF_FREQ],
        paramFinalValues[PARAM_LOCAL_LPF_RESONANCE],
        paramFinalValues[PARAM_LOCAL_HPF_FREQ],
        (paramFinalValues[PARAM_LOCAL_HPF_RESONANCE]), // >> storageManager.devVarA) <<
                                                       // storageManager.devVarA,
        sound->lpfMode,
        sound->volumeNeutralValueForUnison << 1); // Level adjustment for unison now happens
                                                  // *before* the filter!
  }
  ...
  if (synthMode != SYNTH_MODE_RINGMOD) {
    ...
    // PARAM_LOCAL_OSC_x_VOLUME can normally only be up to a quarter of full
    // range, but patching can make it up to full-range overallOscAmplitude
    // (same range as) PARAM_LOCAL_VOLUME, is the same.

    // Let's impose a new limit, that only a total of 4x amplification via
    // patching is possible (not 16x). Chances are, the user won't even need
    // that much, let alone would have the osc volume *and* the synth master
    // volume on full We then have space to make each osc's amplitude 4x what it
    // could have been otherwise

    // If FM, we work the overall amplitude into each oscillator's, to avoid
    // having to do an extra multiplication for every audio sample at the end
    if (synthMode == SYNTH_MODE_FM) {

      // Apply compensation for unison
      overallOscAmplitude =
          multiply_32x32_rshift32_rounded(overallOscAmplitude, sound->volumeNeutralValueForUnison)
          << 3;

      int32_t a =
          multiply_32x32_rshift32(paramFinalValues[PARAM_LOCAL_OSC_A_VOLUME], overallOscAmplitude);
      int32_t b =
          multiply_32x32_rshift32(paramFinalValues[PARAM_LOCAL_OSC_B_VOLUME], overallOscAmplitude);

      // Clip off those amplitudes before they get too high. I think these were
      // originally intended to stop the amplitude rising to more than "4",
      // whatever that meant?
      sourceAmplitudes[0] = getMin(a, (int32_t)134217727);
      sourceAmplitudes[1] = getMin(b, (int32_t)134217727);
    }

    // Or if subtractive, we don't do that, because we do want to apply the
    // overall amplitude *after* the filter
    else {
      if (sound->hasFilters()) {
        sourceAmplitudes[0] =
    ...
    if (shouldAvoidIncrementing) {
      for (int s = 0; s < NUM_SOURCES; s++) { sourceAmplitudesLastTime[s] = sourceAmplitudes[s]; }
    }

    for (int s = 0; s < NUM_SOURCES; s++) {
      sourceAmplitudeIncrements[s] =
          (int32_t)(sourceAmplitudes[s] - sourceAmplitudesLastTime[s]) / numSamples;
    }
    ...
    if (synthMode == SYNTH_MODE_FM) {
      for (int m = 0; m < numModulators; m++) {
        modulatorsActive[m] = (paramFinalValues[PARAM_LOCAL_MODULATOR_0_VOLUME + m] != 0 ||
                               modulatorAmplitudeLastTime[m] != 0);

        if (modulatorsActive[m]) {
          modulatorAmplitudeIncrements[m] =
              (int32_t)(paramFinalValues[PARAM_LOCAL_MODULATOR_0_VOLUME + m] -
  ...
  if (synthMode != SYNTH_MODE_FM) {
    ...
    overallOscillatorAmplitudeIncrement =
        (int32_t)(overallOscAmplitude - overallOscAmplitudeLastTime) / numSamples;

    for (int s = 0; s < NUM_SOURCES; s++) {
      sourceWaveIndexIncrements[s] = (int32_t)(paramFinalValues[PARAM_LOCAL_OSC_A_WAVE_INDEX + s] -
                                               sourceWaveIndexesLastTime[s]) /
                                     numSamples;
    }
  }
  ...
  // Oscillator sync
  if (doingOscSync) {
    for (int u = 0; u < sound->numUnison; u++) { oscSyncPos[u] = unisonParts[u].sources[0].oscPos; }
  }

  // If various conditions are met, we can cut a corner by rendering directly
  // into the Sound's buffer
  ...
  if (sound->clippingAmount ||
      sound->synthMode == SYNTH_MODE_RINGMOD // We could make this one work - but currently the
                                             // ringmod rendering code doesn't really have
                                             // proper amplitude control - e.g. no increments -
                                             // built in, so we rely on the normal final
  ...
  // Otherwise, we need to think about whether we're rendering the same number
  // of channels as the Sound
  else {
...
decidedWhichBufferRenderingInto:
  ...
  for (int s = 0; s < NUM_SOURCES; s++) { sourceAmplitudesNow[s] = sourceAmplitudesLastTime[s]; }

  int32_t amplitudeL, amplitudeR;
  bool    doPanning;

  // If rendering directly into the Sound's buffer, set up for that.
  // Have to modify amplitudes to get the volume right - factoring the "overall"
  // amplitude, which will now not get used in its normal way, into the
  // oscillator/source amplitudes instead
  if (renderingDirectlyIntoSoundBuffer) {
    oscBuffer = soundBuffer;

    // Don't modify amplitudes if we're FM, because for that,
    // overallOscAmplitude has already been factored into the oscillator
    // (carrier) amplitudes
    if (synthMode == SYNTH_MODE_SUBTRACTIVE) {
      for (int s = 0; s < NUM_SOURCES; s++) {
        sourceAmplitudeIncrements[s] =
        ...
        sourceAmplitudesNow[s] =
            multiply_32x32_rshift32(sourceAmplitudesNow[s], overallOscAmplitudeLastTime) << 1;
      }
    }
  }

  // Or if rendering to local Voice buffer, We need to do some other setting up
  // - like wiping the buffer clean first
  else {
    oscBuffer = spareRenderingBuffer[0];

    int32_t const* const oscBufferEnd = oscBuffer + numSamples;

    // If any noise, do that. By cutting a corner here, we do it just once for
    // all "unison", rather than for each unison. Increasing number of unison
    // cuts the volume of the oscillators
    if (paramFinalValues[PARAM_LOCAL_NOISE_VOLUME] != 0 && synthMode != SYNTH_MODE_FM) {

      // This was >>2, but because I had a bug in V2.0.x which made noise too
      // loud if filter on, I'm now making this louder to compensate and remain
      // consistent by going just >>1. So now I really need to make it so that
      // sounds made before V2.0 halve their noise volume... (Hey, did I ever do
      // this? Who knows...)
      int32_t n = paramFinalValues[PARAM_LOCAL_NOISE_VOLUME] >> 1;
      if (sound->hasFilters()) { n = multiply_32x32_rshift32(n, filterGain) << 4; }
    ...
    // Even if first rendering into a local Voice buffer, we'll very often still
    // just do panning at the Sound level
  ...
  if (synthMode == SYNTH_MODE_SUBTRACTIVE) {
    ...
    // First, render any mono sources, and note whether there are any stereo
    // ones
    for (int s = 0; s < NUM_SOURCES; s++) {
      ...
      if ((s == 0) && doingOscSync) { getPhaseIncrements = oscSyncPhaseIncrement; }
      ...
      if (!sound->sources[s].renderInStereo((SampleHolder*)guides[s].audioFileHolder)) {
        renderBasicSource(sound,
                          ...
                          (s == 1) && doingOscSync,
                          oscSyncPos,
                          oscSyncPhaseIncrement,
                          sourceAmplitudeIncrements[s],
                          getPhaseIncrements,
                          getOutAfterGettingPhaseIncrements,
    ...
    if (sourcesToRenderInStereo) {
      ...
      if (!renderingDirectlyIntoSoundBuffer) {
        // If we've already got something mono in the buffer, copy that to the
      ...
      if (!renderingDirectlyIntoSoundBuffer) {
        ...
        // Filters
        filterSets[0]
            .renderLong(oscBuffer, oscBufferEnd, filterSetConfig, sound->lpfMode, numSamples, 2);
        filterSets[1].renderLong(oscBuffer + 1,
                                 oscBufferEnd,
                                 filterSetConfig,
                                 sound->lpfMode,
                                 numSamples,
        ...
        if (!sound->clippingAmount) {
          ...
          do {
            ...
            outputSampleL = multiply_32x32_rshift32_rounded(outputSampleL, overallOscAmplitudeNow)
                            << 1;
            outputSampleR = multiply_32x32_rshift32_rounded(outputSampleR, overallOscAmplitudeNow)
                            << 1;
    ...
    // If any unison part became inactive (for either source), and no
    // noise-source, then it might be time to unassign the voice...
    ...
    // If we rendered sources in stereo, and therefore have already output the
    // osc buffer, skip some stuff
    if (sourcesToRenderInStereo) goto renderingDone;
  }

  // Otherwise (FM and ringmod) we go through each unison first, and for each
  // one we render both sources together
  else {

    // For each unison part
    for (int u = 0; u < sound->numUnison; u++) {

      // Work out the phase increments of the two sources. If these are too
      // high, sourceAmplitudes[s] is set to 0. Yes this will affect all unison
      // parts, which seems like it's not what we want, but since we're
      // traversing the unison parts in ascending frequency, it's fine!
      ...
      if (synthMode == SYNTH_MODE_RINGMOD) {
        ...
        if (sound->hasFilters()) {
          amplitudeForRingMod = multiply_32x32_rshift32_rounded(amplitudeForRingMod, filterGain)
                                << 4;
        }
        ...
        cantBeDoingOscSyncForFirstOsc:
          // Work out pulse width, from parameter. This has no effect if we're
          // not actually using square waves, but just do it anyway, it's a
          // simple calculation
          int32_t pulseWidth =
          ...
          renderOsc(s,
                    oscType,
                    0,
                    spareRenderingBuffer[s + 1],
                    spareRenderingBuffer[s + 1] + numSamples,
                    numSamples,
                    phaseIncrements[s],
                    pulseWidth,
                    ...
                    false,
                    0,
                    doingOscSyncThisOscillator,
                    oscSyncPos[u],
                    phaseIncrements[0],
                    sound->oscRetriggerPhase[s],
                    sourceWaveIndexIncrements[s]);

          // Sine and triangle waves come out bigger in fixed-amplitude
          // rendering (for arbitrary reasons), so we need to compensate
      ...
      // Or if FM
        ...
        if (modulatorsActive[1]) {
          ...
          // If special case where mod1 is modulating mod0 but mod0 is inactive,
          // go away...
          if (sound->modulator1ToModulator0 && !modulatorsActive[0]) goto noModulatorsActive;

          // Render mod1
          renderSineWaveWithFeedback(spareRenderingBuffer[1],
                                     numSamples,
                                     &unisonParts[u].modulatorPhase[1],
                                     modulatorAmplitudeLastTime[1],
                                     phaseIncrementModulator[1],
                                     paramFinalValues[PARAM_LOCAL_MODULATOR_1_FEEDBACK],
                                     &unisonParts[u].modulatorFeedback[1],
                                     false,
          ...
          if (sound->modulator1ToModulator0) {
            ...
            renderFMWithFeedback(spareRenderingBuffer[1],
                                 numSamples,
                                 NULL,
                                 &unisonParts[u].modulatorPhase[0],
                                 modulatorAmplitudeLastTime[0],
                                 phaseIncrementModulator[0],
                                 paramFinalValues[PARAM_LOCAL_MODULATOR_0_FEEDBACK],
                                 &unisonParts[u].modulatorFeedback[0],
                                 modulatorAmplitudeIncrements[0]);
          }

          // Otherwise, so long as modulator0 is in fact active, render it
          ...
          noModulatorsActive:
            for (int s = 0; s < NUM_SOURCES; s++) {
              if (sourceAmplitudes[s]) {
                renderSineWaveWithFeedback(oscBuffer,
                                           numSamples,
                                           &unisonParts[u].sources[s].oscPos,
                                           sourceAmplitudesNow[s],
                                           phaseIncrements[s],
                                           paramFinalValues[PARAM_LOCAL_CARRIER_0_FEEDBACK + s],
                                           &unisonParts[u].sources[s].carrierFeedback,
                                           true,
                                           sourceAmplitudeIncrements[s]);
              }
            }
        ...
        for (int s = 0; s < NUM_SOURCES; s++) {
          if (sourceAmplitudes[s]) {
            renderFMWithFeedbackAdd(oscBuffer,
                                    numSamples,
                                    spareRenderingBuffer[1],
                                    &unisonParts[u].sources[s].oscPos,
                                    sourceAmplitudesNow[s],
                                    phaseIncrements[s],
                                    paramFinalValues[PARAM_LOCAL_CARRIER_0_FEEDBACK + s],
  ...
  // Output mono osc buffer. This is skipped (via goto statement above) if we
  // ended up with a stereo buffer.
  if (!renderingDirectlyIntoSoundBuffer) {
    /*
    do {
...
bool Voice::areAllUnisonPartsInactive(ModelStackWithVoice* modelStack) {
  ...
  if (!modelStack->paramManager->getPatchedParamSet()
           ->params[PARAM_LOCAL_NOISE_VOLUME]
           .containsSomething(-2147483648)) {
...
// Returns false if it takes us above 22.05kHz, in which case it doesn't return
// a valid value
...
int32_t doFMNew(uint32_t carrierPhase, uint32_t phaseShift) {
  // return getSineNew((((*carrierPhase += carrierPhaseIncrement) >> 8) +
  // phaseShift) & 16777215, 24);
...
#define fmVectorLoopComponent(i)                                                                 \
  {                                                                                              \
    uint32_t readOffsetNow = (vgetq_lane_u32(finalPhase, i) >> (32 - SINE_TABLE_SIZE_MAGNITUDE)) \
                             << 2;                                                               \
  ...
  int16x4_t strength2 = vreinterpret_s16_u16(
      vshr_n_u16(vshrn_n_u32(finalPhase, (32 - 16 - SINE_TABLE_SIZE_MAGNITUDE)), 1));
...
void Voice::renderSineWaveWithFeedback(int32_t*  bufferStart,
                                       int       numSamples,
                                       uint32_t* phase,
                                       int32_t   amplitude,
                                       uint32_t  phaseIncrement,
                                       int32_t   feedbackAmount,
                                       int32_t*  lastFeedbackValue,
                                       bool      add,
  ...
  *phase += phaseIncrement * numSamples;
  ...
  if (feedbackAmount) {
    ...
    do {
      ...
      // We do hard clipping of the feedback amount. Doing tanH causes aliasing
      // - even if we used the anti-aliased version. The hard clipping one
      // sounds really solid.
      ...
      if (add)
        *thisSample =
            multiply_accumulate_32x32_rshift32_rounded(*thisSample, feedbackValue, amplitudeNow);
      else
...
void Voice::renderFMWithFeedback(int32_t*  bufferStart,
                                 int       numSamples,
                                 int32_t*  fmBuffer,
                                 uint32_t* phase,
                                 int32_t   amplitude,
...
void Voice::renderFMWithFeedbackAdd(int32_t*  bufferStart,
                                    int       numSamples,
                                    int32_t*  fmBuffer,
                                    uint32_t* phase,
                                    int32_t   amplitude,
...
// This function renders all unison for a source/oscillator. Amplitude and the
// incrementing thereof is done independently for each unison, despite being the
// same for all of them, and you might be wondering why this is. Yes in the case
// of an 8-unison sound it'd work out slightly better to apply amplitude to all
// unison together, but here's why this just generally isn't all that
// advantageous:
//  -   Doing it separately for each unison, as we are, means each unison of
//  *both* sources can just sum itself directly onto the buffer for the whole
//  Voice, or in special cases the Sound,
//      which only had to be wiped clean once. But applying amplitude to the
//      combined unisons of one source would mean we'd need to clear a buffer
//      for that source, then render our potentially just one unison into that
//      (summing to the buffer's existing contents), then as amplitude is
//      applied to the buffer, we'd write the output of that into the Voice
//      buffer (summing to its contents too). So basically an extra level of
//      summing and clearing would have to happen, in addition to the extra
//      copying which is obvious.
//  -   And you might be thinking that for oscillator sync this would work well,
//  because it already applies amplitude in a separate step after rendering the
//  wave.
//      But no - a key part of why this works well as it is is that the summing
//      between unisons only happens along with that amplitude-application,
//      after the initial wave render has finished chopping around the contents
//      of its buffer. Having that summed into an all-unison buffer would still
//      require an additional copying(summing) step, and we might as well just
//      apply amplitude while that's happening, which is exactly how it is
//      currently.

void Voice::renderBasicSource(Sound*                   sound,
                              ParamManagerForTimeline* paramManager,
                              int                      s,
                              int32_t* __restrict__ oscBuffer,
                              int     numSamples,
                              int32_t sourceAmplitude,
                              bool* __restrict__ unisonPartBecameInactive,
                              int32_t overallPitchAdjust,
                              bool    doOscSync,
                              uint32_t* __restrict__ oscSyncPos,
                              uint32_t* __restrict__ oscSyncPhaseIncrements,
                              int32_t amplitudeIncrement,
                              uint32_t* __restrict__ getPhaseIncrements,
                              bool    getOutAfterPhaseIncrements,
  ...
  for (int u = 0; u < sound->numUnison; u++) {
    ...
    pitchTooHigh:
      if (getPhaseIncrements) { getPhaseIncrements[u] = 0; }
      continue;
    }
    ...
    if (sound->sources[s].oscType == OSC_TYPE_SAMPLE) {
      ...
      bool stillOk = voiceUnisonPartSource->getPitchAndSpeedParams(&sound->sources[s],
                                                                   &guides[s],
                                                                   &phaseIncrement,
                                                                   &timeStretchRatio,
                                                                   &noteLengthInSamples);
      ...
      if (tryToStartMidNote) {
        ...
        // Or, normal - it needs a bit more explanation.
          ...
          // We have to ignore any pitch modulation, aka the "patched" value for
          // phaseIncrement: we have to use phaseIncrementStoredValue instead,
          // which is the pre-modulation/patching value. But we also have to
          // forget about the timeStretchRatio we calculated above with the call
          // to getPitchAndSpeedParams(), and instead calculate a special
          // version of this with this call to getSpeedParamForNoSyncing(),
          // which is what getPitchAndSpeedParams() itself calls when not in
          // STRETCH mode, which we've already determined we're not in, and
          // crucially pass it the not-patched
          // voiceUnisonPartSource->phaseIncrementStoredValue. This fix was done
          // in September 2020 after bug report from Clyde.
          uint32_t timeStretchRatioWithoutModulation =
              voiceUnisonPartSource->getSpeedParamForNoSyncing(
                  &sound->sources[s],
                  voiceUnisonPartSource->phaseIncrementStoredValue,
                  ((SampleHolder*)guides[s].audioFileHolder)->neutralPhaseIncrement);

          // Cool, so now we've got phaseIncrement and timeStretchRatio
          // equivalent values which will indicate our correct play position
          // into the sample regardless of pitch modulation (almost always
          // vibrato).
          rawSamplesLate = ((((uint64_t)voiceSample->pendingSamplesLate *
                              voiceUnisonPartSource->phaseIncrementStoredValue) >>
                             24) *
      ...
      if (phaseIncrement != 16777216) {
        ...
        // And if first render, and other conditions met, see if we can use
        // cache. It may seem like it'd be a good idea to try and set this up on
        // note-on, rather than here in the rendering routine, but I tried that
        // and the fact is that it means a bunch of extra computation has to
        // happen to work out pitch and timestretch there as well as here (where
        // it'll be worked out anyway), including checking the result of
        // patching / modulation (and we *do* allow caching where, say, velocity
        // or note is affecting pitch), and stretch-syncing.
        if (!voiceSample->doneFirstRenderYet && !tryToStartMidNote &&
            portaEnvelopePos == 0xFFFFFFFF) { // No porta

          // If looping, make sure the loop isn't too short. If so, caching just
          // wouldn't sound good / accurate
          if (loopingType) {
          ...
          for (int c = 0; c < paramManager->getPatchCableSet()->numUsablePatchCables; c++) {
            ...
            if (cable->destinationParamDescriptor.isSetToParamWithNoSource(
                    PARAM_LOCAL_PITCH_ADJUST) ||
                cable->destinationParamDescriptor.isSetToParamWithNoSource(
                    PARAM_LOCAL_OSC_A_PITCH_ADJUST + s)) {

              // And if it's an envelope or LFO or random...
              if (cable->from == PATCH_SOURCE_ENVELOPE_0 ||
                  cable->from == PATCH_SOURCE_ENVELOPE_1 ||
                ...
                if (sourceValues[PATCH_SOURCE_AFTERTOUCH]) { goto dontUseCache; }
              }

              // TODO: probably need to check for X and Y modulation sources
              // here too...

              else if (cable->from == PATCH_SOURCE_COMPRESSOR) {
                if (sound->globalSourceValues[PATCH_SOURCE_COMPRESSOR]) { goto dontUseCache; }
              }
            }
          }

          {
            ...
            bool everythingOk = voiceSample->possiblySetUpCache(&sound->sources[s].sampleControls,
                                                                &guides[s],
                                                                phaseIncrement,
                                                                timeStretchRatio,
                                                                getPriorityRating(),
      ...
      // We no longer do caching when there's just time stretching with no pitch
      // adjustment, because the time stretching algorithm is so efficient,
      // playing back the cache is hardly any faster than just doing the time
      // stretching (once perc info has been cached) - and, crucially, creating
      // / writing to the cache in the first place is quite inefficient when
      // time stretching, because when we're not writing to the cache, that
      // allows us to do a special optimization not otherwise available (that
      // is, combining the amplitude increments for the hop crossfades with the
      // overall voice ones, and having multiple crossfading hops write directly
      // to the osc buffer).

      bool stillActive = voiceSample->render(&guides[s],
                                             oscBuffer,
                                             numSamples,
                                             sample,
                                             numChannels,
                                             ...
                                             amplitudeIncrement,
                                             interpolationBufferSize,
                                             sound->sources[s].sampleControls.interpolationMode,
                                             getPriorityRating());
    ...
    // Or echoing input
      ...
      if (phaseIncrement != 16777216) {
        ...
        if (!source->livePitchShifter) {
          ...
          LiveInputBuffer* liveInputBuffer =
              AudioEngine::getOrCreateLiveInputBuffer(inputTypeNow, true);

          if (liveInputBuffer) {

            void* memory =
                generalMemoryAllocator.alloc(sizeof(LivePitchShifter), NULL, false, true);

            if (memory) {
              source->livePitchShifter =
                  new (memory) LivePitchShifter(inputTypeNow, phaseIncrement);
      ...
      if (source->livePitchShifter) {
        ...
        source->livePitchShifter->render(oscBuffer,
                                         numSamples,
                                         phaseIncrement,
                                         sourceAmplitude,
                                         amplitudeIncrement,
      ...
      // No pitch shifting
        ...
        // Just left, or just right, or if (stereo but there's only the
        // internal, mono mic)
        ...
        // Stereo
          ...
          do {
            ...
            if (!AudioEngine::renderInStereo) { sampleL = ((sampleL >> 1) + (sampleR >> 1)); }
    ...
    // Or regular wave
      ...
      uint32_t oscRetriggerPhase = sound->oscRetriggerPhase[s]; // Yes we might need this even if
                                                                // not doing osc sync.
      ...
      int32_t* oscBufferEnd = oscBuffer + numSamples; // TODO: we don't really want to be
                                                      // calculating this so early do we?
      ...
      renderOsc(s,
                ...
                amplitudeIncrement,
                doOscSync,
                oscSyncPosThisUnison,
                oscSyncPhaseIncrementsThisUnison,
                oscRetriggerPhase,
                waveIndexIncrement);
    }
  }
...
/*
void renderCrudeSawWaveWithAmplitude(int32_t* __restrict__ thisSample, int32_t
const* bufferEnd, uint32_t phaseNowNow, uint32_t phaseIncrementNow, int32_t
amplitude, int32_t amplitudeIncrement, int numSamples) {
    ...
    while (true) {
        int32x4_t valueVector = vqdmulhq_s32(amplitudeVector,
vreinterpretq_s32_u32(phaseVector)); valueVector = vaddq_s32(valueVector,
existingDataInBuffer); vst1q_s32(thisSample, valueVector); if (thisSample >=
bufferEnd) break; thisSample += 4; existingDataInBuffer = vld1q_s32(thisSample);
        phaseVector = vaddq_u32(phaseVector, phaseIncrementVector);
...
uint32_t renderCrudeSawWaveWithAmplitude(int32_t* thisSample,
  ...
  while (thisSample != remainderSamplesEnd) {
    ...
    amplitudeNow += amplitudeIncrement;
    *thisSample =
        multiply_accumulate_32x32_rshift32_rounded(*thisSample, (int32_t)phaseNowNow, amplitudeNow);
    ++thisSample;
  }
...
void renderPDWave(const int16_t* table,
  ...
  do {
    ...
    *thisPhase += phaseIncrementEachHalf[whichHalfBefore];
    ...
    int32_t value = waveValueFunction(eachTable[whichHalfAfter],
                                      eachTableSize[whichHalfAfter],
                                      *thisPhase,
                                      pulseWidth,
                                      phaseToAdd);
...
const int16_t* sawTables[]          = {NULL,      NULL,       NULL,       NULL,       NULL,
                                       NULL,      sawWave215, sawWave153, sawWave109, sawWave76,
                                       sawWave53, sawWave39,  sawWave27,  sawWave19,  sawWave13,
                                       sawWave9,  sawWave7,   sawWave5,   sawWave3,   sawWave1};
const int16_t* squareTables[]       = {NULL,          NULL,         NULL,          NULL,
                                       NULL,          NULL,         squareWave215, squareWave153,
                                       squareWave109, squareWave76, squareWave53,  squareWave39,
                                       squareWave27,  squareWave19, squareWave13,  squareWave9,
                                       squareWave7,   squareWave5,  squareWave3,   squareWave1};
...
// The lower 8 are from (mystery synth A) - higher than that, it's (mystery
// synth B)
...
__attribute__((optimize("unroll-loops"))) void Voice::renderOsc(int       s,
                                                                int       type,
                                                                int32_t   amplitude,
                                                                int32_t*  bufferStart,
                                                                int32_t*  bufferEnd,
  ...
  // We save a decent bit of processing power by grabbing a local copy of the
  // phase to work with, and just incrementing the startPhase once
  ...
  *startPhase += phaseIncrement * numSamples;
  ...
  // For cases other than sines and triangles, we use these standard table
  // lookup size thingies. We need to work this out now so we can decide whether
  // to switch the analog saw to the digital one
  ...
  if (type == OSC_TYPE_SINE) {
    ...
    // PW for the perfect mathematical/digital square - we'll do it by
    ...
    if (type == OSC_TYPE_SQUARE) {
      ...
      if (doPulseWave) {
        phaseIncrementForCalculations =
            phaseIncrement * 0.6; // Mildly band limit the square waves before
                                  // they get ringmodded to create the pulse
                                  // wave. *0.5 would be no band limiting
      }
    }

    getTableNumber(phaseIncrementForCalculations, &tableNumber, &tableSizeMagnitude);
    // TODO: that should really take into account the phaseIncrement (pitch)
    // after it's potentially been altered for non-square PW below.

    if (type == OSC_TYPE_ANALOG_SAW_2) {
      // Analog saw tables 8 and above are quite saw-shaped and sound relatively
      // similar to the digital saw. So for these, if the CPU load is getting
      // dire, we can do the crude, aliasing digital saw.
      ...
      if (type == OSC_TYPE_ANALOG_SQUARE) {
        ...
        if ((uint32_t)(resetterPhase) >= (uint32_t) - (resetterPhaseIncrement >> 1)) {
          resetterPhaseToMultiply -=
              ((uint32_t)1 << 31); // Count the last little bit of the cycle as actually a
                                   // negative-number bit of the next one.
        }
        ...
        phase = (uint32_t)multiply_32x32_rshift32_rounded((pulseWidthAbsolute >> 1) + 1073741824,
                ...
                << 3;
        phaseIncrement =
            ((uint32_t)multiply_32x32_rshift32_rounded((pulseWidthAbsolute >> 1) + 1073741824,
  ...
  doOscSyncSetup:
    ...
    resetterDivideByPhaseIncrement = // You should >> 47 if multiplying by this.
        (uint32_t)2147483648u / (uint16_t)((resetterPhaseIncrement + 65535) >>
                                           16); // Round resetterPhaseIncrement up first, so
                                                // resetterDivideByPhaseIncrement gets a tiny bit
                                                // smaller, so things multiplied by it don't get a
                                                // bit too big and overflow.
  }
...
skipPastOscSyncStuff:
  ...
  doSine:
    ...
    phase = waveTable->render(wavetableRenderingBuffer,
                              numSamples,
                              phaseIncrement,
                              phase,
                              doOscSync,
      ...
      if (doOscSync) {
        ...
        do {
          ...
          if (applyAmplitude) {
            amplitudeNow += amplitudeIncrement;
            *thisSample =
                multiply_accumulate_32x32_rshift32_rounded(*thisSample, value, amplitudeNow);
          } else
            *thisSample = value << 1;
        } while (++thisSample != bufferEnd);
    ...
    if (type == OSC_TYPE_SAW) {
    doSaw:
      // If frequency low enough, we just use a crude calculation for the wave
      // without anti-aliasing
      if (tableNumber < AudioEngine::cpuDireness + 6) {

        if (!doOscSync) {
          if (applyAmplitude) {
            renderCrudeSawWaveWithAmplitude(bufferStart,
                                            bufferEnd,
                                            phase,
                                            phaseIncrement,
                                            amplitude,
            ...
            renderCrudeSawWaveWithoutAmplitude(bufferStart,
                                               bufferEnd,
                                               phase,
                                               phaseIncrement,
                                               numSamples);
          ...
          do {
            ...
            // Do the reset
            if (resetterPhaseNow < resetterPhaseIncrement) {
              phaseNow = (multiply_32x32_rshift32(
                              multiply_32x32_rshift32(resetterPhaseNow, phaseIncrement),
                              resetterDivideByPhaseIncrement)
                          << 17) +
                         1 + retriggerPhase;
            }
            ...
            if (applyAmplitude) {
              ...
              *thisSample = multiply_accumulate_32x32_rshift32_rounded(
                  *thisSample,
                  (int32_t)phaseNow,
                  amplitudeNow); // Using multiply_accumulate saves like 10%
                                 // here!
            } else
      ...
      if (tableNumber < AudioEngine::cpuDireness + 6) {
        ...
        if (!doOscSync) {
          ...
          if (applyAmplitude) {
            ...
            do {
              ...
              *thisSample = multiply_accumulate_32x32_rshift32_rounded(
                  *thisSample,
                  getSquare(phaseNow, pulseWidth),
                  amplitudeNow); // Using multiply_accumulate saves like 20%
                                 // here, WTF!!!!
              ++thisSample;
            } while (thisSample != bufferEnd);
          }
          ...
          do {
            ...
            if (applyAmplitude) {
              amplitudeNow += amplitudeIncrement;
              *thisSample =
                  multiply_accumulate_32x32_rshift32_rounded(*thisSample,
                                                             getSquare(phaseNow, pulseWidth),
                                                             amplitudeNow);
            } else
              *thisSample = getSquareSmall(phaseNow, pulseWidth);
        ...
        if (doPulseWave) {
          ...
          if (doOscSync) {
            ...
            RENDER_OSC_SYNC(STORE_VECTOR_WAVE_FOR_ONE_SYNC,
    ...
    doAnalogSquare:
      // This sounds different enough to the digital square that we can never
      // just swap back to that to save CPU
      table = analogSquareTables[tableNumber];
    }

    // If we're still here, we need to render the wave according to a table
    // decided above
  ...
  // We never get here. Only by jumping to labels below, which will usually only
  // be if osc sync on. Or wavetable.
...
storePhase:
  if (!(doPulseWave && type != OSC_TYPE_SQUARE)) { *startPhase = phase; }
}
...
bool Voice::doFastRelease(uint32_t releaseIncrement) {
  ...
  // Or if first render not done yet, we actually don't want to hear anything at
  // all, so just unassign it
  else {
    return false;
  }
}
...
// Higher numbers are lower priority. 1 is top priority. Will never return 0,
// because nextVoiceState starts at 1
uint32_t Voice::getPriorityRating() {
  return ((uint32_t)(3 - assignedToSound->voicePriority)
          << 30) // Bits 30-31 - manual priority setting
         + ((uint32_t)getMin(assignedToSound->numVoicesAssigned, 7)
            << 27) // Bits 27-29 - how many voices that Sound has
                   // - that one really does need to go above state, otherwise
                   // "once" samples can still cut out synth drones. In a
                   // perfect world, culling for the purpose of "soliciting" a
                   // Voice would also count the new Voice being solicited,
                   // preferring to cut out that same Sound's old, say, one
                   // Voice, than another Sound's only Voice
         + ((uint32_t)envelopes[0].state << 24) // Bits 24-26 - envelope state
         + ((uint32_t)(-envelopes[0].timeEnteredState) &
            (0xFFFFFFFF >> 8)); // Bits  0-23 - time entered
}
