...
InstrumentClipView::InstrumentClipView() {
  ...
  for (int i = 0; i < editPadPressBufferSize; i++) { editPadPresses[i].isActive = false; }
...
void InstrumentClipView::openedInBackground() {
  ...
  if (renderingToStore) {
    renderMainPads(0xFFFFFFFF,
                   &PadLEDs::imageStore[displayHeight],
                   &PadLEDs::occupancyMaskStore[displayHeight],
                   true);
    renderSidebar(0xFFFFFFFF,
                  &PadLEDs::imageStore[displayHeight],
                  &PadLEDs::occupancyMaskStore[displayHeight]);
  } else {
...
int InstrumentClipView::buttonAction(int x, int y, bool on, bool inCardRoutine) {
  ...
  if (x == scaleModeButtonX && y == scaleModeButtonY) {
    ...
    if (currentSong->currentClip->output->type == INSTRUMENT_TYPE_KIT) {
      if (on) { IndicatorLEDs::indicateAlertOnLed(kitLedX, kitLedY); }
    ...
    if (on) {
      ...
      if (currentUIMode == UI_MODE_NONE || currentUIMode == UI_MODE_SCALE_MODE_BUTTON_PRESSED) {
        ...
        // If user holding shift and we're already in scale mode, cycle through
        // available scales
        ...
        // Or, no shift button - normal behaviour
          ...
          if (!getCurrentClip()->inScaleMode) {
            calculateDefaultRootNote(); // Calculate it now so we can show the
                                        // user even before they've released the
                                        // button
      ...
      // If user is auditioning just one NoteRow, we can go directly into Scale
      // Mode and set that root note
  ...
  // Record button if holding audition pad
    ...
    if (on && currentSong->currentClip->output->type == INSTRUMENT_TYPE_KIT &&
      ...
      if (currentUIMode == UI_MODE_ADDING_DRUM_NOTEROW) {
        ...
        if (noteRow->drum) { noteRow->drum->drumWontBeRenderedForAWhile(); }
        cancelAllAuditioning();
  ...
  // Load / Kit button if creating new NoteRow for Drum
    ...
    if (on) {
      ...
      ModelStackWithNoteRow* modelStackWithNoteRow =
          modelStack->addNoteRow(noteRowIndex, newNoteRow);
  ...
  // Load / kit button if auditioning
    ...
    if (on) {
      ...
      if (currentSong->currentClip->output->type == INSTRUMENT_TYPE_KIT) {
        cutAuditionedNotesToOne();
        int      noteRowIndex;
        NoteRow* noteRow = getCurrentClip()->getNoteRowOnScreen(lastAuditionedYDisplay,
                                                                currentSong,
                                                                &noteRowIndex);
        cancelAllAuditioning();
        if (noteRow->drum) { noteRow->drum->drumWontBeRenderedForAWhile(); }
      ...
      if (currentSong->currentClip->output->type == INSTRUMENT_TYPE_SYNTH) {
        ...
        bool success = soundEditor.setup(getCurrentClip(),
                                         &fileSelectorMenu,
                                         0); // Can't fail because we just set the selected Drum
        if (success) { openUI(&soundEditor); }
      }
    }
  }

  // Kit button. Unlike the other instrument-type buttons, whose code is in
  // InstrumentClipMinder, this one is only allowed in the InstrumentClipView
    ...
    if (on && numEditPadPresses == 1 &&
      ...
      for (i = 0; i < editPadPressBufferSize; i++) {
        if (editPadPresses[i].isActive) {
          ...
          // Note: I should fix this - if deleting a NoteRow of a MIDI drum that
          // we're auditioning via MIDI, this will leave a stuck note...
          ...
          // Can't remember why repopulateNoteRowsOnScreen() doesn't do the
  ...
  // Horizontal encoder button if learn button pressed. Make sure you let the
  // "off" action slide past to the Editor
  else if (x == xEncButtonX && y == xEncButtonY && on &&
           Buttons::isButtonPressed(learnButtonX, learnButtonY)) {
    if (inCardRoutine) return ACTION_RESULT_REMIND_ME_OUTSIDE_CARD_ROUTINE;
  ...
  // Horizontal encoder button
    ...
    if (on && Buttons::isShiftButtonPressed()) {
      ...
      enterUIMode(UI_MODE_HOLDING_HORIZONTAL_ENCODER_BUTTON); // Whether or not we did
                                                              // the "multiply" action
                                                              // above, we need to be in
                                                              // this UI mode, e.g. for
                                                              // rotating individual
                                                              // NoteRow
    }
  ...
  // Vertical encoder button
    ...
    if (isUIModeActiveExclusively(UI_MODE_AUDITIONING)) {
      if (on) {
        ...
        editNumEuclideanEvents(modelStackWithNoteRow,
                               0,
...
void InstrumentClipView::createDrumForAuditionedNoteRow(int drumType) {
  ...
  if (currentUIMode == UI_MODE_ADDING_DRUM_NOTEROW) {
    ...
    if (noteRow->drum) {
      if (drumType != DRUM_TYPE_SOUND && noteRow->drum->type == drumType)
        return; // If it's already that kind of Drum, well, no need to do it
                // again
  ...
  if (drumType == DRUM_TYPE_SOUND) {
    ...
    ((SoundDrum*)newDrum)
        ->modKnobs[6][0]
        .paramDescriptor.setToHaveParamOnly(PARAM_LOCAL_PITCH_ADJUST);
#endif
  }
...
void InstrumentClipView::copyAutomation(int whichModEncoder) {
  ...
  if (modelStack && modelStack->autoParam) {
    ...
    bool isPatchCable = (modelStack->paramCollection ==
                         modelStack->paramManager
                             ->getPatchCableSetAllowJibberish()); // Ok this is cursed, but will
                                                                  // work fine so long as the
                                                                  // possibly invalid memory
                                                                  // here doesn't accidentally
                                                                  // equal
      ...
      if (numNotes > 0) {
        ...
        *prevPointer = newCopiedNoteRow;
        ...
        newCopiedNoteRow->notes =
            (Note*)generalMemoryAllocator.alloc(sizeof(Note) * numNotes, NULL, true);
        ...
        if (currentSong->currentClip->output->type == INSTRUMENT_TYPE_KIT) { // yDisplay for Kits
          ...
          int yVisual =
              currentSong->getYVisualFromYNote(thisNoteRow->y, getCurrentClip()->inScaleMode);
...
void InstrumentClipView::pasteAutomation(int whichModEncoder) {
  ...
  if (action) { action->recordParamChangeIfNotAlreadySnapshotted(modelStackWithAutoParam, false); }
  ...
  modelStackWithAutoParam->autoParam->paste(startPos,
                                            endPos,
                                            scaleFactor,
...
void InstrumentClipView::pasteNotes() {
  ...
  if (currentSong->currentClip->output->type == INSTRUMENT_TYPE_KIT) {
    ...
    for (CopiedNoteRow* thisCopiedNoteRow = firstCopiedNoteRow; thisCopiedNoteRow;
      ...
      bool success =
          thisNoteRow->paste(modelStackWithNoteRow, thisCopiedNoteRow, scaleFactor, endPos, action);
  ...
  // Non-kit
    ...
    // If neither the source nor the destination was a kit Clip, and one had a
    // scale and the other didn't, we want to preserve some scale information
    // which we otherwise wouldn't
    bool shouldPreserveScale =
        (copiedScaleType != SCALE_TYPE_KIT && copiedScaleType != pastedScaleType);
...
void InstrumentClipView::doubleClipLengthAction() {
  ...
  // Add the ConsequenceClipMultiply to the Action. This must happen before
  // calling doubleClipLength(), which may add note changes and deletions,
  // because when redoing, those have to happen after (and they'll have no
  // effect at all, but who cares)
  if (action) {
    void* consMemory = generalMemoryAllocator.alloc(sizeof(ConsequenceInstrumentClipMultiply));

    if (consMemory) {
      ConsequenceInstrumentClipMultiply* newConsequence =
          new (consMemory) ConsequenceInstrumentClipMultiply();
...
void InstrumentClipView::selectEncoderAction(int8_t offset) {
  ...
  // Or set / create a new Drum
    ...
    if (Buttons::isButtonPressed(selectEncButtonX, selectEncButtonY)) {
      ...
      // setSelectedDrum(drumForNewNoteRow); // Can't - it doesn't have a
      // NoteRow, and so we don't really know where its ParamManager is!
...
const uint32_t editPadActionUIModes[] = {UI_MODE_NOTES_PRESSED,
...
int InstrumentClipView::padAction(int x, int y, int velocity) {
  ...
  if (x < displayWidth) {
    ...
    // Perhaps the user wants to enter the SoundEditor via a shortcut. They can
    // do this by holding an audition pad too - but this gets deactivated if
    // they've done any "euclidean" or per-NoteRow editing already by holding
    // down that audition pad, because if they've done that, they're probably
    // not intending to deliberately go into the SoundEditor, but might be
    // trying to edit notes. Which they currently can't do...
    if (velocity &&
  ...
  // If mute pad action
    ...
    } else if (currentSong->currentClip->output->type == INSTRUMENT_TYPE_KIT &&
               lastAuditionedYDisplay == y && isUIModeActive(UI_MODE_AUDITIONING) &&
               getNumNoteRowsAuditioning() == 1) {
      if (velocity) {
        if (isUIModeActiveExclusively(UI_MODE_AUDITIONING)) {
    ...
    maybeRegularMutePress:
      if (isUIModeWithinRange(mutePadActionUIModes) && velocity) { mutePadPress(y); }
    }
  }
  ...
  possiblyAuditionPad:
    if (x == displayWidth + 1) {
      ...
      if (isUIModeActiveExclusively(UI_MODE_MIDI_LEARN)) {
        if (getCurrentUI() == this) {
          ...
          if (currentSong->currentClip->output->type == INSTRUMENT_TYPE_KIT) {
            ...
            view.drumMidiLearnPadPressed(velocity,
                                         thisNoteRow->drum,
                                         (Kit*)currentSong->currentClip->output);
          } else {
            view.melodicInstrumentMidiLearnPadPressed(
                velocity,
      ...
      // Changing the scale:
        ...
        if (velocity && currentSong->currentClip->output->type !=
                            INSTRUMENT_TYPE_KIT) { // We probably couldn't have got this far
                                                   // if it was a Kit, but let's just check
          if (getCurrentClip()->inScaleMode) {
            currentUIMode = UI_MODE_NONE; // So that the upcoming render of the
                                          // sidebar comes out correctly
            changeRootNote(y);
      ...
      // Actual basic audition pad press:
        ...
        if (sdRoutineLock && !allowSomeUserActionsEvenWhenInCardRoutine)
          return ACTION_RESULT_REMIND_ME_OUTSIDE_CARD_ROUTINE; // Allowable
                                                               // sometimes if
                                                               // in card
                                                               // routine.
...
void InstrumentClipView::editPadAction(bool         state,
  ...
  if (state) {
    ...
    if (!isSquareDefined(xDisplay)) { return; }
    ...
    if (!modelStackWithNoteRow->getNoteRowAllowNull()) {
      ...
      // If that just created a new NoteRow for a Kit, then we can't undo any
      // further back than this
      if (instrument->type == INSTRUMENT_TYPE_KIT) { actionLogger.deleteAllLogs(); }
    }

    int32_t effectiveLength = modelStackWithNoteRow->getLoopLength();

    // Now that we've definitely got a NoteRow, check against NoteRow
    // "effective" length here (though it'll very possibly be the same as the
    // Clip length we may have tested against above).
    ...
    if (numEditPadPressesPerNoteRowOnScreen[yDisplay] == 1 &&
      ...
      if (i < editPadPressBufferSize) {
        ...
        // First, figure out the lengh to take the note up to the start of the
        // pressed square. Put it in newLength
        ...
        if (newLength == 0)
          newLength = squareWidth; // Protection - otherwise we could end up
                                   // with a 0-length note!
        ...
        noteRow->clearArea(areaStart,
                           areaWidth,
                           modelStackWithNoteRow,
                           action,
                           clip->getWrapEditLevel(),
        ...
        if (instrument->type == INSTRUMENT_TYPE_KIT) { setSelectedDrum(noteRow->drum); }
      }
    }
      ...
      if (i < editPadPressBufferSize) {
        ...
        if (sound) {
          ...
          if (sampleLength) {
            ...
            // Previously I was having it always jump to a "square" number, but
            // as James Meharry pointed out, what if the Clip is deliberately a
            // non-square length?
            ...
            // If desired note length too long and no existing notes, extend the
            // Clip (or if the NoteRow has independent length, do that instead).
            if (noteRow->hasNoNotes() && !clip->wrapEditing &&
                desiredNoteLength > effectiveLength) {
              squareStart = 0;
              if (noteRow->loopLengthIfIndependent) {
                ...
                // Clip length changing may visually change other rows too, so
                // must re-render them all
          ...
          // Or if general cut-mode samples - but only for kit Clips, not synth
            ...
            if (sampleLength) {
              ...
              // If sample loops, we want to cut out before we get to the
              // loop-point
              if (anyLooping) {
                desiredNoteLength =
                    ((uint64_t)sampleLength << 32) / currentSong->timePerTimerTickBig;
              ...
              // Or if sample doesn't loop, we want to extend just past the end
              // point
              else {
                desiredNoteLength =
        ...
        // Otherwise, we've selected a note
          ...
          if (squareType == SQUARE_NEW_NOTE) {
            ...
            // If we're cross-screen-editing, create other corresponding notes
            // too
            if (clip->wrapEditing) {
              int error =
                  noteRow->addCorrespondingNotes(squareStart,
                                                 desiredNoteLength,
          ...
          // Can only set the mod region after setting the selected drum!
          // Otherwise the params' currentValues don't end up right
          view.setModRegion(
              firstNote->pos,
              getMax((uint32_t)distanceToNextNote + lastNote->pos - firstNote->pos, squareWidth),
              modelStackWithNoteRow->noteRowId);

          // Now that we're holding a note down, get set up for if the user
          // wants to edit its MPE values.
  ...
  // Or if pad press ended...
    ...
    if (i < editPadPressBufferSize) {
      ...
      numericDriver.cancelPopup(); // Crude way of getting rid of the
                                   // probability-editing permanent popup

      uint8_t velocity = editPadPresses[i].intendedVelocity;

      // Must mark it as inactive first, otherwise, the note-deletion code may
      // do so and then we'd do it again here
      endEditPadPress(i);
      ...
      // Close last note nudge action, if there was one - so each such action is
      // for one consistent set of notes
...
Sound* InstrumentClipView::getSoundForNoteRow(NoteRow*                  noteRow,
  ...
  } else if (currentSong->currentClip->output->type == INSTRUMENT_TYPE_KIT && noteRow &&
...
void InstrumentClipView::adjustVelocity(int velocityChange) {
  ...
  for (int i = 0; i < editPadPressBufferSize; i++) {
    if (editPadPresses[i].isActive) {
      ...
      int      noteRowIndex;
      NoteRow* noteRow   = getCurrentClip()->getNoteRowOnScreen(editPadPresses[i].yDisplay,
                                                              currentSong,
                                                              &noteRowIndex);
      ...
      if (editPadPresses[i].isBlurredSquare) {
        ...
        Note* note  = noteRow->notes.getElement(noteI);
        while (note &&
               note->pos - editPadPresses[i].intendedPos < editPadPresses[i].intendedLength) {
...
#endif
            noteRow->changeNotesAcrossAllScreens(note->pos,
                                                 modelStackWithNoteRow,
                                                 action,
                                                 CORRESPONDING_NOTES_ADJUST_VELOCITY,
                                                 velocityChange);
          }
        ...
        editPadPresses[i].intendedVelocity =
            velocitySumThisSquare / numNotesThisSquare; // Get the average. Ideally we'd have done
                                                        // this when first selecting the note too,
                                                        // but I didn't
      }
      ...
      // Only one note in square
        ...
        if (numericDriver.popupActive) {
          ...
          noteRow->changeNotesAcrossAllScreens(editPadPresses[i].intendedPos,
                                               modelStackWithNoteRow,
                                               action,
                                               CORRESPONDING_NOTES_ADJUST_VELOCITY,
                                               velocityChange);
  ...
  if (velocityValue) {
    ...
    if (velocityValue == 255) {
      ...
      // Don't bother trying to think of some smart way to update
      // lastVelocityInteractedWith. It'll get updated when user releases last
      // press.
    } else {
#if HAVE_OLED
            ...
            if (probabilityValue < NUM_PROBABILITY_VALUES + 35) {
              if (prevBase) {
                ...
                if (probabilityValue < NUM_PROBABILITY_VALUES &&
                    getCurrentClip()->doesProbabilityExist(editPadPresses[i].intendedPos,
                                                           probabilityValue,
  ...
  multiplePresses:
    ...
    for (int i = 0; i < editPadPressBufferSize; i++) {
      if (editPadPresses[i].isActive) {
        ...
        if (editPadPresses[i].isBlurredSquare) {
          NoteRow* noteRow =
              getCurrentClip()->getNoteRowOnScreen(editPadPresses[i].yDisplay, currentSong);
          int   noteI = noteRow->notes.search(editPadPresses[i].intendedPos, GREATER_OR_EQUAL);
          Note* note  = noteRow->notes.getElement(noteI);
          if (note) {
            editPadPresses[i].intendedProbability = note->probability; // This might not have been
                                                                       // grabbed properly initially
    ...
    // Decide the probability, based on the existing probability of the leftmost
    // note
    ...
    // Set the probability of the other presses, and update all probabilities
      ...
      if (editPadPresses[i].isActive) {
        ...
        if (editPadPresses[i].isBlurredSquare) {
          ...
          while (note &&
            ...
            // And if not one of the leftmost notes, make it a prev-base one -
            // if we're doing actual percentage probabilities
        ...
        // Or, just 1 note in square
          ...
          // And if not one of the leftmost notes, make it a prev-base one - if
          // we're doing actual percentage probabilities
          if (probabilityValue < NUM_PROBABILITY_VALUES &&
...
void InstrumentClipView::mutePadPress(uint8_t yDisplay) {
  ...
  // We do not want to change the selected Drum if stutter is happening, because
  // the user needs to keep controlling, and eventually stop stuttering on,
  // their current selected Drum
...
NoteRow* InstrumentClipView::createNewNoteRowForKit(ModelStackWithTimelineCounter* modelStack,
                                                    int                            yDisplay,
                                                    int*                           getIndex) {
  InstrumentClip* clip = (InstrumentClip*)modelStack->getTimelineCounter();

  NoteRow* newNoteRow =
      clip->createNewNoteRowForKit(modelStack, (yDisplay < -clip->yScroll), getIndex);
...
ModelStackWithNoteRow*
InstrumentClipView::getOrCreateNoteRowForYDisplay(ModelStackWithTimelineCounter* modelStack,
                                                  int                            yDisplay) {
...
ModelStackWithNoteRow*
InstrumentClipView::createNoteRowForYDisplay(ModelStackWithTimelineCounter* modelStack,
                                             int                            yDisplay) {
...
void InstrumentClipView::recalculateColours() {
  for (int yDisplay = 0; yDisplay < displayHeight; yDisplay++) { recalculateColour(yDisplay); }
}
...
void InstrumentClipView::recalculateColour(uint8_t yDisplay) {
  ...
  getCurrentClip()->getMainColourFromY(
...
int InstrumentClipView::scrollVertical(int scrollAmount, bool inCardRoutine, bool draggingNoteRow) {
  ...
  // Or if not a Kit...
    ...
    if (scrollAmount > 0) {
      newYNote =
          getCurrentClip()->getYNoteFromYDisplay(displayHeight - 1 + scrollAmount, currentSong);
    } else {
  ...
  // Switch off any auditioned notes. But leave on the one whose NoteRow we're
  // moving, if we are
  for (int yDisplay = 0; yDisplay < displayHeight; yDisplay++) {
          ...
          if (editPadPresses[i].deleteOnScroll) {
            ...
            if (mpeParams) {
              int32_t distanceToNextNote =
                  thisNoteRow->getDistanceToNextNote(pos, modelStackWithNoteRow);
              ...
              ModelStackWithParamCollection* modelStackWithParamCollection =
                  modelStackWithNoteRow->addOtherTwoThingsAutomaticallyGivenNoteRow()
                      ->addParamCollection(mpeParams, mpeParamsSummary);
              ...
              for (int m = 0; m < NUM_EXPRESSION_DIMENSIONS; m++) {
                ...
                param->stealNodes(modelStackWithAutoParam,
                                  pos,
                                  distanceToNextNote,
                                  loopLength,
                                  action,
  ...
  // Shift the selected NoteRow, if that's what we're doing. We know we're in
  // Kit mode then
  if (draggingNoteRow) {
    ...
    getCurrentClip()->noteRows.getElement(noteRowToShiftI)->y =
        -32768; // Need to remember not to try and use the yNote value of this
                // NoteRow if we switch back out of Kit mode
  ...
  recalculateColours(); // Don't render - we'll do that after we've dealt with
                        // presses (potentially creating Notes)

  // Switch on any auditioned notes - remembering that the one we're shifting
  // (if we are) was left on before
  ...
  for (int yDisplay = 0; yDisplay < displayHeight; yDisplay++) {
    if (lastAuditionedVelocityOnScreen[yDisplay] != 255) {
      ...
      // Otherwise, switch its audition back on
        ...
        if (!isKit || modelStackWithNoteRow->getNoteRowAllowNull()) {
          ...
          if (modelStackWithNoteRow->getNoteRowAllowNull() &&
            ...
            sendAuditionNote(true,
                             yDisplay,
                             lastAuditionedVelocityOnScreen[yDisplay],
                             0); // Should this technically grab the note-length
                                 // of the note if there is one?
          }
        } else {
      ...
      if (!draggingNoteRow && !drawnNoteCodeYet &&
          auditionPadIsPressed[yDisplay]) { // If we're shiftingNoteRow, no need
                                            // to re-draw the noteCode, because
                                            // it'll be the same
        ...
        if (currentSong->currentClip->output->type == INSTRUMENT_TYPE_SYNTH) {
          if (getCurrentUI() == &soundEditor &&
              soundEditor.getCurrentMenuItem() == &multiRangeMenu) {
  ...
  if (numEditPadPresses > 0) {
    ...
    for (int i = 0; i < editPadPressBufferSize; i++) {
      if (editPadPresses[i].isActive) {
        ...
        bool success = noteRow->attemptNoteAdd(pos,
        ...
        if (success) {
          if (editPadPresses[i].mpeCachedYet) {
            ...
            ParamCollectionSummary* mpeParamsSummary =
                noteRow->paramManager.getExpressionParamSetSummary();
            ExpressionParamSet* mpeParams = (ExpressionParamSet*)mpeParamsSummary->paramCollection;
            ...
            if (mpeParams) {
              ...
              int32_t distanceToNextNote =
                  noteRow->getDistanceToNextNote(pos, modelStackWithNoteRow);
              int loopLength = modelStackWithNoteRow->getLoopLength();
              ...
              for (int m = 0; m < NUM_EXPRESSION_DIMENSIONS; m++) {
                ...
                param->insertStolenNodes(modelStackWithAutoParam,
                                         pos,
                                         distanceToNextNote,
                                         loopLength,
                                         action,
                                         &editPadPresses[i].stolenMPE[m]);
              }
            }
          }
        }
    ...
    checkIfAllEditPadPressesEnded(false); // Don't allow to redraw sidebar - it's going to be
                                          // redrawn below anyway
  }
...
void InstrumentClipView::reassessAllAuditionStatus() {
  for (int yDisplay = 0; yDisplay < displayHeight; yDisplay++) { reassessAuditionStatus(yDisplay); }
}
...
void InstrumentClipView::reassessAuditionStatus(uint8_t yDisplay) {
  ...
  // If some change in the NoteRow's audition status (it's come on or off or had
  // its velocity changed)...
...
void InstrumentClipView::sendAuditionNote(bool     on,
                                          uint8_t  yDisplay,
                                          uint8_t  velocity,
                                          uint32_t sampleSyncLength) {
  ...
  if (instrument->type == INSTRUMENT_TYPE_KIT) {
    ...
    ModelStackWithNoteRow* modelStackWithNoteRow =
        getCurrentClip()->getNoteRowOnScreen(yDisplay,
                                             modelStackWithTimelineCounter); // On *current* clip!

    NoteRow* noteRowOnCurrentClip = modelStackWithNoteRow->getNoteRowAllowNull();

    // There may be no NoteRow at all if a different Clip than the one we're
    // viewing is the activeClip, and it can't be changed
    if (noteRowOnCurrentClip) {
      ...
      if (drum) {
        ...
        if (currentSong->currentClip != instrument->activeClip) {
          ...
          modelStackWithNoteRow = ((InstrumentClip*)instrument->activeClip)
        ...
        if (on) {
          ...
          ((Kit*)instrument)
              ->beginAuditioningforDrum(modelStackWithNoteRow, drum, velocity, zeroMPEValues);
        } else {
    ...
    if (on) {
      ((MelodicInstrument*)instrument)
          ->beginAuditioningForNote(modelStack,
                                    yNote,
                                    velocity,
                                    zeroMPEValues,
                                    MIDI_CHANNEL_NONE,
...
uint8_t InstrumentClipView::getVelocityForAudition(uint8_t yDisplay, uint32_t* sampleSyncLength) {
  ...
  *sampleSyncLength         = 0;
  ...
  if (!playbackHandler.playbackState && numEditPadPressesPerNoteRowOnScreen[yDisplay] > 0) {
    ...
    if (makeCurrentClipActiveOnInstrumentIfPossible(
            modelStack)) { // Should always be true, cos playback is stopped
...
void InstrumentClipView::offsetNoteCodeAction(int newOffset) {
  ...
  if (getCurrentClip()->isScaleModeClip()) {
    ...
    // If not allowed to move, blink the scale mode button to remind the user
    // that that's why
  ...
  // If we're in Kit mode, the NoteRow will exist, or else we wouldn't be
  // auditioning it. But if in other mode, we need to do this
...
Drum* InstrumentClipView::flipThroughAvailableDrums(int newOffset, Drum* drum, bool mayBeNone) {
  ...
  if (newOffset >= 0) {
    while (true) {
      ...
      // Keep going until we get back to where we started, or we're on "none" or
      // "new", or we find an unused Drum.
...
// Beware - supplying shouldRedrawStuff as false will cause the
// activeModControllable to *not* update! Probably never should do this
// anymore...
void InstrumentClipView::setSelectedDrum(Drum* drum, bool shouldRedrawStuff) {

  if (getCurrentUI() != &soundEditor && getCurrentUI() != &sampleBrowser &&
  ...
  if (shouldRedrawStuff) { renderingNeededRegardlessOfUI(0, 0xFFFFFFFF); }
}
...
void InstrumentClipView::auditionPadAction(int velocity, int yDisplay, bool shiftButtonDown) {
    ...
    // If NoteRow doesn't exist here, we'll see about creating one
      ...
      if (velocity) {
        ...
        if (currentUIMode == UI_MODE_NONE) {
          ...
          // Remember what NoteRow was pressed - and limit to being no further
          // than 1 above or 1 below the existing NoteRows
          yDisplayOfNewNoteRow = yDisplay;
          yDisplayOfNewNoteRow =
              getMax((int)yDisplayOfNewNoteRow, (int)-1 - getCurrentClip()->yScroll);
      ...
      // Press-up
        ...
        if (currentUIMode == UI_MODE_ADDING_DRUM_NOTEROW) {
          ...
          if (drumForNewNoteRow) {
            ...
            // Make a new NoteRow
            int      noteRowIndex;
            NoteRow* newNoteRow = createNewNoteRowForKit(modelStackWithTimelineCounter,
                                                         yDisplayOfNewNoteRow,
                                                         &noteRowIndex);
            if (newNoteRow) {
              uiNeedsRendering(this, 0, 1 << yDisplayOfNewNoteRow);
  ...
  if (clipIsActiveOnInstrument && playbackHandler.shouldRecordNotesNow() &&
    ...
    if (velocity) {
      ...
      // If count-in is on, we only got here if it's very nearly finished, so
      // pre-empt that note. This is basic. For MIDI input, we do this in a
      // couple more cases - see noteMessageReceived() in MelodicInstrument and
      // Kit
          ...
          ((MelodicInstrument*)instrument)
              ->earlyNotes.insertElementIfNonePresent(
                  ...
                  getCurrentClip()->allowNoteTails(
                      modelStackWithNoteRowOnCurrentClip)); // NoteRow is
                                                            // allowed to be
                                                            // NULL in this
                                                            // case.
        }
...

  {
    ...
    if (velocity) {
      ...
      auditionPadIsPressed[yDisplay] =
          velocityToSound; // Yup, need to do this even if we're going to do a
                           // "silent" audition, so pad lights up etc.

      if (noteRowOnActiveClip) {
        // Ensure our auditioning doesn't override a note playing in the
        // sequence
        if (playbackHandler.isEitherClockActive() &&
      ...
      if (isKit) {
        ...
        goto getOut; // No need to redraw any squares, because setSelectedDrum()
                     // has done it
      }
    }
    ...
    // Or if auditioning this NoteRow just finished...
      ...
      if (auditionPadIsPressed[yDisplay]) {
        ...
        // Stop the note sounding - but only if a sequenced note isn't in fact
        // being played here.
...
void InstrumentClipView::enterDrumCreator(ModelStackWithNoteRow* modelStack, bool doRecording) {
  ...
  kit->addDrum(newDrum);
  modelStack->getNoteRow()->setDrum(newDrum, kit, modelStack); // Sets noteRow->paramManager to
                                                               // newDrum->backedUpParamManager
  ...
  if (doRecording) {
    ...
    if (success) { audioRecorder.process(); }
  } else {
    success = openUI(&sampleBrowser);
    if (success) {
      PadLEDs::skipGreyoutFade(); // Greyout can't be done at same time as
                                  // horizontal scroll, which is now happening
                                  // probably.
...
void InstrumentClipView::deleteDrum(SoundDrum* drum) {
  ...
  if (noteRow) {
    ...
    if (!noteRow->drum) {
      ...
      if (noteRow->hasNoNotes() && getCurrentClip()->getNumNoteRows() > 1) {
        if (noteRowIndex == 0) { getCurrentClip()->yScroll--; }
  ...
  // We should repopulateNoteRowsOnscreen() and everything, but this will only
  // be called just before the UI sessions starts again anyway
}
...
void InstrumentClipView::someAuditioningHasEnded(bool recalculateLastAuditionedNoteOnScreen) {
  ...
  for (i = 0; i < displayHeight; i++) {
    if (auditionPadIsPressed[i]) {
      // Show this note's noteCode, if the noteCode we were showing before is
      // the note we just stopped auditioning
...
int InstrumentClipView::setupForEnteringScaleMode(int newRootNote, int yDisplay) {
  // Having got to this function, we have recently calculated the default root
  // note
  ...
  if (newRootNote != 2147483647) {
    ...
    pinAnimationToYNote =
        getCurrentClip()->getYNoteFromYDisplay(yDisplay,
                                               currentSong); // This is needed in case we're coming
  ...
  // Otherwise, go with the previously calculated default root note
    ...
    // If there's a root-note (or its octave) currently onscreen, pin animation
    // to that
    for (int i = 0; i < displayHeight; i++) {
    ...
    pinAnimationToYNote =
        getCurrentClip()->getYNoteFromYDisplay(pinAnimationToYDisplay, currentSong);
  }
...
doneLookingForRootNoteOnScreen:
  ...
  currentSong->setRootNote(newRootNote,
                           getCurrentClip()); // Computation to find out what notes in scale
...
void InstrumentClipView::enterScaleMode(uint8_t yDisplay) {
  ...
  for (int i = 0; i < clip->noteRows.getNumElements(); i++) {
    ...
    if ((yDisplayTo >= 0 && yDisplayTo < displayHeight) ||
        (yDisplayFrom >= 0 && yDisplayFrom < displayHeight)) {

      ModelStackWithNoteRow* modelStackWithNoteRow =
          modelStack->addNoteRow(thisNoteRow->y, thisNoteRow);
      ...
      thisNoteRow->renderRow(this,
                             mainColour,
                             tailColour,
                             blurColour,
                             &PadLEDs::imageStore[PadLEDs::numAnimatedRows][0][0],
                             PadLEDs::occupancyMaskStore[PadLEDs::numAnimatedRows],
                             true,
                             modelStackWithNoteRow->getLoopLength(),
                             clip->allowNoteTails(modelStackWithNoteRow),
                             displayWidth,
                             currentSong->xScroll[NAVIGATION_CLIP],
                             currentSong->xZoom[NAVIGATION_CLIP]);
      drawMuteSquare(thisNoteRow,
                     PadLEDs::imageStore[PadLEDs::numAnimatedRows],
                     PadLEDs::occupancyMaskStore[PadLEDs::numAnimatedRows]);
...
int InstrumentClipView::setupForExitingScaleMode() {
  ...
  if (!foundRootNoteOnScreen)
    scrollAdjust =
        getCurrentClip()->getYNoteFromYVisual(getCurrentClip()->yScroll + 1, currentSong) - 1 -
...
// If called from KeyboardScreen, the newRootNote won't correspond to the
// yDisplay, and that's ok
...
bool InstrumentClipView::renderSidebar(uint32_t whichRows,
  ...
  for (int i = 0; i < displayHeight; i++) {
    if (whichRows & (1 << i)) {
      drawMuteSquare(getCurrentClip()->getNoteRowOnScreen(i, currentSong),
                     image[i],
                     occupancyMask[i]);
...
void InstrumentClipView::drawMuteSquare(NoteRow* thisNoteRow,
  ...
  // If user assigning MIDI controls and this NoteRow has a command assigned,
  // flash pink
  ...
  // If user assigning MIDI controls and has this Clip selected, flash to half
  // brightness
  if (view.midiLearnFlashOn && thisNoteRow != NULL &&
      view.thingPressedForMidiLearn == MIDI_LEARN_NOTEROW_MUTE && thisNoteRow->drum &&
      &thisNoteRow->drum->muteMIDICommand == view.learnedThing) {
    thisColour[0] >>= 1;
    thisColour[1] >>= 1;
    thisColour[2] >>= 1;
    *thisOccupancy = 64;
  ...
  drawNormally:
    ...
    if (currentSong->currentClip->output->type == INSTRUMENT_TYPE_KIT) {
      ...
      if (noteRow != NULL && noteRow->drum != NULL &&
        ...
        int totalColour = (uint16_t)rowColour[yDisplay][0] + rowColour[yDisplay][1] +
                          rowColour[yDisplay][2]; // max 765
      ...
      checkIfSelectingRanges:
        ...
        if (getCurrentUI() == &sampleBrowser || getCurrentUI() == &audioRecorder ||
            (getCurrentUI() == &soundEditor &&
             soundEditor.getCurrentMenuItem()->isRangeDependent())) {
          int yNote = getCurrentClip()->getYNoteFromYDisplay(yDisplay, currentSong);
...
static const uint32_t verticalScrollUIModes[] = {UI_MODE_NOTES_PRESSED,
                                                 UI_MODE_AUDITIONING,
                                                 UI_MODE_RECORD_COUNT_IN,
                                                 UI_MODE_DRAGGING_KIT_NOTEROW,
                                                 0};
...
int InstrumentClipView::verticalEncoderAction(int offset, bool inCardRoutine) {
  ...
  if (Buttons::isButtonPressed(yEncButtonX, yEncButtonY)) {
    ...
    if (isUIModeActiveExclusively(UI_MODE_AUDITIONING)) {
      /*
      if (!Buttons::isShiftButtonPressed()) { // Why'd I mandate that shift not
      be pressed?
          // If in kit mode, then we can do it
          if (currentSong->currentClip->output->type == INSTRUMENT_TYPE_KIT) {

              if (inCardRoutine) return
      ACTION_RESULT_REMIND_ME_OUTSIDE_CARD_ROUTINE;

              cutAuditionedNotesToOne();
              return scrollVertical(offset, inCardRoutine, true); // Will delete
      action log in this case
          }

          // Otherwise, remind the user why they can't
          else {
              if (currentSong->currentClip->output->type ==
      INSTRUMENT_TYPE_SYNTH) IndicatorLEDs::indicateAlertOnLed(synthLedX,
      synthLedY); else IndicatorLEDs::indicateAlertOnLed(midiLedX, midiLedY); //
      MIDI
          }
      }
      */
    ...
    // If user not wanting to move a noteCode, they want to transpose the key
      ...
      // Otherwise, transpose single semitone
        ...
        if (!getCurrentClip()->isScaleModeClip()) {
          ...
          // If there are no scale-mode Clips at all, move the root note along
          // as well - just in case the user wants to go back to scale mode (in
          // which case the "previous" root note would be used to help guess
          // what root note to go with)
  ...
  // Or, if shift key is pressed
        ...
        for (int yDisplay = 0; yDisplay < displayHeight; yDisplay++) {
          if (auditionPadIsPressed[yDisplay]) {
            ModelStackWithNoteRow* modelStackWithNoteRow =
                getCurrentClip()->getNoteRowOnScreen(yDisplay, modelStack);
            NoteRow* noteRow = modelStackWithNoteRow->getNoteRowAllowNull();
            if (noteRow) { // This is fine. If we were in Kit mode, we could
                           // only be auditioning if there was a NoteRow already
    ...
    if (whichRowsToRender) { uiNeedsRendering(this, whichRowsToRender, whichRowsToRender); }
  }
...
static const uint32_t noteNudgeUIModes[] = {UI_MODE_NOTES_PRESSED,
                                            UI_MODE_HOLDING_HORIZONTAL_ENCODER_BUTTON,
                                            0};
...
int InstrumentClipView::horizontalEncoderAction(int offset) {
  ...
  if (isUIModeActive(UI_MODE_NOTES_PRESSED)) {
    ...
    if (!Buttons::isShiftButtonPressed()) {
      ...
      // Or, if horizontal encoder held down, nudge note
        ...
        if (sdRoutineLock)
          return ACTION_RESULT_REMIND_ME_OUTSIDE_CARD_ROUTINE; // Just be safe -
                                                               // maybe not
                                                               // necessary
        nudgeNotes(offset);
      }
  ...
  // Auditioning but not holding down <> encoder - edit length of just one row
    ...
    // Unlike for all other cases where we protect against the user accidentally
    // turning the encoder more after releasing their press on it, for this
    // edit-NoteRow-length action, because it's a related action, it's quite
    // likely that the user actually will want to do it after the
    // yes-pressed-encoder-down action, which is "rotate/shift notes in row".
    // So, we have a 250ms timeout for this one.
    else if ((uint32_t)(AudioEngine::audioSampleTimer - timeHorizontalKnobLastReleased) >=
             250 * 44) {
      shouldIgnoreHorizontalScrollKnobActionIfNotAlsoPressedForThisNotePress = false;
  ...
  // Auditioning *and* holding down <> encoder - rotate/shift just one row
    ...
    ModelStackWithNoteRow* modelStackWithNoteRow =
        ((InstrumentClip*)modelStack->getTimelineCounter())
            ->getNoteRowOnScreen(lastAuditionedYDisplay,
  ...
  // Or, maybe shift everything horizontally
    ...
    // If possible, just modify a previous Action to add this new shift amount
    // to it.
    Action* action = actionLogger.firstAction[BEFORE];
    if (action && action->type == ACTION_INSTRUMENT_CLIP_HORIZONTAL_SHIFT &&
        action->openForAdditions && action->currentClip == clip) {

      // If there's no Consequence in the Action, that's probably because we
      // deleted it a previous time with the code just below. Or possibly
      // because the Action was created but there wasn't enough RAM to create
      ...
      // It might look tempting that if we've completed one whole loop, we could
      // delete the Consequence because everything would be back the same - but
      // no! Remember different NoteRows might have different lengths.
    }

    // Or if no previous Action, go create a new one now.
    else {

      action = actionLogger.getNewAction(ACTION_INSTRUMENT_CLIP_HORIZONTAL_SHIFT,
                                         ACTION_ADDITION_NOT_ALLOWED);
      if (action) {
      addConsequenceToAction:
        void* consMemory =
            generalMemoryAllocator.alloc(sizeof(ConsequenceInstrumentClipHorizontalShift));
...
void InstrumentClipView::editNoteRepeat(int offset) {
  ...
  int32_t squareWidth =
      getSquareWidth(editPadPresses[i].xDisplay, modelStackWithNoteRow->getLoopLength());
  ...
  // If "just displaying not editing" or unable to move any further, just
  // display and get out
  if (!offset || newNumNotes < 1 || newNumNotes > squareWidth) {
    ...
    if (offset && lastAction && lastAction->type == ACTION_NOTE_REPEAT_EDIT &&
        lastAction->openForAdditions && lastAction->offset == -offset) {
      actionLogger.undoJustOneConsequencePerNoteRow(
          modelStack->toWithSong()); // Only ok because we're not going to use
                                     // the ModelStackWithTimelineCounter or
                                     // with any more stuff again here.
    }
...
void InstrumentClipView::nudgeNotes(int offset) {
  ...
  // If just popping up number, but multiple presses, we're quite limited with
  // what intelligible stuff we can display
  ...
  // Declare these out here so we can keep the value - we'll use this for the
  // mod region if there was only 1 press
  int32_t  newPos;
  NoteRow* noteRow;
  int      noteRowId;
  ...
  // If the user is nudging back in the direction they just nudged, we can do a
  // (possibly partial) undo, getting back the proper length of any notes that
  // got trimmed etc.

  Action* lastAction = actionLogger.firstAction[BEFORE];
  if (offset && lastAction && lastAction->type == ACTION_NOTE_NUDGE &&
      lastAction->openForAdditions && lastAction->offset == -offset) {
    ...
    for (int i = 0; i < editPadPressBufferSize; i++) {
      if (editPadPresses[i].isActive) {
        ...
        int noteRowIndex;
        noteRow =
            currentClip->getNoteRowOnScreen(editPadPresses[i].yDisplay, currentSong, &noteRowIndex);
  ...
  // Or, if not doing the partial-undo method, we'll just try and do a plain old
  // nudge
  else {
    Action* action = NULL;
    ...
    for (int i = 0; i < editPadPressBufferSize; i++) {
      if (editPadPresses[i].isActive) {
        ...
        if (offset)
          editPadPresses[i].isBlurredSquare =
              true; // So it doesn't get dragged along with a vertical scroll
        ...
        if (offset) {
          // We're going to nudge notes across all screens, but before we do,
          // check if this particular note is gonna collide with anything
          int searchBoundary;
          int searchDirection;
          int n;
          if (offset >= 0) { // Nudging right
            ...
            doCompareNote:
              Note* note = noteRow->notes.getElement(n);
              if (note && note->pos == newPos) {
                newPos = editPadPresses[i].intendedPos; // Make it so the below code just displays
                                                        // the already existing offset
                gotCollision = true;
              }
            }
          } else { // Nudging left
        ...
        if (!gotCollision) {
          ...
          int error = noteRow->nudgeNotesAcrossAllScreens(editPadPresses[i].intendedPos,
                                                          modelStackWithNoteRow,
                                                          action,
                                                          currentClip->getWrapEditLevel(),
                                                          offset);
          ...
          // Nudge automation at NoteRow level, while our ModelStack still has a
          // pointer to the NoteRow
          {
            ...
            noteRow->paramManager.nudgeAutomationHorizontallyAtPos(
                editPadPresses[i].intendedPos,
                offset,
                modelStackWithThreeMainThingsForNoteRow->getLoopLength(),
                action,
                modelStackWithThreeMainThingsForNoteRow,
                distanceTilNext);
          }

          // WARNING! A bit dodgy, but at this stage, we can no longer refer to
          // modelStackWithNoteRow, cos we're going to reuse its parent
          // ModelStackWithTimelineCounter, below.
          ...
          // Nudge automation at Clip level
          {
            ...
            ModelStackWithThreeMainThings* modelStackWithThreeMainThings =
                modelStackWithTimelineCounter->addOtherTwoThingsButNoNoteRow(
                    currentClip->output->toModControllable(),
                    &currentClip->paramManager);
  ...
  ModelStackWithTimelineCounter* modelStackWithTimelineCounter = modelStack->addTimelineCounter(
      modelStack->song->currentClip); // Can finally do this since we're not going to
                                      // use the bare ModelStack for anything else

  if (numEditPadPresses > 1) {
    if (!didAnySuccessfulNudging)
      return; // Don't want to see these "multiple pads moved" messages if in
              // fact none were moved
#if HAVE_OLED
    ...
    if (resultingTotalOffset > 0) {
      ...
      if (resultingTotalOffset > (squareWidth >> 1)) {
        if (!doneAnyNudgingSinceFirstEditPadPress) { offsettingNudgeNumberDisplay = true; }
      } else {
        offsettingNudgeNumberDisplay = false;
      }
  ...
  doneAnyNudgingSinceFirstEditPadPress =
      true; // Even if we didn't actually nudge, we want to record this for the
  ...
  // Otherwise, update it for what they actually intend
    ...
    // view.setModRegion(newPos, max((uint32_t)distanceToNextNote +
    // lastNote->pos - firstNote->pos, squareWidth)); // This is what happens
    // with initial press, kinda different...
  ...
  if (playbackHandler.isEitherClockActive() &&
      modelStackWithTimelineCounter->song->isClipActive(currentClip)) {
    currentClip->expectEvent();
...
void InstrumentClipView::graphicsRoutine() {
  if (!currentSong)
    return; // Briefly, if loading a song fails, during the creation of a new
            // blank one, this could happen.
...
void InstrumentClipView::fillOffScreenImageStores() {
  ...
  // We're also going to fill up an extra, currently-offscreen imageStore row,
  // with all notes currently offscreen
  ...
  if (currentSong->currentClip->output->type == INSTRUMENT_TYPE_KIT) {
    ...
    noteRowIndexBottom =
        getCurrentClip()->noteRows.search(getCurrentClip()->getYNoteFromYDisplay(0, currentSong),
                                          GREATER_OR_EQUAL);
    noteRowIndexTop = getCurrentClip()->noteRows.search(
        getCurrentClip()->getYNoteFromYDisplay(displayHeight, currentSong),
        GREATER_OR_EQUAL);
  }
  ...
  currentSong->currentClip->renderAsSingleRow(modelStack,
                                              this,
                                              xScroll,
                                              xZoom,
                                              PadLEDs::imageStore[0][0],
  ...
  currentSong->currentClip->renderAsSingleRow(modelStack,
                                              ...
                                              PadLEDs::occupancyMaskStore[displayHeight + 1],
                                              false,
                                              noteRowIndexTop,
                                              2147483647,
                                              0,
  ...
  for (int yDisplay = 0; yDisplay < displayHeight; yDisplay++) {
    ...
    if (whichRows & (1 << yDisplay)) {
      ...
      // Otherwise render the row
        ...
        noteRow->renderRow(this,
                           rowColour[yDisplay],
                           rowTailColour[yDisplay],
                           rowBlurColour[yDisplay],
                           image,
                           occupancyMaskOfRow,
                           true,
                           modelStackWithNoteRow->getLoopLength(),
...
void InstrumentClipView::transitionToSessionView() {
  ...
  currentUIMode = UI_MODE_INSTRUMENT_CLIP_COLLAPSING; // Must set this after above render
                                                      // calls, or else they'll see it and
                                                      // not render
...
void InstrumentClipView::playbackEnded() {
  ...
  // Easter egg - if user's holding down a note, we want it to be
  // edit-auditioned again now
  reassessAllAuditionStatus();
}
...
void InstrumentClipView::scrollFinished() {
  if (currentUIMode == UI_MODE_AUDITIONING) {
    uiNeedsRendering(this,
                     0xFFFFFFFF,
                     0); // Needed because sometimes we initiate a scroll before reverting an
                         // Action, so we need to properly render again afterwards
  }
...
void InstrumentClipView::dontDeleteNotesOnDepress() {
  for (int i = 0; i < editPadPressBufferSize; i++) { editPadPresses[i].deleteOnDepress = false; }
}
    ...
    if (kit->selectedDrum && kit->selectedDrum->type != DRUM_TYPE_SOUND) {
      ...
      ModelStackWithNoteRow* modelStackWithNoteRow =
          ((InstrumentClip*)kit->activeClip)
              ->getNoteRowForDrum(modelStackWithTimelineCounter,
                                  kit->selectedDrum); // The NoteRow probably doesn't get
                                                      // referred to...
      ...
      NoteRow*                 noteRow      = modelStackWithNoteRow->getNoteRowAllowNull();
      if (noteRow)
        paramManager = &noteRow->paramManager; // Should be NULL currently, cos
                                               // it's a NonAudioDrum.
      ModelStackWithThreeMainThings* modelStackWithThreeMainThings =
...
void InstrumentClipView::editNumEuclideanEvents(ModelStackWithNoteRow* modelStack,
                                                int                    offset,
                                                int                    yDisplay) {
...

  {
    ...
    if (offset) { // Or if offset is 0, we'll just display the current number,
                  // below, without changing anything
      newNumNotes += offset;
      ...
      Action* lastAction = actionLogger.firstAction[BEFORE];
      // No need to check that lastAction was for the same Clip or anything -
      // the Action gets "closed" manually when we stop auditioning.
      if (lastAction && lastAction->type == ACTION_EUCLIDEAN_NUM_EVENTS_EDIT &&
          lastAction->openForAdditions && lastAction->offset == -offset) {

        char        modelStackMemory2[MODEL_STACK_MAX_SIZE];
        ModelStack* modelStackWithJustSong =
            setupModelStackWithSong(modelStackMemory2, modelStack->song);

        bool revertedWholeAction =
            actionLogger.undoJustOneConsequencePerNoteRow(modelStackWithJustSong);
        if (!revertedWholeAction) goto noteRowChanged;
      }

      else {
        {
          // Make new NoteVector for the new Notes, since ActionLogger should be
          // "stealing" the old data
          NoteVector newNotes;
          if (newNumNotes) {
          ...
          // Delete / steal / consequence-ize the MPE data first, because in
          // order for partial undos to work, this has to be further down the
          // linked list of Consequences than the note-array-change that we do
          // next, below.
          ParamCollectionSummary* mpeParamsSummary =
              noteRow->paramManager.getExpressionParamSetSummary();
          ExpressionParamSet* mpeParams = (ExpressionParamSet*)mpeParamsSummary->paramCollection;
          ...
          if (action) {
            // We "definitely" store the change, because unusually, we may want
            // to revert individual Consequences in the Action one by one
            action->recordNoteArrayChangeDefinitely(clip,
...
// Check UI mode is appropriate before calling this
void InstrumentClipView::rotateNoteRowHorizontally(ModelStackWithNoteRow* modelStack,
                                                   int                    offset,
                                                   int                    yDisplay,
...

  {
    ...
    getNewAction:
      action =
          actionLogger.getNewAction(ACTION_NOTEROW_HORIZONTAL_SHIFT, ACTION_ADDITION_NOT_ALLOWED);
      if (action) {
      addConsequenceToAction:
        void* consMemory = generalMemoryAllocator.alloc(sizeof(ConsequenceNoteRowHorizontalShift));

        if (consMemory) {
          ConsequenceNoteRowHorizontalShift* newConsequence = new (consMemory)
              ConsequenceNoteRowHorizontalShift(modelStack->noteRowId, shiftAmount);
          action->addConsequence(newConsequence);
        }
      }
    }
...
void InstrumentClipView::editNoteRowLength(ModelStackWithNoteRow* modelStack,
                                           int                    offset,
                                           int                    yDisplay) {
  ...
  // If we're not scrolled all the way to the right, go there now. If we were
  // already further right than the end of this NoteRow, it's ok, we'll stay
  // there.
  ...
  int32_t oldPos = modelStack->getLastProcessedPos(); // We have to grab and remember this
                                                      // before an initial revert() ("secret
                                                      // UNDO") potentially stuffs it up
  ...
  if (prevAction && prevAction->openForAdditions &&
      prevAction->type == ACTION_NOTEROW_LENGTH_EDIT && prevAction->currentClip == clip) {
    ...
    // If we're recovering a bit that previously got chopped off, do secret undo
    // to recover any chopped-off notes and automation
    if (offset == 1 && prevCons->backedUpLength > oldLength) {
      ...
      possiblyDoResumePlaybackOnNoteRow:
        // Need to do the resumePlayback that we blocked happening during the
        // revert()
    ...
    // Otherwise, the Action / Consequence is still fine for doing a future
    // UNDO, so just edit length as needed. But we'll still pass the prevAction
    // in, so that anything which wasn't snapshotted yet (because no Notes
    // happened to get trimmed last time) can get snapshotted now.
    else {
      noteRow->setLength(modelStack,
                         newLength,
                         prevAction,
                         oldPos,
...
void InstrumentClipView::reportMPEInitialValuesForNoteEditing(ModelStackWithNoteRow* modelStack,
  ...
  // MPE stuff - if editing note, we need to take note of the initial values
  // which might have been sent before this note-on.
  if (noteRow && view.modLength && modelStack->noteRowId == view.modNoteRowId &&
      modelStack->getTimelineCounter() ==
          view.activeModControllableModelStack.getTimelineCounterAllowNull()) {

    noteRow->paramManager.ensureExpressionParamSetExists(); // If this fails, we'll detect that
                                                            // below.
    ...
    if (mpeParams) {
      ...
      for (int whichExpressionDimension = 0; whichExpressionDimension < NUM_EXPRESSION_DIMENSIONS;
        ...
        mpeValuesAtHighestPressure[0][whichExpressionDimension] =
            mpeValues[whichExpressionDimension];
      }
    }
  }
}
...
void InstrumentClipView::reportMPEValueForNoteEditing(int whichExpressionimension, int32_t value) {
  ...
  if (timeSince >= MPE_RECORD_INTERVAL_TIME) {
    ...
    memmove(mpeValuesAtHighestPressure[1],
            mpeValuesAtHighestPressure[0],
            sizeof(uint16_t) * NUM_EXPRESSION_DIMENSIONS *
                (MPE_RECORD_LENGTH_FOR_NOTE_EDITING - 1));
    mpeValuesAtHighestPressure[0][2] = 0; // Yes only reset the "pressure" of the new/first record
    mpeMostRecentPressure            = 0;
  }

  // Always keep track of the "current" pressure value, so we can decide whether
  // to be recording the other values.
  if (whichExpressionimension == 2) { mpeMostRecentPressure = value >> 16; }
  ...
  dontDeleteNotesOnDepress(); // We know the caller is also manually editing the
                              // AutoParam now too - this counts as an edit, so
                              // we don't want the note deleted on
                              // press-release.
}
...
void InstrumentClipView::reportNoteOffForMPEEditing(ModelStackWithNoteRow* modelStack) {
  ...
  // MPE stuff for note off - if they're still "editing" a note, they'll want
  // the values from half a second ago, or the values from when they pressed
  // hardest.
