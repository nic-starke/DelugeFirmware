...
// e.g. cos we're in the card routine
bool WaveformRenderer::renderFullScreen(Sample*  sample,
                                        uint64_t xScroll,
                                        uint64_t xZoom,
  ...
  // Clear display
  for (int y = 0; y < displayHeight; y++) { memset(thisImage[y], 0, displayWidth * 3); }
...
// Returns false if had trouble loading some (will often not be all) Clusters
bool WaveformRenderer::renderAsSingleRow(Sample*             sample,
                                         int64_t             xScroll,
                                         uint64_t            xZoom,
                                         uint8_t*            thisImage,
                                         WaveformRenderData* data,
  ...
  bool completeSuccess =
      findPeaksPerCol(sample, xScroll, xZoom, data, recorder, xStartSource, xEndSource);
  if (!completeSuccess) return false;
  ...
  for (int xDisplayOutput = xStart; xDisplayOutput < xEnd; xDisplayOutput++) {
    ...
    for (int c = 0; c < 3; c++) {
      ...
      valueHere =
          (valueHere + 6) & ~15; // Limit the heck out of the bit depth, to avoid problem with PIC
                                 // firmware where too many different colour shades cause big
                                 // problems. The 6 is quite arbitrary, but I think it looks good
...
// Value out of 255
int WaveformRenderer::getColBrightnessForSingleRow(int                 xDisplay,
                                                   int32_t             maxPeakFromZero,
                                                   WaveformRenderData* data) {
  ...
  return getMin(peak16 >> 8,
                256); // Max 256 - for now. Looks great and bright.
                      // Must manually limit this, cos if we've ended up with values
                      // higher than our maxPeakFromZero, there'd be trouble otherwise
}

void WaveformRenderer::renderOneColForCollapseAnimation(
    int                 xDisplayWaveform,
    int                 xDisplayOutput,
    int32_t             maxPeakFromZero,
    int                 progress,
    uint8_t             thisImage[][displayWidth + sideBarWidth][3],
  ...
  renderOneColForCollapseAnimationInterpolation(xDisplayOutput,
                                                min24,
                                                max24,
                                                singleSquareBrightness,
                                                progress,
                                                thisImage,
                                                rgb);
}

// For the explode animation. Crams multiple cols of source material into one
// col of output material. Crudely grabs the max values from all cols in range,
// which looks totally fine.
void WaveformRenderer::renderOneColForCollapseAnimationZoomedOut(
    int                 xDisplayWaveformLeftEdge,
    int                 xDisplayWaveformRightEdge,
    int                 xDisplayOutput,
    int32_t             maxPeakFromZero,
    int                 progress,
    uint8_t             thisImage[][displayWidth + sideBarWidth][3],
  ...
  for (int xDisplayDataNow = xDisplayDataLeftEdge; xDisplayDataNow <= xDisplayDataRightEdge;
       xDisplayDataNow++) {
    ...
    int singleSquareBrightness =
        getColBrightnessForSingleRow(xDisplayDataNow, maxPeakFromZero, data);
  ...
  renderOneColForCollapseAnimationInterpolation(xDisplayOutput,
                                                min24Total,
                                                max24Total,
                                                singleSquareBrightnessTotal,
                                                progress,
                                                thisImage,
                                                rgb);
}

// Once we've derived the appropriate data from the waveform for one final col
// of pads, this does the vertical animation according to our current amount of
// expandedness
...
// Returns false if had trouble loading some (will often not be all) Clusters,
// e.g. cos we're in the card routine
bool WaveformRenderer::findPeaksPerCol(Sample*             sample,
                                       int64_t             xScrollSamples,
                                       uint64_t            xZoomSamples,
                                       WaveformRenderData* data,
                                       SampleRecorder*     recorder,
                                       int                 xStart,
                                       int                 xEnd) {
  ...
  for (int col = xStart; col < xEnd; col++) {
    ...
    if (colEndSample >= numValidSamples) {
      ...
      // If we're still recording, we'll just want to come back and render this
      // one when the waveform has grown to cover this whole column
      if (recorder) {
    ...
    int colEndByte =
        colEndSample * sample->numChannels * sample->byteDepth + sample->audioDataStartPosBytes;
    ...
    // Special case to make sure we get initial transient (we know there's more
    // than 1 cluster)
    ...
    // If 3 or more clusters, take 2nd one. TODO: have it take any one which has
    // previously been fully investigated?
      ...
      int unusedBytesAtEndOfPrevCluster =
          (audioFileManager.clusterSize - startByteWithinFirstCluster) %
          (sample->numChannels * sample->byteDepth);
      if (unusedBytesAtEndOfPrevCluster == 0)
        startByteWithinCluster = 0;
      else
        startByteWithinCluster =
            (sample->numChannels * sample->byteDepth) - unusedBytesAtEndOfPrevCluster;
    ...
    if (clusterIndexToDo >= endClusters) { // Could this actually happen?
      ...
      int limit =
          (numValidBytes + sample->audioDataStartPosBytes) & (audioFileManager.clusterSize - 1);

      if (endByteWithinCluster > limit) { endByteWithinCluster = limit; }
    }

    SampleCluster* sampleCluster = sample->clusters.getElement(clusterIndexToDo);

    // If we're wanting to investigate the whole length of one Cluster, and
    // that's already actually been done previously, we can just reuse those
    // findings!
    ...
    // Otherwise, do our normal investigation
      ...
      if (sampleCluster->cluster) {
        ...
        errorCode = "E341"; // Qui got this, around V3.1.3! And Steven G, 3.1.5.
                            // And Brawny, V4.0.1-RC! And then Malte P.
      }

      Cluster* cluster =
          sampleCluster->getCluster(sample, clusterIndexToDo, CLUSTER_LOAD_IMMEDIATELY);
      if (!cluster) {
      cantReadData:
      ...
      if (cluster->numReasonsToBeLoaded <= 0)
        numericDriver.freezeWithError(errorCode); // Branko V got this. Trying to catch E340 below,
                                                  // which Ron R got while recording

      uint32_t numBytesToRead = endByteWithinCluster - startByteWithinCluster;

      // Make the end-byte earlier, so we won't read past the end of the Cluster
      // boundary
      int overshoot = numBytesToRead % (sample->numChannels * sample->byteDepth);
      endByteWithinCluster -= overshoot;

      // However, if that's reduced us to 0 bytes to read, we know we're gonna
      // have to load in the next Cluster to get its sample that's on the
      // boundary
      ...
      if (endByteWithinCluster <= startByteWithinCluster && clusterIndexToDo < endClusters - 1) {
        ...
        if (cluster->numReasonsToBeLoaded <= 0)
          numericDriver.freezeWithError("E342"); // Trying to catch E340 below, which Ron R got
                                                 // while recording
      ...
      // NOTE: from here on, we read *both* channels (if there are two),
      // counting each one as a "sample"

      int numSamplesToRead = numBytesToRead / sample->byteDepth;
      int byteIncrement    = sample->byteDepth;

      // We don't want to read endless samples. If we were gonna read lost, skip
      // some.
      int timesTooManySamples = ((numSamplesToRead - 1) >> SAMPLES_TO_READ_PER_COL_MAGNITUDE) + 1;
      if (timesTooManySamples > 1) {

        // If stereo sample, force an odd number here so we alternate between
      ...
      while (bytePos < endByteWithinCluster) {
        ...
        int32_t individualSampleValue =
            *(int32_t*)&cluster->data[bytePos]; // & sample->bitMask; //
                                                // bitMask hardly matters here
      ...
      if (investigatingAWholeCluster) {
        ...
        // See if we want to include any previously captured maximums and
        // minimums, which might have looked at slightly different values
      ...
      audioFileManager.removeReasonFromCluster(cluster,
                                               "E340"); // Ron R got this, when error was "iiuh"
...
void WaveformRenderer::getColBarPositions(int                 xDisplay,
                                          WaveformRenderData* data,
                                          int32_t*            min24,
                                          int32_t*            max24,
                                          int32_t             valueCentrePoint,
...
void WaveformRenderer::drawColBar(int     xDisplay,
                                  int32_t min24,
                                  int32_t max24,
                                  uint8_t thisImage[][displayWidth + sideBarWidth][3],
                                  int     brightness,
...
void WaveformRenderer::renderOneCol(Sample*             sample,
                                    int                 xDisplay,
                                    uint8_t             thisImage[][displayWidth + sideBarWidth][3],
                                    WaveformRenderData* data,
                                    bool                reversed,
  ...
  if (data->colStatus[xDisplaySource] == COL_STATUS_INVESTIGATED) {
    ...
    getColBarPositions(xDisplaySource,
                       data,
                       &min24,
                       &max24,
                       sample->getFoundValueCentrePoint(),
                       sample->getValueSpan());
