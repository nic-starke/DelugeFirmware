  ...
  if (activeClip) {
    ...
    if (activeInstrumentClip->arpSettings.mode) {
      ...
      uint32_t phaseIncrement =
          activeInstrumentClip->arpSettings.getPhaseIncrement(getFinalParameterValueExp(
              paramNeutralValues[PARAM_GLOBAL_ARP_RATE],
              cableToExpParamShortcut(activeInstrumentClip->arpeggiatorRate)));

      ArpReturnInstruction instruction;

      arpeggiator.render(&activeInstrumentClip->arpSettings,
                         numSamples,
                         gateThreshold,
                         phaseIncrement,
                         &instruction);
      ...
      if (instruction.noteCodeOffPostArp != ARP_NOTE_NONE) {
        noteOffPostArp(instruction.noteCodeOffPostArp,
                       instruction.outputMIDIChannelOff,
                       DEFAULT_LIFT_VALUE); // Is there some better option than using the
                                            // default lift value? The lift event wouldn't
                                            // have occurred yet...
      }
...
void NonAudioInstrument::polyphonicExpressionEventOnChannelOrNote(int newValue,
  ...
  lookAtArpNote:
    ...
    if (arpNote->inputCharacteristics[whichCharacteristic] == channelOrNoteNumber) {
      ...
      // Update the MPE value in the ArpNote. If arpeggiating, it'll get read
      // from there the next time there's a note-on-post-arp. I realise this is
      // potentially frequent writing when it's only going to be read
      // occasionally, but since we're already this far (the Instrument being
      // notified), it's hardly any extra work.
      ...
      if (settings && settings->mode) {
        ...
        // We'll send even if the gate isn't still active. Seems the most
        // sensible. And the release might still be sounding on the connected
        // synth, so this probably makes sense
      }

      // Send this even if arp is on and this note isn't currently sounding: its
      // release might still be
      polyphonicExpressionEventPostArpeggiator(newValue,
...
int32_t NonAudioInstrument::doTickForwardForArp(ModelStack* modelStack, int32_t currentPos) {
  ...
  if (instruction.noteCodeOffPostArp != ARP_NOTE_NONE) {
    noteOffPostArp(instruction.noteCodeOffPostArp,
                   instruction.outputMIDIChannelOff,
                   DEFAULT_LIFT_VALUE); // Is there some better option than using the
                                        // default lift value? The lift event wouldn't have
                                        // occurred yet...
  }
...
// Unlike other Outputs, these don't have ParamManagers backed up at the Song
// level
