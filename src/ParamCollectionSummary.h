...
public:
  ...
  inline void resetInterpolationRecord(int topUintToRepParams) {
    for (int i = topUintToRepParams; i >= 0; i--) { whichParamsAreInterpolating[i] = 0; }
  }

  inline void resetAutomationRecord(int topUintToRepParams) {
    for (int i = topUintToRepParams; i >= 0; i--) { whichParamsAreAutomated[i] = 0; }
  }
  ...
  // The list of these ParamCollectionSummarys, in ParamManager, must be
  // terminated by one whose values are *all* zero. This helps because if we
