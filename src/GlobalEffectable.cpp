...
void GlobalEffectable::initParams(ParamManager* paramManager) {
  ...
  unpatchedParams->params[PARAM_UNPATCHED_GLOBALEFFECTABLE_MOD_FX_RATE]
      .setCurrentValueBasicForSetup(-536870912);
  unpatchedParams->params[PARAM_UNPATCHED_MOD_FX_FEEDBACK].setCurrentValueBasicForSetup(
      -2147483648);
  unpatchedParams->params[PARAM_UNPATCHED_GLOBALEFFECTABLE_MOD_FX_DEPTH]
      .setCurrentValueBasicForSetup(0);
  unpatchedParams->params[PARAM_UNPATCHED_GLOBALEFFECTABLE_DELAY_RATE].setCurrentValueBasicForSetup(
      0);
  unpatchedParams->params[PARAM_UNPATCHED_GLOBALEFFECTABLE_PAN].setCurrentValueBasicForSetup(0);

  unpatchedParams->params[PARAM_UNPATCHED_GLOBALEFFECTABLE_DELAY_AMOUNT]
      .setCurrentValueBasicForSetup(-2147483648);
  unpatchedParams->params[PARAM_UNPATCHED_GLOBALEFFECTABLE_REVERB_SEND_AMOUNT]
      .setCurrentValueBasicForSetup(-2147483648);
  ...
  unpatchedParams->params[PARAM_UNPATCHED_GLOBALEFFECTABLE_SIDECHAIN_VOLUME]
      .setCurrentValueBasicForSetup(-2147483648);
  unpatchedParams->params[PARAM_UNPATCHED_GLOBALEFFECTABLE_PITCH_ADJUST]
      .setCurrentValueBasicForSetup(0);

  unpatchedParams->params[PARAM_UNPATCHED_GLOBALEFFECTABLE_LPF_RES].setCurrentValueBasicForSetup(
      -2147483648);
  unpatchedParams->params[PARAM_UNPATCHED_GLOBALEFFECTABLE_LPF_FREQ].setCurrentValueBasicForSetup(
      2147483647);

  unpatchedParams->params[PARAM_UNPATCHED_GLOBALEFFECTABLE_HPF_RES].setCurrentValueBasicForSetup(
      -2147483648);
  unpatchedParams->params[PARAM_UNPATCHED_GLOBALEFFECTABLE_HPF_FREQ].setCurrentValueBasicForSetup(
      -2147483648);
}
...
void GlobalEffectable::initParamsForAudioClip(ParamManagerForTimeline* paramManager) {
  ...
  paramManager->getUnpatchedParamSet()
      ->params[PARAM_UNPATCHED_GLOBALEFFECTABLE_VOLUME]
      .setCurrentValueBasicForSetup(-536870912);
}
...
GlobalEffectable::getParamFromModEncoder(int                            whichModEncoder,
                                         ModelStackWithThreeMainThings* modelStack,
                                         bool                           allowCreation) {
...
void GlobalEffectable::ensureModFXParamIsValid() {
  while (true) {
    if (currentModFXParam == MOD_FX_PARAM_DEPTH) {
      if (modFXType == MOD_FX_TYPE_FLANGER) { goto ohNo; }
    } else if (currentModFXParam == MOD_FX_PARAM_OFFSET) {
      if (modFXType != MOD_FX_TYPE_CHORUS) { goto ohNo; }
...
void GlobalEffectable::setupFilterSetConfig(FilterSetConfig* filterSetConfig,
                                            int32_t*         postFXVolume,
                                            ParamManager*    paramManager) {

  UnpatchedParamSet* unpatchedParams = paramManager->getUnpatchedParamSet();

  int lpfFrequency = getFinalParameterValueExp(paramNeutralValues[PARAM_LOCAL_LPF_FREQ],
                                               cableToExpParamShortcut(unpatchedParams->getValue(
                                                   PARAM_UNPATCHED_GLOBALEFFECTABLE_LPF_FREQ)));
  int lpfResonance =
      getFinalParameterValueLinear(paramNeutralValues[PARAM_LOCAL_LPF_RESONANCE],
                                   cableToLinearParamShortcut(unpatchedParams->getValue(
                                       PARAM_UNPATCHED_GLOBALEFFECTABLE_LPF_RES)));

  int hpfFrequency = getFinalParameterValueExp(paramNeutralValues[PARAM_LOCAL_HPF_FREQ],
                                               cableToExpParamShortcut(unpatchedParams->getValue(
                                                   PARAM_UNPATCHED_GLOBALEFFECTABLE_HPF_FREQ)));
  int hpfResonance =
      getFinalParameterValueLinear(paramNeutralValues[PARAM_LOCAL_HPF_RESONANCE],
                                   cableToLinearParamShortcut(unpatchedParams->getValue(
  ...
  filterSetConfig->doHPF =
      unpatchedParams->getValue(PARAM_UNPATCHED_GLOBALEFFECTABLE_HPF_FREQ) != -2147483648;

  *postFXVolume = filterSetConfig->init(lpfFrequency,
                                        lpfResonance,
                                        hpfFrequency,
                                        hpfResonance,
                                        lpfMode,
...
void GlobalEffectable::processFilters(StereoSample*    buffer,
                                      int              numSamples,
  ...
  if (filterSetConfig->doLPF) {
    filterSets[0]
        .renderLPFLong(&buffer->l, &(buffer + numSamples)->l, filterSetConfig, lpfMode, 2, 2, 1);
    filterSets[1]
        .renderLPFLong(&buffer->r, &(buffer + numSamples)->r, filterSetConfig, lpfMode, 2, 2, 1);
  }
}
...
void GlobalEffectable::writeParamAttributesToFile(ParamManager* paramManager,
                                                  bool          writeAutomation,
                                                  int32_t*      valuesForOverride) {

  UnpatchedParamSet* unpatchedParams = paramManager->getUnpatchedParamSet();

  unpatchedParams->writeParamAsAttribute("reverbAmount",
                                         PARAM_UNPATCHED_GLOBALEFFECTABLE_REVERB_SEND_AMOUNT,
                                         writeAutomation,
                                         false,
                                         valuesForOverride);
  unpatchedParams->writeParamAsAttribute("volume",
                                         PARAM_UNPATCHED_GLOBALEFFECTABLE_VOLUME,
                                         writeAutomation,
                                         false,
                                         valuesForOverride);
  unpatchedParams->writeParamAsAttribute("pan",
  ...
  if (unpatchedParams->params[PARAM_UNPATCHED_GLOBALEFFECTABLE_PITCH_ADJUST].containsSomething(0))
    unpatchedParams->writeParamAsAttribute("pitchAdjust",
                                           PARAM_UNPATCHED_GLOBALEFFECTABLE_PITCH_ADJUST,
                                           writeAutomation,
                                           false,
                                           valuesForOverride);

  if (unpatchedParams->params[PARAM_UNPATCHED_GLOBALEFFECTABLE_SIDECHAIN_VOLUME].containsSomething(
          -2147483648))
  ...
  unpatchedParams->writeParamAsAttribute("sidechainCompressorShape",
                                         PARAM_UNPATCHED_COMPRESSOR_SHAPE,
                                         writeAutomation,
                                         false,
                                         valuesForOverride);

  unpatchedParams->writeParamAsAttribute("modFXDepth",
                                         PARAM_UNPATCHED_GLOBALEFFECTABLE_MOD_FX_DEPTH,
                                         writeAutomation,
                                         false,
                                         valuesForOverride);
  unpatchedParams->writeParamAsAttribute("modFXRate",
                                         PARAM_UNPATCHED_GLOBALEFFECTABLE_MOD_FX_RATE,
                                         writeAutomation,
                                         false,
                                         valuesForOverride);

  ModControllableAudio::writeParamAttributesToFile(paramManager,
                                                   writeAutomation,
                                                   valuesForOverride);
}

void GlobalEffectable::writeParamTagsToFile(ParamManager* paramManager,
                                            bool          writeAutomation,
                                            int32_t*      valuesForOverride) {
  ...
  unpatchedParams->writeParamAsAttribute("rate",
                                         PARAM_UNPATCHED_GLOBALEFFECTABLE_DELAY_RATE,
                                         writeAutomation,
                                         false,
                                         valuesForOverride);
  unpatchedParams->writeParamAsAttribute("feedback",
  ...
  unpatchedParams->writeParamAsAttribute("frequency",
                                         PARAM_UNPATCHED_GLOBALEFFECTABLE_LPF_FREQ,
                                         writeAutomation,
                                         false,
                                         valuesForOverride);
  unpatchedParams->writeParamAsAttribute("resonance",
                                         PARAM_UNPATCHED_GLOBALEFFECTABLE_LPF_RES,
                                         writeAutomation,
                                         false,
                                         valuesForOverride);
...
void GlobalEffectable::readParamsFromFile(ParamManagerForTimeline* paramManager,
...
bool GlobalEffectable::readParamTagFromFile(char const*              tagName,
                                            ParamManagerForTimeline* paramManager,
                                            int32_t                  readAutomationUpToPos) {
...
int GlobalEffectable::readTagFromFile(char const*              tagName,
                                      ParamManagerForTimeline* paramManager,
                                      int32_t                  readAutomationUpToPos,
                                      Song*                    song) {

  // This is here for compatibility only for people (Lou and Ian) who saved
  // songs with firmware in September 2016
  // if (paramManager && strcmp(tagName, "delay") &&
  // GlobalEffectable::readParamTagFromFile(tagName, paramManager,
  // readAutomation)) {}
...
// Before calling this, check that
// (storageManager.firmwareVersionOfFileBeingRead < FIRMWARE_1P2P0 &&
...
void GlobalEffectable::compensateVolumeForResonance(ParamManagerForTimeline* paramManager) {
  ...
  if (!unpatchedParams->params[PARAM_UNPATCHED_GLOBALEFFECTABLE_LPF_FREQ].isAutomated() &&
      unpatchedParams->params[PARAM_UNPATCHED_GLOBALEFFECTABLE_LPF_FREQ].getCurrentValue() >=
          2147483602 &&
  ...
  if (!unpatchedParams->params[PARAM_UNPATCHED_GLOBALEFFECTABLE_HPF_FREQ].containsSomething(
          -2147483648) &&
      !unpatchedParams->params[PARAM_UNPATCHED_GLOBALEFFECTABLE_LPF_RES].containsSomething(
          -1073741824)) {
...
void GlobalEffectable::setupDelayWorkingState(DelayWorkingState* delayWorkingState,
                                              ParamManager*      paramManager,
  ...
  delayWorkingState->delayFeedbackAmount =
      getFinalParameterValueLinear(paramNeutralValues[PARAM_GLOBAL_DELAY_FEEDBACK],
                                   cableToLinearParamShortcut(unpatchedParams->getValue(
                                       PARAM_UNPATCHED_GLOBALEFFECTABLE_DELAY_AMOUNT)));
...
void GlobalEffectable::processFXForGlobalEffectable(StereoSample*      inputBuffer,
                                                    int                numSamples,
                                                    int32_t*           postFXVolume,
                                                    ParamManager*      paramManager,
  ...
  int32_t modFXRate = getFinalParameterValueExp(paramNeutralValues[PARAM_GLOBAL_MOD_FX_RATE],
                                                cableToExpParamShortcut(unpatchedParams->getValue(
                                                    PARAM_UNPATCHED_GLOBALEFFECTABLE_MOD_FX_RATE)));
  int32_t modFXDepth =
      getFinalParameterValueVolume(paramNeutralValues[PARAM_GLOBAL_MOD_FX_DEPTH],
                                   cableToLinearParamShortcut(unpatchedParams->getValue(
                                       PARAM_UNPATCHED_GLOBALEFFECTABLE_MOD_FX_DEPTH)));

  int modFXTypeNow = getActiveModFXType(paramManager);

  // For GlobalEffectables, mod FX buffer memory is allocated here in the
  // rendering routine - this might seem strange, but it's because unlike for
  // Sounds, the effect can be switched on and off by changing a parameter like
  // "depth".
...
int GlobalEffectable::stringToParam(char const* string) {
  for (int p = PARAM_UNPATCHED_SECTION + NUM_SHARED_UNPATCHED_PARAMS;
       p < PARAM_UNPATCHED_SECTION + MAX_NUM_UNPATCHED_PARAMS;
       p++) {
