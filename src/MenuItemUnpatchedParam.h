...
class MenuItemUnpatchedParam : public MenuItemParam,
                               public MenuItemIntegerContinuous,
                               public MenuItemWithCCLearning {
public:
  MenuItemUnpatchedParam();
  MenuItemUnpatchedParam(char const* newName, int newP)
      : MenuItemParam(newP), MenuItemIntegerContinuous(newName) {}

  void            readCurrentValue();
  void            writeCurrentValue();
  ParamDescriptor getLearningThing() final;
  int             getMaxValue() {
    return MenuItemParam::getMaxValue();
  }
  int getMinValue() {
    return MenuItemParam::getMinValue();
  }
...
public:
  MenuItemUnpatchedParamPan(char const* newName = 0, int newP = 0)
