...
public:
  ...
  int         openXMLFile(FilePointer* filePointer,
  ...
  int         initSD();
  bool        closeFile();
  int         closeFileAfterWriting(char const* path            = NULL,
  ...
  int         loadInstrumentFromFile(Song*           song,
                                     InstrumentClip* clip,
                                     int             instrumentType,
  ...
  int         readMIDIParamFromFile(int32_t              readAutomationUpToPos,
  ...
  void writeAbsoluteSyncLevelToFile(Song*       song,
                                    char const* name,
                                    int         internalValue,
                                    bool        onNewLine = true);
...
private:
  ...
  uint8_t xmlArea;
  bool    xmlReachedEnd;
  int     tagDepthCaller; // How deeply indented in XML the main Deluge classes
                          // think we are, as data being read.
  int tagDepthFile;       // Will temporarily be different to the above as unwanted /
                          // unused XML tags parsed on the way to finding next useful
                          // data.
  int xmlReadCount;
