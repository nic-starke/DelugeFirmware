...
#if SAMPLE_DO_LOCKS
#define LOCK_ENTRY                                     \
  if (lock) { numericDriver.freezeWithError("i024"); } \
  lock = true;
...
struct SampleCacheElement {
  ...
  uint32_t reversed; // Bool would be fine, but got to make it 32-bit for
...
Sample::Sample()
    : percCacheZones{sizeof(SamplePercCacheZone), sizeof(SamplePercCacheZone)},
      caches(sizeof(SampleCacheElement), 4), AudioFile(AUDIO_FILE_TYPE_SAMPLE) {
...
Sample::~Sample() {
  for (int c = 0; c < clusters.getNumElements(); c++) { clusters.getElement(c)->~SampleCluster(); }
    ...
    if (percCacheClusters[reversed]) {
      ...
      for (int c = 0; c < numPercCacheClusters; c++) {
        if (percCacheClusters[reversed][c]) {
          ...
          // Don't bother actually setting our pointer to NULL, cos we're about
          // to deallocate that memory anyway
        }
      }
...
SampleCache* Sample::getOrCreateCache(SampleHolder* sampleHolder,
  ...
  uint64_t lengthInSamplesCached =
      ((uint64_t)(lengthInSamples - skipSamplesAtStart) << 24) / combinedIncrement +
      1; // Not 100% sure on the +1, but better safe than sorry

  // Make it a bit longer, to capture the ring-out of the interpolation /
  // time-stretching
  ...
  if (lengthInBytesCached >= (32 << 20))
    return NULL; // If cache would be more than 32MB, assume that it wouldn't be
                 // very useful to cache it

  int   numClusters = ((lengthInBytesCached - 1) >> audioFileManager.clusterSizeMagnitude) + 1;
  void* memory =
      generalMemoryAllocator.alloc(sizeof(SampleCache) + (numClusters - 1) * sizeof(Cluster*),
                                   NULL,
                                   false,
                                   false);
  ...
  SampleCache* samplePitchAdjustment = new (memory) SampleCache(this,
                                                                numClusters,
                                                                lengthInBytesCached,
                                                                phaseIncrement,
                                                                timeStretchRatio,
...
void Sample::deleteCache(SampleCache* cache) {
  ...
  /*
  int i = pitchAdjustmentCaches.search(cache->phaseIncrement,
  cache->skipSamplesAtStart, cache->reversed, GREATER_OR_EQUAL); if (i <
  pitchAdjustmentCaches.getNumElements()) { SampleCacheElement* element =
...
int Sample::fillPercCache(TimeStretcher* timeStretcher,
  ...
  // int lengthInSamplesAfterReduction = ((lengthInSamples +
  // (PERC_BUFFER_REDUCTION_SIZE >> 1)) >> PERC_BUFFER_REDUCTION_MAGNITUDE);
  int lengthInSamplesAfterReduction =
      ((lengthInSamples - 1) >> PERC_BUFFER_REDUCTION_MAGNITUDE) + 1;
  lengthInSamplesAfterReduction = getMax(lengthInSamplesAfterReduction,
                                         1); // Can't allocate less than 1 byte

  bool percCacheDoneWithClusters =
      (lengthInSamplesAfterReduction >= (audioFileManager.clusterSize >> 1));

  if (percCacheDoneWithClusters) {
    if (!percCacheClusters[reversed]) {
      numPercCacheClusters =
          ((lengthInSamplesAfterReduction - 1) >> audioFileManager.clusterSizeMagnitude) +
          1; // Stores this number for the future too
      int memorySize = numPercCacheClusters * sizeof(Cluster*);
      percCacheClusters[reversed] =
          (Cluster**)generalMemoryAllocator.alloc(memorySize, NULL, false, true);
      if (!percCacheClusters[reversed]) {
        LOCK_EXIT
  ...
  if (i >= 0 && i < percCacheZones[reversed].getNumElements()) {
    ...
    // Primarily, we check here whether this zone ends after our start-pos.
    // However, we also test positive if the zone's end is *almost* as far along
    // as our start-pos but not quite. In such a case, it still makes sense to
    // continue adding to that zone, starting a little further back than we had
    // planned to. This prevents the situation where time-stretching is on
    // extremely fast and each call to this function is so much further along
    // that a new zone is created every time, leading to thousands of zones, so
    // huge overhead each time we want to insert or delete. Instead, this new
    // method will cause the zones to clump together, or better yet just manage
    // to cover the whole area in one zone. This is far more efficient in every
    // way - remember that each zone will have a number of
    // samplesAtStartWhichShouldBeReplaced, so ending up with thousands of zones
    // is just a terrible idea.
    if ((percCacheZone->endPos - startPosSamples) * playDirection >=
        -2048) { // -2048 helps massively. Not sure if we can go lower. Also
                 // tested -4096 - same result. Not fine-tuned beyond that

      // Reset startPosSamples back to the zone endPos, which may have been a
      // bit further back. That's the place where we're guaranteed that there's
      // still a perc cache Cluster (I think? Or unless it's the first sample of
      // a new one?)
      startPosSamples = percCacheZone->endPos; // This can end up as -1! Because
                                               // endPos can - see its comment.

      // If the (potentially made-later) start pos is already beyond the
      // waveform, get out (otherwise we'd be prone to an error getting the perc
      // Cluster below. Fixed Aug 2021
      if (!reversed) {
        ...
        if (startPosSamples < 0) { goto doReturnNoError; }
      }

      // First, update our "current pos for perc cache filling and reading"
      // sorta thing so no one steals the first Cluster we're gonna need. This
      // is especially important for just now while we're gonna be reading some
      // of this Cluster, but also we want to keep it in memory for next time we
      // come back here.
      int percClusterIndexStart;
      if (percCacheDoneWithClusters) {
        percClusterIndexStart =
            (uint32_t)startPosSamples >>
        ...
        if (!clusterHere) {
          ...
          // That's actually allowed if we're right at the start of that
          // cluster. But otherwise...
          if (startPosSamples &
              ((1 << audioFileManager.clusterSizeMagnitude + PERC_BUFFER_REDUCTION_MAGNITUDE) -
               1)) {
            Uart::println(startPosSamples);
            numericDriver.freezeWithError("E139"); // If Cluster has been stolen, the zones should
                                                   // have been updated, so we shouldn't be here
          }
        }
#endif
        if (clusterHere)
          timeStretcher->rememberPercCacheCluster(
              clusterHere); // If at start of new cluster, there might not be
                            // one allocated here yet
      }

      // If it ends after our end-pos too, we're done
      if ((percCacheZone->endPos - endPosSamples) * playDirection >= 0) {

        // But first, if our perc cache is done with Clusters, see if our endPos
        // has a different perc cache Cluster than our startPos, and if so,
        // store it. (It won't be more than 1 Cluster ahead, cos remember the
        // data is so compacted that each perc cache Cluster stores like 90
        // seconds.)
        if (percCacheDoneWithClusters) {

          // I think the fact that we subtract playDirection here means that we
          // look at the cluster for the very last existing sample, so even if
          // we've actually filled up right up to the cluster boundary but not
          // allocated a next one, it should be fine, ya know?
          ...
          if (percClusterIndexEnd != percClusterIndexStart) {
            ...
            if (!percCacheClusters[reversed][percClusterIndexEnd])
              numericDriver.freezeWithError("E141"); // If Cluster has been stolen, the zones should
                                                     // have been updated, so we shouldn't be here
#endif
            timeStretcher->rememberPercCacheCluster(
                percCacheClusters[reversed][percClusterIndexEnd]);
          }
        }

        // We're now guaranteed to have a bunch of perc cache secured in RAM,
        // un-stealable. So we can take a breather and know we won't need access
        // to the source Clusters for it anytime very soon
  ...
  // If still here, need to create element. And we know that perc cache Clusters
  // will be allocated and remembered if necessary
  if (!reversed) i++;

  error = percCacheZones[reversed].insertAtIndex(
      i,
      1,
      this); // Tell it not to steal other perc cache zones from this
             // Sample, which would result in modification of the same
             // array during operation. Fortunately it also has a lock to
             // alert if that actually somehow happened, too.
  if (error) {
    LOCK_EXIT
    return error;
  }
  ...
  percCacheZone =
      new (percCacheZones[reversed].getElementAddress(i)) SamplePercCacheZone(startPosSamples);
...
doLoading:
  ...
  if (iNext >= 0 && iNext < percCacheZones[reversed].getNumElements()) {
    ...
    if ((endPosSamples - nextPercCacheZone->startPos) * playDirection >= 0) {
      ...
      endPosSamplesLimit = nextPercCacheZone->startPos +
                           nextPercCacheZone->samplesAtStartWhichShouldBeReplaced * playDirection;

      if ((endPosSamples - endPosSamplesLimit) * playDirection >= 0) {
        // TODO: what if that next zone doesn't extend all the way to the end we
        // want?
  ...
  if (numSamples <= 0)
    goto getOut; // This probably would have already been dealt with above - not
                 // quite sure
  ...
  do {
    ...
    if (sourceClusterIndex >= getFirstClusterIndexWithNoAudioData() ||
        sourceClusterIndex < getFirstClusterIndexWithAudioData()) { // Wait, this shouldn't
    ...
    if (percCacheDoneWithClusters) {
      int percClusterIndex = startPosSamples >> (audioFileManager.clusterSizeMagnitude +
      ...
      if (!percCacheClusters[reversed][percClusterIndex]) {
        // Uart::println("allocating perc cache
        // Cluster!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
        //  We tell it not to steal any other per cache Cluster from this Sample
        //  - not because those Clusters are definitely a high priority to keep,
        //  but because doing so would probably alter our percCacheZones, which
        //  we're currently working with, which could really muck things up.
        //  Scenario only discovered Jan 2021.
        percCacheClusters[reversed][percClusterIndex] = audioFileManager.allocateCluster(
            reversed ? CLUSTER_PERC_CACHE_REVERSED : CLUSTER_PERC_CACHE_FORWARDS,
            false,
            this); // Doesn't add reason. Call to
                   // rememberPercCacheCluster() below will
        if (!percCacheClusters[reversed][percClusterIndex]) {
      ...
      int samplesLeftThisDestCluster =
          reversed ? (posWithinPercClusterBig + 1) :
                     ((audioFileManager.clusterSize << PERC_BUFFER_REDUCTION_MAGNITUDE) -
                      posWithinPercClusterBig);
      numSamplesThisClusterReadWrite =
          getMin(numSamplesThisClusterReadWrite, samplesLeftThisDestCluster);
    }
    ...
    Cluster* cluster = clusters.getElement(sourceClusterIndex)
                           ->cluster; // Don't call getcluster() - that would add a reason, and
                                      // potentially do loading and stuff.
    ...
    // Do some stuff here kind of ahead of time, before we go and decrement
    // numSamplesThisClusterReadWrite
    numSamples -= numSamplesThisClusterReadWrite;
    percCacheZone->endPos += numSamplesThisClusterReadWrite *
                             playDirection; // Do this now, in case the next Cluster fails
    ...
    do {
      ...
      int numSamplesLeftThisPercPixelSegment =
          reversed ?
              (startPosSamples + 1 + (PERC_BUFFER_REDUCTION_SIZE >> 1)) &
                  (PERC_BUFFER_REDUCTION_SIZE - 1) :
              PERC_BUFFER_REDUCTION_SIZE - ((startPosSamples + (PERC_BUFFER_REDUCTION_SIZE >> 1)) &
                                            (PERC_BUFFER_REDUCTION_SIZE - 1));
      ...
      numSamplesThisPercPixelSegment =
          getMin(numSamplesThisPercPixelSegment, numSamplesLeftThisPercPixelSegment);

      char* endPos = currentPos + numSamplesThisPercPixelSegment * posIncrement;

      int32_t angle;

      while (true) { // I've put reasonable effort into benchmarking / optimizing
                     // this loop - I don't think it can be improved much more
        int32_t thisSampleRead = *(int32_t*)currentPos >> 2; // Have to make it smaller even if only
                                                             // one, so the "angle" doesn't overflow
        if (numChannels == 2) { thisSampleRead += *(int32_t*)(currentPos + byteDepth) >> 2; }
        ...
        for (int p = 0; p < DIFFERENCE_LPF_POLES; p++) {
          ...
          percCacheZone->angleLPFMem[p] +=
              distanceToGo >> 9; // multiply_32x32_rshift32_rounded(distanceToGo,
                                 // 1 << 23); //distanceToGo >> 9;
  ...
  percCacheZone->samplesAtStartWhichShouldBeReplaced =
      getMax(2048,
             (percCacheZone->endPos - percCacheZone->startPos) *
                 playDirection); // 2048 is fairly arbitrary
  ...
  // Though that'd only very rarely happen, and only hold us up very briefly
...
getOut:
  // If we failed to do the loading we wanted to, e.g. because of insufficient
  // RAM, we need to make sure we didn't leave a 0-length zone, cos that's
  // invalid.
  if (percCacheZone->endPos == percCacheZone->startPos) {
    percCacheZones[reversed].deleteAtIndex(i);
  }

  // Unlock now that we've finished dealing with the percCacheZones array. If
  // the below call to updateClustersForPercLookahead() wants to steal any perc
  // cache Clusters and consequently modify that array, it's allowed to.
  LOCK_EXIT
...
bool Sample::getAveragesForCrossfade(int32_t* totals,
  ...
  // This can happen. Not 100% sure if it should, but we'll return false just
  // below in this case anyway, so I think it's ok
  ...
  if (playDirection == 1)
    if (readByte < (int32_t)audioDataStartPosBytes + halfCrossfadeLengthBytes)
      return false;
             ...
             (int32_t)(audioDataStartPosBytes + audioDataLengthBytes - halfCrossfadeLengthBytes))
      return false;

  int endReadByte = readByte + lengthToAverageEach * TIME_STRETCH_CROSSFADE_NUM_MOVING_AVERAGES *
                                   bytesPerSample * playDirection;
  ...
  for (int i = 0; i < TIME_STRETCH_CROSSFADE_NUM_MOVING_AVERAGES; i++) {
    ...
    if (ALPHA_OR_BETA_VERSION && (readByte < audioDataStartPosBytes - 1 ||
    ...
    do {
      ...
      if (ALPHA_OR_BETA_VERSION && (whichCluster < getFirstClusterIndexWithAudioData() ||
      ...
      do {
        ...
        if (numChannelsNow == 2) { totals[i] += *(int32_t*)(currentPos + byteDepthNow) >> 16; }
...
uint8_t* Sample::prepareToReadPercCache(int  pixellatedPos,
  ...
  int realPos =
      (pixellatedPos << PERC_BUFFER_REDUCTION_MAGNITUDE) + (PERC_BUFFER_REDUCTION_SIZE >> 1);
  int i =
      percCacheZones[reversed].search(realPos + 1 - reversed, reversed ? GREATER_OR_EQUAL : LESS);
  ...
  *earliestPixellatedPos = (zone->startPos + (PERC_BUFFER_REDUCTION_SIZE >> 1) * playDirection) >>
                           PERC_BUFFER_REDUCTION_MAGNITUDE;
  *latestPixellatedPos = (zone->endPos - (PERC_BUFFER_REDUCTION_SIZE >> 1) * playDirection) >>
                         PERC_BUFFER_REDUCTION_MAGNITUDE;
  ...
  // Or if Cluster-based perc cache...
    ...
    // Constrain to Cluster boundaries. This will theoretically hurt the sound a
    // tiny bit... once every 90 seconds. No one will ever know
    ...
    return (uint8_t*)percCacheClusters[reversed][ourCluster]->data -
           (ourCluster * audioFileManager.clusterSize);
  }
}
...
void Sample::percCacheClusterStolen(Cluster* cluster) {
  ...
  // TODO: while inside this, don't allow further editing to
  // percCacheZones[reversed]

  int leftBorder = cluster->clusterIndex
                   << (audioFileManager.clusterSizeMagnitude + PERC_BUFFER_REDUCTION_MAGNITUDE);
  ...
  if (iEarlier >= 0 && iEarlier < percCacheZones[reversed].getNumElements()) {
    SamplePercCacheZone* zoneEarlier =
        (SamplePercCacheZone*)percCacheZones[reversed].getElementAddress(iEarlier);
    ...
    if ((zoneEarlier->endPos - earlierBorder) * playDirection > 0) {
      ...
      if ((zoneEarlier->endPos - laterBorder) * playDirection > 0) {
        ...
        int oldSamplesAtStartWhichShouldBeReplaced =
            zoneEarlier->samplesAtStartWhichShouldBeReplaced;

        zoneEarlier->startPos                            = laterBorder;
        zoneEarlier->samplesAtStartWhichShouldBeReplaced = 0;

        int iNew = reversed ? (iEarlier + 1) : iEarlier;
        // This is reasonably likely to fail, cos it might want to allocate new
        // memory, but that's not allowed if it's currently allocating a
        // Cluster, which it will be if this Cluster got stolen, which is why
        // we're here. Oh well
        int error = percCacheZones[reversed].insertAtIndex(
            iNew,
            1,
            this); // Also specify not to steal perc cache Clusters
                   // from this Sample. Could that actually even happen
                   // given the above comment? Not sure.
        if (error) {
  ...
  if ((iLater - iEarlier) * playDirection > 0) {
    ...
    SamplePercCacheZone* zoneLater =
        (SamplePercCacheZone*)percCacheZones[reversed].getElementAddress(iLater);
...
int Sample::getFirstClusterIndexWithNoAudioData() {
  uint32_t clusterIndex = ((audioDataStartPosBytes + audioDataLengthBytes - 1) >>
                           audioFileManager.clusterSizeMagnitude) +
                          1; // Rounds up
...
float getPeakIndexFloat(int i, int32_t peakValue, int32_t prevValue, int32_t nextValue) {
  ...
  int32_t totalDistance =
      peakValue - lowerValue; // Distance from lower neighbouring height to peak height
...
// Returns strength
int Sample::investigateFundamentalPitch(int       fundamentalIndexProvided,
                                        int       tableSize,
                                        int32_t*  heightTable,
                                        uint64_t* sumTable,
                                        float*    floatIndexTable,
                                        float*    getFundamentalIndex,
                                        int       numDoublings,
                                        bool      doPrimeTest) {
  ...
  while (true) {

    {
      ...
      float searchCentre = fundamentalIndexForContinuedHarmonicInvestigation * h +
                           0.5; // Will round when converted to int
  ...
  examineHarmonic:
    ...
    int32_t strengthThisHarmonic =
        ((uint64_t)heightRelativeToSurroundings * (uint64_t)heightRightHere) >> 20;
    ...
    if (h == 1) {
      ...
      float distanceToGo =
          newEstimatedFundamentalIndex - fundamentalIndexForContinuedHarmonicInvestigation;
      ...
      fundamentalIndexForContinuedHarmonicInvestigation +=
          distanceToGo * heightRelativeToSurroundingsFloat;
    ...
    // After working far enough into the table, we want to stop adjusting the
    // pitch we're going to output, because the higher harmonics tend to be a
    // bit sharp, at least initially, on a lot of acoustic instruments.
    ...
    float fundamentalPeriod =
        (float)PITCH_DETECT_WINDOW_SIZE / fundamentalIndexForContinuedHarmonicInvestigation;
  ...
  *getFundamentalIndex = fundamentalIndexToReturn;
  ...
  if (true || doPrimeTest) {
    for (int p = 0; p < NUM_PRIMES; p++) {
      ...
      // if (primeTotals[p] >= (total - primeTotals[p]) / (thisPrime - 1) *
      // threshold) return 0;
  ...
  // 1/2 second delay: 0.65 is max for vibraphone. Never quite get all piano
  // working
...
#define MIN_ACCURATE_FREQUENCY \
  (1638400 >> (PITCH_DETECT_WINDOW_SIZE_MAGNITUDE)) // In Hz I think? Could even go +2 here
                                                    // and even a 54Hz sound is ok
#define MAX_LENGTH_DOUBLINGS (16 - PITCH_DETECT_WINDOW_SIZE_MAGNITUDE)

// We want a fairly small window. Any bigger, and it'll fail to find the tones
// in short, percussive yet tonal sounds. Or if we were to go much smaller than
// this, we might incorrectly see low frequencies. Already, this is too small to
// very accurately pick up low frequencies, so when one is detected, a second
// pass is done on downsampled (squished in) audio data, to pick it up more
// accurately
...
float Sample::determinePitch(bool  doingSingleCycle,
  ...
  // Allocate space for both the real and imaginary number buffers - the
  // imaginary one is tacked on the end
  ...
  int32_t* fftInput =
      (int32_t*)generalMemoryAllocator.alloc(fftInputSize + fftOutputSize + floatIndexTableSize,
                                             NULL,
                                             false,
                                             true);
  if (!fftInput) { return 0; }
  ...
  if (sampleRate >= 88200) { defaultLengthDoublings++; }
  ...
  while (maxFreqHere < MIN_ACCURATE_FREQUENCY) {
    ...
    if (lengthDoublings >= 10)
      return 0; // Keep things sane / from overflowing, which I saw happen when
                // lengthDoublings got to 15. Happened when another error led to
                // maxFreq being insanely low like almost 0
    maxFreqHere *= 2;
  }
...
startAgain:
  ...
  Cluster* cluster = clusters.getElement(currentClusterIndex)
                         ->getCluster(this, currentClusterIndex, CLUSTER_LOAD_IMMEDIATELY);
  if (!cluster) {
  ...
  // If stereo sample, we want to blend left and right together, and the easiest
  // way is to use our existing "averaging" system
  ...
  continueWhileLoop:
    ...
    // We may want to average several samples into just one - crudely
    // downsampling, but the aliasing shouldn't hurt us
    for (int i = 0; i < (1 << lengthDoublingsNow); i++) {
      ...
      int32_t individualSampleValue =
          *(int32_t*)&cluster
               ->data[(currentOffset & (audioFileManager.clusterSize - 1)) - 4 + byteDepth] &
          bitMask;
      ...
      if (currentOffset >= audioDataLengthBytes + audioDataStartPosBytes) { goto doneReading; }
      ...
      // Rudimentary audio start-detection. We need this, because detecting the
      // tone of percussive sounds relies on having our window at just the
      // moment when they hit
      if (!beginningOffsetForPitchDetectionFound) {
        int32_t absoluteValue =
            (individualSampleValue < 0) ? -individualSampleValue : individualSampleValue;
        ...
        // If our grabbed window would end beyond the end of the audio file,
        // shift it left
        beginningOffsetForPitchDetection = getMin(
            beginningOffsetForPitchDetection,
            (int32_t)(audioDataStartPosBytes + audioDataLengthBytes -
                      (PITCH_DETECT_WINDOW_SIZE << lengthDoublings) * numChannels * byteDepth));

        // TODO: it's not quite perfect doing that and storing the result,
        // because lengthDoublings will sometimes be different

        // And now make sure that hasn't pushed it further back left than where
        // we are right now
    ...
    // Do hanning window
    int32_t hanningValue =
        interpolateTableSigned(writeIndex, PITCH_DETECT_WINDOW_SIZE_MAGNITUDE, hanningWindow, 8);
...
doneReading:
  ...
  if (!beginningOffsetForPitchDetectionFound) {
    ...
    // If we haven't done so yet, see if we can just go again, with a reduced
    // threshold derived from the actual volume of the sound
  ...
  */
  ...
  // Go through complex-number FFT result, converting to positive
  // (pythagorassed) heights
  ...
  // Go through again doing the running sum, interpolating exact peak
  // frequencies, and deleting everything that's not a peak
  for (int i = 0; i < (PITCH_DETECT_WINDOW_SIZE >> 1); i++) {
    ...
    // Don't bother with anything under the threshold - mostly just for
    // efficiency, since the threshold is very low and won't cause much
    // real-world difference. Don't do it below a certain freq though - we
    // absolutely need even the tiniest peaks down in the 30hz kind of range
    // (see Leo's pianos)
    ...
    if (i >= 1 && shouldWriteZeroBack) { fftHeights[i - 1] = 0; }
  ...
  int   minFundamentalPeakIndex =
      (float)PITCH_DETECT_WINDOW_SIZE / minFundamentalPeriod; // Rounds down

  int   maxFreqAdjusted      = maxFreqHz * (1 << lengthDoublings);
  float maxFundamentalPeriod = (float)sampleRate / maxFreqAdjusted;
  int   maxFundamentalPeakIndex =
      (float)PITCH_DETECT_WINDOW_SIZE / maxFundamentalPeriod + 1; // Rounds up
  ...
  for (int i = minFundamentalPeakIndex; i < maxFundamentalPeakIndex; i++) {
    ...
    if (!(peakCount & 7))
      AudioEngine::routineWithClusterLoading(); // --------------------------------------
                                                // // 15 works. 7 is extra safe
    peakCount++;

    float fundamentalIndexHere;
    int   strengthHere = investigateFundamentalPitch(i,
                                                   (PITCH_DETECT_WINDOW_SIZE >> 1),
                                                   fftHeights,
                                                   (uint64_t*)fftOutput,
                                                   floatIndexTable,
                                                   &fundamentalIndexHere,
                                                   lengthDoublings,
                                                   doPrimeTest);
  ...
  // If frequency too low, go again, taking a longer length into account, for
  // better accuracy
  if (freqBeforeAdjustment < MIN_ACCURATE_FREQUENCY &&
    ...
    Uart::printlnfloat(freq);
#endif
    // Only do one doubling at a time - this can help to correct an incorrect
    // reading
...
void Sample::convertDataOnAnyClustersIfNecessary() {
  if (rawDataFormat) {
    for (int c = getFirstClusterIndexWithAudioData(); c < getFirstClusterIndexWithNoAudioData();
         c++) {
      Cluster* cluster = clusters.getElement(c)->cluster;
      if (cluster) {
...
int32_t Sample::getMaxPeakFromZero() {
  ...
  return lshiftAndSaturate(halfValue,
                           1); // Does the <<1 and saturates it - this was necessary, it was
                               // overflowing sometimes - I think when the audio clipped
}
...
void Sample::finalizeAfterLoad(uint32_t fileSize) {
  ...
  // If floating point file, Clusers can only be float-processed (as they're
  // loaded) once we've found the data start-pos, which we just did, and since
  // we've already loaded that first cluster which contains data, we'd better
  ...
  audioDataLengthBytes =
      lengthInSamples * bytesPerSample; // Make sure it's an exact number of samples
...
void Sample::numReasonsDecreasedToZero(char const* errorCode) {
  ...
  for (int c = 0; c < clusters.getNumElements(); c++) {
    ...
    if (cluster) {
      ...
      if (cluster->clusterIndex != c)
        numericDriver.freezeWithError(errorCode); // Leo got! Aug 2020. Suspect some sort of memory
                                                  // corruption... And then Michael got, Feb 2021

      if (cluster->numReasonsToBeLoaded < 0) { numericDriver.freezeWithError("E076"); }
  ...
  if (numClusterReasons) {
    ...
    numericDriver.freezeWithError(
        "E078"); // LegsMechanical got, V4.0.0-beta2.
