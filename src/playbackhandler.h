...
#define PLAYBACK_CLOCK_EITHER_ACTIVE \
  (PLAYBACK_CLOCK_INTERNAL_ACTIVE | PLAYBACK_CLOCK_EXTERNAL_ACTIVE)
...
public:
  ...
  void    setupPlaybackUsingInternalClock(int  buttonPressLatencyForTempolessRecord = 0,
                                          bool allowCountIn                         = true);
  void    setupPlaybackUsingExternalClock(bool switchingFromInternalClock = false,
                                          bool fromContinueCommand        = false);
  void    setupPlayback(int     newPlaybackState,
                        int32_t playFromPos,
  ...
  bool    usingAnalogClockInput; // Value is only valid if usingInternalClock is
                                 // false
  uint8_t recording;
  bool    ignoringMidiClockInput;

  int32_t posToNextContinuePlaybackFrom; // This will then have 1 subtracted
                                         // from it when actually physically set
  uint32_t timeLastMIDIStartOrContinueMessageSent;

  // Timer ticks
  int64_t lastTimerTickActioned;  // Not valid while playback being set up
  int64_t nextTimerTickScheduled; // *Yes* valid while (internal clock) playback
                                  // being set up. Will be zero during that time
  ...
  uint32_t timePerInputTickMovingAverage; // 0 means that a default will be set
  ...
  unsigned long timeVeryFirstInputTick; // Very first tick received in playback. Only used
                                        // for tempo magnitude matching
  ...
  int64_t lastSwungTickActioned; // Will be set to a phony-ish "0" while
                                 // playback being set up
  ...
  uint32_t analogOutTicksPPQN; // Relative to the "external world", which also
                               // means relative to the displayed tempo
  ...
  void     pitchBendReceived(MIDIDevice* fromDevice,
                             uint8_t     channel,
                             uint8_t     data1,
                             uint8_t     data2,
                             bool*       doingMidiThru);
  void     midiCCReceived(MIDIDevice* fromDevice,
                          uint8_t     channel,
                          uint8_t     ccNumber,
                          uint8_t     value,
                          bool*       doingMidiThru);
  void     aftertouchReceived(MIDIDevice* fromDevice,
                              int         channel,
                              int         value,
                              int         noteCode,
  ...
  void noteMessageReceived(MIDIDevice* fromDevice,
                           bool        on,
                           int         channel,
                           int         note,
                           int         velocity,
  ...
  void  finishTempolessRecording(bool startPlaybackAgain,
...
private:
  ...
  uint32_t veryCurrentTimePerInternalTickInverse;   // Recalculated at every
                                                    // received clock message
  ...
  void getAnalogOutTicksToInternalTicksRatio(uint32_t* internalTicksPer,
                                             uint32_t* analogOutTicksPer);
  void getMIDIClockOutTicksToInternalTicksRatio(uint32_t* internalTicksPer,
  ...
  void     scheduleTriggerClockOutTickParamsKnown(uint32_t analogOutTicksPer,
                                                  uint64_t fractionLastTimerTick,
                                                  uint64_t fractionNextAnalogOutTick);
  void     scheduleMIDIClockOutTickParamsKnown(uint32_t midiClockOutTicksPer,
                                               uint64_t fractionLastTimerTick,
