...
public:
  ...
  bool renderMainPads(uint32_t whichRows,
                      uint8_t  image[][displayWidth + sideBarWidth][3],
                      uint8_t  occupancyMask[][displayWidth + sideBarWidth],
                      bool     drawUndefinedArea = true);
  void performActualRender(uint32_t whichRows,
                           uint8_t* image,
                           uint8_t  occupancyMask[][displayWidth + sideBarWidth],
                           int32_t  xScroll,
                           uint32_t xZoom,
                           ...
                           int      imageWidth,
                           bool     drawUndefinedArea = true);
  bool renderSidebar(uint32_t whichRows,
                     uint8_t  image[][displayWidth + sideBarWidth][3],
                     uint8_t  occupancyMask[][displayWidth + sideBarWidth]);
  ...
  void reportMPEInitialValuesForNoteEditing(ModelStackWithNoteRow* modelStack,
  ...
  bool                  auditioningSilently; // Sometimes the user will want to hold an audition
                                             // pad without actually sounding the note, by
                                             // holding an encoder
  bool fileBrowserShouldNotPreview;          // Archaic leftover feature that users
                                             // wouldn't let me get rid of
...
private:
  ...
  bool         editedAnyPerNoteRowStuffSinceAuditioningBegan; // Because in this case we
                                                              // can assume that if they
                                                              // press a main pad while
                                                              // auditioning, they're
                                                              // not intending to do
                                                              // that shortcut into the
                                                              // SoundEditor!
  ...
  uint32_t
      timeHorizontalKnobLastReleased; // Only to be looked at if
                                      // shouldIgnoreHorizontalScrollKnobActionIfNotAlsoPressedForThisNotePress
                                      // is true after they rotated a
                                      // NoteRow and might now be wanting
                                      // to instead edit its length after
                                      // releasing the knob
  ...
  void rotateNoteRowHorizontally(ModelStackWithNoteRow* modelStack,
                                 int                    offset,
                                 int                    yDisplay,
  ...
  ModelStackWithNoteRow* createNoteRowForYDisplay(ModelStackWithTimelineCounter* modelStack,
                                                  int                            yDisplay);
  ModelStackWithNoteRow* getOrCreateNoteRowForYDisplay(ModelStackWithTimelineCounter* modelStack,
                                                       int                            yDisplay);
};
