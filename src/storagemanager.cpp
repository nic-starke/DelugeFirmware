...
// Because FATFS and FIL objects store buffers for SD read data to be read to
// via DMA, we have to space them apart from any other data so that invalidation
// and stuff works
...
void StorageManager::printIndents() {
  for (int i = 0; i < indentAmount; i++) { write("\t"); }
}
...
char const* StorageManager::readTagName() {
  ...
  while (readCharXML(&thisChar)) {
    switch (thisChar) {
      ...
      default:
        if (!charPos) { tagDepthFile++; }

        // Store this character, if space in our un-ideal buffer
        if (charPos < FILENAME_BUFFER_SIZE - 1) { stringBuffer[charPos++] = thisChar; }
    }
  }
...
char const* StorageManager::readNextAttributeName() {
  ...
  while (readCharXML(&thisChar)) {
    switch (thisChar) {
      ...
      case '<': // This is an error - there definitely shouldn't be a '<' inside a
                // tag! TODO: make way to return error
...
noMoreAttributes:
  ...
  // Here, we're in IN_ATTRIBUTE_NAME, and we're not allowed to leave this while
  // loop until our xmlArea changes to something else
  // - or there's an error or file-end, in which case we'll return error below
doReadName:
  xmlArea = IN_ATTRIBUTE_NAME;
  tagDepthFile++;
  fileBufferCurrentPos--; // This means we don't need to call
                          // readXMLFileClusterIfNecessary()
  ...
  do {
    ...
    while (fileBufferCurrentPos < currentReadBufferEndPos) {
      ...
      switch (thisChar) {
        ...
        // If we get a close-tag name, it means we saw some sorta attribute name
        // with no value, which isn't allowed, so treat it as invalid
        case '>':
          xmlArea = BETWEEN_TAGS;
          goto noMoreAttributes;
    ...
    reachedNameEnd:
      ...
      // If possible, just return a pointer to the chars within the existing
      // buffer
...
char const* StorageManager::readNextTagOrAttributeName() {
  ...
  switch (xmlArea) {
    ...
    default:
      ...
      numericDriver.freezeWithError("E365"); // Can happen with invalid files, though I'm
                                             // implementing error checks whenever a user alerts me
                                             // to a scenario. Fraser got this, Nov 2021.
#else
      __builtin_unreachable();
#endif
      break;

    case IN_ATTRIBUTE_VALUE: // Could have been left here during a char-at-a-time
                             // read
    ...
    case PAST_ATTRIBUTE_NAME:
      while (readCharXML(&thisChar)) {
        switch (thisChar) {
          ...
          default:
            return false; // There shouldn't be any other characters. If there are,
                          // that's an error
        }
      }
...
void StorageManager::xmlReadDone() {
  xmlReadCount++; // Increment first, cos we don't want to call SD routine
                  // immediately when it's 0
...
void StorageManager::skipUntilChar(char endChar) {
  ...
  readXMLFileClusterIfNecessary(); // Does this need to be here? Originally I
                                   // didn't have it...
...
// Returns memory error. If error, caller must deal with the fact that the
// end-character hasn't been reached
int StorageManager::readStringUntilChar(String* string, char endChar) {
  ...
  do {
    ...
    if (numCharsHere) {
      int error = string->concatenateAtPos(&fileClusterBuffer[fileBufferCurrentPos],
                                           newStringPos,
                                           numCharsHere);
...
// Unlike readUntilChar(), above, does not put a null character at the end of
// the returned "string". And, has a preset number of chars. And, returns NULL
// when nothing more to return. numChars must be <= FILENAME_BUFFER_SIZE
char const* StorageManager::readNextCharsOfTagOrAttributeValue(int numChars) {
  ...
  do {
    ...
    while (fileBufferCurrentPos < currentReadBufferEndPosNow) {
      if (fileClusterBuffer[fileBufferCurrentPos] == charAtEndOfValue) { goto reachedEndCharEarly; }
      fileBufferCurrentPos++;
    }

    int numCharsHere = fileBufferCurrentPos - bufferPosAtStart;

    // If we were able to just read the whole thing in one go, just return a
    // pointer to the chars within the existing buffer
    if (numCharsHere == numChars) {
      xmlReadDone();
      return &fileClusterBuffer[bufferPosAtStart];
    }

    // Otherwise, so long as we read something, add it to our buffer we're
...
// Will always skip up until the end-char, even if it doesn't like the contents
// it sees
int32_t StorageManager::readIntUntilChar(char endChar) {
  ...
  if (false) {
  getOut:
    if (thisChar != endChar) { skipUntilChar(endChar); }
  }
...
char const* StorageManager::readTagOrAttributeValue() {
  ...
  switch (xmlArea) {
    ...
    case IN_TAG_PAST_NAME: // Could happen if trying to read a value but instead
                           // of a value there are multiple more contents, like
                           // attributes etc. Obviously not "meant" to happen, but
                           // we need to cope.
      return "";
...
int StorageManager::getNumCharsRemainingInValue() {
  ...
  while (pos < currentReadBufferEndPos && fileClusterBuffer[pos] != charAtEndOfValue) { pos++; }
...
bool StorageManager::readXMLFileClusterIfNecessary() {
  ...
  if (fileBufferCurrentPos >= audioFileManager.clusterSize) {
    ...
    if (!result) { xmlReachedEnd = true; }
    return result;
  }

  // Watch out for end of file
  if (fileBufferCurrentPos >= currentReadBufferEndPos) { xmlReachedEnd = true; }
...
void StorageManager::exitTag(char const* exitTagName) {
  ...
  while (tagDepthFile >= tagDepthCaller) {
    ...
    switch (xmlArea) {
      ...
      case IN_ATTRIBUTE_VALUE: // Could get left in here after a char-at-a-time
                               // read
        skipUntilChar(charAtEndOfValue);
        xmlArea = IN_TAG_PAST_NAME;
        // No break
      ...
      default:
        if (ALPHA_OR_BETA_VERSION)
          numericDriver.freezeWithError("AAAA"); // Really shouldn't be possible anymore, I feel
                                                 // fairly certain...
        __builtin_unreachable();
    }
  }
...
int StorageManager::checkSpaceOnCard() {
  ...
  return fileSystemStuff.fileSystem.free_clst ? NO_ERROR :
                                                ERROR_SD_CARD_FULL; // This doesn't seem to always
                                                                    // be 100% accurate...
}
...
tryAgain:
  ...
  processError:
    ...
    cutFolderPathAndTryCreating:
      ...
      if (result == FR_OK)
        goto tryAgain;
      else if (result == FR_NO_PATH) {    // If that folder couldn't be created because its
                                          // parent folder didn't exist...
        triedCreatingFolder = false;      // Let it do multiple tries again
        goto cutFolderPathAndTryCreating; // Go and try creating the parent
                                          // folder
      } else
        goto processError;
    }
    ...
    // Otherwise, just return the appropriate error.
      ...
      if (error == ERROR_SD_CARD)
        error = ERROR_WRITE_FAIL; // Get a bit more specific if we only got the
                                  // most general error.
      return error;
    }
  }
...
int StorageManager::writeBufferToFile() {
  UINT    bytesWritten;
  FRESULT result =
      f_write(&fileSystemStuff.currentFile, fileClusterBuffer, fileBufferCurrentPos, &bytesWritten);
  if (result != FR_OK || bytesWritten != fileBufferCurrentPos) { return ERROR_SD_CARD; }
...
int StorageManager::closeFileAfterWriting(char const* path,
                                          char const* beginningString,
                                          char const* endString) {
  if (fileAccessFailedDuring)
    return ERROR_WRITE_FAIL; // Calling f_close if this is false might be
                             // dangerous - if access has failed, we don't want
                             // it to flush any data to the card or anything
  int error = writeBufferToFile();
...
bool StorageManager::lseek(uint32_t pos) {
  ...
  if (result != FR_OK) { fileAccessFailedDuring = true; }
...
int StorageManager::tryReadingFirmwareTagFromFile(char const* tagName,
  ...
  if (!strcmp(tagName, "firmwareVersion")) {
    ...
    firmwareVersionOfFileBeingRead    = stringToFirmwareVersion(firmwareVersionString);
                   ...
                   "earliestCompatibleFirmware")) { // If this tag doesn't
                                                    // exist, it's from old
                                                    // firmware so is ok
    char const* firmwareVersionString = readTagOrAttributeValue();
...
bool StorageManager::readXMLFileCluster() {
  ...
  FRESULT result = f_read(&fileSystemStuff.currentFile,
                          (UINT*)fileClusterBuffer,
                          audioFileManager.clusterSize,
...
bool StorageManager::closeFile() {
  if (fileAccessFailedDuring)
    return false; // Calling f_close if this is false might be dangerous - if
                  // access has failed, we don't want it to flush any data to
...
// You should call this before you're gonna do any accessing - otherwise any
// errors won't reflect if there's in fact just no card inserted.
int StorageManager::initSD() {
  ...
  if ((status & STA_NOINIT) == 0) { return NO_ERROR; }

  // But if there's no card present, we're in trouble
  if (status & STA_NODISK) { return ERROR_SD_CARD_NOT_PRESENT; }
...
int StorageManager::loadInstrumentFromFile(Song*           song,
  ...
  // Check that a ParamManager was actually loaded for the Instrument, cos if
  // not, that'd spell havoc
  if (!song->getBackedUpParamManagerPreferablyWithClip(
          (ModControllableAudio*)newInstrument->toModControllable(),
          NULL)) {

    // Prior to V2.0 (or was it only in V1.0 on the 40-pad?) Kits didn't have
    // anything that would have caused the paramManager to be created when we
    // read the Kit just now. So, just make one.
    if (firmwareVersionOfFileBeingRead < FIRMWARE_2P0P0_BETA &&
      ...
      ((Kit*)newInstrument)
          ->compensateInstrumentVolumeForResonance(&paramManager,
                                                   song); // Necessary?
  ...
  if (newInstrument->type == INSTRUMENT_TYPE_KIT) {
    ...
    for (Drum* thisDrum = kit->firstDrum; thisDrum; thisDrum = thisDrum->next) {
      if (thisDrum->type == DRUM_TYPE_SOUND) {
        ...
        if (!currentSong->getBackedUpParamManagerPreferablyWithClip(
                soundDrum,
  ...
  newInstrument->loadAllAudioFiles(mayReadSamplesFromFiles); // Needs name, directory and slots set
                                                             // first, above.
...
Instrument* StorageManager::createNewInstrument(uint8_t       newInstrumentType,
  ...
  if (!instrumentMemory) { return NULL; }
...
Instrument* StorageManager::createNewNonAudioInstrument(int instrumentType, int slot, int subSlot) {
  int size =
      (instrumentType == INSTRUMENT_TYPE_MIDI_OUT) ? sizeof(MIDIInstrument) : sizeof(CVInstrument);
...
// This has now mostly been replaced by an equivalent-ish function in
// InstrumentClip. Now, this will only ever be called in two scenarios:
// -- Pre-V2.0 files, so we know there's no mention of bend or aftertouch in
// this case where we have a ParamManager.
// -- When reading a MIDIInstrument, so we know there's no ParamManager (I
// checked), so no need to actually read the param.
int StorageManager::readMIDIParamFromFile(int32_t              readAutomationUpToPos,
  ...
  while (*(tagName = readNextTagOrAttributeName())) {
    if (!strcmp(tagName, "cc")) {
      ...
      // TODO: Pre-V2.0 files could still have CC74, so ideally I'd move that to
      // "expression" params here...
      exitTag("cc");
...
// For a bunch of params like this, e.g. for syncing delay, LFOs, arps, the
// value stored in the file is relative to the song insideWorldTickMagnitude -
// so that if someone loads a preset into a song with a different
...
void StorageManager::writeAbsoluteSyncLevelToFile(Song*       song,
                                                  ...
                                                  bool        onNewLine) {
  writeAttribute(name,
                 song->convertSyncLevelFromInternalValueToFileValue(internalValue),
                 onNewLine);
}
