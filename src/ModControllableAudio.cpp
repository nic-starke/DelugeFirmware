...
ModControllableAudio::~ModControllableAudio() {
  // delay.discardBuffers(); // No! The DelayBuffers will themselves destruct
  // and do this

  // Free the mod fx memory
  if (modFXBuffer) { generalMemoryAllocator.dealloc(modFXBuffer); }
}
...
void ModControllableAudio::initParams(ParamManager* paramManager) {
  ...
  unpatchedParams->params[PARAM_UNPATCHED_SAMPLE_RATE_REDUCTION].setCurrentValueBasicForSetup(
      -2147483648);

  unpatchedParams->params[PARAM_UNPATCHED_BITCRUSHING].setCurrentValueBasicForSetup(-2147483648);

  unpatchedParams->params[PARAM_UNPATCHED_COMPRESSOR_SHAPE].setCurrentValueBasicForSetup(
      -601295438);
}
...
void ModControllableAudio::processFX(StereoSample*      buffer,
  ...
  if (modFXType != MOD_FX_TYPE_NONE) {
    ...
    if (modFXType == MOD_FX_TYPE_FLANGER || modFXType == MOD_FX_TYPE_PHASER) {
      ...
      squared2         = (multiply_32x32_rshift32(squared2, squared2) >> 4) *
                 23; // 22 // Make bigger to have more of a volume cut happen at
                     // high resonance
      //*postFXVolume = (multiply_32x32_rshift32(*postFXVolume, 2147483647 -
      // squared2) >> 1) * 3;
      *postFXVolume = multiply_32x32_rshift32(*postFXVolume, 2147483647 - squared2);
      if (modFXType == MOD_FX_TYPE_FLANGER) *postFXVolume <<= 1;
      // Though, this would be more ideally placed affecting volume before the
      // flanger
      ...
      modFXDelayOffset = multiply_32x32_rshift32(
          modFXMaxDelay,
          (unpatchedParams->getValue(PARAM_UNPATCHED_MOD_FX_OFFSET) >> 1) + 1073741824);
    ...
    do {
      ...
      if (modFXType == MOD_FX_TYPE_PHASER) {
        ...
        int32_t _a1 = 1073741824 - multiply_32x32_rshift32_rounded(
                                       (((uint32_t)lfoOutput + (uint32_t)2147483648) >> 1),
                                       modFXDepth);

        phaserMemory.l =
            currentSample->l + (multiply_32x32_rshift32_rounded(phaserMemory.l, feedback) << 1);
        phaserMemory.r =
            currentSample->r + (multiply_32x32_rshift32_rounded(phaserMemory.r, feedback) << 1);
        ...
        for (int i = 0; i < PHASER_NUM_ALLPASS_FILTERS; i++) {
          ...
          phaserMemory.l =
              (multiply_32x32_rshift32_rounded(phaserMemory.l, -_a1) << 2) + allpassMemory[i].l;
          allpassMemory[i].l =
              (multiply_32x32_rshift32_rounded(phaserMemory.l, _a1) << 2) + whatWasInput.l;

          phaserMemory.r =
              (multiply_32x32_rshift32_rounded(phaserMemory.r, -_a1) << 2) + allpassMemory[i].r;
          allpassMemory[i].r =
              (multiply_32x32_rshift32_rounded(phaserMemory.r, _a1) << 2) + whatWasInput.r;
        }
        ...
        int32_t delayTime =
            multiply_32x32_rshift32(lfoOutput, thisModFXDelayDepth) + modFXDelayOffset;
        ...
        int32_t scaledValue1L =
            multiply_32x32_rshift32_rounded(modFXBuffer[sample1Pos & modFXBufferIndexMask].l,
                                            strength1);
        int32_t scaledValue2L =
            multiply_32x32_rshift32_rounded(modFXBuffer[(sample1Pos - 1) & modFXBufferIndexMask].l,
                                            strength2);
        int32_t modFXOutputL = scaledValue1L + scaledValue2L;

        int32_t scaledValue1R =
            multiply_32x32_rshift32_rounded(modFXBuffer[sample1Pos & modFXBufferIndexMask].r,
                                            strength1);
        int32_t scaledValue2R =
            multiply_32x32_rshift32_rounded(modFXBuffer[(sample1Pos - 1) & modFXBufferIndexMask].r,
                                            strength2);
  ...
  if (thisDoBass || thisDoTreble) {
    ...
    if (thisDoTreble) {
      trebleFreq =
          getExp(700000000, (unpatchedParams->getValue(PARAM_UNPATCHED_TREBLE_FREQ) >> 5) * 6);
    }
  ...
  if (delayWorkingState->doDelay) {
    ...
    // If just a single buffer is being used for reading and writing, we can
    // consider making a 2nd buffer
    if (!delay.secondaryBuffer.isActive()) {

      // If resampling previously recorded as happening, or just about to be
      ...
      if (delay.primaryBuffer.isResampling ||
        ...
        // If spinning at double native rate, there's no real need to be using
        // such a big buffer, so make a new (smaller) buffer at our new rate
        ...
        // If spinning below native rate, the quality's going to be suffering,
        // so make a new buffer whose native rate is half our current rate
        // (double the quality)
    ...
    if (delay.analog) {

      {
        int32_t* workingBufferPos = delayWorkingBuffer;
        do {
          delay.impulseResponseProcessor.process(workingBufferPos[0],
                                                 workingBufferPos[1],
                                                 &workingBufferPos[0],
                                                 &workingBufferPos[1]);
...

      {
        ...
        do {
          ...
          // delay.impulseResponseProcessor.process(fromDelayL, fromDelayR,
          // &fromDelayL, &fromDelayR);

          // Reduce headroom, since this sounds ok with analog sim
          workingBufferPos[0] =
              getTanHUnknown(
                  multiply_32x32_rshift32(fromDelayL, delayWorkingState->delayFeedbackAmount),
                  analogDelaySaturationAmount)
              << 2;
          workingBufferPos[1] =
              getTanHUnknown(
                  multiply_32x32_rshift32(fromDelayR, delayWorkingState->delayFeedbackAmount),
                  analogDelaySaturationAmount)
              << 2;
      ...
      int32_t* workingBufferPos = delayWorkingBuffer;
      do {
        // Leave more headroom, because making it clip sounds bad with pure
        // digital
        workingBufferPos[0] =
            signed_saturate(multiply_32x32_rshift32(workingBufferPos[0],
                                                    delayWorkingState->delayFeedbackAmount),
                            32 - 3)
            << 2;
        workingBufferPos[1] =
            signed_saturate(multiply_32x32_rshift32(workingBufferPos[1],
                                                    delayWorkingState->delayFeedbackAmount),
                            32 - 3)
            << 2;
    ...
    // HPF on delay output, to stop it "farting out". Corner frequency is
    // somewhere around 40Hz after many repetitions
    {
      int32_t* workingBufferPos = delayWorkingBuffer;
      do {
...

    {
      StereoSample* currentSample    = buffer;
      int32_t*      workingBufferPos = delayWorkingBuffer;
      // Go through what we grabbed, sending it to the audio output buffer, and
      // also preparing it to be fed back into the delay
      do {
        ...
        if (delay.analog) {
            // Reduce headroom, since this sounds ok with analog sim
            fromDelayL = getTanH(multiply_32x32_rshift32(fromDelayL,
        delayWorkingState->delayFeedbackAmount), 8) << 2; fromDelayR =
        getTanH(multiply_32x32_rshift32(fromDelayR,
        delayWorkingState->delayFeedbackAmount), 8) << 2;
        }

        else {
            fromDelayL = signed_saturate(multiply_32x32_rshift32(fromDelayL,
        delayWorkingState->delayFeedbackAmount), 32 - 3) << 2; fromDelayR =
        signed_saturate(multiply_32x32_rshift32(fromDelayR,
        delayWorkingState->delayFeedbackAmount), 32 - 3) << 2;
        }
        */

        /*
        // HPF on delay output, to stop it "farting out". Corner frequency is
        somewhere around 40Hz after many repetitions int32_t distanceToGoL =
        fromDelayL - delay.postLPFL; delay.postLPFL += distanceToGoL >> 11;
    ...
    // And actually feedback being applied back into the actual delay primary
    // buffer...
    if (delay.primaryBuffer.isActive()) {
      ...
      if (!delay.primaryBuffer.isResampling) {
        ...
        do {
          delay.primaryBuffer.writeNativeAndMoveOn(workingBufferPos[0],
                                                   workingBufferPos[1],
                                                   &writePos);
      ...
      // Resampling
        ...
        do {
          ...
          delay.primaryBuffer.writeResampled(workingBufferPos[0],
                                             workingBufferPos[1],
                                             primaryStrength1,
                                             primaryStrength2,
                                             &delayPrimarySetup);
    ...
    if (delay.secondaryBuffer.isActive()) {
      ...
      wrapped = false; // We want to disregard whatever the primary buffer told
                       // us, and just use the secondary one now
      ...
      // Resampled
        ...
        do {
          ...
          // Write to secondary buffer
          delay.secondaryBuffer.writeResampled(workingBufferPos[0],
                                               workingBufferPos[1],
                                               secondaryStrength1,
                                               secondaryStrength2,
                                               &delaySecondarySetup);
      ...
      if (delay.sizeLeftUntilBufferSwap < 0) { delay.copySecondaryToPrimary(); }
    }

    if (wrapped) { delay.hasWrapped(); }
  }
}
...
void ModControllableAudio::processReverbSendAndVolume(StereoSample* buffer,
  ...
  int32_t reverbSendAmountAndPostFXVolume = multiply_32x32_rshift32(postFXVolume, reverbSendAmount)
                                            << 5;
  ...
  postFXAndReverbVolumeL = postFXAndReverbVolumeR =
      (multiply_32x32_rshift32(postReverbVolume, postFXVolume) << 5);

  // The amplitude increment applies to the post-FX volume. We want to have it
  // just so that we can respond better to sidechain volume ducking, which is
  // done through post-FX volume.
  if (doAmplitudeIncrement) {
    amplitudeIncrementL = amplitudeIncrementR =
        (multiply_32x32_rshift32(postFXVolume, amplitudeIncrement) << 5);
  }
  ...
  do {
    ...
    if (reverbSendAmount != 0) {
      *(reverbBuffer++) += multiply_32x32_rshift32(processingSample.l + processingSample.r,
...
void ModControllableAudio::processSRRAndBitcrushing(StereoSample* buffer,
  ...
  if (isBitcrushingEnabled(paramManager)) {
    uint32_t positivePreset =
        (paramManager->getUnpatchedParamSet()->getValue(PARAM_UNPATCHED_BITCRUSHING) +
         2147483648) >>
        29;
  ...
  if (srrEnabled) {
    ...
    // This function, slightly unusually, uses 22 bits to represent "1". That's
    // 4194304. I tried using 24, but stuff started clipping off where I needed
    // it if sample rate too low

    uint32_t positivePreset =
        paramManager->getUnpatchedParamSet()->getValue(PARAM_UNPATCHED_SAMPLE_RATE_REDUCTION) +
        2147483648;
    ...
    // int32_t highSampleRateIncrement = getExp(4194304,
    // -(int32_t)(positivePreset >> 3)); // This would work too
    ...
    do {
      ...
      if (lowSampleRatePos < 4194304) {
        ...
        // Set the "time" at which we want to "grab" our next sample for
        // down-conversion.
        lowSampleRatePos += lowSampleRateIncrement;

        // "Re-sync" the up-conversion spinner.
        // I previously had it using strength2 instead of "lowSampleRatePos &
        // 16777215", but that just works better. Ah, writing the massive
        // explanation would take ages.
        highSampleRatePos = multiply_32x32_rshift32_rounded(lowSampleRatePos & 4194303,
                                                            highSampleRateIncrement << 8)
                            << 2;
      }
      lowSampleRatePos -= 4194304; // We're one step closer to grabbing our next
                                   // sample for down-conversion
      lastSample = *currentSample;

      // Convert up
      int32_t strength2 = getMin(highSampleRatePos,
                                 (uint32_t)4194303); // Would only overshoot if we raised the
                                                     // sample rate during playback
      int32_t strength1 = 4194303 - strength2;
  ...
  if (stutterer.status == STUTTERER_STATUS_RECORDING) {
    ...
    do {
      ...
      // Or, resampling tick-along
        ...
        // stutterer.buffer.writeResampled(thisSample->l, thisSample->r,
        // strength1, strength2, &delayBufferSetup);
      }
    ...
    if (stutterer.sizeLeftUntilRecordFinished < 0) { stutterer.status = STUTTERER_STATUS_PLAYING; }
  }
...
int32_t ModControllableAudio::getStutterRate(ParamManager* paramManager) {
  UnpatchedParamSet* unpatchedParams = paramManager->getUnpatchedParamSet();
  int32_t            rate            = getFinalParameterValueExp(
  ...
  if (stutterer.sync != 0) {
    ...
    int lShiftAmount = stutterer.sync + 6 -
...
void ModControllableAudio::initializeSecondaryDelayBuffer(
    int32_t newNativeRate,
  ...
  if (result == NO_ERROR) {
    ...
    // 2 different options here for different scenarios. I can't very clearly
    // remember how to describe the difference
...
inline void ModControllableAudio::doEQ(bool     doBass,
                                       bool     doTreble,
                                       int32_t* inputL,
                                       int32_t* inputR,
                                       int32_t  bassAmount,
...
void ModControllableAudio::writeTagsToFile() {
  ...
  if (midiKnobArray.getNumElements()) {
    ...
    for (int k = 0; k < midiKnobArray.getNumElements(); k++) {
      ...
      knob->midiInput.writeAttributesToFile(MIDI_MESSAGE_CC); // Writes channel and CC, but not
                                                              // device - we do that below.
      ...
      // Because we manually called LearnedMIDI::writeAttributesToFile() above,
      // we have to give the MIDIDevice its own tag, cos that can't be written
      // as just an attribute.
...
void ModControllableAudio::writeParamAttributesToFile(ParamManager* paramManager,
  ...
  unpatchedParams->writeParamAsAttribute("stutterRate",
                                         PARAM_UNPATCHED_STUTTER_RATE,
                                         writeAutomation,
                                         false,
                                         valuesForOverride);
  unpatchedParams->writeParamAsAttribute("sampleRateReduction",
                                         PARAM_UNPATCHED_SAMPLE_RATE_REDUCTION,
                                         writeAutomation,
                                         false,
                                         valuesForOverride);
  unpatchedParams->writeParamAsAttribute("bitCrush",
                                         PARAM_UNPATCHED_BITCRUSHING,
                                         writeAutomation,
                                         false,
                                         valuesForOverride);
  unpatchedParams->writeParamAsAttribute("modFXOffset",
                                         PARAM_UNPATCHED_MOD_FX_OFFSET,
                                         writeAutomation,
                                         false,
                                         valuesForOverride);
  unpatchedParams->writeParamAsAttribute("modFXFeedback",
...
void ModControllableAudio::writeParamTagsToFile(ParamManager* paramManager,
  ...
  unpatchedParams->writeParamAsAttribute("bass",
                                         PARAM_UNPATCHED_BASS,
                                         writeAutomation,
                                         false,
                                         valuesForOverride);
  unpatchedParams->writeParamAsAttribute("treble",
                                         PARAM_UNPATCHED_TREBLE,
                                         writeAutomation,
                                         false,
                                         valuesForOverride);
  unpatchedParams->writeParamAsAttribute("bassFrequency",
                                         PARAM_UNPATCHED_BASS_FREQ,
                                         writeAutomation,
                                         false,
                                         valuesForOverride);
  unpatchedParams->writeParamAsAttribute("trebleFrequency",
...
int ModControllableAudio::readTagFromFile(char const*              tagName,
  ...
  // All of this is here for compatibility only for people (Lou and Ian) who
  // saved songs with firmware in September 2016
  // if (paramManager && ModControllableAudio::readParamTagFromFile(tagName,
  // paramManager, readAutomation)) {}
  ...
  if (!strcmp(tagName, "lpfMode")) {
    ...
    delay.sync = 0; // Default, in case this info wasn't included... although it
                    // always is since July 2015

    while (*(tagName = storageManager.readNextTagOrAttributeName())) {

      // These first two ensure compatibility with old files (pre late 2016 I
      // think?)
...
ModControllableAudio::getParamFromMIDIKnob(MIDIKnob*                      knob,
  ...
  ModelStackWithAutoParam* modelStackWithAutoParam =
      paramCollection->getAutoParamFromId(modelStackWithParamId);

  return modelStackWithAutoParam;
}

ModelStackWithThreeMainThings*
ModControllableAudio::addNoteRowIndexAndStuff(ModelStackWithTimelineCounter* modelStack,
                                              int                            noteRowIndex) {
  ...
  if (noteRowIndex != -1) {
    ...
    if (modelStack->timelineCounterIsSet()) {
      ...
      paramManager = modelStack->song->getBackedUpParamManagerPreferablyWithClip(
          this,
          NULL); // Could be NULL if a NonAudioInstrument - those don't back
                 // up any paramManagers (when they even have them).
    }
  }
...
bool ModControllableAudio::offerReceivedCCToLearnedParams(MIDIDevice*                    fromDevice,
  ...
  for (int k = 0; k < midiKnobArray.getNumElements(); k++) {
    ...
    if (knob->midiInput.equalsNoteOrCC(fromDevice, channel, ccNumber)) {
      ...
      // Only if this exact TimelineCounter is having automation step-edited, we
      // can set the value for just a region.
      int32_t modPos    = 0;
      int32_t modLength = 0;
      ...
      // Ok, that above might have just changed modelStack->timelineCounter. So
      // we're basically starting from scratch now from that.
      ModelStackWithThreeMainThings* modelStackWithThreeMainThings =
      ...
      if (modelStackWithParam->autoParam) {
        ...
        if (knob->relative) {
          ...
          int knobPos =
              modelStackWithParam->paramCollection->paramValueToKnobPos(previousValue,
                                                                        modelStackWithParam);
        ...
        int32_t newValue =
            modelStackWithParam->paramCollection->knobPosToParamValue(newKnobPos,
                                                                      modelStackWithParam);

        modelStackWithParam->autoParam->setValuePossiblyForRegion(newValue,
                                                                  modelStackWithParam,
                                                                  modPos,
                                                                  modLength);
      }
...
bool ModControllableAudio::offerReceivedPitchBendToLearnedParams(
  ...
  for (int k = 0; k < midiKnobArray.getNumElements(); k++) {
    ...
    // If this is the knob...
    if (knob->midiInput.equalsNoteOrCC(
            fromDevice,
            channel,
...
void ModControllableAudio::beginStutter(ParamManagerForTimeline* paramManager) {
  ...
  // You'd think I should apply "false" here, to make it not add extra space to
  // the buffer, but somehow this seems to sound as good if not better (in terms
  // of ticking / crackling)...
...
// paramManager is optional - if you don't send it, it won't change the stutter
// rate
void ModControllableAudio::endStutter(ParamManagerForTimeline* paramManager) {
  ...
  if (paramManager) {
    ...
    // Normally we shouldn't call this directly, but it's ok because automation
    // isn't allowed for stutter anyway
...
void ModControllableAudio::clearModFXMemory() {
  if (modFXType == MOD_FX_TYPE_FLANGER || modFXType == MOD_FX_TYPE_CHORUS) {
    if (modFXBuffer) { memset(modFXBuffer, 0, modFXBufferSize * sizeof(StereoSample)); }
...
bool ModControllableAudio::setModFXType(int newType) {
  ...
  // For us ModControllableAudios, this is really simple. Memory gets allocated
  // in GlobalEffectable::processFXForGlobalEffectable(). This function is
  // overridden in Sound
  modFXType = newType;
  return true;
}
...
bool ModControllableAudio::learnKnob(MIDIDevice*     fromDevice,
  ...
  if (midiChannel >= 16) {
    ...
    overwroteExistingKnob = (modKnobs[modKnobMode][whichKnob].s != s ||
  ...
  midiKnobFound:
    ...
    knob->relative =
        (whichKnob != 128); // Guess that it's relative, unless this is a pitch-bend "knob"
  }
...
bool ModControllableAudio::unlearnKnobs(ParamDescriptor paramDescriptor, Song* song) {
  ...
  // I've deactivated the unlearning of mod knobs, mainly because, if you want
  // to unlearn a MIDI knob, you might not want to also deactivate a mod knob to
  // the same param at the same time
...
int ModControllableAudio::stringToParam(char const* string) {
  for (int p = PARAM_UNPATCHED_SECTION; p < PARAM_UNPATCHED_SECTION + NUM_SHARED_UNPATCHED_PARAMS;
       p++) {
...
ModelStackWithAutoParam*
ModControllableAudio::getParamFromModEncoder(int                            whichModEncoder,
                                             ModelStackWithThreeMainThings* modelStack,
                                             bool                           allowCreation) {
