...
// These refer to how many samples to wait *after* the override length, which is
// specified above. And these only apply if we hit a non-interpolated node
// (which, remember, is all of them for MIDI CCs
...
// This is mostly for "expression" params, which we frequently want to bump back
// to 0 - often when there is no automation, or when playback is stopped.
void AutoParam::setCurrentValueWithNoReversionOrRecording(ModelStackWithAutoParam const* modelStack,
  ...
  modelStack->paramCollection->notifyParamModifiedInSomeWay(modelStack,
                                                            oldValue,
                                                            false,
                                                            automatedNow,
                                                            automatedNow);
...
// livePos may be supplied as -1, meaning get it live. Or, you can override this
// by specifying one. The main purpose of mayDeleteNodesInLinearRun is so that
// it can be prevented from happening for MPE recording - even though MPE will
// normally interpolate for internal synths, we want to preserve the original
// MIDI as closely as possible in case they later switch it from Synth to MIDI
// output.
void AutoParam::setCurrentValueInResponseToUserInput(int32_t                        value,
  ...
  bool isPlaying = (playbackHandler.isEitherClockActive() && !playbackHandler.ticksLeftInCountIn &&
                    (!modelStack->timelineCounterIsSet() ||
                     modelStack->getTimelineCounter()->isPlayingAutomationNow()));
  ...
  if (isPlaying) {
    ...
    if (playbackHandler.recording && modelStack->timelineCounterIsSet()) {
      ...
      // 1. While recording, any nodes in that region are going to be ignored
      // anyway.
      // 2. If recording is exited, we want to have a 0.2s transition before
      // going back to the next node
      ...
      // If the Clip is too short to meaningfully record anything / not cause an
      // error
      ...
      // Since May 2020, we don't interpolate the start of the region if there
      // was not another node just before it. Perhaps in a perfect world, we'd
      // also consider how different the new value is from the old value, and
      // maybe even insert a note to interpolate from.
      bool shouldInterpolateRegionStart = false;

      if (!reversed && nodes.getNumElements()) { // Yeah turns out we just don't need the
                                                 // result from this if we're reversed. RIP
                                                 // the work I put into making this code
                                                 // reverse-compatible.
        int prevNodeI = nodes.search(livePos + (int)reversed, reversed ? GREATER_OR_EQUAL : LESS);
        if (prevNodeI >= 0 && prevNodeI < nodes.getNumElements()) { // If there was a Node before
                                                                    // livePos...
        investigatePrevNode:
          ...
          if (ticksAgo <= 0) { ticksAgo += effectiveLength; }
          shouldInterpolateRegionStart =
              (ticksAgo * timePerInternalTick < TIME_TO_INTERPOLATE_WITHIN);
          ...
          // If livePos was close enough to 0 that we need to look at wrapped
          // Nodes back around on the right...
      ...
      // - recording MPE (not mono expression) *linearly*. Just insert one node,
      // and that can change the value everywhere from here to the next node -
      // we don't need to preserve the "original" value in any part of that
      // region because it's going to get overridden any time another note is
      // inserted into it, anyway. And we want our value to last as long as
      // possible, for the note's release-tail.
      if (doMPEMode) {
      ...
      // Or, normal case.
        ...
        nodes.testSequentiality("E435"); // drbourbon got, when check was inside homogenizeRegion().
                                         // Now trying to work out where that came from. March 2022.
#endif

        leftI = homogenizeRegion(modelStack,
                                 livePos,
                                 ticksToClear,
                                 value,
                                 shouldInterpolateLeft,
                                 true,
                                 effectiveLength,
                                 reversed,
                                 posAtWhichPlaybackWillCut);
        if (leftI == -1) goto getOut;
      ...
      // Now that we've definitely left any previous nodes alone, see if they
      // form a linear run and we can delete some of them? We'll just not bother
      // with this if reversed, for now... or ever...
      if (mayDeleteNodesInLinearRun && !reversed &&
        ...
        bool backtrackingCouldLoopBackToEnd =
            modelStack->getTimelineCounter()
                ->backtrackingCouldLoopBackToEnd(); // Wait, I can no longer see
                                                    // why this matters...
        int prevI = leftI - 1;
        if (prevI == -1) {
      ...
      if (nodes.getNumElements()) {
        ...
        if (rightmostNode->pos >= effectiveLength) { numericDriver.freezeWithError("llll"); }
      }
#endif
    }
  ...
  if (!nodes.getNumElements() || !isPlaying) {
    if (value != currentValue) { actionLogger.recordUnautomatedParamChange(modelStack); }
  }
...
bool AutoParam::deleteRedundantNodeInLinearRun(int     lastNodeInRunI,
  ...
  // But first, now that we've moved on from prevNode, see if prevNode concluded
  // a linear run of nodes for which we can now delete the middle node
  ...
  if (lastNodeInRun->interpolated || !middleNodeInRun->interpolated) {
    ...
    removeMiddleNodeInRun:
      ...
      // float timeFraction = (float)howFarAfter(middleNodeInRun->pos,
      // firstNodeInRun->pos, clip) / howFarAfter(prevNode->pos,
      // firstNodeInRun->pos, clip);
...
// Beware. As this will do no notifying, the caller must ensure that any
// required notification is done. I.e. a ParamSet must be notified if automation
// is deleted.
...
int32_t AutoParam::processCurrentPos(ModelStackWithAutoParam const* modelStack,
  ...
  if (howFarUntilThisNode) {
    if (reversed)
      howFarUntilThisNode = -howFarUntilThisNode; // Adjust for direction. No need to do until we
  ...
  */
  ...
  // Stop any pre-existing interpolation (though we might set up some more,
  // below)
  valueIncrementPerHalfTick = 0;
  ...
  if (reversed) {
    ...
    if (!noNeedToJumpToValue)
      valueJustReached = nodeToLeft->value; // At the time of this condition, weInterpolatedHere
                                            // still means the interpolation to our *left*.
    // noNeedToJumpToValue = noNeedToJumpToValue || nodeToRight->interpolated;
    // // If playing reversed, we probably want to jump directly to the value of
    // the node to the left,
    //  unless from the node right here there's a slope left *and* a slope right
    //  (cos if there's no slope right we'll already be at the correct value
    //  from before).
  ...
  if (renewedOverridingAtTime) {
    ...
    if (renewedOverridingAtTime == 1) {
      ...
      // If not recording, but we're still latching, well latching is only meant
      // to happen as a result of recording, so they must have just cancelled
      // recording. So, exit out of latching and just give them normal
      // overriding.
      else {
        renewedOverridingAtTime = AudioEngine::audioSampleTimer -
                                  SAMPLES_TO_CLEAR_AFTER_RECORD; // Copied from below. Specifics
                                                                 // don't really matter - this is a
                                                                 // rare case
        if (renewedOverridingAtTime <= 1) renewedOverridingAtTime = 0xFFFFFFFF;
        goto getOut; // Nothing else to do. We don't want to even obey any
                     // automation just now.
      }
    }

    timeSinceOverrideEnd = AudioEngine::audioSampleTimer - (uint32_t)renewedOverridingAtTime -
                           SAMPLES_TO_IGNORE_AFTER_BEGIN_OVERRIDE;

    // If we overrode (turned the knob) less than 0.2s ago and are not
    // recording, then we don't want to obey any automation at all
    if (timeSinceOverrideEnd <= 0 && !playbackHandler.isCurrentlyRecording()) { goto getOut; }

    // if (timeSinceOverrideEnd >=
    // UNINTERPOLATED_NODE_CANCELS_OVERRIDING_AFTER_SAMPLES * 2) goto
    // yesCancelOverriding; // Fix I played with in Aug 2021, seems good - I
    // think that if it's been "quite a while", this new node should just cancel
    // overriding no matter what, right?

    // If not interpolating, let it choose to get out of overriding before
    // looking at doing recording (I'm not 100% sure that this was the best
    // way...)
    shouldCancelOverridingNow =
        mayInterpolate ?
            !nextNodeInOurDirection->interpolated &&
                timeSinceOverrideEnd >=
                    UNINTERPOLATED_NODE_CANCELS_OVERRIDING_AFTER_SAMPLES : // For non-MIDI params,
                                                                           // this actually doesn't
                                                                           // appear necessary and
                                                                           // overriding instead
                                                                           // gets cancelled like 70
                                                                           // lines below. But I've
                                                                           // left it here for
                                                                           // safety. This whole
                                                                           // function is so
                                                                           // complicated...
                                                                           timeSinceOverrideEnd >=
                UNINTERPOLATED_NODE_CANCELS_OVERRIDING_AFTER_SAMPLES; // Whereas
                                                                      // for
                                                                      // MIDI
                                                                      // CCs,
                                                                      // for
    ...
    // Otherwise...
      ...
      // If recording, modify the node we just reached to resemble the
      // drifting-back-to-the-automation that's happening live, now
      if (playbackHandler.isCurrentlyRecording()) {
      recordOverNodeJustReached:

        // Back up state if necessary. It normally would have already been, but
        // not if the user only just activated recording while already
        // overriding!
        Action* action = actionLogger.getNewAction(ACTION_RECORD, true);
        ...
        // I used to add 3 onto the end of this cos it helped with ensuring a
        // nice drift-back when we didn't have any latching. But now we do so
        // it's unnecessary...
        // ...and also, adding any extra constant on here causes latching to
        // sometimes cancel, because an actual record action homogenizes a
        // region that doesn't include that extra constant bit of length, so can
        // cause a next tick to happen soon enough to cause latching to cancel,
        // just below
        ...
        if (insertingNodeAtEndOfClearing) {
          ...
          // Special case for song params for recording session->arrangement.
          // TODO: think about reversing for this
          if (playbackHandler.recording == RECORDING_ARRANGEMENT &&
            ...
            if (nodes.search(1, LESS) >= 0) {
              insertingNodeAtEndOfClearing =
                  false; // Set this to false, but that doesn't mean we want to
          ...
          // Or, normal case - we need to insert a node where the overriding
          // ends
          else {
            if (reversed)
              ticksToClear = -ticksToClear; // From here on, ticksToClear is
                                            // negative if we're reversing. But
                                            // we only use it one more time.
            posOverridingEnds             = nodeJustReached->pos + ticksToClear;
            int32_t posAtWhichClipWillCut = modelStack->getPosAtWhichPlaybackWillCut();

            // If eating past the point where the Clip will cut, just make sure
            // don't put anything past that. And don't interpolate. If recording
            // to arrangement, do this too, and it'll have the reverse effect -
            // extending overriding out to the "end of time" at 2147483647
            if (reversed) {
              if (posOverridingEnds <= posAtWhichClipWillCut) {
                posOverridingEnds = posAtWhichClipWillCut;
                // TODO: could make it not interpolate and set its value to our
                // new value?
              }
        ...
        // Or if next node's actually coming up quite soon, cancel latching if
        // it was on
        else {
          if (renewedOverridingAtTime == 1) {

            // If the upcoming node is non-interpolated, we want no overriding
            // at all so we can jump directly to it
            if (!nextNodeInOurDirection->interpolated) {
              renewedOverridingAtTime = 0;
              // That's how overriding is most often cancelled for non-MIDI
              // params. But for some reason, MIDI CCs, even if we removed their
              // overriding cancellation code above, don't seem affected by
              // this...
            }

            // Or if it is interpolated, we'll just do regular overriding so we
            // can drift into it
            else {
              // Pretend that it began SAMPLES_TO_CLEAR_AFTER_RECORD samples ago
              // - because we had to wait that long to get to this node just now
              // after we recorded a value
              renewedOverridingAtTime =
                  AudioEngine::audioSampleTimer - SAMPLES_TO_CLEAR_AFTER_RECORD;
              if (renewedOverridingAtTime <= 1) renewedOverridingAtTime = 0xFFFFFFFF;
            }
      ...
      adjustNodeJustReached:
        ...
        if (!didPinpong) { nodeJustReached->value = currentValue; }
        nodeJustReached->interpolated = true;
        // TODO: if reversing, should we set the one to the right to
        // interpolating too?
        noNeedToJumpToValue = true;

        bool needToReGetNextNode = false;

        // Having changed that node's value, there's a chance it may have made
        // the node before it redundant
        if (!reversed) {
          needToReGetNextNode =
              deleteRedundantNodeInLinearRun(iJustReached,
                                             effectiveLength); // Shouldn't I make it so this
                                                               // doesn't get called for MPE?
        }

        if (insertingNodeAtEndOfClearing) {
          int iNew = nodes.insertAtKey(posOverridingEnds); // Can only do this now, after updating
                                                           // nodeJustReached, above
          if (iNew != -1) {
            iRight                               = iNew;
            ...
            */
            if (!reversed) {
              needToReGetNextNode = deleteRedundantNodeInLinearRun(
                  iRight,
                  effectiveLength); // If returns false, storage wasn't changed,
                                    // and we've got the next node right here!
                                    // Shouldn't I make it so this doesn't get
                                    // called for MPE?
            }
          }
        }

        // Figure out what's the next node, again - because we just possibly
        // deleted a node, and that's possibly changed the storage
  ...
  // If this node we've just reached wasn't interpolated, and automation is not
  // overridden (which may have only just become the case), we need to jump to
  // the node's value. (Or, it'll be the value of the node to the left if the
  ...
  if ((!noNeedToJumpToValue || mustUpdateValueAtEveryNode) && !renewedOverridingAtTime) {
    ...
    // The call to notifyParamModifiedInSomeWay() below normally has the ability
    // to delete this AutoParam, which we want it not to. It won't if we still
    // contain automation, which I think we have to... Let's just verify that.
#if ALPHA_OR_BETA_VERSION
...
getOut:
  ...
  // Ok, no node should be at or past the effectiveLength. Sometimes somehow
  // this is still happening - see
  // https://forums.synthstrom.com/discussion/4499/v4-0-0-beta8-freeze-while-recording-long-mpe-clips-jjjj
  ...
  if (nodes.getNumElements()) {
    ...
    if (rightmostNode->pos >= effectiveLength) {
      ...
      // numericDriver.freezeWithError("jjjj"); // drbourbon got! And Quixotic7,
      // on V4.0.0-beta8.
    }
  }
...
// You now much check before calling this that interpolation should happen at
// all
void AutoParam::setupInterpolation(ParamNode* nextNodeInOurDirection,
                                   int32_t    effectiveLength,
                                   int32_t    currentPos,
                                   bool       reversed) {

  if (renewedOverridingAtTime == 1)
    return; // If it's latched-until-next-node-hit, we're not allowed to
            // interpolate.
  ...
  // If automation still overridden (at least to some extent), limit how fast
  // interpolation can occur
  if (renewedOverridingAtTime) {

    int32_t timeSinceOverridden = AudioEngine::audioSampleTimer -
    ...
    // Or if still going...
      ...
      // If we didn't even have to limit it, there's no need to be overriding
      // anymore
      else
        renewedOverridingAtTime = 0;
    }
...
bool AutoParam::tickSamples(int numSamples) {
  ...
  int32_t oldValue = currentValue;
  currentValue += multiply_32x32_rshift32_rounded(valueIncrementPerHalfTick,
...
// For MPE when a note gets deleted, and we want to just simply delete nodes and
// let previous ones' value spill into this area.
void AutoParam::deleteNodesWithinRegion(ModelStackWithAutoParam const* modelStack,
  ...
  if (length >= effectiveLength) {
    ...
    if (wrapping) {
      if (resultingIndexes[0]) { nodes.deleteAtIndex(0, resultingIndexes[0]); }
      int numAtEnd = nodes.getNumElements() - resultingIndexes[1];
      if (numAtEnd) { nodes.deleteAtIndex(resultingIndexes[1], numAtEnd); }
    }

    else {
      int numToDelete = resultingIndexes[1] - resultingIndexes[0];
      if (numToDelete) { nodes.deleteAtIndex(resultingIndexes[0], numToDelete); }
    }
...
void AutoParam::setValueForRegion(uint32_t                       pos,
  ...
  // If the user is holding down a pad for an extended NoteRow, which is beyond
  // the length of the Clip, and they're trying to edit this Param for the Clip,
  // well that can't happen because they're then trying to edit beyond the
  ...
  // First, special case if our region covers the whole NoteRow / Clip /
  // TimelineCounter
  if (length == effectiveLength) {
    if (isAutomated()) {
      deleteAutomation(action, modelStack);
  ...
  // Or, normal case
...
#if ALPHA_OR_BETA_VERSION
    nodes.testSequentiality("E441"); // drbourbon got, when check was inside homogenizeRegion(). Now
                                     // trying to work out where that came from. March 2022. Sven
                                     // got, oddly while editing note velocity. Then again by
                                     // "Adding some snares while playing".
#endif
    ...
    mostRecentI =
        nodes.search(modelStack->getLivePos() + !modelStack->isCurrentlyPlayingReversed(), LESS);
...
int AutoParam::homogenizeRegion(ModelStackWithAutoParam const* modelStack,
                                ...
                                int32_t                        effectiveLength,
                                bool                           reversed,
                                int32_t                        posAtWhichClipWillCut) {

#if ALPHA_OR_BETA_VERSION
  // Chasing "E433" / "GGGG" error (probably now largely solved - except got
  // E435, see below).
  if (length <= 0) numericDriver.freezeWithError("E427");
  if (startPos < 0) numericDriver.freezeWithError("E437");
  // nodes.testSequentiality("E435"); // drbourbon got! March 2022. Now moved
  // check to each caller.
  if (nodes.getNumElements() && nodes.getFirst()->pos < 0) numericDriver.freezeWithError("E436");
    // Should probably also check that stuff doesn't exist too far right - but
  ...
  if (!reversed) {
    ...
    if (length >= maxLength) {
      ...
      if (anyWrap)
        edgePositions[REGION_EDGE_RIGHT] = 0; // Gotta wrap it - we're not allowed a node right at
                                              // e.g. the Clip length point.
    }

    // Or if we didn't do that, there could be a loop-point, which we treat
    // almost the same - except we don't let it limit our region length - we
    // just wrap our region end around.
    else {
      edgePositions[REGION_EDGE_RIGHT] = edgePositions[REGION_EDGE_LEFT] + length;
      anyWrap                          = (edgePositions[REGION_EDGE_RIGHT] >= effectiveLength);
      if (anyWrap) {
        // But wait - if we're linearly recording, then it's not so much a
        // "loop" point, because the Clip will extend. So ensure we don't wrap
        // back to the start, past the current loopLength.
  ...
  // Or, playing reversed...
    ...
    // just wrap our region end around.
      ...
      if (anyWrap) { edgePositions[REGION_EDGE_LEFT] += effectiveLength; }
    }
  }
  ...
  // Ok, edgeIndexes and edgePositions are now ordered so as to be accessible
  // with REGION_EDGE_LEFT (0) and REGION_EDGE_RIGHT (1)
  ...
  for (int i = 0; i < 2; i++) {
    if (edgeIndexes[i] < nodes.getNumElements()) {
      ...
      if (potentialEdgeNode->pos == edgePositions[i]) { edgeNodes[i] = potentialEdgeNode; }
    }
  }
  ...
  if (edgeNodes[!reversed]) {
    if (reversed && !edgeNodes[!reversed]->interpolated)
      goto getValueNormalWay; // If reversed, and the node isn't interpolated,
                              // we'd actually need the value of the
                              // further-left node.
  ...
  if (!edgeNodes[REGION_EDGE_RIGHT]) { // If we don't have a rightmost node
                                       // yet...
    ...
    // Otherwise, insert one
      ...
      // Theoretically we'd re-get the other edgeNode here - but in fact, if it
      // already existed, we won't access it again anyway.
    }
  ...
  // Now delete extra nodes. This first bit will delete all of them if no wrap,
  // or the before-wrap bit if there is a wrap.
  ...
  if (numToDelete) {
    ...
    // Theoretically we'd decrease edgeIndexes[REGION_EDGE_RIGHT] if no wrap,
    // but it never gets used again in that case of no-wrap.
  }
...
#if ALPHA_OR_BETA_VERSION
  nodes.testSequentiality("E433"); // Was "GGGG". Leo got. Sven got. (Probably now solved). (Nope,
                                   // Michael got on V4.1.0-alpha10 (OLED)!)
  if (nodes.getNumElements()) {
    ParamNode* rightmostNode = nodes.getElement(nodes.getNumElements() - 1);
    if (rightmostNode->pos >= effectiveLength) { numericDriver.freezeWithError("iiii"); }
  }
#endif
...
void AutoParam::homogenizeRegionTestSuccess(int  pos,
                                            int  regionEnd,
                                            int  startValue,
...
// The reason for specifying whether we're reversing is that at the exact pos of
// a non-interpolating node, where the value abruptly changes, well whether we
// want the value to the left or the right depends on which direction we're
// going.
int32_t
AutoParam::getValueAtPos(uint32_t pos, ModelStackWithAutoParam const* modelStack, bool reversed) {
  ...
  if (ticksSinceLeftNode < 0) { // If pos we're looking at is left of leftmost...
    ...
    if (lengthBeforeLoop == 2147483647) { // If infinite length - and we know we're interpolating -
                                          // well we'd have arrived at the next node value
  ...
  if (ticksBetweenNodes <= 0) { // If pos we're looking at is right of rightmost...
    int32_t lengthBeforeLoop = modelStack->getLoopLength();
    if (lengthBeforeLoop == 2147483647) { // If infinite length, we'll still be on prev node's
                                          // value, despite us interpolating
...
void AutoParam::setPlayPos(uint32_t pos, ModelStackWithAutoParam const* modelStack, bool reversed) {
  ...
  if (nodes.getNumElements()) {
    ...
    ParamNode* nextNodeOurDirection =
        nodes.getElement(rightI); // This will initially point to the node to the right,
                                  // regardless of direction; it'll be corrected to left if we're
                                  // reversed below.
...
int AutoParam::beenCloned(bool copyAutomation, int32_t reverseDirectionWithLength) {
  ...
  if (copyAutomation) {
    ...
    if (reverseDirectionWithLength && numNodes) {
      ParamNodeVector oldNodes = nodes; // Sneakily and temporarily clone this - still pointing to
                                        // the old AutoParam's nodes' memory.
      nodes.init();
      ...
      oldNodes.init(); // Because this is about to get destructed, we need to
                       // stop it pointing to the old AutoParam's nodes' memory,
                       // cos we don't want that getting deallocated.
    }
...
void AutoParam::generateRepeats(uint32_t oldLength, uint32_t newLength, bool shouldPingpong) {
  ...
  // When recording session to arranger, you may occasionally end up with nodes
  // beyond the Clip's length. These need to be removed now
  ...
  if (shouldPingpong) {
    ...
    // We may have to create a new node at pos 0 to represent the fact that a
    // pingpong would suddenly occur in the middle of an interpolating bit
    // between nodes. Actually let's just always put it there, so the beginning
    // doesn't suddenly sound different if we have an odd number of repeats or
    // something.
    if (nothingAtZero) {

      // This block is a quick simple alternative to calling getValueAtPos(),
      // which would also require a modelStack and check for a bunch of
      // unnecessary stuff.
      ParamNode* nodeBeforeWrap = (ParamNode*)nodes.getElementAddress(nodes.getNumElements() - 1);

      bool nodeAfterWrapIsInterpolated =
          nodeAfterWrap->interpolated; // Make copy, cos we need this even after
                                       // our pointer is no longer valid, cos we
                                       // insert below.
      ...
      if (nodeAfterWrapIsInterpolated) {
        ...
        valueAtZero =
            nodeBeforeWrap->value + (valueDistance * ticksSinceLeftNode / ticksBetweenNodes);
      } else {
    ...
    int numRepeats = (uint32_t)(newLength - 1) / oldLength +
                     1; // Rounded up. Including first "repeat", which already exists.
    ...
    for (int r = 1; r < numRepeats; r++) {
      ...
      for (int iNewWithinRepeat = 0; iNewWithinRepeat < numNodesBefore; iNewWithinRepeat++) {
        ...
        // If reversing, we have to change the characteristics given to this
        // node.
        if (r & 1) {
    ...
    if (numToDelete) { nodes.deleteAtIndex(newNumNodes, numToDelete); }
  }
...
void AutoParam::appendParam(AutoParam* otherParam,
  ...
  // We may have to create a new node at pos 0 (of the new repeat) to represent
  // the fact that a pingpong would suddenly occur in the middle of an
  // interpolating bit between nodes. Hopefully the note at actual pos 0 got
  // created back when generateRepeats got called initially for this, at the
  // start of recording etc.
  if (pingpongingGenerally && nothingAtZero && nodeAfterWrap->interpolated) {
    ...
    // nothingAtZero = false; // Unlike in generateRepeats(), above, the node
    // we've added is not a part of the same array that represents our source
    // material.
  }
...
void AutoParam::deleteNodesBeyondPos(int32_t pos) {
  ...
  if (numToDelete) { nodes.deleteAtIndex(i, numToDelete); }
}
...
void AutoParam::trimToLength(uint32_t                       newLength,
  ...
  // To ensure that the effective value at pos 0 remains the same even after
  // earlier nodes deleted, we might need to add a new, non-interpolating node
  // there.
  bool needNewNodeAt0 =
      nodes.getFirst()->pos; // Deactivated for now, but I'm going to enable in the
                             // ModelStacks branch, where we have a TimelineCounter here.
  int32_t oldValueAt0;
  ...
  // If still at least 2 nodes afterwards (1 is not allowed, actually wait it is
  // now but let's keep this safe for now)...
  if (newNumNodes >= 2) {
    ...
    addNewNodeAt0IfNecessary:
      if (needNewNodeAt0) {
        int error = nodes.insertAtIndex(0);
        if (!error) { // Should be fine cos we just deleted some, so some free
                      // RAM
    ...
    // Or if action...
      ...
      // Or, if we need to snapshot, work with that
        ...
        // We've kept the original Nodes separate in memory, so can steal them
        // into an undo-accessible snapshot
  ...
  // Or if no nodes afterwards
    ...
    if (action) action->recordParamChangeIfNotAlreadySnapshotted(modelStack,
                                                                 true); // Steal
    nodes.empty();                 // Delete them - either if no action, or if the above chose
                                   // not to steal them.
...
// Returns error code.
// If you're gonna call this, you probably need to tell the ParamSet that this
// Param has automation now, if it does. Or, to make things easier, you should
// just call the ParamSet instead, if possible.
int AutoParam::readFromFile(int32_t readAutomationUpToPos) {

  // Must first delete any automation because sometimes, due to that annoying
  // support I have to do for late-2016 files, we'll be overwriting a cloned
  // ParamManager, which might have had automation.
  ...
  if (*(uint16_t*)firstChars != 'x0') {
    ...
    for (int i = 2; i < 12 && (buffer[i] = storageManager.readNextCharOfTagOrAttributeValue());
         i++) {}
    buffer[11]   = 0;
    ...
    while (true) {
      ...
      if (numElementsToAllocateFor <= 0) {
        ...
        if (charsRemaining) {
          ...
          // Allocate space for the right number of notes, and remember how long
          // it'll be before we need to do this check again
          numElementsToAllocateFor = (uint32_t)(charsRemaining - 1) / 16 + 1;
          nodes.ensureEnoughSpaceAllocated(numElementsToAllocateFor); // If it returns false... oh
      ...
      // If we've reached the end of our allowed timeline length for
      // automation...
      if (pos >= readAutomationUpToPos) {

        // If there's a node actually right on the end-point - well, firmware
        // <= 3.1.5 sometimes put one there when it should have been at pos 0.
        // So, reinterpret that data to make it right.
        if (pos == readAutomationUpToPos) {
...
void AutoParam::paste(int32_t                        startPos,
  ...
  // Think about whether we want to insert a "restart node" - one at the endpos
  // (or the endpos wrapped back to 0)
  int32_t restartPos;

  // If our pasting region reaches the end of the Clip/NoteRow...
  if (endPos >= effectiveLength) {

    // Well if our pasting region also begins right at the start of the
    // Clip/NoteRow, then we're overwriting everything, so don't need a restart
    // node.
...
finishedConsideringRestartNode:
  ...
  if (numToDelete > 0) { nodes.deleteAtIndex(iDeleteBegin, numToDelete); }
  ...
  for (int n = 0; n < copiedParamAutomation->numNodes; n++) {
    ...
    // previous node - or past the end of the screen / Clip
...
void AutoParam::copy(int32_t                        startPos,
  ...
  if (copiedParamAutomation->numNodes > 0) {
    ...
    copiedParamAutomation->nodes = (ParamNode*)generalMemoryAllocator.alloc(
...
// Or it'd ideally be 18 for 14-bit pitch bend data, but that'd be a bit
// overkill.
int AutoParam::makeInterpolationGoodAgain(int32_t clipLength, int quantizationRShift) {
  ...
  for (int i = 0; i < stopAtElement; i++) {
    ...
    if (thisNode->interpolated) {
      ...
      // This function deals with "small" values, which for CCs will be between
      // -64 and 64. Yup, they're bidirectional.
      ...
      for (int j = 1; j < maxJ; j++) {
        ...
        if (newSmallValue == (1 << (31 - quantizationRShift)))
          newBigValue = 2147483647; // E.g. if a CC value has come out as high
                                    // as 64, make sure it fits into the 32-bit
                                    // signed number when we left-shift.
        else
...
void AutoParam::deleteTime(int32_t                  startPos,
  ...
  // No need to do any revertability with an Action here -
  // ParamCollection::backUpAllAutomatedParamsToAction() should have already
  // been called.
  ...
  if (numToDelete > 0) {
    ...
    // We might want to put a new node right at the cut-point if not already one
    // there

    bool    shouldAddNodeAtPos0 = false;
    int32_t oldValue;

    // If we're chopping off the final node, we'll want to put one at pos 0 if
    // none there
    if (end >= nodes.getNumElements()) {
      shouldAddNodeAtPos0 = nodes.getFirst()->pos;
      if (shouldAddNodeAtPos0) { oldValue = getValueAtPos(0, modelStack); }
    }

    // Or, if we're not chopping off the final node, we'll want to put one at
    // end of deleted region (which becomes the same as the start), if none
    // there
    ...
    if (shouldAddNodeAtPos0) {
      int error = nodes.insertAtIndex(0); // Shouldn't ever fail as we told it not to
                                          // shorten its memory previously
      if (!error) {
        ParamNode* newNode    = nodes.getElement(0);
        newNode->value        = oldValue;
        newNode->pos          = 0;
        newNode->interpolated = false;
        start++; // Cos we've shifted everything along in the list by inserting
                 // at index 0
      }
    }
  }
...
allDeleted:
  ...
  // If only one node left, that's not allowed, so delete that too. Actually it
  // is allowed now, but let's keep this safe
  if (nodes.getNumElements() == 1) { nodes.deleteAtIndex(0); }
}
...
void AutoParam::moveRegionHorizontally(ModelStackWithAutoParam const* modelStack,
  ...
  if (action) { action->recordParamChangeDefinitely(modelStack, false); }
  ...
  if (endPos > lengthBeforeLoop) { // Wrap
    ...
    if (resultingIndexes[1] != resultingIndexes[0]) { // Hmm I don't think we quite want to do this
                                                      // check...

      // If moving them right, that's pretty easy. Nothing can even wrap because
      // our moving bit slams into a brick wall to the right.
      if (offset == 1) {
      ...
      // Moving left
        ...
        // Or if we don't have space to the left...
          ...
          if (leftMostNode->pos) {
            ...
            // Well actually, if it's right there, we'll just reuse it and
            // delete our old one
            if (lastNode->pos == lengthBeforeLoop - 1) {
              *lastNode = *leftMostNode;
            }

            // Otherwise, have to delete our old one and put a new one at the
            // end.
            else {
              ParamNode tempNode = *leftMostNode;
              nodes.deleteAtIndex(0, 1, false);
              nodes.insertAtIndex(lastNodeIndex); // Yes, lastNodeIndex is still correct. And,
                                                  // this really shouldn't be able to give an
                                                  // error...
            ...
            resultingIndexes[1]--; // There's one less node we'll have to move
                                   // below
          }
        }
    ...
    if (!node->interpolated) {
      ...
      if (newNodePos < 0) {
        ...
        nextNodeI = nodes.getNumElements() -
                    2; // Subtract 2 instead of the normal 1 cos we're about to delete
      ...
      doWrap:
        ...
        // If that next node is at the pos we're wanting to nudge to, and would
        // hence get deleted, we can just copy to it
        if (nextNode->pos == newNodePos) {

          // But if that would all mean that we'd actually end up with only 1
          // node, well that's not allowed. (Actually it is now, but let's be
          // safe.)
          if (nodes.getNumElements() == 1) {
            nodes.empty();
          }

          // But yeah normally that'll be fine - just go copy to that node we've
          // collided with
          else
            goto setNodeValue;
        }
        ...
        // Otherwise, create a new node
          ...
          nextNodeI = nodes.getNumElements();
          {
            int error = nodes.insertAtIndex(nextNodeI); // This shouldn't be able to fail, cos we
                                                        // just deleted a node
      ...
      // Or if no wrapping
        ...
        // If no previously existing nodes further in that direction at all,
        // that's easy
        if (nextNodeI < 0 || nextNodeI >= nodes.getNumElements()) {
        }

        // Or if some node, have a look at it, and delete it if it's been
        // collided with
        else {
          ParamNode* nextNode = (ParamNode*)nodes.getElementAddress(nextNodeI);
          if (nextNode->pos == newNodePos) { nodes.deleteAtIndex(nextNodeI); }
        }
      }
    }
  }
...
void AutoParam::stealNodes(ModelStackWithAutoParam const* modelStack,
  ...
  if (stolenNodeRecord) {
    ...
    if (numNodesToStealTotal) {
      ...
      void* memory =
          generalMemoryAllocator.alloc(numNodesToStealTotal * sizeof(ParamNode), NULL, false, true);
      if (memory) {
  ...
  if (numNodesToStealAfterWrap) { nodes.deleteAtIndex(0, numNodesToStealAfterWrap); }
...
void AutoParam::insertStolenNodes(ModelStackWithAutoParam const* modelStack,
  ...
  for (int sourceI = 0; sourceI < stolenNodeRecord->num; sourceI++) {
    ...
    if (stolenNode->pos >= regionLength)
      break; // If our destination region is shorter than that of the stolen
             // nodes
...
// Maybe I ought to make this take cut-points into consideration... but I can't
// see a need yet...
