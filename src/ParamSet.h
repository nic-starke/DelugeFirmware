...
// ParamSet specifies a lot of stuff about how the params will be stored -
// there's always a fixed number, and they don't need other info stored besides
// their index - like MIDI CC, or patch cable details. This differs from other
// inheriting classes of ParamCollection.
...
public:
  ...
  void    processCurrentPos(ModelStackWithParamCollection* modelStack,
                            int                            ticksSkipped,
                            bool                           reversed,
                            bool                           didPingpong,
                            bool                           mayInterpolate) final;
  void    writeParamAsAttribute(char const* name,
                                int         p,
  ...
  void    generateRepeats(ModelStackWithParamCollection* modelStack,
                          uint32_t                       oldLength,
                          uint32_t                       newLength,
  ...
  void    appendParamCollection(ModelStackWithParamCollection* modelStack,
                                ...
                                bool                           pingpongingGenerally) final;
  ...
  void    trimToLength(uint32_t                       newLength,
                       ModelStackWithParamCollection* modelStack,
                       Action*                        action,
                       bool                           maySetupPatching) final;
  void    nudgeNonInterpolatingNodesAtPos(int32_t                        pos,
                                          int                            offset,
  ...
  void shiftHorizontally(ModelStackWithParamCollection* modelStack,
                         int32_t                        amount,
                         int32_t                        effectiveLength) final;
  ...
  void
  deleteTime(ModelStackWithParamCollection* modelStack, int32_t startPos, int32_t lengthToDelete);
  ...
  ModelStackWithAutoParam* getAutoParamFromId(ModelStackWithParamId* modelStack,
                                              bool                   allowCreation = true) final;
  void                     notifyParamModifiedInSomeWay(ModelStackWithAutoParam const* modelStack,
                                                        int32_t                        oldValue,
  ...
  AutoParam params[1]; // Total hack - we declare this last, then the subclasses
                       // "extend" it by having extra unused space after it
...
public:
  ...
  int getNumParams() {
    return MAX_NUM_UNPATCHED_PARAMS;
  }
...
public:
  ...
  int getNumParams() {
    return NUM_EXPRESSION_DIMENSIONS;
  }
  void notifyParamModifiedInSomeWay(ModelStackWithAutoParam const* modelStack,
                                    int32_t                        oldValue,
                                    bool                           automationChanged,
  ...
  void    moveRegionHorizontally(ModelStackWithParamCollection* modelStack,
                                 int32_t                        pos,
                                 int32_t                        length,
                                 int                            offset,
                                 int32_t                        lengthBeforeLoop,
  ...
  // bendRanges being stored here in ExpressionParamSet still seems like the
  // best option. I was thinking storing them in the ParamManager would make
  // more sense, except for one thing
  // - persistence when preset/Instrument changes. ExpressionParamSets do this
  // unique thing where they normally aren't "stolen" or "backed up" - unless
  // the last Clip is being deleted, in which case they do move to the
  // backedUpParamManager. This is exactly the persistence we want for
  // bendRanges too.
