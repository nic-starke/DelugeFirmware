...
// These numbers are what get stored just in the internal Deluge flash memory to
// represent things.
...
public:
  MIDIInputChannel() {
    bendRange = 0; // Means not set; don't copy value. Also, note this is the
                   // "main" bend range; there isn't one for finger-level
                   // because this is a non-MPE single MIDI channel.
...
// These never get destructed. So we're safe having various Instruments etc
...
public:
  ...
  MIDIPort ports[2]; // I think I used an array here so the settings menu could
                     // deal with either one easily - which doesn't seem like a
                     // very strong reason really...

  // These are stored as full-range 16-bit values (scaled up from 7 or 14-bit
  // MIDI depending on which), and you'll want to scale this up again to 32-bit
  // to use them. X and Y may be both positive and negative, and Z may only be
  // positive (so has been scaled up less from incoming bits). These default to
  // 0. These are just for MelodicInstruments. For Drums, the values get stored
  // in the Drum itself.
  ...
  uint8_t connectionFlags; // 0 if not connected. For USB devices, the bits
                           // signal a connection of the corresponding
                           // connectedUSBMIDIDevices[]. Of course there'll
                           // usually just be one bit set, unless two of the
                           // same device are connected.

protected:
  virtual void writeReferenceAttributesToFile() = 0; // These go both into MIDIDEVICES.XML
                                                     // and also any song/preset files where
                                                     // there's a reference to this Device.
