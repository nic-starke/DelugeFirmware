...
void LivePitchShifterPlayHead::render(int32_t* __restrict__ outputBuffer,
                                      int      numSamples,
                                      int      numChannels,
  ...
  if (mode == PLAY_HEAD_MODE_REPITCHED_BUFFER) {
    do {
      ...
      *outputBuffer +=
          multiply_32x32_rshift32_rounded(repitchedBuffer[repitchedBufferReadPos * numChannels],
                                          amplitude)
          << 5;
      outputBuffer++;
      ...
      if (numChannels == 2) {
        *outputBufferR +=
            multiply_32x32_rshift32_rounded(repitchedBuffer[repitchedBufferReadPos * 2 + 1],
                                            amplitude)
            << 5;
        outputBufferR++;
      }
    ...
    do {
      ...
      if (numSamplesToJumpForward) {
        ...
        if (numSamplesToJumpForward > INTERPOLATION_MAX_NUM_SAMPLES) {
          rawBufferReadPos =
              (rawBufferReadPos + (numSamplesToJumpForward - INTERPOLATION_MAX_NUM_SAMPLES)) &
              (INPUT_RAW_BUFFER_SIZE - 1);
          numSamplesToJumpForward = INTERPOLATION_MAX_NUM_SAMPLES; // Shouldn't be necesssary, but
                                                                   // for some reason this seems to
                                                                   // do some optimization and speed
                                                                   // things up. Re-test?
        }
        ...
        while (numSamplesToJumpForward--) {
          ...
          if (numChannels == 2) {
            interpolationBuffer[1][0][numSamplesToJumpForward] =
                rawBuffer[rawBufferReadPos * 2 + 1] >> 16;
          }
    ...
    do {
      ...
      *outputBuffer +=
          multiply_32x32_rshift32_rounded(rawBuffer[rawBufferReadPos * numChannels], amplitude)
          << 4;
      outputBuffer++;

      if (numChannels == 2) {
        *outputBuffer +=
            multiply_32x32_rshift32_rounded(rawBuffer[rawBufferReadPos * 2 + 1], amplitude) << 4;
        outputBuffer++;
      }
      rawBufferReadPos = (rawBufferReadPos + 1) & (INPUT_RAW_BUFFER_SIZE - 1);
    } while (outputBuffer != outputBufferEnd);
...
// Returns how much longer (in raw samples) this play-head could play for before
// it reaches "now" time (which is itself moving forward) and runs out of audio
...
#if INPUT_ENABLE_REPITCHED_BUFFER
  if (mode == PLAY_HEAD_MODE_REPITCHED_BUFFER) {
    howFarBack = (uint32_t)(repitchedBufferWritePos - repitchedBufferReadPos) &
                 (INPUT_REPITCHED_BUFFER_SIZE - 1);
  } else
#endif
      if (mode == PLAY_HEAD_MODE_RAW_REPITCHING) {
    uint32_t howFarBackRaw =
        (uint32_t)(liveInputBuffer->numRawSamplesProcessed - rawBufferReadPos) &
        (INPUT_RAW_BUFFER_SIZE - 1);
...
#if INPUT_ENABLE_REPITCHED_BUFFER
  if (mode == PLAY_HEAD_MODE_REPITCHED_BUFFER) {
    uint32_t howFarBackRepitched =
        (uint32_t)(livePitchShifter->repitchedBufferWritePos - repitchedBufferReadPos) &
        (INPUT_REPITCHED_BUFFER_SIZE - 1);
    return ((uint64_t)howFarBackRepitched * phaseIncrement) >> 24;
  } else
#endif
      if (mode == PLAY_HEAD_MODE_RAW_REPITCHING) {
    return (uint32_t)(liveInputBuffer->numRawSamplesProcessed - rawBufferReadPos) &
           (INPUT_RAW_BUFFER_SIZE - 1);
  }
...
void LivePitchShifterPlayHead::fillInterpolationBuffer(LiveInputBuffer* liveInputBuffer,
  ...
  for (int c = 0; c < numChannels; c++) {
    for (int i = 1; i <= INTERPOLATION_MAX_NUM_SAMPLES; i++) {
      ...
      interpolationBuffer[c][0][i - 1] =
          (pos < liveInputBuffer->numRawSamplesProcessed) ?
...
void LivePitchShifterPlayHead::interpolate(int32_t* sampleRead,
                                           int      numChannelsNow,
                                           int      whichKernel) {
#include "interpolate.h"
}

void LivePitchShifterPlayHead::interpolateLinear(int32_t* sampleRead,
                                                 int      numChannelsNow,
                                                 int      whichKernel) {
