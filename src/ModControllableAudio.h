...
public:
  ...
  void         processReverbSendAndVolume(StereoSample* buffer,
                                          int           numSamples,
  ...
  int          readTagFromFile(char const*              tagName,
                               ...
                               Song*                    song);
  void         processSRRAndBitcrushing(StereoSample* buffer,
                                        int           numSamples,
                                        int32_t*      postFXVolume,
                                        ParamManager* paramManager);
  static void  writeParamAttributesToFile(ParamManager* paramManager,
                                          bool          writeAutomation,
                                          int32_t*      valuesForOverride = NULL);
  static void  writeParamTagsToFile(ParamManager* paramManager,
                                    bool          writeAutomation,
                                    int32_t*      valuesForOverride = NULL);
  ...
  bool         offerReceivedCCToLearnedParams(MIDIDevice*                    fromDevice,
                                              uint8_t                        channel,
                                              uint8_t                        ccNumber,
                                              uint8_t                        value,
  ...
  bool         offerReceivedPitchBendToLearnedParams(MIDIDevice*                    fromDevice,
                                                     ...
                                                     ModelStackWithTimelineCounter* modelStack,
                                                     int                            noteRowIndex = -1);
  virtual bool learnKnob(MIDIDevice*     fromDevice,
...
protected:
  void    processFX(StereoSample*      buffer,
                    int                numSamples,
                    int                modFXType,
                    int32_t            modFXRate,
                    int32_t            modFXDepth,
...
private:
  void                           initializeSecondaryDelayBuffer(int32_t newNativeRate,
                                                                bool    makeNativeRatePreciseRelativeToOtherBuffer);
  void                           doEQ(bool     doBass,
                                      bool     doTreble,
                                      int32_t* inputL,
                                      int32_t* inputR,
                                      int32_t  bassAmount,
