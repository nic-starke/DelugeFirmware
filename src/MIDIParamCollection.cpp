...
MIDIParamCollection::MIDIParamCollection(ParamCollectionSummary* summary)
  ...
  summary->whichParamsAreAutomated[0] =
      1; // Just to indicate there could be some automation, cos we don't
         // actually use this variable properly.
...
void MIDIParamCollection::deleteAllParams(Action* action, bool deleteStorageToo) {
  for (int i = 0; i < params.getNumElements(); i++) {
    ...
    if (action) { midiParam->param.deleteAutomationBasicForSetup(); }
    midiParam->~MIDIParam();
  }
...
void MIDIParamCollection::beenCloned(bool copyAutomation, int32_t reverseDirectionWithLength) {
  ...
  // And now, copy the memory for the automation data that each member of params
  // references
  for (int i = 0; i < params.getNumElements(); i++) {
...
void MIDIParamCollection::setPlayPos(uint32_t                       pos,
  ...
  // Bend param is the only one which is actually gonna maybe want to set up
  // some interpolation - but for the other ones we still need to initialize
  // them and crucially make sure automation overriding is switched off
  for (int i = 0; i < params.getNumElements(); i++) {
    MIDIParam*               midiParam = params.getElement(i);
    AutoParam*               param     = &midiParam->param;
    ModelStackWithAutoParam* modelStackWithAutoParam =
        modelStack->addAutoParam(midiParam->cc, param);
...
void MIDIParamCollection::appendParamCollection(ModelStackWithParamCollection* modelStack,
  ...
  MIDIParamCollection* otherMIDIParamCollection =
      (MIDIParamCollection*)otherModelStack->paramCollection;
  for (int i = 0; i < otherMIDIParamCollection->params.getNumElements(); i++) {
    ...
    if (j != -1) {
      ...
      midiParam->param.appendParam(&otherMidiParam->param,
                                   oldLength,
...
void MIDIParamCollection::processCurrentPos(ModelStackWithParamCollection* modelStack,
  ...
  if (ticksTilNextEvent <= 0) {
    ...
    for (int i = 0; i < params.getNumElements(); i++) {
      ...
      int32_t ticksTilNextEventThisParam = param->processCurrentPos(modelStackWithAutoParam,
                                                                    reversed,
                                                                    didPingpong,
                                                                    false,
                                                                    true); // No interpolating
  ...
  for (int i = 0; i < params.getNumElements(); i++) {
    ...
    if (midiParam->param.isAutomated()) {
      ModelStackWithAutoParam* modelStackWithParam =
          modelStack->addAutoParam(midiParam->cc, &midiParam->param);
...
ModelStackWithAutoParam* MIDIParamCollection::getAutoParamFromId(ModelStackWithParamId* modelStack,
  ...
  MIDIParam* midiParam =
      params.getOrCreateParamFromCC(modelStack->paramId, defaultValue, allowCreation);
  AutoParam* param = NULL;
  if (midiParam) param = &midiParam->param;
  return modelStack->addAutoParam(param);
}

void MIDIParamCollection::sendMIDI(int     masterChannel,
                                   int     cc,
                                   int32_t newValue,
  ...
  midiEngine.sendCC(masterChannel,
                    cc,
                    newValueSmall + 64,
...
void MIDIParamCollection::grabValuesFromPos(uint32_t                       pos,
  ...
  for (int i = 0; i < params.getNumElements(); i++) {
    ...
    // With MIDI, we only want to send these out if the param is actually
    // automated and the value is actually different
...
void MIDIParamCollection::nudgeNonInterpolatingNodesAtPos(
    int32_t                        pos,
    int                            offset,
...
void MIDIParamCollection::notifyParamModifiedInSomeWay(ModelStackWithAutoParam const* modelStack,
  ...
  if (modelStack->song->isOutputActiveInArrangement((MIDIInstrument*)modelStack->modControllable)) {
    ...
    if (currentValueChanged) {
      ...
      sendMIDI(masterChannel,
               modelStack->paramId,
               modelStack->autoParam->getCurrentValue(),
               midiOutputFilter);
    }
  }
}
