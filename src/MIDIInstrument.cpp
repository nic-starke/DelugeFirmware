...
// Returns whether any change made. For MIDI Instruments, this has no
// consequence
...
bool MIDIInstrument::doesAutomationExistOnMIDIParam(ModelStackWithThreeMainThings* modelStack,
  ...
  ModelStackWithAutoParam* modelStackWithAutoParam =
      getParamToControlFromInputMIDIChannel(cc, modelStack);
...
void MIDIInstrument::modButtonAction(uint8_t                  whichModButton,
  ...
  // Otherwise, if we're leaving this mod function or anything else is
  // happening, we want to be sure that stutter has stopped
  if (currentUIMode == UI_MODE_SELECTING_MIDI_CC) {
    currentUIMode = UI_MODE_NONE;
#if HAVE_OLED
    OLED::removePopup();
...
MIDIInstrument::getParamFromModEncoder(int                            whichModEncoder,
                                       ModelStackWithThreeMainThings* modelStack,
                                       bool                           allowCreation) {

  if (!modelStack->paramManager) { // Could be NULL - if the user is holding down an
                                   // audition pad in Arranger, and we have no Clips
  noParam:
...
ModelStackWithAutoParam*
MIDIInstrument::getParamToControlFromInputMIDIChannel(int                            cc,
                                                      ModelStackWithThreeMainThings* modelStack) {
  ...
  return summary->paramCollection->getAutoParamFromId(
      ...
      true); // Yes we do want to force creating it even if we're not recording
             // - so the level indicator can update for the user
}
...
bool MIDIInstrument::writeDataToFile(Clip* clipForSavingOutputOnly, Song* song) {
  // NonAudioInstrument::writeDataToFile(clipForSavingOutputOnly, song); //
  // Nope, this gets called within the below call
  ...
  if (editedByUser) { // Otherwise, there'll be nothing in here
    ...
    if (clipForSavingOutputOnly || !midiInput.containsSomething())
      return false; // If we don't need to write a "device" tag, opt not to end
                    // the opening tag
...
bool MIDIInstrument::readTagFromFile(char const* tagName) {
  ...
  if (!strcmp(tagName, "modKnobs")) {
    readModKnobAssignmentsFromFile(
        MAX_SEQUENCE_LENGTH); // Not really ideal, but we don't know the number
                              // and can't easily get it. I think it'd only be
                              // relevant for pre-V2.0 song file... maybe?
...
// paramManager is sometimes NULL (when called from the above function), for
// reasons I've kinda forgotten, yet everything seems to still work...
...
int MIDIInstrument::moveAutomationToDifferentCC(int                            oldCC,
  ...
  ModelStackWithAutoParam* modelStackWithAutoParam =
      getParamToControlFromInputMIDIChannel(oldCC, modelStack);
  ...
  MIDIParamCollection* midiParamCollection =
      modelStackWithAutoParam->paramManager->getMIDIParamCollection();
  ...
  // Expression param
    ...
    if (modelStackWithAutoParam->paramCollection !=
        modelStack->paramManager->getExpressionParamSet())
...
int MIDIInstrument::moveAutomationToDifferentCC(int                            offset,
  ...
  // Need to pick a new cc which is blank on all Clips' ParamManagers with this
  // Instrument For each Clip in session and arranger for specific Output (that
  // Output is "this")
...
traverseClips:
  ...
  // And then tell all Clips' ParamManagers with this Instrument to change that
  // CC For each Clip in session and arranger for specific Output (that Output
  // is "this")
...
void MIDIInstrument::offerReceivedNote(ModelStackWithTimelineCounter* modelStackWithTimelineCounter,
  ...
  if (midiInput.channelOrZone == receivedChannel) {
    ...
    // If it's a MIDI Clip, and it's outputting on the same channel as this MIDI
    // message came in, don't do MIDI thru!
    if (doingMidiThru && type == INSTRUMENT_TYPE_MIDI_OUT &&
        receivedChannel == channel) { // We'll just say don't do anything to midi-thru if any
                                      // MPE in the picture, for now
      *doingMidiThru = false;
    }
  }

  NonAudioInstrument::offerReceivedNote(modelStackWithTimelineCounter,
                                        fromDevice,
                                        on,
                                        receivedChannel,
                                        note,
...
void MIDIInstrument::noteOnPostArp(int noteCodePostArp, ArpNote* arpNote) {
  ...
  if (activeClip) { arpSettings = &((InstrumentClip*)activeClip)->arpSettings; }
  ...
  // Or if MPE, we have to decide on a member channel to assign note to...
    ...
    int lowestMemberChannel =
        (channel == MIDI_CHANNEL_MPE_LOWER_ZONE) ?
            1 :
            MIDIDeviceManager::highestLastMemberChannelOfUpperZoneOnConnectedOutput;
    int highestMemberChannel =
        (channel == MIDI_CHANNEL_MPE_LOWER_ZONE) ?
            MIDIDeviceManager::lowestLastMemberChannelOfLowerZoneOnConnectedOutput :
            14;

    uint8_t numNotesPreviouslyActiveOnMemberChannel[15];
    memset(numNotesPreviouslyActiveOnMemberChannel,
           0,
           sizeof(numNotesPreviouslyActiveOnMemberChannel));

    int outputMemberChannelWithNoteSharingInputMemberChannel = 16; // 16 means "none", I think

    if (!arpIsOn) {
      // Count up notes per member channel. This traversal will *not* find the
      // new note that we're switching on, which will have had its toMIDIChannel
      // set to 16 by Arpeggiator (we'll decide and set it below).
      for (int n = 0; n < arpeggiator.notes.getNumElements(); n++) {
        ...
        if (thisArpNote->outputMemberChannel >= 1 && thisArpNote->outputMemberChannel <= 14) {
          ...
          // If this note is coming in live from the same member channel as the
          // one we wish to switch on now, that's a good clue that we should
          // group them together at the output. (Final decision to be made
          // further below.)
    ...
    // See which member channel fits criteria best. The MPE spec has guidelines
    // about what criteria to use.
    ...
    for (int c = lowestMemberChannel; c <= highestMemberChannel; c++) {
      ...
      uint32_t goodness =
          ((256 - numNotesPreviouslyActiveOnMemberChannel[c]) << 16) | timeSinceNoteOff;
    ...
    // If we weren't able to get an output member channel all to ourselves, a
    // better option (if it exists) would be to group with note(s) which shared
    // an input member channel
    ...
    // TODO: It'd be good to be able to group them according to them having
    // similar MPE data, which could happen if they were originally recorded via
    // the same input member channel. This would actually be really easy to do
    // reasonably well.

    // Ok. We have our new member channel.

    arpNote->outputMemberChannel = outputMemberChannel;
    arpeggiator.outputMIDIChannelForNoteCurrentlyOnPostArp =
        outputMemberChannel; // Needed if arp on

    int16_t mpeValuesAverage[NUM_EXPRESSION_DIMENSIONS]; // We may fill this up and
                                                         // point to it, or not
    int16_t const* mpeValuesToUse = arpNote->mpeValues;

    // If other notes are already being output on this member channel, we'll
    // need to average MPE values and stuff
    if (numNotesPreviouslyActiveOnMemberChannel[outputMemberChannel]) {
      int     numNotesFound = 0;
      int32_t mpeValuesSum[NUM_EXPRESSION_DIMENSIONS]; // We'll be summing
                                                       // 16-bit values into
                                                       // this 32-bit container,
                                                       // so no overflowing
      memset(mpeValuesSum, 0, sizeof(mpeValuesSum));

      for (int n = 0; n < arpeggiator.notes.getNumElements();
           n++) { // This traversal will include the original note, which will
                  // get counted up too
      ...
      // There's a chance that this new average will be the same as whatever
      // previous ones were output on this member channel. We could check for
      // this and omit re-sending them in that case, but there's very little to
      // be gained by that added complexity.
    }

    // Ok, now we'll output MPE values - which will either be the values for
    // this exact note we're outputting, or if it's sharing a member channel,
    // it'll be the average values we worked out above.
...
// And obviously you can't call this unless you know that this Instrument
// sendsToMPE().
void MIDIInstrument::outputAllMPEValuesOnMemberChannel(int16_t const* mpeValuesToUse,
                                                       int            outputMemberChannel) {
  { // X
    ...
    midiEngine.sendPitchBend(outputMemberChannel,
                             outputValue14Unsigned & 127,
                             outputValue14Unsigned >> 7,
                             channel);
  }
...
void MIDIInstrument::noteOffPostArp(int noteCodePostArp, int oldOutputMemberChannel, int velocity) {
  ...
  // Or, MPE
    ...
    if (oldOutputMemberChannel <= 14) {
      ...
      // And now, if this note was sharing a member channel with any others, we
      // want to send MPE values for those new averages
      int     numNotesFound = 0;
      int32_t mpeValuesSum[NUM_EXPRESSION_DIMENSIONS]; // We'll be summing
                                                       // 16-bit values into
                                                       // this 32-bit container,
      ...
      for (int n = 0; n < arpeggiator.notes.getNumElements();
           n++) { // This traversal will not include the original note, which
                  // has already been deleted from the array.
        ArpNote* lookingAtArpNote = (ArpNote*)arpeggiator.notes.getElementAddress(n);
...
void MIDIInstrument::allNotesOff() {
  ...
  // Otherwise, got to send message on all MPE member channels. At least I think
  // that's right. The MPE spec talks about sending "all *sounds* off" on just
  // the master channel, but doesn't mention all *notes* off.
  else {
...
// The arpNote actually already contains a stored version of value32 - except
// it's been reduced to 16-bit, so we may as well use the 32-bit version here.
// Although, could it have even got more than 14 bits of meaningful value in the
// first place?
void MIDIInstrument::polyphonicExpressionEventPostArpeggiator(int      value32,
  ...
  if (!sendsToMPE()) {
    ...
    if (whichExpressionDimension == 2) {
      midiEngine.sendPolyphonicAftertouch(channel,
                                          value32 >> 24,
  ...
  // Or if we do have MPE output...
    ...
    if (!settings || !settings->mode) { // Only if not arpeggiating...
      int     numNotesFound = 0;
      int32_t mpeValuesSum  = 0; // We'll be summing 16-bit values into this
                                 // 32-bit container, so no overflowing
      ...
      if (numNotesFound > 1) {
        ...
        int lastValue7Or14    = whichExpressionDimension ?
                                    mpeOutputMemberChannels[memberChannel]
                                     .lastYAndZValuesSent[whichExpressionDimension - 1] :
    ...
    switch (whichExpressionDimension) {
      case 0: { // X
        ...
        midiEngine.sendPitchBend(memberChannel,
                                 value14Unsigned & 127,
                                 value14Unsigned >> 7,
                                 channel);
        break;
