...
void MemoryRegion::sanityCheck() {
  ...
  for (int j = 0; j < emptySpaces.getNumElements(); j++) {
    ...
    if (emptySpaceRecord->address == (uint32_t)0xc0080bc) { count++; }
  }
...
// Okay this is me being experimental and trying something you're not supposed
// to do - using static variables in place of stack ones within a function. It
// seemed to give a slight speed up, but it's probably quite circumstantial, and
...
// Specify the address and size of the actual memory region not including its
// headers, which this function will write and don't have to contain valid data
// yet. spaceSize can even be 0 or less if you know it's going to get merged.
inline void MemoryRegion::markSpaceAsEmpty(uint32_t address,
                                           uint32_t spaceSize,
                                           bool     mayLookLeft,
                                           bool     mayLookRight) {
  ...
  if (mayLookLeft) {
    ...
    if ((*lookLeft & SPACE_TYPE_MASK) == SPACE_HEADER_EMPTY) {
      ...
      // Set up the default option - that we are going to merge with the left
      // record. This may get overridden below.
      recordToMergeWith = &emptySpaceToLeft;

      // If we're not allowed to also look right, or there's no unused space
      // there, we want to just go directly to replacing this old record
      ...
      // If we are still here, we are going to merge right as well as left, so
      // gather a little bit more info
      ...
      EmptySpaceRecord* recordToDelete =
          &emptySpaceToRight; // The default, which we may override below.
      ...
      if (emptySpaceToRight.length > emptySpaceToLeft.length ||
          (emptySpaceToRight.length == emptySpaceToLeft.length &&
           emptySpaceToRight.address >
               address)) { // address here is same as, and best thought of as,
                           // emptySpaceToLeft.address. Hang on, wouldn't the
                           // space to the right always have a higher address?
        recordToMergeWith = &emptySpaceToRight;
        recordToDelete    = &emptySpaceToLeft;
      }
      // Otherwise, do the opposite - delete the "right" space's record. That
      // default was set back up above.

      int nextIndex;
      biggerRecordSearchFromIndex =
          emptySpaces.searchMultiWordExact((uint32_t*)recordToDelete, &nextIndex);

      if (biggerRecordSearchFromIndex == -1) { // It might not have been found if array got full so
                                               // there was no record for this empty space.
        biggerRecordSearchFromIndex = nextIndex;
      } else {
        // TODO: Ideally we like to combine this deletion with the below
        // reorganisation of records. But this is actually very complicated, and
  ...
  // Even if we didn't merge left, we may still want to merge right - and in
  // this case, where that is the only merge we are doing, it's much simpler.
  if (mayLookRight) {
    uint32_t* __restrict__ lookRight = (uint32_t*)(address + spaceSize + 4);
  ...
  justInsertRecord:
    ...
    if (i == -1) { // Array might have gotten full. This has to be coped with.
                   // Perhaps in a perfect world we should opt to throw away the
                   // smallest empty space to make space for this one if this
                   // one is bigger?
  ...
  goingToReplaceOldRecord:
    int i = emptySpaces.searchMultiWordExact((uint32_t*)recordToMergeWith,
                                             &insertRangeBegin,
                                             biggerRecordSearchFromIndex);
    if (i == -1) { // The record might not exist because there wasn't room to
                   // insert it when the empty space was created.
#if ALPHA_OR_BETA_VERSION
    ...
    if (i < emptySpaces.getNumElements() - 1) {
      ...
      // If that next "bigger" record to the right in the array is actually
      // smaller than our new space, we'll have to rearrange some elements.
...
uint32_t MemoryRegion::freeSomeStealableMemory(int   totalSizeNeeded,
                                               void* thingNotToStealFrom,
                                               int* __restrict__ foundSpaceSize) {

#if TEST_GENERAL_MEMORY_ALLOCATION
  skipConsistencyCheck = true; // Things will not be in an inspectable state
  ...
  for (int q = 0; q < NUM_STEALABLE_QUEUES; q++, currentTraversalNo++) {
    ...
    // If we already (more or less) know there isn't a long enough run,
    // including neighbouring memory, in this queue, skip it.
  ...
  startAgain:
    ...
    if (lastTraversalQueue <= q) {
      ...
      // If that previous look was in a different queue, it won't have been
      // included in longestRunSeenInThisQueue, so we have to invalidate that.
      // TODO: could we just lower it to the longest-run record for that other
      // queue? Yes, done.
      if (lastTraversalQueue < q &&
    ...
    // If we're forbidden from stealing from a particular thing (usually
    // SampleCache), then make sure we don't
    if (!stealable->mayBeStolen(thingNotToStealFrom)) {
      numRefusedTheft++;

      // If we've done this loads of times, it'll be seriously hurting CPU
      // usage. There's a particular case to be careful of - if project contains
      // just one long pitch-adjusted sound / AudioClip and nothing else, it'll
      // cache it, but after some number of minutes, it'll run out of new
      // Clusters to write the cache to, and it'll start trying to steal from
      // the cache-Cluster queue, and hit all of these ones of its own at the
      // same time.
    ...
    // If we're not in the last queue, and we haven't tried this too many times
    // yet, check whether it was actually in the right queue
    if (q < NUM_STEALABLE_QUEUES - 1 && numberReassessed < 4) {
      numberReassessed++;

      int appropriateQueue = stealable->getAppropriateQueue();

      // If it was in the wrong queue, put it in the right queue and start again
      // with the next one in our queue
    ...
    // Otherwise, see if available neighbouring memory adds up to make enough in
    // total
    NeighbouringMemoryGrabAttemptResult result =
        attemptToGrabNeighbouringMemory(stealable,
                                        spaceSize,
                                        amountToExtend,
                                        amountToExtend,
                                        thingNotToStealFrom,
                                        currentTraversalNo,
                                        true);
    // We also told that function to steal the initial main Stealable we are
    // looking at, once it has ascertained that there is enough memory in total.
    // Previously I attempted to have it steal everything but that central
    // Stealable, and we would steal that afterwards, down below, but this could
    // go wrong as thefts occurring in the above call to
    // attemptToGrabNeighbouringMemory() could themselves cause other memory to
    // be deallocated or shortened - and what if this happened to our main,
    // central Stealable before we actually steal it? This was certainly a
    // problem in automated testing, though I haven't quite wrapped my head
    // around whether this would quite occur under real operation - but oh well,
    // there is no harm in taking the safe option.

    // If that couldn't be done (in which case the original, central Stealable
    // won't have been stolen either), move on to next Stealable to assess
    if (!result.address) {
...
foundIt:
  stealable->steal("i007"); // Warning - for perc cache Cluster, stealing one can cause it to
                            // want to allocate more memory for its list of zones
  stealable->~Stealable();
...
// If getBiggestAllocationPossible is true, this will treat requiredSize as a
// minimum, and otherwise get as much empty RAM as possible. But, it won't
// "steal" any more than it has to go get that minimum size.
void* MemoryRegion::alloc(uint32_t  requiredSize,
  ...
  // Here we're doing a search just on one 32-bit word of the key (that's
  ...
  gotEmptySpace:
    ...
    usedWholeSpace:
      ...
      *footer             = headerData;
      ...
      {
        // Or even if it wasn't the leftmost record, we might still be able to
        // just simply update - if our new value is still bigger than the record
        // to the left.
        EmptySpaceRecord* nextSmallerRecord =
            (EmptySpaceRecord*)emptySpaces.getElementAddress(i - 1);
  ...
  noEmptySpace:
    ...
    if (!getBiggestAllocationPossible) {
      ...
      if (requiredSize && extraSpaceSizeWithoutItsHeaders > 0) {
        allocatedSize = requiredSize;
        markSpaceAsEmpty(allocatedAddress + allocatedSize + 8,
                         extraSpaceSizeWithoutItsHeaders,
                         false,
                         false);
      }
    }
  ...
  uint32_t headerData =
      (makeStealable ? SPACE_HEADER_STEALABLE : SPACE_HEADER_ALLOCATED) | allocatedSize;
  *header = headerData;
  *footer = headerData;
...
uint32_t MemoryRegion::shortenRight(void* address, uint32_t newSize) {
  ...
  newSize = (newSize + 3) & 0b11111111111111111111111111111100; // Round new size up
                                                                // to 4-byte boundary
  ...
  uint32_t* __restrict__ lookRight =
      (uint32_t*)((uint32_t)address + oldAllocatedSize +
                  4); // Looking to what's directly right of our old allocated space

  int newSizeLowerLimit = oldAllocatedSize;
  if ((*lookRight & SPACE_TYPE_MASK) != SPACE_HEADER_EMPTY) {
    // If the thing directly to the right is not empty space, we have to make
    // sure that we leave at least enough space for an empty space node
    newSizeLowerLimit -= 8;
  }
...
uint32_t MemoryRegion::shortenLeft(void*    address,
  ...
  uint32_t* __restrict__ lookLeft =
      (uint32_t*)((uint32_t)address - 8); // Looking to what's directly left of
                                          // our old allocated space

  int newSizeLowerLimit = oldAllocatedSize;
  if ((*lookLeft & SPACE_TYPE_MASK) != SPACE_HEADER_EMPTY) {
    // If the thing directly to the left is not empty space, we have to make
    // sure that we leave at least enough space for an empty space node
    newSizeLowerLimit -= 8;
  }
...
void MemoryRegion::writeTempHeadersBeforeASteal(uint32_t newStartAddress, uint32_t newSize) {
  ...
  // Because the steal() function is allowed to deallocate or shorten other
  // existing memory, we'd better get our headers in order so it sees that we've
  // claimed what we've claimed so far
...
// Will grab one item of empty or stealable space to the right of the supplied
// allocation. Returns new size, or same size if couldn't extend.
...
MemoryRegion::attemptToGrabNeighbouringMemory(void*    originalSpaceAddress,
                                              int      originalSpaceSize,
                                              int      minAmountToExtend,
                                              int      idealAmountToExtend,
                                              void*    thingNotToStealFrom,
                                              uint32_t markWithTraversalNo,
  ...
  // Go through twice - once not actually grabbing but just exploring, and then
  // a second time actually grabbing
  for (int actuallyGrabbing = 0; actuallyGrabbing < 2; actuallyGrabbing++) {
    ...
    uint32_t* __restrict__ lookRight =
        (uint32_t*)((uint32_t)originalSpaceAddress + originalSpaceSize + 4);
    uint32_t* __restrict__ lookLeft = (uint32_t*)((uint32_t)originalSpaceAddress - 8);

  tryNotStealingFirst:
    // At each point in the exploration, we want to first look both left and
    // right before "stealing", as opposed to just grabbing unused space, in
    // case there's actually more unused space in one of the directions, which
    // would of course be preferable.
    for (int tryingStealingYet = 0; tryingStealingYet < 2; tryingStealingYet++) {

      // If we're going to try stealing, well let's not do that if we've
      // actually found the min amount of memory - to reduce disruption
          ...
          case SPACE_HEADER_EMPTY:
            ...
            if (actuallyGrabbing) {
...
#if TEST_GENERAL_MEMORY_ALLOCATION
                if (!success) {
                  // TODO: actually, this is basically ok - it should be allowed
                  // to not find the key, because the empty space might indeed not
                  // have a record if there wasn't room to insert one when the
                  // empty space was created.
              ...
              // Or if stealable space...
                ...
                // Because the steal() function is allowed to deallocate or
                // shorten other existing memory, we'd better get our headers in
                // order so it sees that we've claimed what we've claimed so far
                writeTempHeadersBeforeASteal(toReturn.address,
                                             originalSpaceSize + toReturn.amountsExtended[0] +
              ...
              // Can only change these after potentially putting those temp
              // headers in, above
            ...
            if (amountOfExtraSpaceFoundSoFar >= idealAmountToExtend) { goto gotEnoughMemory; }

            // Whether or not actually grabbing, if that was Stealable space we
            // just found, go back and try looking at more, further memory - first
            // prioritizing unused empty space, in case we just stumbled on more
    ...
    // When we get here, we've just tried stealing (and found nothing further in
    // either direction on this particular try)

    // If haven't even got min amount...
    if (amountOfExtraSpaceFoundSoFar < minAmountToExtend) {

      // If we somehow grabbed without finding min amount, then that shouldn't
      // have happened!
#if TEST_GENERAL_MEMORY_ALLOCATION
      if (actuallyGrabbing) {
        Uart::println("grabbed extension without reaching min size"); // This happened
                                                                      // recently!
  ...
  gotEnoughMemory : {}
    // There's a small chance it will have found a bit less memory the second
    // time through if stealing an allocation resulted in another little bit of
    // memory being freed, that adding onto the discovered amount, and getting
    // us less of a surplus while still reaching the desired (well actually the
    // min) amount
  }
...
void MemoryRegion::extend(void*    address,
                          ...
                          uint32_t* __restrict__ getAmountExtendedRight,
                          void* thingNotToStealFrom) {
  ...
  NeighbouringMemoryGrabAttemptResult grabResult =
      attemptToGrabNeighbouringMemory(address,
                                      oldAllocatedSize,
                                      minAmountToExtend,
                                      idealAmountToExtend,
                                      thingNotToStealFrom);
  ...
  int surplusWeGot =
      grabResult.amountsExtended[0] + grabResult.amountsExtended[1] - (int)idealAmountToExtend;
  if (surplusWeGot > 8) {
    ...
    if (grabResult.amountsExtended[0] > 8) {
      ...
      amountToCutRightIncludingHeaders =
          getMin(amountToCutRightIncludingHeaders, grabResult.amountsExtended[0]);
      ...
      markSpaceAsEmpty((uint32_t)address + oldAllocatedSize + grabResult.amountsExtended[0] + 8,
                       amountToCutRightIncludingHeaders - 8,
                       false,
                       false);
    }
  ...
  *getAmountExtendedRight = grabResult.amountsExtended[0];
  ...
  uint32_t newSize =
      oldAllocatedSize + grabResult.amountsExtended[0] + grabResult.amountsExtended[1];
