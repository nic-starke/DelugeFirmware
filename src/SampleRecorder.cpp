...
SampleRecorder::SampleRecorder() {
  ...
  currentRecordClusterIndex = -1; // Put things in valid state so if we get destructed before any
                                  // recording, it's all ok
  firstUnwrittenClusterIndex = 0;
}

SampleRecorder::~SampleRecorder() {
  Uart::println("~SampleRecorder()");
  if (sample) { detachSample(); }
}

// This can be called when this SampleRecorder is destructed routinely - or
// earlier if we've aborted and the sample file is being deleted IMPORTANT!!!!
// You have to set sample to NULL after calling this, if not destructing
void SampleRecorder::detachSample() {

  // If we were holding onto the reasons for the first couple of Clusters,
  // release them now
  if (keepingReasonsForFirstClusters) {
    int numClustersToRemoveFor =
        getMin(NUM_CLUSTERS_LOADED_AHEAD, sample->clusters.getNumElements());
  ...
  if (currentRecordCluster)
    removeForClustersUntilIndex++; // If there's a currentRecordCluster (usually
                                   // will be if aborting), need to remove its
                                   // "reason" too
...
int SampleRecorder::setup(int  newNumChannels,
  ...
  void* sampleMemory = generalMemoryAllocator.alloc(
  ...
  sample->addReason(); // Must call this so it's protected from stealing, before
                       // we call initialize().
  ...
  currentRecordCluster =
      sample->clusters.getElement(0)->getCluster(sample,
                                                 0,
                                                 CLUSTER_DONT_LOAD); // Adds a "reason" to it, too
  if (!currentRecordCluster) {
  ...
  sample->audioDataLengthBytes   = 0x8FFFFFFFFFFFFFFF; // If you ever change this value, update the
                                                     // check for it in SampleManager::loadCluster()
  sample->sampleRate = 44100;
  sample->workOutBitMask();

  currentRecordCluster->loaded = true; // I think this is ok - mark it as loaded even though we're
                                       // yet to record into it
  ...
  // Apart from the MIX option, all other audio sources are fed to us during the
  // "outputting" routine. Occasionally, there'll be some more of that going to
  // happen for the previous render, so we have to compensate for that
  if (mode != AUDIO_INPUT_CHANNEL_MIX)
    numSamplesToRunBeforeBeginningCapturing +=
        AudioEngine::getNumSamplesLeftToOutputFromPreviousRender();
  ...
  if (mode < AUDIO_INPUT_CHANNEL_FIRST_INTERNAL_OPTION) {
    ...
    numSamplesToRunBeforeBeginningCapturing -=
        buttonPressLatency; // Compensate for button press latency. We only do
                            // this for external sources

    // If doing extra margins...
    if (recordingExtraMargins) {

      // Everything will be fine, so long as the button press latency we
      // compensated for isn't, like, as big as the RX buffer
      sample->fileLoopStartSamples = SSI_RX_BUFFER_NUM_SAMPLES - (SSI_TX_BUFFER_NUM_SAMPLES << 1) +
                                     numSamplesToRunBeforeBeginningCapturing;
      numSamplesToRunBeforeBeginningCapturing = 0;

      sourcePos +=
          (SSI_TX_BUFFER_NUM_SAMPLES << (NUM_MONO_INPUT_CHANNELS_MAGNITUDE +
                                         1)); // I think the +1 was just because it needs to move
                                              // two tx buffers' length for some reason...
    ...
    // Or if not doing extra margins...
      ...
      // If the button press latency we're compensating for is more than the
      // audio latency, we have to adjust stuff, to grab some audio from just
      // back in time
      if (numSamplesToRunBeforeBeginningCapturing < 0) {

        sourcePos += numSamplesToRunBeforeBeginningCapturing *
                     NUM_MONO_INPUT_CHANNELS; // This might be negative!
  ...
  int byteDepth        = 3;
  int lengthSec        = 5; // Mark it as 5 seconds long initially. We'll update that
                            // later when we know how long it actually is
  ...
  writeInt32(&writePos,
             audioDataLengthBytesAsWrittenToFile + sample->audioDataStartPosBytes -
                 8);                 // Chunk size
  ...
  if (recordingExtraMargins) {
    ...
    writeInt32(&writePos,
               (1000000000 + (sample->sampleRate >> 1)) /
                   sample->sampleRate); // Nanoseconds per sample
...
// Beware! This could get called during card routine - e.g. if user stopped
// playback. So we'll just store a changed status, then do the descrutcion and
// file deletion when we know we're out of the card routine. Also, this gets
// called in audio routine! So don't do anything drastic.
...
int SampleRecorder::cardRoutine() {
  ...
  aborted:
    if (sample) { // This might get called multiple times, so check we haven't
                  // already detached it.

      // Note: if this abort() is due to a song-swap (loading a different song),
      // then samples is about to be searched for temp ones to delete, and we'll
      // need to have deleted it here before that trips over us. Previously
      // caused an E281. So, for that to happen,
      // SampleManager::deleteAnyTempRecordedSamplesFromMemory() (indirectly)
      // calls us here first.

      detachSample(); // Does not set sample to NULL - we do that below

#if ALPHA_OR_BETA_VERSION
      // It should be impossible that anyone else still holds a "reason" to this
      // Sample, as we can only be "aborted" before
      // AudioClip::finishLinearRecording() is called, and it's only then at the
      // AudioClip becomes a "reason".
      if (sample->numReasonsToBeLoaded) numericDriver.freezeWithError("E282");
#endif

      if (haveAddedSampleToArray) { // We only add it to the array when the file
                                    // is created.
    ...
    if (!filePathCreated.isEmpty()) {
      ...
      // If this was the most recent recording in this category, tick the
      // counter backwards - so long as either the delete was successful or it
      // was for an AudioClip, which means the file is in the TEMP folder and
    ...
    // Normally we now just await deletion - except if a pointer is still being
    // held elsewhere - which I think can only happen from AudioRecorder. Or if
    // the abort comes from a failure within this class and the AudioClip hasn't
    // realised yet?
  ...
  if (!hadCardError) {
    ...
    if (filePathCreated.isEmpty()) {
      ...
      // Note: we couldn't pass the actual Sample pointer into this function,
      // cos the Sample might get destructed during the card access! (Though
...
int SampleRecorder::writeAnyCompletedClusters() {
  while (firstUnwrittenClusterIndex < currentRecordClusterIndex) {
    ...
    // If there was an error, we can only return now after removing that reason,
    // because we'd already incremented firstUnwrittenClusterIndex, and we can't
    // leave that incremented without removing the reason
...
int SampleRecorder::writeOneCompletedCluster() {
  ...
  firstUnwrittenClusterIndex++; // Have to increment this before writing, cos
                                // while writing, the audio routine will be
                                // called, and we need to be counting this
                                // cluster as "written", as in too late for it
                                // to be modified (by writing a final length to
                                // it)
  ...
  if (!keepingReasonsForFirstClusters || writingClusterIndex >= NUM_CLUSTERS_LOADED_AHEAD) {
    ...
    if (!cluster->numReasonsHeldBySampleRecorder)
      numericDriver.freezeWithError("E347"); // Leo got!!! And Vinz, and keyman. May be solved now
                                             // that fixed so detachSample() doesn't get called
                                             // during card routine.
    cluster->numReasonsHeldBySampleRecorder--;
...
int SampleRecorder::finalizeRecordedFile() {
  ...
  // In the very rare case where we've already got between 1 and 5 bytes
  // overhanging the end of our current cluster, we need to allocate a new one
  // right now
  ...
  if (bytesTilClusterEnd < 0) {
    ...
    if (error == ERROR_MAX_FILE_SIZE_REACHED) {
    } // So incredibly unlikely. But no real problem - we maybe just lose a byte
      // or two
      ...
      // Having just created a new cluster, there'll be one more completed one
      // to write
  ...
  // And we probably need to write some of the final cluster(s) to file. (If
  // it's NULL, it means that it couldn't be created, cos or RAM or file size
  // limit.)
  if (currentRecordCluster) {
    ...
    // Having incremented firstUnwrittenClusterIndex, we need to remove the
    // "reason" for that final cluster. Normally that happens in
    // writeAnyCompletedClusters(), but well this cluster wasn't "complete" so
    // we're doing the whole thing here instead
    ...
    currentRecordCluster = NULL; // But currentRecordClusterIndex now refers to
                                 // a cluster that'll never exist
  }

  uint32_t idealFileSizeBeforeAction =
  ...
  // TODO: in a perfect world, where we're not deleting a channel, we'd go
  // backwards from the last Cluster, because that's the most likely to still be
  // in memory
  ...
  // Or if no action or shifting was required...
    ...
    // If we made the file too long, because we then compensated for button
    // latency and are throwing away the last little bit, then truncate it
    if (capturedTooMuch) {
      Uart::println("truncating");
      uint32_t correctLength = sample->audioDataStartPosBytes +
                               sample->audioDataLengthBytes; // These were written to in
                                                             // totalSampleLengthNowKnown().
    ...
    // If the actual audio data length we ended up with is not the same as was
    // written in the headers in the first cluster (very likely; various
    // reasons)
    if (sample->audioDataLengthBytes != audioDataLengthBytesAsWrittenToFile ||
      ...
      Cluster*       cluster            = firstSampleCluster->getCluster(
          sample,
          0,
      ...
      if (cluster) {
        ...
        // If that failed, well, that's a shame, but we don't need to do
        // anything
  ...
  sample->audioDataLengthBytes =
      ...
      (sample->byteDepth * sample->numChannels); // Ensure whole number of samples (surely it
                                                 // already would be though?)
...
void SampleRecorder::updateDataLengthInFirstCluster(Cluster* cluster) {
  ...
  *(uint32_t*)&cluster->data[4] =
      audioDataLengthBytesAsWrittenToFile + sample->audioDataStartPosBytes - 8;

  // Write data chunk size
  *(uint32_t*)&cluster->data[sample->audioDataStartPosBytes - 4] =
...
int SampleRecorder::writeCluster(int32_t clusterIndex, int numBytes) {
  ...
  if (result || numBytes != numBytesWritten) { return ERROR_SD_CARD; }

  sampleCluster = sample->clusters.getElement(
      clusterIndex); // MUST re-get this - while writing above, the audio
                     // routine is being called, and that could allocate new
                     // SampleClusters and move them around!
...
int SampleRecorder::createNextCluster() {
  ...
  Cluster* oldRecordCluster = currentRecordCluster; // Cos we're gonna set that to NULL just below
                                                    // here, but still want to be able to access the
                                                    // old one a bit further down

  currentRecordClusterIndex++; // Mark record-cluster we were on as finished

  currentRecordCluster = NULL; // Note that we haven't yet created our next record-cluster - we'll
                               // do that below if no error first; and if there is an error and we
                               // don't create one, this has to remain NULL to indicate that we
                               // never created one

  // If this new cluster would actually put us past the 4GB limit...
  if (currentRecordClusterIndex >=
      (1 << (MAX_FILE_SIZE_MAGNITUDE - audioFileManager.clusterSizeMagnitude))) {

    // See if we actually already had any bytes to write into that new cluster
    // we can't have...
    int bytesTilClusterEnd = (uint32_t)clusterEndPos - (uint32_t)writePos;
    if (bytesTilClusterEnd < 0) {
      numSamplesCaptured--;
  ...
  // If couldn't allocate cluster (would normally only happen if no SD card
  // present so recording only to RAM)
  if (!currentRecordCluster) {
  ...
  // Copy those extra bytes from the end of the old record cluster to the start
  // of the new cluster
  memcpy(currentRecordCluster->data,
         &oldRecordCluster->data[audioFileManager.clusterSize],
...
// Gets called when we've captured all the samples of audio that we wanted -
// either as a direct result of user action, or after being fed a few more
// samples to make up for latency.
...
// Only call this after checking that status <
// RECORDER_STATUS_FINISHED_CAPTURING_BUT_STILL_WRITING Watch out - this could
// be called during SD writing - including during cardRoutine() for this class!
void SampleRecorder::feedAudio(int32_t* __restrict__ inputAddress, int numSamples, bool applyGain) {
  ...
  do {
    ...
    // If haven't actually started recording yet cos we're compensating for
    // lag...
    if (numSamplesBeenRunning < (uint32_t)numSamplesToRunBeforeBeginningCapturing) {
      int numSamplesTilBeginRecording =
          numSamplesToRunBeforeBeginningCapturing - numSamplesBeenRunning;
    ...
    // Or, if properly recording...
      ...
      int32_t* endInputNow =
          inputAddress + (numSamplesThisCycle << NUM_MONO_INPUT_CHANNELS_MAGNITUDE);

      char* __restrict__ writePosNow = writePos;

      // Balanced input. For this, we skip a bunch of stat-grabbing, cos we knob
      // this is just for AudioClips. We also know that applyGain is false -
      // that's just for the MIX option
...
void SampleRecorder::endSyncedRecording(int buttonLatencyForTempolessRecording) {
  ...
  int numMoreSamplesTilEndLoopPoint =
...
void SampleRecorder::totalSampleLengthNowKnown(uint32_t totalLengthSamples,
                                               uint32_t loopEndPointSamples) {
  ...
  // If we haven't written the first cluster yet, quick - update it with the
  // actual length
  if (firstUnwrittenClusterIndex == 0) {
    SampleCluster* firstSampleCluster = sample->clusters.getElement(0);
    Cluster*       cluster = firstSampleCluster->cluster; // It should still be there, cos it hasn't
                                                          // been written to card yet
    if (ALPHA_OR_BETA_VERSION && !cluster) numericDriver.freezeWithError("E274");

    audioDataLengthBytesAsWrittenToFile = sample->audioDataLengthBytes;
    loopEndSampleAsWrittenToFile =
        sample->fileLoopEndSamples; // Even if we're not actually writing loop
                                    // points to the file, this is harmless
    updateDataLengthInFirstCluster(cluster);
  }
}
...
void SampleRecorder::setExtraBytesOnPreviousCluster(Cluster* currentCluster,
...
int SampleRecorder::alterFile(int      action,
                              int      lshiftAmount,
                              uint32_t idealFileSizeBeforeAction,
  ...
  Cluster* currentReadCluster = sample->clusters.getElement(0)->getCluster(
      ...
      0,
      CLUSTER_LOAD_IMMEDIATELY); // Remember, this adds a "reason"
  if (!currentReadCluster) { return ERROR_SD_CARD; }
  ...
  Cluster* currentWriteCluster = sample->clusters.getElement(0)->getCluster(
      sample,
      0,
      CLUSTER_DONT_LOAD); // Remember, this adds a "reason"
  // That one can't fail, fortunately, cos we already grabbed Cluster 0 above,
  // so it exists
  ...
  // TODO: this is really inefficient - checks a bunch of stuff for every single
  // audio sample. Should check in advance how many samples we can process at a
  // time

  while (true) {

    if (!(count & 0b11111111)) { // 10x 1's seems to work ok. So we go down to 8
                                 // to be sure
    ...
    *(writePos++)      = *(processedPos++);
    ...
    // If need to advance write-head past the end of a cluster, then we'll write
    // that current cluster to disk and carry on
    int writeOvershot =
        (uint32_t)writePos - (uint32_t)&currentWriteCluster->data[audioFileManager.clusterSize];
    if (writeOvershot >= 0) {
      ...
      if ((sdAddress - fileSystemStuff.fileSystem.database) &
          (fileSystemStuff.fileSystem.csize - 1))
      ...
      // Grab any overshot / extra bytes from the end of the Cluster we just
      // finished...
      ...
      // And from the Cluster we just finished, give the Cluster *before that*
      // the extra bytes from its start
      ...
      if (result) {
      writeFailed:
        // Before we get out, remove "reasons" from the clusters we've been
        // reading from
      ...
      // Get the new / next Cluster, but don't insist on actually reading from
      // the card, cos we're gonna overwrite it with new data anyway
      ...
      if (!currentWriteCluster) { goto writeFailed; }

      // Bug hunting - newly gotten Cluster
      currentWriteCluster->numReasonsHeldBySampleRecorder++;

      // Ok, and those extra bytes that we grabbed from the end of the previous
      // Cluster - paste them into the beginning of the new current Cluster
      if (writeOvershot) memcpy(currentWriteCluster->data, extraBytes, writeOvershot);

      // And get ready to write to the new current Cluster - from the next
      // sample, which might not be perfectly aligned to the Cluster start
      writePos = &currentWriteCluster->data[writeOvershot];
    }

    // If we're in the final read-Cluster and reached the end, then all that's
    // left to do is flush out what we have left to write (max 1 cluster), and
    // get out.
    ...
    // Advance read-head. We read one Cluster ahead, so we can access its "extra
    // bytes"
    if (readPos >= &currentReadCluster->data[audioFileManager.clusterSize]) {
      ...
      int overshot =
          (uint32_t)readPos - (uint32_t)&currentReadCluster->data[audioFileManager.clusterSize];
      ...
      if (currentReadClusterIndex < numClustersBeforeAction - 1) {
        nextReadCluster =
            sample->clusters.getElement(currentReadClusterIndex + 1)
                ->getCluster(sample,
                             currentReadClusterIndex + 1,
                             CLUSTER_LOAD_IMMEDIATELY); // Remember, this adds a "reason"
  ...
  if (bytesToWriteFinalCluster) { // If there is in fact anything to flush out
                                  // to the file / card...

    // And from this final Cluster, give the Cluster *before that* the extra
    ...
    // If writing disk failed, above, we've now removed that "reason", so we can
    // get out
    if (result) return ERROR_SD_CARD;
...
int SampleRecorder::truncateFileDownToSize(uint32_t newFileSize) {
  ...
  // Update the Sample object to indicate the correct size. Do this before we
  // risk errors below

  uint64_t numClustersAfterAction =
      ((newFileSize - 1) >> audioFileManager.clusterSizeMagnitude) + 1;
