...
// Called when we navigate between "root" UIs, like sessionView,
// instrumentClipView, etc.
...
UI* getCurrentUI() {
  if (numUIsOpen == 0)
    return lastUIBeforeNullifying; // Very ugly work-around to stop everything
                                   // breaking
  return uiNavigationHierarchy[numUIsOpen - 1];
}

// This will be NULL while waiting to swap songs, so you'd better check for this
// anytime you're gonna call a function on the result!
RootUI* getRootUI() {
...
bool openUI(UI* newUI) {
  ...
  if (!success) {
    numUIsOpen--;
    PadLEDs::reassessGreyout();
    oldUI->focusRegained(); // Or maybe we should instead let the caller deal
                            // with this failure, and call this if they wish?
  }
#if HAVE_OLED
  renderUIsForOled();
#endif
...
void renderUIsForOled() {
  int u = numUIsOpen - 1;
  while (u && uiNavigationHierarchy[u]->oledShowsUIUnderneath) { u--; }
...
void uiNeedsRendering(UI* ui, uint32_t whichMainRows, uint32_t whichSideRows) {
  ...
  // We might be in the middle of an audio routine or something, so just see
  // whether the selected bit of the UI is visible
...
void doAnyPendingUIRendering() {
  ...
  if (uartGetTxBufferSpace(UART_ITEM_PIC_PADS) <=
      (NUM_BYTES_IN_MAIN_PAD_REDRAW + NUM_BYTES_IN_SIDEBAR_REDRAW) * 2)
    return; // Trialling the *2 to fix flickering when flicking through presets
            // very fast
  ...
  // Clear the overall instructions - so it may now be written to again during
  // this function call
  whichMainRowsNeedRendering = whichSideRowsNeedRendering = 0;
...
// Checks that all of the currently active UI modes are within the list of modes
// provided. As well as making things tidy, the main point of this is to still
// return true when more than one of the modes on the list provided is active.
...
// You can safely call this even if you don't know whether said UI mode is
// active.
