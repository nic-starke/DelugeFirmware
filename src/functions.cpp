...
// This is just the range of the user-defined "preset" value, it doesn't apply
// to the outcome of patch cables
int32_t getParamRange(int p) {
  switch (p) {
    ...
    case PARAM_LOCAL_LPF_FREQ:
      ...
      // For phase width, we have this higher (than I previously did) because
      // these are hibrid params, meaning that with a source (e.g. LFO) patched to
      // them, the might have up to 1073741824 added to them
      // - which would take us to the max user "preset value", which is what we
      // want for phase width
    default:
      return 1073741824;
  }
}
...
int32_t getParamNeutralValue(int p) {
  switch (p) {
    ...
    case PARAM_LOCAL_MODULATOR_0_PITCH_ADJUST:
    case PARAM_LOCAL_MODULATOR_1_PITCH_ADJUST:
      return 16777216; // Means we have space to 8x (3-octave-shift) the pitch if
                       // we want... (wait, I've since made it 16x smaller)
...
void functionsInit() {
  ...
  for (int p = 0; p < NUM_PARAMS; p++) { paramRanges[p] = getParamRange(p); }

  for (int p = 0; p < NUM_PARAMS; p++) { paramNeutralValues[p] = getParamNeutralValue(p); }
}

int32_t getFinalParameterValueHybrid(int32_t paramNeutralValue, int32_t patchedValue) {
  // Allows for max output values of +- 1073741824, which the panning code
  // understands as the full range from left to right
...
int32_t getFinalParameterValueVolume(int32_t paramNeutralValue, int32_t patchedValue) {
  ...
  // patchedValue's range is ideally +- 536870912, but may get up to 1610612736
  // due to multiple patch cables having been multiplied

  // No need for max/min here - it's already been taken care of in
  // patchAllCablesToParameter(),
  // ... or if we got here from patchSourceToAllExclusiveCables(), there's no
  // way it could have been too big
  int32_t positivePatchedValue = patchedValue + 536870912;

  // positivePatchedValue's range is ideally 0 ("0") to 1073741824 ("2"), but
  // potentially up to 2147483647 ("4"). 536870912 represents "1".

  // If this parameter is a volume one, then apply a parabola curve to the
  // patched value, at this late stage
  /*
  if (isVolumeParam) {
      // But, our output value can't get bigger than 2147483647 ("4"), which
  means we have to clip our input off at 1073741824 ("2") if
  (positivePatchedValue >= 1073741824) positivePatchedValue = 2147483647; else {
          //int32_t madeSmaller = positivePatchedValue >> 15;
          //positivePatchedValue = (madeSmaller * madeSmaller) << 1;
          positivePatchedValue = (positivePatchedValue >> 15) *
  (positivePatchedValue >> 14);
      }
  }
  */

  // This is a temporary (?) fix I've done to allow FM modulator amounts to get
  // past where I clipped off volume params. So now volumes can get higher too.
  // Problem? Not sure.

  // But, our output value can't get bigger than 2147483647 ("4"), which means
  // we have to clip our input off at 1073741824 ("2")
  positivePatchedValue = (positivePatchedValue >> 16) * (positivePatchedValue >> 15);

  // return multiply_32x32_rshift32(positivePatchedValue, paramNeutralValue) <<
  // 5;
...
bool paramNeedsLPF(int p, bool fromAutomation) {
  switch (p) {
    ...
    // For many params, particularly volumes, we do want the param LPF if the user
    // adjusted it, so we don't get stepping, but if it's from step automation, we
    // do want it to adjust instantly, so the new step is instantly at the right
    // volume
    case PARAM_GLOBAL_VOLUME_POST_FX:
    case PARAM_GLOBAL_VOLUME_POST_REVERB_SEND:
    case PARAM_GLOBAL_REVERB_AMOUNT:
...
int32_t getExp(int32_t presetValue, int32_t adjustment) {
  ...
  int32_t adjustedPresetValue =
      multiply_32x32_rshift32(presetValue,
                              interpolateTable(adjustment & 67108863, 26, expTableSmall));
...
bool isAudioFilename(char const* filename) {
  ...
  return (dotPos != 0 && (!strcasecmp(dotPos, ".WAV") || !strcasecmp(dotPos, ".AIF") ||
...
int32_t combineHitStrengths(int32_t strength1, int32_t strength2) {
  // Ideally, we'd do pythagoras on these. But to save computation time, we'll
  // just go half way between the biggest one and the sum
...
bool shouldDoPanning(int32_t panAmount, int32_t* amplitudeL, int32_t* amplitudeR) {
  ...
  int32_t panOffset =
      getMax((int32_t)-1073741824, (int32_t)(getMin((int32_t)1073741824, (int32_t)panAmount)));
...
const int32_t pythagTable[257] = {
    1073741824, 1073750016, 1073774592, 1073815549, 1073872888, 1073946604, 1074036696, 1074143157,
    1074265984, 1074405171, 1074560712, 1074732599, 1074920825, 1075125381, 1075346257, 1075583445,
    1075836932, 1076106708, 1076392760, 1076695075, 1077013639, 1077348439, 1077699458, 1078066682,
    1078450093, 1078849675, 1079265409, 1079697276, 1080145258, 1080609334, 1081089484, 1081585686,
    1082097918, 1082626157, 1083170380, 1083730563, 1084306681, 1084898708, 1085506620, 1086130388,
    1086769986, 1087425386, 1088096559, 1088783476, 1089486107, 1090204422, 1090938390, 1091687979,
    1092453157, 1093233892, 1094030150, 1094841897, 1095669100, 1096511721, 1097369728, 1098243082,
    1099131748, 1100035689, 1100954867, 1101889243, 1102838780, 1103803438, 1104783177, 1105777957,
    1106787739, 1107812480, 1108852139, 1109906675, 1110976045, 1112060206, 1113159115, 1114272729,
    1115401003, 1116543893, 1117701353, 1118873340, 1120059807, 1121260708, 1122475997, 1123705628,
    1124949552, 1126207724, 1127480095, 1128766616, 1130067241, 1131381920, 1132710604, 1134053244,
    1135409791, 1136780194, 1138164404, 1139562371, 1140974043, 1142399370, 1143838302, 1145290786,
    1146756771, 1148236205, 1149729037, 1151235215, 1152754686, 1154287397, 1155833296, 1157392330,
    1158964447, 1160549592, 1162147713, 1163758756, 1165382668, 1167019394, 1168668882, 1170331077,
    1172005924, 1173693371, 1175393362, 1177105843, 1178830760, 1180568058, 1182317683, 1184079580,
    1185853694, 1187639971, 1189438356, 1191248793, 1193071229, 1194905608, 1196751875, 1198609975,
    1200479854, 1202361457, 1204254728, 1206159612, 1208076055, 1210004001, 1211943397, 1213894186,
    1215856315, 1217829727, 1219814369, 1221810186, 1223817123, 1225835126, 1227864139, 1229904109,
    1231954981, 1234016700, 1236089213, 1238172465, 1240266402, 1242370970, 1244486115, 1246611783,
    1248747921, 1250894475, 1253051391, 1255218616, 1257396097, 1259583780, 1261781612, 1263989541,
    1266207514, 1268435477, 1270673379, 1272921166, 1275178788, 1277446191, 1279723323, 1282010133,
    1284306569, 1286612580, 1288928113, 1291253119, 1293587545, 1295931341, 1298284456, 1300646840,
    1303018442, 1305399211, 1307789099, 1310188054, 1312596028, 1315012970, 1317438832, 1319873563,
    1322317116, 1324769441, 1327230490, 1329700214, 1332178565, 1334665495, 1337160955, 1339664900,
    1342177280, 1344698049, 1347227159, 1349764564, 1352310217, 1354864072, 1357426081, 1359996200,
    1362574382, 1365160581, 1367754753, 1370356851, 1372966831, 1375584648, 1378210257, 1380843613,
    1383484673, 1386133393, 1388789728, 1391453635, 1394125071, 1396803992, 1399490356, 1402184119,
    1404885240, 1407593675, 1410309382, 1413032321, 1415762448, 1418499723, 1421244103, 1423995549,
    1426754019, 1429519473, 1432291869, 1435071169, 1437857331, 1440650316, 1443450084, 1446256597,
    1449069814, 1451889697, 1454716207, 1457549306, 1460388955, 1463235115, 1466087750, 1468946821,
    1471812291, 1474684123, 1477562279, 1480446723, 1483337417, 1486234326, 1489137413, 1492046642,
    1494961978, 1497883384, 1500810825, 1503744266, 1506683672, 1509629008, 1512580239, 1515537331,
    1518500250,
};
...
const int16_t lanczosKernel[257] = {
    32767, 32753, 32711, 32641, 32544, 32419, 32266, 32087, 31880, 31647, 31388, 31103, 30793,
    30458, 30099, 29717, 29311, 28884, 28434, 27964, 27474, 26964, 26435, 25889, 25326, 24748,
    24154, 23546, 22925, 22291, 21647, 20992, 20328, 19656, 18977, 18291, 17600, 16905, 16207,
    15507, 14806, 14105, 13404, 12706, 12010, 11318, 10631, 9950,  9275,  8607,  7948,  7298,
    6658,  6028,  5410,  4804,  4211,  3631,  3066,  2515,  1980,  1460,  956,   470,   0,
    -452,  -886,  -1303, -1700, -2080, -2440, -2782, -3104, -3407, -3691, -3956, -4202, -4429,
    -4637, -4826, -4997, -5149, -5283, -5399, -5498, -5579, -5644, -5691, -5723, -5739, -5740,
    -5726, -5698, -5656, -5601, -5533, -5453, -5361, -5259, -5146, -5023, -4891, -4751, -4602,
    -4446, -4284, -4115, -3941, -3761, -3578, -3391, -3200, -3007, -2812, -2616, -2419, -2222,
    -2024, -1828, -1632, -1439, -1247, -1058, -872,  -689,  -510,  -336,  -165,  0,     160,
    315,   465,   608,   746,   877,   1002,  1120,  1232,  1336,  1434,  1525,  1609,  1686,
    1756,  1819,  1875,  1925,  1967,  2003,  2032,  2055,  2071,  2081,  2086,  2084,  2077,
    2064,  2046,  2023,  1995,  1963,  1926,  1886,  1841,  1793,  1742,  1687,  1630,  1570,
    1508,  1444,  1379,  1311,  1243,  1173,  1103,  1032,  961,   890,   820,   749,   679,
    610,   542,   475,   410,   346,   283,   222,   164,   107,   52,    0,     -50,   -98,
    -143,  -186,  -226,  -263,  -298,  -330,  -360,  -387,  -411,  -433,  -452,  -468,  -482,
    -494,  -503,  -510,  -515,  -517,  -518,  -516,  -513,  -508,  -501,  -492,  -483,  -471,
    -459,  -445,  -430,  -415,  -398,  -381,  -364,  -346,  -327,  -309,  -290,  -271,  -252,
    -234,  -215,  -197,  -180,  -163,  -146,  -130,  -115,  -101,  -87,   -74,   -63,   -52,
    -42,   -33,   -25,   -19,   -13,   -8,    -5,    -2,    -1,    0,
};
...
int32_t doLanczos(int32_t* data, int32_t pos, uint32_t posWithinPos, int memoryNumElements) {
  ...
  // In a "perfect" world we'd <<1 after this, but no real need since loudness
  // is going to get normalized anyway, and actually we'd probably get some
  // overflows if we did...
...
int32_t
doLanczosCircular(int32_t* data, int32_t pos, uint32_t posWithinPos, int memoryNumElements) {
  ...
  int32_t value = 0;
  for (int i = 0; i < LANCZOS_A; i++) {
    value += multiply_32x32_rshift32_rounded(
        strengthL[i],
        data[(pos - i + memoryNumElements) & (memoryNumElements - 1)]);
  }
  for (int i = 0; i < LANCZOS_A; i++) {
    value += multiply_32x32_rshift32_rounded(strengthR[i],
                                             data[(pos + 1 + i) & (memoryNumElements - 1)]);
  }
...
int strcmpspecial(char const* first,
  ...
  bool previousWasPotentialNoteLetter = false; // FYI, this can only get set to true if
                                               // shouldInterpretNoteNames is true
  ...
  while (true) {
    ...
    if (*first == 0) {
        if (*second == 0) {
            ...
            return strcasecmp(first, second); // Have to check that they're
    exactly the same, regardless of numbers
        }
        else return -1;
    }
    ...
    */
    ...
    if (*first == 0 || *second == 0) {
      ...
      */
      return strcasecmp(firstInitially,
                        secondInitially); // Have to check that they're exactly
                                          // the same, regardless of numbers
    }

    // Only parse negative numbers as part of a note name - that's where they're
    // most commonly used, and elsewhere it's hard to know if the '-' is just a
    // notational dash
    bool firstIsNegativeNumber  = (previousWasPotentialNoteLetter && *first == '-' &&
                                  *(first + 1) >= '0' && *(first + 1) <= '9');
    bool secondIsNegativeNumber = (previousWasPotentialNoteLetter && *second == '-' &&
                                   *(second + 1) >= '0' && *(second + 1) <= '9');
    ...
    // Otherwise...
      ...
      if (previousWasPotentialNoteLetter) {
        ...
        // If found at least one flat or sharp, go do another whole comparison -
        // there might be some numbers next
        if (foundAtLeastOneFlatOrSharp) continue;

        // Or, if didn't find any (more) flats or sharps...
        else {
          // We know that it's not the case that both are numbers. But if one is
          // and the other isn't, we can conclude that the one with the number
          // has a proper note-name, and the other doesn't - it's just a word
          // happening to start with that letter. We choose to put notes before
          // non-notes. Without this step, we got a weird loop
          // if (firstIsNumber) return -1;
          // if (secondIsNumber) return 1;
        }

        if (firstNoteLetter < secondNoteLetter)
          return -1;
        else if (firstNoteLetter > secondNoteLetter)
          return 1;

        // Or if we're still here, carry on. Backtrack on any flats or sharps
        // seen.
        previousWasPotentialNoteLetter = false;
        first                          = firstPosBeforeFlatOrSharp;
        second                         = secondPosBeforeFlatOrSharp;
      ...
      // If we're doing note ordering, and if both characters are potential note
      // letters...
...
int stringToFirmwareVersion(char const* firmwareVersionString) {
  ...
  } else if (!strcmp(firmwareVersionString,
                     "2.2.0-alpha")) { // Same as next one
...
int encodeIterationDependence(int divisor, int iterationWithinDivisor) {
  ...
  for (int i = 2; i < divisor; i++) { value += i; }
