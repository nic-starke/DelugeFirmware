...
bool VoiceUnisonPartSource::noteOn(Voice*                    voice,
                                   Source*                   source,
                                   VoiceSamplePlaybackGuide* guide,
  ...
  if (synthMode != SYNTH_MODE_FM && source->oscType == OSC_TYPE_SAMPLE) {
    ...
    if (!guide->audioFileHolder || !guide->audioFileHolder->audioFile ||
      ...
      return true; // We didn't succeed, but don't want to stop the whole Voice
                   // from sounding necessarily

    if (!voiceSample) { // We might actually already have one, and just be
                        // restarting this voice
    ...
    return voiceSample->setupClusersForInitialPlay(guide,
...
bool VoiceUnisonPartSource::getPitchAndSpeedParams(Source*                   source,
  ...
  if (guide->sequenceSyncLengthTicks) {
    ...
    *noteLengthInSamples =
        (playbackHandler.getTimePerInternalTickBig() * guide->sequenceSyncLengthTicks) >>
    ...
    if (source->sampleControls.pitchAndSpeedAreIndependent) {
      *timeStretchRatio =
          (uint64_t)16777216 * (uint64_t)sampleLengthInSamples / *noteLengthInSamples;

      // And if pitch was manually adjusted too, that's fine - counteract that
      // by adjusting the time-stretch amount more
      if (*phaseIncrement != pitchAdjustNeutralValue) {
        *timeStretchRatio =
            ((uint64_t)*timeStretchRatio * pitchAdjustNeutralValue) / (uint32_t)*phaseIncrement;
      }

      // Or if no manual pitch adjustment...
      else {

        // If we'd only be time-stretching a tiiiny bit (1/10th of an octave
        // either direction)...
    ...
    // Or if pitch-stretch, achieve syncing that way
      ...
      // But first, if pitch was manually adjusted as well, counteract that by
      // adjusting the time-stretch amount more
      ...
      *phaseIncrement =
          (uint64_t)*phaseIncrement * (uint64_t)sampleLengthInSamples / *noteLengthInSamples;
...
// This normally only gets called from the getPitchAndSpeedParams() function,
// above, but occasionally we'll also call it from Voice::renderBasicSource()
// when doing a "late start" on a Sample and we need to disregard any pitch
// modulation, so send this the non-modulated phaseIncrement.
uint32_t VoiceUnisonPartSource::getSpeedParamForNoSyncing(Source* source,
  ...
  // If pitch and time being treated independently, then achieve this by
  // adjusting stretch to counteract pitch
  if (source->sampleControls.pitchAndSpeedAreIndependent) {
    if (phaseIncrement != pitchAdjustNeutralValue) {
      timeStretchRatio = ((uint64_t)pitchAdjustNeutralValue << 24) / (uint32_t)phaseIncrement;
    }
  ...
  // And whether or not that was the case, if there's a manual adjustment to
  // time-stretch, apply that now
  if (source->timeStretchAmount) {
    timeStretchRatio =
        ((uint64_t)timeStretchRatio * timeStretchAdjustTable[source->timeStretchAmount + 48]) >> 24;
  }
