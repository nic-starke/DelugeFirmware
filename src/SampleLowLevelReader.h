...
public:
  ...
  void jumpForwardLinear(int      numChannels,
                         int      byteDepth,
                         uint32_t bitMask,
                         int      jumpAmount,
  ...
  void fillInterpolationBufferRetrospectively(Sample* sample,
                                              int     bufferSize,
                                              int     startI,
                                              int     playDirection);
  void jumpBackSamples(Sample* sample, int numToJumpBack, int playDirection);
  void setupForPlayPosMovedIntoNewCluster(SamplePlaybackGuide* guide,
                                          Sample*              sample,
                                          int                  bytePosWithinNewCluster,
                                          int                  byteDepth);
  bool setupClusersForInitialPlay(SamplePlaybackGuide* guide,
                                  Sample*              sample,
                                  int                  byteOvershoot  = 0,
  ...
  bool changeClusterIfNecessary(SamplePlaybackGuide* guide,
                                Sample*              sample,
                                bool                 loopingAtLowLevel,
                                int                  priorityRating = 1);
  bool considerUpcomingWindow(SamplePlaybackGuide* guide,
                              Sample*              sample,
                              int*                 numSamples,
                              int32_t              phaseIncrement,
                              bool                 loopingAtLowLevel,
  ...
  int32_t getPlayByteLowLevel(Sample*              sample,
  ...
  bool    setupClustersForPlayFromByte(SamplePlaybackGuide* guide,
                                       Sample*              sample,
  ...
  void readSamplesNative(int32_t** __restrict__ oscBufferPos,
                         int            numSamplesTotal,
                         Sample*        sample,
                         int            jumpAmount,
                         int            numChannels,
  ...
  void readSamplesResampled(int32_t** __restrict__ oscBufferPos,
                            int      numSamples,
                            Sample*  sample,
                            int      jumpAmount,
                            int      numChannels,
  ...
  bool readSamplesForTimeStretching(int32_t*             oscBufferPos,
                                    SamplePlaybackGuide* guide,
                                    Sample*              sample,
                                    int                  numSamples,
                                    int                  numChannels,
  ...
  void bufferIndividualSampleForInterpolation(uint32_t bitMask,
                                              int      numChannels,
                                              int      byteDepth,
                                              char*    playPosNow);
  ...
  char*    clusterStartLocation; // You're allowed to read from this location, but
                                 // not move any further "back" past it
...
private:
  bool
  assignClusters(SamplePlaybackGuide* guide, Sample* sample, int clusterIndex, int priorityRating);
  bool fillInterpolationBufferForward(SamplePlaybackGuide* guide,
                                      Sample*              sample,
