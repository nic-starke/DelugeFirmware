...
SampleCluster::SampleCluster() {
  ...
  minValue                = 127;
  maxValue                = -128;
  sdAddress               = 0; // 0 means invalid, and we check for this as a last resort
                               // before writing
}
...
Cluster* SampleCluster::getCluster(Sample*  sample,
  ...
  if (!cluster) {
...
#if 1 || ALPHA_OR_BETA_VERSION // Switching permanently on for now, as users on
                               // on V4.0.x have been getting E341.
    if (cluster->numReasonsToBeLoaded < 1)
      numericDriver.freezeWithError(
          "i005"); // Diversifying Qui's E341. It should actually be exactly 1
    ...
    // Sometimes we don't actually want to load at all - if we're re-processing
    // a WAV file and want to overwrite a whole Cluster
    ...
    justEnqueue:
      ...
      audioFileManager.enqueueCluster(cluster, priorityRating); // TODO: If that fails, it'll just
                                                                // get awkwardly forgotten about
#if 1 || ALPHA_OR_BETA_VERSION // Switching permanently on for now, as users on
                               // on V4.0.x have been getting E341.
      if (cluster && cluster->numReasonsToBeLoaded <= 0)
        numericDriver.freezeWithError("i027"); // Diversifying Ron R's i004, which was diversifying
                                               // Qui's E341
#endif
    }
    ...
    // Or if want to try to load now...
      ...
      if (!result) {
        ...
        // If also an acceptable option, then just enqueue it, and we'll keep
        // the "reason" and return the pointer
        if (loadInstruction == CLUSTER_LOAD_IMMEDIATELY_OR_ENQUEUE) goto justEnqueue;

        // Or if it was a must-load-now...
        // Free and remove our link to the unloaded Cluster - otherwise the next
        // time we try to load it, it'd still exist but never get enqueued for
        // loading
        audioFileManager.deallocateCluster(
            cluster); // This removes the 1 reason that it'd still have

        if (error)
          *error = ERROR_UNSPECIFIED; // TODO: get actual error. Although
                                      // sometimes it'd just be a "can't do it
                                      // now cos card's being accessed, and
  ...
  // Or if it had previously been created...
...
#if 1 || ALPHA_OR_BETA_VERSION // Switching permanently on for now, as users on
                               // V4.0.x have been getting E341.
    if (cluster && cluster->numReasonsToBeLoaded < 0)
      numericDriver.freezeWithError("i028"); // bnhrsch got this!!
#endif

    // If they'd prefer it loaded immediately and it's not loaded, try speeding
    // loading along
    if ((loadInstruction == CLUSTER_LOAD_IMMEDIATELY ||
         loadInstruction == CLUSTER_LOAD_IMMEDIATELY_OR_ENQUEUE) &&
        !cluster->loaded) {
    ...
    audioFileManager.addReasonToCluster(cluster);
                               ...
                               // on V4.0.x have been getting E341.
    if (cluster && cluster->numReasonsToBeLoaded <= 0)
      numericDriver.freezeWithError("i025"); // Diversifying Ron R's i004, which
                                             // was diversifying Qui's E341
#endif
  }
