...
Song::Song() : backedUpParamManagers(sizeof(BackedUpParamManager)) {
  ...
  xZoom[NAVIGATION_CLIP] =
      increaseMagnitude(DEFAULT_CLIP_LENGTH, insideWorldTickMagnitude - displayWidthMagnitude);
...
Song::~Song() {
  ...
  // Free all ParamManagers which are backed up. The actual vector memory
  // containing all the BackedUpParamManager objects will be freed by the Vector
  // destructor
...
void Song::setupDefault() {
  ...
  if (whichScale == PRESET_SCALE_NONE) {
    whichScale = 0; // Major. Still need the *song*, (as opposed to the Clip) to
                    // have a scale
...
void Song::deleteAllBackedUpParamManagers(bool shouldAlsoEmptyVector) {
  for (int i = 0; i < backedUpParamManagers.getNumElements(); i++) {
    ...
    BackedUpParamManager* backedUp =
        (BackedUpParamManager*)backedUpParamManagers.getElementAddress(i);
...
void Song::deleteAllBackedUpParamManagersWithClips() {
  ...
  // We'll aim to repeatedly find the longest runs possible of ones with Clips,
  // to delete all in one go

  for (int i = 0; i < backedUpParamManagers.getNumElements(); i++) {
    BackedUpParamManager* firstBackedUp =
        (BackedUpParamManager*)backedUpParamManagers.getElementAddress(i);
  ...
  keepSearchingForward:
    ...
    // If still here, this is the first one with a Clip for this
    // ModControllable. Find the end of this ModControllable's ones
    int endIThisModControllable =
        backedUpParamManagers.search((uint32_t)modControllable + 4,
                                     GREATER_OR_EQUAL,
                                     searchedUpToAndIncluding + 1); // Search just by first word

    // But if that next one, for the next ModControllable, also has a Clip, we
    // can just keep looking forwards til we find one with no Clip
    ...
    // i will increment, which is fine, cos we've already determined that the
    // next element (if there is one) has no Clip, so we can skip it
  }
}
...
bool Song::ensureAtLeastOneSessionClip() {
  ...
  if (!sessionClips.getNumElements()) {
    ...
    result = Browser::findAnUnlaunchedPresetIncludingWithinSubfolders(NULL,
    ...
    if (!result.error) {
      ...
      result.error = storageManager.loadInstrumentFromFile(this,
                                                           firstClip,
                                                           INSTRUMENT_TYPE_SYNTH,
                                                           false,
                                                           &newInstrument,
    ...
    couldntLoad:
      ...
      int error2 = newInstrument->dirPath.set("SYNTHS"); // Use new error2 variable to preserve
                                                         // error result from before.
      if (error2) {
      gotError2:
        result.error = error2;
        goto reallyScrewed;
    ...
    ModelStackWithTimelineCounter* modelStackWithTimelineCounter =
        modelStack->addTimelineCounter(firstClip);
...
traverseClips:
  ...
  // Adjust scroll for Clips with the scale. Crudely - not as high quality as
  // happens for the Clip being processed in enterScaleMode();
  ...
  int rootNoteChangeEffect =
      rootNoteChange * (12 - numModeNotes) / 12; // I wasn't quite sure whether this should use
                                                 // numModeNotes or oldNumModeNotes
...
// Sets up a mode-note, optionally specifying that we prefer it a semitone
// higher, although this may be overridden by what actual note is present
...
traverseClips:
  ...
  // If we were shifting the root note, then we actually want to shift all the
  // other scale-notes
  if (yVisualWithinOctave == 0) {
    for (int i = 1; i < numModeNotes; i++) { modeNotes[i] -= change; }
    rootNote += change;
  }
...
bool Song::mayMoveModeNote(int16_t yVisualWithinOctave, int8_t newOffset) {
  ...
  if (yVisualWithinOctave == 0 && newOffset == -1) {
    return (modeNotes[numModeNotes - 1] < 11); // May ony move down if the top note in scale isn't
                                               // directly below (at semitone 11)
  }
    ...
    return (
        (newOffset == 1 &&                      // We're moving up and
         ...
         (yVisualWithinOctave == numModeNotes - 1 || // Either we're the top note, so don't
                                                     // need to check for a higher neighbour
          modeNotes[yVisualWithinOctave + 1] >
              modeNotes[yVisualWithinOctave] +
                  1) // Or the next note up has left us space to move up
         ) ||
        (newOffset == -1 &&                    // We're moving down and
         modeNotes[yVisualWithinOctave] > 1 && // We're not already at the bottom of the scale
                                               // (apart from the root note) and
         modeNotes[yVisualWithinOctave - 1] <
...
void Song::inputTickScalePotentiallyJustChanged(uint32_t oldScale) {
  ...
  // Chances are we'll have to change the input tick magnitude to account for
  // the magnitudinal difference between say a 1-based and 9-based time
  ...
  // We then do a very similar process again, to calculate
  // insideWorldTickMagnitudeOffsetFromBPM in such a way that, say, 8th-notes
  // will always appear about the same length to the user
...
// This is a little bit of an ugly hack - normally this is true, but we'll
// briefly set it to false while doing that "revert" that we do when
// (re)lengthening a Clip
bool allowResyncingDuringClipLengthChange = true;

// If action is NULL, that means this is being called as part of an undo, so
// don't do any extra stuff. Currently mayReSyncClip is only set to false in a
// call that happens when we've just finished recording that Clip.
void Song::setClipLength(Clip* clip, uint32_t newLength, Action* action, bool mayReSyncClip) {
  ...
  if (action)
    action->recordClipLengthChange(clip,
                                   oldLength); // Records just the simple fact
                                               // that clip->length has changed

  char                           modelStackMemory[MODEL_STACK_MAX_SIZE];
  ModelStackWithTimelineCounter* modelStack =
      setupModelStackWithSong(modelStackMemory, this)->addTimelineCounter(clip);

  if (newLength < oldLength) { clip->lengthChanged(modelStack, oldLength, action); }
  ...
  if (playbackHandler.isEitherClockActive() && isClipActive(clip)) {
    ...
    if (mayReSyncClip) {
      ...
      playbackHandler.expectEvent(); // Is this maybe redundant now that
                                     // Arranger has a reSyncClip()?
    }
...
void Song::doubleClipLength(InstrumentClip* clip, Action* action) {
  ...
  clip->increaseLengthWithRepeats(modelStack,
                                  oldLength << 1,
...
Clip* Song::getNextSessionClipWithOutput(int offset, Output* output, Clip* prevClip) {
  ...
  int oldIndex = -1;
  if (prevClip) { oldIndex = sessionClips.getIndexForClip(prevClip); }
...
void Song::writeToFile() {
  ...
  if (lastClipInstanceEnteredStartPos != -1) {
    storageManager.writeAttribute("currentTrackInstanceArrangementPos",
  ...
  for (int i = 0; i < numModeNotes; i++) { storageManager.writeTag("modeNote", modeNotes[i]); }
  ...
  for (int s = 0; s < MAX_NUM_SECTIONS; s++) {
    if (true || sections[s].launchMIDICommand.containsSomething() || sections[s].numRepetitions) {
      ...
      if (sections[s].launchMIDICommand.containsSomething()) {
        // Annoyingly, I used one-off tag names here, rather than it conforming
        // to what the LearnedMIDI class now uses.
        storageManager.writeAttribute("midiCommandChannel",
                                      sections[s].launchMIDICommand.channelOrZone,
                                      false);
        storageManager.writeAttribute("midiCommandNote",
                                      sections[s].launchMIDICommand.noteOrCC,
                                      false);
  ...
  if (arrangementOnlyClips.getNumElements()) {
    ...
    for (int c = 0; c < arrangementOnlyClips.getNumElements(); c++) {
      ...
      if (!clip->output->clipHasInstance(clip))
        continue; // Get rid of any redundant Clips. There shouldn't be any, but
                  // occasionally they somehow get left over.
      clip->writeToFile(this);
    }
...
int Song::readFromFile() {
  ...
  for (int s = 0; s < MAX_NUM_SECTIONS; s++) { sections[s].numRepetitions = -1; }
    ...
    switch (*(uint32_t*)tagName) {
        ...
        while (*(tagName = storageManager.readNextTagOrAttributeName())) {
          if (!strcmp(tagName, "roomSize")) {
            ...
            if (widthInt == -2147483648)
              widthInt = 2147483647; // Was being saved incorrectly in V2.1.0-beta1 and
                                     // alphas, so we fix it on read here!
      ...
      case 'rcSx':
        ...
        switch (*(((uint32_t*)tagName) + 1)) {
          ...
          // "xScrollSongView"
          case 0x536c6c6f:
            if (*(((uint32_t*)tagName) + 2) == 0x56676e6f &&
                *(((uint32_t*)tagName) + 3) == 0x00776965) {
          ...
          case 0x416c6c6f:
            if (*(((uint32_t*)tagName) + 2) == 0x6e617272 &&
                *(((uint32_t*)tagName) + 3) == 0x656d6567 &&
      ...
      unknownTag:
        ...
        if (!strcmp(tagName, "xZoomArrangementView")) {
          ...
          storageManager.exitTag("xZoomArrangementView");
                         ...
                         "inArrangementView")) { // For V2.0 pre-beta songs. There'd
                                                 // be another way to detect this...
          ...
          // swingInterval, unlike other "sync" type params, we're going to read
          // as just its plain old int value, and only shift it by
          // insideWorldTickMagnitude after reading the whole song. This is
          // because these two attributes could easily be stored in either order
          // in the file, so we won't know both until the end. Also, in firmware
          // pre V3.1.0-alpha, all "sync" values were stored as plain old ints, to
          // be read irrespective of insideWorldTickMagnitude
          ...
          globalEffectable.modKnobMode =
              getMin(globalEffectable.modKnobMode, (uint8_t)(NUM_MOD_BUTTONS - 1));
          ...
          while (*(tagName = storageManager.readNextTagOrAttributeName())) {
            if (!strcmp(tagName, "section")) {
              ...
              while (*(tagName = storageManager.readNextTagOrAttributeName())) {
                ...
                // Annoyingly, I used one-off tag names here, rather than it
                // conforming to what the LearnedMIDI class now uses.
          ...
          while (*(tagName = storageManager.readNextTagOrAttributeName())) {
            ...
            loadOutput:
              error =
                  newOutput->readFromFile(this, NULL, 0); // If it finds any default params, it'll
                                                          // make a ParamManager and "back it up"
        ...
        else if (!strcmp(tagName, "arrangementOnlyTracks") ||
                 !strcmp(tagName, "arrangementOnlyClips")) {
  ...
  if (storageManager.firmwareVersionOfFileBeingRead >= FIRMWARE_3P1P0_ALPHA2) {
    // Basically, like all other "sync" type parameters, the file value and
    // internal value are different for swingInterval. But unlike other ones,
    // which get converted as we go, we do this one at the end once we know we
...
traverseClips:
  for (int c = 0; c < clipArray->getNumElements(); c++) {
    ...
    if (storageManager.firmwareVersionOfFileBeingRead < FIRMWARE_1P5P0_PREBETA &&
      ...
      if (((InstrumentClip*)thisClip)->arpSettings.mode &&
          !((InstrumentClip*)thisClip)->arpSettings.syncLevel) {
        ParamManagerForTimeline* thisParamManager = &thisClip->paramManager;
        thisParamManager->getPatchedParamSet()->params[PARAM_GLOBAL_ARP_RATE].shiftValues(
            (1 << 30) + (1 << 28));
      }
    }
  }
  ...
  // Match all ClipInstances up with their Clip. And while we're at it, check if
  // any Outputs are soloing in arranger
  for (Output* thisOutput = firstOutput; thisOutput; thisOutput = thisOutput->next) {
    ...
    // If saved before V2.1, set sample-based synth instruments to linear
    // interpolation, cos that's how it was
  ...
  if (storageManager.firmwareVersionOfFileBeingRead < FIRMWARE_1P2P0) {
    ...
    deleteAllBackedUpParamManagers(true); // Before V1.2, lots of extras of these could be created
                                          // during loading
  ...
  if (syncScalingClip) {
    ...
    syncScalingClip             = NULL; // We shouldn't have set this manually earlier,
                                        // anyway - we just saved hassle
  ...
  int playbackWillStartInArrangerAtPos =
      playbackHandler.playbackState ? lastClipInstanceEnteredStartPos : -1;
...
int Song::readClipsFromFile(ClipArray* clipArray) {
  ...
  while (*(tagName = storageManager.readNextTagOrAttributeName())) {
    ...
    readClip:
      ...
      clipArray->insertClipAtIndex(newClip,
                                   clipArray->getNumElements()); // We made sure enough space, above
...
// Needs to be in a separate function than the above because the main song XML
// file needs to be closed first before this is called, because this will open
// other (sample) files
...
traverseClips:
  for (int c = 0; c < clipArray->getNumElements(); c++) {
    ...
    // If not reading files, high chance that we'll be searching through memory
    // a lot and not reading the card (which would call the audio routine), so
    // we'd better call the audio routine here.
    ...
    Clip* clip = clipArray->getClipAtIndex(c);
    if (clip->type == CLIP_TYPE_AUDIO) { ((AudioClip*)clip)->loadSample(mayActuallyReadFiles); }
  }
  if (clipArray != &arrangementOnlyClips) {
    clipArray = &arrangementOnlyClips;
    goto traverseClips;
...
traverseClips:
  for (int c = 0; c < clipArray->getNumElements(); c++) {
    ...
    if (!clip->isActiveOnOutput() &&
        clip != view.activeModControllableModelStack.getTimelineCounterAllowNull()) {
      deleteClipObject(clip, false, INSTRUMENT_REMOVAL_NONE);
...
traverseClips2:
  ...
  // Can't delete the ones with no Clips, cos these might be needed by their
  // ModControllables
...
void Song::renderAudio(StereoSample* outputBuffer,
  ...
  // A "post-FX volume" calculation also happens in audioDriver.render(), which
  // is a bit more relevant really because that's where filters are happening
  ...
  for (Output* output = firstOutput; output; output = output->next) {
    ...
    bool isClipActiveNow =
        (output->activeClip && isClipActive(output->activeClip->getClipBeingRecordedFrom()));

    // AudioEngine::logAction("outp->render");
    output->renderOutput(modelStack,
                         outputBuffer,
  ...
  // If recording the "MIX", this is the place where we want to grab it - before
  // any master FX or volume applied Go through each SampleRecorder, feeding
  // them audio
  ...
  globalEffectable.processFXForGlobalEffectable(outputBuffer,
                                                numSamples,
                                                &volumePostFX,
                                                &paramManager,
                                                &delayWorkingState,
  ...
  int32_t reverbSendAmount = getFinalParameterValueVolume(
      paramNeutralValues[PARAM_GLOBAL_REVERB_AMOUNT],
      cableToLinearParamShortcut(paramManager.getUnpatchedParamSet()->getValue(
          PARAM_UNPATCHED_GLOBALEFFECTABLE_REVERB_SEND_AMOUNT)));

  globalEffectable.processReverbSendAndVolume(outputBuffer,
                                              numSamples,
                                              reverbBuffer,
                                              volumePostFX,
                                              postReverbVolume,
...
void Song::setTimePerTimerTick(uint64_t newTimeBig, bool shouldLogAction) {
  ...
  if (shouldLogAction) { actionLogger.recordTempoChange(timePerTimerTickBig, newTimeBig); }
  ...
  if (currentSong == this && (playbackHandler.playbackState & PLAYBACK_CLOCK_INTERNAL_ACTIVE)) {
    ...
    // Using intermediary float here is best: newTimeBig might be a huge number,
    // and below about 2bpm (so not that low), multiplying it by
    // timeSinceLastTimerTick can overflow a uint64_t
    ...
    playbackHandler.timeNextTimerTickBig =
        (uint64_t)(AudioEngine::audioSampleTimer + timeTilNextTimerTick) << 32;
  }
...
void Song::resyncLFOsAndArpeggiators() {
  ...
  for (Output* thisOutput = firstOutput; thisOutput; thisOutput = thisOutput->next) {
    ...
    // Yes, do it even for Clips that aren't actually "playing" / active in the
    // Song
  }
}
...
NoteRow* Song::findNoteRowForDrum(Kit* kit, Drum* drum, Clip* stopTraversalAtClip) {
  ...
  // If currently swapping an Instrument, it can't be assumed that all
  // arranger-only Clips for this Instrument are in its clipInstances, which
  // otherwise is a nice time-saver

  // For each Clip in session and arranger for specific Output - but if
  // currentlySwappingInstrument, use master list for arranger Clips
  ClipArray* clipArray                  = &sessionClips;
...
Song::findParamManagerForDrum(Kit* kit, Drum* drum, Clip* stopTraversalAtClip) {
...
void Song::setupPatchingForAllParamManagersForDrum(SoundDrum* drum) {
  ...
  // We don't know the Kit / Instrument. That's unfortunate. We'll work it out
  // if we can
  Output* output = NULL;
...
traverseClips:
  for (int c = 0; c < numElements; c++) {
    ...
    if (noteRow) {
      if (!output)
        output = instrumentClip->output; // The "if" check at the start should
                                         // be unnecessary... but just in case.

      if (noteRow->paramManager.containsAnyMainParamCollections()) {

        ModelStackWithParamCollection* modelStackWithParamCollection =
            noteRow->paramManager.getPatchCableSet(
  ...
  // If we just finished iterating the sessionClips, we next want to do the
  // arrangementOnlyClips.
  if (!doingClipsProvidedByOutput && clipArray == &sessionClips) {
    // If Instrument is currently being "swapped", or if we still don't actually
    // know the Output, then we have to do things the slow way and search every
    // arrangementOnlyClip
    ...
    // Or more ideally, we would have found the Output along the way, so we can
    // just grab its arrangement-only Clips directly from it
    else {
      doingClipsProvidedByOutput = true;
      numElements                = output->clipInstances.getNumElements();
      goto traverseClips;
...
traverseClips:
  for (int c = 0; c < numElements; c++) {
    ...
    ModelStackWithThreeMainThings* modelStackWithThreeMainThings =
        modelStack->addParamManager(&clip->paramManager);
    ModelStackWithParamCollection* modelStackWithParamCollection =
...
// kit is required, fortunately. Unlike some of the other "for drum" functions
// here.
void Song::grabVelocityToLevelFromMIDIDeviceAndSetupPatchingForAllParamManagersForDrum(
    MIDIDevice* device,
    SoundDrum*  drum,
    Kit*        kit) {
...
void Song::grabVelocityToLevelFromMIDIDeviceAndSetupPatchingForEverything(MIDIDevice* device) {
  // if (!device->hasDefaultVelocityToLevelSet()) return; // In this case, we'll
  // take 0 to actually mean zero.
...
int Song::cycleThroughScales() {
  ...
  // Firstly, make all current mode notes as high as they can possibly go, so
  // there'll be no crossing over when we go to actually do it, below For each
  // InstrumentClip in session and arranger
  ClipArray* clipArray = &sessionClips;
traverseClips:
  for (int c = 0; c < clipArray->getNumElements(); c++) {
    Clip* clip = clipArray->getClipAtIndex(c);
  ...
  for (int n = 1; n < 7; n++) { modeNotes[n] = 5 + n; }
...
traverseClips2:
  ...
  for (int n = 1; n < 7; n++) { modeNotes[n] = presetScaleNotes[newScale][n]; }
...
traverseClips:
  for (int c = 0; c < clipArray->getNumElements(); c++) {
    ...
    instrumentClip->ensureInaccessibleParamPresetValuesWithoutKnobsAreZero(
...
void Song::deleteClipObject(Clip* clip,
  ...
  if (!songBeingDestroyedToo) {
    ...
    char                           modelStackMemory[MODEL_STACK_MAX_SIZE];
    ModelStackWithTimelineCounter* modelStack =
        setupModelStackWithTimelineCounter(modelStackMemory, this, clip);
...
void Song::turnSoloingIntoJustPlaying(bool getRidOfArmingToo) {
  ...
  // Stop all other playing-but-not-soloing Clips, and turn all soloing Clip
  // into playing Clip! For each Clip in session
  for (int c = 0; c < sessionClips.getNumElements(); c++) {
    Clip* clip = sessionClips.getClipAtIndex(c);
    ...
    if (getRidOfArmingToo) { clip->armState = ARM_STATE_OFF; }
  }
...
uint32_t Song::getTimePerTimerTickRounded() {
  return (uint64_t)((uint64_t)timePerTimerTickBig + (uint64_t)(uint32_t)2147483648uL) >>
         32; // Rounds
}
...
void Song::deleteOutputThatIsInMainList(
    Output* output,
    bool    stopAnyAuditioningFirst // Usually true, but if deleting while loading
                                    // a Song due to invalid data, we don't want
                                    // this, and it'd cause an error.
) {
...
int Song::removeOutputFromMainList(
  ...
  *prevPointer = output->next;
  ...
  // If the removed Output was soloing, and we haven't yet seen any other
  // soloing Outputs, we'd better check out the rest
...
// Any audio routine calls that happen during the course of this function won't
// have access to either the old or new Instrument, because neither will be in
// the master list when they happen
void Song::replaceInstrument(Instrument* oldOutput,
                             Instrument* newOutput,
                             bool        keepNoteRowsWithMIDIInput) {

  // We don't detach the Instrument's activeClip here anymore. This happens in
  // InstrumentClip::changeInstrument(), called below. If we changed it here,
  // near-future calls to the audio routine could cause new voices to be
  ...
  *prevPointer    = oldOutput->next;
  ...
  // Migrate input MIDI channel / device. Putting this up here before any calls
  // to changeInstrument() is good, because then if a default velocity is set,
  // for the MIDIDevice, that gets grabbed by the Clip's ParamManager during
  // that call.
...
traverseClips:
  for (int c = 0; c < numElements; c++) {
    ...
    if (oldOutput->type != OUTPUT_TYPE_AUDIO) {
      ...
      int error =
          instrumentClip->changeInstrument(modelStackWithTimelineCounter,
                                           newOutput,
                                           NULL,
    ...
    // If this is the first Clip we dealt with, tell all the rest of the Clips
    // to just clone it from this one (if there isn't already a ParamManager
    // backed up in memory for them)
    if (!favourClipForCloningParamManager) { favourClipForCloningParamManager = clip; }
  }
  if (!doingArrangementClips) {
    doingArrangementClips = true;
...
void Song::deleteOrAddToHibernationListOutput(Output* output) {
  // If un-edited (which will include all CV Instruments, and any MIDI without
  // mod knob assignments)
...
void Song::deleteOrHibernateOutput(Output* output) {
  // If edited (which won't include any CV Instruments), just hibernate it. Only
  // allowed for audio Instruments
...
void Song::deleteOrHibernateOutputIfNoClips(Output* output) {
  ...
  if (!output->activeClip) { deleteOrHibernateOutput(output); }
}
...
AudioOutput* Song::getAudioOutputFromName(String* name) {
  for (Output* thisOutput = firstOutput; thisOutput; thisOutput = thisOutput->next) {
    if (thisOutput->type == OUTPUT_TYPE_AUDIO) {
      if (thisOutput->name.equalsCaseIrrespective(name)) { return (AudioOutput*)thisOutput; }
    }
  }
  return NULL;
}
...
Instrument* Song::getInstrumentFromPresetSlot(int         instrumentType,
  ...
  if (searchHibernating) {
    for (Output* thisOutput = firstHibernatingInstrument; thisOutput;
...
int Song::getNumOutputs() {
  int count = 0;
  for (Output* output = firstOutput; output; output = output->next) { count++; }
  return count;
}
...
// Supply stealInto to have it delete the "backed up" element, putting the
// contents into stealInto.
ParamManager* Song::getBackedUpParamManagerForExactClip(ModControllableAudio* modControllable,
  ...
  if (stealInto) {
    stealInto->stealParamCollectionsFrom(&elementCorrectClip->paramManager,
                                         true); // Steal expression params too - if they're here
                                                // (slightly rare case).
...
// If none for the correct Clip, return one for a different Clip - prioritizing
// NULL Clip. Returns NULL if couldn't find one. Supply stealInto to have it
// delete the "backed up" element, putting the contents into stealInto.
ParamManager* Song::getBackedUpParamManagerPreferablyWithClip(ModControllableAudio* modControllable,
                                                              Clip*                 clip,
                                                              ParamManager*         stealInto) {

  int iAnyClip = backedUpParamManagers.search((uint32_t)modControllable,
                                              GREATER_OR_EQUAL); // Search just by first word
  if (iAnyClip >= backedUpParamManagers.getNumElements()) return NULL;
  BackedUpParamManager* elementAnyClip =
      (BackedUpParamManager*)backedUpParamManagers.getElementAddress(iAnyClip);
  if (elementAnyClip->modControllable != modControllable)
    return NULL; // If nothing with even the correct modControllable at all, get
                 // out
...
// Steals stuff.
// shouldStealExpressionParamsToo should only be true to save expression params
// from being destructed (e.g. if the Clip is being destructed).
...
void Song::deleteBackedUpParamManagersForClip(Clip* clip) {
  ...
  // Ok, this is the one sticky one where we actually do have to go through
  // every element
  int i = 0;
  ...
  while (i < backedUpParamManagers.getNumElements()) {
    ...
    if (backedUp->clip == clip) {
      ...
      // We ideally want to just set the Clip to NULL. We can just do this if
      // the previous element didn't have the same ModControllable
      if (i == 0 || ((BackedUpParamManager*)backedUpParamManagers.getElementAddress(i - 1))
                            ->modControllable != backedUp->modControllable) {
        backedUp->clip = NULL;
        i++;
      }
      ...
      // Othwerwise...
        ...
        int                   j = backedUpParamManagers.search((uint32_t)modControllable,
                                             GREATER_OR_EQUAL,
                                             0,
        ...
        // Otherwise, we insert before it
          ...
          // If RAM error (surely would never happen since we just deleted an
          // element)...
          if (error) {
            // Don't increment i, as we've deleted an element instead
          }
...
// TODO: should we also check whether any arranger clips active and playing in
// session mode? For next function too...
...
bool Song::doesNonAudioSlotHaveActiveClipInSession(int instrumentType, int slot, int subSlot) {
  ...
  for (int c = 0; c < sessionClips.getNumElements(); c++) {
    ...
    if (isClipActive(clip) && clip->type == CLIP_TYPE_INSTRUMENT) {
      ...
      if (instrument->type == instrumentType &&
          ((NonAudioInstrument*)instrument)->channel == slot &&
          (instrumentType == INSTRUMENT_TYPE_CV ||
           ((MIDIInstrument*)instrument)->channelSuffix == subSlot)) {
        return true;
      }
    }
  }
...
void Song::restoreClipStatesBeforeArrangementPlay() {
  ...
  // Do not set the Instruments' activeClips. We want them to stay as they were
  // when the song ended
}
...
// This is only called right after Song loaded, so we can assume all Instruments
// have a NULL activeClip It's not possible for this to stop there from being
// more than zero soloing Clips
void Song::sortOutWhichClipsAreActiveWithoutSendingPGMs(ModelStack* modelStack,
                                                        int playbackWillStartInArrangerAtPos) {

  AudioEngine::logAction("aaa5.11");

  // If beginning playback in arranger, that's where we figure out which Clips
  // are active - on their Outputs, and generally
  if (playbackWillStartInArrangerAtPos != -1) {
    anyClipsSoloing = false;

    // We still want as many Outputs as possible to have activeClips, even if
    // those Clips are not "active". First, try arranger-only Clips.
    for (Output* output = firstOutput; output; output = output->next) {

      // Don't do any additional searching of session Clips, cos it'd be really
      // inefficient searching all session Clips for each Output
      output->pickAnActiveClipForArrangementPos(modelStack,
                                                playbackWillStartInArrangerAtPos,
                                                PGM_CHANGE_SEND_NEVER);
    }
  }

  // If not about to start playback in arranger, we give the active session
  // Clips first dibs on being active on their Output
  else {
    ...
    for (Output* output = firstOutput; output; output = output->next) {
      ...
      // Don't do any additional searching of session Clips, cos it'd be really
      // inefficient searching all session Clips for each Instrument
  ...
  // And finally, go through session Clips again, giving any more to Instruments
  // that can be given
  for (int c = 0; c < sessionClips.getNumElements(); c++) {
    Clip* clip = sessionClips.getClipAtIndex(c);
    if (!(count & 7)) {
    ...
    // And if beginning arranger playback, some additional setting up to do at
    // the same time
    if (playbackWillStartInArrangerAtPos != -1) {
  ...
  // Now, we want to ensure (in case of bad song file data) that any Output that
  // doesn't have an activeClip (aka doesn't have ANY Clip) definitely does have
  // a backedUpParamManager. Only for audio Instruments
  for (Output* output = firstOutput; output;) {

    Output* nextOutput = output->next;

    // Also while we're at it, for SoundInstruments (Synths), get them to grab a
    // copy of the arp settings
    ...
    // Ok, back to the main task - if there's no activeClip...
      ...
      if (output->type == INSTRUMENT_TYPE_SYNTH || output->type == INSTRUMENT_TYPE_KIT) {
        if (!getBackedUpParamManagerPreferablyWithClip(
...
#endif
          deleteOutputThatIsInMainList(output,
                                       false); // Do *not* try to stop any auditioning first. There
                                               // is none, and doing so would/did cause an E170.
          goto getOut;
        }
      }
      ...
      for (Drum* thisDrum = kit->firstDrum; thisDrum; thisDrum = thisDrum->next) {
        if (thisDrum->type == DRUM_TYPE_SOUND) {
          ...
          if (!getBackedUpParamManagerPreferablyWithClip(soundDrum,
            ...
            if (!findParamManagerForDrum(kit, soundDrum)) { // If no ParamManager with a NoteRow
                                                            // somewhere...
...
// Can assume that no soloing when this is called, i.e. any Clips in here which
// say they're active actually are
...
void Song::ensureAllInstrumentsHaveAClipOrBackedUpParamManager(
  ...
  for (Instrument* thisInstrument = firstHibernatingInstrument; thisInstrument;
    ...
    if (thisInstrument->type != INSTRUMENT_TYPE_SYNTH &&
        thisInstrument->type != INSTRUMENT_TYPE_KIT)
      continue;
    ...
    if (getClipWithOutput(thisInstrument)) {
      numericDriver.freezeWithError("E056"); // gtridr got, V4.0.0-beta2. Before I fixed memory
                                             // corruption issues, so hopefully could just be that.
    }

    else {
      if (!getBackedUpParamManagerPreferablyWithClip(
              (ModControllableAudio*)thisInstrument->toModControllable(),
              NULL)) {
...
// Normally we leave detachClipsToo as false, becuase we need to keep them
// attached because resumeClipsClonedForArrangementRecording() is about to be
// called, and needs them attached, and will detach them itself
...
void Song::clearArrangementBeyondPos(int32_t pos, Action* action) {
  ...
  for (Output* thisOutput = firstOutput; thisOutput; thisOutput = thisOutput->next) {
    ...
    // We go through deleting the ClipInstances one by one. This is actually
    // quite inefficient, but complicated to improve on because the deletion of
    // the Clips themselves, where there are arrangement-only ones, causes the
    // calling of output->pickAnActiveClipIfPossible. So we have to ensure that
    // extra ClipInstances don't exist at any instant in time, or else it'll
    // look at those to pick the new activeClip, which might not exist anymore.
    for (int j = thisOutput->clipInstances.getNumElements() - 1; j >= i; j--) {
      ...
      deletingClipInstanceForClip(thisOutput,
                                  clip,
                                  action,
                                  true); // Could be bad that this calls the audio routine before
                                         // we've actually deleted the ClipInstances...
    }
...
// Note: in most cases (when action supplied), will try to pick a new activeClip
// even if told not to. But this should be ok
void Song::deletingClipInstanceForClip(Output* output,
                                       Clip*   clip,
                                       Action* action,
                                       bool    shouldPickNewActiveClip) {

  // If clipInstance had a Clip, and it's a (white) arrangement-only Clip, then
  // the whole Clip needs deleting.
  if (clip && clip->isArrangementOnlyClip()) {
    ...
    if (action) {
      ...
      // That call will call pickAnActiveClipIfPossible() whether we like it or
      // not...
    }
    ...
    if (!deletionDone) { // If not enough memory to create undo-history...
      ...
      // Will not remove Instrument from Song. Will call audio routine! That's
      // why we deleted the ClipInstance first
...
bool Song::canOldOutputBeReplaced(Clip* clip, int* availabilityRequirement) {
  // If Clip has an "instance" within its Output in arranger, then we can only
  // change the entire Output to a different Output
  if (clip->output->clipHasInstance(clip)) {
    ...
    if (availabilityRequirement) {
      // If Clip is "active", just make sure we pick an Output that doesn't have
      // a Clip "active" in session
      if (isClipActive(clip)) {
    ...
    // We still may as well replace the Output so long as it doesn't have any
    // *other* Clips
...
void Song::instrumentSwapped(Instrument* newInstrument) {
  ...
  // If we're playing, in this arrangement mode... (TODO: what if it just
  // switched on while we were loading?)
  ...
  // Or if not, is there another Clip which is active, which needs sorting out
  // with the newInstrument?
  else {
    ...
    if (thisClip) {
      ...
      // Assert that thisClip is the active Clip with this Instrument - make any
      // other Clips inactive (activity status could have changed while we were
      // loading...)
...
Instrument* Song::changeInstrumentType(Instrument* oldInstrument, int newInstrumentType) {
  ...
  if (newInstrumentType == INSTRUMENT_TYPE_MIDI_OUT || newInstrumentType == INSTRUMENT_TYPE_CV) {
    ...
    newInstrument =
        storageManager.createNewNonAudioInstrument(newInstrumentType, newSlot, newSubSlot);
    if (!newInstrument) {
  ...
  // Synth or Kit
    ...
    result =
        Browser::findAnUnlaunchedPresetIncludingWithinSubfolders(this,
                                                                 newInstrumentType,
...
AudioOutput* Song::getFirstAudioOutput() {
  for (Output* output = firstOutput; output; output = output->next) {
    if (output->type == OUTPUT_TYPE_AUDIO) { return (AudioOutput*)output; }
  }
  return NULL;
}
...
AudioOutput* Song::createNewAudioOutput(Output* replaceOutput) {
  ...
  // Set input channel to previously used one. If none selected, see what's in
  // Song
  ...
  backUpParamManager((ModControllableAudio*)newOutput->toModControllable(),
                     NULL,
                     &newParamManager,
                     true);
...
// Must supply a char[5] buffer. Or char[30] should be more than adequate for
// OLED.
...
Instrument* Song::getNonAudioInstrumentToSwitchTo(int     newInstrumentType,
  ...
  while (true) {
    ...
    newInstrument =
        getInstrumentFromPresetSlot(newInstrumentType,
                                    newSlot,
                                    newSubSlot,
                                    NULL,
                                    NULL,
...
void Song::removeSessionClip(Clip* clip, int clipIndex, bool forceClipsAboveToMoveVertically) {
  ...
  // Must unsolo the Clip before we delete it, in case its play-pos needs to be
  // grabbed for another Clip
  if (clip->soloingInSessionMode) { session.unsoloClip(clip); }
  ...
  for (int i = 0; i < output->clipInstances.getNumElements(); i++) {
    ...
    lookAtNextOne:
      if (i + 1 < output->clipInstances.getNumElements() &&
        ...
        // See if next ClipInstance has the same Clip and lines up as a
        // repeat...
        ...
        if (nextClipInstance->clip == clip && startPos + lengthGotUpTo == nextClipInstance->pos) {
          ...
          arrangement.rowEdited(output,
                                nextClipInstance->pos,
                                nextClipInstance->pos + nextClipInstance->length,
                                clip,
                                NULL);
      ...
      if (deletedAnyElements)
        clipInstance =
            output->clipInstances.getElement(i); // Gotta re-get, since storage has changed

      // If we'd already found one, we'll have to create a clone for this one -
  ...
  // If there was at least one instance, don't properly delete the Clip - just
  // put it in the arranger only. But do stop it playing!
  if (foundAtLeastOneInstanceInArranger) {
  ...
  AudioEngine::mustUpdateReverbParamsBeforeNextRender =
      true; // Necessary? Maybe the Instrument would get deleted from the master
            // list?
}
...
bool Song::deletePendingOverdubs(Output* onlyWithOutput,
                                 int*    originalClipIndex,
                                 bool    createConsequencesForOtherLinearlyRecordingClips) {

  // You'd kind of think that we'd want to just not bother with this if playback
  // isn't active, but we're not allowed to apply that logic, cos this will get
  // called as playback ends, but after playbackState is set to 0
  ...
  for (int c = sessionClips.getNumElements() - 1; c >= 0; c--) {
    ...
    if (clip->isPendingOverdub && (!onlyWithOutput || clip->output == onlyWithOutput)) {
      ...
      if (originalClipIndex && *originalClipIndex > c) { (*originalClipIndex)--; }
...
Clip* Song::createPendingNextOverdubBelowClip(Clip* clip, int clipIndex, int newOverdubNature) {
  ...
  // No automatic overdubs are allowed during soloing, cos that's just too
  // complicated
  ...
  if (newClip) {
    ...
    if (clipIndex != songViewYScroll) { songViewYScroll++; }
...
void Song::swapClips(Clip* newClip, Clip* oldClip, int clipIndex) {
  ...
  if (oldClip == getSyncScalingClip()) { syncScalingClip = newClip; }

  if (oldClip == currentClip) { currentClip = newClip; }
...
Clip* Song::replaceInstrumentClipWithAudioClip(Clip* oldClip, int clipIndex) {
  ...
  if (playbackHandler.playbackState && isClipActive(oldClip)) {
    ...
    // Must unsolo the Clip before we delete it, in case its play-pos needs to
    // be grabbed for another Clip
    if (oldClip->soloingInSessionMode) { session.unsoloClip(oldClip); }
  }
...
void Song::changeSwingInterval(int newValue) {
  ...
  if (playbackHandler.playbackState & PLAYBACK_CLOCK_INTERNAL_ACTIVE) {
    ...
    if (startOfSwingWindow != playbackHandler.lastTimerTickActioned) {
      playbackHandler.timeLastTimerTickBig =
          (uint64_t)playbackHandler.getInternalTickTime(startOfSwingWindow) << 32;
...
bool Song::isPlayingAutomationNow() {
  return (currentPlaybackMode == &arrangement ||
...
// I think I created this function to be called during the actioning of a swung
// tick, when we know that no further swung ticks have passed since the last
// actioned one
...
void Song::setDefaultVelocityForAllInstruments(uint8_t newDefaultVelocity) {
  ...
  for (Instrument* instrument = firstHibernatingInstrument; instrument;
...
int Song::convertSyncLevelFromFileValueToInternalValue(int fileValue) {
  ...
  // The file value is relative to insideWorldTickMagnitude etc, though if
  // insideWorldTickMagnitude is 1 (which used to be the default), it comes out
  // as the same value anyway (kind of a side point)

  if (fileValue == 0) return 0; // 0 means "off"
  int internalValue =
      fileValue + 1 - (insideWorldTickMagnitude + insideWorldTickMagnitudeOffsetFromBPM);
  if (internalValue < 1)
    internalValue = 1;
...
int Song::convertSyncLevelFromInternalValueToFileValue(int internalValue) {
  ...
  int fileValue =
      internalValue - 1 + (insideWorldTickMagnitude + insideWorldTickMagnitudeOffsetFromBPM);
...
void Song::midiDeviceBendRangeUpdatedViaMessage(ModelStack* modelStack,
  ...
  for (Output* thisOutput = currentSong->firstOutput; thisOutput; thisOutput = thisOutput->next) {
    thisOutput->offerBendRangeUpdate(modelStack,
                                     device,
                                     channelOrZone,
                                     whichBendRange,
                                     bendSemitones);
...
int Song::addInstrumentsToFileItems(int instrumentType) {
  ...
  // First, check all other Instruments in memory, in case the next one in line
  // isn't saved
  for (; thisOutput; thisOutput = thisOutput->next) {
    ...
    if (!thisItem) { return ERROR_INSUFFICIENT_RAM; }
...
ModelStackWithThreeMainThings* Song::setupModelStackWithSongAsTimelineCounter(void* memory) {
  return setupModelStackWithThreeMainThingsButNoNoteRow(memory,
                                                        this,
                                                        &globalEffectable,
                                                        this,
                                                        &paramManager);
...
ModelStackWithThreeMainThings* Song::addToModelStack(ModelStack* modelStack) {
  return modelStack->addTimelineCounter(this)->addOtherTwoThingsButNoNoteRow(&globalEffectable,
                                                                             &paramManager);
}
...
traverseClips:
    ...
    if (clipArray != &arrangementOnlyClips) { clipArray = &arrangementOnlyClips;
goto traverseClips; }
...
output->clipInstances.getNumElements(); goto traverseClips; }
    ...
    // For each Clip in session and arranger for specific Output - but if
currentlySwappingInstrument, use master list for arranger Clips ClipArray*
clipArray = &sessionClips; bool doingClipsProvidedByOutput = false;
decideNumElements:
    int numElements = clipArray->getNumElements();
traverseClips:
    for (int c = 0; c < numElements; c++) {
    ...
    if (!doingClipsProvidedByOutput && clipArray == &sessionClips) {
        if (currentlySwappingInstrument) { clipArray = &arrangementOnlyClips;
goto decideNumElements; } else { doingClipsProvidedByOutput = true; numElements
= output->clipInstances.getNumElements(); goto traverseClips; }
    }
 */
