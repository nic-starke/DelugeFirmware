...
void Browser::emptyFileItems() {
  ...
  for (int i = 0; i < fileItems.getNumElements();) {
    ...
    i++;
    if (!(i & 63)) { //  &127 was even fine, even with only -Og compiler
                     //  optimization.
...
#define FILE_ITEMS_MAX_NUM_ELEMENTS_FOR_NAVIGATION \
  20 // It "should" be able to be way less than this.
...
void Browser::cullSomeFileItems() {
  ...
  int numFileItemsDeletingNow =
      fileItems.getNumElements() - (maxNumFileItemsNow >> 1); // May get modified below.
  ...
  deleteFromLeftSide:
    ...
    firstFileItemRemaining =
        ((FileItem*)fileItems.getElementAddress(numFileItemsDeletingNow))->displayName;
  ...
  // Or if we've been using a search term *and* searching both directions, try
  // to tend towards keeping equal amounts of FileIems either side.
  else {
    ...
    // Or, vice versa.
      ...
      int newNumFilesDeleting =
          (fileItems.getNumElements() - foundIndex) >>
...
int Browser::readFileItemsForFolder(char const*  filePrefixHere,
                                    bool         allowFolders,
                                    char const** allowedFileExtensionsHere,
                                    char const*  filenameToStartAt,
                                    int          newMaxNumFileItems,
  ...
  if (result) { return fresultToDelugeErrorCode(result); }
...
void Browser::deleteFolderAndDuplicateItems(int instrumentAvailabilityRequirement) {
  ...
  for (int readI = 0; readI < fileItems.getNumElements(); readI++) {
    ...
    if (readI < fileItems.getNumElements() - 1) {
      ...
      // If we're a folder, and the next item is a file of the same name, delete
      // this item.
      if (readItem->isFolder) {
        if (!nextItem->isFolder) {
          ...
          if (!memcasecmp(readItem->filename.get(), nextItemFilename, nameLength)) {
            if (nextItemFilename[nameLength] == '.' &&
                !strchr(&nextItemFilename[nameLength + 1], '.')) {
              goto deleteThisItem;
            }
          }
        }
      ...
      // Or if we have an Instrument, and the next item is a file of the same
      // name, delete the next item.
        ...
        if (!nextItem->instrument && !nextItem->isFolder) {
          if (!strcasecmp(readItem->displayName, nextItem->displayName)) {
            // if
            // (readItem->filename.equalsCaseIrrespective(&nextItem->filename))
            // {
            nextItem->~FileItem();
            readI++;
            nextItem = (FileItem*)fileItems.getElementAddress(readI + 1);
            // That may set it to an invalid address, but in that case, it won't
            // get read.
          }
        }
      ...
      checkAvailabilityRequirement:
        ...
        if (readItem->instrumentAlreadyInSong) {
          ...
          deleteThisItem:
            readItem->~FileItem();
            continue;
          } else if (instrumentAvailabilityRequirement ==
      ...
      // Or if next item has an Instrument, and we're just a file...
        ...
        if (!strcasecmp(readItem->displayName,
                        nextItem->displayName)) { // And if same name...
          goto deleteThisItem;
        }
      }
    } else {
    ...
    if (writeAddress != readItem) { memcpy(writeAddress, readItem, sizeof(FileItem)); }
    writeI++;
  }

  int numToDelete = fileItems.getNumElements() - writeI;
  if (numToDelete > 0) { fileItems.deleteAtIndex(writeI, numToDelete); }

  // Our system of keeping FileItems from getting too full by deleting elements
  // from its ends as we go could have caused bad results at the edges of the
  // above, so delete a further one element at each end as needed.
  if (firstFileItemRemaining) { fileItems.deleteAtIndex(0); }
  if (lastFileItemRemaining) { fileItems.deleteAtIndex(fileItems.getNumElements() - 1); }
}

// song may be supplied as NULL, in which case it won't be searched for
// Instruments; sometimes this will get called when the currentSong is not set
// up.
int Browser::readFileItemsFromFolderAndMemory(Song*       song,
                                              int         instrumentType,
...
tryReadingItems:
  int error = readFileItemsForFolder(filePrefixHere,
                                     allowFolders,
                                     allowedFileExtensions,
                                     filenameToStartAt,
                                     FILE_ITEMS_MAX_NUM_ELEMENTS,
  ...
  if (error) {
    ...
    if (error == ERROR_FOLDER_DOESNT_EXIST) {
      if (defaultDirToAlsoTry) {
        ...
        // Or if we have already tried it and it didn't exist, try creating
        // it...
        else {
          // But not if we already tried.
          if (triedCreatingFolder) { return error; }
    ...
    if (fileItems.getNumElements()) {
      ...
      if (!allowFoldersSharingNameWithFile) {
        deleteFolderAndDuplicateItems(
            AVAILABILITY_ANY); // I think this is right - was
                               // AVAILABILITY_INSTRUMENT_UNUSED until 2023-01
      }
    }
  }
  return NO_ERROR;
...
// If HAVE_OLED, then you should make sure renderUIsForOLED() gets called after
// this. instrumentTypeToLoad must be set before calling this.
int Browser::arrivedInNewFolder(int         direction,
                                char const* filenameToStartAt,
                                char const* defaultDirToAlsoTry) {
  arrivedAtFileByTyping = false;
...
tryReadingItems:
  ...
  int  newCatalogSearchDirection =
      doWeHaveASearchString ? CATALOG_SEARCH_BOTH : CATALOG_SEARCH_RIGHT;
  int error = readFileItemsFromFolderAndMemory(currentSong,
  ...
  if (fileItems.getNumElements()) {
    ...
    noExactFileFound:
      ...
      // Normally, we'll need to just use one of the ones we found. (That's just
      // always the first one, I think...)
      if (!allowBrandNewNames || direction) {

        // But since we're going to just use the first file, if we've deleted
        // items at the start (meaning we had a search string), we need to go
        // back and get them.
      ...
      // But sometimes...
        ...
        // Ok so we didn't find an exact file, we've just entered the browser,
        // and we're allowed new names. So, choose a brand new name (if there
        // wasn't already a new one nominated).
    ...
    // We found an exact file. But if we've just entered the Browser and are
    // allowed, then we need to find a new subslot variation. Come up with a new
    // name variation.
    error = setEnteredTextFromCurrentFilename();
    if (error) goto gotErrorAfterAllocating;
#if 1 || !HAVE_OLED
      ...
      while (true) {
        ...
        *subSlotPos = 'A' + thisSlot.subSlot;
        ...
        if (!foundExactHere) {
          ...
          goto setEnteredTextAndUseFoundFile; // If we're stuck on the "Z"
                                              // subslot.
        }
      }
    ...
    if (length > 0) {
      ...
      } else if (length >= 2 && enteredTextChars[length - 2] >= '0' &&
                 enteredTextChars[length - 2] <= '9' &&
                 ((enteredTextChars[length - 1] >= 'a' &&
                   enteredTextChars[length - 1] < 'z') // That's *less than*, because if it's Z,
                                                       // we'll have to doNormal.
    ...
    doNormal: // FileItem* currentFile =
              // (FileItem*)fileItems.getElementAddress(fileIndexSelected);
      String endSearchString;
      // error = currentFile->getFilenameWithoutExtension(&endSearchString); if
      // (error) goto gotErrorAfterAllocating;
    ...
    tryAgain:
      ...
      if (delimeterAddress) {
        ...
        if (error)
          goto gotErrorAfterAllocating; // Colon is the next character after the
                                        // ascii digits, so searching for this
                                        // will get us past the final number
                                        // present.
      } else {
      noNumberYet:
        if (delimeterChar == '_') {
  ...
  // Or if no files found at all...
    ...
    pickBrandNewNameIfNoneNominated:
      if (enteredText.isEmpty()) {
        ...
        // Because that will have cleared out all the FileItems, we need to get
        // them again. Actually there would kinda be a way around doing this...
        error = readFileItemsFromFolderAndMemory(currentSong,
                                                 255,
                                                 "SONG",
                                                 enteredText.get(),
                                                 NULL,
...
Browser::findAnUnlaunchedPresetIncludingWithinSubfolders(Song* song,
                                                         int   instrumentType,
                                                         int   availabilityRequirement) {
...
goAgain:
  ...
  toReturn.error = readFileItemsFromFolderAndMemory(song,
                                                    instrumentType,
                                                    getThingName(instrumentType),
                                                    searchNameLocalCopy.get(),
                                                    NULL,
                                                    true);
  ...
  // If that folder-read gave us no files, that's gotta mean we got to the end
  // of the folder.
  if (!fileItems.getNumElements()) {

    // If we weren't yet looking at subfolders, do that now, going back to the
    // start of this folder's contents.
    if (!doingSubfolders) {
    startDoingFolders:
  ...
  FileItem* rightmostFileItemBeforeFiltering =
      (FileItem*)fileItems.getElementAddress(fileItems.getNumElements() - 1);
  toReturn.error =
      lastFileItemDisplayNameBeforeFiltering.set(rightmostFileItemBeforeFiltering->displayName);
  ...
  if (!doingSubfolders) {
    ...
    // Ok, we found none. Should we do some more reading of the folder contents,
    // to get more files, or are there no more?
    if (numFileItemsDeletedAtEnd) {
      searchNameLocalCopy.set(&lastFileItemDisplayNameBeforeFiltering); // Can't fail.
      goto goAgain;
    }

    // Ok, we've looked at every file, and none were presets we could use. So
    // now we want to look in subfolders. Do we still have the "start" of our
    // folder's contents in memory?
  ...
  for (i = 0; i < fileItems.getNumElements(); i++) {
    toReturn.fileItem = (FileItem*)fileItems.getElementAddress(i);
    if (toReturn.fileItem->isFolder) { goto doThisFolder; }
  }
  if (numFileItemsDeletedAtEnd) {
  ...
  doThisFolder:
    ...
    // Call self
    toReturn = findAnUnlaunchedPresetIncludingWithinSubfolders(song,
                                                               instrumentType,
...
ReturnOfConfirmPresetOrNextUnlaunchedOne
Browser::confirmPresetOrNextUnlaunchedOne(int     instrumentType,
                                          String* searchName,
                                          int     availabilityRequirement) {
  ReturnOfConfirmPresetOrNextUnlaunchedOne toReturn;
...
doReadFiles:
  toReturn.error = readFileItemsFromFolderAndMemory(currentSong,
                                                    instrumentType,
                                                    getThingName(instrumentType),
                                                    searchNameLocalCopy.get(),
                                                    NULL,
  ...
  justGetAnyPreset: // This does *not* favour the currentDir, so you should
                    // exhaust all avenues before calling this.
    toReturn.error = currentDir.set(getInstrumentFolder(instrumentType));
    if (toReturn.error) goto doReturn;
    toReturn = findAnUnlaunchedPresetIncludingWithinSubfolders(currentSong,
                                                               instrumentType,
                                                               availabilityRequirement);
    goto doReturn;
  } else if (toReturn.error) {
  ...
  sortFileItems();
  if (!fileItems.getNumElements()) {
    if (shouldJustGrabLeftmost) { goto justGetAnyPreset; }
  ...
  // If we've shot off the end of the list, that means our searched-for preset
  // didn't exist or wasn't available, and any subsequent ones which at first
  // made it onto the (possibly truncated) list also weren't available.
  if (!fileItems.getNumElements()) {
    if (numFileItemsDeletedAtEnd) { // Probably couldn'g happen anymore...
      ...
      // If we've already been trying to grab just any preset within this
      // folder, well that's failed.
      ...
      // Or, if we've actually managed to fit the whole folder contents into our
      // fileItems...
      else {
        // Well, if there's still nothing in that, then we really need to give
        // up.
        if (!fileItems.getNumElements()) { goto justGetAnyPreset; }
...
goBackOne: // Ok, due to not bothering to reload fileItems if we need to look
           // too far back, we may sometimes fail to see an empty slot further
           // back when later ones are taken. Oh well.
  i--;
  int freeSlotNumber;
  if (i < 0) {
  noMoreToLookAt:
...
// Caller must call emptyFileItems() at some point after calling this function -
// unless an error is returned. Caller must remove OLED working animation after
// calling this too.
PresetNavigationResult Browser::doPresetNavigation(int         offset,
...
readAgainWithSameOffset:
  toReturn.error = readFileItemsForFolder(getThingName(instrumentType),
                                          false,
                                          allowedFileExtensionsXML,
                                          oldNameString.get(),
                                          FILE_ITEMS_MAX_NUM_ELEMENTS_FOR_NAVIGATION,
                                          newCatalogSearchDirection);
  ...
  /*
      if (i >= fileItems.getNumElements()) { // If not found *and* we'd be past
  the end of the list... if (offset >= 0) i = 0; else i =
  fileItems.getNumElements() - 1; goto doneMoving;
      }
      else {
          int oldNameLength = strlen(oldNameChars);
          FileItem* searchResultItem =
  (FileItem*)fileItems.getElementAddress(i); if (memcasecmp(oldNameChars,
  searchResultItem->displayName, oldNameLength)) { notFound:   if (offset < 0) {
                  i--;
          ...
          if (searchResultItem->filenameIncludesExtension) {
              if (strrchr(searchResultItem->displayName, '.') !=
  &searchResultItem->displayName[oldNameLength]) goto notFound;
          }
          else {
              if (searchResultItem->displayName[oldNameLength] != 0) goto
  notFound;
          }
      }
  */
...
doneMoving:
  ...
  // Unlike in ClipMinder, there's no need to check whether we came back to the
  // same Instrument, cos we've specified that we were looking for "unused" ones
  // only

  if (!toReturn.fileItem->instrument) {
    toReturn.error = storageManager.loadInstrumentFromFile(currentSong,
                                                           NULL,
                                                           instrumentType,
                                                           false,
                                                           &toReturn.fileItem->instrument,
                                                           &toReturn.fileItem->filePointer,
                                                           &newName,
                                                           &Browser::currentDir);
    if (toReturn.error) goto emptyFileItemsAndReturn;
...
void Browser::selectEncoderAction(int8_t offset) {
  ...
  if (fileIndexSelected < 0) { // If no file selected and we were typing a new name?
    ...
    // If user is holding shift, skip past any subslots. And on numeric Deluge,
    // user may have chosen one digit to "edit".
#if !HAVE_OLED
...
bool Browser::predictExtendedText() {
  ...
  DWORD     oldClust    = 0;
  if (oldFileItem) { oldClust = oldFileItem->filePointer.sclust; }
...
doSearch:
  ...
  if (i >= fileItems.getNumElements()) {
    ...
    // If we haven't yet done a whole new read from the SD card etc, from within
    // this function, do that now.
    if (!doneNewRead) {
    doNewRead:
      doneNewRead = true;
      error       = readFileItemsFromFolderAndMemory(
          ...
          true,
          0,
          CATALOG_SEARCH_BOTH); // This could probably actually be made to work
                                      // with searching left only...
      if (error) {
      gotErrorAfterAllocating:
        emptyFileItems();
        goto gotError;
  ...
  // If it didn't match exactly, that's ok, but we need to try some other stuff
  // before we accept that result.
  ...
  if (fileItem->filePointer.sclust != oldClust) { currentFileChanged(0); }
...
void Browser::renderOLED(uint8_t image[][OLED_MAIN_WIDTH_PIXELS]) {
  ...
  if (fileIndexSelected == -1) {
    ...
    searchForChar:
      ...
      if (!finalCharAddress) { // Shouldn't happen... or maybe for in-memory
                               // presets?
        finalChar = 0;
        goto searchForChar;
      }
      ...
      if (isSelectedIndex) {
        drawTextForOLEDEditing(textStartX,
        ...
        OLED::drawStringFixedLength(displayName,
                                    displayStringLength,
                                    textStartX,
                                    yPixel,
...
Slot Browser::getSlot(char const* displayName) {
  ...
  if (*charPos == '0') { // If first digit is 0, then no more digits allowed.
    ...
    while (*charPos >= '0' && *charPos <= '9' && charPos < (displayName + 3)) { charPos++; }
  }
  ...
  switch (subSlotChar) {
    ...
    case 'A' ... 'Z': {
      ...
      // case '.':
      // if (strchr(charPos + 1, '.')) goto nonNumeric; // Ensure no more dots
      // after this dot.
      //  No break.
  ...
  doQWERTYDisplay:
    ...
    if (enteredText.isEmpty() && fileIndexSelected == -1) {
      ...
      if (filePrefix) {
        ...
        numericDriver.setTextAsSlot(thisSlot.slot,
                                    thisSlot.subSlot,
                                    (fileIndexSelected != -1),
                                    true,
                                    numberEditPos,
...
int Browser::buttonAction(int x, int y, bool on, bool inCardRoutine) {
    ...
    if (!currentUIMode && on) {
      ...
      if (currentFileItem) {
        if (currentFileItem->isFolder) {
          numericDriver.displayPopup(HAVE_OLED ? "Folders cannot be deleted on the Deluge" :
                                                 "CANT");
  ...
  // Back button
    ...
    if (on && !currentUIMode) { return backButtonAction(); }
  } else
...
// Virtual function - may be overridden, by child classes that need to do more
// stuff, e.g. SampleBrowser needs to mute any previewing Sample.
...
void Browser::sortFileItems() {
  ...
  // If we're just wanting to look to one side or the other of a given filename,
  // then delete everything in the other direction.
  ...
  // If we'd previously deleted items from either end of the list (apart from
  // due to search direction as above), we need to now delete any items which
  // would have fallen in that region.
