...
Drum* SoundDrum::clone() {
    void* drumMemory = generalMemoryAllocator.alloc(sizeof(SoundDrum), NULL,
false, true); if (!drumMemory) return NULL; SoundDrum* newDrum = new
(drumMemory) SoundDrum();
...
void SoundDrum::noteOn(ModelStackWithThreeMainThings* modelStack,
  ...
  if (polyphonic == POLYPHONY_CHOKE) { kit->choke(); }

  Sound::noteOn(modelStack,
                &arpeggiator,
                NOTE_FOR_DRUM,
                mpeValues,
                sampleSyncLength,
...
void SoundDrum::polyphonicExpressionEventOnChannelOrNote(int newValue,
                                                         int whichExpressionDimension,
                                                         int channelOrNoteNumber,
                                                         int whichCharacteristic) {
  // Because this is a Drum, we disregard the noteCode (which is what
  // channelOrNoteNumber always is in our case - but yeah, that's all
  // irrelevant.
