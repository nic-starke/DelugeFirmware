...
uint8_t DelayBuffer::init(uint32_t newRate, uint32_t failIfThisSize, bool includeExtraSpace) {
  ...
  bufferStart =
      (StereoSample*)generalMemoryAllocator.alloc(sizeIncludingExtra * sizeof(StereoSample),
                                                  NULL,
                                                  false,
                                                  true);
...
void DelayBuffer::makeNativeRatePreciseRelativeToOtherBuffer(DelayBuffer* otherBuffer) {
  ...
  nativeRate = round((double)DELAY_BUFFER_NEUTRAL_SIZE * (double)16777216 *
                     otherBufferAmountTooFast / (double)size);
}
...
void DelayBuffer::setupForRender(int32_t userDelayRate, DelayBufferSetup* setup) {
  if (userDelayRate != nativeRate) {
    if (!isResampling && bufferStart != 0) {
      ...
      // Because we're switching from direct writing to writing in "triangles",
      // we need to adjust the data we last wrote directly so that it meshes
      // with the upcoming triangles. Assuming that the delay rate has only
      // changed slightly at this stage, this is as simple as removing a quarter
      // of the last written value, and putting that removed quarter where the
      // "next" write-pos is. That's because the triangles are 4 samples wide
      // total (2 samples either side)
  ...
  if (isResampling) {
    ...
    setup->actualSpinRate =
        ((uint64_t)userDelayRate << 24) / nativeRate;                // 1 is represented as 16777216
    ...
    if (setup->actualSpinRate < 16777216) {
      ...
      // This seems to be the best option. speedMultiple is set to the smallest
      // multiple of delay.speed which is greater than 65536. Means the
      // "triangles" link up, and are at least as wide as a frame of the write
      // buffer. Does that make sense?
      setup->rateMultiple = (setup->actualSpinRate >> 8) * (timesSlowerRead + 1);

      // This was tricky to work out. Needs to go up with delay.speed because
      // this means less "density". And squarely down with writeRateMultiple
      // because more of that means more "triangle area", or more stuff written
      // each time.
      // uint32_t delayWriteSizeAdjustment2 = (((uint32_t)delay.speed << 16) /
      // (((uint32_t)(speedMultiple >> 2) * (uint32_t)(speedMultiple >> 2)) >>
      // 11));
      setup->writeSizeAdjustment =
          (uint32_t)0xFFFFFFFF /
          (setup->rateMultiple * (timesSlowerRead + 1)); // Equivalent to one order of magnitude
                                                         // bigger than the above line
    }

    // If buffer spinning fast
    else {

      // First, let's limit sped up writing to only work perfectly up to 8x
      // speed, for safety (writing faster takes longer). No need to adjust
      // divideByRate to compensate - it's going to sound shoddy anyway
      setup->spinRateForSpedUpWriting = getMin(setup->actualSpinRate, (int32_t)16777216 * 8);

      // We want to squirt the most juice right at the "main" write pos - but we
      // want to spread it wider too. A basic version of this would involve the
      // triangle's base being as wide as 2 samples if we were writing at the
      // native sample rate. However I've stretched the triangle twice as wide
      // so that at the native sample rate it's the same width as the
      // slowed-down algorithm below, so there's no click when switching between
      // the two. This does mean we lose half the bandwidth. That's done with
      // the following 2 lines of code, and the fact that the actual writes
      // below are <<3 instead of <<4.
      setup->spinRateForSpedUpWriting = setup->spinRateForSpedUpWriting <<=
          1;                     // Woah, did I mean to write "<<=" ?
      setup->divideByRate >>= 1; // We may change this because sped up writing
                                 // is the only thing it'll be used for
    }
  }
}
