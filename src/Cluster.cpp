...
void Cluster::convertDataIfNecessary() {
  ...
  if (sample->rawDataFormat) {
    ...
    if (clusterIndex < startCluster) { // Hmm, there must have been a case where
                                       // this happens...
      return;
    }
    ...
    if (sample->rawDataFormat == RAW_DATA_ENDIANNESS_WRONG_24) {
      ...
      while (true) {
        char const* endPosNow =
            pos + 1024; // Every this many bytes, we'll pause and do an audio routine
...
void Cluster::steal(char const* errorCode) {
  ...
  // Ok, we're now gonna decide what to do according to the actual "type" field
  // for this Cluster.
  switch (type) {
    ...
    case CLUSTER_SAMPLE_CACHE:
      ...
      // If first Cluster, delete whole cache. Wait, no, something might still be
