...
/*
 * Sound can be either an Instrument or a Drum, in the form of SoundInstrument
 * or SoundDrum respectively. These classes are implemented using “multiple
 * inheritance”, which is sacrilegious to many C++ programmers. I (Rohan)
 * consider it to be a more or less appropriate solution in this case and a few
 * others in the Deluge codebase where it’s used. It’s a little while though
 * since I’ve sat and thought about what the alternatives could be and whether
 * anything else would be appropriate.
 *
 * Anyway, Sound (which may be named a bit too broadly) basically means a synth
 * or sample, or any combination of the two. And, to reiterate the above, it can
 * exist as a “synth” as the melodic Output of one entire Clip(s), or as just a
 * Drum - one of the many items in a Kit, normally associated with a row of
 * notes.
 */
...
public:
  ...
  int32_t paramFinalValues[NUM_PARAMS -
                           FIRST_GLOBAL_PARAM]; // This is for the *global* params only, and begins
                                                // with FIRST_GLOBAL_PARAM, so subtract that from
                                                // your p value before accessing this array!
  int32_t globalSourceValues[FIRST_LOCAL_SOURCE];

  uint32_t sourcesChanged; // Applies from first source up to
  ...
  int32_t monophonicExpressionValues
      [NUM_EXPRESSION_DIMENSIONS]; // I really didn't want to store these here,
                                   // since they're stored in the ParamManager,
                                   // but.... complications! Always 0 for Drums
                                   // - that was part of the problem - a Drum's
                                   // main ParamManager's expression data has
                                   // been sent to the "polyphonic" bit, and we
                                   // don't want it to get referred to twice.
                                   // These get manually refreshed in
                                   // setActiveClip().
  ...
  uint32_t startSkippingRenderingAtTime; // Valid when not 0. Allows a wait-time
                                         // before render skipping starts, for
                                         // if mod fx are on
  ...
  void patchedParamPresetValueChanged(uint8_t                   p,
                                      ModelStackWithSoundFlags* modelStack,
                                      int32_t                   oldValue,
                                      int32_t                   newValue);
  void render(ModelStackWithThreeMainThings* modelStack,
  ...
  void
  ensureInaccessibleParamPresetValuesWithoutKnobsAreZero(ModelStackWithThreeMainThings* modelStack);
  void ensureInaccessibleParamPresetValuesWithoutKnobsAreZeroWithMinimalDetails(
  ...
  int8_t  getKnobPos(uint8_t                  p,
                     ParamManagerForTimeline* paramManager,
                     uint32_t                 timePos,
                     TimelineCounter*         counter);
  int32_t getKnobPosBig(int                      p,
                        ParamManagerForTimeline* paramManager,
                        uint32_t                 timePos,
                        TimelineCounter*         counter);
  bool    learnKnob(MIDIDevice*     fromDevice,
  ...
  int  readFromFile(ModelStackWithModControllable* modelStack,
  ...
  void noteOn(ModelStackWithThreeMainThings* modelStack,
  ...
  void noteOnPostArpeggiator(ModelStackWithSoundFlags* modelStack,
  ...
  void
  modButtonAction(uint8_t whichModButton, bool on, ParamManagerForTimeline* paramManager) final;
  bool        modEncoderButtonAction(uint8_t                        whichModEncoder,
                                     bool                           on,
                                     ModelStackWithThreeMainThings* modelStack) final;
  static void writeParamsToFile(ParamManager* paramManager, bool writeAutomation);
  static void readParamsFromFile(ParamManagerForTimeline* paramManager,
                                 int32_t                  readAutomationUpToPos);
  static bool readParamTagFromFile(char const*              tagName,
                                   ParamManagerForTimeline* paramManager,
                                   int32_t                  readAutomationUpToPos);
  static void initParams(ParamManager* paramManager);
  static int  createParamManagerForLoading(ParamManagerForTimeline* paramManager);
  int32_t     hasAnyTimeStretchSyncing(ParamManagerForTimeline* paramManager,
                                       bool                     getSampleLength = false,
                                       int                      note            = 0);
  int32_t
  hasCutOrLoopModeSamples(ParamManagerForTimeline* paramManager, int note, bool* anyLooping = NULL);
  ...
  ModelStackWithAutoParam* getParamFromModEncoder(int                            whichModEncoder,
                                                  ModelStackWithThreeMainThings* modelStack,
                                                  bool allowCreation = true) final;
  void                     reassessRenderSkippingStatus(ModelStackWithSoundFlags* modelStack,
                                                        bool                      shouldJustCutModFX = false);
  void                     getThingWithMostReverb(Sound**                   soundWithMostReverb,
  ...
  inline int32_t getSmoothedPatchedParamValue(
      int           p,
  ...
  void                     notifyValueChangeViaLPF(int                                  p,
  ...
  ModelStackWithAutoParam* getParamFromMIDIKnob(MIDIKnob*                      knob,
                                                ModelStackWithThreeMainThings* modelStack) final;
...
private:
  ...
  int      readTagFromFile(char const*              tagName,
                           ParamManagerForTimeline* paramManager,
                           int32_t                  readAutomationUpToPos,
                           ArpeggiatorSettings*     arpSettings,
                           Song*                    song);
