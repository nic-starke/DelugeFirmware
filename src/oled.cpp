...
const uint8_t synthIcon[] =
    {0b11111110, 0b11100000, 0b00000000, 0b11111110, 0b00000000, 0b11100000, 0b11111110, 0};
...
void clearMainImage() {
  ...
  memset(oledMainImage,
         0,
         sizeof(oledMainImage)); // Takes about 1 fast-timer tick (whereas entire
                                 // rendering takes around 8 to 15). So, not worth
                                 // trying to use DMA here or anything.
}

// Clears area *inclusive* of maxX, but not maxY? Stupid.
void clearAreaExact(int     minX,
                    int     minY,
                    int     maxX,
                    int     maxY,
  ...
  if (firstRowPixelWithin) {
    ...
    if (willDoLastRow && firstRow == lastRow) { firstRowMask &= lastRowMask; }
    for (int x = minX; x <= maxX; x++) { image[firstRow][x] &= firstRowMask; }
  ...
  if (willDoLastRow) {
    lastCompleteRow--;
    for (int x = minX; x <= maxX; x++) { image[lastRow][x] &= lastRowMask; }
  }
...
void drawGraphicMultiLine(uint8_t const* graphic,
                          int            startX,
                          int            startY,
                          int            width,
                          uint8_t*       image,
  ...
  while (true) {
    ...
    while (currentPos < endPos) {
      uint32_t data = *(uint16_t*)graphicPos; // Cleverly read 2 bytes in one go. Doesn't really
                                              // speed things up. I should try addressing display
                                              // vertically, so I can do 32 bits in one go on both
                                              // the graphic and the display...
      *currentPos |= data >> yOffsetNegative;

      //*currentPos |= (*graphicPos >> yOffsetNegative) | (*(graphicPos + 1) <<
      // yOffset);
      currentPos++;
      graphicPos += numBytesTall;
    }
...
void drawString(char const* string,
                int         pixelX,
                int         pixelY,
                uint8_t*    image,
                int         imageWidth,
...
void drawStringFixedLength(char const* string,
                           int         length,
                           int         pixelX,
                           int         pixelY,
                           uint8_t*    image,
...
void drawStringCentred(char const* string,
                       int         pixelY,
                       uint8_t*    image,
                       int         imageWidth,
                       int         textWidth,
...
void drawStringAlignRight(char const* string,
                          int         pixelY,
                          uint8_t*    image,
                          int         imageWidth,
                          int         textWidth,
...
void drawStringCentredShrinkIfNecessary(char const* string,
                                        int         pixelY,
                                        uint8_t*    image,
                                        int         imageWidth,
                                        int         textWidth,
...
void drawChar(uint8_t  theChar,
  ...
  renderFont:
    ...
    int scaledFontWidth = (uint16_t)(descriptor->w_px * textHeight + (fontNativeHeight >> 1) - 1) /
                          (uint8_t)fontNativeHeight;
#else
    ...
    drawGraphicMultiLine(&font[descriptor->glyph_index + scrollPos * bytesPerCol],
                         pixelX,
                         pixelY,
                         textWidth,
                         image,
...
void drawScreenTitle(char const* title) {
  ...
  OLED::drawString(title,
                   ...
                   TEXT_TITLE_SIZE_Y);
  OLED::drawHorizontalLine(extraY + 11 + OLED_MAIN_TOPMOST_PIXEL,
                           0,
...
void drawConsoleTopLine() {
  drawHorizontalLine(consoleItems[numConsoleItems - 1].minY - 1,
                     consoleMinX + 1,
                     consoleMaxX - 1,
...
int setupConsole(int height) {
  ...
  if (numConsoleItems) {
    ...
    for (int i = numConsoleItems; i > 0; i--) { consoleItems[i] = consoleItems[i - 1]; }
    ...
    if (howMuchTooLow > 0) {
      ...
      for (int i = numConsoleItems; i >= 0; i--) { // numConsoleItems hasn't been updated yet -
      ...
      moveAreaUpCrude(consoleMinX,
                      consoleItems[numConsoleItems].minY - 1,
                      consoleMaxX,
                      consoleItems[1].maxY + howMuchTooLow,
                      howMuchTooLow,
                      oledMainConsoleImage); // numConsoleItems hasn't been updated yet -
                                             // there's actually one more
    }
  }
  ...
  clearAreaExact(consoleMinX,
                 consoleItems[0].minY,
                 consoleMaxX,
                 consoleItems[0].maxY,
                 oledMainConsoleImage);

  drawVerticalLine(consoleMinX,
                   consoleItems[0].minY - 1,
                   consoleItems[0].maxY,
                   oledMainConsoleImage);
  drawVerticalLine(consoleMaxX,
                   consoleItems[0].minY - 1,
                   consoleItems[0].maxY,
                   oledMainConsoleImage);
...
void copyBackgroundAroundForeground(uint8_t backgroundImage[][OLED_MAIN_WIDTH_PIXELS],
                                    uint8_t foregroundImage[][OLED_MAIN_WIDTH_PIXELS],
                                    int     minX,
                                    int     minY,
                                    int     maxX,
                                    int     maxY) {
  ...
  int firstRow = minY >> 3;
  int lastRow  = maxY >> 3;
  if (firstRow) { memcpy(foregroundImage, backgroundImage, OLED_MAIN_WIDTH_PIXELS * firstRow); }
  ...
  for (int row = firstRow; row <= lastRow; row++) {
    ...
    memcpy(&foregroundImage[row][maxX + 1],
           &backgroundImage[row][maxX + 1],
           OLED_MAIN_WIDTH_PIXELS - maxX - 1);
  }
  ...
  if (numMoreRows > 0) {
    memcpy(foregroundImage[lastRow + 1],
           backgroundImage[lastRow + 1],
           OLED_MAIN_WIDTH_PIXELS * numMoreRows);
  }
}
...
void sendMainImage() {
  ...
  if (numConsoleItems) {
    copyBackgroundAroundForeground(oledMainImage,
                                   oledMainConsoleImage,
                                   consoleMinX,
                                   consoleItems[numConsoleItems - 1].minY - 1,
                                   consoleMaxX,
                                   OLED_MAIN_HEIGHT_PIXELS - 1);
    backgroundImage = oledMainConsoleImage;
  }
  if (oledPopupWidth) {
    copyBackgroundAroundForeground(backgroundImage,
                                   oledMainPopupImage,
                                   popupMinX,
                                   popupMinY,
                                   popupMaxX,
...
void breakStringIntoLines(char const* text, TextLineBreakdown* textLineBreakdown) {
  ...
  char const* lineEnd =
      lineStart + textLineBreakdown->maxCharsPerLine; // Default to it being the max length - we'll
                                                      // only use this if no "spaces" found.
...
void drawPermanentPopupLookingText(char const* text) {
  ...
  for (int l = 0; l < textLineBreakdown.numLines; l++) {
    int textPixelX =
        (OLED_MAIN_WIDTH_PIXELS - (TEXT_SPACING_X * textLineBreakdown.lineLengths[l])) >> 1;
    drawStringFixedLength(textLineBreakdown.lines[l],
                          textLineBreakdown.lineLengths[l],
                          textPixelX,
                          textPixelY,
                          oledMainImage[0],
                          OLED_MAIN_WIDTH_PIXELS,
...
void updateWorkingAnimation() {
  ...
  for (int i = 0; i < 3; i++) { buffer[i] = (i <= workingAnimationCount) ? '.' : ' '; }
...
void consoleTextIfAllBootedUp(char const* text) {
  if (usbInitializationPeriodComplete) { consoleText(text); }
}
}
...
void setupSideScroller(int         index,
                       char const* text,
                       int         startX,
                       int         endX,
                       int         startY,
...
void scrollingAndBlinkingTimerEvent() {
  ...
  for (int s = 0; s < NUM_SIDE_SCROLLERS; s++) {
    ...
    if (scroller->text) {
      ...
      // Ok, have to render.
      clearAreaExact(scroller->startX,
                     scroller->startY,
                     scroller->endX - 1,
                     scroller->endY,
                     oledMainImage);
      drawString(scroller->text,
                 scroller->startX,
                 scroller->startY,
                 oledMainImage[0],
                 OLED_MAIN_WIDTH_PIXELS,
                 scroller->textSpacingX,
                 scroller->textSizeY,
                 scroller->pos,
                 scroller->endX);
      if (scroller->doHilight) {
        invertArea(scroller->startX,
                   scroller->endX - scroller->startX,
                   scroller->startY,
                   scroller->endY,
                   &OLED::oledMainImage[0]);
      }
  ...
  if (!finished) {
    ...
    for (int s = 0; s < NUM_SIDE_SCROLLERS; s++) { sideScrollers[s].finished = false; }
  }
...
void consoleTimerEvent() {
  ...
  // Or if console moving down
      ...
      if (consoleItems[numConsoleItems - 1].minY < OLED_MAIN_HEIGHT_PIXELS) {
          uiTimerManager.setTimer(TIMER_DISPLAY,
  CONSOLE_ANIMATION_FRAME_TIME_SAMPLES);
      }
...
checkTimeTilTimeout:
  ...
  // Or if it hasn't timed out, then provided we didn't already want a real-soon
  // callback, come back when it does time out
  else if (!timeTilNext) {
    timeTilNext = timeLeft;
  }

  if (timeTilNext) { uiTimerManager.setTimerSamples(TIMER_OLED_CONSOLE, timeTilNext); }
...
void freezeWithError(char const* text) {
  ...
  OLED::drawString("Press select knob to",
                   0,
                   yPixel,
                   OLED::oledMainImage[0],
                   OLED_MAIN_WIDTH_PIXELS,
  ...
  yPixel += TEXT_SPACING_Y;
  OLED::drawString("attempt resume. Then",
                   0,
                   yPixel,
                   OLED::oledMainImage[0],
                   OLED_MAIN_WIDTH_PIXELS,
  ...
  yPixel += TEXT_SPACING_Y;
  OLED::drawString("save to new file.",
                   0,
                   yPixel,
                   OLED::oledMainImage[0],
                   OLED_MAIN_WIDTH_PIXELS,
  ...
  // DMACn(OLED_SPI_DMA_CHANNEL).CHCFG_n = 0b00000000001000000000001001101000 |
  // (OLED_SPI_DMA_CHANNEL & 7);
  ...
  // spiTransferQueueReadPos = (spiTransferQueueReadPos + 1) &
  // (SPI_TRANSFER_QUEUE_SIZE - 1);
