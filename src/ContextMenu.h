...
public:
  ...
  virtual int getAcceptButtonX() {
    return selectEncButtonX;
  }
  virtual int getAcceptButtonY() {
    return selectEncButtonY;
  }

  int currentOption; // Don't make static. We'll have multiple nested
                     // ContextMenus open at the same time

  char const** basicOptions;
  int          basicNumOptions;
#if HAVE_OLED
  void renderOLED(uint8_t image[][OLED_MAIN_WIDTH_PIXELS]);
  int  scrollPos; // Don't make static. We'll have multiple nested ContextMenus
...
public:
  ...
  virtual int getAcceptButtonX() final {
    return saveButtonX;
  }
  virtual int getAcceptButtonY() final {
    return saveButtonY;
  }
};
