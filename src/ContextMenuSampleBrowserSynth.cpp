...
bool ContextMenuSampleBrowserSynth::isCurrentOptionAvailable() {
  ...
  // Multisamples (load entire folder and auto-detect ranges). Will delete all
  // previous Ranges.
  if (currentOption == 0) return (soundEditor.currentSound->getSynthMode() != SYNTH_MODE_RINGMOD);

  // Apart from that option, none of the other ones are valid if currently
  // sitting on a folder-name.
  ...
  switch (currentOption) {
    ...
    case 2: // Single-cycle - available even if we're locked (because of there
            // being other Ranges) to this happening either in Sample or WaveTable
            // mode. Although, the user could get an error after selecting the
            // option - e.g. if we're locked to WaveTable mode, they've selected
            // single-cycle, and it realises it can't do this combination of
            // things because it's a stereo file.
      return true;
...
bool ContextMenuSampleBrowserSynth::acceptCurrentOption() {
  ...
  switch (currentOption) {
    ...
    case 3: // WaveTable
      return sampleBrowser.claimCurrentFile(1, 1,
                                            2); // Could probably also be 0,0,2
    default:
      __builtin_unreachable();
      return false;
  }
