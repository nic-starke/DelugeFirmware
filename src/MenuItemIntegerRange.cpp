...
MenuItemIntegerRange::MenuItemIntegerRange(char const* newName, int newMin, int newMax)
    : MenuItemRange(newName) {
  minValue = newMin;
  maxValue = newMax;
}
...
#if HAVE_OLED
  if (lower != upper) { soundEditor.editingRangeEdge = RANGE_EDIT_LEFT; }
#endif
}
