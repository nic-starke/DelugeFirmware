...
int ConsequenceClipBeginLinearRecord::revert(int time, ModelStack* modelStack) {
  ...
  if (time == BEFORE) {
    ...
    if (modelStack->song->isClipActive(clip)) {
      ...
      // Or if Clip is soloing, deactivating that would be a can of worms,
      // whereas this whole auto-deactivation feature is really just intended as
      // an unessential thing the user will find helpful in most cases
      if (clip->soloingInSessionMode) return NO_ERROR;

      // Or if we're viewing the Clip, don't deactivate it, cos it's a massive
      // hassle, and confusing, for user to go out and reactivate it
  ...
  // Going forwards...
    ...
    if (!modelStack->song->isClipActive(clip)) {
      ...
      // Check that the reason it's showing as "inactive" isn't just that
      // there's another Clip soloing - in which case we don't want to toggle
      // its status
