...
public:
  ...
  int8_t  transpose;
  int8_t  cents;
  int32_t pitchBend; // (1 << 23) represents one semitone. So full 32-bit range
                     // can be +-256 semitones. This is different to the
                     // equivalent calculation in Voice, which needs to get
                     // things into a number of octaves.
};
...
public:
  ...
  // When one or more note-on is pending, this is the latest time that one of
  // them last switched off. But it seems I only use this very coarsely - more
  // to see if we're still in the same audio frame than to measure time exactly.
