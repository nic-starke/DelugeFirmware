...
int AudioFile::loadFile(AudioFileReader* reader, bool isAiff, bool makeWaveTableWorkAtAllCosts) {
  ...
  while (bytePos < reader->fileSize) {
    ...
    thisChunk.length                       = (thisChunk.length + 1) &
                       ~(uint32_t)1; // If chunk size is odd, skip the extra byte of padding at the
                                     // end too. Weird RIFF file requirement.
    ...
    if (!isAiff) {
      ...
      switch (thisChunk.name) {
        ...
        case 'atad': {
          ...
          doSetupWaveTable:
            if (byteDepth == 255)
              return ERROR_FILE_UNSUPPORTED; // If haven't found "fmt " tag yet, we don't know the
                                             // bit depth or anything. Shouldn't happen.

            if (numChannels != 1)
              return ERROR_FILE_NOT_LOADABLE_AS_WAVETABLE_BECAUSE_STEREO; // Stereo files not
                                                                          // useable as WaveTable,
                                                                          // ever.

            // If this isn't actually a wavetable-specifying file or at least a wavetable-looking
            // length, and the user isn't insisting, then opt not to do it.
        ...
        case ' tmf': {
          ...
          if (numChannels != 1 && numChannels != 2) { return ERROR_FILE_UNSUPPORTED; }
        ...
        case 'lpms': {
          if (type == AUDIO_FILE_TYPE_SAMPLE) {
            ...
            if ((midiNote || midiPitchFraction) && midiNote < 128)
              ((Sample*)this)->midiNoteFromFile =
                  (float)midiPitchFraction / ((uint64_t)1 << 32) + midiNote;
    ...
    // ------ AIFF ------
      ...
      switch (thisChunk.name) {
        ...
        case 'MMOC': {
          ...
          if (byteDepth > 1) { rawDataFormat = RAW_DATA_ENDIANNESS_WRONG_16 + byteDepth - 2; }
          ...
          if (type == AUDIO_FILE_TYPE_SAMPLE) {
            ...
            if (sampleRate < 5000 || sampleRate > 96000) { return ERROR_FILE_UNSUPPORTED; }
...
bool AudioFile::mayBeStolen(void* thingNotToStealFrom) {
  ...
  // If we were stolen, sampleManager.audioFiles would get an entry deleted from it, and that's not
  // allowed while it's being inserted to, which is when we'd be provided it as the
  // thingNotToStealFrom.
  return (thingNotToStealFrom != &audioFileManager.audioFiles);

  // We don't have to worry about e.g. a Sample being stolen as we try to allocate a Cluster for it
  // in the same way as we do with SampleCaches - because in a case like this, the Sample would have
  // a reason and so not be stealable.
}
