...
Compressor::Compressor() {
  ...
  // I'm so sorry, this is incredibly ugly, but in order to decide the default
  // sync level, we have to look at the current song, or even better the one
  // being preloaded. Default sync level is used obviously for the default synth
  // sound if no SD card inserted, but also some synth presets, possibly just
  // older ones, are saved without this so it can be set to the default at the
  // time of loading.
...
int32_t Compressor::getActualAttackRate() {
  ...
  if (sync == 0)
    ...
    alteredAttack =
        multiply_32x32_rshift32(attack << 11, playbackHandler.getTimePerInternalTickInverse());
...
int32_t Compressor::getActualReleaseRate() {
  ...
  if (sync == 0)
    ...
    alteredRelease =
        multiply_32x32_rshift32(release << 13, playbackHandler.getTimePerInternalTickInverse()) >>
        (9 - sync);
  }
...
int32_t Compressor::render(uint16_t numSamples, int32_t shapeValue) {
  ...
  if (pendingHitStrength != 0) {
    ...
    // Only actually do anything if this hit is going to cause a bigger dip than
    // we're already currently experiencing
    if (newOffset < lastValue) {
      envelopeOffset = newOffset;

      // If attack is all the way down, jump directly to release stage
      if (attack == attackRateTable[0] << 2) { goto prepareForRelease; }
  ...
  if (status == ENVELOPE_STAGE_ATTACK) {
    ...
    // lastValue = (multiply_32x32_rshift32(envelopeHeight, decayTable4[pos >>
    // 13]) << 1) + envelopeOffset; // Goes down quickly at first. Bad lastValue
    // = (multiply_32x32_rshift32(envelopeHeight, 2147483647 - (pos << 8)) << 1)
    // + envelopeOffset; // Straight line
    lastValue =
        (multiply_32x32_rshift32(envelopeHeight, (2147483647 - getDecay4(8388608 - pos, 23)))
         << 1) +
        envelopeOffset; // Goes down slowly at first. Great squishiness
    // lastValue = (multiply_32x32_rshift32(envelopeHeight, (2147483647 -
    // decayTable8[1023 - (pos >> 13)])) << 1) + envelopeOffset; // Even slower
    // to accelerate. Loses punch slightly lastValue =
    // (multiply_32x32_rshift32(envelopeHeight, (sineWave[((pos >> 14) + 256) &
    // 1023] >> 1) + 1073741824) << 1) + envelopeOffset; // Sine wave. Sounds a
    // bit flat lastValue = (multiply_32x32_rshift32(envelopeHeight, 2147483647
    // - pos * (pos >> 15)) << 1) + envelopeOffset; // Parabola. Not bad, but
    // doesn't quite have punchiness
  ...
  doRelease:
    ...
    lastValue =
        2147483647 - envelopeHeight + (multiply_32x32_rshift32(preValue, envelopeHeight) << 1);

    // lastValue = 2147483647 - (multiply_32x32_rshift32(decayTable8[pos >> 13],
    // envelopeHeight) << 1); // Upside down exponential curve lastValue =
    // 2147483647 - (((int64_t)((sineWave[((pos >> 14) + 256) & 1023] >> 1) +
    // 1073741824) * (int64_t)envelopeHeight) >> 31); // Sine wave. Not great
    // lastValue = (multiply_32x32_rshift32(pos * (pos >> 15), envelopeHeight)
    // << 1); // Parabola. Doesn't "punch".
  } else { // Off
  doOff:
    lastValue = 2147483647;
  }
