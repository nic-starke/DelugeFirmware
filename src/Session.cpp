...
void Session::armAllClipsToStop(int afterNumRepeats) {
  ...
  uint8_t  launchStatus = investigateSyncedLaunch(waitForClip,
  ...
  if (launchStatus == LAUNCH_STATUS_NOTHING_TO_SYNC_TO) {
    // We'd never actually get here, because there always are Clips playing if
    // this function gets called I think
  }
    ...
    scheduleLaunchTiming(playbackHandler.getActualSwungTickCount() + ticksTilSwap,
                         afterNumRepeats,
                         quantization);
  }
...
void Session::armNextSection(int oldSection, int numRepetitions) {
  ...
  if (currentSong->sessionClips.getClipAtIndex(0)->section != oldSection) {
    ...
    for (int c = 1; c < currentSong->sessionClips.getNumElements();
         c++) { // NOTE: starts at 1, not 0
      Clip* clip = currentSong->sessionClips.getClipAtIndex(c);

      if (clip->section == oldSection) {
        int newSection = currentSong->sessionClips.getClipAtIndex(c - 1)
                             ->section; // Grab section from next Clip down
        userWantsToArmClipsToStartOrSolo(newSection,
                                         NULL,
                                         true,
                                         false,
                                         false,
...
bool Session::giveClipOpportunityToBeginLinearRecording(Clip* clip,
                                                        int   clipIndex,
  ...
  bool currentClipHasSameOutput =
      (currentSong->currentClip && currentSong->currentClip->output ==
                                       clip->output); // Must do this before calling
                                                      // opportunityToBeginLinearLoopRecording(),
                                                      // which may clone a new Output
  ...
  bool begun =
      clip->opportunityToBeginSessionLinearRecording(modelStack,
                                                     &newOutputCreated,
  ...
  if (begun) {
    ...
    if (clip->overdubNature != OVERDUB_NORMAL && playbackHandler.isEitherClockActive()) {
      ...
      if (clip->getCurrentlyRecordingLinearly() &&
          clip->overdubNature == OVERDUB_CONTINUOUS_LAYERING) {
  ...
  if (newOutputCreated) {
    ...
    if (getRootUI() == &arrangerView) {
      ...
      if (getCurrentUI() == &arrangerView) { arrangerView.exitSubModeWithoutAction(); }
...
void Session::doLaunch() {
  ...
  currentSong->deactivateAnyArrangementOnlyClips(); // In case any still playing after
                                                    // switch from arrangement
  ...
  // First do a loop through all Clips seeing which ones are going to launch, so
  // we can then go through again and deactivate those Outputs' other Clips
  for (int c = currentSong->sessionClips.getNumElements() - 1; c >= 0; c--) {
    ...
    // If this one's gonna launch to become active on its output (i.e. not gonna
    // clone its output) when it wasn't before...
    if (clip->armState && !currentSong->isClipActive(clip) &&
      ...
      if (alreadyLaunchedFor) {
        ...
        // No need to make note of that again, but we do get dibs if we're gonna
        // be soloing
    ...
    if (clip->soloingInSessionMode) {
      ...
      // If it's not armed, or its arming is just to stop recording, then it's
      // still gonna be soloing afterwards
      ...
      if (clip->armState == ARM_STATE_ON_TO_SOLO)
        ...
        if (clip->soloingInSessionMode || clip->activeIfNoSolo) { goto yesSomeActiveAfter; }
      }
    }
  }

  // Normally it's enough that we set alreadyGotItsNewClip and
  // isGettingSoloingClip on just the Outputs who have a Clip launching. But in
  // the case where soloing is stopping entirely, other Clips are going to be
  // launching, so we'll need to actually set these on all Outputs.
  ...
  // Ok, now action the stopping of all Clips which need to stop - including
  // ones which weren't actually armed to stop but need to stop in order to make
  // way for other ones which were armed to start. But we can't action the
  // starting of any Clips yet, until all stopping is done.
  for (int c = currentSong->sessionClips.getNumElements() - 1; c >= 0; c--) {
    Clip*                          clip = currentSong->sessionClips.getClipAtIndex(c);
    ...
    if (clip->wasActiveBefore) {
      ...
      if (clip->armState == ARM_STATE_ON_TO_SOLO) {
        ...
        // We were active before, and we'll still be active, so no big change,
        // just this:
        ...
        if (clip->getCurrentlyRecordingLinearly()) {
          clip->finishLinearRecording(modelStackWithTimelineCounter,
                                      NULL); // Won't be a pending overdub - those aren't allowed if
                                             // we're gonna be soloing
      ...
      // If armed to stop
        ...
        // If output-recording (resampling) is stopping, we don't actually want
        // to deactivate this Clip
        if (playbackHandler.stopOutputRecordingAtLoopEnd) { goto probablyJustKeepGoing; }
        ...
        doFinishLinearRecording:
          ...
          if (nextPendingOverdub) {
            nextPendingOverdub->copyBasicsFrom(clip); // Copy this again, in case it's changed since
                                                      // it was created
          }
      ...
      probablyJustKeepGoing:
        ...
        if (!currentSong->anyClipsSoloing && anySoloingAfter) {
          if (!stoppedLinearRecording && clip->getCurrentlyRecordingLinearly()) {
            ...
            goto becameInactiveBecauseOfAnotherClipSoloing; // Specifically do
                                                            // not change
                                                            // clip->activeIfNoSolo!
          }
        }
        ...
        if (outputsLaunchedFor.lookup((uint32_t)output)) {
          ...
          // If we're linearly recording, we want to stop that as well as
  ...
  // Now's the point where old linear recording has ended, and new is yet to
  // begin. So separate any Actions, for separate undoability
  actionLogger.closeAction(ACTION_RECORD);

  bool    sectionWasJustLaunched  = (lastSectionArmed < 254);
  bool    anyLinearRecordingAfter = false;
  int32_t distanceTilLaunchEvent  = 0; // For if clips automatically armed cos
                                       // they just started recording a loop
  ...
  for (int c = currentSong->sessionClips.getNumElements() - 1; c >= 0; c--) {
    ...
    // If we didn't already deal with this Clip, meaning it wasn't active before
    // this launch event...
    if (!clip->wasActiveBefore) {

      bool wasArmedToStartSoloing = (clip->armState == ARM_STATE_ON_TO_SOLO);

      // If it's not armed, normally nothing needs to happen of course - it can
      // just stay inactive
      if (!clip->armState) {

        // But if other soloing has stopped and we're suddenly to become active
        // as a result...
      ...
      probablyBecomeActive:
        // If the Output already got its new Clip, then this Clip has missed out
        // and can't become active on it
        ...
        doNormalLaunch:
          ...
          output = clip->output; // A new Output may have been created as
                                 // recording began
          ...
          output->setActiveClip(
              modelStackWithTimelineCounter); // Must be after
                                              // giveClipOpportunityToBeginLinearRecording(),
                                              // cos this call clears any
                                              // recorded-early notes
    ...
    // If we found a playing Clip outside of the armed section, or vice versa,
    // then we can't say we legitimately just launched a section
    if (clipActiveAfter != (clip->section == lastSectionArmed)) { sectionWasJustLaunched = false; }
  }

  currentSong->anyClipsSoloing = anySoloingAfter;

  // If some Clips are playing and they're all in the same section, we want to
  // arm the next section
  ...
  // Otherwise...
    ...
    if (!anyClipsStillActiveAfter) {
      ...
      // If we're using the internal clock, we have the power to stop playback
      // entirely
      if (playbackHandler.playbackState & PLAYBACK_CLOCK_INTERNAL_ACTIVE) {
        ...
        // Or if the action was to manually stop all sections, which could
        // happen if the last section in the song was playing, or if the user
        // ended output recording with playback
    ...
    // Or if some Clips still active...
      ...
      if (distanceTilLaunchEvent) {
        scheduleLaunchTiming(playbackHandler.lastSwungTickActioned + distanceTilLaunchEvent,
                             1,
  ...
  // If we were doing linear recording before, but we just stopped, then exit
  // RECORD mode, as indicated on LED
...
// Taking sync-scaling and the Clip's length into account, puts us at the place
// in the Clip as if playback had occurred under these conditions since the
// input clock started. Presumably we'd call this if the conditions have changed
// (e.g. sync-scaling changed) and we want to restore order
void Session::reSyncClipToSongTicks(Clip* clip) {
  ...
  // I've sort of forgotten why this bit here is necessary (well, I know it
  // deals with the skipping of ticks). Could it just be put into the setPos()
  // function? I basically copied this from Editor::launchClip()
...
// Will appropriately change a Clip's play-pos to sync it to input MIDI clock or
// another appropriate Clip's play-pos. Currently called only when a Clip is
// created or resized. In some cases, it'll determine that it doesn't want to
// resync the Clip - e.g. if there's nothing "nice" to sync it to. But if
// mustSetPosToSomething is supplied as true, then we'll make sure we still set
// the pos to something / sync it to something (because it presumably didn't
// have a valid pos yet). Check playbackHandler.isEitherClockActive() before
// calling this.
void Session::reSyncClip(ModelStackWithTimelineCounter* modelStack,
  ...
  if (playbackHandler.isEitherClockActive() && modelStack->song->isClipActive(clip)) {
    ...
    if (playbackHandler.playbackState & PLAYBACK_CLOCK_EXTERNAL_ACTIVE) {
      ...
      // If this is in fact the sync-scaling Clip, we want to resync all Clips.
      // The song will have already updated its inputTickMagnitude
      if (clip == modelStack->song->getSyncScalingClip()) {
        playbackHandler.resyncInternalTicksToInputTicks(modelStack->song);
      }

      // Otherwise, set its position according to the incoming clock count.
      // (Wait, why on earth do I have it not syncing to other Clips in such
      // cases? Some weird historical Deluge relic?)
      else {
        ...
        doReSyncToSongTicks:
          ...
          // ...but only if Clip length is a square multiple of input clock
          // scaling
          uint32_t a = modelStack->song->getInputTickScale();
          while (a < clip->loopLength) { a <<= 1; }
          if (a == clip->loopLength) { goto doReSyncToSongTicks; }
        }
      }
    }

    // Or if playing from internal clock, then try to sync to another Clip with
    // a similar-looking length (i.e. hopefully the same time signature)
    else {
      Clip* syncToClip =
          modelStack->song->getLongestActiveClipWithMultipleOrFactorLength(clip->loopLength,
                                                                           false,
                                                                           clip);
      if (syncToClip) {
        ...
        // Only call "resume" if pos actually changed. This way, we can save
        // some dropping out of AudioClips
        ...
        // For AudioClips, even if we're not gonna call resumePlayback(), we
        // still need to do some other stuff if length has been changed (which
        // it probably has if we're here)
      doAudioClipStuff:
  ...
  // Or if Deluge *is* playing session...
    ...
    if (forceLateStart) {
      ...
      // Otherwise, normal case - arm this Clip to stop soloing
        ...
        // armClips0(0, clip, false, forceLateStart); // Force "late start" if
        // user holding shift button
        clip->armState = ARM_STATE_ON_NORMAL;
        int64_t wantToStopAtTime =
            playbackHandler.getActualSwungTickCount() -
            clip->getClipToRecordTo()->getActualCurrentPosAsIfPlayingInForwardDirection() +
            clip->loopLength;
...
// Beware - calling this might insert or delete a Clip! E.g. if we disarm a Clip
// that had a pending overdub, the overdub will get deleted. clipIndex is
// optional.
void Session::toggleClipStatus(Clip* clip, int* clipIndex, bool doInstant, int buttonPressLatency) {
  ...
  // Or, if some other Clip is soloed, just toggle the playing status - it won't
  // make a difference
  else if (currentSong->getAnyClipsSoloing()) {
    clip->activeIfNoSolo = !clip->activeIfNoSolo;

    // If became "active" (in background behind soloing), need to "deactivate"
    // any other Clips - still talking about in the "background" here.
    if (clip->activeIfNoSolo) {
  ...
  // Or if no other Clip was soloed...
    ...
    if (!clip->activeIfNoSolo) {
      ...
      if (!playbackHandler.isEitherClockActive()) {
        ...
        // Special case if doing tempoless recording elsewhere - this action
        // stops that
      ...
      if (playbackHandler.playbackState) {
        ...
        // Session active
          ...
          if (doInstant) {
            ...
            // Linear recording - stopping instantly in this case means reducing
            // the Clip's length and arming to stop recording real soon, at next
            // tick
            if (clip->getCurrentlyRecordingLinearly()) {
              cancelAllArming();
              cancelAllLaunchScheduling();
              Action* action = actionLogger.getNewAction(ACTION_RECORD, true);
              currentSong->setClipLength(clip,
                                         clip->getLivePos() + 1,
                                         action,
            ...
            // Or normal
              ...
              if (playbackHandler.recording == RECORDING_ARRANGEMENT) {
                clip->getClipToRecordTo()->endInstance(
                    playbackHandler.getActualArrangementRecordPos());
              }
...
void Session::armClipToStopAction(Clip* clip) {
  ...
  int32_t actualCurrentPos =
      (uint32_t)clip->getClipToRecordTo()->getActualCurrentPosAsIfPlayingInForwardDirection() %
      (uint32_t)clip->loopLength;
  int64_t wantToStopAtTime =
      playbackHandler.getActualSwungTickCount() - actualCurrentPos + clip->loopLength;
...
void Session::soloClipAction(Clip* clip, int buttonPressLatency) {
  ...
  // Or if it wasn't...
    ...
    if (!playbackHandler.isEitherClockActive()) {
      ...
      userWantsToArmClipsToStartOrSolo(0,
                                       clip,
                                       false,
                                       ...
                                       1,
                                       true,
                                       ARM_STATE_ON_TO_SOLO); // Force "late start" if user holding
                                                              // shift button
    }
  }
...
void Session::armSection(uint8_t section, int buttonPressLatency) {
  ...
  // Get rid of soloing. And if we're not a "share" section, get rid of arming
  // too
  currentSong->turnSoloingIntoJustPlaying(currentSong->sections[section].numRepetitions != -1);

  // If every Clip in this section is already playing, and no other Clips are
  // (unless we're a "share" section), then there's no need to launch the
  // section because it's already playing. So, make sure this isn't the case
  // before we go and do anything more
  for (int c = 0; c < currentSong->sessionClips.getNumElements(); c++) {
    Clip* clip = currentSong->sessionClips.getClipAtIndex(c);
    ...
    // If a Clip in another section is playing and we're not a "share"
    // section...
...
yupThatsFine:
  ...
  // Or if Deluge playing
    ...
    userWantsToArmClipsToStartOrSolo(section,
                                     NULL,
                                     stopAllOtherClips,
                                     false,
                                     false); // Don't allow "late start". It's too fiddly to
                                             // implement, and rarely even useful for sections
...
// Can sorta be applicable either then !playback state, or when tempoless
// recording.
void Session::armSectionWhenNeitherClockActive(ModelStack* modelStack,
                                               int         section,
  ...
  for (int c = 0; c < modelStack->song->sessionClips.getNumElements(); c++) {
    ...
    if (stopAllOtherClips && clip->section != section && clip->activeIfNoSolo) {
      // thisClip->expectNoFurtherTicks(currentSong); // No, don't need this,
      // cos it's not playing!
      clip->activeIfNoSolo = false;
    }
  }
}
...
void Session::scheduleOverdubToStartRecording(Clip* overdub, Clip* clipAbove) {
  ...
  if (!playbackHandler.isEitherClockActive()) { return; }

  Clip* waitForClip = clipAbove;
  if (!waitForClip || !currentSong->isClipActive(waitForClip)) {
    waitForClip = currentSong->getLongestActiveClipWithMultipleOrFactorLength(overdub->loopLength,
                                                                              true,
                                                                              overdub);
  }
  ...
  if (launchStatus == LAUNCH_STATUS_NOTHING_TO_SYNC_TO) {
    playbackHandler.endPlayback();
    playbackHandler.setupPlaybackUsingInternalClock(); // We're restarting playback, but it
                                                       // was already happening, so no need
                                                       // for PGMs
  }
...
void Session::userWantsToArmClipsToStartOrSolo(uint8_t section,
  ...
  if (clip) {
    ...
    // Now (Nov 2020), we're going to call
    // getLongestActiveClipWithMultipleOrFactorLength() for all launching of a
    // Clip (not a section). It seems that for the past several years(?) this
    // was broken and would always just wait for the longest Clip.
    waitForClip = currentSong->getLongestActiveClipWithMultipleOrFactorLength(
        clip->loopLength); // Allow it to return our same Clip if it wants - and
                           // if it's active, which could be what we want in the
                           // case of arming-to-solo if Clip already active
  ...
  // ... or if launching a whole section
    ...
    // We don't want to call getLongestActiveClipWithMultipleOrFactorLength(),
    // because when launching a new section, the length of any of the new Clips
    // being launched is irrelevant, cos they won't be playing at the same time
    // as any previously playing ones (normally). Also, this is how it seems to
    // have worked for several years(?) until the bugfix above, and I wouldn't
    // want the behaviour changing on any users.
  ...
  uint8_t  launchStatus = investigateSyncedLaunch(waitForClip,
                                                 &currentPosWithinQuantization,
                                                 &quantization,
                                                 longestStartingClipLength,
                                                 allowSubdividedQuantization);
  ...
  // This case, too, can only actually happen if no Clips are playing
    ...
    // Or, if a whole section...
      ...
      for (int l = 0; l < currentSong->sessionClips.getNumElements(); l++) {
        ...
        if (thisClip->section == section) {
          ...
          // If we're arming a section, we know there's no soloing or armed
          // Clips, so that's easy. Only arm if it's not playing
          if (!thisClip->activeIfNoSolo) { armClipLowLevel(thisClip, armState); }
        }
      }
    }
  }
    ...
    armClipsToStartOrSoloWithQuantization(currentPosWithinQuantization,
                                          quantization,
                                          section,
                                          stopAllOtherClips,
                                          clip,
...
int Session::investigateSyncedLaunch(Clip*     waitForClip,
  ...
  if (!waitForClip) {
    ...
    // Otherwise...
      ...
      // If a clock is coming in or out, or metronome is on, use that to work
      // out the loop point
      if ((playbackHandler.playbackState & PLAYBACK_CLOCK_EXTERNAL_ACTIVE) ||
          playbackHandler.midiOutClockEnabled || playbackHandler.metronomeOn ||
        ...
        // If using internal clock (meaning metronome or clock output is on),
        // just quantize to one bar. This is potentially imperfect.
        if (playbackHandler.playbackState & PLAYBACK_CLOCK_INTERNAL_ACTIVE) {
          *quantization = oneBar;
        }

        // Otherwise, quantize to: the sync scale, magnified up to be at least 3
        // beats long
        else {

          // Work out the length of 3 beats, given the length of 1 bar. Or if 1
          // bar is already too short, just use that, to avoid bugs
          uint32_t threeBeats;
          if (oneBar >= 2)
      ...
      // Or if using internal clock with no metronome or clock incoming or
      // outgoing, then easy - we can really just restart playback
      else {
  ...
  // Or, the more normal case where some Clips were already playing
    ...
    // Quantize the launch to the length of the already-playing long Clip, or if
    // the new Clip / section fits into it a whole number of times, use that
    // length
...
// Returns whether we are now armed. If not, it means it's just done the swap
...
bool Session::armForSongSwap() {
  ...
  if (launchStatus == LAUNCH_STATUS_NOTHING_TO_SYNC_TO) {
    ...
    playbackHandler.endPlayback();
    playbackHandler.setupPlaybackUsingInternalClock(); // No need to send PGMs - they're
                                                       // sent in doSongSwap().
    return false;
  }
...
void Session::armClipsToStartOrSoloWithQuantization(uint32_t pos,
  ...
  // We want to allow the launch point to be a point "within" the longest Clip,
  // at multiple lengths of our shortest launching Clip
  pos = pos % quantization;

  bool doLateStart = forceLateStart;

  // If we were doing this just for one Clip (so a late-start might be allowed
  // too)...
  if (clip) {
    if (!doLateStart && allowLateStart) { // Reminder - late start is never allowed for sections
                                          // - just cos it's not that useful, and tricky to
                                          // implement

      // See if that given point was only just reached a few milliseconds ago -
      // in which case we'll do a "late start"
  ...
  // Or, if we were doing it for a whole section - which means that we know
  // armState == ARM_STATE_ON_NORMAL, and no late-start
  else {
    OpenAddressingHashTableWith32bitKey outputsWeHavePickedAClipFor;

    // Ok, we're going to do a big complex thing where we traverse just once (or
    // occasionally twice) through all sessionClips. Reverse order so behaviour
    // of this new code is the same as the old code
    for (int c = currentSong->sessionClips.getNumElements() - 1; c >= 0; c--) {
      Clip* thisClip = currentSong->sessionClips.getClipAtIndex(c);
      ...
      if (thisClip->section == section) {
        ...
        // Because we're arming a section, we know there's no soloing Clips, so
        // that's easy.
        ...
        if (alreadyPickedAClip) {
          ...
          if (!output->nextClipFoundShouldGetArmed) {
            ...
            // We're gonna make this Clip active, but we may have already tried
            // to make a previous one on this Output active, so go back through
            // all previous ones and deactivate / disarm them
        ...
        wantActive:
          ...
          if (thisClip->activeIfNoSolo) {
            ...
            if (thisClip->armState) { thisClip->armState = ARM_STATE_OFF; }
            output->nextClipFoundShouldGetArmed = true;
          }

          // Or if it's inactive (the most normal case), we want to arm it to
          // launch.
          else {
      ...
      // Or if thisClip is in a different section, and if we're going to stop
      // all such Clips...
      else {
        ...
        // Or, if we don't want to stop it because of its section, we'll need to
        // make sure that it just doesn't share an Output with one of the clips
        // which is gonna get launched, in our new section. Unfortunately, we
        // haven't seen all of those yet, so we'll have to come through and do
        // this in a separate, second traversal - see next comment below
      }
    }

    // Ok, and as mentioned in the big comment directly above, if we're not
    // doing a stopAllOtherClips, only now are we in a position to know which of
    // those other Clips we still will need to stop because of their Output
    if (!stopAllOtherClips) {

      for (int c = 0; c < currentSong->sessionClips.getNumElements(); c++) {
        Clip* thisClip = currentSong->sessionClips.getClipAtIndex(c);

        // Ok, so if it's from another section (only those, because we've
        // already dealt with the ones in our section)...
        if (thisClip->section != section) {

          // And if it's currently active...
          if (thisClip->activeIfNoSolo) {

            // If we've already picked a Clip for this same Output, we
            // definitely don't want this one remaining active, so arm it to
            // stop
...
void Session::armClipToStartOrSoloUsingQuantization(Clip*    thisClip,
  ...
  if (armState == ARM_STATE_ON_TO_SOLO) {
    ...
    if (doLateStart) {
      ...
      // currentSong->deactivateAnyArrangementOnlyClips(); // In case any left
      // playing after switch from arrangement

      bool wasAlreadyActive = currentSong->isClipActive(thisClip);

      soloClipRightNow(modelStack);

      if (!wasAlreadyActive) { goto setPosAndStuff; }
    }
  ...
  // Arm to start regular play
    ...
    if (doLateStart) {
      ...
      // Must call this before setPos, because that does stuff with
      // ParamManagers
      currentSong->assertActiveness(modelStack,
                                    playbackHandler.getActualArrangementRecordPos() - pos);

    setPosAndStuff:
      // pos is a "live" pos, so we have to subtract swungTicksSkipped before
      // setting the Clip's lastProcessedPos, because it's soon going to be
      // jumped forward by that many ticks
...
void Session::armClipLowLevel(Clip* clipToArm,
                              int   armState,
                              bool  mustUnarmOtherClipsWithSameOutput) {
  ...
  if (mustUnarmOtherClipsWithSameOutput) {
    ...
    for (int c = 0; c < currentSong->sessionClips.getNumElements(); c++) {
      ...
      if (clip != clipToArm && !clip->soloingInSessionMode && !clip->activeIfNoSolo &&
          clip->armState && clip->output == clipToArm->output) {
        clip->armState = ARM_STATE_OFF;
      }
    }
  }
...
// Exactly what the return values of 255 and 254 mean has been lost, but they're
// treated as interchangeable by the function that calls this anyway
...
// This is a little bit un-ideal, but after an undo or redo, this will be
// called, and it will tell every active Clip to potentially expect a note or
// automation event - and to re-get all current automation values. I wish we
// could easily just do this to the Clips that need it, but we don't store an
// easy list of just the Clips affected by each Action. This is only to be
...
bool Session::endPlayback() {
  ...
  for (int c = 0; c < currentSong->sessionClips.getNumElements(); c++) {
    ...
    clip->cancelAnyArming();
    if (currentSong->isClipActive(clip)) { clip->expectNoFurtherTicks(currentSong); }
  }

  currentSong->deactivateAnyArrangementOnlyClips(); // In case any still playing after
                                                    // switch from arrangement

  // If we were waiting for a launch event, we've now stopped so will never
  // reach that point in time, so we'd better swap right now.
...
void Session::resetPlayPos(int32_t newPos, bool doingComplete, int buttonPressLatency) {
  ...
  // This function may begin a tempoless record - but it doesn't actually know
  // or need to know whether that's the resulting outcome

  AudioEngine::bypassCulling = true;

  currentSong->deactivateAnyArrangementOnlyClips(); // In case any still playing after
                                                    // switch from arrangement.
                                                    // Remember, this function will be
                                                    // called on playback begin, song
                                                    // swap, and more
  ...
  if (playbackHandler.recording == RECORDING_ARRANGEMENT) {
    ...
    currentSong->paramManager.setPlayPos(arrangement.playbackStartedAtPos,
  ...
  for (int c = currentSong->sessionClips.getNumElements() - 1; c >= 0; c--) {
    ...
    // Sometimes, after finishing a tempoless record, a new pending overdub will
    // have been created, and we need to act on it here
    ...
    // If Clip active, or if it's a pending overdub, which means we wanna make
    // it active...
    if (currentSong->isClipActive(clip)) {
    ...
    yeahNahItsOn:
      ...
      if (doingComplete) {
        // If starting after 0, must do the "resume" function, to get samples
        // playing from right point, etc
        if (newPos) {
        ...
        // Otherwise, so long as not doing count-in, begin linear recording
          ...
          if (clip->armState == ARM_STATE_ON_NORMAL) { // What's this for again? Auto arming of
                                                       // sections? I think not linear
                                                       // recording...
            distanceTilLaunchEvent = getMax(distanceTilLaunchEvent, clip->loopLength);
          }
        }
      ...
      clip->output->setActiveClip(modelStackWithTimelineCounter); // Not sure quite why we needed to
                                                                  // set this here?
    }
  }
  ...
  if (doingComplete) {
    ...
    // If just became armed (audio clip began recording)... The placement of
    // this probably isn't quite ideal...
    else if (distanceTilLaunchEvent) {
      scheduleLaunchTiming(playbackHandler.lastSwungTickActioned + distanceTilLaunchEvent,
                           1,
                           distanceTilLaunchEvent);
      armingChanged(); // This isn't really ideal. Is here for AudioClips which
                       // just armed themselves in setPos() call
    }
  }
}

// TODO: I'd like to have it so this doesn't get called on the first tick of
// playback - now that this function is also responsible for doing the
// incrementing. It works fine because we supply the increment as 0 in that
// case, but it'd be more meaningful this proposed new way...
...
bool Session::considerLaunchEvent(int32_t numTicksBeingIncremented) {
  ...
  // We now increment the currentPos of all Clips before doing any launch event
  // - so that any new Clips which get launched won't then get their pos
  // incremented

  // For each Clip in session and arranger (we include arrangement-only Clips,
  // which might still be left playing after switching from arrangement to
  // session)
  ClipArray* clipArray = &currentSong->sessionClips;
traverseClips:
  for (int c = 0; c < clipArray->getNumElements(); c++) {
  ...
  if (launchEventAtSwungTickCount &&
      playbackHandler.lastSwungTickActioned >= launchEventAtSwungTickCount) {
    ...
    if (numRepeatsTilLaunch <= 0) {
      ...
      if (preLoadedSong) {
        ...
        if (currentPlaybackMode == &arrangement) { return swappedSong; }

        currentPlaybackMode->resetPlayPos(
            0); // activeClips have been set up already / PGMs have been sent.
                // Calling this will resync arpeggiators though...
        // If switching to arranger, that'll happen as part of doSongSwap(),
        // above
      ...
      // Or if Clips launching...
        ...
        // NOTE: we do NOT want to set playbackHandler.swungTicksSkipped to 0
        // here, cos that'd mess up all the other Clips!
        ...
        if (!playbackHandler.isEitherClockActive()) { return swappedSong; }
      }
    }
    ...
    // Or if repeats do remain, just go onto the next one
      ...
      launchEventAtSwungTickCount =
          playbackHandler.lastSwungTickActioned + currentArmedLaunchLengthForOneRepeat;
#if HAVE_OLED
      if (getCurrentUI() == &loadSongUI)
  ...
  // If this is the first tick, we have to do some stuff to arm the first
  // song-section change
...
void Session::doTickForward(int posIncrement) {
  ...
  if (launchEventAtSwungTickCount) {
    int32_t ticksTilLaunchEvent =
        launchEventAtSwungTickCount - playbackHandler.lastSwungTickActioned;

    playbackHandler.swungTicksTilNextEvent =
        getMin(ticksTilLaunchEvent, playbackHandler.swungTicksTilNextEvent);
  }
  ...
  if (playbackHandler.recording == RECORDING_ARRANGEMENT) {
    ModelStackWithThreeMainThings* modelStackWithThreeMainThings =
        currentSong->addToModelStack(modelStack);

    if (currentSong->paramManager.mightContainAutomation()) {
      currentSong->paramManager.processCurrentPos(modelStackWithThreeMainThings,
                                                  posIncrement,
                                                  false);
      playbackHandler.swungTicksTilNextEvent = getMin(playbackHandler.swungTicksTilNextEvent,
                                                      currentSong->paramManager.ticksTilNextEvent);
    }
  }

  // Tell all the Clips that it's tick time. Including arrangement-only Clips,
  // which might still be left playing after switching from arrangement to
  // session For each Clip in session and arranger
  ClipArray* clipArray = &currentSong->sessionClips;
traverseClips:
  for (int c = 0; c < clipArray->getNumElements(); c++) {
    Clip* clip = clipArray->getClipAtIndex(c);
    ...
    // No need to do the actual incrementing - that's been done for all Clips
    // (except ones which have only just launched), up in considerLaunchEvent()

    clip->processCurrentPos(modelStackWithTimelineCounter,
                            posIncrement); // May create new Clip and put it in the ModelStack -
                                           // we'll check below.

    // NOTE: posIncrement is the number of ticks which we incremented by in
    // considerLaunchEvent(). But for Clips which were only just launched in
    // there, well the won't have been incremented, so it would be more correct
    // if posIncrement were 0 here. But, I don't believe there's any ill-effect
    // from having a posIncrement too big in this case. It's just not super
    // elegant.

    // New Clip may have been returned for AudioClips being recorded from
    // session to arranger
  ...
  // Do arps too (hmmm, could we want to do this in considerLaunchEvent()
  // instead, just like the incrementing?)
  for (Output* thisOutput = currentSong->firstOutput; thisOutput; thisOutput = thisOutput->next) {
    ...
    playbackHandler.swungTicksTilNextEvent =
        getMin(ticksTilNextArpEvent, playbackHandler.swungTicksTilNextEvent);
  }

  /*
  for (Instrument* thisInstrument = currentSong->firstInstrument;
  thisInstrument; thisInstrument = thisInstrument->next) { if
  (thisInstrument->activeClip && isClipActive(thisInstrument->activeClip)) {
...
void Session::resyncToSongTicks(Song* song) {
  ...
  for (int c = 0; c < song->sessionClips.getNumElements(); c++) {
    ...
    if (song->isClipActive(clip)) { reSyncClipToSongTicks(clip); }
  }
}
...
void Session::unsoloClip(Clip* clip) {
  ...
  // If any other Clips are still soloing, or this Clip isn't active outside of
  // solo mode, we need to shut that Clip up
  ...
  // Re-activate *other* Clips (i.e. not this one) if this was the only soloed
  // Clip
...
void Session::soloClipRightNow(ModelStackWithTimelineCounter* modelStack) {
  ...
  if (!anyOthersAlreadySoloed) {
    ...
    if (playbackHandler.isEitherClockActive()) {
      ...
      // - *and* also cancel any other arming, unless it's arming to become
      // soloed. Non-solo-related arming is not allowed when Clips are soloing.
      for (int c = 0; c < modelStack->song->sessionClips.getNumElements(); c++) {
        ...
        if (thisClip != clip) { // Only *other* Clips
          if (thisClip->activeIfNoSolo) {
            ...
            if (playbackHandler.recording == RECORDING_ARRANGEMENT) {
              thisClip->getClipToRecordTo()->endInstance(
                  playbackHandler.getActualArrangementRecordPos());
            }
          }

          // As noted above, non-solo arming is not allowed now that there will
          // be a Clip soloing.
    ...
    if (cancelledAnyArming) { launchSchedulingMightNeedCancelling(); }

    // Might need to activate this Clip
    if (!clip->activeIfNoSolo) { goto doAssertThisClip; }
  }
...
int32_t Session::getPosAtWhichClipWillCut(ModelStackWithTimelineCounter const* modelStack) {
  ...
  // If recording arrangement, pretend it's gonna cut at the end of the current
  // length, cos we're actually going to auto-extend it when we get there, so we
  // don't want any wrapping-around happening
  ...
  if (willClipContinuePlayingAtEnd(modelStack)) {
    cutPos = clip->currentlyPlayingReversed ? (-2147483648) :
                                              2147483647; // If it's gonna loop, it's not gonna cut
  }
  ...
  if (clip->sequenceDirectionMode == SEQUENCE_DIRECTION_PINGPONG) {
    if (clip->currentlyPlayingReversed) {
      if (cutPos < 0) {
        cutPos =
            clip->lastProcessedPos ? 0 : -clip->loopLength; // Check we're not right at pos 0, as we
                                                            // briefly will be when we pingpong at
                                                            // the right-hand end of the Clip/etc.
      }
    } else {
...
bool Session::willClipContinuePlayingAtEnd(ModelStackWithTimelineCounter const* modelStack) {
  ...
  if (!modelStack->song->isClipActive(clip))
    return false; // If Clip not active, just say it won't loop. We need that,
                  // cos an AudioClip's Sample may keep playing just after its
                  // Clip has stopped, and we don't wanna think it needs to loop

  // Note: this isn't quite perfect - it doesn’t know if Clip will cut out due
  // to another one launching. But the ill effects of this are pretty minor.
  bool willLoop = !launchEventAtSwungTickCount // If no launch event scheduled, obviously
                                               // it'll loop
                  || numRepeatsTilLaunch > 1   // If the launch event is gonna just trigger
                                               // another repeat, it'll loop
                  || clip->armState != ARM_STATE_ON_NORMAL // If not armed, or armed to solo,
                                                           // it'll loop (except see above)
                  || (clip->soloingInSessionMode &&
                      clip->activeIfNoSolo); // We know from the previous test that clip is
                                             // armed. If it's soloing, that means it's armed
                                             // to stop soloing. And if it is activeIfNoSolo,
                                             // that means it'll keep playing, if we assume
                                             // *all* clips are going to stop soloing (a false
                                             // positive here doesn't matter too much)

  // Ok, that's most of our tests done. If one of them gave a true, we can get
  // out now.
  if (willLoop) return true;

  // Otherwise, one final test, which needed a bit of pre-logic.
  int32_t ticksTilReachLoopPoint = clip->currentlyPlayingReversed ?
                                       clip->lastProcessedPos :
                                       (clip->loopLength - clip->lastProcessedPos);
  return (launchEventAtSwungTickCount - playbackHandler.lastSwungTickActioned >
          ticksTilReachLoopPoint); // If launch event is after loop-point, it'll
                                   // loop.
}
...
bool Session::deletingClipWhichCouldBeAbandonedOverdub(Clip* clip) {
  ...
  bool shouldBeActiveWhileExistent =
      clip->activeIfNoSolo; // Yep, this works better (in some complex scenarios
                            // I tested) than calling isClipActive(), which
                            // would take soloing into account

  if (shouldBeActiveWhileExistent &&
      !(playbackHandler.playbackState && currentPlaybackMode == &arrangement)) {
    int   newClipIndex;
    Clip* newClip =
        currentSong->getSessionClipWithOutput(clip->output, -1, clip, &newClipIndex, true);
    if (newClip) { toggleClipStatus(newClip, &newClipIndex, true, 0); }
  }
