...
#define numInputTicksToAllowTempoTargeting \
...
#define MAX_SEQUENCE_LENGTH \
  1610612736                       // The biggest multiple of 3 which can fit in a signed 32-bit int
...
#define FILENAME_BUFFER_SIZE \
  256 // Don't ever make this less! The zoom rendering code uses this buffer for
      // its stuff
...
#define audioEngineBufferSize \
  128 // Maximum num samples that may be processed in one "frame". Actual size
      // of output buffer is in ssi.h
...
// Linear params have different sources multiplied together, then multiplied by
// the neutral value
// -- and "volume" ones get squared at the end

// Hybrid params have different sources added together, then added to the
// neutral value

// Exp params have different sources added together, converted to an exponential
// scale, then multiplied by the neutral value
...
#define PARAM_PLACEHOLDER_RANGE \
  89 // Not a real param. For the purpose of reading old files from before
     // V3.2.0
...
#define PARAM_STATIC_COMPRESSOR_VOLUME \
  164 // Only used for the reverb compressor. Normally this is done with
      // patching
#define PARAM_STATIC_PATCH_CABLE 190 // Special case

#define PERC_BUFFER_REDUCTION_MAGNITUDE \
  7 // This is about right. Making it smaller didn't help. Tried it as 9, and
    // I'm pretty sure some fast percussive details were lost in the output
...
#define INPUT_ENABLE_REPITCHED_BUFFER \
  0 // Experimental, from when developing input pitch shifting. Probably won't
    // actually work now, if it ever did!
...
#define TIME_STRETCH_CROSSFADE_MOVING_AVERAGE_LENGTH \
  35 // Anywhere between 30 and 40 seemed ideal. Point of interest - high
     // numbers (e.g. I tried 140) screw up the high notes, so more is not more!

#define TIME_STRETCH_ENABLE_BUFFER \
  0 // I think this was an experimental mode which allowed the pitch-change
    // effect (i.e. windowed sinc interpolation) to be stored and reused between
    // the two time-stretch play-heads. Probably won't work anymore. From
    // memory, wasn't very beneficial, especially relative to its complexity and
    // potential bugginess.
...
#define PITCH_DETECT_WINDOW_SIZE_MAGNITUDE \
  13 // We don't want the window too short, or some sounds / harmonics can be
     // missed during the attack
...
// Let's just do a 100 sample crossfade. Even 12 samples actually sounded fine
// for my voice - just obviously not so good for a low sine wave. Of course, if
// like 60 samples are being processed at a time under CPU load, then this might
// end up as low as 40.
...
#define STEALABLE_QUEUE_NO_SONG_SAMPLE_DATA_CONVERTED \
  1 // E.g. from floating point file, or wrong endianness AIFF file.
...
#define STEALABLE_QUEUE_CURRENT_SONG_SAMPLE_DATA_PERC_CACHE \
  9 // This one is super valuable and compacted data - lots of work to load it
    // all again
...
#define WAVETABLE_MIN_CYCLE_SIZE \
  8 // Not 4 - because NE10 can't do FFTs that small unless we enable its
    // additional C code, which would take up program size for little gain.
...
#define MIDI_OUTPUT_FILTER_NO_MPE \
  0 // To be used instead of MIDI_CHANNEL_MPE_LOWER_ZONE etc for functions that
    // require a "midi output filter". Although in fact, any number <16 or >=18
    // would work, the way I've defined it.
