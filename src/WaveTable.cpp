...
// This "should" handle inputs up to size 65536. Actually probably a bit bigger.
// Maybe even 131071?
void dft_r2c(ne10_fft_cpx_int32_t* __restrict__ out, int32_t const* __restrict__ in, uint32_t N) {
  ...
  for (uint32_t i = 0; i <= (N >> 1); i++) {
    ...
    do {
      ...
      sReal =
          multiply_accumulate_32x32_rshift32_rounded(sReal,
                                                     inputValueReal,
                                                     cosValue); // We lose one order of magnitude
                                                                // here, which is made up for below.
...
#define MAX_POSSIBLE_NUM_BANDS \
  ((FFT_CONFIG_MAX_MAGNITUDE - 1) >> (NUM_OCTAVES_BETWEEN_WAVETABLE_BANDS - 1))

#define WAVETABLE_ALLOW_INTERNAL_MEMORY 0

#define WAVETABLE_NUM_DUPLICATE_SAMPLES_AT_END_OF_CYCLE \
  7 // That's in samples - it'll be twice as many bytes.
#define SHOULD_DISCARD_WAVETABLE_DATA_WITH_INSUFFICIENT_HF_CONTENT 0

int WaveTable::setup(Sample*          sample,
                     int              rawFileCycleSize,
  ...
  // We do some checking here for conditions which make it completely impossible
  // for a file to be a WaveTable. As opposed for checks before this function is
  // called, for softer conditions which merely determine that this file might
  // not be *intended* as a WaveTable, but the user have have overridden.
  if (rawFileCycleSize < WAVETABLE_MIN_CYCLE_SIZE)
    return ERROR_FILE_NOT_LOADABLE_AS_WAVETABLE; // See comment on minimum FFT
                                                 // size - click thru to
  ...
  if (!rawFileCycleSizeIsAPowerOfTwo) {
    ...
    if (originalSampleLengthInSamples < (rawFileCycleSize << 1) &&
      ...
      // In order to represent all frequency information in the cycle once we
      // render it out to a power-of-two size, well we'll need to go slightly
      // bigger rather than slightly smaller. Also, the slightly bigger size is
      // needed so that sufficient memory is allocated to do all the
      // calculations in.
      initialBandCycleMagnitude++;
    }

    // Otherwise, if multiple cycles (wavetable), not allowed - it'd take too
    // long to process, and no one has wavetables of weird sizes anyway.
    else
  ...
  // Or, normal case where cycle size *is* a power of two.
    ...
    numCycles = originalSampleLengthInSamples >>
                initialBandCycleMagnitude; // Will round down, which is what we want.
    if (numCycles < 1)
      return ERROR_FILE_NOT_LOADABLE_AS_WAVETABLE; // Surely can't happen,
                                                   // becacuse of the checks /
                                                   // limiting we've done above?
  }

tryGettingFFTConfig:
  AudioEngine::logAction("Getting fft config");
  ne10_fft_r2c_cfg_int32_t fftCFGForInitialBand =
      FFTConfigManager::getConfig(initialBandCycleMagnitude);
  AudioEngine::logAction("Got fft config");

  // If that returned NULL, normally we can just opt to not do FFTs and have
  // just 1 band. But in the case where the original wasn't a power-of-two size,
  // we're gonna have to do FFTs (as well as one DFT)...
  if (!fftCFGForInitialBand && !rawFileCycleSizeIsAPowerOfTwo) {

    // Actually screw it, this is so rare, it's easier just to make a rule that
    // for non-power-of-two we *have* to render it out to that bigger size. Cos
    // if we allow a smaller initial band, we don't have enough space in it for
    // the dummy writing we do during load, which could be avoided, but yeah
    // this is so rare.
    return ERROR_INSUFFICIENT_RAM;

    /*
    // So, see if there's a smaller FFT config we can still get.
    if (initialBandCycleMagnitude <= 3) return ERROR_INSUFFICIENT_RAM; // But,
    can't go smaller than size 8 (2^3) - see comment on min size below.
    initialBandCycleMagnitude--;
  ...
  int initialBandCycleSizeNoDuplicates =
      1 << initialBandCycleMagnitude; // This will usually be the same as
                                      // rawFileCycleSize, but not when that's
                                      // not a power of two.

  uint8_t error;

  // numBands is set such that the "smallest" band will have 8 samples per
  // cycle. Not 4 - because NE10 can't do FFTs that small unless we enable its
  // additional C code, which would take up program size for little advantage.
  {
    int numBands =
        fftCFGForInitialBand ?
            ((initialBandCycleMagnitude - 2) >> (NUM_OCTAVES_BETWEEN_WAVETABLE_BANDS - 1)) :
            1;

    error = bands.insertAtIndex(0, numBands); // Don't refer to numBands after this! (Why? Because
                                              // we might end up using less after all?)
  }
  if (error) {
  gotError:
    return error;
  ...
  AudioEngine::routineWithClusterLoading(); // TODO: the routine calls in this
                                            // function might be more than
                                            // needed - I didn't profile very
                                            // closely.
  AudioEngine::logAction("about to set up bands");

  for (int b = 0; b < bands.getNumElements(); b++) {
    int cycleSizeNoDuplicates =
        initialBandCycleSizeNoDuplicates >> (b * NUM_OCTAVES_BETWEEN_WAVETABLE_BANDS);

    int bandSizeSamplesWithDuplicates =
        numCycles * (cycleSizeNoDuplicates + WAVETABLE_NUM_DUPLICATE_SAMPLES_AT_END_OF_CYCLE);
    int bandSizeBytesWithDuplicates = bandSizeSamplesWithDuplicates
                                      << 1; // All bands contain just 16-bit data.
                                            // Ironically we'll even do that if the source file was
                                            // just 8-bit, but that's really uncommon.
    void* bandDataMemory =
        generalMemoryAllocator.alloc(bandSizeBytesWithDuplicates + sizeof(WaveTableBandData),
                                     NULL,
                                     false,
                                     WAVETABLE_ALLOW_INTERNAL_MEMORY,
                                     true); // Stealable!
    if (!bandDataMemory) {
      error = ERROR_INSUFFICIENT_RAM;
      // All bands from this one onwards still have undefined data, so gotta get
      // rid of them before anything else tries to do anything with them.
  ...
  // Create the temporary memory where we'll store the 32-bit int version of the
  // current cycle being read, to perform the FFT on. This will also be used for
  // outputting the time-domain cycle data back out of the inverse FFT, so
  // ensure it's big enough for that if our biggest band is bigger than the
  // cycle size in the file (i.e. if that's not a power-of-two).
  ...
  int32_t* __restrict__ currentCycleInt32 =
      (int32_t*)generalMemoryAllocator.alloc(currentCycleMemorySize * sizeof(int32_t),
                                             NULL,
                                             false,
                                             true); // Internal RAM is good, and it's only temporary
  if (!currentCycleInt32) {
    error = ERROR_INSUFFICIENT_RAM;
    goto gotError2;
  }

  // And temporary FFT output (frequency domain) buffer. The same sizing
  // considerations as above are needed, and we use that same decision here
  // - except for frequency-domain complex numbers, we only need to store half
  // of it, plus one.
  ne10_fft_cpx_int32_t* __restrict__ frequencyDomainData =
      (ne10_fft_cpx_int32_t*)generalMemoryAllocator.alloc(((currentCycleMemorySize >> 1) + 1) *
  ...
  int16_t* __restrict__ initialBandWritePos =
      initialBand->dataAccessAddress; // Even for non-power-of-two cycle size files,
                                      // we'll still write the data in here even though
                                      // it's not needed and will be overwritten, just
                                      // since we're using the same code as for
                                      // power-of-two.
  ...
  if (!sample) {
    reader->jumpForwardToBytePos(audioDataStartPosBytes); // In case reader wasn't quite up to here
                                                          // yet! Can happen in AIFF files, with
                                                          // their "offset"
  }

  char const* sourceBuffer =
      storageManager.fileClusterBuffer; // This will get changed if we're converting an
                                        // existing Sample in memory.
...
#define MAGNITUDE_REDUCTION_FOR_FFT 12
  ...
  bool needToMisalignData =
      (rawDataFormat == RAW_DATA_FINE || rawDataFormat == RAW_DATA_UNSIGNED_8);
  ...
  for (int cycleIndex = 0; cycleIndex < numCycles; cycleIndex++) {
    ...
    int16_t* nativeBandCycleStartPos =
        initialBandWritePos; // Store this so we can refer to it at the end of
                             // the cycle, for duplicating values

    int32_t* cycleBufferDestination         = currentCycleInt32;
    int      sourceBytesLeftToCopyThisCycle = rawFileCycleSize * byteDepth;

    // Read some clusters of audio data - either from existing Sample in memory,
    // or from file.
    do {
      ...
      if (sample) {
        ...
        // If we need to load a new Cluster now (which might not be the case if
        // we've just switched into a new Cycle which starts still within the
        // same Cluster)...
      ...
      // Or, if reading data afresh from a file...
        ...
        if (byteIndexWithinCluster < 0)
          reader->byteIndexWithinCluster -= byteIndexWithinCluster; // Will put it right at the
                                                                    // start of the next cluster, to
                                                                    // force it to read from card
...
#define CONVERT_AND_STORE_SAMPLE                                                    \
  {                                                                                 \
    if (rawDataFormat == RAW_DATA_FLOAT) {                                          \
      ...
      if (swappingEndianness) { value32 = swapEndianness32(value32); }              \
      value32 &= bitMask;                                                           \
      if (rawDataFormat == RAW_DATA_UNSIGNED_8) { value32 += (1 << 31); }           \
    }                                                                               \
    *(cycleBufferDestination++) =                                                   \
        value32 >> MAGNITUDE_REDUCTION_FOR_FFT; /* Store full 32-bit value in       \
                                                   cycle buffer (reduced in         \
                                                   magnitude for FFT). */           \
    *(initialBandWritePos++) = value32 >> 16;   /* Bands only store 16-bit data. */ \
      ...
      if (byteIndexWithinCluster < 0) {
        ...
        char const* source =
            sourceBuffer + byteIndexWithinClusterMisaligned; // Misalign pointer for reading
                                                             // start of this buffer, for byte
                                                             // depth and also to correct for
                                                             // the overlap.

        bytesOverlappingFromLastCluster &=
            ((uint32_t)0xFFFFFFFF >> ((4 + byteIndexWithinClusterMisaligned) *
                                      8)); // Remember, this data was obtained with the pointer
                                           // already correctly misaligned for byte depth.
        ...
        if (!sample) {
          reader->byteIndexWithinCluster +=
              byteDepth + byteIndexWithinCluster; // +byteIndexWithinCluster because we
                                                  // already moved forward an extra
                                                  // -byteIndexWithinCluster, above.
        }
      ...
      // Stop before we get to the final sample that overlaps the end of the
      // cluster, if that happens.
      ...
      // Read all the data we can before reaching either the end of the cycle,
      // or the end of the file cluster
      ...
      if (byteIndexWithinCluster > audioFileManager.clusterSize - byteDepth) {
        ...
        byteIndexWithinCluster -=
            audioFileManager.clusterSize; // Might end up negative, indicating that we've
    ...
    // Ok, we've finished reading one wave cycle (which potentially spanned
    // multiple file clusters).
    uartPrint("\nCycle: ");
    uartPrintNumber(cycleIndex);

    // If it wasn't a power-of-two size, we have to do a DFT even just to get
    // the first band's useable time-domain data.
    if (!rawFileCycleSizeIsAPowerOfTwo) {
      AudioEngine::logAction("dft start");
      dft_r2c(frequencyDomainData, currentCycleInt32, rawFileCycleSize);
      // That function does indeed take ages, as shown by the AUDIO_LOG. But
      // somehow, I'm not hearing voices drop... how?
      AudioEngine::logAction("dft done");

      // Because the raw file cycle size was a bit smaller than the initial band
      // cycle size that we're going to be expanding it out to, we're going to
      // be feeding a slightly wider bit of (freq domain) data back into the FFT
      // to get time domain data (below), so have to clear the extra memory part
      // to zeroes now.
      ...
      // Remember, for non-power-of-two raw files, this data just gets
      // overwritten later anyway, and we only write it in the first place just
      // because the same code does this for power-of-two. But we need to at
      // least move the pointer along so we don't overwrite anything we
      // shouldn't. Adctually, because non-power-of-two raw files are only
      // allowed to be single-cycle, this isn't strictly necessary, but let's do
      // it for completeness and in case I ever change that rule.
      initialBandWritePos =
          &initialBand->dataAccessAddress[(initialBandCycleSizeNoDuplicates +
                                           WAVETABLE_NUM_DUPLICATE_SAMPLES_AT_END_OF_CYCLE) *
                                          (cycleIndex + 1)];
    }

    // Or if it *was* a power-of-two size, we can just do an easier FFT. (And
    // also further down, we won't have to re-render time-domain data for this
    // initial band.)
    else {

      // Write the duplicate values for this initial band - do this now, since
      // we already have the final, useable time-domain data for it.
      for (int i = 0; i < WAVETABLE_NUM_DUPLICATE_SAMPLES_AT_END_OF_CYCLE; i++) {
        *(initialBandWritePos++) = *(nativeBandCycleStartPos++);
      }

      // In fact, if there are no further bands (usually won't happen unless out
      // of RAM), we can get out as there's nothing more to do.
      if (bands.getNumElements() <= 1) { continue; }

      // Perform the FFT, to frequency domain
      ne10_fft_r2c_1d_int32_neon(frequencyDomainData,
                                 currentCycleInt32,
                                 fftCFGForInitialBand,
                                 false);
    }
    ...
    // Scan the frequency domain of this native cycle, as loaded from the raw
    // file (so not necessary power-of-two size), looking at harmonics
    for (int i = 1; i <= (rawFileCycleSize >> 1); i++) {
      int32_t thisValue = (fastPythag(frequencyDomainData[i].r, frequencyDomainData[i].i) >> 6) *
                          i; // Slope the high frequencies up, 6dB per octave, for fair
                             // comparison.
      ...
      // If this harmonic's magnitude is still not too much quieter than the
      // biggest one, we'd better take note that it exists.
      if (thisValue >= (biggestValue >> 5)) { highestSignificantHarmonicIndex = i; }
    }

    WaveTableBand* band = initialBand;
    int            b    = 0;

    // If there's no harmonic content high-enough frequency to warrant this band
    // existing...
    if (SHOULD_DISCARD_WAVETABLE_DATA_WITH_INSUFFICIENT_HF_CONTENT &&
        highestSignificantHarmonicIndex <=
            (rawFileCycleSize >> (1 + NUM_OCTAVES_BETWEEN_WAVETABLE_BANDS))) {

      // If haven't "started" this band yet, update record to show it starts
      // from this cycle - which it might because the next cycle might be
      // needed, and we have a rule that we start one cycle early.
      if (!(startedBandsYet & 1)) { band->fromCycleNumber = cycleIndex; }
    }

    // Or if there *is* enough high-freq content, take note that it's all still
    // happening.
    else {
      startedBandsYet |= 1;
    ...
    // If raw file wasn't power-of-two cycle size, then even this initial band
    // needs the same treatment as the higher ones - we have to render its
    // time-domain data by doing an FFT from the freq-domain.
    if (!rawFileCycleSizeIsAPowerOfTwo) { goto transformBandToTimeDomain; }

    // Otherwise, if we're still here, we'll only need to render time-domain
    // data for bands other than the initial one.

    // For each band "higher" up (smaller cycle; less high harmonics), do all
    // the stuff for it - for this cycle.
    for (b = 1; b < bands.getNumElements(); b++) {

      {
        band = (WaveTableBand*)bands.getElementAddress(b);

        // If we've already chopped off the previous highest significant
        // harmonic, search for a new one.
        if (highestSignificantHarmonicIndex > (band->cycleSizeNoDuplicates >> 1)) {
          ...
          for (int i = (band->cycleSizeNoDuplicates >> 1); i >= 1; i--) {
            ...
            int32_t thisValue   = (pythagValue >> 6) * i; // Slope the high frequencies up, 6dB
                                                          // per octave, for fair comparison.

            // If this harmonic's magnitude is still not too much quieter than
            // the biggest one, we'd better take note that it exists.
        ...
        // If there's no harmonic content high-enough frequency to warrant this
        // band existing...
        if (SHOULD_DISCARD_WAVETABLE_DATA_WITH_INSUFFICIENT_HF_CONTENT &&
            highestSignificantHarmonicIndex <=
          ...
          // from this cycle - which it might because the next cycle might be
          // needed, and we have a rule that we start one cycle early.
          if (!(startedBandsYet & (1 << b))) { band->fromCycleNumber = cycleIndex; }
        }
        ...
        // The highest frequency we can represent (the Nyquist freq) is about to
        // lose all trace of its imaginary component (fun fact - that's just
        // what happens)
        // - so cheat a little and put that into the real component.
        ne10_fft_cpx_int32_t* nyquistFreq =
            &frequencyDomainData[(band->cycleSizeNoDuplicates >> 1)];
    ...
    transformBandToTimeDomain:
      ne10_fft_r2c_cfg_int32_t fftCFGThisBand =
          FFTConfigManager::getConfig(band->cycleSizeMagnitude); // Just gets the config, doesn't do
                                                                 // the FFT.
      ...
      // Do the FFT, putting the output, time-domain data back in the temp
      // currentCycleInt32 buffer.
      ne10_fft_c2r_1d_int32_neon(currentCycleInt32, frequencyDomainData, fftCFGThisBand, false);

      int16_t* __restrict__ destination =
          &band->dataAccessAddress[(band->cycleSizeNoDuplicates +
                                    WAVETABLE_NUM_DUPLICATE_SAMPLES_AT_END_OF_CYCLE) *
                                   cycleIndex];

      // Copy 32-bit time domain data to final 16-bit destination
      for (int i = 0; i < band->cycleSizeNoDuplicates; i++) {
        destination[i] = signed_saturate(
            currentCycleInt32[i] >> (16 - MAGNITUDE_REDUCTION_FOR_FFT + initialBandCycleMagnitude),
  ...
  // Dispose of bands that didn't end up getting used, or such portions of their
  // memory.
  for (int b = bands.getNumElements() - 1; b >= 0;
    ...
    // Otherwise, can we shorten the memory?
      ...
      if (band->fromCycleNumber) {
        ...
        uint32_t amountShortened = generalMemoryAllocator.shortenLeft(
            band->data,
            idealAmountToShorten,
            sizeof(WaveTableBandData)); // Tell it to move the WaveTableBandData
                                        // "header" forward
...
WaveTable::doRenderingLoopSingleCycle(int32_t* __restrict__ thisSample,
                                      int32_t const* bufferEnd,
...
#define rshiftAmount \
  ((32 + INTERPOLATION_MAX_NUM_SAMPLES_MAGNITUDE) - 16 - numBitsInWindowedSyncTableSize + 1)

    uint32_t rshifted = ((uint32_t)-phase) >>
                        (rshiftAmount - bandCycleSizeMagnitude); // Warning - rshiftAmount is 13, so
                                                                 // bandCycleSizeMagnitude better
                                                                 // not be bigger than that!
    ...
    int windowedSincTableLineOffsetBytes =
        ...
        (32 + INTERPOLATION_MAX_NUM_SAMPLES_MAGNITUDE - numBitsInWindowedSyncTableSize - 5 -
         bandCycleSizeMagnitude); // The -5 is for 32 bytes (16 samples) per
                                  // line in the windowed sinc table.
    ...
    /*
    int16x8x4_t windowedSincReadValues = vld4q_s16(sincKernelReadPos); //
    Insanely, I could not get this to work. Tried reading 2 values, too. I just
    get some noise from final output.

    for (int i = 0; i < (INTERPOLATION_MAX_NUM_SAMPLES >> 3); i++) {
        int16x8_t difference = vsubq_s16(windowedSincReadValues.val[i + 2],
    windowedSincReadValues.val[i]); int16x8_t multipliedDifference =
    vqdmulhq_n_s16(difference, strength2); kernelVector[i] =
    vaddq_s16(windowedSincReadValues.val[i], multipliedDifference);
    }
    */
    ...
    for (int i = 0; i < (INTERPOLATION_MAX_NUM_SAMPLES >> 3); i++) {
      ...
      if (i == 0)
        multiplied =
            vmull_s16(vget_low_s16(kernelVector[i]), vget_low_s16(interpolationBuffer.val[i]));
      else
        multiplied = vmlal_s16(multiplied,
                               vget_low_s16(kernelVector[i]),
                               vget_low_s16(interpolationBuffer.val[i]));

      multiplied = vmlal_s16(multiplied,
                             vget_high_s16(kernelVector[i]),
                             vget_high_s16(interpolationBuffer.val[i]));
    }
...
__attribute__((optimize("unroll-loops"))) void
WaveTable::doRenderingLoop(int32_t* __restrict__ thisSample,
                           int32_t const* bufferEnd,
                           int            firstCycleNumber,
                           WaveTableBand* __restrict__ bandHere,
                           uint32_t phase,
                           uint32_t phaseIncrement,
                           uint32_t crossCycleStrength2,
                           int32_t  crossCycleStrength2Increment,
                           const int16_t* __restrict__ kernel) {
  ...
  do {
    ...
    // Grab the actual waveform data from memory, for both cycles that we need
    // for this sample
    ...
    */
    ...
    for (int c = 0; c < 2; c++) {
      ...
      for (int i = 0; i < (INTERPOLATION_MAX_NUM_SAMPLES >> 3); i++) {
        ...
        if (i == 0)
          multiplied =
              vmull_s16(vget_low_s16(kernelVector[i]), vget_low_s16(interpolationBuffer[c].val[i]));
        else
          multiplied = vmlal_s16(multiplied,
                                 vget_low_s16(kernelVector[i]),
        ...
        multiplied = vmlal_s16(multiplied,
                               vget_high_s16(kernelVector[i]),
                               vget_high_s16(interpolationBuffer[c].val[i]));
      }

      twosies[c] = vadd_s32(vget_high_s32(multiplied), vget_low_s32(multiplied));
    }

    // We now have one value for each cycle, so linearly interpolate between
    // those.
    ...
    int32_t waveTableFinalValue = multiply_accumulate_32x32_rshift32_rounded(
        value1 >> 1,
        difference,
        crossCycleStrength2 >> 1); // Have to make value1 a magnitude smaller, because the
                                   // difference is getting a magnitude smaller as a multiplication
                                   // like this always does.
...
uint32_t WaveTable::render(int32_t* __restrict__ outputBuffer,
                           int      numSamples,
                           uint32_t phaseIncrement,
                           uint32_t phase,
                           bool     doOscSync,
                           uint32_t resetterPhaseThisCycle,
  ...
  if (numCycles > 1) {
    ...
    // Haven't yet investigated why, but we do need to use the "rounded"
    // multiply functions here, otherwise get a click when moving to wav pos 0
    // if numSamples is 32 (when testing performance).
    int32_t waveIndexScaled = multiply_32x32_rshift32_rounded(waveIndexMultiplier, waveIndex);
    int32_t waveIndexIncrementScaled =
        multiply_32x32_rshift32_rounded(waveIndexMultiplier, waveIndexIncrement);

    int lshiftAmountToGetCrossCycleStrength =
        32 + numCycleTransitionsNextPowerOf2Magnitude -
        NUM_BITS_IN_WAVE_INDEX_SCALED_INPUT; // Always positive.
    int32_t crossCycleStrength2Increment = waveIndexIncrementScaled
                                           << lshiftAmountToGetCrossCycleStrength;

  startRenderingACycle:
    int numSamplesThisCycle = numSamplesLeftToDo;

    int firstCycleNumber = waveIndexScaled >> (NUM_BITS_IN_WAVE_INDEX_SCALED_INPUT -
    ...
    // See if this rendering window will span multiple cycles as waveIndex is
    // incremented?
    if (firstCycleNumber != firstCycleNumberAfterAllIncrements) {
      ...
      int numIncrementsWeCanDoNow = (uint32_t)waveIndexDistanceUntilFinalBigValueStillInThisCycle /
                                    (uint32_t)waveIndexIncrementScaledAbs;
      numSamplesThisCycle =
          numIncrementsWeCanDoNow + 1; // +1 because we can only have to increment *between* the
                                       // samples we're rendering. If can only do 1 increment, we
                                       // can still render 2 samples.
    ...
    // If this band doesn't have data for either of the two cycle indexes we'll
    // be interpolating between...
    while (bandHere->fromCycleNumber > firstCycleNumber ||
           bandHere->toCycleNumber <= firstCycleNumber + 1) {

      bHere++;
      if (bHere == bands.getNumElements()) { goto doneRenderingACycle; }
...

    {
      ...
      if (doOscSync) {
        ...
        RENDER_OSC_SYNC(RENDER_WAVETABLE_LOOP,
                        0,
  ...
  // Or, if we're actually a single-cycle waveform, with only one cycle...
    ...
    if (doOscSync) {
      ...
      RENDER_OSC_SYNC(RENDER_SINGLE_CYCLE_WAVEFORM_LOOP,
                      0,
                      0,
...
#endif
      generalMemoryAllocator.putStealableInQueue(band->data,
