...
protected:
  ...
  Instrument* currentInstrument; // Although this is only needed by the child class
                                 // LoadInstrumentPresetUI, we cut a corner by including
                                 // it here so our functions can set it to NULL, which
                                 // is needed. This is the Instrument we're currently
                                 // scrolled onto. Might not be actually loaded (yet)?
                                 // We do need this, separate from the current FileItem,
                                 // because if user moves onto a folder, the
                                 // currentInstrument needs to remain the same.
};
