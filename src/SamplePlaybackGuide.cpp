...
uint64_t SamplePlaybackGuide::getSyncedNumSamplesIn() {
  ...
  uint32_t currentTickWithinSample =
      playbackHandler.getCurrentInternalTickCount(&timeSinceLastInternalTick) -
  ...
  if (timeSinceLastInternalTick >= timePerInternalTick)
    timeSinceLastInternalTick =
        timePerInternalTick - 1; // Ensure it doesn't get bigger.
...
int32_t SamplePlaybackGuide::adjustPitchToCorrectDriftFromSync(VoiceSample* voiceSample,
  ...
  // Not if not following external clock source, or clusters not set up yet (in
  // the case of a very-late-start), there's no need
  if (!(playbackHandler.playbackState & PLAYBACK_CLOCK_EXTERNAL_ACTIVE) ||
      !voiceSample->clusters[0])
