...
void MenuItemUnpatchedParam::readCurrentValue() {
  soundEditor.currentValue =
      (((int64_t)soundEditor.currentParamManager->getUnpatchedParamSet()->getValue(getP()) +
        2147483648) *
           50 +
       2147483648) >>
      32;
...
ModelStackWithAutoParam* MenuItemUnpatchedParam::getModelStack(void* memory) {
  ...
  int                            p       = getP();
  return modelStack->addParam(summary->paramCollection,
                              summary,
                              p,
...
void MenuItemUnpatchedParam::writeCurrentValue() {
  ...
  modelStackWithParam->autoParam->setCurrentValueInResponseToUserInput(getFinalValue(),
                                                                       modelStackWithParam);
}
...
// ---------------------------------------
void MenuItemUnpatchedParamPan::drawValue() { // TODO: should really combine
                                              // this with the "patched" version
  uint8_t drawDot = 255;                      // soundEditor.doesParamHaveAnyCables(getP()) ? 3 :
                                              // 255;
  char buffer[5];
...
void MenuItemUnpatchedParamPan::readCurrentValue() {
  soundEditor.currentValue =
      ((int64_t)soundEditor.currentParamManager->getUnpatchedParamSet()->getValue(getP()) * 64 +
       2147483648) >>
      32;
}
