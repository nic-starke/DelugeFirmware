...
void BidirectionalLinkedList::addToEnd(BidirectionalLinkedListNode* node) {
  ...
  *endNode.prevPointer = node;
  ...
  // Have deactivated this, because some of the lists will have up to 2000
  // elements in them on boot, and searching through all of these causes voice
  // culls
  // test();
#endif
}
...
void BidirectionalLinkedList::test() {
  ...
  if (first == NULL) { numericDriver.freezeWithError("E005"); }
  ...
  while (true) {
    if (thisNode->prevPointer != prevPointer) { numericDriver.freezeWithError("E006"); }

    if (thisNode == &endNode) break; // We got to the end

    // Check the node references its list correctly
    if (thisNode->list != this) { numericDriver.freezeWithError("E007"); }

    count++;

    // Check we're not spiralling around forever
    if (count > 2048) { numericDriver.freezeWithError("E008"); }
...
// It's intended that you may call this function even if you're not sure whether
// the node is in a list or not
...
void BidirectionalLinkedListNode::insertOtherNodeBefore(BidirectionalLinkedListNode* otherNode) {
  ...
  // If we're not already in a list, that means we also don't have a valid
  // prevPointer, so everything's about to break. This happened!
...
// Ok this is a little bit dangerous - you'd better make damn sure list is set
// before calling this!
