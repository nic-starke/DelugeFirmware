...
void Drum::drumWontBeRenderedForAWhile() { // This is virtual, and gets extended
                                           // in SoundDrum
  unassignAllVoices();
}
...
void Drum::expressionEventPossiblyToRecord(ModelStackWithTimelineCounter* modelStack,
  ...
  // Ok we have to first combine the expression inputs that the user might have
  // sent at both MPE/polyphonic/finger level, *and* at channel/instrument
  // level. Yes, we combine these here at the input before the data gets
  // recorded or sounded, because unlike for Instruments, we're a Drum, and all
  // we have is the NoteRow level to store this stuff.
  lastExpressionInputsReceived[level][whichExpressionimension] = newValue >> 8; // Store value
  int32_t combinedValue =
      (int32_t)newValue +
      ((int32_t)lastExpressionInputsReceived[!level][whichExpressionimension] << 8);
