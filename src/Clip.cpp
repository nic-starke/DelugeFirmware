...
Clip::Clip(int newType) : type(newType) {
  ...
  activeIfNoSolo       = true;
  wasActiveBefore      = false; // Want to set this default in case a Clip was
                                // created during playback
...
// This is more exhaustive than copyBasicsFrom(), and is designed to be used
// *between* different Clip types, just for the things which Clips have in
// common
...
#if HAVE_SEQUENCE_STEP_CONTROL
  if (currentlyPlayingReversed) { actualPos = loopLength - actualPos; }
#endif
...
#if HAVE_SEQUENCE_STEP_CONTROL
  if (currentlyPlayingReversed) { posToReturn = loopLength - posToReturn; }
#endif
  return posToReturn;
}
...
// Note: it's now the caller's job to increment currentPos before calling this!
// But we check here whether it's looped and needs setting back to "0". We may
// change the TimelineCounter in the modelStack if new Clip got created.
void Clip::processCurrentPos(ModelStackWithTimelineCounter* modelStack, uint32_t ticksSinceLast) {

  // Firstly, a bit of stuff that has to be dealt with ideally before calling
  // posReachedEnd(), and definitely before we think about pingponging while in
  // reverse. The consequence of not doing this is only apparent in one special
  // case, where a NoteRow contains just one tail-less sound (e.g. kick sample)
  // right on beat "0". Pingponging breaks in such a case. Reason being, when
  // coming back from right to left, lastProcessedPos keeps going into the
  // negative, because there are no events, and eventually reaches -loopLength
  // when we get here. We need to have our
  // wrapping-negative-lastProcessedPos-to-positive code here at the start, so
  // it can be correctly wrapped up to exactly 0 - because various places below
  // check if lastProcessedPos == 0.
  if (currentlyPlayingReversed) {
    // So yeah, if actually got left of zero, it's time to loop/wrap. Normally
    // this wouldn't happen if pingponging because direction changes right when
    // we hit zero. Except the case discussed above, where actually our getting
    // left of 0 is something that happens when we start moving in reverse,
    // regardless of whether it happened as part of a pingpong.
    if (lastProcessedPos < 0) {

      // But in some cases, we might have got here and still need to pingpong
      // (if length changed or something?), so go check that. Actually wait,
      // don't, because doing one normal pingpong from forward to reverse will
      // put us in this position, and we don't want to do a second pingpong
      // right after, or else there's effectively no proper pingpong! if
      // (sequenceDirectionMode == SEQUENCE_DIRECTION_PINGPONG) goto
      // doPingpongReversed;
  ...
  // If we've reached the end, need to call posReachedEnd() - but that just
  // deals with stuff like extending and appending Clips. It doesn't deal with
  // the wrapping and pingponging stuff that this function is mostly concerned
  // with.
  int endPos = currentlyPlayingReversed ? 0 : loopLength;
  if (lastProcessedPos == endPos && repeatCount >= 0) {
    posReachedEnd(modelStack); // This may alter length, changing what happens in the
                               // below if statements, which is why we can't combine this.
  ...
  playingForwardNow:
    ...
    if (ticksTilEnd <= 0) { // Yes, note it might not always arrive directly at the end. When
      ...
      ticksTilEnd += loopLength; // Yes, we might not be right at the loop point
                                 // - see comment above.
    }
  }
  ...
  if (paramManager.mightContainAutomation()) {
    ...
    bool mayInterpolate =
        (output->type != INSTRUMENT_TYPE_MIDI_OUT && output->type != INSTRUMENT_TYPE_CV);
...
// Virtual function - gets extended by both InstrumentClip and AudioClip. They
// both invoke this, and are also required to call setPosForParamManagers() or
// do something equivalent - and that "something equivalent" allows
// InstrumentClip to save time by iterating through NoteRows only once.
void Clip::setPos(ModelStackWithTimelineCounter* modelStack,
                  int32_t                        newPos,
                  bool                           useActualPosForParamManagers) {

  // It's a bit complex and maybe not the best way for stuff to work, but newPos
  // may be negative because Session::armClipToStartOrSoloUsingQuantization()
  // subtracts playbackHandler.getNumSwungTicksInSinceLastActionedSwungTick()
  // from it. And this bit of old code here deals with that.
  if (newPos < 0) {
    newPos += loopLength;
    repeatCount = -1;
  }
  ...
  currentlyPlayingReversed =
      (sequenceDirectionMode == SEQUENCE_DIRECTION_REVERSE // Syncing pingponging with repeatCount
                                                           // is particularly important for when
                                                           // resuming after
  ...
  if (currentlyPlayingReversed) {
    if (newPos)
      ...
      repeatCount--; // Cos it's going to get incremented as a side effect of
                     // reversed clips starting at pos 0 after which they'll
                     // immediately wrap
  }

  lastProcessedPos = newPos;

  expectEvent(); // Remember, this is a virtual function call - extended in
                 // InstrumentClip
}
...
bool Clip::opportunityToBeginSessionLinearRecording(ModelStackWithTimelineCounter* modelStack,
  ...
  *newOutputCreated = false;
  ...
  if (playbackHandler.recording && wantsToBeginLinearRecording(modelStack->song)) {
    ...
    Action* action =
        actionLogger.getNewAction(ACTION_RECORD,
                                  true); // Allow addition to existing Action - one might
                                         // have already been created because note recorded
                                         // slightly early just before end of count-in

    if (isPendingOverdub) {
      *newOutputCreated = cloneOutput(modelStack);

      if (action) {
        action->recordClipExistenceChange(modelStack->song,
                                          &modelStack->song->sessionClips,
                                          this,
                                          CREATE);
        ...
        if (*newOutputCreated) {
          ...
          if (consMemory) {
            ConsequenceOutputExistence* cons =
                new (consMemory) ConsequenceOutputExistence(output, CREATE);
      ...
      if (action) {
        ...
        if (consMemory) {
          ConsequenceClipBeginLinearRecord* cons =
              new (consMemory) ConsequenceClipBeginLinearRecord(this);
          action->addConsequence(cons);
        }
      }
    }
...
int Clip::resumeOriginalClipFromThisClone(ModelStackWithTimelineCounter* modelStackOriginal,
  ...
  originalClip->activeIfNoSolo = true; // Must set this before calling setPos, otherwise,
                                       // ParamManagers won't know to expectEvent()

  originalClip->setPos(modelStackOriginal,
                       lastProcessedPos,
                       true); // Deliberately leave lastProcessedPos as a pos potentially far
                              // beyond the length of the original Clip. setPos() will see this
                              // and wrap the position itself - including for individual NoteRows
                              // with independent length.
...
void Clip::beginInstance(Song* song, int32_t arrangementRecordPos) {
  ...
  // If there's a previous instance, make sure it doesn't cut into the new one.
  // This is only actually necessary if doing a "late start"
  if (clipInstanceI) {
    ...
    if (maxLength <= 0) { // Shouldn't normally go below 0...
      song->deletingClipInstanceForClip(output,
                                        clipInstance->clip,
                                        NULL,
                                        false); // Calls audio routine...
      clipInstanceI--;
...
void Clip::endInstance(int32_t arrangementRecordPos, bool evenIfOtherClip) {
  ...
  if (clipInstanceI >= 0) {
    ...
    // evenIfOtherClip is an emergency hung over New Year's Day 2019 fix to the
    // problem where this could get called on the wrong Clip (same Instrument
    // though) because getClipToRecordTo() returns wrong Clip because activeClip
    // has already been changed on Instrument because other Clip became it
    // already in same launch.
...
// Returns error code
// This whole function is virtual and overridden in (and sometimes called from)
// InstrumentClip, so don't worry about MIDI / CV cases - they're dealt with
// there
...
void Clip::readTagFromFile(char const* tagName, Song* song, int32_t* readAutomationUpToPos) {
  ...
  if (!strcmp(tagName, "isPlaying")) {
    ...
    *readAutomationUpToPos = loopLength;
    ...
    // This is naughty - inputTickScaleClip shouldn't be accessed directly. But
    // for simplicity, I'm using it to hold this Clip for now, and then in
    // song.cpp this gets made right in a moment...
...
void Clip::prepareForDestruction(ModelStackWithTimelineCounter* modelStack,
  ...
  Output* oldOutput = output; // There won't be an Instrument if the song is being deleted
                              // because it wasn't completely loaded
  ...
  if (output) {
    ...
    if (isActiveOnOutput() && playbackHandler.isEitherClockActive())
      expectNoFurtherTicks(modelStack->song); // Still necessary? Actually maybe... I can see
                                              // that this would at least cause an AudioClip to
                                              // abortRecording()...

    detachFromOutput(modelStack, false);
  }

  if (oldOutput) { // One case where there won't be an Output is if the song is
                   // being deleted because it wasn't able to be completely
                   // loaded
...
void Clip::posReachedEnd(ModelStackWithTimelineCounter* modelStack) {
  // If linear recording (which means it must be a loop / session playback if we
  // reached the end)
  if (getCurrentlyRecordingLinearly()) {

    // If they exited recording mode (as in the illuminated RECORD button),
    // don't auto extend
    ...
    // Otherwise, do auto extend
      ...
      // For InstrumentClips only, we record and make undoable the length-change
      // here. For AudioClips, on the other hand, it happens in one go at the
      // end of the recording - because for those, if recording is aborted
      // part-way, the whole Clip is deleted. But, don't do this if this Clips
      // still would get deleted as an "abandoned overdub" (meaning it has no
      // notes), cos if that happened, we definitely don't want to have a
      ...
      if (true || type != CLIP_TYPE_AUDIO) {
        ...
        if (action) { action->recordClipLengthChange(this, oldLength); }
      }
    }
  }
}

// Caller must call resumePlayback on this Clip, unless you have a good reason
// not to.
...
void Clip::drawUndefinedArea(int32_t       xScroll,
                             uint32_t      xZoom,
  ...
  if (greyStart < 0)
    greyStart = 0; // This actually happened in a song of Marek's, due to
                   // another bug, but best to check for this
...
void Clip::outputChanged(ModelStackWithTimelineCounter* modelStack, Output* newOutput) {
  ...
  // If we're currently playing and this Clip is active, we need to make it the
  // Instrument's activeClip
  ...
  yesMakeItActive:
    ...
    // In any case, we want the newInstrument to have an activeClip, and if it
    // doesn't yet have one, the supplied Clip makes a perfect candidate
...
int Clip::solicitParamManager(Song*         song,
                              ...
                              Clip*         favourClipForCloningParamManager) {

  // Occasionally, like for AudioClips changing their Output, they will actually
  // have a paramManager already, so everything's fine and we can return
  ...
  if (!paramManager.containsAnyMainParamCollections()) {
    ...
    if (favourClipForCloningParamManager) {
      ...
      // Let's first just see if there already was a *perfect* backed-up one for
      // this *exact* Clip, that we could just have. If so, great, we're done.
      if (song->getBackedUpParamManagerForExactClip((ModControllableAudio*)modControllable,
                                                    this,
                                                    &paramManager)) {
      ...
      paramManager.cloneParamCollectionsFrom(&favourClipForCloningParamManager->paramManager,
                                             false,
                                             true);
      // That might not work if there was insufficient RAM - very unlikely - but
      // we'll still try the other options below
    }
    ...
    if (!paramManager.containsAnyMainParamCollections()) {
      ...
      // Unless I've done something wrong, there *has* to be another Clip if the
      // Output didn't have a backed-up ParamManager. But, just in case
      else {
...
int Clip::beginLinearRecording(ModelStackWithTimelineCounter* modelStack, int buttonPressLatency) {
  if (!getRootUI() || !getRootUI()->toClipMinder()) { modelStack->song->currentClip = this; }
  return NO_ERROR;
}
...
bool Clip::possiblyCloneForArrangementRecording(ModelStackWithTimelineCounter* modelStack) {
  ...
  if (playbackHandler.recording == RECORDING_ARRANGEMENT && playbackHandler.isEitherClockActive() &&
    ...
    if (output->activeClip && output->activeClip->beingRecordedFromClip == this) {
      ...
      // If it can't be found (should be impossible), we'll just get out and
      // leave everything the same, so at least nothing will crash
      ...
      if (type == CLIP_TYPE_AUDIO) {
        ...
        // So, we want to create a bunch of repeats. Often there'll be many at
        // the start which just repeat with untouched params, so that can all be
        // one ClipInstance
        ...
        if (repeatCount >= 1) {
          ...
          // And then we'll need a new ClipInstance for this new instance that
          // we're gonna record some automation on
          clipInstanceI++;
      ...
      int error = clone(modelStack, true); // Puts the cloned Clip into the
                                           // modelStack. Flattens reversing.
      if (error) return false;
      ...
      if (type == CLIP_TYPE_INSTRUMENT) {
        ...
        newClip->increaseLengthWithRepeats(
            modelStack,
            newLength,
      ...
      newClip->activeIfNoSolo = true; // Must set this before calling setPos, otherwise,
                                      // ParamManagers won't know to expectEvent()
