...
CVInstrument::CVInstrument() : NonAudioInstrument(INSTRUMENT_TYPE_CV) {
  ...
  cachedBendRanges[BEND_RANGE_FINGER_LEVEL] =
...
void CVInstrument::polyphonicExpressionEventPostArpeggiator(int      newValue,
...
void CVInstrument::updatePitchBendOutput(bool outputToo) {
  ...
  // If couldn't update bend ranges that way, no worries - we'll keep our cached
  // ones, cos the user probably intended that.

  int32_t totalBendAmount = // (1 << 23) represents one semitone. So full 32-bit
                            // range can be +-256 semitones. This is different
                            // to the equivalent calculation in Voice, which
                            // needs to get things into a number of octaves.
...
bool CVInstrument::setActiveClip(ModelStackWithTimelineCounter* modelStack, int maySendMIDIPGMs) {
  ...
  if (clipChanged) {
    ...
    if (expressionParams) {
      ...
      monophonicPitchBendValue = 0;
      // TODO: grab bend ranges here too - when we've moved them to the
      // ParamManager?
    }

    updatePitchBendOutput(false); // Don't change the CV output voltage right now (we could, but
                                  // this Clip-change might come with a note that's going to sound
                                  // "now" anyway...)
                                  // - but make it so the next note which sounds will have our new
                                  // correct bend value / range.
  }
