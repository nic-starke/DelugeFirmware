...
// Destructs the actual MultiRanges, but doesn't actually deallocate the memory,
// aka calling empty() on the Array - the caller must do this.
...
int32_t Source::getLengthInSamplesAtSystemSampleRate(int note, bool forTimeStretching) {
  ...
  if (range) {
    return ((SampleHolder*)range->getAudioFileHolder())
        ->getLengthInSamplesAtSystemSampleRate(forTimeStretching);
  } else
...
// This function has to give the same result as
// Sound::renderingVoicesInStereo(). The duplication is for optimization.
...
int Source::loadAllSamples(bool mayActuallyReadFiles) {
  for (int e = 0; e < ranges.getNumElements(); e++) {
    ...
    if (!(e & 3))
      AudioEngine::routineWithClusterLoading(); // --------------------------------------
                                                // // 3 works, 7 occasionally
                                                // drops voices - for
                                                // multisampled synths
    if (mayActuallyReadFiles && shouldAbortLoading()) return ERROR_ABORTED_BY_USER;
    ranges.getElement(e)->getAudioFileHolder()->loadFile(sampleControls.reversed,
                                                         false,
...
MultiRange* Source::getOrCreateFirstRange() {
  if (!ranges.getNumElements()) {
    MultiRange* newRange = ranges.insertMultiRange(
        0); // Default option - allowed e.g. for a new Sound where the current
            // process is the Ranges get set up before oscType is switched over
            // to SAMPLE - but this can't happen for WAVETABLE so that's ok
...
// If setting to SAMPLE or WAVETABLE, you must call unassignAllVoices before
// this, because ranges is going to get emptied.
void Source::setOscType(int newType) {
  ...
  possiblyDeleteRanges:
    ...
    doChangeType:
      ...
      getOrCreateFirstRange(); // Ensure there's at least 1. If this returns
                               // NULL and we're in the SoundEditor or
