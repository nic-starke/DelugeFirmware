...
// We keep a separate FIL object here so we can be recording to a file at the
// same time as another file is open for reading. It no longer needs to be in
// this struct
...
bool AudioRecorder::opened() {
  ...
  bool success =
      setupRecordingToFile(inStereo ? AUDIO_INPUT_CHANNEL_STEREO : AUDIO_INPUT_CHANNEL_LEFT,
                           newNumChannels,
...
void AudioRecorder::renderOLED(uint8_t image[][OLED_MAIN_WIDTH_PIXELS]) {
  OLED::drawStringCentred("Recording",
                          15,
                          image[0],
...
bool AudioRecorder::setupRecordingToFile(int newMode, int newNumChannels, int folderID) {
  ...
  recorder =
      AudioEngine::getNewRecorder(newNumChannels, folderID, newMode, INTERNAL_BUTTON_PRESS_LATENCY);
  if (!recorder) {
  ...
  recordingSource = newMode; // This sets recording to begin happening even as
...
bool AudioRecorder::beginOutputRecording() {
  bool success =
      setupRecordingToFile(AUDIO_INPUT_CHANNEL_OUTPUT, 2, AUDIO_RECORDING_FOLDER_RESAMPLE);

  if (success) { IndicatorLEDs::blinkLed(recordLedX, recordLedY, 255, 1); }

  AudioEngine::bypassCulling =
      true; // Not 100% sure if this will help. Leo was getting culled voices
            // right on beginning resampling via an audition pad. But I'd more
            // expect it to happen after the first render-window, which this
...
void AudioRecorder::endRecordingSoon(int buttonLatency) {
  ...
  if (recorder &&
      recorder->status == RECORDER_STATUS_CAPTURING_DATA) { // Make sure we don't call the same
                                                            // thing multiple times - I think
                                                            // there's a few scenarios where
                                                            // this could happen
#if HAVE_OLED
...
void AudioRecorder::slowRoutine() {
  if (recordingSource == AUDIO_INPUT_CHANNEL_OUTPUT) {
    if (recorder->status >= RECORDER_STATUS_COMPLETE) {
      IndicatorLEDs::setLedState(recordLedX,
                                 recordLedY,
  ...
  while (true) {
    ...
    if (recorder->status >= RECORDER_STATUS_COMPLETE || recorder->hadCardError) {
      ...
      if (recorder->status == RECORDER_STATUS_ABORTED || recorder->hadCardError) {
        ...
        soundEditor.currentMultiRange->getAudioFileHolder()->filePath.set(
...
int AudioRecorder::buttonAction(int x, int y, bool on, bool inCardRoutine) {
  ...
  // We don't actually wrap up recording here, because this could be in fact
  // called from the SD writing routines as they wait - that'd be a tangle.
