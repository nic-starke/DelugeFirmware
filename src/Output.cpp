...
void Output::pickAnActiveClipIfPossible(ModelStack* modelStack,
  ...
  if (!activeClip) {
    ...
    if (searchSessionClipsIfNeeded) {
      // If still here, might need to search session Clips (we've already
      // effectively searched arrangement-only Clips)
    ...
    if (setupWithoutActiveClipIfNeeded) { setupWithoutActiveClip(modelStack); }
  }
}
...
void Output::pickAnActiveClipForArrangementPos(ModelStack* modelStack,
  ...
  // First, see if there's an earlier-starting ClipInstance that's still going
  // at this pos
  ...
  // If not, see if there's a later one we can just have
    ...
    // If still here, we didn't find anything, so try the regular
    // pickAnActiveClipIfPossible(), which will search earlier ClipInstances
    // too, then session Clips, and failing that, a backedUpParamManager
    if (arrangementPos != 0) { pickAnActiveClipIfPossible(modelStack, true, maySendMIDIPGMs); }
  }
}
...
ParamManager* Output::getParamManager(Song* song) {
  ...
  if (activeClip) {
    ...
    ParamManager* paramManager =
        song->getBackedUpParamManagerPreferablyWithClip((ModControllableAudio*)toModControllable(),
                                                        NULL);
...
// Most classes inheriting from Output actually override this with their own
// version...
      ...
      if (numElementsToAllocateFor <= 0) {
        ...
        if (charsRemaining) {
          ...
          clipInstances.ensureEnoughSpaceAllocated(
              numElementsToAllocateFor); // If it returns false... oh well.
                                         // We'll fail later
        }
      }
...
int Output::possiblyBeginArrangementRecording(Song* song, int newPos) {
  ...
  if (!newClip) { return ERROR_INSUFFICIENT_RAM; }

  // We can only insert the ClipInstance after the call to
  // createNewClipForArrangementRecording(), because that ends up calling
  // Song::getClipWithOutput(), which searches through ClipInstances for this
  // Output!
  ...
  // Set the ClipInstance's length to just 1, which kinda is how long it
  // logically "is" at this point in time before recording has started. This
  // leaves space directly after it, which the user might choose to suddenly to
  // create another ClipInstance in. Oh and also, this sets the ClipInstance's
  // Clip to the new Clip we created, above
...
void Output::endAnyArrangementRecording(Song*    song,
                                        int32_t  actualEndPosInternalTicks,
  ...
  if (recordingInArrangement) {
    ...
    if (ALPHA_OR_BETA_VERSION && clipInstance->clip != activeClip)
      numericDriver.freezeWithError("E262"); // Michael B got, in 3.2.0-alpha10. Possibly a general
    ...
    // But if that's taken us back to or before the clipInstance start, go
    // forward a square
    ...
    // If we didn't have to do any extra adjusting above, and we rounded
    // backwards, possibly give another later option
    if (!hadToShuffleOver && actualEndPosInternalTicks > quantizedEndPos) {
      ...
      if (nextClipInstance && nextClipInstance->pos < alternativeLaterEndPos) {
        ...
        // And if that's ended up just being the same as our main option, ditch
        // it
  ...
  skipThat : {}
    activeClip->quantizeLengthForArrangementRecording(modelStack,
                                                      lengthSoFarInternalTicks,
                                                      timeRemainder,
                                                      quantizedEndPos - clipInstance->pos,
                                                      alternativeLongerLength);
...
void Output::endArrangementPlayback(Song* song, int32_t actualEndPos, uint32_t timeRemainder) {
  ...
  if (!recordingInArrangement) {
    ...
    if (clipInstance && clipInstance->clip) {
      ...
      if (endPos > actualEndPos) { clipInstance->clip->expectNoFurtherTicks(song); }
    }
  }
