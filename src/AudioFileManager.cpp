...
DWORD get_fat_from_fs(/* 0xFFFFFFFF:Disk error, 1:Internal
...
char const* const audioRecordingFolderNames[] = {"SAMPLES/CLIPS",
                                                 "SAMPLES/RECORD",
                                                 "SAMPLES/RESAMPLE"};
...
void AudioFileManager::init() {
  ...
  void* temp =
      generalMemoryAllocator.alloc(clusterSizeAtBoot + CACHE_LINE_SIZE * 2, NULL, false, false);
...
void AudioFileManager::cardReinserted() {
  ...
  if (fileSystemStuff.fileSystem.csize * 512 > clusterSize) {
    ...
    // But, if it's still not as big as it was when we booted up, that's still
    // manageable
  ...
  // If cluster size decreased, we have to stop all current samples from ever
  // sounding again. Pretty big trouble really...
  ...
  clusterSizeChangedButItsOk:
    ...
    for (int e = 0; e < audioFiles.getNumElements(); e++) {
      ...
      // If AudioFile isn't used currently, take this opportunity to remove it
      // from memory
  ...
  // Or if cluster size stayed the same...
    ...
    for (int e = 0; e < audioFiles.getNumElements(); e++) {
      ...
      // If Sample isn't used currently, take this opportunity to remove it from
      // memory
      if (!thisAudioFile->numReasonsToBeLoaded) {
        deleteUnusedAudioFileFromMemory(thisAudioFile, e);
        e--;
      ...
      // Or if it is still used by someone...
        ...
        if (thisAudioFile->type == AUDIO_FILE_TYPE_SAMPLE) {
          ...
          // If address of first sector remained unchanged, we can be sure
          // enough that the file hasn't been changed
          ...
          // Or if we're still here, the file's fine - who knows, maybe it's
          // even fine again after it wasn't for a while (e.g. if the user
          // temporarily had a different card inserted)
  ...
  MIDIDeviceManager::readDevicesFromFile(); // Hopefully we can do this now.
                                            // It'll only happen if it wasn't
                                            // able to do it before.
}

// Call this after deleting the current (or in other words previous) Song from
// memory - meaning there won't be any further reason we'd ever move these temp
// samples into the permanent sample folder, meaning we don't want them in
// memory listed with their would-be real permanent filenames. Also, we won't be
// needing to play them back again. You must not call this during the card or
// audio routines.
void AudioFileManager::deleteAnyTempRecordedSamplesFromMemory() {

  // Also though, in case any of these Samples were still being recorded before
  // the Song-delete, we need to make sure SampleRecorder::cardRoutine() gets
  // called first to "detach" the Sample from the recorder. So, do this:
  ...
  for (int e = 0; e < audioFiles.getNumElements(); e++) {
    ...
    if (audioFile->type == AUDIO_FILE_TYPE_SAMPLE) {
      ...
      if (!((Sample*)audioFile)->tempFilePathForRecording.isEmpty()) {
        ...
        // if (ALPHA_OR_BETA_VERSION && audioFile->numReasons)
        // numericDriver.freezeWithError("E281"); // It definitely shouldn't
        // still have any reasons
        //  No - it could still have a reason - the reason of its
        //  SampleRecorder. Scenario where this happened was: recording
        //  AudioClip (instance) into Arranger when loading a new song, first
        //  causes Arranger playback to switch to Session playback, which causes
        //  finishLinearRecording() on AudioClip, so when song-swap does happen,
        //  the AudioClip no longer has a recorder, so the recorder doesn't
        //  clear stuff, and it's still not quite yet finalized the file, so
        //  still holds the "reason" to the Sample.
        //  TODO: although the Sample doesn't store a pointer to the
        //  SampleRecorder, we could easily search for it - and delete it and
        //  its "reason"?

        highestUsedAudioRecordingNumberNeedsReChecking[AUDIO_RECORDING_FOLDER_CLIPS] =
            true; // We know Sample belonged to an AudioClip originally
                  // because only those ones can be TEMP
        highestUsedAudioRecordingNumber[AUDIO_RECORDING_FOLDER_CLIPS] =
            -1; // We may have deleted several, so do make sure we go and
                // re-check from 0
...
// Oi, don't even think about modifying this to take a Sample* pointer - cos the
// whole Sample could get deleted during the card access.
int AudioFileManager::getUnusedAudioRecordingFilePath(String*   filePath,
  ...
  if (highestUsedAudioRecordingNumberNeedsReChecking[folderID]) {
    ...
    if (result == FR_OK) {
      ...
      while (true) {
        ...
        if (__builtin_expect((*(uint32_t*)staticFNO.altname & 0x00FFFFFF) == 0x00434552,
                             1)) {                               // "REC"
  ...
  if (doingTempFolder) {
    error = tempFilePathForRecording->concatenate(
        &filePath->get()[strlen(audioRecordingFolderNames[folderID])]);
    if (error) return error;
  }
...
bool AudioFileManager::tryToDeleteAudioFileFromMemoryIfItExists(char const* filePath) {
  ...
  for (int t = 0; t < 2; t++) { // Got to do this twice, just in case there's a
                                // Sample and a WaveTable.
    ...
    if (audioFile->numReasonsToBeLoaded)
      return false; // Alert - not only is it in memory, but it also can't be
                    // deleted
...
void AudioFileManager::deleteUnusedAudioFileFromMemory(AudioFile* audioFile, int i) {
  ...
  // audioFile->remove(); // Remove from the unused AudioFiles list, where this
  // already must have been. Actually no, the destructor does this anyway.
  audioFile->~AudioFile();
...
int AudioFileManager::setupAlternateAudioFileDir(String*     newPath,
                                                 char const* rootDir,
...
int AudioFileManager::setupAlternateAudioFilePath(String* newPath,
                                                  int     dirPathLength,
                                                  String* oldPath) {
  int error = newPath->concatenateAtPos(&oldPath->get()[8],
                                        dirPathLength); // The [8] skips us past "SAMPLES/"
  if (error) return error;
...
AudioFile* AudioFileManager::getAudioFileFromFilename(String*      filePath,
                                                      bool         mayReadCard,
                                                      uint8_t*     error,
                                                      FilePointer* suppliedFilePointer,
                                                      int          type,
  ...
  *error = NO_ERROR;
  ...
  // If that basic search by the file's "normal" path already found it, then
  // great.
  if (foundExact) {
  successfullyFoundInMemory:
    ...
    // If correct type...
    if (foundAudioFile->type == type) { return foundAudioFile; }
    ...
    doTryOffset:
      ...
      if (foundAudioFile2->type == type &&
          !strcasecmp(filePath->get(), foundAudioFile2->filePath.get())) {
    ...
    // If here, we didn't find the correct type, but we did find an AudioFile
    // for the correct filePath, just the wrong type.

    // If we want WaveTable but got Sample, we can convert. (Otherwise, we
    // can't.)
    if (type == AUDIO_FILE_TYPE_WAVETABLE) {
      ...
      // And if the user isn't insisting, then some other signs show that we
      // probably don't want to load this as a WaveTable
      if (!makeWaveTableWorkAtAllCosts) {
        ...
        // If this isn't actually a wavetable-specifying file or at least a
        // wavetable-looking length, and the user isn't insisting, then opt not
        // to do it.
    ...
    // Or if we want Sample but got Wavetable, can't convert, so we'll have to
    // load from file after all. Reset filePath if needed (pretty unlikely
    // scenario).
    else {
      if (!backedUpFilePath.isEmpty()) { filePath->set(&backedUpFilePath); }
    }
  }
  ...
  // Or, if that didn't find the audio file in memory...
    ...
    // If we're loading a preset (not a Song, and not just browsing audio
    // files), we should search in memory for the alternate path
    if (alternateLoadDirStatus == ALTERNATE_LOAD_DIR_MIGHT_EXIST ||
      ...
      if (thingTypeBeingLoaded != THING_TYPE_SONG) {
        ...
        *error               = searchPath.concatenate(fileName);
        ...
        if (foundExact) {
          // Tiny bit cheeky, but we're going to update the file path actually
          // stored in the AudioFile to reflect this alternate location, which
          // will no longer be considered alternate.
...
tryLoadingFromCard:
  ...
  */
  ...
  // Otherwise go on the filePath
    ...
    // If we know the alternate load directory actually exists, then we should
    // try that first, cos there's a high chance the file is in there
    ...
    tryAlternateDoesExist:
      ...
      // We'll first try the long file name, which contains all the folder names
      // from the original path. This is how collect-media saves look - for
      // Songs. But, if that original path didn't begin with "SAMPLES/", we
      // can't do that.
      if (memcasecmp(filePath->get(), "SAMPLES/", 8)) goto tryNextAlternate;

      *error = setupAlternateAudioFilePath(&proposedFileName, 0, filePath);
      if (*error)
        return NULL; // This is to generate just the name of the file - not an
                     // entire path with folders - despite the function being
                     // called ...Path.
    ...
    tryAnAlternate:
      ...
      alternateFailed:
        ...
        tryNextAlternate:
          // Next up we'll try looking for just the filename that the original
          // file had, without any added folder names. This allows users to copy
          // files into folders for instruments more easily, and have them load.
      ...
      *error = usingAlternateLocation.concatenate(&proposedFileName);
      ...
      if (thingTypeBeingLoaded == THING_TYPE_SYNTH || thingTypeBeingLoaded == THING_TYPE_KIT) {
        // Special rule for loading presets with files in their dedicated
        // "alternate" folder: must update the AudioFile's filePath to point to
        // that alternate location - and then treat them as normal (not
        // alternate).
    ...
    tryRegular:
      ...
      // If that didn't work, try the alternate load directory, if we didn't
      // already and it potentially exists
      if (result != FR_OK) {
  ...
  void* audioFileMemory =
      generalMemoryAllocator.alloc(memorySizeNeeded, NULL, false, true, true); // Stealable!
  if (!audioFileMemory) {
  ramError:
    *error = ERROR_INSUFFICIENT_RAM;
    goto cantLoadFile;
  ...
  if (type == AUDIO_FILE_TYPE_SAMPLE) {
    ...
    audioFile->addReason(); // So it's protected while setting up. Must do this
  ...
  // Or if WaveTable, we're going to read the file more normally through FatFS,
  // so we want to "open" it.
  else {
...
ensureSafeThenCheckError:
  ...
  if (*error) {
  audioFileError:
    audioFile->~AudioFile(); // Have to call this! This removes the pointers back to
                             // the Sample / SampleClusters from any Clusters.
    generalMemoryAllocator.dealloc(audioFileMemory);
    return NULL;
  }
...
void AudioFileManager::testQueue() {
  ...
  queues[LOADED_SAMPLE_CHUNK_ALLOCATION_QUEUE_NORMAL].first; while
  (loadedSampleChunk !=
  ...
  != &loadedSampleChunk->nextAvailableLoadedSampleChunk) { Uart::println("error
  ---------------------------------");
      ...
      if
  (loadedSampleChunk->nextAvailableLoadedSampleChunk->prevAvailableLoadedSampleChunkPointer
  != &loadedSampleChunk->nextAvailableLoadedSampleChunk) { Uart::println("abc
  ---------------------------------");
          Uart::print(loadedSampleChunk->sample->fileName);
          Uart::print(", part ");
          Uart::println(loadedSampleChunk->chunkIndex);
...
Cluster*
AudioFileManager::allocateCluster(int type, bool shouldAddReasons, void* dontStealFromThing) {

  void* clusterMemory =
      generalMemoryAllocator.alloc(clusterObjectSize, NULL, false, false, true, dontStealFromThing);
  ...
  if (shouldAddReasons) { addReasonToCluster(cluster); }
...
bool AudioFileManager::loadCluster(Cluster* cluster, int minNumReasonsAfter) {
  ...
  if (currentlyAccessingCard)
    return false; // Could happen if we're trying to render a waveform but we're
                  // actually already inside the SD routine
  ...
  if (cluster->type != CLUSTER_SAMPLE)
    numericDriver.freezeWithError(
        "E205"); // Chris F got this, so gonna leave checking in release build

#if ALPHA_OR_BETA_VERSION
  if (cluster->numReasonsToBeLoaded <= 0)
    numericDriver.freezeWithError("E204"); // Ok, I think we know there's at least 1 reason at the
                                           // point this function's called, because it'd only be in
                                           // the loading queue if it had a "reason".
  if (!sample) numericDriver.freezeWithError("E206");
#endif

  addReasonToCluster(cluster); // So that it can't accidentally hit 0 reasons while we're
                               // loading it, cos then it might get deallocated.
  ...
  // If this is the last Cluster, and we do know what the audio data length
  // is...
  if (sample->audioDataLengthBytes && sample->audioDataLengthBytes != 0x8FFFFFFFFFFFFFFF) {
    ...
    if (bytesToRead < clusterSize) { numSectors = ((bytesToRead - 1) >> 9) + 1; }
  ...
  if (cluster->numReasonsToBeLoaded < minNumReasonsAfter + 1)
    numericDriver.freezeWithError("i039"); // It's +1 because we haven't removed
                                           // this function's "reason" yet.
#endif
  ...
  if (uSec > 7000) { Uart::println(uSec); }
#endif
  ...
  if (cluster->numReasonsToBeLoaded < minNumReasonsAfter + 1)
    numericDriver.freezeWithError("i038"); // It's +1 because we haven't removed
                                           // this function's "reason" yet.
#endif
...
#if ALPHA_OR_BETA_VERSION
  if (cluster->numReasonsToBeLoaded < minNumReasonsAfter + 1)
    numericDriver.freezeWithError("i040"); // It's +1 because we haven't removed
                                           // this function's "reason" yet.
#endif
  ...
  if (clusterIndex > 0) {
    ...
    if (prevCluster && prevCluster->loaded) {
      ...
      // We first copy our first 7 bytes from here to the end of the prev
      // Cluster...
      ...
      if (sample->rawDataFormat == RAW_DATA_ENDIANNESS_WRONG_24) {
        ...
        // If we hadn't previously written the "extra" bytes to the end of the
        // prev Cluster and converted them, do so now...
        if (!prevCluster->extraBytesAtEndConverted) {

          uint32_t bytesBeforeStartOfCluster =
              clusterIndex * clusterSize - sample->audioDataStartPosBytes;
          int bytesUnconvertedBeforeCluster = bytesBeforeStartOfCluster % 3;
          if (bytesUnconvertedBeforeCluster) {

            // There'll be one word in there which hasn't yet been converted. Do
            // it now. (We've probably just copied over the next one and a bit,
            // which already was converted)
            ...
            // And now, copy 2 bytes back to this Cluster (that's the maximum
            // that the float could have been overhanging the boundary)
      ...
      // Or, all other types of raw data conversion
        ...
        // If we haven't previously written the "extra" bytes to the end of the
        // prev Cluster and converted them, do so now...
        if (!prevCluster->extraBytesAtEndConverted) {
          ...
          if (misalignment) {
            ...
            // There'll be one word in there which hasn't yet been converted. Do
            // it now. (We've probably also just moved over the next one too,
            // which already was converted)
            ...
            // And now, copy 3 bytes back to this Cluster (that's the maximum
            // that the float could have been overhanging the boundary)
    ...
    if (nextCluster && nextCluster->loaded) {
      ...
      if (sample->rawDataFormat == RAW_DATA_ENDIANNESS_WRONG_24) {
        ...
        if (bytesUnconvertedBeforeNextCluster) {
          ...
          // If we had't previously converted the first couple of bytes of the
          // next Cluster...
          if (!nextCluster->extraBytesAtStartConverted) {

            // We first copy the next Cluster first 7 bytes to the end of this
            // Cluster
            memcpy(&cluster->data[clusterSize], nextCluster->data, 7);
          }

          // Or, if we *had* previously converted the first bytes of the next
          // Cluster...
          else {
          ...
          // which already was converted)
          uint8_t* thisNumber =
              (uint8_t*)&cluster->data[clusterSize - bytesUnconvertedBeforeNextCluster];
          ...
          // If we had't previously converted the first couple of bytes of the
          // next Cluster, do so now...
          if (!nextCluster->extraBytesAtStartConverted) {
            nextCluster->extraBytesAtStartConverted = true;

            // And now, copy 2 bytes back to the next Cluster (that's the
            // maximum that the 24-bit int could have been overhanging the
            // boundary)
      ...
      // Or, all other types of raw data conversion
        ...
        if (misalignment) {
          ...
          if (!nextCluster->extraBytesAtStartConverted) {
            ...
            // There'll be one word in there which hasn't yet been converted
            // from float. Do it now
            sample->convertOneData(thisNumber);

            // And now, copy 3 bytes back to the next Cluster (that's the
            // maximum that the float could have been overhanging the boundary)
          ...
          // Cluster...
            ...
            // And now just copy the converted-from-float first bytes from the
            // next Cluster to the end of this one
            goto copy7ToMe;
          }
        } else {
          goto copy7ToMe;
...
// Only needs calling a couple times per second. Must be called outside of the
// audio / SD-reading routine Call this repeatedly so SD card is re-initialized
// on re-insert before we actually urgently need audio from it
...
void AudioFileManager::loadAnyEnqueuedClusters(int maxNum, bool mayProcessUserActionsBetween) {
  ...
  if (clusterBeingLoaded)
    return; // One might be having stuff done to it, like having its data
            // converted, but not actually reading the card right now
  if (AudioEngine::audioRoutineLocked) return; // Not sure if this should be neccesary?

  // Cannot call any functions in here which will read the SD card, other than
  // loadCluster(), otherwise that'll re-call this function!

  if (cardEjected || cardDisabled) {

  performActionsAndGetOut:
    if (mayProcessUserActionsBetween) { playbackHandler.slowRoutine(); }
    return;
  }
  ...
  while (true) {
    ...
    // We now have an opportunity, since we're not reading the card, to process
    // any pending user actions like undo / redo.
    if (mayProcessUserActionsBetween) { playbackHandler.slowRoutine(); }

    Cluster* cluster = loadingQueue.grabHead();
    if (!cluster) break;

    // cluster has at least 1 "reason". If it didn't, it would have been removed
    // from the load-queue
    ...
    if (!success) {
      ...
      // If the Cluster is now down to 0 reasons (i.e. it lost a reason while
      // being loaded), then it's already been made "available" and we don't
      // have a problem
      if (!cluster->numReasonsToBeLoaded) {
      }

      // Otherwise, there are still "reasons" waiting for this Cluster to become
      // loaded, so we need to put it back in the loading queue. Presumably it
      // won't actually get loaded for a while - only when the user re-inserts
      // the card
      else {
        ...
        enqueueCluster(cluster); // TODO: If that fails, it'll just get
                                 // awkwardly forgotten about

        // Also, return now. Normally we stay here til there's nothing left in
        // the load-queue, but now that would leave us in an infinite loop!
        break;
      }
    }
...
// Currently there's no risk of trying to enqueue a cluster multiple times,
// because this function only gets called after it's freshly allocated
...
void AudioFileManager::removeReasonFromCluster(Cluster* cluster, char const* errorCode) {
  ...
  if (cluster == clusterBeingLoaded &&
      cluster->numReasonsToBeLoaded < minNumReasonsForClusterBeingLoaded) {
  ...
  if (cluster->numReasonsToBeLoaded == 0) {
    ...
    // If it's still in the load queue, remove it from there. (We know that it
    // isn't in the process of being loaded right now because that would have
    // added a "reason", so we wouldn't be here.)
    if (loadingQueue.removeIfPresent(cluster)) {
      ...
      generalMemoryAllocator.putStealableInAppropriateQueue(
          cluster); // It contains data we may want at some future point, so
                    // file it away
    }
...
#else
    numericDriver.displayPopup(errorCode); // For non testers, just display the
                                           // error code without freezing
...
bool AudioFileManager::loadingQueueHasAnyLowestPriorityElements() {
  ...
  return (numElements &&
          ((PriorityQueueElement*)audioFileManager.loadingQueue.getElementAddress(numElements - 1))
