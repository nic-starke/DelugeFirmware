  ...
  if (currentUIMode == UI_MODE_SELECTING_MIDI_CC) {
    if (editingMIDICCForWhichModKnob < NUM_PHYSICAL_MOD_KNOBS) {
      ...
      if (!Buttons::isButtonPressed(selectEncButtonX, selectEncButtonY)) {
        newCC = instrument->changeControlNumberForModKnob(offset,
                                                          editingMIDICCForWhichModKnob,
                                                          instrument->modKnobMode);
        view.setKnobIndicatorLevels();
      } else {
        newCC = instrument->moveAutomationToDifferentCC(offset,
                                                        editingMIDICCForWhichModKnob,
                                                        instrument->modKnobMode,
      ...
      bool automationExists =
          instrument->doesAutomationExistOnMIDIParam(modelStackWithThreeMainThings, newCC);
...
void InstrumentClipMinder::createNewInstrument(int newInstrumentType) {
  ...
  Instrument*             newInstrument =
      storageManager.createNewInstrument(newInstrumentType, &newParamManager);
  if (!newInstrument) {
...
#if HAVE_OLED
  char const* message =
      (newInstrumentType == INSTRUMENT_TYPE_KIT) ? "New kit created" : "New synth created";
  ...
  if (shouldReplaceWholeInstrument) {
    // newInstrument->loadAllSamples(true); // There'll be no samples cos it's
    // new and blank
    currentSong->backUpParamManager((ModControllableAudio*)newInstrument->toModControllable(),
                                    NULL,
                                    &newParamManager,
                                    true); // This is how we feed a ParamManager into the
  ...
  // Or if just adding new Instrument
    ...
    int error =
        getCurrentClip()->changeInstrument(modelStack,
                                           newInstrument,
                                           &newParamManager,
...
void InstrumentClipMinder::setLedStates() {
  IndicatorLEDs::setLedState(synthLedX,
                             synthLedY,
                             getCurrentClip()->output->type == INSTRUMENT_TYPE_SYNTH);
  IndicatorLEDs::setLedState(kitLedX,
                             kitLedY,
                             getCurrentClip()->output->type == INSTRUMENT_TYPE_KIT);
  IndicatorLEDs::setLedState(midiLedX,
                             midiLedY,
                             getCurrentClip()->output->type == INSTRUMENT_TYPE_MIDI_OUT);
...
int InstrumentClipMinder::buttonAction(int x, int y, bool on, bool inCardRoutine) {
  ...
  if (currentUIMode == UI_MODE_HOLDING_SAVE_BUTTON && on) {
    ...
    if (x == synthButtonX && y == synthButtonY) {
      ...
      if (getCurrentClip()->output->type == INSTRUMENT_TYPE_KIT) { goto yesSaveInstrument; }
    }
  }
