...
public:
  ...
  void    setCurrentValueInResponseToUserInput(int32_t                        value,
                                               ModelStackWithAutoParam const* modelStack,
                                               bool                           shouldLogAction = true,
                                               int32_t                        livePos = -1,
                                               bool mayDeleteNodesInLinearRun         = true,
                                               bool isMPE                             = false);
  int32_t processCurrentPos(ModelStackWithAutoParam const* modelStack,
                            bool                           reversed,
                            bool                           didPinpong,
                            bool                           mayInterpolate             = true,
                            bool                           mustUpdateValueAtEveryNode = false);
  void    setValueForRegion(uint32_t                       pos,
                            uint32_t                       length,
                            int32_t                        value,
                            ModelStackWithAutoParam const* modelStack,
                            int                            actionType = ACTION_NOTE_EDIT);
  void    setValuePossiblyForRegion(int32_t                        value,
                                    ModelStackWithAutoParam const* modelStack,
                                    int32_t                        pos,
                                    int32_t                        length,
                                    bool                           mayDeleteNodesInLinearRun = true);
  int32_t
  getValueAtPos(uint32_t pos, ModelStackWithAutoParam const* modelStack, bool reversed = false);
  ...
  void deleteAutomation(Action*                        action,
                        ModelStackWithAutoParam const* modelStack,
  ...
  static bool containedSomethingBefore(bool     wasAutomatedBefore,
  ...
  void        copy(int32_t                        startPos,
                   int32_t                        endPos,
                   CopiedParamAutomation*         copiedParamAutomation,
                   bool                           isPatchCable,
                   ModelStackWithAutoParam const* modelStack);
  void        paste(int32_t                        startPos,
                    int32_t                        endPos,
                    float                          scaleFactor,
                    ModelStackWithAutoParam const* modelStack,
                    CopiedParamAutomation*         copiedParamAutomation,
  ...
  void appendParam(AutoParam* otherParam,
                   int32_t    oldLength,
  ...
  void nudgeNonInterpolatingNodesAtPos(int32_t                        pos,
                                       ...
                                       Action*                        action,
                                       ModelStackWithAutoParam const* modelStack);
  void stealNodes(ModelStackWithAutoParam const* modelStack,
                  int32_t                        pos,
                  int32_t                        regionLength,
                  int32_t                        loopLength,
                  Action*                        action,
                  StolenParamNodes*              stolenNodeRecord = NULL);
  void insertStolenNodes(ModelStackWithAutoParam const* modelStack,
                         int32_t                        pos,
                         int32_t                        regionLength,
                         int32_t                        loopLength,
                         Action*                        action,
                         StolenParamNodes*              stolenNodeRecord);
  void moveRegionHorizontally(ModelStackWithAutoParam const* modelStack,
                              int32_t                        pos,
                              int32_t                        length,
                              int                            offset,
                              int32_t                        lengthBeforeLoop,
                              Action*                        action);
  void
  deleteNodesWithinRegion(ModelStackWithAutoParam const* modelStack, int32_t pos, int32_t length);
  int setNodeAtPos(int32_t pos, int32_t value, bool shouldInterpolate);
  int homogenizeRegion(ModelStackWithAutoParam const* modelStack,
                       int32_t                        startPos,
                       int                            length,
                       int                            startValue,
  ...
  int32_t
       getDistanceToNextNode(ModelStackWithAutoParam const* modelStack, int32_t pos, bool reversed);
  void setCurrentValueWithNoReversionOrRecording(ModelStackWithAutoParam const* modelStack,
                                                 int32_t                        value);
  ...
  uint32_t renewedOverridingAtTime; // If 0, it's off. If 1, it's latched until
                                    // we hit some nodes / automation

  // "Latching" happens when you start recording values, but then stops if you
  // arrive at any pre-existing values. So it only works in empty stretches of
  // time.

private:
  bool deleteRedundantNodeInLinearRun(int     lastNodeInRunI,
                                      int32_t effectiveLength,
                                      bool    mayLoopAroundBackToEnd = true);
  void setupInterpolation(ParamNode* nextNode,
                          int32_t    effectiveLength,
                          int32_t    currentPos,
                          bool       reversed);
  void homogenizeRegionTestSuccess(int  pos,
                                   int  regionEnd,
                                   int  startValue,
