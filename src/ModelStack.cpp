...
ModelStackWithTimelineCounter::addNoteRowAndExtraStuff(int      noteRowIndex,
  ...
  toReturn->modControllable = (isKit && newNoteRow->drum) ? newNoteRow->drum->toModControllable() :
                                                            output->toModControllable();
  toReturn->paramManager    = &newNoteRow->paramManager;
  return toReturn;
}

// This could set a NULL NoteRow if it's not found. This would hopefully get
// picked up on call to getNoteRow(), which checks.
...
bool ModelStackWithNoteRow::isCurrentlyPlayingReversed() const {
  ...
  // Under a few different conditions, we just use the parent Clip's reversing
  // status.
  if (!noteRow ||
...
int32_t ModelStackWithNoteRow::getLastProcessedPos() const {
  ...
  if (noteRow && noteRow->hasIndependentPlayPos()) {
    ...
    // I have a feeling I should sort of be taking noteRowsNumTicksBehindClip
    // into account here - but I know it's usually zero when this gets called,
    // and perhaps the other times I want to ignore it? Should probably
    // investigate further.
  } else {
...
// That's *cut* - as in, cut out abruptly. If it's looping, and the user isn't
// stopping it, that's not a cut. A cut could be if the Session-Clip is armed to
// stop, or if we're getting to the end of a ClipInstance in Arranger
int32_t ModelStackWithNoteRow::getPosAtWhichPlaybackWillCut() const {
  if (noteRow && noteRow->hasIndependentPlayPos()) {
    if (currentPlaybackMode == &session) {

      // Might need Arrangement-recording logic here, like in
      // Session::getPosAtWhichClipWillCut()... FYI that function's code
      // basically mirrors this one - you should look at them together.
      ...
      if (session.willClipContinuePlayingAtEnd(toWithTimelineCounter())) {
        ...
        cutPos = noteRow->lastProcessedPosIfIndependent +
                 ticksTilLaunchEvent; // Might return a pos beyond the loop
                                      // length - maybe that's what we want?
      }
      ...
      if (noteRow->getEffectiveSequenceDirectionMode(this) == SEQUENCE_DIRECTION_PINGPONG) {
        if (reversed) {
          if (cutPos < 0) {
            cutPos = noteRow->lastProcessedPosIfIndependent ?
                         0 :
                         -getLoopLength(); // Check we're not right at pos 0, as we
...
ModelStackWithThreeMainThings*
ModelStackWithNoteRow::addOtherTwoThingsAutomaticallyGivenNoteRow() const {
  ModelStackWithThreeMainThings* toReturn    = (ModelStackWithThreeMainThings*)this;
  ...
  if (flagValue == FLAG_TBD) {
    flagValue =
        ((Sound*)modControllable)
            ->isSourceActiveEverDisregardingMissingSample(s,
