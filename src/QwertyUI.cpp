...
#if HAVE_OLED
void QwertyUI::drawTextForOLEDEditing(int     xPixel,
                                      int     xPixelMax,
                                      int     yPixel,
                                      int     maxNumChars,
  ...
  int minXScroll =
      getMin(enteredTextEditPos + 3 - maxNumChars, displayStringLength - maxNumChars + atVeryEnd);
  ...
  int maxXScroll =
      getMin(displayStringLength - maxNumChars + atVeryEnd,
  ...
  OLED::drawString(&displayName[scrollPosHorizontal],
                   xPixel,
                   yPixel,
                   image[0],
  ...
  // int hilightEndX = xPixel + TEXT_SIZE_X * (displayStringLength -
  // scrollPosHorizontal); if (hilightEndX > OLED_MAIN_WIDTH_PIXELS ||
  // !enteredTextEditPos) hilightEndX = OLED_MAIN_WIDTH_PIXELS;
...
void QwertyUI::displayText(bool blinkImmediately) {
  ...
  int  encodedEditPos = numericDriver.getEncodedPosFromLeft(enteredTextEditPos,
                                                           enteredText.get(),
                                                           &encodedEditPosAndAHalf);

  bool encodedEndPosAndAHalf;
  int  encodedEndPos = numericDriver.getEncodedPosFromLeft(totalTextLength,
                                                          enteredText.get(),
                                                          &encodedEndPosAndAHalf);
  ...
  if (totalTextLength == enteredTextEditPos || enteredText.get()[enteredTextEditPos] == ' ') {
    ...
    encodedEditPosAndAHalf = false; // Hard to put into words why this is needed, but without it,
                                    // the blinking _ after a . just won't blink
  }
  ...
  // Set the text, replacing the bottom layer - cos in some cases, we want this
  // to slip under an existing loading animation layer
  numericDriver.setText(enteredText.get(),
                        false,
                        255,
                        true,
                        blinkMask,
...
int QwertyUI::padAction(int x, int y, int on) {
  ...
  // Enter
    ...
    if (on) {
      ...
      // If currently loading preset, definitely don't abort that - make the
      // user wait and press button again when finished
      else if (currentUIMode == UI_MODE_LOADING_BUT_ABORT_IF_SELECT_ENCODER_TURNED) {
  ...
  // Normal keys
    ...
    if (on) {
      ...
      // Otherwise, if we still might want to use this press...
        ...
        // If this char was already predicted, that's easy and we can just move
        // forward
        if (charCaseEqual(enteredText.get()[enteredTextEditPos], newChar)) {
          ...
          // But if otherwise accessing card, not fine - e.g. if loading song
          // visual preview
        ...
        // Otherwise, char is all new, so add it on
          ...
          // Or if the card is just generally being accessed (e.g. samples being
          // buffered), come back soon, because we couldn't do anything like a
          // "prediction" right now
            ...
            bool success = predictExtendedText(); // This may get cut short if user
                                                  // interrupts by pressing another pad
            if (!success) { enteredTextEditPos--; }

            predictionInterrupted = false;
          }
        }

        displayText(); // We could actually skip this if the user had intervened
                       // during our own predictExtendedText() call above...
                       // kinda...
      }
    }
  }
...
void QwertyUI::processBackspace() {
  if (enteredTextEditPos > 0) { enteredTextEditPos--; }
...
int QwertyUI::horizontalEncoderAction(int offset) {
  if (offset == 1) {
    ...
    // If already at far right end, just see if we can predict any further
    // characters
