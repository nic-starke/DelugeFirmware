...
// Returns error, but NO_ERROR doesn't necessarily mean there's now a file
// loaded - it might be that filePath was NULL, but that's not a problem. Or
// that the SD card would need to be accessed but we didn't have permission for
// that (!mayActuallyReadFile).
int AudioFileHolder::loadFile(bool         reversed,
  ...
  if (filePath.isEmpty())
    return NO_ERROR; // This could happen if the filename tag wasn't present in
                     // the file

  uint8_t    error;
  AudioFile* newAudioFile = audioFileManager.getAudioFileFromFilename(&filePath,
                                                                      mayActuallyReadFile,
                                                                      &error,
                                                                      filePointer,
                                                                      audioFileType,
  ...
  if (newAudioFile) {
    ...
    // We only actually set it after already setting it up, processing the
    // wavetable, etc. - so there's no risk of the audio routine trying to sound
    // it before it's all set up.
...
void AudioFileHolder::setAudioFile(AudioFile* newAudioFile,
                                   bool       reversed,
                                   bool       manuallySelected,
                                   int        clusterLoadInstruction) {
  if (audioFile) {
  ...
  if (audioFile) { audioFile->addReason(); }
}
