...
protected:
  ...
  virtual bool predictExtendedText() {
    return true;
  } // Returns whether we're allowed that new character.
  void         drawKeys();
  virtual void processBackspace(); // May be called in card routine
  virtual void enterKeyPress() = 0;

  // This may be called in card routine so long as either !currentFileExists
  // (which is always the case in a processBackspace()), or we are not
  // LoadSongUI
#if HAVE_OLED
  char const*  title;
  virtual void displayText(bool blinkImmediately = false) = 0;
  void         drawTextForOLEDEditing(int     textStartX,
                                      int     xPixelMax,
                                      int     yPixel,
                                      int     maxChars,
