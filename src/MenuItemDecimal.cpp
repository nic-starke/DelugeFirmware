...
void MenuItemDecimal::beginSession(MenuItem* navigatedBackwardFrom) {
  ...
  for (int i = 0; i < soundEditor.numberEditPos; i++) { soundEditor.numberEditSize *= 10; }
...
void MenuItemDecimal::drawPixelsForOled() {
  ...
  if (numDecimalPlaces) {
    ...
    memmove(&buffer[numCharsBeforeDecimalPoint + 1],
...
void MenuItemDecimal::drawActualValue(bool justDidHorizontalScroll) {
  ...
  if (strlen(outputText) > 4) { outputText[4] = 0; }
