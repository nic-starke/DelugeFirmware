...
void NumericLayerScrollingText::isNowOnTop() {
  if (length > NUMERIC_DISPLAY_LENGTH) { uiTimerManager.setTimer(TIMER_DISPLAY, initialDelay); }
...
bool NumericLayerScrollingText::callBack() {
  ...
  bool reachedEnd =
      (currentPos == 0 || (currentPos >= length - NUMERIC_DISPLAY_LENGTH && currentDirection == 1));

  if (reachedEnd) { currentDirection = -currentDirection; }
