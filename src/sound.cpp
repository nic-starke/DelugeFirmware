...
const PatchableInfo patchableInfoForSound = {
    (int32_t)(offsetof(Sound, paramFinalValues) - offsetof(Sound, patcher) -
              (FIRST_GLOBAL_PARAM * sizeof(int32_t))),
...
Sound::Sound() : patcher(&patchableInfoForSound) {
  ...
  for (int s = 0; s < NUM_SOURCES; s++) { oscRetriggerPhase[s] = 0xFFFFFFFF; }
  for (int m = 0; m < numModulators; m++) { modulatorRetriggerPhase[m] = 0; }

  for (int i = 0; i < NUM_EXPRESSION_DIMENSIONS; i++) { monophonicExpressionValues[i] = 0; }
  ...
  postReverbVolumeLastTime = -1; // Special state to make it grab the actual
                                 // value the first time it's rendered

  // LFO
  lfoGlobalWaveType  = LFO_TYPE_TRIANGLE;
  lfoLocalWaveType   = LFO_TYPE_TRIANGLE;
  lfoGlobalSyncLevel = 0; // This may be set without calling the setter
                          // function, because we're setting it to 0
  ...
  modKnobs[5][0].paramDescriptor.setToHaveParamAndSource(PARAM_LOCAL_PITCH_ADJUST,
                                                         PATCH_SOURCE_LFO_GLOBAL);

#if DELUGE_MODEL != DELUGE_MODEL_40_PAD
  modKnobs[6][1].paramDescriptor.setToHaveParamOnly(PARAM_UNPATCHED_SECTION +
                                                    PARAM_UNPATCHED_STUTTER_RATE);
  modKnobs[6][0].paramDescriptor.setToHaveParamOnly(PARAM_UNPATCHED_SECTION +
                                                    PARAM_UNPATCHED_SOUND_PORTA);

  modKnobs[7][1].paramDescriptor.setToHaveParamOnly(PARAM_UNPATCHED_SECTION +
                                                    PARAM_UNPATCHED_SAMPLE_RATE_REDUCTION);
  modKnobs[7][0].paramDescriptor.setToHaveParamOnly(PARAM_UNPATCHED_SECTION +
...
void Sound::initParams(ParamManager* paramManager) {
  ...
  patchedParams->params[PARAM_LOCAL_OSC_A_PITCH_ADJUST].setCurrentValueBasicForSetup(
      0); // Don't change
  patchedParams->params[PARAM_LOCAL_OSC_B_PITCH_ADJUST].setCurrentValueBasicForSetup(
      0); // Don't change
  patchedParams->params[PARAM_LOCAL_MODULATOR_0_PITCH_ADJUST].setCurrentValueBasicForSetup(
      0); // Don't change
  patchedParams->params[PARAM_LOCAL_MODULATOR_1_PITCH_ADJUST].setCurrentValueBasicForSetup(
      0); // Don't change
}
...
void Sound::setupAsDefaultSynth(ParamManager* paramManager) {
  ...
  paramManager->getPatchCableSet()->patchCables[0].setup(PATCH_SOURCE_NOTE,
                                                         PARAM_LOCAL_LPF_FREQ,
                                                         0x08F5C28C);
  paramManager->getPatchCableSet()->patchCables[1].setup(PATCH_SOURCE_ENVELOPE_1,
                                                         PARAM_LOCAL_LPF_FREQ,
                                                         0x1C28F5B8);
  paramManager->getPatchCableSet()->patchCables[2].setup(PATCH_SOURCE_VELOCITY,
                                                         PARAM_LOCAL_LPF_FREQ,
                                                         0x0F5C28F0);
  paramManager->getPatchCableSet()->patchCables[3].setup(PATCH_SOURCE_VELOCITY,
                                                         PARAM_LOCAL_VOLUME,
                                                         0x3FFFFFE8);
...
void Sound::possiblySetupDefaultExpressionPatching(ParamManager* paramManager) {
  ...
  if (storageManager.firmwareVersionOfFileBeingRead < FIRMWARE_4P0P0_BETA) {
    ...
    if (!paramManager->getPatchCableSet()->isSourcePatchedToSomethingManuallyCheckCables(
            PATCH_SOURCE_AFTERTOUCH) &&
        !paramManager->getPatchCableSet()->isSourcePatchedToSomethingManuallyCheckCables(
            PATCH_SOURCE_X) &&
        !paramManager->getPatchCableSet()->isSourcePatchedToSomethingManuallyCheckCables(
            PATCH_SOURCE_Y)) {
...
void Sound::setupDefaultExpressionPatching(ParamManager* paramManager) {
  ...
  patchCableSet->patchCables[patchCableSet->numPatchCables++].setup(
      PATCH_SOURCE_AFTERTOUCH,
      PARAM_LOCAL_VOLUME,
      getParamFromUserValue(PARAM_STATIC_PATCH_CABLE, 33));
  ...
  if (synthMode == SYNTH_MODE_FM) {
    patchCableSet->patchCables[patchCableSet->numPatchCables++].setup(
        ...
        getParamFromUserValue(PARAM_STATIC_PATCH_CABLE, 15));
  } else {
    patchCableSet->patchCables[patchCableSet->numPatchCables++].setup(
        PATCH_SOURCE_Y,
        PARAM_LOCAL_LPF_FREQ,
        getParamFromUserValue(PARAM_STATIC_PATCH_CABLE, 20));
  }
}
...
bool Sound::setModFXType(int newType) {
  if (newType == MOD_FX_TYPE_FLANGER || newType == MOD_FX_TYPE_CHORUS) {
    if (!modFXBuffer) {
      ...
      modFXBuffer =
          (StereoSample*)generalMemoryAllocator.alloc(modFXBufferSize * sizeof(StereoSample),
                                                      NULL,
                                                      false,
                                                      true);
...
void Sound::patchedParamPresetValueChanged(uint8_t                   p,
  ...
  // If we just enabled an oscillator, we need to calculate voices' phase
  // increments
  if (oldValue == -2147483648 && newValue != -2147483648) {

    // This will make inactive any voiceSources which currently have no volume.
    // Ideally we'd only tell it to do the consideration for the oscillator in
...
void Sound::recalculatePatchingToParam(uint8_t p, ParamManagerForTimeline* paramManager) {
  ...
  if (destination) {
    sourcesChanged |= destination->sources; // Pretend those sources have changed, and the param
                                            // will update - for each Voice too if local.
  }
...
#define ENSURE_PARAM_MANAGER_EXISTS                                                             \
  ...
  UnpatchedParamSet*      unpatchedParams =                                                     \
      (UnpatchedParamSet*)unpatchedParamsSummary->paramCollection;                              \
...
// paramManager only required for old old song files, or for presets (because
// you'd be wanting to extract the defaultParams into it). arpSettings optional
// - no need if you're loading a new V2.0 song where Instruments are all
// separate from Clips and won't store any arp stuff.
int Sound::readTagFromFile(char const*              tagName,
  ...
  // Backwards-compatible reading of old-style oscs, from pre-mid-2016 files
    ...
    while (*(tagName = storageManager.readNextTagOrAttributeName())) {
      ...
      if (!strcmp(tagName, "type")) {
        ...
        patchedParams->readParam(patchedParamsSummary,
    ...
    while (*(tagName = storageManager.readNextTagOrAttributeName())) {
      if (!strcmp(tagName, "type")) {
        ...
        patchedParams->readParam(patchedParamsSummary,
                                 ...
                                 readAutomationUpToPos);
        // setParamPresetValue(PARAM_LOCAL_OSC_B_PHASE_WIDTH,
        // stringToInt(storageManager.readTagContents()) << 1); // Special case
        // - pw values are stored at half size in file
        storageManager.exitTag("phaseWidth");
    ...
    while (*(tagName = storageManager.readNextTagOrAttributeName())) {
      ...
      if (!strcmp(tagName, "rate")) { // This is here for compatibility only for
                                      // people (Lou and Ian) who saved songs
        ...
        storageManager.exitTag("mode");
                         ...
                         "gate")) { // This is here for compatibility only for
                                    // people (Lou and Ian) who saved songs with
                                    // firmware in September 2016
        ENSURE_PARAM_MANAGER_EXISTS
        unpatchedParams->readParam(unpatchedParamsSummary,
    ...
    storageManager.exitTag("noiseVolume");
                   ...
                   "portamento")) { // This is here for compatibility only for
                                    // people (Lou and Ian) who saved songs with
                                    // firmware in September 2016
    ENSURE_PARAM_MANAGER_EXISTS
    unpatchedParams->readParam(unpatchedParamsSummary,
  ...
  // For backwards compatibility. If off, switch off for all operators
    ...
    if (!value) {
      for (int s = 0; s < NUM_SOURCES; s++) { oscRetriggerPhase[s] = 0xFFFFFFFF; }
      for (int m = 0; m < numModulators; m++) { modulatorRetriggerPhase[m] = 0xFFFFFFFF; }
    }
  ...
  // Stuff from the early-2016 format, for compatibility
    ...
    sources[0].oscType = OSC_TYPE_SAMPLE;
    paramManager->getPatchedParamSet()
        ->params[PARAM_LOCAL_ENV_0_ATTACK]
        .setCurrentValueBasicForSetup(getParamFromUserValue(PARAM_LOCAL_ENV_0_ATTACK, 0));
    paramManager->getPatchedParamSet()
        ->params[PARAM_LOCAL_ENV_0_DECAY]
        .setCurrentValueBasicForSetup(getParamFromUserValue(PARAM_LOCAL_ENV_0_DECAY, 20));
    paramManager->getPatchedParamSet()
        ->params[PARAM_LOCAL_ENV_0_SUSTAIN]
        .setCurrentValueBasicForSetup(getParamFromUserValue(PARAM_LOCAL_ENV_0_SUSTAIN, 50));
    paramManager->getPatchedParamSet()
        ->params[PARAM_LOCAL_ENV_0_RELEASE]
        .setCurrentValueBasicForSetup(getParamFromUserValue(PARAM_LOCAL_ENV_0_RELEASE, 0));
    paramManager->getPatchedParamSet()
        ->params[PARAM_LOCAL_OSC_A_VOLUME]
        .setCurrentValueBasicForSetup(getParamFromUserValue(PARAM_LOCAL_OSC_B_VOLUME, 50));
    paramManager->getPatchedParamSet()
        ->params[PARAM_LOCAL_OSC_B_VOLUME]
        .setCurrentValueBasicForSetup(getParamFromUserValue(PARAM_LOCAL_OSC_B_VOLUME, 0));
    ...
    sources[0].cents = (getMax((int8_t)-50,
                               getMin((int8_t)50,
                                      newCents))); // We don't need to call the setTranspose method
                                                   // here, because this will get called soon
                                                   // anyway, once the sample rate is known
    storageManager.exitTag("cents");
    ...
    while (*(tagName = storageManager.readNextTagOrAttributeName())) {
      // Because this is for old, early-2016 format, there'll only be seconds
      // and milliseconds in here, not samples
    ...
    while (*(tagName = storageManager.readNextTagOrAttributeName())) {
      if (!strcmp(tagName, "modKnob")) {
        ...
        if (k < NUM_MOD_BUTTONS) { // Ensure we're not loading more than
                                   // actually fit in our array
          if (p != PARAM_NONE && p != PARAM_PLACEHOLDER_RANGE) { // Discard any unlikely "range"
                                                                 // ones from before V3.2.0, for
                                                                 // complex reasons
        ...
        if (w == NUM_PHYSICAL_MOD_KNOBS) {
          ...
          // If this is a 40-pad Deluge reading a drum preset made for the
          // 144-pad, make "custom 1" be pitch
#if DELUGE_MODEL == DELUGE_MODEL_40_PAD
    ...
    bool result = setModFXType(
        stringToFXType(storageManager.readTagOrAttributeValue())); // This might not work if
                                                                   // not enough RAM
    ...
    while (*(tagName = storageManager.readNextTagOrAttributeName())) {
      ...
      if (!strcmp(tagName, "type")) {
        bool result = setModFXType(
            stringToFXType(storageManager.readTagOrAttributeValue())); // This might not work
                                                                       // if not enough RAM
        if (!result) numericDriver.displayError(ERROR_INSUFFICIENT_RAM);
        storageManager.exitTag("type");
        ...
        // This is for compatibility with old files. Some reverse calculation
        // needs to be done.
        int     finalValue = storageManager.readTagOrAttributeValueInt();
        int32_t i =
            (1 - pow(1 - ((float)finalValue / 2147483648), (float)1 / 3)) / 0.74 * 4294967296 -
            2147483648;
        ENSURE_PARAM_MANAGER_EXISTS
    ...
    int result =
        ModControllableAudio::readTagFromFile(tagName, paramManager, readAutomationUpToPos, song);
    if (result == NO_ERROR) {
...
// Exists for the purpose of potentially correcting an incorrect file as it's
// loaded
void Sound::ensureKnobReferencesCorrectVolume(Knob* knob) {
  int p = knob->paramDescriptor.getJustTheParam();

  if (p == PARAM_GLOBAL_VOLUME_POST_REVERB_SEND || p == PARAM_GLOBAL_VOLUME_POST_FX ||
      p == PARAM_LOCAL_VOLUME) {
...
// p=255 means we're just querying the source to see if it can be patched to
// anything
uint8_t Sound::maySourcePatchToParam(uint8_t s, uint8_t p, ParamManager* paramManager) {
  ...
  if (p != 255 && s != 255 && s >= FIRST_LOCAL_SOURCE && p >= FIRST_GLOBAL_PARAM)
    return PATCH_CABLE_ACCEPTANCE_DISALLOWED; // Can't patch local source to
                                              // global param
  ...
  switch (p) {
    ...
    case PARAM_LOCAL_VOLUME:
      return (s != PATCH_SOURCE_ENVELOPE_0 &&
              s != PATCH_SOURCE_ENVELOPE_1     // No envelopes allowed to be patched to
                                               // volume - this is hardcoded elsewhere
              && s != PATCH_SOURCE_COMPRESSOR) // Don't let the compressor patch
                                               // to local volume - it's supposed
                                               // to go to global volume
                 ?
    ...
    case PARAM_LOCAL_OSC_A_PHASE_WIDTH:
      ...
      // if (getSynthMode() == SYNTH_MODE_FM || (sources[0].oscType !=
      // OSC_TYPE_SQUARE && sources[0].oscType != OSC_TYPE_JUNO60_SUBOSC)) return
      // PATCH_CABLE_ACCEPTANCE_DISALLOWED;
      break;
    case PARAM_LOCAL_OSC_A_VOLUME:
      if (getSynthMode() == SYNTH_MODE_RINGMOD) return PATCH_CABLE_ACCEPTANCE_DISALLOWED;
    case PARAM_LOCAL_OSC_A_PITCH_ADJUST:
      return (isSourceActiveEverDisregardingMissingSample(0, paramManager)) ?
                 PATCH_CABLE_ACCEPTANCE_ALLOWED :
    ...
    case PARAM_LOCAL_CARRIER_0_FEEDBACK:
      ...
      return (isSourceActiveEver(0, paramManager) &&
              patchedParams->params[PARAM_LOCAL_CARRIER_0_FEEDBACK].containsSomething(
                  -2147483648)) ?
                 PATCH_CABLE_ACCEPTANCE_ALLOWED :
                 PATCH_CABLE_ACCEPTANCE_EDITABLE;
    ...
    case PARAM_LOCAL_OSC_B_PHASE_WIDTH:
      if (getSynthMode() == SYNTH_MODE_FM) return PATCH_CABLE_ACCEPTANCE_DISALLOWED;
      // if (getSynthMode() == SYNTH_MODE_FM || (sources[1].oscType !=
      // OSC_TYPE_SQUARE && sources[1].oscType != OSC_TYPE_JUNO60_SUBOSC)) return
      // PATCH_CABLE_ACCEPTANCE_DISALLOWED;
      break;
    case PARAM_LOCAL_OSC_B_VOLUME:
      if (getSynthMode() == SYNTH_MODE_RINGMOD) return PATCH_CABLE_ACCEPTANCE_DISALLOWED;
    case PARAM_LOCAL_OSC_B_PITCH_ADJUST:
      return (isSourceActiveEverDisregardingMissingSample(1, paramManager)) ?
                 PATCH_CABLE_ACCEPTANCE_ALLOWED :
                 PATCH_CABLE_ACCEPTANCE_EDITABLE;
    ...
    case PARAM_LOCAL_CARRIER_1_FEEDBACK:
      ...
      return (isSourceActiveEver(1, paramManager) &&
              patchedParams->params[PARAM_LOCAL_CARRIER_1_FEEDBACK].containsSomething(
                  -2147483648)) ?
                 PATCH_CABLE_ACCEPTANCE_ALLOWED :
                 PATCH_CABLE_ACCEPTANCE_EDITABLE;
    ...
    case PARAM_LOCAL_NOISE_VOLUME:
      ...
      // return (rangeAdjustedParam != PARAM_NONE); // Ideally we'd also check
      // whether the range-adjusted param actually has something patched to it
    ...
    case PARAM_LOCAL_MODULATOR_0_PITCH_ADJUST:
      if (synthMode != SYNTH_MODE_FM) return PATCH_CABLE_ACCEPTANCE_DISALLOWED;
      return (patchedParams->params[PARAM_LOCAL_MODULATOR_0_VOLUME].containsSomething(
                 -2147483648)) ?
                 PATCH_CABLE_ACCEPTANCE_ALLOWED :
                 PATCH_CABLE_ACCEPTANCE_EDITABLE;
    ...
    case PARAM_LOCAL_MODULATOR_0_FEEDBACK:
      ...
      return (patchedParams->params[PARAM_LOCAL_MODULATOR_0_VOLUME].containsSomething(
                  -2147483648) &&
              patchedParams->params[PARAM_LOCAL_MODULATOR_0_FEEDBACK].containsSomething(
                  -2147483648)) ?
                 PATCH_CABLE_ACCEPTANCE_ALLOWED :
                 PATCH_CABLE_ACCEPTANCE_EDITABLE;
    ...
    case PARAM_LOCAL_MODULATOR_1_PITCH_ADJUST:
      if (synthMode != SYNTH_MODE_FM) return PATCH_CABLE_ACCEPTANCE_DISALLOWED;
      return (patchedParams->params[PARAM_LOCAL_MODULATOR_1_VOLUME].containsSomething(
                 -2147483648)) ?
                 PATCH_CABLE_ACCEPTANCE_ALLOWED :
                 PATCH_CABLE_ACCEPTANCE_EDITABLE;
    ...
    case PARAM_LOCAL_MODULATOR_1_FEEDBACK:
      ...
      return (patchedParams->params[PARAM_LOCAL_MODULATOR_1_VOLUME].containsSomething(
                  -2147483648) &&
              patchedParams->params[PARAM_LOCAL_MODULATOR_1_FEEDBACK].containsSomething(
                  -2147483648)) ?
                 PATCH_CABLE_ACCEPTANCE_ALLOWED :
                 PATCH_CABLE_ACCEPTANCE_EDITABLE;

    case PARAM_GLOBAL_LFO_FREQ:
      return (lfoGlobalSyncLevel == 0) ? PATCH_CABLE_ACCEPTANCE_ALLOWED :
                                         PATCH_CABLE_ACCEPTANCE_DISALLOWED;

      // Nothing may patch to post-fx volume. This is for manual control only. The
      // compressor patches to post-reverb volume, and everything else patches to
      // per-voice, "local" volume
    case PARAM_GLOBAL_VOLUME_POST_FX:
      return PATCH_CABLE_ACCEPTANCE_DISALLOWED;

    case PARAM_LOCAL_PITCH_ADJUST:
      if (s == PATCH_SOURCE_X)
        return PATCH_CABLE_ACCEPTANCE_DISALLOWED; // No patching X to pitch. This
                                                  // happens automatically.
      break;

    case PARAM_GLOBAL_VOLUME_POST_REVERB_SEND: // Only the compressor can patch to
                                               // here
      if (s != PATCH_SOURCE_COMPRESSOR) return PATCH_CABLE_ACCEPTANCE_DISALLOWED;
      break;

      // In a perfect world, we'd only allow patching to LFO rates if the LFO as a
      // source is itself patched somewhere usable
  }
...
void Sound::noteOn(ModelStackWithThreeMainThings* modelStack,
  ...
  if (paramManager->getPatchedParamSet()->params[PARAM_LOCAL_NOISE_VOLUME].containsSomething(
          -2147483648))
    goto allFine;
...
allFine:
  ...
  getArpBackInTimeAfterSkippingRendering(
      arpSettings); // Have to do this before telling the arp to noteOn()

  ArpReturnInstruction instruction;
  instruction.sampleSyncLengthOn = sampleSyncLength;

  // We used to not have to worry about the arpeggiator if one-shot samples etc.
  // But now that we support MPE, we do need to keep track of all sounding
  // notes, even one-shot ones, and the "arpeggiator" is where this is stored.
  // These will get left here even after the note has long gone (for sequenced
  // notes anyway), but I can't actually find any negative consequence of this,
  // or need to ever remove them en masse.
  arpeggiator
      ->noteOn(arpSettings, noteCodePreArp, velocity, &instruction, fromMIDIChannel, mpeValues);
...
void Sound::noteOnPostArpeggiator(ModelStackWithSoundFlags* modelStack,
    ...
    for (int v = ends[0]; v < ends[1]; v++) {
      ...
      if (polyphonic == POLYPHONY_MONO || thisVoice->envelopes[0].state >= ENVELOPE_STAGE_RELEASE ||
          !allowNoteTails(modelStack,
                          true)) { // allowNoteTails() is very nearly exactly what we want
                                   // to be calling here, though not named after the thing
                                   // we're looking for here

        // If non-FM and all active sources are samples, do a fast-release (if
        // not already fast-releasing). Otherwise, just unassign (cut instantly)
        if (synthMode == SYNTH_MODE_FM) {
        justUnassign:
          // Ideally, we want to save this voice to reuse. But we can only do
          // that for the first such one
          if (!voiceToReuse) {
  ...
  if (polyphonic == POLYPHONY_LEGATO && voiceForLegato) {
    ...
    voiceForLegato->changeNoteCode(modelStackWithVoice,
                                   noteCodePreArp,
                                   noteCodePostArp,
                                   fromMIDIChannel,
                                   mpeValues);
    ...
    if (voiceToReuse) {
      ...
      numVoicesAssigned++;
      reassessRenderSkippingStatus(modelStack); // Since we potentially just changed
                                                // numVoicesAssigned from 0 to 1.
    ...
    bool success = newVoice->noteOn(modelStackWithVoice,
                                    noteCodePreArp,
                                    noteCodePostArp,
                                    velocity,
                                    sampleSyncLength,
  ...
  lastNoteCode = noteCodePostArp; // Store for porta. We store that at both
...
void Sound::allNotesOff(ModelStackWithThreeMainThings* modelStack, ArpeggiatorBase* arpeggiator) {
  ...
  if (!modelStack->paramManager)
    numericDriver.freezeWithError(
        "E403"); // Previously we were allowed to receive a NULL paramManager,
                 // then would just crudely do an unassignAllVoices(). But I'm
...
void Sound::noteOffPostArpeggiator(ModelStackWithSoundFlags* modelStack, int noteCode) {
  ...
  for (int v = ends[0]; v < ends[1]; v++) {
    ...
    if ((thisVoice->noteCodeAfterArpeggiation == noteCode || noteCode == -32768) &&
        thisVoice->envelopes[0].state < ENVELOPE_STAGE_RELEASE) { // Don't bother if it's already
                                                                  // "releasing"
      ...
      if (arpSettings && arpSettings->mode) { goto justSwitchOff; }

      // If we're in LEGATO or true-MONO mode and there's another note we can
      // switch back to...
      if ((polyphonic == POLYPHONY_LEGATO || polyphonic == POLYPHONY_MONO) && !isDrum() &&
          allowNoteTails(modelStackWithVoice)) { // If no note-tails (i.e. yes one-shot
                                                 // samples etc.), the Arpeggiator will be
                                                 // full of notes which might not be active
                                                 // anymore, cos we were keeping track of
                                                 // them for MPE purposes.
        Arpeggiator* arpeggiator = &((SoundInstrument*)this)->arpeggiator;
        if (arpeggiator->hasAnyInputNotesActive()) {
          ArpNote* arpNote = (ArpNote*)arpeggiator->notes.getElementAddress(
              arpeggiator->notes.getNumElements() - 1);
          int newNoteCode = arpNote->inputCharacteristics[MIDI_CHARACTERISTIC_NOTE];

          if (polyphonic == POLYPHONY_LEGATO) {
            thisVoice->changeNoteCode(modelStackWithVoice,
                                      newNoteCode,
                                      newNoteCode,
            ...
            noteOnPostArpeggiator(
                ...
                newNoteCode,
                newNoteCode,
                arpeggiator->lastVelocity, // Interesting - I've made it keep the
                                           // velocity of presumably the note we just
                                           // switched off. I must have decided that
                                           // sounded best? I think I vaguely remember.
                arpNote->mpeValues,        // ... We take the MPE values from the
                                           // "keypress" associated with the new note
                                           // we'll sound, though.
                0,
                0,
                0,
...
bool Sound::allowNoteTails(ModelStackWithSoundFlags* modelStack, bool disregardSampleLoop) {
  // Return yes unless all active sources are play-once samples, or envelope 0
  // has no sustain

  // If arp on, then definitely yes
  ArpeggiatorSettings* arpSettings =
      getArpSettings((InstrumentClip*)modelStack->getTimelineCounterAllowNull());
  ...
  // After that if not subtractive (so no samples) or there's some noise, we
  // definitely can have tails
  if (synthMode != SYNTH_MODE_SUBTRACTIVE || modelStack->paramManager->getPatchedParamSet()
                                                 ->params[PARAM_LOCAL_NOISE_VOLUME]
                                                 .containsSomething(-2147483648))
    return true;

  // If we still don't know, just check there's at least one active oscillator
  // that isn't a one-shot sample without a loop-end point
...
int32_t Sound::hasAnyTimeStretchSyncing(ParamManagerForTimeline* paramManager,
  ...
  for (int s = 0; s < NUM_SOURCES; s++) {
    ...
    bool sourceEverActive =
        s ? isSourceActiveEver(1, paramManager) : isSourceActiveEver(0, paramManager);
...
int32_t
Sound::hasCutOrLoopModeSamples(ParamManagerForTimeline* paramManager, int note, bool* anyLooping) {
  ...
  for (int s = 0; s < NUM_SOURCES; s++) {
    ...
    if (sources[s].oscType != OSC_TYPE_SAMPLE) {
      return 0;
    } else if (sources[s].repeatMode == SAMPLE_REPEAT_CUT ||
               sources[s].repeatMode == SAMPLE_REPEAT_LOOP) {
      ...
      // TODO: need a bit here to take into account the fact that the note pitch
      // may well have lengthened or shortened the sample
...
bool Sound::allowsVeryLateNoteStart(InstrumentClip* clip, ParamManagerForTimeline* paramManager) {
  ...
  // Basically, if any wave-based oscillators active, or one-shot samples, that
  // means no not allowed
  for (int s = 0; s < NUM_SOURCES; s++) {
    ...
    switch (sources[s].oscType) {
      ...
      case OSC_TYPE_SAMPLE:
        if (sources[s].repeatMode == SAMPLE_REPEAT_ONCE ||
            !sources[s].hasAtLeastOneAudioFileLoaded())
          return false; // Not quite sure why the must-be-loaded requirement -
                        // maybe something would break if it tried to do a late
                        // start otherwise?
        break;
...
bool Sound::isSourceActiveCurrently(int s, ParamManagerForTimeline* paramManager) {
  return (synthMode == SYNTH_MODE_RINGMOD ||
          getSmoothedPatchedParamValue(PARAM_LOCAL_OSC_A_VOLUME + s, paramManager) !=
              -2147483648) &&
...
bool Sound::isSourceActiveEverDisregardingMissingSample(int s, ParamManager* paramManager) {
  return (
      ...
      paramManager->getPatchedParamSet()->params[PARAM_LOCAL_OSC_A_VOLUME + s].containsSomething(
          -2147483648) ||
...
bool Sound::renderingOscillatorSyncEver(ParamManager* paramManager) {
  if (!oscillatorSync) return false;
  if (synthMode == SYNTH_MODE_FM) return false;
  return (paramManager->getPatchedParamSet()->params[PARAM_LOCAL_OSC_B_VOLUME].containsSomething(
              -2147483648) ||
          synthMode == SYNTH_MODE_RINGMOD);
}
...
// Unlike most functions, this one accepts modelStack as NULL, because when
// unassigning all voices e.g. on song swap, we won't have it.
void Sound::reassessRenderSkippingStatus(ModelStackWithSoundFlags* modelStack,
                                         bool                      shouldJustCutModFX) {

  ArpeggiatorSettings* arpSettings =
      getArpSettings(); // TODO: should get the caller to provide this, cos they
                        // usually already have it. In fact, should put this on
                        // the ModelStack, cos many deeper-nested functions
                        // called by this one need it too!

  bool skippingStatusNow =
      (!numVoicesAssigned && !delay.repeatsUntilAbandon && !stutterer.status &&
    ...
    if (skippingStatusNow) {
      ...
      if (modFXType) {
        ...
        if (!startSkippingRenderingAtTime) {
          ...
          // But wait, first, maybe we actually have just been instructed to cut
          // the MODFX tail
          ...
          int waitSamples = (modFXType == MOD_FX_TYPE_CHORUS) ?
                                (20 * 44) :
                                (90 * 441); // 20 and 900 mS respectively. Lots is required
                                            // for feeding-back flanger or phaser
        ...
        // Or if already waiting, see if the wait is over yet
          ...
          // Ok, we wanted to check that before manually cutting the MODFX tail,
          // to save time, but that's still an option...
...
// fromAutomation means whether the changes was caused by automation playing
// back - as opposed to the user turning the knob right now
void Sound::notifyValueChangeViaLPF(int                                  p,
  ...
  if (!shouldDoParamLPF) {
    ...
    if (paramLPF.p == p) { paramLPF.p = PARAM_LPF_OFF; }
    goto dontDoLPF;
  }
  ...
  if (paramNeedsLPF(p, fromAutomation)) {
    ...
    if (paramLPF.p != PARAM_LPF_OFF) {
      ...
      if (paramLPF.p != p) {
        ...
        ModelStackWithThreeMainThings* modelStackCopy =
            (ModelStackWithThreeMainThings*)modelStackMemory;

        stopParamLPF(modelStackCopy->addSoundFlags());
      }

      // Otherwise, keep its current state, and just tell it it's going
      // somewhere new
      else {
...
void Sound::doParamLPF(int numSamples, ModelStackWithSoundFlags* modelStack) {
  ...
  int32_t diff =
      (modelStack->paramManager->getPatchedParamSet()->getValue(paramLPF.p) >> 8) - (oldValue >> 8);
  ...
  if (diff == 0) {
    ...
    // patchedParamPresetValueChanged(paramLPF.p, notifySound, oldValue,
    // paramLPF.currentValue);
...
// voices e.g. on song swap, we won't have it.
void Sound::stopParamLPF(ModelStackWithSoundFlags* modelStack) {
  bool wasActive = paramLPF.p != PARAM_LPF_OFF;
  if (wasActive) {
    int p      = paramLPF.p;
    paramLPF.p = PARAM_LPF_OFF; // Must do this first, because the below call
                                // will involve the Sound calling us back for
                                // the current value
    if (modelStack)
      patchedParamPresetValueChanged(p,
                                     modelStack,
                                     paramLPF.currentValue,
...
void Sound::render(ModelStackWithThreeMainThings* modelStack,
  ...
  if (paramManager->getPatchCableSet()->isSourcePatchedToSomething(PATCH_SOURCE_COMPRESSOR)) {
    if (sideChainHitPending) { compressor.registerHit(sideChainHitPending); }

    int32_t old                                 = globalSourceValues[PATCH_SOURCE_COMPRESSOR];
    globalSourceValues[PATCH_SOURCE_COMPRESSOR] = compressor.render(
        numSamples,
        paramManager->getUnpatchedParamSet()->getValue(PARAM_UNPATCHED_COMPRESSOR_SHAPE));
  ...
  int32_t reverbSendAmount = multiply_32x32_rshift32_rounded(
                                 reverbAmountAdjust,
  ...
  if (arpSettings && arpSettings->mode) {
    ...
    uint32_t           gateThreshold =
        (uint32_t)unpatchedParams->getValue(PARAM_UNPATCHED_SOUND_ARP_GATE) + 2147483648;
    uint32_t phaseIncrement = arpSettings->getPhaseIncrement(
        paramFinalValues[PARAM_GLOBAL_ARP_RATE - FIRST_GLOBAL_PARAM]);
  ...
  delayWorkingState.delayFeedbackAmount =
      paramFinalValues[PARAM_GLOBAL_DELAY_FEEDBACK - FIRST_GLOBAL_PARAM];
  ...
  if (numVoicesAssigned) {
    ...
    // Very often, we'll just apply panning here at the Sound level rather than
    // the Voice level
    ...
    filterSetConfig.doLPF =
        (thisHasFilters &&
         (lpfMode == LPF_MODE_TRANSISTOR_24DB_DRIVE ||
          paramManager->getPatchCableSet()->doesParamHaveSomethingPatchedToIt(
              PARAM_LOCAL_LPF_FREQ) ||
    ...
    // Each voice will potentially alter the "sources changed" flags, so store a
    // backup to restore between each voice
    /*
    ...
    */
    ...
    for (int v = ends[0]; v < ends[1]; v++) {
      ...
      if (!doneFirstVoice) {
          if (numVoicesAssigned > 1) {
              memcpy(backedUpSourcesChanged,
      &sourcesChanged[FIRST_LOCAL_SOURCE], FIRST_UNCHANGEABLE_SOURCE -
      FIRST_LOCAL_SOURCE); doneFirstVoice = true;
          }
      }
      else memcpy(&sourcesChanged[FIRST_LOCAL_SOURCE], backedUpSourcesChanged,
      FIRST_UNCHANGEABLE_SOURCE - FIRST_LOCAL_SOURCE);
      */

      ModelStackWithVoice* modelStackWithVoice = modelStackWithSoundFlags->addVoice(thisVoice);

      bool stillGoing = thisVoice->render(modelStackWithVoice,
                                          soundBuffer,
                                          numSamples,
                                          renderingInStereo,
    ...
    if (!renderingInStereo) {
      // We know that nothing's patched to pan, so can read it in this very
      // basic way.
    ...
    if (!renderingInStereo) { memset(&soundBuffer[numSamples], 0, numSamples * sizeof(int32_t)); }
  }

  int32_t postFXVolume = paramFinalValues[PARAM_GLOBAL_VOLUME_POST_FX - FIRST_GLOBAL_PARAM];
  int32_t postReverbVolume =
      paramFinalValues[PARAM_GLOBAL_VOLUME_POST_REVERB_SEND - FIRST_GLOBAL_PARAM];
  ...
  processFX((StereoSample*)soundBuffer,
            numSamples,
            modFXType,
            modFXRate,
            modFXDepth,
  ...
  int32_t postReverbSendVolumeIncrement =
      (int32_t)(postReverbVolume - postReverbVolumeLastTime) / numSamples;

  processReverbSendAndVolume((StereoSample*)soundBuffer,
                             numSamples,
                             reverbBuffer,
                             postFXVolume,
  ...
  // Unlike all the other possible reasons we might want to start skipping
  // rendering, delay.repeatsUntilAbandon may have changed state just now.
...
void Sound::startSkippingRendering(ModelStackWithSoundFlags* modelStack) {
  ...
  // compressor.status = ENVELOPE_STAGE_OFF; // Was this doing anything? Have
  // removed, to make all of this completely reversible without doing anything
...
void Sound::stopSkippingRendering(ArpeggiatorSettings* arpSettings) {
  if (skippingRendering) {
    ...
    int32_t modFXTimeOff = AudioEngine::audioSampleTimer -
                           timeStartedSkippingRenderingModFX; // This variable is a good indicator
    ...
    if (modFXTimeOff) {
      ...
      // if
      // (paramManager->getPatchCableSet()->isSourcePatchedToSomething(PATCH_SOURCE_COMPRESSOR))
      // {
      ...
      // clearModFXMemory(); // No need anymore, now we wait for this to
      // basically empty before starting skipping
    }
...
void Sound::unassignAllVoices() {
  ...
  for (int v = ends[0]; v < ends[1]; v++) {
    ...
    AudioEngine::activeVoices.checkVoiceExists(
        thisVoice,
        this,
        "E203"); // ronronsen got error!
                 // https://forums.synthstrom.com/discussion/4090/e203-by-changing-a-drum-kit#latest
    AudioEngine::unassignVoice(thisVoice,
                               this,
                               NULL,
  ...
  if (ALPHA_OR_BETA_VERSION) {
    if (numVoicesAssigned > 0)
      numericDriver.freezeWithError(
          "E070"); // ronronsen got error!
                   // https://forums.synthstrom.com/discussion/4090/e203-by-changing-a-drum-kit#latest
    else if (numVoicesAssigned < 0)
      numericDriver.freezeWithError(
          "E071"); // ronronsen got error!
                   // https://forums.synthstrom.com/discussion/4090/e203-by-changing-a-drum-kit#latest
  }

  // reassessRenderSkippingStatus(); // Nope, this will get called in
  // voiceUnassigned(), which gets called for each voice we unassign above.
}
...
void Sound::confirmNumVoices(char const* error) {
  ...
  for (Voice* thisVoice = audioDriver.voices; thisVoice != endAssignedVoices;
  thisVoice++) { if (thisVoice->assignedToSound == this) { voiceCount++;
  ...
  if (sources[0].sample) {
      int totalNumReasons = sources[0].sample->getTotalNumReasons(reasonCount +
  reasonCountSources); if (totalNumReasons != reasonCount + reasonCountSources)
  {
...
void Sound::resyncGlobalLFO() {
  if (lfoGlobalSyncLevel != 0) {
    ...
    timeStartedSkippingRenderingLFO =
        AudioEngine::audioSampleTimer; // Resets the thing where the number of
                                       // samples skipped is later converted
...
bool Sound::learnKnob(MIDIDevice*     fromDevice,
  ...
  // If a MIDI knob
    ...
    return ModControllableAudio::learnKnob(fromDevice,
                                           paramDescriptor,
                                           whichKnob,
                                           modKnobMode,
                                           midiChannel,
...
void Sound::ensureInaccessibleParamPresetValuesWithoutKnobsAreZero(Song* song) {
  ...
  while (true) {
    ...
    BackedUpParamManager* backedUp =
        (BackedUpParamManager*)song->backedUpParamManagers.getElementAddress(i);
  ...
  song->ensureInaccessibleParamPresetValuesWithoutKnobsAreZero(
      this); // What does this do exactly, again?
}
...
void Sound::ensureInaccessibleParamPresetValuesWithoutKnobsAreZeroWithMinimalDetails(
...
// Song may be NULL
void Sound::ensureInaccessibleParamPresetValuesWithoutKnobsAreZero(
  ...
  for (int i = 0; i < sizeof(patchedParamsWhichShouldBeZeroIfNoKnobAssigned); i++) {
    ModelStackWithParamId* modelStackWithParamId = modelStackWithParamCollection->addParamId(
        patchedParamsWhichShouldBeZeroIfNoKnobAssigned[i]);
...
void Sound::ensureParamPresetValueWithoutKnobIsZero(ModelStackWithAutoParam* modelStack) {
  ...
  // If the param is automated, we'd better not try setting it to 0 - the user
  // probably wants the automation
...
void Sound::doneReadingFromFile() {
  ...
  for (int s = 0; s < NUM_SOURCES; s++) { sources[s].doneReadingFromFile(this); }

  setupUnisonDetuners(NULL);

  for (int m = 0; m < numModulators; m++) { recalculateModulatorTransposer(m, NULL); }
}
...
void Sound::calculateEffectiveVolume() {
  ...
  // (float)getParamNeutralValue(PARAM_LOCAL_VOLUME) / sqrt(numUnison);
  volumeNeutralValueForUnison = (float)134217728 / sqrtf(numUnison);
}

// May change mod knob functions. You must update mod knob levels after calling
// this
void Sound::setSynthMode(uint8_t value, Song* song) {

  unassignAllVoices(); // This saves a lot of potential problems, to do with
                       // samples playing. E002 was being caused
  ...
  if (synthMode == SYNTH_MODE_FM && oldSynthMode != SYNTH_MODE_FM) {
    for (int f = 0; f < NUM_MOD_BUTTONS; f++) {
      if (modKnobs[f][0].paramDescriptor.isJustAParam() &&
          modKnobs[f][1].paramDescriptor.isJustAParam()) {
...
void Sound::setNumUnison(int newNum, ModelStackWithSoundFlags* modelStack) {
  ...
  numUnison = newNum;
  setupUnisonDetuners(modelStack); // Can handle NULL. Also calls
    ...
    for (int v = ends[0]; v < ends[1]; v++) {
      ...
      if (synthMode == SYNTH_MODE_SUBTRACTIVE) {
        ...
        for (int s = 0; s < NUM_SOURCES; s++) {
          ...
          if (sourceEverActive && synthMode != SYNTH_MODE_FM &&
              sources[s].oscType == OSC_TYPE_SAMPLE && thisVoice->guides[s].audioFileHolder &&
              thisVoice->guides[s].audioFileHolder->audioFile) {

            // For samples, set the current play pos for the new unison part, if
            // num unison went up
            if (newNum > oldNum) {
              ...
              if (newPart->active) {
                ...
                if (!newPart->voiceSample) {
                  ...
                  newVoiceSample->cloneFrom(oldVoiceSample); // Just clones the SampleLowLevelReader
                                                             // stuff
                  newVoiceSample->pendingSamplesLate = oldVoiceSample->pendingSamplesLate;

                  newVoiceSample->doneFirstRenderYet = true;

                  // Don't do any caching for new part. Old parts will stop
                  // using their cache anyway because their pitch will have
                  // changed
                  newVoiceSample->stopUsingCache(
                      &thisVoice->guides[s],
                      (Sample*)thisVoice->guides[s].audioFileHolder->audioFile,
...
// paramManager only required for old old song files, or for presets (because
// you'd be wanting to extract the defaultParams into it) arpSettings optional -
// no need if you're loading a new V2.0+ song where Instruments are all separate
// from Clips and won't store any arp stuff
int Sound::readFromFile(ModelStackWithModControllable* modelStack,
  ...
  while (*(tagName = storageManager.readNextTagOrAttributeName())) {
    int result = readTagFromFile(tagName,
                                 &paramManager,
                                 readAutomationUpToPos,
                                 arpSettings,
                                 modelStack->song);
  ...
  if (paramManager.containsAnyMainParamCollections()) {
    ...
    modelStack->song->backUpParamManager(this,
                                         (Clip*)modelStack->getTimelineCounterAllowNull(),
                                         &paramManager,
                                         true);
  }
...
int Sound::createParamManagerForLoading(ParamManagerForTimeline* paramManager) {
  ...
  paramManager->getUnpatchedParamSet()
      ->params[PARAM_UNPATCHED_COMPRESSOR_SHAPE]
...
void Sound::compensateVolumeForResonance(ModelStackWithThreeMainThings* modelStack) {
  ...
  if (storageManager.firmwareVersionOfFileBeingRead < FIRMWARE_1P2P0 &&
    ...
    int32_t compensation =
        interpolateTableSigned(patchedParams->getValue(PARAM_LOCAL_LPF_RESONANCE) + 2147483648,
                               32,
    ...
    patchCableSet->setupPatching(
        modelStackWithParamCollection); // So that we may then call
...
int Sound::readSourceFromFile(int                      s,
                              ParamManagerForTimeline* paramManager,
                              int32_t                  readAutomationUpToPos) {
  ...
  while (*(tagName = storageManager.readNextTagOrAttributeName())) {
    ...
    */
      ...
      source->sampleControls.pitchAndSpeedAreIndependent =
      ...
      while (*(tagName = storageManager.readNextTagOrAttributeName())) {
        ...
        if (!strcmp(tagName, "sampleRange") || !strcmp(tagName, "wavetableRange")) {
          ...
          while (*(tagName = storageManager.readNextTagOrAttributeName())) {
            ...
            if (!strcmp(tagName, "fileName")) {
              ...
              if (!strcmp(tagName, "zone")) {
                ...
                ((SampleHolderForVoice*)holder)->cents =
                    storageManager.readTagOrAttributeValueInt();
                storageManager.exitTag("cents");
              } else
                goto justExitTag;
          ...
          if (i < source->ranges.getNumElements()) {
            MultisampleRange* existingRange =
                (MultisampleRange*)source->ranges.getElementAddress(i);
...
void Sound::writeSourceToFile(int s, char const* tagName) {
  ...
  if (source->oscType == OSC_TYPE_SAMPLE &&
      synthMode != SYNTH_MODE_FM) { // Don't combine this with the above "if" -
                                    // there's an "else" below
    ...
    storageManager.writeAttribute("timeStretchEnable",
                                  source->sampleControls.pitchAndSpeedAreIndependent);
    ...
    for (int e = 0; e < numRanges; e++) {
      ...
      if (numRanges > 1) {
        ...
        if (e != numRanges - 1) { storageManager.writeAttribute("rangeTopNote", range->topNote); }
      }

      storageManager.writeAttribute("fileName",
                                    range->sampleHolder.audioFile ?
  ...
  // Otherwise, if we're *not* a (multi)sample, here's the other option, which
  // includes (multi)wavetable
  else {
    ...
    if (source->oscType == OSC_TYPE_WAVETABLE && synthMode != SYNTH_MODE_FM) {
      ...
      for (int e = 0; e < numRanges; e++) {
        ...
        if (numRanges > 1) { storageManager.closeTag(); }
      }
...
bool Sound::readParamTagFromFile(char const*              tagName,
  ...
  if (!strcmp(tagName, "arpeggiatorGate")) {
    ...
    paramManager->getPatchCableSet()->readPatchCablesFromFile(readAutomationUpToPos);
    storageManager.exitTag("patchCables");
  } else if (ModControllableAudio::readParamTagFromFile(tagName,
                                                        paramManager,
                                                        readAutomationUpToPos)) {
  }
...
void Sound::writeParamsToFile(ParamManager* paramManager, bool writeAutomation) {
  ...
  unpatchedParams->writeParamAsAttribute("arpeggiatorGate",
                                         PARAM_UNPATCHED_SOUND_ARP_GATE,
                                         writeAutomation);
  unpatchedParams->writeParamAsAttribute("portamento",
                                         PARAM_UNPATCHED_SOUND_PORTA,
                                         writeAutomation);
  unpatchedParams->writeParamAsAttribute("compressorShape",
                                         PARAM_UNPATCHED_COMPRESSOR_SHAPE,
                                         writeAutomation);

  patchedParams->writeParamAsAttribute("oscAVolume", PARAM_LOCAL_OSC_A_VOLUME, writeAutomation);
  patchedParams->writeParamAsAttribute("oscAPulseWidth",
                                       PARAM_LOCAL_OSC_A_PHASE_WIDTH,
                                       writeAutomation);
  patchedParams->writeParamAsAttribute("oscAWavetablePosition",
                                       PARAM_LOCAL_OSC_A_WAVE_INDEX,
                                       writeAutomation);
  patchedParams->writeParamAsAttribute("oscBVolume", PARAM_LOCAL_OSC_B_VOLUME, writeAutomation);
  patchedParams->writeParamAsAttribute("oscBPulseWidth",
                                       PARAM_LOCAL_OSC_B_PHASE_WIDTH,
                                       writeAutomation);
  patchedParams->writeParamAsAttribute("oscBWavetablePosition",
                                       PARAM_LOCAL_OSC_B_WAVE_INDEX,
                                       writeAutomation);
  ...
  patchedParams->writeParamAsAttribute("modulator1Amount",
                                       PARAM_LOCAL_MODULATOR_0_VOLUME,
                                       writeAutomation);
  patchedParams->writeParamAsAttribute("modulator1Feedback",
                                       PARAM_LOCAL_MODULATOR_0_FEEDBACK,
                                       writeAutomation);
  patchedParams->writeParamAsAttribute("modulator2Amount",
                                       PARAM_LOCAL_MODULATOR_1_VOLUME,
                                       writeAutomation);
  patchedParams->writeParamAsAttribute("modulator2Feedback",
                                       PARAM_LOCAL_MODULATOR_1_FEEDBACK,
                                       writeAutomation);

  patchedParams->writeParamAsAttribute("carrier1Feedback",
                                       PARAM_LOCAL_CARRIER_0_FEEDBACK,
                                       writeAutomation);
  patchedParams->writeParamAsAttribute("carrier2Feedback",
                                       PARAM_LOCAL_CARRIER_1_FEEDBACK,
                                       writeAutomation);

  patchedParams->writeParamAsAttribute("pitchAdjust",
                                       PARAM_LOCAL_PITCH_ADJUST,
                                       writeAutomation,
                                       true);
  patchedParams->writeParamAsAttribute("oscAPitchAdjust",
                                       PARAM_LOCAL_OSC_A_PITCH_ADJUST,
                                       writeAutomation,
                                       true);
  patchedParams->writeParamAsAttribute("oscBPitchAdjust",
                                       PARAM_LOCAL_OSC_B_PITCH_ADJUST,
                                       writeAutomation,
                                       true);
  patchedParams->writeParamAsAttribute("mod1PitchAdjust",
                                       PARAM_LOCAL_MODULATOR_0_PITCH_ADJUST,
                                       writeAutomation,
                                       true);
  patchedParams->writeParamAsAttribute("mod2PitchAdjust",
                                       PARAM_LOCAL_MODULATOR_1_PITCH_ADJUST,
                                       writeAutomation,
                                       true);
  ...
  patchedParams->writeParamAsAttribute("delayFeedback",
                                       PARAM_GLOBAL_DELAY_FEEDBACK,
                                       writeAutomation);
...
void Sound::writeToFile(bool                 savingSong,
  ...
  for (int k = 0; k < NUM_MOD_BUTTONS; k++) {
    for (int w = 0; w < NUM_PHYSICAL_MOD_KNOBS; w++) {
      ...
      storageManager.writeAttribute("controlsParam",
                                    paramToString(knob->paramDescriptor.getJustTheParam()),
                                    false);
      if (!knob->paramDescriptor.isJustAParam()) {
...
ModelStackWithAutoParam* Sound::getParamFromModEncoder(int whichModEncoder,
  ...
  // If setting up a macro by holding its encoder down, the knobs will represent
  // macro control-amounts rather than actual "params", so there's no "param".
...
Sound::getParamFromModEncoderDeeper(int                            whichModEncoder,
                                    ModelStackWithThreeMainThings* modelStack,
                                    bool                           allowCreation) {
...
bool Sound::modEncoderButtonAction(uint8_t                        whichModEncoder,
  ...
  if (ourModKnob->paramDescriptor.isSetToParamWithNoSource(PARAM_UNPATCHED_SECTION +
                                                           PARAM_UNPATCHED_STUTTER_RATE)) {
    if (on) {
  ...
  // Switch sidechain sync level
    ...
    if (on) {
      ...
      if (currentSong) { // Bit of a hack just referring to currentSong in
                         // here...
        insideWorldTickMagnitude = (currentSong->insideWorldTickMagnitude +
                                    currentSong->insideWorldTickMagnitudeOffsetFromBPM);
      } else
  ...
  // Switching between LPF, HPF and EQ
    ...
    if (on && synthMode != SYNTH_MODE_FM) {
      ourModKnob->paramDescriptor.setToHaveParamOnly(PARAM_UNPATCHED_SECTION +
  ...
  else if (ourModKnob->paramDescriptor.isSetToParamWithNoSource(PARAM_UNPATCHED_SECTION +
                                                                PARAM_UNPATCHED_TREBLE)) {
    if (on && synthMode != SYNTH_MODE_FM) {
...
// This can get called either for hibernation, or because drum now has no active
// noteRow
void Sound::wontBeRenderedForAWhile() {
  ModControllableAudio::wontBeRenderedForAWhile();

  unassignAllVoices(); // Can't remember if this is always necessary, but it is
  ...
  reassessRenderSkippingStatus(NULL,
                               true); // Tell it to just cut the MODFX tail - we
                                      // needa change status urgently!

  // If it still thinks it's meant to be rendering, we did something wrong
  if (ALPHA_OR_BETA_VERSION && !skippingRendering) { numericDriver.freezeWithError("E322"); }
}

void Sound::detachSourcesFromAudioFiles() {
  for (int s = 0; s < NUM_SOURCES; s++) { sources[s].detachAllAudioFiles(); }
}

void Sound::deleteMultiRange(int s, int r) {
  // Because range storage is about to change, must unassign all voices, and
  // make sure no more can be assigned during memory allocation
  unassignAllVoices();
...
bool Sound::renderingVoicesInStereo(ModelStackWithSoundFlags* modelStack) {
  ...
  // Stereo live-input
  if ((sources[0].oscType == OSC_TYPE_INPUT_STEREO ||
       sources[1].oscType == OSC_TYPE_INPUT_STEREO) &&
      (AudioEngine::micPluggedIn || AudioEngine::lineInPluggedIn))
    return true;

  if (modelStack->paramManager->getPatchCableSet()->doesParamHaveSomethingPatchedToIt(
          PARAM_LOCAL_PAN))
    return true;
  ...
  for (int s = 0; s < NUM_SOURCES; s++) {
    ...
    if (source->oscType == OSC_TYPE_SAMPLE) { // Just SAMPLE, because WAVETABLEs can't be stereo.
      ...
      // If multiple ranges, we have to come back and examine Voices to see
      // which are in use
      if (numRanges > 1) {
  ...
  // Ok, if that determined that either source has multiple samples (multisample
  // ranges), we now have to investigate each Voice
...
int Sound::stringToParam(char const* string) {
  ...
  for (int p = PARAM_UNPATCHED_SECTION + NUM_SHARED_UNPATCHED_PARAMS;
       p < PARAM_UNPATCHED_SECTION + MAX_NUM_UNPATCHED_PARAM_FOR_SOUNDS;
       p++) {
    if (!strcmp(string, Sound::paramToString(p))) return p;
  }

  if (!strcmp(string, "range"))
    return PARAM_PLACEHOLDER_RANGE; // For compatibility reading files from
                                    // before V3.2.0
...
ModelStackWithAutoParam* Sound::getParamFromMIDIKnob(MIDIKnob*                      knob,
  ...
  ModelStackWithAutoParam* modelStackWithAutoParam =
      summary->paramCollection->getAutoParamFromId(modelStackWithParamId,
                                                   true); // Allow patch cable creation. TODO:
