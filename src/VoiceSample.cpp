...
void VoiceSample::noteOn(SamplePlaybackGuide* guide, uint32_t samplesLate, int priorityRating) {
  ...
  pendingSamplesLate = samplesLate; // We store this to deal with later, because in order to deal
                                    // with this we need to know the pitch-adjustment, and that's
                                    // not calculated yet
  oscPos                          = 0;
...
bool VoiceSample::noteOffWhenLoopEndPointExists(Voice* voice, VoiceSamplePlaybackGuide* guide) {
  ...
  if (cache) {
    ...
    if (timeStretcher)
      ...
      return reassessReassessmentLocation(guide,
                                          (Sample*)guide->audioFileHolder->audioFile,
                                          voice->getPriorityRating());
      // That's only going to make reassessmentLocation later, so no need to
      // check we haven't shot past it I think...
    }
  }
}
...
void VoiceSample::setupCacheLoopPoints(SamplePlaybackGuide* guide,
                                       Sample*              sample,
                                       int                  loopingType) {
  ...
  uint64_t combinedIncrement =
      ((uint64_t)(uint32_t)cache->phaseIncrement * (uint32_t)cache->timeStretchRatio) >> 24;

  bool endPointIsRightAtEnd =
      (guide->playDirection == 1) ?
          (guide->endPlaybackAtByte ==
  ...
  // End point: if it's right at the actual end of the sample, we can just fill
  // the cache til it's full.
  ...
  // Yes looping
    ...
    uint32_t loopStartPointBytes =
        (guide->playDirection == 1) ? loopStartPointBytesRaw - sample->audioDataStartPosBytes :
                                      sample->audioDataStartPosBytes +
                                          sample->audioDataLengthBytes - loopStartPointBytesRaw - 1;
    ...
    cacheLoopEndPointBytes =
        loopEndPointCombinedIncrements * CACHE_BYTE_DEPTH * sample->numChannels;
  }
}
...
int VoiceSample::attemptLateSampleStart(SamplePlaybackGuide* voiceSource,
                                        Sample*              sample,
                                        int64_t              rawSamplesSinceStart,
                                        int                  numSamples) {

  int bytesPerSample = sample->numChannels * sample->byteDepth;

  int64_t startAtByte = rawSamplesSinceStart * bytesPerSample * voiceSource->playDirection +
                        voiceSource->startPlaybackAtByte;

  // If we've already passed the end of the sample (how would that occur in the
  // real world, again?)
  ...
  if (startAtClusterIndex >= sample->getFirstClusterIndexWithNoAudioData())
    numericDriver.freezeWithError(
        "E366"); // This can occur if some overflowing happened on the previous
                 // check due to an insanely high rawSamplesSinceStart being
                 // supplied due to some other bug. This was a problem around
                 // V3.1.0 release, so currently keeping this check even outside
                 // of ALPHA_OR_BETA_VERSION. Sven got! 4.0.0-beta4.

  int finalClusterIndex =
      voiceSource->getFinalClusterIndex(sample, cache); // Think this is right...

  int clusterIndex = startAtClusterIndex;

  // We load our new Clusters into a secondary array first, to preserve the
  // reason-holding power of whatever is already in our main one until we
  // unassign them below
  ...
  for (int l = 0; l < NUM_CLUSTERS_LOADED_AHEAD; l++) {
    ...
    // Grab it.
    newClusters[l] = sample->clusters.getElement(clusterIndex)
                         ->getCluster(sample, clusterIndex, CLUSTER_ENQUEUE);

    // If failure (would only happen in insanely rare case where there's no free
    // RAM)
    if (l == 0 && !newClusters[l]) { return LATE_START_ATTEMPT_FAILURE; }
  ...
  // Remove all old reasons - there might be some if this function has been
  // called multiple times while we wait for Clusters to load
  unassignAllReasons();

  // Copy in the new reasons we just made
  memcpy(clusters, newClusters, sizeof(clusters));

  // TODO: lots of this code is kinda tied to there being just two clusters
  // looked-ahead (wait, not any more right?)
  ...
  if (clusters[0]->loaded) {
    ...
    goodToGo:
      setupForPlayPosMovedIntoNewCluster(voiceSource,
                                         sample,
                                         bytesPosWithinCluster,
                                         sample->byteDepth);
    ...
    // Or, if we're actually not very far into the first Cluster, that's fine
    // too - the second one should still have time to load
    else {
      int numBytesIn;
      ...
      if (numBytesIn < (audioFileManager.clusterSize >> 1)) { goto goodToGo; }
    }
  }

  // If still here, that didn't work, so we have to wait, and come back later
  // when hopefully some loading has taken place
...
bool VoiceSample::fudgeTimeStretchingToAvoidClick(Sample*              sample,
  ...
  int32_t playByte = getPlayByteLowLevel(sample, guide) -
                     sample->audioDataStartPosBytes; // Allow for this to be negative. I'm not
                                                     // sure if it could in this exact case of
                                                     // "fudging", but see the similar code
                                                     // below in weShouldBeTimeStretchingNow()
                                                     // - better safe than sorry.
  int32_t playSample = divide_round_negative(playByte, sample->numChannels * sample->byteDepth);

  bool success = timeStretcher->init(sample,
                                     this,
                                     guide,
                                     (int64_t)playSample << 24,
                                     sample->numChannels,
                                     phaseIncrement,
                                     16777216,
                                     playDirection,
                                     priorityRating,
                                     numSamplesTilLoop,
                                     0); // Tell it no looping
  if (!success) {
    Uart::println("fudging FAIL!!!!");
    return false; // It's too late to salvage anything - our play pos has
                  // probably been mucked around
  }

  success =
      reassessReassessmentLocation(guide,
                                   sample,
                                   priorityRating); // Got to - because time stretching affects the
                                                    // SampleLowLevelReader's adherence to markers
  // That's only going to make reassessmentLocation later, so no need to check
  // we haven't shot past it I think...
  if (!success) {
    Uart::println("fudging FAIL!!!!");
    return false;
  }
...
// Returns false if becoming unassigned now
bool VoiceSample::weShouldBeTimeStretchingNow(Sample*              sample,
                                              SamplePlaybackGuide* guide,
                                              int                  numSamples,
                                              int32_t              phaseIncrement,
  ...
  if (!timeStretcher) {
    ...
    int32_t playByte = getPlayByteLowLevel(sample, guide) -
                       sample->audioDataStartPosBytes; // May return negative number - I think
                                                       // particularly if we're going in reversed
                                                       // and just cancelled reading from cache
    int32_t playSample = divide_round_negative(playByte, sample->numChannels * sample->byteDepth);

    timeStretcher->init(sample,
                        this,
                        guide,
                        (int64_t)playSample << 24,
                        sample->numChannels,
  ...
  // Enforce a limit on how many samples can be rendered in one call to
  // fillPercCache() 32 is about minimum to avoid "hitting front edge" of perc
  // cache when sped up double. We'll do 32 if we're writing to main cache, cos
  // that makes it extra important that we get the best sound. Otherwise, go
  // real easy on CPU
  int maxNumSamplesToProcess = numSamples * ((cache && writingToCache) ? 32 : 6);

  if (timeStretchRatio != 16777216)
    sample->fillPercCache(timeStretcher,
                          playSample,
                          playSample + (phaseIncrement >> 10) * playDirection,
                          playDirection,
                          maxNumSamplesToProcess);

  return true;
}

bool VoiceSample::stopReadingFromCache() {
  // Have to check Cluster is loaded, because we chose not to check this before,
  // cos we didn't know if we'd actually be reading from it
...
// Returns false if fail, which can happen if we've actually ended up past the
...
bool VoiceSample::stopUsingCache(SamplePlaybackGuide* guide,
  ...
  // If we were *writing* to the cache, nothing needs to change other than our
  // discarding it. But if we were reading from it...
  if (!writingToCache) {

    if (!stopReadingFromCache()) return false;
    // If time-stretching needs to be started back up, that'll happen below in
    // call to considerTimeStretching()
  }

  cache = NULL;

  // Now that cache is off, the SampleLowLevelReader probably needs to obey loop
  // points (if no time stretching), Although, as a side note, if we just
  // abandoned reading cache, we might be just about to set time stretching up.
  if (shouldObeyMarkers()) {
    bool stillGoing = reassessReassessmentLocation(
        guide,
        sample,
        priorityRating); // Returns false if fail, which can happen if we've
                         // actually ended up past the finalClusterIndex cos we
                         // were reading cache before
    if (!stillGoing) return false;

    // This step added Sept 2020 after finding another similar bug which made me
    // fairly sure this needs to be here, to ensure currentPlayPos isn't past
    // the new reassessmentLocation
...
bool VoiceSample::render(SamplePlaybackGuide* guide,
                         int32_t* __restrict__ outputBuffer,
                         int     numSamples,
                         Sample* sample,
                         int     sampleSourceNumChannels,
                         int     loopingType,
                         int32_t phaseIncrement,
                         int32_t timeStretchRatio,
                         int32_t amplitude,
  ...
  // If there's a cache, check some stuff. Do this first, cos this can cause us
  // to return
  if (cache) {

    // If relevant params have changed since before, we have to stop using the
    // cache which those params previously described
    if (phaseIncrement != cache->phaseIncrement || timeStretchRatio != cache->timeStretchRatio ||
        (phaseIncrement != 16777216 && (desiredInterpolationMode != INTERPOLATION_MODE_SMOOTH ||
      ...
      if (needToAvoidClick) {
        ...
        success = weShouldBeTimeStretchingNow(sample,
                                              guide,
                                              numSamples,
                                              phaseIncrement,
                                              timeStretchRatio,
    ...
    // Or if no Clusters got stolen, check some other stuff
      ...
      if (writingToCache) { // Writing
        ...
        // If cache is time-stretched and we're almost at the end of what was
        // written, we'd better switch out of cache-reading mode now so we can
        // use that last little bit of the cache to crossfade smoothly out of it
        if (timeStretchRatio != 16777216 && cache->writeBytePos < cacheEndPointBytes &&
            cache->writeBytePos < cacheLoopEndPointBytes &&
            cache->writeBytePos < cacheBytePos + (TIME_STRETCH_DEFAULT_FIRST_HOP_LENGTH *
                                                  CACHE_BYTE_DEPTH * sampleSourceNumChannels)) {
          ...
          // Beware - having just stopped reading from cache, our currentPlayPos
          // might be way out of bounds, potentially delivering even a negative
          // sample / byte number. This needs to be maintained cohesively until
          // it's ultimately dealt with.
          ...
          // Check that all that setting up didn't steal any of our cache to the
          // left of where we are now - in which case we can't continue to write
          // to it, and there's nothing else we want it for, so forget about it.
          // Also can't continue to write if doing linear interpolation now
          if (cache->writeBytePos < cacheBytePos ||
              (phaseIncrement != 16777216 &&
               interpolationBufferSize != INTERPOLATION_MAX_NUM_SAMPLES)) {
            cache = NULL;
          }

          // Or if we're still all good
          else {
            cache->setWriteBytePos(cacheBytePos); // Re-write the last little bit of the cache,
  ...
  if (!cache || writingToCache) {
    ...
    if (timeStretchRatio != 16777216) {
      bool stillGoing = weShouldBeTimeStretchingNow(sample,
                                                    guide,
                                                    numSamples,
                                                    phaseIncrement,
                                                    timeStretchRatio,
      ...
      // If writing to cache, there's a chance that setting up time stretching
      // and generating perc data could have stolen Clusters, so we might have
      // to stop writing to cache
      if (cache && writingToCache) {
        if (cache->writeBytePos < cacheBytePos) {
          bool success =
              stopUsingCache(guide, sample, priorityRating, loopingType == LOOP_LOW_LEVEL);
          if (!success) return false;
        }
      }
    }

    // If we shouldn't be time stretching now, but if it remains set up from
    // before, stop it. We'd know there's no cache in this case
    else {
      if (timeStretcher && !fudging) {

        // We're only allowed to stop once all the play-pos's line up, otherwise
        // there's a big ol' click
        bool canExit;
        ...
        {
          canExit =
              (currentPlayPos == timeStretcher->olderPartReader.currentPlayPos &&
        ...
        if (canExit) {
          ...
          // Bugfix Sept 2020. Could end up beyond reassessmentLocation
          // otherwise, violating assumptions if playing Sample at non-native
          // rate.
          stillGoing = changeClusterIfNecessary(guide,
                                                sample,
                                                loopingType == LOOP_LOW_LEVEL,
                                                priorityRating);
          if (!stillGoing) return false;
          // Or, if changeClusterIfNecessary() returns false, should we continue
          // outputting those zeros through the interpolation buffer for a bit
          // or something?
        }
      }
    }
...
timeStretchingConsidered:
  ...
  int numChannelsInOutputBuffer =
      (sampleSourceNumChannels == 2 && AudioEngine::renderInStereo) ? 2 : 1;
  int      whichKernel;
  uint64_t combinedIncrement;
  ...
  // Loop over and over until we've read all the samples we want, until
  // numSamples is 0
  ...
  readCachedWindow:
    ...
    if (bytesTilLoopEndPoint <= 0) { // Might be less than 0 if it was just changed... although the
                                     // code that does that is suppose to also detect that we're
                                     // past it and restart the loop...
      Uart::println("Loop endpoint reached, reading cache");
      // Jump back to the loop start point. We'll find out in a moment whether
      // that Cluster still exists (though it will if the one we were just at
      // existed)
    ...
    if (bytesTilCacheEnd == 0) {
      ...
      // If we're here, then timeStretchRatio should be 16777216, and
      // phaseIncrement should *not*
      ...
      if (ALPHA_OR_BETA_VERSION && phaseIncrement == 16777216)
        numericDriver.freezeWithError("E241"); // If this were the case, there'd
                                               // be no reason to have a cache
      ...
      // I think the reason I have it doing reassessReassessmentLocation() here
      // is because while writing cache, low-level loop points aren't obeyed and
      // we loop at the writing-to-cache level instead?
      bool stillGoing = reassessReassessmentLocation(
          guide,
          sample,
          priorityRating); // Returns false if fail, which can happen if we've
                           // actually ended up past the finalClusterIndex cos
                           // we were reading cache before
      // TODO: need to call changeClusterIfNecessary()? Or are we guaranteed not
      // to have passed that loop point / reassessment location?
      if (!stillGoing) return false;

      // This step added Sept 2020 after finding another similar bug which made
      // me fairly sure this needs to be here, to ensure currentPlayPos isn't
      // past the new reassessmentLocation
      stillGoing =
          changeClusterIfNecessary(guide, sample, loopingType == LOOP_LOW_LEVEL, priorityRating);
      if (!stillGoing) return false;
    ...
    if (ALPHA_OR_BETA_VERSION && !cacheCluster) { // If it got stolen - but we should have already
                                                  // detected this above
      numericDriver.freezeWithError("E157");
    }
    int32_t* __restrict__ readPos =
        (int32_t*)&cacheCluster->data[bytePosWithinCluster - 4 + CACHE_BYTE_DEPTH];

    int32_t sampleRead[2]; // Somehow works out a tiny bit faster having it as
                           // an array

    sampleRead[0] = *readPos; // Do first read up here so there's time for the
    ...
    int samplesTilThisWindowEnd = (uint32_t)(bytesTilThisWindowEnd - 1) /
    ...
    if (samplesTilThisWindowEnd < numSamplesThisCacheRead)
      numSamplesThisCacheRead = samplesTilThisWindowEnd; // Only do this after we're sure we're not
                                                         // cancelling caching

    if (ALPHA_OR_BETA_VERSION && numSamplesThisCacheRead <= 0)
      numericDriver.freezeWithError("E156");

    // Ok, now we know how many samples we can read from the cache right now. Do
    // it. Note: I tried putting this loop in another function, but it just
    // wasn't quite as fast, even with it set to inline.
    ...
    while (true) {
      ...
      *outputBufferWritePos =
          multiply_accumulate_32x32_rshift32_rounded(existingValueL,
                                                     sampleRead[0],
                                                     amplitude); // Yup, accumulate is faster
      outputBufferWritePos++;

      // Right channel. Surprisingly, putting this as an "else" inside the above
      // "numChannels == 2" was slightly slower
    ...
    // Need to also keep track of the un-cached play-pos so we can switch back
    // if needed
    ...
    int32_t  uncachedBytePos =
        (playDirection == 1) ? sample->audioDataStartPosBytes + uncachedSamplePos * bytesPerSample :
                                sample->audioDataStartPosBytes + sample->audioDataLengthBytes -
                                   (uncachedSamplePos + 1) * bytesPerSample;

    int uncachedClusterIndex = uncachedBytePos >> audioFileManager.clusterSizeMagnitude;

    // Sometimes our cache will extend a little beyond the end of the waveform
    // (to capture the interpolation or time-stretching ring-out). It's
    // basically ok for our current Cluster index and currentPlayPos to sit
    // outside the waveform - this will be dealt with if we do stop using the
    // cache.

    // But, if we're more than 1 cluster outside of the waveform, let's just not
    // be silly.
    if (uncachedClusterIndex < sample->getFirstClusterIndexWithAudioData() - 1 ||
      ...
      unassignAllReasons(); // Remember, this doesn't cut the voice - just sets
                            // clusters[0] to NULL.
      currentPlayPos = 0;
    }
    ...
    // But if that hasn't happened...
      ...
      // Need to make sure we don't go past finalClusterIndex, or else, if cache
      // is cancelled and loop points obeyed again, we're in a Cluster we're not
      // allowed in! Instead, stay in the Cluster we are allowed in,
      // currentPlayPos will end up outside the bounds of that Cluster, that'll
      // soon be picked up on, and a loop point will get obeyed or something.
  ...
  // Or, not replaying cached stuff (but potentially writing into the cache for
  // next time)
  else {
  ...
  uncachedPlayback:
    ...
    if (cache) {
      ...
      if (cachingBytesTilLoopEnd <= 0) { // Might be less than 0 if it was just changed... although
                                         // the code that does that is suppose to also detect that
                                         // we're past it and restart the loop...
        Uart::println("Loop endpoint reached, writing cache");
        switchToReadingCacheFromWriting();
        goto readCachedWindow;
      }

      // If reached end of actual sample waveform, we're done! We know we're not
      // looping, cos the above condition would have triggered
      ...
      if (bytePosWithinCluster == 0) {
        ...
        if (!setupSuccess) {
          // Cancel cache writing. Everything else is still the same - we
          // weren't *reading* the cache, remember
          bool stopSuccess =
              stopUsingCache(guide,
                             sample,
                             priorityRating,
      ...
      if (ALPHA_OR_BETA_VERSION && !cacheCluster)
        numericDriver.freezeWithError("E166"); // Check that the Cluster hasn't been stolen - but
                                               // this should have been detected right at the start
      ...
      int cachingBytesTilUncachedReadEnd =
          getMin(cachingBytesTilClusterEnd, cachingBytesTilLoopEnd);
      cachingBytesTilUncachedReadEnd =
          getMin(cachingBytesTilUncachedReadEnd, cachingBytesTilWaveformEnd);

#if CACHE_BYTE_DEPTH == 3
      int cachingSamplesTilUncachedReadEnd =
          (uint32_t)(cachingBytesTilUncachedReadEnd - 1) /
              (uint8_t)(sampleSourceNumChannels * CACHE_BYTE_DEPTH) +
          1; // Round up
#else
      int cachingSamplesTilUncachedReadEnd =
          cachingBytesTilUncachedReadEnd >> CACHE_BYTE_DEPTH_MAGNITUDE;
      ...
      if (cachingSamplesTilUncachedReadEnd < numSamplesThisUncachedRead)
        numSamplesThisUncachedRead =
            cachingSamplesTilUncachedReadEnd; // Only do this after we're sure
                                              // we're not cancelling caching
    ...
    uncachedPlaybackNotWriting:
      ...
      // If time stretching but not synced, now is the time to check loop / end
      // point
      if (timeStretcher && !guide->sequenceSyncLengthTicks) {
        ...
        int64_t combinedIncrementingThisUncachedRead =
            combinedIncrement * numSamplesThisUncachedRead;
      ...
      assessLoopPointAgainTimestretched:
        ...
        int bytePosAfterThisUncachedRead =
        ...
        // If we will have reached the reassessment-point, by the end of this
        // "uncached read"...
        if (overshootBytes > 0) {
          ...
          int loopEndAtSample =
              (reassessmentPosBytesRelativeToAudioDataStart >= 0) ?
                  ...
                  -1; // This is plausible, cos going in reverse, the "end
                      // pos" is -1 sample

          int64_t combinedIncrementingLeftToDo =
              ((int64_t)loopEndAtSample << 24) - timeStretcher->samplePosBig;
          int64_t combinedIncrementingLeftToDoAbsolute =
              combinedIncrementingLeftToDo * playDirection;

          // Maybe we're at the reassessment-point right now, so need to
          // reassess...
          if (combinedIncrementingLeftToDoAbsolute <
              (int64_t)combinedIncrement) { // It seems that without the (int64_t)
                                            // enforced, it was behaving wrong

            // If want to actually loop... (Remember though, this whole bit
            // doesn't apply to synced samples / AudioClips.)
            if (loopingType) { // For either type of looping

              Uart::println("loop point reached, timestretching");

              int32_t newSamplePos = (uint32_t)(guide->getBytePosToStartPlayback(true) -
                                                sample->audioDataStartPosBytes) /
              ...
              timeStretcher->reInit(newSamplePosBigOvershot,
                                    guide,
                                    this,
                                    sample,
...
#if ALPHA_OR_BETA_VERSION
              if (count >= 1024) { numericDriver.freezeWithError("E169"); }
              count++;
#endif
              goto assessLoopPointAgainTimestretched;
            }

            // Or, if want to stop at this reassessment point we just reached,
            // that's easy
            else {
              return false;
            }
          ...
          // Or otherwise, just shorten this uncached-read to stop right at the
          // reassessment-point, so we can reassess immediately after
          else {
            int combinedIncrementsLeft =
    ...
    numSamples -= numSamplesThisUncachedRead; // Do it now because we're about to start
                                              // decrementing numSamplesThisUncachedRead
    ...
    readNonTimestretched:
      ...
      bool stillActive =
          considerUpcomingWindow(guide,
                                 sample,
                                 &numSamplesThisNonTimestretchedRead,
                                 phaseIncrement,
                                 loopingType,
                                 interpolationBufferSize,
                                 (cache != NULL),
                                 priorityRating); // Keep it reading silence forever so we can
                                                  // definitely fill up the cache
      if (!stillActive) { return false; }

      // No pitch adjustment - in which case we know we're not writing to cache
      // either
      if (phaseIncrement == 16777216) {

        readSamplesNative((int32_t**)&outputBufferWritePos,
                          numSamplesThisNonTimestretchedRead,
                          sample,
                          jumpAmount,
                          sampleSourceNumChannels,
                          numChannelsInOutputBuffer,
                          &amplitude,
                          amplitudeIncrement);
      }

      // Yes pitch adjustment - meaning we might be wanting to write to a cache,
      // too...
      else {

        // That call to considerUpcomingWindow() just might have led to our
        // cache having Clusters stolen!
        ...
        // This stuff is in its own function rather than here in VoiceSample
        // because for some reason it's faster (re-check that?)
        readSamplesResampled((int32_t**)&outputBufferWritePos,
                             numSamplesThisNonTimestretchedRead,
                             sample,
                             jumpAmount,
                             sampleSourceNumChannels,
      ...
      if (cache) {
        cacheBytePos +=
            numSamplesThisNonTimestretchedRead * CACHE_BYTE_DEPTH * sampleSourceNumChannels;
    ...
    // Or, if yes time stretching
      ...
      if (cache) { // If writing cache...
        ...
        // Because we're writing cache, and if both play-heads have finished up,
        // clearing that cache is all that's actually required!
        if (!timeStretcher->playHeadStillActive[PLAY_HEAD_OLDER] &&
            !timeStretcher->playHeadStillActive[PLAY_HEAD_NEWER]) {

          memset(cacheWritePos,
                 0,
                 numSamplesThisUncachedRead * CACHE_BYTE_DEPTH * sampleSourceNumChannels);
        ...
        memset(timeStretchResultWritePos,
               0,
               numSamplesThisUncachedRead * 4 * sampleSourceNumChannels);
      ...
      int numSamplesThisUncachedReadUntouched =
          numSamplesThisUncachedRead; // This one won't get decremented between
                                      // multiple timestretch iterations
    ...
    readTimestretched:
      ...
      if (timeStretcher->playHeadStillActive[PLAY_HEAD_NEWER]) {
        ...
        // Basically, if other hops have already happened in this routine, we
        // want to defer doing one, to spread the CPU load. But, the more times
        // we've been skipped in this way, the more we start insisting that yes,
        // it's our turn.
        ...
        if (wantToDoHopNow) {
          if (allowedToDoHop) {
            bool success = timeStretcher->hopEnd(guide,
                                                 this,
                                                 sample,
                                                 sampleSourceNumChannels,
                                                 timeStretchRatio,
                                                 ...
                                                 priorityRating);
            if (!success)
              return false; // Can fail if tried to jump to a bit in an unloaded
                            // Cluster. Shouldn't actually happen

            // Check this again, cos newer play-head can become inactive in
            // hopEnd(). This probably isn't really crucial. Added June 2019
      ...
      // TODO: would I save CPU if I also made it stop right at the end of the
      // crossfade?
      ...
      bool    olderPlayHeadAudibleHere = (timeStretcher->playHeadStillActive[PLAY_HEAD_OLDER] &&
                                       timeStretcher->crossfadeProgress < 16777216);
      ...
      if (timeStretcher->newerHeadReadingFromBuffer &&
        ...
        int samplesTilChangeover =
            (timeStretcher->bufferWritePos - timeStretcher->newerBufferReadPos) &
      ...
      // If there's a cache, we don't want the per-play-head functions to apply
      // the overall voice amplitude envelope - we'll do that at the end when we
      // copy from the cache into the output buffer
      if (cache) {
        preCacheAmplitude          = 2147483647;
        preCacheAmplitudeIncrement = 0;
      } else {
        // We >>1 to match the fact that in the yes-cache case, above, the max
        // we can set this to is 2147483647, and we compensate for both these
        // things by having <<1'd amplitude for time-stretching, at the start of
        // this func. This isn't quite ideal
        preCacheAmplitude          = amplitude >> 1;
        preCacheAmplitudeIncrement = amplitudeIncrement >> 1;
      }

      // If the older play-head is still fading out, which means we need to work
      // out the crossfade envelope too...
      if (olderPlayHeadAudibleHere) {

        // Use linear crossfades. These sound less jarring when doing short
        // hops. Square root ones sound better for longer hops, with more
        // difference in material between hops (i.e. time more stretched)
        ...
        timeStretcher->crossfadeProgress +=
            timeStretcher->crossfadeIncrement * numSamplesThisTimestretchedRead;

        int32_t newerHopAmplitudeAfter = lshiftAndSaturate(timeStretcher->crossfadeProgress, 7);

        int32_t newerHopAmplitudeIncrement =
            (int32_t)(newerHopAmplitudeAfter - newerHopAmplitudeNow) /
            (int32_t)numSamplesThisTimestretchedRead;
        int32_t olderHopAmplitudeIncrement = -newerHopAmplitudeIncrement;

        int32_t hopAmplitudeChange =
            multiply_32x32_rshift32(preCacheAmplitude, newerHopAmplitudeIncrement) << 1;

        newerAmplitudeIncrementNow = preCacheAmplitudeIncrement + hopAmplitudeChange;
        newerSourceAmplitudeNow = multiply_32x32_rshift32(preCacheAmplitude, newerHopAmplitudeNow)
                                  << 1;

        olderAmplitudeIncrementNow = preCacheAmplitudeIncrement - hopAmplitudeChange;
        olderSourceAmplitudeNow = multiply_32x32_rshift32(preCacheAmplitude, olderHopAmplitudeNow)
                                  << 1;
      }
      ...
      bool swappingOrder =
          (timeStretcher->playHeadStillActive[PLAY_HEAD_NEWER] &&
           timeStretcher->bufferFillingMode == BUFFER_FILLING_OLDER &&
           (timeStretcher->playHeadStillActive
                [PLAY_HEAD_OLDER])); // ||
    ...
    readNewerHead:
      if (timeStretcher->playHeadStillActive[PLAY_HEAD_NEWER]) {
        ...
        // If reading buffer...
        if (timeStretcher->newerHeadReadingFromBuffer) {
          timeStretcher->readFromBuffer(timeStretchResultWritePos,
                                        writePosNowR,
                                        numSamplesThisTimestretchedRead,
                                        sampleSourceNumChannels,
                                        numChannelsInOutputBuffer,
                                        ...
                                        &timeStretcher->newerBufferReadPos,
                                        writeIncrement);
        }
...
#endif
        {
          bool success = readSamplesForTimeStretching(
              timeStretchResultWritePos,
              guide,
              sample,
              numSamplesThisTimestretchedRead,
              sampleSourceNumChannels,
              numChannelsInTimeStretchResult,
              phaseIncrement,
              newerSourceAmplitudeNow,
              newerAmplitudeIncrementNow,
              (loopingType == LOOP_LOW_LEVEL),
    ...
    headsFinishedReading:
      ...
      timeStretcher->samplePosBig +=
          (int64_t)combinedIncrement * numSamplesThisTimestretchedRead * playDirection;
      ...
      if (timeStretcher->bufferFillingMode != BUFFER_FILLING_OFF) {
        ...
        if (timeStretcher->newerHeadReadingFromBuffer &&
          ...
          // if (timeStretcher->bufferFillingMode == BUFFER_FILLING_OLDER)
          // Uart::println(" - was filling older");
      ...
      if (!cache) { amplitude += amplitudeIncrement * numSamplesThisTimestretchedRead; }

      numSamplesThisUncachedRead -= numSamplesThisTimestretchedRead;

      if (numSamplesThisUncachedRead) {
        timeStretchResultWritePos +=
            numSamplesThisTimestretchedRead * numChannelsInTimeStretchResult;
        goto readTimestretched;
      }

      // If we were writing to a temp buffer instead of the output buffer
      // because we need to cache, now we copy from the temp buffer to both the
      // cache and the output buffer
      if (cache) {
        ...
        // We'll do an ugly hack here - fudging is where a TimeStretcher is
        // added just at the end of a play-through, to do a crossfade. This
        // leads to a rare case where if we're recording a cache, our
        // whether-there's-a-TimeStretcher changes, but we still keep the same
        // cache. So, we have to compensate for a difference in cached volume.
        bool fudgingNow = fudging;

        char* __restrict__ cacheWritePosNow = cacheWritePos;

        // Check that all of our sample-reading above hasn't stolen any of our
        // cache Clusters.
        if (cache->writeBytePos != cacheBytePos) {
          bool success =
              stopUsingCache(guide, sample, priorityRating, loopingType == LOOP_LOW_LEVEL);
          if (!success)
            return false; // Is this ideal? Didn't give much consideration when
                          // I wrote this line
        ...
        while (true) {
          ...
          *outputBufferWritePos =
              multiply_accumulate_32x32_rshift32_rounded(existingValueL,
                                                         sampleRead[0],
                                                         amplitude); // amplitude is modified above
          outputBufferWritePos++;

          // Right channel
          if (numChannelsInOutputBuffer == 2) {
            int32_t existingValueR = *outputBufferWritePos;
            *outputBufferWritePos  = multiply_accumulate_32x32_rshift32_rounded(existingValueR,
                                                                               sampleRead[1],
                                                                               amplitude);
            outputBufferWritePos++;
          }
        ...
        if (cache) { // Might not be true anymore if we had to abandon the cache
                     // just above cos it got Clusters stolen.
          cacheWritePos = cacheWritePosNow; // Not necessary I don't think - cacheWritePos
                                            // doesn't get used again does it?

          cacheBytePos +=
              numSamplesThisUncachedReadUntouched * CACHE_BYTE_DEPTH * sampleSourceNumChannels;
          cache->writeBytePos = cacheBytePos; // These two were and are now still the same
        }
      }
...
bool VoiceSample::sampleZoneChanged(SamplePlaybackGuide* voiceSource,
  ...
  // If cache, then update cache loop points - but not if it was the start
  // marker that was moved, cos that means we'll stop using cache altogether
  ...
  if (markerType == MARKER_START) {
    ...
    if (cache) {
      bool success =
          stopUsingCache(voiceSource, sample, priorityRating, loopingType == LOOP_LOW_LEVEL);
      if (!success) return false;
    }
    ...
    if (cache) {
      ...
      loopBackToStartCached:
        ...
        switchToReadingCacheFromWriting();
        cacheBytePos =
            cacheLoopEndPointBytes - cacheLoopLengthBytes; // This is allowed to be a bit rough
        // In a perfect world we'd check the Clusters are loaded and stuff, but
        // that'll get checked soon enough, and what would we do if they weren't
        // loaded anyway
      }
    }
      ...
      if (timeStretcher) {
        ...
        if (((VoiceSamplePlaybackGuide*)voiceSource)->shouldObeyLoopEndPointNow()) {
          int bytePos = timeStretcher->getSamplePos(voiceSource->playDirection) *
                        ...
                        sample->audioDataStartPosBytes;
          int overshootBytes =
              (bytePos - ((VoiceSamplePlaybackGuide*)voiceSource)->loopEndPlaybackAtByte) *
              voiceSource->playDirection;

          if (overshootBytes >= 0) { goto loopBackToStartTimeStretched; }
        }
      } else {
    ...
    // Or if no cache...
      ...
      if (timeStretcher) {
        ...
        if (voiceSource->endPlaybackAtByte &&
            (forAudioClip || !((VoiceSamplePlaybackGuide*)voiceSource)
          ...
          int bytePos = timeStretcher->getSamplePos(voiceSource->playDirection) *
                        ...
                        sample->audioDataStartPosBytes;
          int overshootBytes =
              (bytePos - voiceSource->endPlaybackAtByte) * voiceSource->playDirection;
        ...
        justDoReassessment:
          ...
          // I think this function has already taken care of making sure we're
          // not past the new reassessmentLocation...
        }
      }
    }
  }
  ...
  loopBackToStartTimeStretched:
    ...
    // Must call this before ending time stretching, cos the presence of the
    // TimeStretcher causes the SampleLowLevelReader to ignore markers, which is
    // how we want it (no, wait, that's wrong I think? I've undone that for now
    // anyway)
    setupClusersForInitialPlay(voiceSource, sample, 0, true, priorityRating);
  }
...
int32_t VoiceSample::getPlaySample(Sample* sample, SamplePlaybackGuide* guide) {
  if (timeStretcher) {
    ...
    int bytePosFromAudioDataStart =
        getPlayByteLowLevel(sample, guide) - sample->audioDataStartPosBytes;
...
// If returns false, means everything's failed badly and must cut whole
// VoiceSource (instantUnassign)
bool VoiceSample::possiblySetUpCache(SampleControls*      sampleControls,
  ...
  if (guide->sequenceSyncLengthTicks &&
      (playbackHandler.playbackState & PLAYBACK_CLOCK_EXTERNAL_ACTIVE))
    return true; // No syncing to external clock
  if (sampleControls->interpolationMode != INTERPOLATION_MODE_SMOOTH) return true;

  bool mayCreate =
      (sampleControls->getInterpolationBufferSize(phaseIncrement) == INTERPOLATION_MAX_NUM_SAMPLES);
  cache = ((Sample*)guide->audioFileHolder->audioFile)
              ->getOrCreateCache((SampleHolder*)guide->audioFileHolder,
                                 phaseIncrement,
                                 timeStretchRatio,
                                 guide->playDirection == -1,
                                 mayCreate,
  ...
  if (cache) {
    ...
    bool result = reassessReassessmentLocation(guide,
                                               (Sample*)guide->audioFileHolder->audioFile,
                                               priorityRating);
    // Probably no need to check we haven't shot past the new
    // reassessmentLocation, since surely that's going to be further into the
    // future now we're not obeying loop points at low level?
