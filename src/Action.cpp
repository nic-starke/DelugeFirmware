...
int Action::revert(int time, ModelStack* modelStack) {
  ...
  // If we're a record-arrangement-from-session Action, there's a trick - we
  // know that whether we're being undone or redone, this will involve clearing
  // the arrangement to the right of a certain pos. So we'll do that, and we'll
  // record the Consequences involved in doing so, so that this Action can then
  // be reverted in the opposite direction next time
  ...
  while (thisConsequence) {
    if (!error) {
      ...
      // Can't quite remember why, but we don't wanna revert param changes for
      // arrangement-record actions
      if (type == ACTION_ARRANGEMENT_RECORD && thisConsequence->type == CONSEQUENCE_PARAM_CHANGE) {
        ...
        // If an error occurs, keep swapping the order cos it's too late to
        // stop, but don't keep calling the things
      }
    }
    ...
    if (type == ACTION_ARRANGEMENT_RECORD) {
      ...
      thisConsequence->prepareForDestruction(
          AFTER,
          modelStack->song); // Have to put AFTER. See the effect this will have
                             // in ConsequenceCDelete::prepareForDestruction()
    ...
    // Or, normal case
      ...
      // Reverse the order, for next time we revert this, which will be in the
      // other direction
  ...
  if (type != ACTION_ARRANGEMENT_RECORD) { firstConsequence = newFirstConsequence; }
...
void Action::recordParamChangeIfNotAlreadySnapshotted(ModelStackWithAutoParam const* modelStack,
  ...
  if (containsConsequenceParamChange(modelStack->paramCollection, modelStack->paramId)) {
    ...
    // Except, if we were planning to steal the data, well we'd better pretend
    // we've just done that by deleting it instead.
    if (stealData) { modelStack->autoParam->nodes.empty(); }
    return;
  }
...
void Action::recordParamChangeDefinitely(ModelStackWithAutoParam const* modelStack,
  ...
  if (consMemory) {
    ConsequenceParamChange* newCons =
        new (consMemory) ConsequenceParamChange(modelStack, stealData);
...
bool Action::containsConsequenceNoteArrayChange(InstrumentClip* clip,
  ...
  for (Consequence** prevPointer = &firstConsequence; *prevPointer;
...
int Action::recordNoteArrayChangeDefinitely(InstrumentClip* clip,
  ...
  return NO_ERROR; // Though we wouldn't know if there was a RAM error as
                   // ConsequenceNoteArrayChange tried to clone the data...
}
...
void Action::recordClipLengthChange(Clip* clip, int32_t oldLength) {
  ...
  if (consMemory) {
    ConsequenceClipLength* consequenceClipLength =
        new (consMemory) ConsequenceClipLength(clip, oldLength);
...
bool Action::recordClipExistenceChange(Song* song, ClipArray* clipArray, Clip* clip, int type) {
  ...
  ConsequenceClipExistence* consequence =
      new (consMemory) ConsequenceClipExistence(clip, clipArray, type);
  if (type == DELETE) {
...
void Action::updateYScrollClipViewAfter(InstrumentClip* clip) {
  ...
  if (numClipStates != currentSong->sessionClips.getNumElements() +
