...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = storageManager.devVarA;
  }
  void writeCurrentValue() {
    storageManager.devVarA = soundEditor.currentValue;
  }
  int getMaxValue() {
    return 512;
  }
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = storageManager.devVarB;
  }
  void writeCurrentValue() {
    storageManager.devVarB = soundEditor.currentValue;
  }
  int getMaxValue() {
    return 512;
  }
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = storageManager.devVarC;
  }
  void writeCurrentValue() {
    storageManager.devVarC = soundEditor.currentValue;
  }
  int getMaxValue() {
    return 1024;
  }
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = storageManager.devVarD;
  }
  void writeCurrentValue() {
    storageManager.devVarD = soundEditor.currentValue;
  }
  int getMaxValue() {
    return 1024;
  }
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = storageManager.devVarE;
  }
  void writeCurrentValue() {
    storageManager.devVarE = soundEditor.currentValue;
  }
  int getMaxValue() {
    return 1024;
  }
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = storageManager.devVarF;
  }
  void writeCurrentValue() {
    storageManager.devVarF = soundEditor.currentValue;
  }
  int getMaxValue() {
    return 1024;
  }
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = storageManager.devVarG;
  }
  void writeCurrentValue() {
    storageManager.devVarG = soundEditor.currentValue;
  }
  int getMaxValue() {
    return 1024;
  }
...
public:
  MenuItemModulatorSubmenu(char const* newName        = NULL,
                           MenuItem**  newFirstItem   = NULL,
...
public:
  MenuItemActualSourceSubmenu(char const* newName        = 0,
                              MenuItem**  newItems       = 0,
                              int         newSourceIndex = 0)
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = soundEditor.currentSound->transpose;
  }
  void writeCurrentValue() {
  ...
  uint8_t getPatchedParamIndex() {
    return PARAM_LOCAL_PITCH_ADJUST;
  }
  uint8_t getP() {
    return PARAM_LOCAL_PITCH_ADJUST;
  }
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = soundEditor.currentSound->synthMode;
  }
  void writeCurrentValue() {
...
public:
  MenuItemBendSubmenu(char const* newName = NULL, MenuItem** newItems = NULL)
  ...
  bool isRelevant(Sound* sound, int whichThing) {
    return (currentSong->currentClip->output->type == INSTRUMENT_TYPE_SYNTH ||
            currentSong->currentClip->output->type ==
                INSTRUMENT_TYPE_CV); // Drums within a Kit don't need the two-item
                                     // submenu - they have their own single item.
  }
} bendMenu;
...
class MenuItemLPFFreq final : public MenuItemPatchedParamIntegerNonFM{
  public : MenuItemLPFFreq(char const* newName = 0, int newP = 0) :
      MenuItemPatchedParamIntegerNonFM(newName, newP){}
#if !HAVE_OLED
  void drawValue() {
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = soundEditor.currentModControllable->lpfMode;
  }
  void writeCurrentValue() {
    soundEditor.currentModControllable->lpfMode = soundEditor.currentValue;
  }
  char const** getOptions() {
  ...
  int getNumOptions() {
    return NUM_LPF_MODES;
  }
  bool isRelevant(Sound* sound, int whichThing) {
    return (!sound || sound->synthMode != SYNTH_MODE_FM);
  }
} lpfModeMenu;
...
class MenuItemHPFFreq final : public MenuItemPatchedParamIntegerNonFM{
  public : MenuItemHPFFreq(char const* newName = 0, int newP = 0) :
      MenuItemPatchedParamIntegerNonFM(newName, newP){}
#if !HAVE_OLED
  void drawValue() {
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = soundEditor.currentSound->polyphonic;
  }
  void writeCurrentValue() {
  ...
  int getNumOptions() { // Hack-ish way of hiding the "choke" option when not
                        // editing a Kit
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = *soundEditor.currentPriority;
  }
  void writeCurrentValue() {
    *soundEditor.currentPriority = soundEditor.currentValue;
  }
  char const** getOptions() {
...
public:
  MenuItemRetriggerPhase(char const* newName = NULL, bool newForModulator = false)
      : MenuItemDecimal(newName) {
...
public:
  ...
  bool isRelevant(Sound* sound, int whichThing) {
    return (sound->getSynthMode() != SYNTH_MODE_RINGMOD);
  }
} sourceVolumeMenu;
...
public:
  ...
  void writeCurrentValue() {
    ...
    if (oldValue == OSC_TYPE_INPUT_L || oldValue == OSC_TYPE_INPUT_R ||
        oldValue == OSC_TYPE_INPUT_STEREO || oldValue == OSC_TYPE_SAMPLE ||
        oldValue == OSC_TYPE_WAVETABLE // Haven't actually really determined if
                                       // this needs to be here - maybe not?

        || newValue == OSC_TYPE_INPUT_L || newValue == OSC_TYPE_INPUT_R ||
        newValue == OSC_TYPE_INPUT_STEREO || newValue == OSC_TYPE_SAMPLE ||
        newValue == OSC_TYPE_WAVETABLE) { // Haven't actually really determined if this
                                          // needs to be here - maybe not?
  ...
  // char const** getOptions() { static char const* options[] = {"SINE",
  // "TRIANGLE", "SQUARE", "SAW", "MMS1", "SUB1", "SAMPLE", "INL", "INR",
  // "INLR", "SQ50", "SQ02", "SQ01", "SUB2", "SQ20", "SA50", "S101", "S303",
  // "MMS2", "MMS3", "TABLE"}; return options; }
  char const** getOptions() {
#if HAVE_OLED
    ...
    static char const* options[] = {"SINE",
                                    ...
                                    "Input (stereo)",
                                    NULL};
    inLText[5]                   = ((AudioEngine::micPluggedIn || AudioEngine::lineInPluggedIn ||
                   DELUGE_MODEL == DELUGE_MODEL_40_PAD)) ?
                                       ' ' :
                                       0;
#else
    ...
    static char const* options[]  = {"SINE",
                                     ...
                                     "INR",
                                     "INLR"};
    inLText[2]                    = ((AudioEngine::micPluggedIn || AudioEngine::lineInPluggedIn ||
                   DELUGE_MODEL == DELUGE_MODEL_40_PAD)) ?
                                        'L' :
                                        0;
#endif
...
public:
  ...
  int checkPermissionToBeginSession(Sound* sound, int whichThing, MultiRange** currentRange) {
    ...
    return soundEditor.checkPermissionToBeginSessionForRangeSpecificParam(sound,
                                                                          whichThing,
                                                                          false,
                                                                          currentRange);
  }
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = soundEditor.currentSource->sampleControls.reversed;
  }
  void writeCurrentValue() {
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = soundEditor.currentSource->repeatMode;
  }
  void writeCurrentValue() {
    ...
    if (currentUIMode == UI_MODE_HOLDING_AFFECT_ENTIRE_IN_SOUND_EDITOR &&
      ...
      for (Drum* thisDrum = kit->firstDrum; thisDrum; thisDrum = thisDrum->next) {
        if (thisDrum->type == DRUM_TYPE_SOUND) {
          ...
          // Automatically switch pitch/speed independence on / off if
          // stretch-to-note-length mode is selected
    ...
    // We need to re-render all rows, because this will have changed whether
    // Note tails are displayed. Probably just one row, but we don't know which
  ...
  int getNumOptions() {
    return NUM_REPEAT_MODES;
  }
} sampleRepeatMenu;

class MenuItemSampleStart final : public MenuItemSampleLoopPoint {
public:
  MenuItemSampleStart(char const* newName = NULL) : MenuItemSampleLoopPoint(newName) {
...
public:
  MenuItemSampleEnd(char const* newName = NULL) : MenuItemSampleLoopPoint(newName) {
...
public:
  ...
  void readCurrentValue() {
    ...
    if (soundEditor.currentMultiRange &&
        soundEditor.currentSound->getSynthMode() != SYNTH_MODE_FM &&
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = soundEditor.currentSource->timeStretchAmount;
  }
  void writeCurrentValue() {
...
public:
  MenuItemPulseWidth(char const* newName = 0, int newP = 0)
      : MenuItemSourceDependentPatchedParam(newName, newP) {}
  ...
  void readCurrentValue() {
    soundEditor.currentValue =
        ((int64_t)soundEditor.currentParamManager->getPatchedParamSet()->getValue(getP()) * 100 +
         2147483648) >>
        32;
  }
  bool isRelevant(Sound* sound, int whichThing) {
    ...
    return (oscType != OSC_TYPE_SAMPLE && oscType != OSC_TYPE_INPUT_L &&
            oscType != OSC_TYPE_INPUT_R && oscType != OSC_TYPE_INPUT_STEREO);
  }
} pulseWidthMenu;
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = soundEditor.currentSound->oscillatorSync;
  }
  void writeCurrentValue() {
    soundEditor.currentSound->oscillatorSync = soundEditor.currentValue;
  }
  bool isRelevant(Sound* sound, int whichThing) {
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = soundEditor.currentSound->numUnison;
  }
  void writeCurrentValue() {
    char                      modelStackMemory[MODEL_STACK_MAX_SIZE];
    ModelStackWithSoundFlags* modelStack =
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = soundEditor.currentSound->unisonDetune;
  }
  void writeCurrentValue() {
    char                      modelStackMemory[MODEL_STACK_MAX_SIZE];
    ModelStackWithSoundFlags* modelStack =
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = soundEditor.currentArpSettings->mode;
  }
  void writeCurrentValue() {

    // If was off, or is now becoming off...
    if (soundEditor.currentArpSettings->mode == ARP_MODE_OFF ||
        soundEditor.currentValue == ARP_MODE_OFF) {
      if (currentSong->currentClip->isActiveOnOutput()) {
        char                           modelStackMemory[MODEL_STACK_MAX_SIZE];
        ModelStackWithThreeMainThings* modelStack =
            soundEditor.getCurrentModelStack(modelStackMemory);
        ...
        if (soundEditor.editingCVOrMIDIClip()) {
          ...
          soundEditor.currentSound->allNotesOff(
              ...
              soundEditor.currentSound->getArp()); // Must switch off all notes when switching arp
                                                   // on / off
    ...
    // Only update the Clip-level arp setting if they hadn't been playing with
    // other synth parameters first (so it's clear that switching the arp on or
    // off was their main intention)
    if (!soundEditor.editingKit()) {
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = soundEditor.currentArpSettings->syncLevel;
  }
  void writeCurrentValue() {
    soundEditor.currentArpSettings->syncLevel = soundEditor.currentValue;
  }
} arpSyncMenu;
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = soundEditor.currentArpSettings->numOctaves;
  }
  void writeCurrentValue() {
    soundEditor.currentArpSettings->numOctaves = soundEditor.currentValue;
  }
  int getMinValue() {
    return 1;
  }
...
public:
  MenuItemArpGate(char const* newName = NULL, int newP = 0)
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue =
        (((int64_t)((InstrumentClip*)currentSong->currentClip)->arpeggiatorGate + 2147483648) * 50 +
         2147483648) >>
        32;
  }
  void writeCurrentValue() {
...
public:
  MenuItemArpRate(char const* newName = NULL, int newP = 0)
      : MenuItemPatchedParamInteger(newName, newP) {}
  bool isRelevant(Sound* sound, int whichThing) {
    return !soundEditor.editingCVOrMIDIClip();
  }
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue =
        (((int64_t)((InstrumentClip*)currentSong->currentClip)->arpeggiatorRate + 2147483648) * 50 +
         2147483648) >>
        32;
  }
  void writeCurrentValue() {
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue =
        (int32_t)soundEditor.currentSound->modulatorTranspose[soundEditor.currentSourceIndex] *
            100 +
  ...
  void writeCurrentValue() {
    ...
    soundEditor.currentSound->setModulatorTranspose(soundEditor.currentSourceIndex,
                                                    semitones - 256,
                                                    modelStack);
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = soundEditor.currentSound->modulator1ToModulator0;
  }
  void writeCurrentValue() {
    soundEditor.currentSound->modulator1ToModulator0 = soundEditor.currentValue;
  }
  char const** getOptions() {
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = soundEditor.currentSound->lfoGlobalWaveType;
  }
  void writeCurrentValue() {
    soundEditor.currentSound->setLFOGlobalWave(soundEditor.currentValue);
  }
} lfo1TypeMenu;

class MenuItemLFO1Rate final : public MenuItemPatchedParamInteger {
public:
  MenuItemLFO1Rate(char const* newName = 0, int newP = 0)
      : MenuItemPatchedParamInteger(newName, newP) {}
  bool isRelevant(Sound* sound, int whichThing) {
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = soundEditor.currentSound->lfoGlobalSyncLevel;
  }
  void writeCurrentValue() {
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = soundEditor.currentSound->lfoLocalWaveType;
  }
  void writeCurrentValue() {
    soundEditor.currentSound->lfoLocalWaveType = soundEditor.currentValue;
  }
} lfo2TypeMenu;
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = soundEditor.currentModControllable->clippingAmount;
  }
  void writeCurrentValue() {
    soundEditor.currentModControllable->clippingAmount = soundEditor.currentValue;
  }
  int getMaxValue() {
    return 15;
  }
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = soundEditor.currentModControllable->modFXType;
  }
  void writeCurrentValue() {
...
public:
  MenuItemModFXFeedback(char const* newName = 0, int newP = 0)
      : MenuItemUnpatchedParam(newName, newP) {}
  bool isRelevant(Sound* sound, int whichThing) {
    return (!sound || sound->modFXType == MOD_FX_TYPE_FLANGER ||
            sound->modFXType == MOD_FX_TYPE_PHASER); // TODO: really want to receive a
                                                     // ModControllableAudio here!
  }
} modFXFeedbackMenu;

class MenuItemModFXDepth final : public MenuItemPatchedParamInteger {
public:
  MenuItemModFXDepth(char const* newName = 0, int newP = 0)
      : MenuItemPatchedParamInteger(newName, newP) {}
  bool isRelevant(Sound* sound, int whichThing) {
...
public:
  MenuItemModFXOffset(char const* newName = 0, int newP = 0)
      : MenuItemUnpatchedParam(newName, newP) {}
  bool isRelevant(Sound* sound, int whichThing) {
    return (!sound || sound->modFXType == MOD_FX_TYPE_CHORUS); // TODO: really want to receive a
                                                               // ModControllableAudio here!
  }
} modFXOffsetMenu;
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = soundEditor.currentModControllable->delay.pingPong;
  }
  void writeCurrentValue() {
    soundEditor.currentModControllable->delay.pingPong = soundEditor.currentValue;
  }
} delayPingPongMenu;
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = soundEditor.currentModControllable->delay.analog;
  }
  void writeCurrentValue() {
    soundEditor.currentModControllable->delay.analog = soundEditor.currentValue;
  }
  char const** getOptions() {
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = soundEditor.currentModControllable->delay.sync;
  }
  void writeCurrentValue() {
    soundEditor.currentModControllable->delay.sync = soundEditor.currentValue;
  }
} delaySyncMenu;
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = round(AudioEngine::reverb.getroomsize() * 50);
  }
  void writeCurrentValue() {
    AudioEngine::reverb.setroomsize((float)soundEditor.currentValue / 50);
  }
  int getMaxValue() {
    return 50;
  }
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = round(AudioEngine::reverb.getdamp() * 50);
  }
  void writeCurrentValue() {
    AudioEngine::reverb.setdamp((float)soundEditor.currentValue / 50);
  }
  int getMaxValue() {
    return 50;
  }
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = round(AudioEngine::reverb.getwidth() * 50);
  }
  void writeCurrentValue() {
    AudioEngine::reverb.setwidth((float)soundEditor.currentValue / 50);
  }
  int getMaxValue() {
    return 50;
  }
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue =
        ((uint64_t)soundEditor.currentSound->sideChainSendLevel * 50 + 1073741824) >> 31;
  }
  void writeCurrentValue() {
    if (soundEditor.currentValue == 50)
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = AudioEngine::reverbCompressorVolume / 21474836;
  }
  void writeCurrentValue() {
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = soundEditor.currentCompressor->sync;
  }
  void writeCurrentValue() {
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue =
        (((int64_t)AudioEngine::reverbCompressorShape + 2147483648) * 50 + 2147483648) >> 32;
  }
  void writeCurrentValue() {
  ...
  bool isRelevant(Sound* sound, int whichThing) {
    return (AudioEngine::reverbCompressorVolume >= 0);
  }
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = ((InstrumentClip*)currentSong->currentClip)->midiBank;
  }
  void writeCurrentValue() {
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = ((InstrumentClip*)currentSong->currentClip)->midiSub;
  }
  void writeCurrentValue() {
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = ((InstrumentClip*)currentSong->currentClip)->midiPGM;
  }
  void writeCurrentValue() {
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue =
        ((AudioClip*)currentSong->currentClip)->sampleHolder.transpose * 100 +
  ...
  ParamDescriptor getLearningThing() {
    ...
    paramDescriptor.setToHaveParamOnly(PARAM_UNPATCHED_SECTION +
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue =
        (((int64_t)((AudioClip*)currentSong->currentClip)->attack + 2147483648) * 50 +
         2147483648) >>
        32;
  }
  void writeCurrentValue() {
    ((AudioClip*)currentSong->currentClip)->attack =
        (uint32_t)soundEditor.currentValue * 85899345 - 2147483648;
  }
  int getMaxValue() {
    return 50;
...
class MenuItemAudioClipLPFFreq final : public MenuItemUnpatchedParam{
  public : MenuItemAudioClipLPFFreq(char const* newName = 0, int newP = 0) :
      MenuItemUnpatchedParam(newName, newP){}
#if !HAVE_OLED
  void drawValue() {
    if (soundEditor.currentValue == 50) {
...
class MenuItemAudioClipHPFFreq final : public MenuItemUnpatchedParam{
  public : MenuItemAudioClipHPFFreq(char const* newName = 0, int newP = 0) :
      MenuItemUnpatchedParam(newName, newP){}
#if !HAVE_OLED
  void drawValue() {
    if (soundEditor.currentValue == 0) {
...
public:
  ...
  void selectEncoderAction(int offset) { // We override this to set min value to 1. We don't inherit
                                         // any getMinValue() function to override more easily
    soundEditor.currentValue += offset;
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = cvEngine.minGateOffTime;
  }
  void writeCurrentValue() {
    cvEngine.minGateOffTime = soundEditor.currentValue;
  }
} gateOffTimeMenu;
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue =
        (int32_t)cvEngine.cvChannels[soundEditor.currentSourceIndex].transpose * 100 +
...
public:
  MenuItemGateSelection(char const* newName = NULL) : MenuItemSelection(newName) {
    ...
    static char const* options[] = {"Gate output 1",
                                    "Gate output 2",
                                    "Gate output 3",
                                    "Gate output 4",
                                    "Minimum off-time",
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = currentSong->swingInterval;
  }
  void writeCurrentValue() {
    currentSong->changeSwingInterval(soundEditor.currentValue);
  }
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = FlashStorage::recordQuantizeLevel;
  }
  void writeCurrentValue() {
    FlashStorage::recordQuantizeLevel = soundEditor.currentValue;
  }
} recordQuantizeMenu;
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = FlashStorage::audioClipRecordMargins;
  }
  void writeCurrentValue() {
    FlashStorage::audioClipRecordMargins = soundEditor.currentValue;
  }
} recordMarginsMenu;
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = playbackHandler.countInEnabled;
  }
  void writeCurrentValue() {
    playbackHandler.countInEnabled = soundEditor.currentValue;
  }
} recordCountInMenu;
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = PadLEDs::flashCursor;
  }
  void writeCurrentValue() {
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = AudioEngine::inputMonitoringMode;
  }
  void writeCurrentValue() {
    AudioEngine::inputMonitoringMode = soundEditor.currentValue;
  }
  char const** getOptions() {
  ...
  int getNumOptions() {
    return NUM_INPUT_MONITORING_MODES;
  }
} monitorModeMenu;
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = FlashStorage::sampleBrowserPreviewMode;
  }
  void writeCurrentValue() {
    FlashStorage::sampleBrowserPreviewMode = soundEditor.currentValue;
  }
  char const** getOptions() {
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = soundEditor.shortcutsVersion;
  }
  void writeCurrentValue() {
    soundEditor.setShortcutsVersion(soundEditor.currentValue);
  }
  char const** getOptions() {
    static char const* options[] = {
#if HAVE_OLED
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = FlashStorage::keyboardLayout;
  }
  void writeCurrentValue() {
    FlashStorage::keyboardLayout = soundEditor.currentValue;
  }
  char const** getOptions() {
    static char const* options[] = {
      "QWERTY",
...
class MenuItemFirmwareVersion final
    : public
      MenuItem{public : MenuItemFirmwareVersion(char const* newName = 0) : MenuItem(newName){}
#if HAVE_OLED
               void drawPixelsForOled() {
                 OLED::drawStringCentredShrinkIfNecessary(firmwareString,
                                                          22,
                                                          OLED::oledMainImage[0],
                                                          OLED_MAIN_WIDTH_PIXELS,
                                                          18,
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = midiEngine.midiThru;
  }
  void writeCurrentValue() {
    midiEngine.midiThru = soundEditor.currentValue;
  }
} midiThruMenu;
...
public:
  ...
  void writeCurrentValue() {
    ...
    currentSong->grabVelocityToLevelFromMIDIDeviceAndSetupPatchingForEverything(
        soundEditor.currentMIDIDevice);
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = MIDIDeviceManager::differentiatingInputsByDevice;
  }
  void writeCurrentValue() {
    MIDIDeviceManager::differentiatingInputsByDevice = soundEditor.currentValue;
  }
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = playbackHandler.midiOutClockEnabled;
  }
  void writeCurrentValue() {
    playbackHandler.setMidiOutClockMode(soundEditor.currentValue);
  }
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = playbackHandler.midiInClockEnabled;
  }
  void writeCurrentValue() {
    playbackHandler.setMidiInClockEnabled(soundEditor.currentValue);
  }
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = playbackHandler.tempoMagnitudeMatchingEnabled;
  }
  void writeCurrentValue() {
    playbackHandler.tempoMagnitudeMatchingEnabled = soundEditor.currentValue;
  }
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = playbackHandler.analogInTicksPPQN;
  }
  void writeCurrentValue() {
    ...
    if ((playbackHandler.playbackState & PLAYBACK_CLOCK_EXTERNAL_ACTIVE) &&
        playbackHandler.usingAnalogClockInput)
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = playbackHandler.analogClockInputAutoStart;
  }
  void writeCurrentValue() {
    playbackHandler.analogClockInputAutoStart = soundEditor.currentValue;
  }
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = playbackHandler.analogOutTicksPPQN;
  }
  void writeCurrentValue() {
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = FlashStorage::defaultScale;
  }
  void writeCurrentValue() {
    FlashStorage::defaultScale = soundEditor.currentValue;
  }
  int getNumOptions() {
    return NUM_PRESET_SCALES + 2;
  }
  char const** getOptions() {
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = FlashStorage::defaultVelocity;
  }
  void writeCurrentValue() {
...
public:
  ...
  void readCurrentValue() {
    soundEditor.currentValue = FlashStorage::defaultMagnitude;
  }
  void writeCurrentValue() {
...
SoundEditor::SoundEditor() {
  ...
  new (&tempoMagnitudeMatchingMenu)
      MenuItemTempoMagnitudeMatching(HAVE_OLED ? "Tempo magnitude matching" : "MAGN");
  static MenuItem* midiClockMenuItems[] = {&midiClockInStatusMenu,
                                           &midiClockOutStatusMenu,
                                           &tempoMagnitudeMatchingMenu,
                                           NULL};

  // MIDI commands menu
  new (&playbackRestartMidiCommand)
      MenuItemMidiCommand("Restart", GLOBAL_MIDI_COMMAND_PLAYBACK_RESTART);
  new (&playMidiCommand) MenuItemMidiCommand("PLAY", GLOBAL_MIDI_COMMAND_PLAY);
  new (&recordMidiCommand)
      MenuItemMidiCommand(HAVE_OLED ? "Record" : "REC", GLOBAL_MIDI_COMMAND_RECORD);
  ...
  static MenuItem* midiMenuItems[] = {&midiClockMenu,
                                      &midiThruMenu,
                                      &midiCommandsMenu,
                                      &midiInputDifferentiationMenu,
                                      &midiDevicesMenu,
  ...
  static MenuItem* midiDeviceMenuItems[] = {&mpeDirectionSelectorMenu,
                                            &defaultVelocityToLevelMenu,
                                            NULL};
  ...
  new (&triggerClockOutMenu)
      MenuItemSubmenu(HAVE_OLED ? "Output" : "OUT", triggerClockOutMenuItems);
  ...
  static MenuItem* defaultsMenuItems[] = {&defaultTempoMenu,
                                          &defaultSwingMenu,
                                          &defaultKeyMenu,
                                          &defaultScaleMenu,
                                          &defaultVelocityMenu,
  ...
  static MenuItem* recordMenuItems[] = {&recordCountInMenu,
                                        &recordQuantizeMenu,
                                        &recordMarginsMenu,
                                        &monitorModeMenu,
                                        NULL};
  ...
  static MenuItem* coloursMenuItems[] = {&activeColourMenu,
                                         &stoppedColourMenu,
                                         &mutedColourMenu,
                                         &soloColourMenu,
                                         NULL};
  ...
  static MenuItem* layoutMenuItems[] = {&shortcutsVersionMenu,
                                        &keyboardLayoutMenu,
                                        &coloursSubmenu,
                                        NULL};
  ...
  new (&triggerClockMenu)
      MenuItemSubmenu(HAVE_OLED ? "Trigger clock" : "TCLOCK", triggerClockMenuItems);
  ...
  new (&sampleBrowserPreviewModeMenu)
      MenuItemSampleBrowserPreviewMode(HAVE_OLED ? "Sample preview" : "PREV");
  ...
  // Modulator menu
  new (&modulatorTransposeMenu)
      MenuItemModulatorTranspose("Transpose", PARAM_LOCAL_MODULATOR_0_PITCH_ADJUST);
  new (&modulatorVolume) MenuItemSourceDependentPatchedParamFM(HAVE_OLED ? "Level" : "AMOUNT",
                                                               PARAM_LOCAL_MODULATOR_0_VOLUME);
  new (&modulatorFeedbackMenu)
      MenuItemSourceDependentPatchedParamFM("FEEDBACK", PARAM_LOCAL_MODULATOR_0_FEEDBACK);
  ...
  static MenuItem* modulatorMenuItems[] = {&modulatorVolume,
                                           &modulatorTransposeMenu,
                                           &modulatorFeedbackMenu,
                                           &modulatorDestMenu,
                                           &modulatorPhaseMenu,
  ...
  new (&sourceVolumeMenu)
      MenuItemSourceVolume(HAVE_OLED ? "Level" : "VOLUME", PARAM_LOCAL_OSC_A_VOLUME);
  ...
  static MenuItem* oscMenuItems[] = {&oscTypeMenu,
                                     &sourceVolumeMenu,
                                     &sourceWaveIndexMenu,
                                     &sourceFeedbackMenu,
                                     &fileSelectorMenu,
  ...
  static MenuItem* envMenuItems[] = {&envAttackMenu,
                                     &envDecayMenu,
                                     &envSustainMenu,
                                     &envReleaseMenu,
                                     NULL};
  ...
  static MenuItem* arpMenuItems[] = {&arpModeMenu,
                                     &arpSyncMenu,
                                     &arpOctavesMenu,
                                     &arpGateMenu,
                                     &arpGateMenuMIDIOrCV,
  ...
  static MenuItem* voiceMenuItems[] =
      {&polyphonyMenu, &unisonMenu, &portaMenu, &arpMenu, &priorityMenu, NULL};
  ...
  static MenuItem* modFXMenuItems[] =
      {&modFXTypeMenu, &modFXRateMenu, &modFXFeedbackMenu, &modFXDepthMenu, &modFXOffsetMenu, NULL};
  ...
  new (&bassFreqMenu)
      MenuItemUnpatchedParam(HAVE_OLED ? "Bass frequency" : "BAFR", PARAM_UNPATCHED_BASS_FREQ);
  new (&trebleFreqMenu)
      MenuItemUnpatchedParam(HAVE_OLED ? "Treble frequency" : "TRFR", PARAM_UNPATCHED_TREBLE_FREQ);
  ...
  static MenuItem* delayMenuItems[] = {&delayFeedbackMenu,
                                       &delayRateMenu,
                                       &delayPingPongMenu,
                                       &delayAnalogMenu,
                                       &delaySyncMenu,
  ...
  new (&compressorVolumeShortcutMenu)
      MenuItemCompressorVolumeShortcut("Volume ducking",
  ...
  new (&compressorShapeMenu)
      MenuItemUnpatchedParamUpdatingReverbParams("SHAPE", PARAM_UNPATCHED_COMPRESSOR_SHAPE);
  ...
  static MenuItem* sidechainMenuItemsForSound[]  = {&sidechainSendMenu,
                                                    ...
                                                    &compressorShapeMenu,
                                                    NULL};
  static MenuItem* sidechainMenuItemsForReverb[] = {&reverbCompressorVolumeMenu,
                                                    &sidechainSyncMenu,
                                                    &compressorAttackMenu,
                                                    &compressorReleaseMenu,
                                                    &reverbCompressorShapeMenu,
  ...
  new (&reverbCompressorMenu) MenuItemCompressorSubmenu(HAVE_OLED ? "Reverb sidechain" : "SIDE",
                                                        sidechainMenuItemsForReverb,
                                                        true);
  ...
  new (&bitcrushMenu)
      MenuItemUnpatchedParam(HAVE_OLED ? "Bitcrush" : "CRUSH", PARAM_UNPATCHED_BITCRUSHING);
  static MenuItem* fxMenuItems[] =
      {&modFXMenu, &eqMenu, &delayMenu, &reverbMenu, &clippingMenu, &srrMenu, &bitcrushMenu, NULL};

  // Bend ranges
  new (&mainBendRangeMenu) MenuItemBendRangeMain("Normal");
  new (&perFingerBendRangeMenu)
      MenuItemBendRangePerFinger(HAVE_OLED ? "Poly / finger / MPE" : "MPE");
  static MenuItem* bendMenuItems[] = {&mainBendRangeMenu, &perFingerBendRangeMenu, NULL};

  // Clip-level stuff
  new (&sequenceDirectionMenu)
      MenuItemSequenceDirection(HAVE_OLED ? "Play direction" : "DIRECTION");

  // Root menu
  new (&source0Menu)
      MenuItemActualSourceSubmenu(HAVE_OLED ? "Oscillator 1" : "OSC1", oscMenuItems, 0);
  new (&source1Menu)
      MenuItemActualSourceSubmenu(HAVE_OLED ? "Oscillator 2" : "OSC2", oscMenuItems, 1);
  new (&modulator0Menu)
      MenuItemModulatorSubmenu(HAVE_OLED ? "FM modulator 1" : "MOD1", modulatorMenuItems, 0);
  new (&modulator1Menu)
      MenuItemModulatorSubmenu(HAVE_OLED ? "FM modulator 2" : "MOD2", modulatorMenuItems, 1);
  new (&masterTransposeMenu) MenuItemMasterTranspose(HAVE_OLED ? "Master transpose" : "TRANSPOSE");
  new (&vibratoMenu)
      MenuItemFixedPatchCableStrength("VIBRATO", PARAM_LOCAL_PITCH_ADJUST, PATCH_SOURCE_LFO_GLOBAL);
  new (&noiseMenu) MenuItemPatchedParamIntegerNonFM(HAVE_OLED ? "Noise level" : "NOISE",
  ...
  new (&compressorMenu)
      MenuItemCompressorSubmenu("Sidechain compressor", sidechainMenuItemsForSound, false);
  new (&bendMenu) MenuItemBendSubmenu("Bend range", bendMenuItems); // The submenu
  new (&drumBendRangeMenu)
      MenuItemBendRangePerFinger("Bend range"); // The single option available for Drums
  new (&volumeMenu)
      MenuItemPatchedParamInteger(HAVE_OLED ? "Level" : "VOLUME", PARAM_GLOBAL_VOLUME_POST_FX);
  ...
  static MenuItem* soundEditorRootMenuItemsMIDIOrCV[] = {&midiPGMMenu,
                                                         &midiBankMenu,
                                                         &midiSubMenu,
                                                         &arpMenu,
                                                         &bendMenu,
  ...
  new (&soundEditorRootMenuMIDIOrCV)
      MenuItemSubmenu("MIDI inst.", soundEditorRootMenuItemsMIDIOrCV);
  ...
  new (&audioClipSampleMarkerEditorMenuEnd)
      MenuItemAudioClipSampleMarkerEditor("WAVEFORM", MARKER_END);
  ...
  // LPF menu
  new (&audioClipLPFFreqMenu)
      MenuItemAudioClipLPFFreq("Frequency", PARAM_UNPATCHED_GLOBALEFFECTABLE_LPF_FREQ);
  new (&audioClipLPFResMenu)
      MenuItemUnpatchedParam("Resonance", PARAM_UNPATCHED_GLOBALEFFECTABLE_LPF_RES);
  static MenuItem* audioClipLPFMenuItems[] = {&audioClipLPFFreqMenu,
                                              &audioClipLPFResMenu,
                                              &lpfModeMenu,
                                              NULL};

  // HPF menu
  new (&audioClipHPFFreqMenu)
      MenuItemAudioClipHPFFreq("Frequency", PARAM_UNPATCHED_GLOBALEFFECTABLE_HPF_FREQ);
  new (&audioClipHPFResMenu)
      MenuItemUnpatchedParam("Resonance", PARAM_UNPATCHED_GLOBALEFFECTABLE_HPF_RES);
  ...
  new (&audioClipModFXRateMenu)
      MenuItemUnpatchedParam("RATE", PARAM_UNPATCHED_GLOBALEFFECTABLE_MOD_FX_RATE);
  new (&audioClipModFXDepthMenu)
      MenuItemUnpatchedParam("DEPTH", PARAM_UNPATCHED_GLOBALEFFECTABLE_MOD_FX_DEPTH);
  static MenuItem* audioClipModFXMenuItems[] = {&audioClipModFXTypeMenu,
                                                &audioClipModFXRateMenu,
                                                &modFXFeedbackMenu,
                                                &audioClipModFXDepthMenu,
                                                &modFXOffsetMenu,
  ...
  // Delay menu
  new (&audioClipDelayFeedbackMenu)
      MenuItemUnpatchedParam("AMOUNT", PARAM_UNPATCHED_GLOBALEFFECTABLE_DELAY_AMOUNT);
  new (&audioClipDelayRateMenu)
      MenuItemUnpatchedParam("RATE", PARAM_UNPATCHED_GLOBALEFFECTABLE_DELAY_RATE);
  ...
  static MenuItem* audioClipFXMenuItems[] = {&audioClipModFXMenu,
                                             &eqMenu,
                                             &audioClipDelayMenu,
                                             &audioClipReverbMenu,
                                             &clippingMenu,
  ...
  new (&audioClipCompressorVolumeMenu)
      MenuItemUnpatchedParamUpdatingReverbParams("Volume ducking",
                                                 PARAM_UNPATCHED_GLOBALEFFECTABLE_SIDECHAIN_VOLUME);
  static MenuItem* audioClipSidechainMenuItems[] = {&audioClipCompressorVolumeMenu,
                                                    &sidechainSyncMenu,
                                                    &compressorAttackMenu,
                                                    &compressorReleaseMenu,
                                                    &compressorShapeMenu,
  ...
  new (&audioClipCompressorMenu)
      MenuItemSubmenu("Sidechain compressor", audioClipSidechainMenuItems);
  new (&audioClipLevelMenu) MenuItemUnpatchedParam(HAVE_OLED ? "Level" : "VOLUME",
  ...
  new (&soundEditorRootMenuAudioClip)
      MenuItemSubmenu("Audio clip", soundEditorRootMenuItemsAudioClip);
...
bool SoundEditor::opened() {
  bool success = beginScreen(); // Could fail for instance if going into
                                // WaveformView but sample not found on card, or
                                // going into SampleBrowser but card not present
  if (!success)
    return true; // Must return true, which means everything is dealt with -
                 // because this UI would already have been exited if there was
                 // a problem
...
void SoundEditor::setLedStates() {
  IndicatorLEDs::setLedState(saveLedX,
                             saveLedY,
                             false); // In case we came from the save-Instrument UI

  IndicatorLEDs::setLedState(synthLedX,
                             synthLedY,
                             !inSettingsMenu() && !editingKit() && currentSound);
  IndicatorLEDs::setLedState(kitLedX, kitLedY, !inSettingsMenu() && editingKit() && currentSound);
  IndicatorLEDs::setLedState(midiLedX,
                             midiLedY,
                             !inSettingsMenu() && currentSong->currentClip->output->type ==
                                                      INSTRUMENT_TYPE_MIDI_OUT);
  IndicatorLEDs::setLedState(cvLedX,
                             cvLedY,
                             !inSettingsMenu() &&
                                 currentSong->currentClip->output->type == INSTRUMENT_TYPE_CV);
      ...
      if (on) {
        ...
        if (newItem) {
          if (newItem != (MenuItem*)0xFFFFFFFF) {
            ...
            int result = newItem->checkPermissionToBeginSession(currentSound,
                                                                currentSourceIndex,
                                                                &currentMultiRange);
  ...
  // Save button
    ...
    if (on && currentUIMode == UI_MODE_NONE && !inSettingsMenu() && !editingCVOrMIDIClip() &&
      ...
      if (Buttons::isShiftButtonPressed()) {
        if (getCurrentMenuItem() == &multiRangeMenu) { multiRangeMenu.deletePress(); }
      } else {
...
void SoundEditor::goUpOneLevel() {
  do {
    ...
    navigationDepth--;
  } while (!getCurrentMenuItem()->checkPermissionToBeginSession(currentSound,
                                                                currentSourceIndex,
                                                                &currentMultiRange));
  numericDriver.setNextTransitionDirection(-1);
...
bool SoundEditor::beginScreen(MenuItem* oldMenuItem) {
  ...
  if (getCurrentUI() != &soundEditor && getCurrentUI() != &sampleBrowser &&
      getCurrentUI() != &audioRecorder && getCurrentUI() != &sampleMarkerEditor &&
  ...
  if (!inSettingsMenu() && currentItem != &sampleStartMenu && currentItem != &sampleEndMenu &&
      ...
      currentItem != &audioClipSampleMarkerEditorMenuEnd && currentItem != &fileSelectorMenu &&
    ...
    if (currentSong->currentClip->type == CLIP_TYPE_AUDIO) {
      ...
      // First, see if there's a shortcut for the actual MenuItem we're
      // currently on
      for (x = 0; x < 15; x++) {
        for (y = 0; y < displayHeight; y++) {
          if (paramShortcutsForAudioClips[x][y] == currentItem) {
            // if (x == 10 && y < 6 && editingReverbCompressor()) goto stopThat;
            // if (currentParamShorcutX != 255 && (x & 1) && currentSourceIndex
            // == 0) goto stopThat;
    ...
    // Or the "normal" case, for Sounds
      ...
      // Failing that, if we're doing some patching, see if there's a shortcut
      // for that *param*
      if (currentParamShorcutX == 255) {
        ...
        if (paramLookingFor != 255) {
          for (int x = 0; x < 15; x++) {
            for (int y = 0; y < displayHeight; y++) {
              if (paramShortcutsForSounds[x][y] &&
                  paramShortcutsForSounds[x][y] != comingSoonMenu &&
          ...
          for (int y = 0; y < displayHeight; y++) {
            ...
            if (source < NUM_PATCH_SOURCES)
              sourceShortcutBlinkFrequencies[x][y] =
                  currentItem->shouldBlinkPatchingSourceShortcut(source,
                                                                 &sourceShortcutBlinkColours[x][y]);
          }
        }
      }
    }
...
void SoundEditor::blinkShortcut() {
  ...
  // We have to blink params and shortcuts at slightly different times, because
  // blinking two pads on the same row at same time doesn't work
...
void SoundEditor::selectEncoderAction(int8_t offset) {
  ...
  if (currentSound) {
    if (getCurrentMenuItem()->selectEncoderActionEditsInstrument())
      markInstrumentAsEdited(); // TODO: make reverb and reverb-compressor stuff
                                // exempt from this

    // If envelope param preset values were changed, there's a chance that there
    // could have been a change to whether notes have tails
    char                      modelStackMemory[MODEL_STACK_MAX_SIZE];
    ModelStackWithSoundFlags* modelStack = getCurrentModelStack(modelStackMemory)->addSoundFlags();

    bool hasNoteTailsNow = currentSound->allowNoteTails(modelStack);
    if (hadNoteTails != hasNoteTailsNow) { uiNeedsRendering(&instrumentClipView, 0xFFFFFFFF, 0); }
  }
...
void SoundEditor::markInstrumentAsEdited() {
  if (!inSettingsMenu()) { ((Instrument*)currentSong->currentClip->output)->beenEdited(); }
}
...
int SoundEditor::potentialShortcutPadAction(int x, int y, bool on) {
  ...
  if (on && isUIModeWithinRange(shortcutPadUIModes)) {
    ...
    if (currentSong->currentClip->type == CLIP_TYPE_AUDIO) {
      ...
      if (x <= 14) { item = paramShortcutsForAudioClips[x][y]; }
      ...
      doSetup:
        if (item) {
          ...
          if (!setupSuccess) { return ACTION_RESULT_DEALT_WITH; }
          ...
          if (getCurrentUI() != &soundEditor) {
            if (getCurrentUI() == &sampleMarkerEditor) {
              ...
              renderingNeededRegardlessOfUI(); // Not sure if this is 100%
                                               // needed... some of it is.
            } else {
              openUI(&soundEditor);
            }
          }
      ...
      // Shortcut to patch a modulation source to the parameter we're already
      // looking at
      ...
      getOut:
        ...
        while (true) {
          ...
          MenuItem* newMenuItem =
              menuItemNavigationRecord[newNavigationDepth]->patchingSourceShortcutPress(
                  source,
          ...
          // Otherwise...
            ...
            if (newMenuItem && newMenuItem->checkPermissionToBeginSession(currentSound,
                                                                          currentSourceIndex,
                                                                          &currentMultiRange)) {
...
int SoundEditor::padAction(int x, int y, int on) {
  ...
  if (currentUIMode == UI_MODE_NONE && on) {
    ...
    // Dear tinkerers and open-sourcers, please don't use or publicise this
    // feature. If it goes wrong, your Deluge is toast.
    if (getCurrentMenuItem() == &firmwareVersionMenu &&
      ...
      if (matrixDriver.isUserDoingBootloaderOverwriteAction()) {
        ...
        if (available) { openUI(&contextMenuOverwriteBootloader); }
      }
    }
...
void SoundEditor::modEncoderAction(int whichModEncoder, int offset) {
  ...
  if (currentUIMode == UI_MODE_MIDI_LEARN) {
...
#if DELUGE_MODEL == DELUGE_MODEL_40_PAD
      IndicatorLEDs::indicateAlertOnLed(songViewLedX,
                                        songViewLedY); // Really should indicate it on "Clip
                                                       // View", but that's already blinking
#else
    ...
    // Otherwise, everything's fine
      ...
      getCurrentMenuItem()->learnKnob(NULL,
                                      whichModEncoder,
                                      currentSong->currentClip->output->modKnobMode,
                                      255);
    }
  }
...
bool SoundEditor::setup(Clip* clip, const MenuItem* item, int sourceIndex) {
  ...
  // This isn't a very nice solution, but we have to set currentParamManager
  // before calling checkPermissionToBeginSession(), because in a minority of
  // cases, like "patch cable strength" / "modulation depth", it needs this.
  ...
  if (currentModControllable) { currentCompressor = &currentModControllable->compressor; }
...
int SoundEditor::checkPermissionToBeginSessionForRangeSpecificParam(
    Sound*       sound,
    int          whichThing,
  ...
  if (soundEditor.editingKit() ||
      (automaticallySelectIfOnlyOne && source->ranges.getNumElements() == 1)) {
    *previouslySelectedRange = firstRange;
    return MENU_PERMISSION_YES;
  }

  if (getCurrentUI() == &soundEditor && *previouslySelectedRange &&
      currentSourceIndex == whichThing)
    return MENU_PERMISSION_YES;
...
ModelStackWithThreeMainThings* SoundEditor::getCurrentModelStack(void* memory) {
  ...
  if (currentSong->currentClip->output->type == INSTRUMENT_TYPE_KIT) {
    ...
    if (selectedDrum) {
      noteRow = ((InstrumentClip*)currentSong->currentClip)
                    ->getNoteRowForDrum(selectedDrum, &noteRowIndex);
    }
  }

  return setupModelStackWithThreeMainThingsIncludingNoteRow(memory,
                                                            currentSong,
...
void SoundEditor::mpeZonesPotentiallyUpdated() {
  if (getCurrentUI() == this) {
    ...
    if (currentMenuItem == &mpeZoneNumMemberChannelsMenu) { currentMenuItem->readValueAgain(); }
  }
}
