...
public:
  ...
  String             name; // Contains the display name as the user sees it.
                           // E.g. on numeric Deluge, SYNT000 will be just "0". Definitely
                           // no leading zeros, so not "000". On OLED Deluge I thiiink
                           // SYNT000 would be "SYNT000"? Definitely does not contain the
                           // ".XML" on the end.
  Output*       next;
  const uint8_t type;
  ...
  bool nextClipFoundShouldGetArmed; // Temp thing for
                                    // Session::armClipsToStartOrSoloWithQuantization

  // reverbAmountAdjust has "1" as 67108864
  // Only gets called if there's an activeClip
  virtual void renderOutput(ModelStack*   modelStack,
  ...
  setActiveClip(ModelStackWithTimelineCounter* modelStack,
                int maySendMIDIPGMs = PGM_CHANGE_SEND_ONCE); // Will have no effect if it
                                                             // already had that Clip
  void pickAnActiveClipForArrangementPos(ModelStack* modelStack,
                                         int         arrangementPos,
                                         int         maySendMIDIPGMs);
  void pickAnActiveClipIfPossible(ModelStack* modelStack,
  ...
  virtual void getThingWithMostReverb(Sound**                   soundWithMostReverb,
                                      ParamManager**            paramManagerWithMostReverb,
                                      GlobalEffectableForClip** globalEffectableWithMostReverb,
                                      int32_t*                  highestReverbAmountFound) {}
  virtual bool offerReceivedPitchBendToLearnedParams(MIDIDevice*                    fromDevice,
                                                     uint8_t                        channel,
                                                     uint8_t                        data1,
                                                     uint8_t                        data2,
  ...
  virtual void offerReceivedCCToLearnedParams(MIDIDevice*                    fromDevice,
                                              uint8_t                        channel,
                                              uint8_t                        ccNumber,
                                              uint8_t                        value,
                                              ModelStackWithTimelineCounter* modelStack) {
  ...
  virtual int readFromFile(Song*   song,
                           Clip*   clip,
                           int32_t readAutomationUpToPos); // I think that supplying clip
                                                           // here is only a hangover from
                                                           // old pre-2.0 files...
  ...
  virtual bool writeDataToFile(Clip* clipForSavingOutputOnly,
                               Song* song); // Returns true if it's ended the opening tag and
                                            // gone into the sub-tags
  ...
  virtual void resyncLFOs(){}; // No activeClip needed. Call anytime the Instrument comes
                               // into existence on the main list thing
  ...
  virtual void offerReceivedNote(ModelStackWithTimelineCounter* modelStackWithTimelineCounter,
  ...
  virtual void offerReceivedPitchBend(ModelStackWithTimelineCounter* modelStackWithTimelineCounter,
                                      ...
                                      uint8_t                        data2,
                                      bool*                          doingMidiThru) {}
  virtual void offerReceivedCC(ModelStackWithTimelineCounter* modelStackWithTimelineCounter,
                               MIDIDevice*                    fromDevice,
                               uint8_t                        channel,
                               uint8_t                        ccNumber,
                               uint8_t                        value,
  ...
  virtual void offerBendRangeUpdate(ModelStack* modelStack,
                                    MIDIDevice* device,
                                    int         channelOrZone,
                                    int         whichBendRange,
                                    int         bendSemitones) {}
