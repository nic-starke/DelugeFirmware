...
bool TimeStretcher::init(Sample*              sample,
  ...
  for (int l = 0; l < NUM_CLUSTERS_LOADED_AHEAD; l++) { clustersForPercLookahead[l] = NULL; }

  for (int l = 0; l < 2; l++) { percCacheClustersNearby[l] = NULL; }
  ...
  if (phaseIncrement)
    reassessWhetherToBeFillingBuffer(phaseIncrement,
                                     timeStretchRatio,
                                     BUFFER_FILLING_NEWER,
                                     numChannels);

  newerHeadReadingFromBuffer = false;

  // If that did set up buffering, from the newer play-head, then just set up
  // the "older" play-head to read from the buffer
  ...
  if (fudgingNumSamplesTilLoop) {
    ...
    if (phaseIncrement != 16777216) {
      fudgingNumSamplesTilLoop =
          ((uint64_t)fudgingNumSamplesTilLoop * (uint32_t)phaseIncrement + (1 << 23)) >>
          24; // Round
    }

    int bytesPerSample = sample->byteDepth * sample->numChannels;

    int newBytePos = guide->getBytePosToStartPlayback(true) -
                     fudgingNumSamplesTilLoop * bytesPerSample * playDirection;

    int32_t startByte = sample->audioDataStartPosBytes;
    if (playDirection != 1)
      startByte +=
          sample->audioDataLengthBytes - bytesPerSample; // The actual first sample of the waveform
    ...
    bool success =
        setupNewPlayHead(sample, voiceSample, guide, newBytePos, 0, priorityRating, loopingType);
    if (!success) return false;

    samplesTilHopEnd = 2147483647; // We don't want to do a hop-end

    crossfadeIncrement = (uint32_t)(16777216 + fudgingNumSamplesTilLoop) /
                         (uint32_t)fudgingNumSamplesTilLoop; // Round up
    crossfadeProgress = 0;
  }
  ...
  // Normal case
    ...
    // The fine-tuning of the first hop length is important for allowing
    // individual drum hits to sound shorter when sped up. We also add a slight
    // random element so that if many AudioClips or other Sounds begin and do
    // time-stretching at the same time, they won't all hit the CPU with their
    // first hop at the exact same time, which would cause a spike
...
void TimeStretcher::reInit(int64_t              newSamplePosBig,
  ...
  // If the newer play-head is still active, we'll have a hop soon, so playback
  // will soon we coming from the start of the waveform again like we want, and
  // we don't have to do anything.
  //
  // Or if the newer play-head is inactive, force a hop now. Is this the best
  // way? Not sure if maybe we wanna instead just do an init() or at least force
  // the new play-head to exactly the start-pos-sample or something?
...
bool TimeStretcher::hopEnd(SamplePlaybackGuide* guide,
  ...
  // Trying to track down Steven's E133 - percCacheClusterNearby pointing to
  // things with no reasons left
  for (int l = 0; l < 2; l++) {
  ...
  */
  ...
  // AudioEngine::bypassCulling = true; // This is kinda dangerous, but I think
  // it should kinda be ok...
  ...
  olderPartReader.cloneFrom(voiceSample,
                            true); // Steals all reasons from the VoiceSample
  ...
  // If guide is synced to the actual sequence's ticks, we can peeeerfectly get
  // the pos we want. Note: slight imperfection but it should be fine - if an
  // AudioClip has had its position "resumed" to somewhere else due to a
  // <>+play, it could still have its envelope fading out, yet now also be
  // doingLateStart, meaning we're still rendering its old position, but now
  // this new hop is going to start from the new, resume()d current play-pos.
  // Should be fine and if anything should sound not bad... but maybe beware?
  // Should we refuse to do a new hop, or to make this call to
  // guide->getSyncedNumSamplesIn(), if an AudioClip is fading/releasing out?
  if (guide->sequenceSyncLengthTicks && playbackHandler.isEitherClockActive()) {
    ...
    // There should be no risk of that passing the end of the sample zone I
    // think...
  ...
  // Or if outside of that...
    ...
    crossfadeProportional = interpolateTableSigned(position, 27, crossfadeProportionalCoarse, 2)
                            << 8;
    ...
    // lookahead = interpolateTableSigned(position, 27, lookaheadCoarse, 2) >>
    // 16;
  }
  ...
  */
...
#if !MEASURE_HOP_END_PERFORMANCE
  minBeamWidth += (multiply_32x32_rshift32(minBeamWidth,
                                           multiply_32x32_rshift32(getNoise(), randomElement << 8))
                   << 2);
#endif
  ...
  if (playDirection != 1)
    waveformStartByte += sample->audioDataLengthBytes -
                         bytesPerSample; // The actual first sample of the waveform in our given
                                         // direction, regardless of our elected start-point

  // If this is for some looping piece of audio (possibly an AudioClip, but also
  // a looping instrument sample or something, see if we want to place our next
  // hop in the pre-margin. Remember, for AudioClips which are not going to loop
  // another time (determined by currentPlaybackMode->willClipLoopAtEnd()), this
  // will be set as false. That check is done in AudioClip::render()
    ...
    if (numBytesOfPreMarginAvailable > 0) {
      ...
      uint32_t loopEndSample =
          (uint32_t)(guide->getBytePosToEndOrLoopPlayback() -
                     sample->audioDataStartPosBytes) // This will refer to the
                                                     // loop point - not the
                                                     // actual end of the waveform
          / (uint8_t)(sample->numChannels * sample->byteDepth);

      int sourceSamplesTilLoop = (int32_t)(loopEndSample - samplePos) * playDirection;

      if (sourceSamplesTilLoop >=
          0) { // This can fail for an AudioClip if we've just stopped playback

        int outputSamplesTilLoop =
            (((uint64_t)sourceSamplesTilLoop << 24) + (combinedIncrement >> 1)) /
            combinedIncrement; // Round
        ...
        if (outputSamplesTilLoop < ANTI_CLICK_CROSSFADE_LENGTH) {
          ...
          if ((int32_t)(newHeadBytePos - waveformStartByte) * playDirection >= 0) {
            ...
            // Note: we don't check that the relevant cluster has loaded. I
            // think nothing too bad will happen if it's not...

            crossfadeLengthSamples = getMax(outputSamplesTilLoop, 10); // Min crossfade length

            samplesTilHopEnd =
                minBeamWidth >> 2; // Bigger sounds bad. Need to make smaller to match similarly
                                   // resulting deduction which happens in the "normal" case
            ...
            // cos our new play-head needs to remain perfectly aligned with the
            // start of the cache.
        ...
        // Otherwise, just make sure we come back not long after the ideal time
        // to do a crossfade back to before the start
        else {
...

  {
    ...
    uint8_t* percCache = sample->prepareToReadPercCache(beamPosAtTop,
                                                        playDirection,
    ...
    if (percCache) {
      ...
      for (uint32_t beamWidthNow = minBeamWidth; beamWidthNow < maxBeamWidth;
        ...
        int beamBackEdge =
            beamPosAtTop + (int32_t)(((int64_t)beamWidthNow * (timeStretchRatio - 16777216)) >>
                                     (25 + PERC_BUFFER_REDUCTION_MAGNITUDE)) *
                               playDirection;
        int beamFrontEdge =
            beamPosAtTop + (int32_t)(((uint64_t)beamWidthNow * (timeStretchRatio + 16777216)) >>
                                     (25 + PERC_BUFFER_REDUCTION_MAGNITUDE)) *
                               playDirection;
        ...
        if (pixellatedBeamWidth) { // It might be zero near the start
          ...
          while ((beamBackEdge - furthestBackSearched) * playDirection < 0) {
            furthestBackSearched -= playDirection; // Yes, do the decrement before reading in this
                                                   // one case
            int percHere = percCache[furthestBackSearched];
            totalPercussiveness += percHere;
          }

          // If our current average percussiveness is >= the previous best
          // average (calculated without divisions)
    ...
    int beamBackEdge =
        samplePos + (int32_t)(((int64_t)bestBeamWidth * (timeStretchRatio - 16777216)) >> 25) *
    ...
    int32_t waveformStartSample =
        (playDirection == 1) ?
            ...
            sample->lengthInSamples - 1; // The actual first sample of the waveform in our given
                                         // direction, regardless of our elected start-point
    int32_t waveformEndSample = (playDirection == 1) ?
                                    sample->lengthInSamples :
                                    -1; // The actual last sample of the waveform in our given
                                        // direction, regardless of our elected start-point

    // Still must make sure we didn't go back beyond the start of the waveform,
    // which can end up happening from the heavily pixellated search thing above
    ...
    samplesTilHopEnd = ((uint64_t)bestBeamWidth << 24) /
                       (uint32_t)phaseIncrement; // That's the beamWidthOnRepitchedWaveform
    if (samplesTilHopEnd < 1)
      samplesTilHopEnd = 1; // Can happen if pitch up very high and also
                            // time-stretch sped up a lot

    crossfadeLengthSamples =
        multiply_32x32_rshift32_rounded(samplesTilHopEnd, crossfadeProportional) +
        crossfadeAbsolute * 4;
    ...
    // Make sure we haven't shot past end of waveform. If so, we don't want this
    // new play-head sounding
...
skipPercStuff:
  ...
  // Search for minimum phase disruption on crossfade. Crucially, the exact
  // instant in time we're going to be examining is not the beginning play-point
  // of the new play-head, but the point half-way through the crossfade later.
  // Remember that!
  if (playHeadStillActive[PLAY_HEAD_OLDER]) { // Added condition, Aug 2019.
                                              // Surely this makes sense...
    int lengthToAverageEach =
        ((uint64_t)phaseIncrement * TIME_STRETCH_CROSSFADE_MOVING_AVERAGE_LENGTH) >> 24;
    lengthToAverageEach = getMax(lengthToAverageEach, 1);
    lengthToAverageEach =
        getMin(lengthToAverageEach,
               TIME_STRETCH_CROSSFADE_MOVING_AVERAGE_LENGTH * 2); // Keep things sensible
    ...
    if (oldHeadBytePos < (int32_t)sample->audioDataStartPosBytes)
      goto skipSearch; // Would probably be possible on a pitch-adjusted
                       // reversed waveform if we'd got past the end and were
                       // "buffering zeros"
    success = sample->getAveragesForCrossfade(oldHeadTotals,
                                              oldHeadBytePos,
    ...
    success = sample->getAveragesForCrossfade(newHeadTotals,
                                              newHeadBytePos,
                                              crossfadeLengthSamplesSource,
                                              playDirection,
                                              lengthToAverageEach);
    ...
    int samplePos =
        (uint32_t)(newHeadBytePos - sample->audioDataStartPosBytes) / (uint8_t)bytesPerSample;
    ...
    int maxSearchSize =
        (samplesTilHopEnd * 40) >> 8; // I don't remember the logic behind this line...
    ...
    int limit =
        (sample->sampleRate / 45) >> 1; // Allow tracking down to around 45Hz, at input. We >>1
                                        // again because this limit is just for searching in one
                                        // direction, and we're going to do both directions.
    ...
    // Do the search. We'll come back again to search in the other direction
    // too. Or we may come back sooner if we quickly decide that the other
    // search direction would be better
  ...
  startSearch:
    ...
    do { // This will get restarted multiple times as we cross cluster
      ...
      // Alright, read those samples for our currently worked out little bit
      // until we reach a cluster boundary or something
      int endOffset = offsetNow + numSamplesThisRead * bytesPerSampleTimesSearchDirection;
      do { // For each individual sample (well, actually, we're gonna read like
           // 3 different, spaced ones at a time)
        ...
        for (; i < TIME_STRETCH_CROSSFADE_NUM_MOVING_AVERAGES + 1;
             i++) { // Manually unrolling didn't speed up

          thisRunningTotal = newHeadRunningTotals[i - 1] -
                             readValueRelativeToBothDirections; // Subtracts the one from last
                                                                // iteration
        entryPoint:

          int32_t readValueHere = *(int32_t*)currentPos[i] >> 16;

          if (numChannels == 2) { readValueHere += *(int32_t*)(currentPos[i] + byteDepth) >> 16; }

          currentPos[i] += bytesPerSampleTimesSearchDirection;

          readValueRelativeToBothDirections =
              readValueHere * searchDirectionRelativeToPlayDirection;
        ...
        // If our very first read is worse, let's switch search direction right
        // now - that'll improve our odds
        if (offsetNow == 0 && searchDirectionRelativeToPlayDirection == 1 &&
            !numFullDirectionsSearched && differenceAbs > bestDifferenceAbs) {
        ...
        if (((uint32_t)thisTotalChange >> 31) != ((uint32_t)lastTotalChange >> 31)) {
          ...
          // Try going in between the samples for the most accurate positioning,
          // lining-up-wise. The benefit of this is visible on a spectrum
          // analysis if you're pitching a high-pitched sine wave right down,
          // while also time stretching it
          if (phaseIncrement != 16777216 &&
              (thisOffsetIsBestMatch ||
               bestOffset == offsetNow - bytesPerSampleTimesSearchDirection)) { // If best was
                                                                                // this one or
                                                                                // last one
          ...
          // After sign has flipped a certain number of times (in total,
          // including both search directions), we can be fairly sure we've
          // found a good fit This needs to be 4. Any less, and we start getting
          // lots of bad alignments - I did tests. But 4 sounds basically as
          // good as no limit.
          timesSignFlipped++;
#if !MEASURE_HOP_END_PERFORMANCE
  ...
  stopSearch:
    ...
    // The above is supposed to not go back beyond the start of the waveform,
    // but there must be some bug because it does. Until I fix that, this check
    // ensures we stay within the waveform
...
skipSearch:
  ...
  if (bufferFillingMode != BUFFER_FILLING_OFF // If not OFF, it can only be OLDER or NEITHER - it
                                              // gets changed above
      && phaseIncrement != 16777216) {

    if (!olderPartReader.clusters[0]) { Uart::println("aaa"); }

    int32_t bytesBehind =
        (olderPartReader.getPlayByteLowLevel(sample, guide) - newHeadBytePos) * playDirection;
    ...
    uint32_t samplesBehind =
        (uint32_t)bytesBehind / (uint8_t)(sample->numChannels * sample->byteDepth);
    int samplesBehindOnRepitchedWaveform =
        ((uint64_t)samplesBehind << 24) / (uint32_t)phaseIncrement;
    int maxSamplesBehind = TIME_STRETCH_BUFFER_SIZE - (SSI_TX_BUFFER_NUM_SAMPLES - 1);

    // Check we're not earlier by too much - which would usually be because we
    // just looped
    ...
    if (!samplesBehindOnRepitchedWaveform) {
      ...
      newerBufferReadPos = (uint32_t)(bufferWritePos - samplesBehindOnRepitchedWaveform) &
  ...
  optForDirectReading:
    ...
    bool success = setupNewPlayHead(sample,
                                    voiceSample,
                                    guide,
                                    newHeadBytePos,
                                    additionalOscPos,
...
#if TIME_STRETCH_ENABLE_BUFFER
    // We've stopped writing to the buffer for now, but it might still be being
    // read from
    if (bufferFillingMode == BUFFER_FILLING_OLDER) bufferFillingMode = BUFFER_FILLING_NEITHER;
  }

  int newBufferFillingMode = BUFFER_FILLING_NEWER; // Letting it fill from the older buffer
                                                   // initially caused problems
  reassessWhetherToBeFillingBuffer(phaseIncrement,
                                   timeStretchRatio,
                                   newBufferFillingMode,
                                   numChannels);

#else
  // If no one's reading from the buffer anymore, stop filling it
  if (buffer && !olderHeadReadingFromBuffer) { // olderHeadReadingFromBuffer will always
                                               // be false - we set it above, at the start
...
bool TimeStretcher::setupNewPlayHead(Sample*              sample,
                                     ...
                                     SamplePlaybackGuide* guide,
                                     int                  newHeadBytePos,
                                     int                  additionalOscPos,
                                     int                  priorityRating,
                                     int                  loopingType) {
  bool success =
      voiceSample->setupClustersForPlayFromByte(guide, sample, newHeadBytePos, priorityRating);
  if (!success) return false;
  ...
  success = voiceSample->changeClusterIfNecessary(
      ...
      priorityRating); // Set looping as false - change in June 2019. Pretty
                       // sure this is right...
  if (!success) return false;
...
#if TIME_STRETCH_ENABLE_BUFFER
void TimeStretcher::reassessWhetherToBeFillingBuffer(int32_t phaseIncrement,
                                                     int32_t timeStretchRatio,
                                                     int     newBufferFillingMode,
                                                     int     numChannels) {
  ...
  // Or if we have been filling buffer...
    ...
    // If no one's reading from the buffer anymore, stop filling it
    if (!newerHeadReadingFromBuffer && !olderHeadReadingFromBuffer &&
...
bool TimeStretcher::allocateBuffer(int numChannels) {
  buffer = (int32_t*)generalMemoryAllocator
               .alloc(TIME_STRETCH_BUFFER_SIZE * sizeof(int32_t) * numChannels, NULL, false, true);
  return (buffer != NULL);
}

void TimeStretcher::readFromBuffer(int32_t* __restrict__ oscBufferPos,
                                   int     numSamples,
                                   int     numChannels,
                                   int     numChannelsAfterCondensing,
                                   int32_t sourceAmplitudeNow,
                                   int32_t amplitudeIncrementNow,
                                   int* __restrict__ bufferReadPos) {
  ...
  do {
    ...
    *oscBufferPos +=
        multiply_32x32_rshift32(sampleRead[0],
                                sourceAmplitudeNow); // sourceAmplitude is modified above
    oscBufferPos++;
...
// And just to be super clear, this is for remembering links to *PERC CACHE
// Clusters*! Not just regular audio data Clusters.
void TimeStretcher::rememberPercCacheCluster(Cluster* cluster) {

  if (percCacheClustersNearby[0] == cluster || percCacheClustersNearby[1] == cluster) { return; }
...
// This is just responsible for adding reasons to the upcoming Clusters of
// sample audio data that the perc lookahead is going to need in the next little
// while, to reserve it and hopefully make sure it's loaded and in memory when
// we need it.
void TimeStretcher::updateClustersForPercLookahead(Sample*  sample,
  ...
  if (!clustersForPercLookahead[0] || clustersForPercLookahead[0]->clusterIndex != clusterIndex) {
    ...
    for (int l = 0; l < NUM_CLUSTERS_LOADED_AHEAD; l++) {
      ...
      clustersForPercLookahead[l] = sample->clusters.getElement(nextClusterIndex)
                                        ->getCluster(sample, nextClusterIndex, CLUSTER_ENQUEUE);
...
void TimeStretcher::setupCrossfadeFromCache(SampleCache* cache, int cacheBytePos, int numChannels) {
  ...
  int bytesTilCacheClusterEnd = audioFileManager.clusterSize - bytePosWithinCluster +
                                (CACHE_BYTE_DEPTH * numChannels -
                                 1); // Add one-byte-less-than-a-sample to it, so it'll round up
  ...
  // If we're really unlucky, allocating the buffer may have stolen from the
  // cache
  ...
  int samplesTilThisWindowEnd =
      ...
      (uint8_t)(numChannels * CACHE_BYTE_DEPTH); // Will round up, cos we did
                                                 // that additional bit above
#else
...
#if TIME_STRETCH_ENABLE_BUFFER
  bufferWritePos =
      TIME_STRETCH_BUFFER_SIZE - 1; // To trick it out of trying to do a "normal" thing later
  bufferFillingMode = BUFFER_FILLING_OFF;
#endif
}
...
outputBuffer[1] = a * sample->percCacheMemory[(timeStretcher->distanceSinceStart
>> PERC_BUFFER_REDUCTION_MAGNITUDE) >> 24];
//if (samplesTilHopEnd <= 3) outputBuffer[1] = 536870911;
if (hopStrength >= ((16777216 + (16777216 >> lShiftAmount)) >> 1) &&
!lastThingSilentNow) { lastThingSilentNow = true; outputBuffer[1] = -536870911;
}
*/
