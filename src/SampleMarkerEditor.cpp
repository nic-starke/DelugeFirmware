...
bool SampleMarkerEditor::opened() {
  ...
  if (getRootUI() != &instrumentClipView) { renderingNeededRegardlessOfUI(0, 0xFFFFFFFF); }
...
void SampleMarkerEditor::writeValue(uint32_t value, int markerTypeNow) {
  ...
  if (clipType == CLIP_TYPE_AUDIO) {
    audioClipActive = (playbackHandler.isEitherClockActive() &&
  ...
  if (clipType == CLIP_TYPE_AUDIO) {
    ...
    soundEditor.currentSound->sampleZoneChanged(markerTypeNow,
                                                soundEditor.currentSourceIndex,
                                                modelStack);
...
void SampleMarkerEditor::getColsOnScreen(MarkerColumn* cols) {
  ...
  if (currentSong->currentClip->type != CLIP_TYPE_AUDIO) {
    ...
    cols[MARKER_LOOP_START].colOnScreen = cols[MARKER_LOOP_START].pos ?
                                              getStartColOnScreen(cols[MARKER_LOOP_START].pos) :
                                              -2147483648;
...
void SampleMarkerEditor::selectEncoderAction(int8_t offset) {
  ...
  int32_t newMarkerPos =
      (markerType < MARKER_LOOP_END) ? getStartPosFromCol(newCol) : getEndPosFromCol(newCol);
  ...
  if (oldCol >= 0 && oldCol < displayWidth) {
    ...
    getColsOnScreen(cols);
    newCol = cols[markerType].colOnScreen; // It might have changed, and despite having a newCol
                                           // variable above, that's only our desired value - we
...
int SampleMarkerEditor::padAction(int x, int y, int on) {
  ...
  if (currentUIMode != UI_MODE_AUDITIONING) { // Don't want to do this while auditioning - too
  ...
  // Mute pads
    ...
    if (on) {
      ...
      for (int m = 0; m < NUM_MARKER_TYPES; m++) {
        if (cols[m].colOnScreen == x) {
          if (markerPressed != -1)
            return ACTION_RESULT_DEALT_WITH; // Get out if there are two markers
                                             // occupying the same col we
                                             // pressed
          markerPressed = m;
        }
      }
      ...
      if (currentUIMode == UI_MODE_HOLDING_SAMPLE_MARKER) {
        ...
        if (currentSong->currentClip->type == CLIP_TYPE_INSTRUMENT) {
          ...
          int markerHeld = -1;
          for (int m = 0; m < NUM_MARKER_TYPES; m++) {
            if (cols[m].colOnScreen == pressX) { markerHeld = m; }
          }
          ...
          ensureNotPastSampleLength:
            // Loop start and end points are not allowed to be further right
            // than the sample waveform length
            ...
            if (cols[MARKER_LOOP_START].colOnScreen >= x)
              return ACTION_RESULT_DEALT_WITH; // Will be a big negative number
                                               // if inactive
            if (cols[MARKER_END].colOnScreen <= x) return ACTION_RESULT_DEALT_WITH;
          ...
          // Or if a loop point and they pressed the end marker, remove the loop
          // point
      ...
      // Or if user not already holding a marker down...
        ...
        // Otherwise, move the current marker to where we tapped
...

          {
            ...
            // Only the END marker, and only in some cases, is allowed to be
            // further right than the waveform length
            if (markerType == MARKER_END && shouldAllowExtraScrollRight()) {
              if (x > cols[markerType].colOnScreen && value < cols[markerType].pos)
                return ACTION_RESULT_DEALT_WITH; // Probably not actually
                                                 // necessary
    ...
    // Release press
      ...
      if (currentUIMode == UI_MODE_HOLDING_SAMPLE_MARKER) {
        if (x == pressX && y == pressY) { currentUIMode = UI_MODE_NONE; }
      }
    }
  }
...
int SampleMarkerEditor::horizontalEncoderAction(int offset) {
  ...
  // We're quite likely going to need to read the SD card to do either scrolling
  // or zooming
  if (sdRoutineLock) return ACTION_RESULT_REMIND_ME_OUTSIDE_CARD_ROUTINE;
  ...
  if (isUIModeActive(UI_MODE_HOLDING_HORIZONTAL_ENCODER_BUTTON)) {
    if (isUIModeWithinRange(zoomUIModes)) {
      success = waveformBasicNavigator.zoom(offset,
                                            shouldAllowExtraScrollRight(),
                                            colsToSend,
                                            markerType);
...
int SampleMarkerEditor::timerCallback() {
  ...
  if (x < 0 || x >= displayWidth)
    return ACTION_RESULT_DEALT_WITH; // Shouldn't happen, but let's be safe -
                                     // and not set the timer again if it's
                                     // offscreen

  blinkInvisible = !blinkInvisible;

  // Clear col
  for (int y = 0; y < displayHeight; y++) { memset(PadLEDs::image[y][x], 0, 3); }
...
int SampleMarkerEditor::verticalEncoderAction(int offset, bool inCardRoutine) {
  ...
  int result = instrumentClipView.verticalEncoderAction(
      offset,
      inCardRoutine); // Must say these buttons were not pressed, or
                      // else editing might take place
  ...
  if (!currentlyAccessingCard && !(getNoise() >> 27)) {
    ...
    if (r < 64) {
      ...
      if (!soundEditor.currentSource->reversed) {
        ...
        // writeValue(soundEditor.currentMultisampleRange->sample->lengthInSamples,
        // MARKER_END);

        // int newStartPos =
        // soundEditor.currentMultisampleRange->sample->lengthInSamples;// -
        // (((uint32_t)getNoise() % (44100 * 120)) + 10 * 44100);
    ...
    if (soundEditor.currentSound->hasAnyVoices()) {
      ...
      if (assignedVoice) {
        VoiceUnisonPartSource* part =
            &assignedVoice->unisonParts[soundEditor.currentSound->numUnison >> 1]
...
void SampleMarkerEditor::renderForOneCol(
    int           xDisplay,
    uint8_t       thisImage[displayHeight][displayWidth + sideBarWidth][3],
    MarkerColumn* cols) {

  waveformRenderer.renderOneCol(waveformBasicNavigator.sample,
                                xDisplay,
                                thisImage,
...
void SampleMarkerEditor::renderMarkersForOneCol(
    int           xDisplay,
    uint8_t       thisImage[displayHeight][displayWidth + sideBarWidth][3],
    MarkerColumn* cols) {
  ...
  if (markerType != MARKER_NONE) {
    ...
    for (int m = 0; m < NUM_MARKER_TYPES; m++) {
      markersActiveHere |= (xDisplay == cols[m].colOnScreen && !(blinkInvisible && markerType == m))
                           << m;
    }
...
void SampleMarkerEditor::renderOLED(uint8_t image[][OLED_MAIN_WIDTH_PIXELS]) {
  ...
  printSeconds:
    ...
    // Maybe we just want to display millisecond resolution (that's S with 3
    // decimal places)...
    ...
    memmove(&buffer[length - numDecimalPlaces + 1],
