...
int MenuItemFileSelector::checkPermissionToBeginSession(Sound*       sound,
                                                        int          whichThing,
  ...
  bool can = (sound->getSynthMode() == SYNTH_MODE_SUBTRACTIVE ||
              (sound->getSynthMode() == SYNTH_MODE_RINGMOD &&
               sound->sources[whichThing].oscType == OSC_TYPE_WAVETABLE));

  if (!can) { return MENU_PERMISSION_NO; }
