...
GeneralMemoryAllocator::GeneralMemoryAllocator() {
  lock = false;
  regions[MEMORY_REGION_SDRAM].setup(emptySpacesMemory,
                                     sizeof(emptySpacesMemory),
                                     EXTERNAL_MEMORY_BEGIN,
                                     EXTERNAL_MEMORY_END);
  regions[MEMORY_REGION_INTERNAL].setup(emptySpacesMemoryInternal,
                                        sizeof(emptySpacesMemoryInternal),
                                        (uint32_t)&__heap_start,
                                        INTERNAL_MEMORY_END - 8192);
...
// Watch the heck out - in the older V3.1 branch, this had one less argument -
// makeStealable was missing - so in code from there, thingNotToStealFrom could
// be interpreted as makeStealable! requiredSize 0 means get biggest allocation
// available.
void* GeneralMemoryAllocator::alloc(uint32_t  requiredSize,
                                    uint32_t* getAllocatedSize,
                                    bool      mayDeleteFirstUndoAction,
  ...
  if (lock)
    return NULL; // Prevent any weird loops in freeSomeStealableMemory(), which
                 // mostly would only be bad cos they could extend the stack an
                 // unspecified amount
  ...
  if (mayUseOnChipRam
    ...
    void* address = regions[MEMORY_REGION_INTERNAL].alloc(requiredSize,
                                                          getAllocatedSize,
                                                          makeStealable,
                                                          thingNotToStealFrom,
  ...
  lock          = true;
  void* address = regions[MEMORY_REGION_SDRAM].alloc(requiredSize,
                                                     getAllocatedSize,
                                                     makeStealable,
                                                     thingNotToStealFrom,
                                                     getBiggestAllocationPossible);
...
int GeneralMemoryAllocator::getRegion(void* address) {
  return ((uint32_t)address >= (uint32_t)INTERNAL_MEMORY_BEGIN) ? MEMORY_REGION_INTERNAL :
                                                                  MEMORY_REGION_SDRAM;
}
...
uint32_t GeneralMemoryAllocator::shortenLeft(void*    address,
                                             uint32_t amountToShorten,
                                             uint32_t numBytesToMoveRightIfSuccessful) {
  return regions[getRegion(address)].shortenLeft(address,
                                                 amountToShorten,
                                                 numBytesToMoveRightIfSuccessful);
}

void GeneralMemoryAllocator::extend(void*    address,
  ...
  *getAmountExtendedRight = 0;
  ...
  lock = true;
  regions[getRegion(address)].extend(address,
                                     minAmountToExtend,
                                     idealAmountToExtend,
...
void GeneralMemoryAllocator::putStealableInQueue(Stealable* stealable, int q) {
  ...
  region->stealableClusterQueueLongestRuns[q] =
      0xFFFFFFFF; // TODO: actually investigate neighbouring memory "run".
}
...
public:
  void steal() {
    ...
    // The steal() function is allowed to deallocate or shorten some other
    // allocations, too
...
bool skipConsistencyCheck = false; // Sometimes we want to make sure this check isn't happen, while
                                   // things temporarily are not in an inspectable state
...
void GeneralMemoryAllocator::checkEverythingOk(char const* errorString) {
  ...
  for (int i = 0; i < regions[MEMORY_REGION_SDRAM].emptySpaces.getNumElements(); i++) {
    EmptySpaceRecord* record =
        (EmptySpaceRecord*)regions[MEMORY_REGION_SDRAM].emptySpaces.getElementAddress(i);
...
void GeneralMemoryAllocator::testMemoryDeallocated(void* address) {
  for (int i = 0; i < NUM_TEST_ALLOCATIONS; i++) {
    if (testAllocations[i] == address) { testAllocations[i] = NULL; }
  }
}
...
void GeneralMemoryAllocator::testShorten(int i) {
  ...
  if (a < 128) {
    ...
    int newSize = ((uint32_t)getRandom255() << 17) | ((uint32_t)getRandom255() << 9) |
  ...
  while (1) {
    ...
    for (int i = 0; i < NUM_TEST_ALLOCATIONS; i++) {
      ...
      if (testAllocations[i]) {
        ...
        if (spaceTypes[i] == SPACE_HEADER_STEALABLE
            //|| (uint32_t)testAllocations[i] >= (uint32_t)INTERNAL_MEMORY_BEGIN
            //// If on-chip memory, this is the only option
            || getRandom255() < 128) {
          ...
          if (a < 192) {
            ...
            testAllocations[i] = NULL; // Set this to NULL temporarily so we don't do deallocate
                                       // or shorten it during a steal()

            extend(allocationAddress,
                   minAmountToExtend,
                   idealAmountToExtend,
                   &amountExtendedLeft,
                   &amountExtendedRight);
      ...
      int desiredSize = ((uint32_t)getRandom255() << 9) |
                        ((uint32_t)getRandom255() << 1); // (uint32_t)getRandom255() << 17) |
      ...
      if (!testAllocations[i]) {
        ...
        if (testAllocations[i]) {
          ...
          // if ((uint32_t)testAllocations[i] >=
          // (uint32_t)INTERNAL_MEMORY_BEGIN) actualSize = desiredSize; // If
          // on-chip memory
