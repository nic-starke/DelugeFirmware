...
int ParamManager::setupUnpatched() {
  void* memoryUnpatched =
      generalMemoryAllocator.alloc(sizeof(UnpatchedParamSet), NULL, false, true);
...
void ParamManager::stealParamCollectionsFrom(ParamManager* other, bool stealExpressionParams) {
  ...
  // If we're planning to steal expression params, and yes "other" does in fact
  // have them...
  if (stealExpressionParams && other->summaries[stopAtOther].paramCollection) {

    // If "here" has them too, we'll just keep these, and destruct "other"'s
    // ones
  ...
  int i;
  for (i = 0; i < stopAtOther; i++) { summaries[i] = other->summaries[i]; }

  summaries[stopAtOther] = hereMpeParamsOrNull; // Could the expression params, or NULL
  if (hereMpeParamsOrNull.paramCollection)
    summaries[stopAtOther + 1] = {0}; // If that was expression params, write
                                      // the actual terminating NULL here
                                      // - but not otherwise, cos we could have
                                      // overflowed past the array's size!
...
int ParamManager::cloneParamCollectionsFrom(ParamManager* other,
  ...
  if (mpeParamsOrNullHere.paramCollection)
    cloneExpressionParams = false; // If we already have expression params, then
                                   // just don't clone from "other".

  // First, allocate the memories
  ParamCollectionSummary
      newSummaries[PARAM_COLLECTIONS_STORAGE_NUM]; // Temporary separate storage, so we can
                                                   // clone from self (when this function is
                                                   // called from beenCloned()).

  ParamCollectionSummary* __restrict__ newSummary = newSummaries;
  ParamCollectionSummary* otherSummary = other->summaries; // Not __restrict__, because other might
                                                           // be the same as this!
  ...
  while (otherSummary != otherStopAt) {
    ...
    newSummary->paramCollection = (ParamCollection*)generalMemoryAllocator.alloc(
        otherSummary->paramCollection->objectSize,
        NULL,
        false,
        true); // To cut corners, we store this currently blank/undefined
               // memory in our array of type ParamCollectionSummary
  ...
  while (otherSummary != otherStopAt) {
    ...
    memcpy(newSummary->paramCollection,
           otherSummary->paramCollection,
           otherSummary->paramCollection->objectSize);

    newSummary->paramCollection->beenCloned(
        copyAutomation,
        reverseDirectionWithLength); // Ignore error - just means automation
                                     // doesn't get cloned.
  ...
  *newSummary = mpeParamsOrNullHere;
  if (mpeParamsOrNullHere.paramCollection) { // Check first, otherwise we'll
                                             // overflow the array, I think...
    newSummary++;
    *newSummary = {0}; // Mark end of list
  }

  // And finally, copy the pointers and flags from newSummaries to our permanent
  // summaries array.
  newSummary                            = newSummaries;
...
int ParamManager::beenCloned(int32_t reverseDirectionWithLength) {
  return cloneParamCollectionsFrom(this,
                                   true,
                                   true,
                                   reverseDirectionWithLength); // *Does* clone expression params
}
...
bool ParamManager::ensureExpressionParamSetExists(bool forDrum) {
  ...
  if (!summaries[offset].paramCollection) {
    ...
    summaries[offset].paramCollection =
        new (memory) ExpressionParamSet(&summaries[offset], forDrum);
...
ParamManager::getPatchCableSet(ModelStackWithThreeMainThings const* modelStack) {
#if ALPHA_OR_BETA_VERSION
...
// You'll usually want to call mightContainAutomation() before bothering with
// this, to save time.
void ParamManagerForTimeline::processCurrentPos(ModelStackWithThreeMainThings* modelStack,
...
#if ALPHA_OR_BETA_VERSION
  ensureSomeParamCollections(); // If you're going to delete this and allow
                                // none, make sure you replace the "do" below
                                // with its "while".
#endif
  ...
  if (ticksTilNextEvent <= 0) {
    ...
    summary->paramCollection->processCurrentPos(modelStackWithParamCollection,
                                                ticksSkipped,
                                                reversed,
                                                didPingpong,
                                                mayInterpolate);
...
void ParamManagerForTimeline::expectEvent(ModelStackWithThreeMainThings const* modelStack) {
  ...
  if (playbackHandler.isEitherClockActive() &&
      (!timelineCounter || timelineCounter->isPlayingAutomationNow())) {
    ticksTilNextEvent = 0;
    if (timelineCounter) timelineCounter->expectEvent();
  }
}

// Very minimal function - doesn't take a ModelStack, because we use this to
// decide whether we even need to create / populate the ModelStack.
bool ParamManagerForTimeline::mightContainAutomation() {

  ParamCollectionSummary* summary = summaries;
  while (summary->paramCollection) {
    if (summary->containsAutomation()) { return true; }
    summary++;
  }
...
void ParamManager::notifyParamModifiedInSomeWay(ModelStackWithAutoParam const* modelStack,
  ...
  if (automationChanged && paramAutomatedNow) { toForTimeline()->expectEvent(modelStack); }

  if (currentValueChanged) { view.notifyParamAutomationOccurred(this); }
}
...
void ParamManagerForTimeline::shiftHorizontally(ModelStackWithThreeMainThings* modelStack,
  ...
  summary->paramCollection->shiftHorizontally(modelStackWithParamCollection,
                                              amount,
                                              effectiveLength);
...
void ParamManagerForTimeline::trimToLength(uint32_t                       newLength,
  ...
  summary->paramCollection->trimToLength(newLength,
                                         modelStackWithParamCollection,
                                         action,
                                         maySetupPatching);
...
void ParamManagerForTimeline::generateRepeats(ModelStackWithThreeMainThings* modelStack,
  ...
  summary->paramCollection->generateRepeats(modelStackWithParamCollection,
                                            oldLength,
                                            newLength,
                                            shouldPingpong);
...
void ParamManagerForTimeline::appendParamManager(ModelStackWithThreeMainThings* modelStack,
  ...
  otherModelStack->paramManager->toForTimeline()
      ->ensureSomeParamCollections(); // If you're going to delete this and
                                      // allow none, make sure you replace the
                                      // "do" below with its "while".
#endif
  ...
  summary->paramCollection->appendParamCollection(modelStackWithParamCollection,
                                                  otherModelStackWithParamCollection,
                                                  oldLength,
...
ticksTilNextEvent = 0; // Should probably really call expectEvent(), but we're only called when
                       // a tick is just about to happen anyway, so shouldn't matter
}
...
void ParamManagerForTimeline::tickSamples(int                            numSamples,
  ...
  // Beware - for efficiency, the caller of this sometimes pre-checks whether to
  // even call this at all
...
void ParamManagerForTimeline::nudgeAutomationHorizontallyAtPos(
    int32_t                        pos,
    int                            offset,
  ...
  while (summary->paramCollection) {
    ...
    if (moveMPEDataWithinRegionLength && i == getExpressionParamSetOffset()) {
      ((ExpressionParamSet*)summary->paramCollection)
          ->moveRegionHorizontally(modelStackWithParamCollection,
                                   pos,
                                   moveMPEDataWithinRegionLength,
                                   offset,
                                   lengthBeforeLoop,
                                   action);
    }
    ...
    // Normal case
      ...
      summary->paramCollection->nudgeNonInterpolatingNodesAtPos(pos,
                                                                offset,
                                                                lengthBeforeLoop,
                                                                action,
                                                                modelStackWithParamCollection);
