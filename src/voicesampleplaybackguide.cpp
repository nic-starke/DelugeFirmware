...
void VoiceSamplePlaybackGuide::setupPlaybackBounds(bool reversed) {
  ...
  if (loopStartPlaybackAtSample) {
    loopStartPlaybackAtByte =
        sample->audioDataStartPosBytes + loopStartPlaybackAtSample * bytesPerSample;
  } else {
  ...
  if (loopEndPlaybackAtSample)
    loopEndPlaybackAtByte =
        sample->audioDataStartPosBytes + loopEndPlaybackAtSample * bytesPerSample;
}

// This is, whether to obey the loop-end point as opposed to the actual
// end-of-sample point (which sometimes might cause looping too)
...
// This is actually an important function whose output is the basis for a lot of
// stuff
