...
void String::clear(bool destructing) {
  if (stringMemory) {
    ...
    if (!destructing) { stringMemory = NULL; }
  }
}
...
int String::set(char const* newChars, int newLength) {
  ...
  if (newLength == -1) { newLength = strlen(newChars); }
  ...
  if (stringMemory) {

    {
      ...
      int numReasons = getNumReasons();
      if (numReasons > 1) { goto clearAndAllocateNew; }
      ...
      if (extraMemoryNeeded <= 0) {
        ...
        generalMemoryAllocator.extend(stringMemory - 4,
                                      extraMemoryNeeded,
                                      extraMemoryNeeded,
                                      &amountExtendedLeft,
                                      &amountExtendedRight);
        ...
        if (amountExtendedLeft + amountExtendedRight >= extraMemoryNeeded) { goto doCopy; }
      }
    }
...
void String::beenCloned() {
  if (stringMemory) { setNumReasons(getNumReasons() + 1); }
}
...
int String::shorten(int newLength) {
  if (!newLength)
    ...
    if (oldNumReasons > 1) {
      ...
      memcpy(newStringMemory, stringMemory,
             newLength); // The ending 0 will get set below
...
int String::concatenateAtPos(char const* newChars, int pos, int newCharsLength) {
  if (pos == 0) { return set(newChars, newCharsLength); }

  if (newCharsLength == -1) { newCharsLength = strlen(newChars); }

  if (newCharsLength == 0) { return shorten(pos); }
  ...
  if (extraBytesNeeded > 0) {
    ...
    if (amountExtendedLeft || amountExtendedRight) {
      ...
      if (amountExtendedLeft) {
        memmove(stringMemory - amountExtendedLeft - 4,
                stringMemory - 4,
