...
struct MPEOutputMemberChannel {
  ...
  int16_t  lastXValueSent;       // The actual 14-bit number. But signed (goes positive
                                 // and negative).
  int8_t lastYAndZValuesSent[2]; // The actual 7-bit numbers. Y goes both
...
public:
  ...
  int  readModKnobAssignmentsFromFile(int32_t                  readAutomationUpToPos,
  ...
  int  getFirstUnusedCC(ModelStackWithThreeMainThings* modelStack,
                        int                            direction,
                        int                            startAt,
                        int                            stopAt);
  int  moveAutomationToDifferentCC(int oldCC, int newCC, ModelStackWithThreeMainThings* modelStack);
  int  moveAutomationToDifferentCC(int                            offset,
                                   int                            whichModEncoder,
                                   int                            modKnobMode,
...
protected:
  void polyphonicExpressionEventPostArpeggiator(int      newValue,
