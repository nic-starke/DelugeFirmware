...
MIDIDevice::MIDIDevice() {
  ...
  // These defaults for MPE are prescribed in the MPE standard. Wish we had the
  // same for regular MIDI
...
void MIDIDevice::dataEntryMessageReceived(ModelStack* modelStack, int channel, int msb) {
  ...
  if (!inputChannels[channel].rpnMSB && !inputChannels[channel].rpnLSB) {
    ...
    if (zone >= 0) { // If it *is* MPE-related...
      ...
      // Or, member channel (in which case it should be applied for all member
      // channels, according to the MPE spec.
      else {
    ...
    // Inform the Song
    modelStack->song->midiDeviceBendRangeUpdatedViaMessage(modelStack,
                                                           this,
                                                           channelOrZone,
                                                           whichBendRange,
                                                           msb);
    ...
    if (msb < 16) {
      ...
      if (channel == 0) {
        ...
        ports[MIDI_DIRECTION_INPUT_TO_DELUGE]
            .moveUpperZoneOutOfWayOfLowerZone(); // Move other zone out of the
                                                 // way if necessary (MPE spec
                                                 // says to do this).

      resetBendRanges: // Have to reset pitch bend range for zone, according to
                       // MPE spec. Unless we just deactivated the MPE zone...
        if (msb) {
        ...
        // TODO: we're supposed to also ensure no notes left on by channels no
        // longer in use...
      ...
      // Upper zone
        ...
        ports[MIDI_DIRECTION_INPUT_TO_DELUGE]
            .moveLowerZoneOutOfWayOfUpperZone(); // Move other zone out of the
                                                 // way if necessary (MPE spec
                                                 // says to do this).

        goto resetBendRanges;
      }
      // Any other channels "are invalid and should be ignored", according to
      // MPE spec.
    }
  }
}
...
// Not to be called for MIDIDeviceUSBHosted. readAHostedDeviceFromFile() handles
// that and needs to read the name and ids.
...
void MIDIDevice::writeDefinitionAttributesToFile() { // These only go into
                                                     // MIDIDEVICES.XML.
    ...
    if (!strcmp(tagName, "mpeLowerZone")) {
      ...
      while (*(tagName = storageManager.readNextTagOrAttributeName())) {
        if (!strcmp(tagName, "numMemberChannels")) {
          ...
          if (!mpeLowerZoneLastMemberChannel) { // If value was already set,
                                                // then leave it - the user or
                                                // an MCM might have changed it
            ...
            if (newMPELowerZoneLastMemberChannel >= 0 && newMPELowerZoneLastMemberChannel < 16) {
              ...
              moveLowerZoneOutOfWayOfUpperZone(); // Move self out of way of
                                                  // other - just in case user
                                                  // or MCM has set other and
      ...
      while (*(tagName = storageManager.readNextTagOrAttributeName())) {
        if (!strcmp(tagName, "numMemberChannels")) {
          ...
          if (mpeUpperZoneLastMemberChannel ==
              15) { // If value was already set, then leave it - the user or an
            ...
            if (numUpperMemberChannels >= 0 && numUpperMemberChannels < 16) {
              ...
              moveUpperZoneOutOfWayOfLowerZone(); // Move self out of way of
                                                  // other - just in case user
                                                  // or MCM has set other and
                                                  // that's the important one
                                                  // they want now.
...
void MIDIDeviceUSB::sendMCMsNowIfNeeded() {
  if (needsToSendMCMs) {
    needsToSendMCMs--;
    if (!needsToSendMCMs) { sendAllMCMs(); }
  }
}
...
void MIDIDeviceUSBUpstream::writeReferenceAttributesToFile() {
  storageManager.writeAttribute("port",
                                "upstreamUSB",
                                false); // Same line. Usually the user wouldn't have default
                                        // velocity sensitivity set for their computer.
}
...
void MIDIDeviceDINPorts::writeReferenceAttributesToFile() {
  storageManager.writeAttribute("port",
                                "din",
                                false); // Same line. Usually the user wouldn't
                                        // have default velocity sensitivity set
}
...
void MIDIDeviceDINPorts::sendMessage(uint8_t statusType,
                                     uint8_t channel,
                                     uint8_t data1,
                                     uint8_t data2) {
