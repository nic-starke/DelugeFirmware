...
void init() {
  ...
  // TODO: If I'm going to recall MPE zones from flash mem or file, for the din
  // port, I'd better call recountSmallestMPEZones after doing that.
...
// Gets called within UITimerManager, which may get called during SD card
// routine.
void slowRoutine() {
...
extern "C" void
giveDetailsOfDeviceBeingSetUp(int ip, char const* name, uint16_t vendorId, uint16_t productId) {
...
MIDIDeviceUSBHosted*
getOrCreateHostedMIDIDeviceFromDetails(String* name, uint16_t vendorId, uint16_t productId) {
  ...
  for (int i = 0; i < hostedMIDIDevices.getNumElements(); i++) {
    ...
    if (candidate->vendorId == vendorId && candidate->productId == productId) {
      ...
      if (gotAName && !candidate->name.equals(name)) { hostedMIDIDevices.renameMember(i, name); }
      return candidate;
    }
  }
  ...
  if (error) { numericDriver.freezeWithError("E405"); }
#endif
...
extern "C" void hostedDeviceConfigured(int ip, int midiDeviceNum) {
  ...
  MIDIDeviceUSBHosted* device =
      getOrCreateHostedMIDIDeviceFromDetails(&usbDeviceCurrentlyBeingSetUp[ip].name,
  ...
  connectedDevice->canHaveMIDISent =
      (bool)strcmp(device->name.get(), "Synthstrom MIDI Foot Controller");

  device->connectedNow(midiDeviceNum);
  recountSmallestMPEZones(); // Must be called after setting
  ...
  if (!error) { consoleTextIfAllBootedUp(text.get()); }
#else
...
extern "C" void configuredAsPeripheral(int ip) {
  ...
  anyUSBSendingStillHappening[ip] =
      0; // Initialize this. There's obviously nothing sending yet right now.
...
extern "C" void detachedAsPeripheral(int ip) {
  ...
  anyUSBSendingStillHappening[ip] = 0; // Reset this again. Been meaning to do this, and can no
                                       // longer quite remember reason or whether technically
                                       // essential, but adds to safety at least.
...
MIDIDevice* readDeviceReferenceFromFile() {
  ...
  if (!name.isEmpty() || vendorId) {
    return getOrCreateHostedMIDIDeviceFromDetails(&name,
                                                  vendorId,
...
worthIt:
  ...
  if (upstreamUSBMIDIDevice.worthWritingToFile())
    upstreamUSBMIDIDevice.writeToFile("upstreamUSBDevice");

  for (int d = 0; d < hostedMIDIDevices.getNumElements(); d++) {
    MIDIDeviceUSBHosted* device = (MIDIDeviceUSBHosted*)hostedMIDIDevices.getElement(d);
    if (device->worthWritingToFile()) { device->writeToFile("hostedUSBDevice"); }
  }
...
void readAHostedDeviceFromFile() {
  ...
  while (*(tagName = storageManager.readNextTagOrAttributeName())) {
    ...
    checkDevice:
      if (!device) {
        if (!name.isEmpty() || vendorId) {
          device = getOrCreateHostedMIDIDeviceFromDetails(&name,
                                                          vendorId,
                                                          productId); // Will return NULL if error.
        }
      }
...
void ConnectedUSBMIDIDevice::bufferMessage(uint32_t fullMessage) {
  ...
  if (numMessagesQueued >= 16) {
    midiEngine.flushUSBMIDIOutput(); // TODO: this is actually far from perfect - what
                                     // if already sending - and if we want to
                                     // wait/check for that, we should be calling the
                                     // routine. And ideally, we'd be able to flush
                                     // for just one device.
    numMessagesQueued = 0;
  }
...
/*
    for (int d = 0; d < hostedMIDIDevices.getNumElements(); d++) {
        MIDIDeviceUSBHosted* device =
   (MIDIDeviceUSBHosted*)hostedMIDIDevices.getElement(d);
